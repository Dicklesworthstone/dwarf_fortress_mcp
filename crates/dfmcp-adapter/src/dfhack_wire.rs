...
//! Safe-Rust client for DFHack's supported native remote protocol and the
//! read-only `dfmcp_bridge` plugin service.
//!
...
pub const MAX_TEXT_NOTIFICATION_TOTAL_BYTES: usize = 256 * 1024;
...
impl BridgeManifest {
    pub fn validate(&self) -> Result<()> {
        validate_text(&self.bridge_version, "bridge version", 64)?;
        validate_text(&self.dfhack_version, "DFHack version", 128)?;
        validate_text(&self.df_version, "Dwarf Fortress version", 128)?;
...
fn decode_bind_reply(bytes: &[u8]) -> Result<i16> {
    ...
    if id < FIRST_PLUGIN_METHOD_ID {
        return Err(error(
            ErrorCode::AdapterRejected,
            "DFHack assigned a reserved core method ID to the plugin",
        ));
    }
    Ok(id)
}
...
fn decode_observation_reply(
    ...
    if !include_names && citizens.iter().any(|citizen| !citizen.name.is_empty()) {
        return Err(error(
            ErrorCode::AdapterRejected,
    ...
    if !complete && returned != requested_maximum {
        return Err(error(
            ErrorCode::AdapterRejected,
...
mod tests {
    ...
    fn credentials_debug_output_redacts_token() -> Result<()> {
        let credentials = BridgeCredentials::new(
            vec![b'x'; MIN_BRIDGE_TOKEN_BYTES],
            vec![b'n'; MIN_NONCE_BYTES],
        )?;
        let rendered = format!("{credentials:?}");
        assert!(rendered.contains("<redacted>"));
        assert!(!rendered.contains(&"x".repeat(MIN_BRIDGE_TOKEN_BYTES)));
        Ok(())
    }
