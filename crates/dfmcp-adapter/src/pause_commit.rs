...
fn remaining(context: &OperationContext, elapsed: Duration) -> Result<Duration> {
    ...
    Duration::from_millis(context.budget.max_wall_millis)
        ...
        .ok_or_else(|| {
            DfmcpError::new(ErrorCode::BudgetExceeded, "pause commit deadline exhausted")
        })
}
fn ambiguous<S: EffectJournalStorage>(
...
pub fn commit_once<S: EffectJournalStorage, Q: PauseCommitSource>(
    ...
    token: &[u8; 16],
    context: &OperationContext,
) -> Result<PauseCommitOutcome> {
    let started = Instant::now();
    commit_with_clock(journal, source, key, plan, token, context, || {
        started.elapsed()
    })
}
...
fn commit_with_clock<S: EffectJournalStorage, Q: PauseCommitSource>(
    ...
    if journal.read_only() {
        return Err(DfmcpError::new(
            ...
            "offline evidence recovery cannot commit effects",
        ));
    }
    if key.is_empty() || key.len() > 512 || key.chars().any(char::is_control) {
        return Err(DfmcpError::new(
            ErrorCode::InvalidRequest,
            "invalid pause idempotency key",
        ));
    }
    ...
    let record = journal.lookup(key).cloned().ok_or_else(|| {
        DfmcpError::new(
            ErrorCode::InvalidRequest,
            "pause effect must be durably prepared before commit",
        )
    })?;
    ...
    match reconcile_reply(journal, key, plan, &effect, context) {
        Ok(record) => Ok(PauseCommitOutcome {
            record,
