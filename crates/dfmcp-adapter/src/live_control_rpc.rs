...
#[derive(Clone,Debug,PartialEq,Eq)]pub struct PauseEffect{pub known:bool,pub applied:bool,pub paused:bool,pub observed_tick:u64,pub prepare_token:Vec<u8>,pub receipt_digest:Vec<u8>}
fn decode(data:&[u8],nonce:&[u8])->Result<(JobsManifest,PauseEffect)>{
    ...
    let manifest=JobsManifest{generation:m.number(6)?,df_version:m.text(7)?,dfhack_version:m.text(8)?};if manifest.generation==0{return Err(invalid("invalid control generation"));}
    ...
    Ok((manifest,PauseEffect{known,applied,paused,observed_tick,prepare_token:token,receipt_digest:receipt}))
}
...
impl<S:Read+Write> ControlRpcClient<S>{
    ...
    pub fn poisoned(&self)->bool{self.fenced}pub fn fence(&mut self){self.fenced=true;}
    fn invoke(&mut self,method:i16,key:&str,digest:Digest32,prepare:Option<&[u8]>,paused:Option<bool>,tick:Option<u64>)->Result<PauseEffect>{
        ...
        let result=(||{let response=call(&mut self.stream,method,&request(&self.token,&self.nonce,Some(key),Some(digest),prepare,paused,tick),4096)?;
            ...
            self.manifest=manifest;Ok(effect)})();if result.is_err(){self.fenced=true;}result
    }
