...
fn malformed() -> DfmcpError {
    invalid("invalid control/1.7 native RPC framing or protobuf")
}
fn io_failure(_: io::Error) -> DfmcpError {
    failure(
        ErrorCode::AdapterUnavailable,
        "control RPC I/O failed or exceeded its deadline",
    )
}
fn checked_timeout(timeout: Duration) -> Result<Duration> {
    if timeout < Duration::from_millis(1) || timeout > Duration::from_secs(60) {
        return Err(failure(
            ...
            "control deadline must be 1..60000 milliseconds",
        ));
    }
    Ok(timeout)
}
fn remaining_timeout(timeout: Duration, elapsed: Duration) -> Result<Duration> {
    ...
    timeout
        ...
        .ok_or_else(|| {
            failure(
                ErrorCode::BudgetExceeded,
                "control deadline exhausted before handshake",
            )
        })
}
...
impl<'a> Message<'a> {
    fn parse(input: &'a [u8], maximum_field: u32) -> Result<Self> {
        if input.len() > MAX_RPC_BYTES {
            return Err(failure(
                ErrorCode::BudgetExceeded,
                "control RPC payload exceeds 8 KiB",
            ));
        }
        let mut offset = 0;
...
fn call<S: Read + Write>(
    ...
    if request.len() > MAX_RPC_BYTES || limit > MAX_RPC_BYTES {
        return Err(failure(
            ErrorCode::BudgetExceeded,
            "control RPC request or reply limit exceeds 8 KiB",
        ));
    }
    let length = i32::try_from(request.len()).map_err(|_| malformed())?;
    stream
        .write_all(&header(method, length))
        .map_err(io_failure)?;
    ...
    for _ in 0..=MAX_NOTIFICATIONS {
        ...
        if length > ceiling {
            return Err(failure(
                ErrorCode::BudgetExceeded,
                "control RPC reply exceeds its byte bound",
            ));
        }
        let mut payload = vec![0u8; length];
        stream.read_exact(&mut payload).map_err(io_failure)?;
        if id == -1 {
            return Ok(payload);
        }
        ...
        if notification_bytes > MAX_TEXT_NOTIFICATION_TOTAL_BYTES {
            return Err(failure(
                ErrorCode::BudgetExceeded,
                "control RPC text notifications exceeded their aggregate bound",
            ));
        }
    }
    Err(failure(
        ErrorCode::BudgetExceeded,
        "control RPC emitted too many text notifications",
    ))
}
fn bind<S: Read + Write>(stream: &mut S, name: &str) -> Result<i16> {
    let mut body = Vec::new();
    for (field, value) in [(1, name), (2, REQUEST_TYPE), (3, REPLY_TYPE), (4, PLUGIN)] {
        bytes(&mut body, field, value.as_bytes());
    }
    let response = call(stream, 0, &body, 1024)?;
    let id = i16::try_from(Message::parse(&response, 1)?.number(1)?)
        .map_err(|_| invalid("control method ID overflow"))?;
    if id < 2 {
        return Err(invalid("control method resolved to reserved core ID"));
    }
    Ok(id)
}
fn request(
    ...
    number(&mut out, 4, 7);
    if let Some(key) = key {
        bytes(&mut out, 5, key.as_bytes());
    }
    if let Some(digest) = digest {
        bytes(&mut out, 6, digest.as_bytes());
    }
    if let Some(prepare) = prepare {
        bytes(&mut out, 7, prepare);
    }
    if let Some(paused) = paused {
        number(&mut out, 8, u64::from(paused));
    }
    if let Some(tick) = tick {
        number(&mut out, 9, tick);
    }
    out
}
...
fn decode(data: &[u8], nonce: &[u8]) -> Result<(ControlManifest, PauseEffect)> {
    ...
    if message.number(4)? != 1 || message.number(5)? != 7 {
        return Err(failure(
            ErrorCode::VersionMismatch,
            "control bridge must be exactly profile 1.7",
        ));
    }
    if message.bytes(3, 64)? != nonce {
        return Err(invalid("control nonce mismatch"));
    }
    let accepted = message.boolean(1)?;
    let code = message.number(2)?;
    if !accepted {
        if code == 0
            || [9, 10, 11, 12, 13, 14]
                .into_iter()
                .any(|field| message.has(field))
        {
            return Err(malformed());
        }
        return Err(failure(
    ...
    let manifest = ControlManifest {
        ...
        df_version: message.text(7, 128)?,
        dfhack_version: message.text(8, 128)?,
    };
    if manifest.generation == 0 || manifest.generation == u64::MAX {
        return Err(invalid("invalid control generation"));
    }
    let prepare_token = if message.has(9) {
        message.bytes(9, 16)?.to_vec()
    } else {
        Vec::new()
    };
    let known = if message.has(10) {
        message.boolean(10)?
    } else {
        false
    };
    let applied = if message.has(11) {
        message.boolean(11)?
    } else {
        false
    };
    let paused = if message.has(12) {
        message.boolean(12)?
    } else {
        false
    };
    let observed_tick = if message.has(13) {
        message.number(13)?
    } else {
        0
    };
    let receipt_digest = if message.has(14) {
        message.bytes(14, 32)?.to_vec()
    } else {
        Vec::new()
    };
    if message.has(9) && prepare_token.len() != 16 {
        return Err(malformed());
    }
    if message.has(14) && receipt_digest.len() != 32 {
        return Err(malformed());
    }
    ...
    if known && [11, 12, 13].into_iter().any(|field| !message.has(field)) {
        return Err(malformed());
    }
    if applied && (!known || receipt_digest.is_empty()) {
        return Err(malformed());
    }
    if code == 5 && (!known || applied) {
        return Err(malformed());
    }
    if !known && (message.has(11) || message.has(12) || message.has(13) || message.has(14)) {
        return Err(malformed());
    }
    Ok((
        manifest.clone(),
        PauseEffect {
            bridge_generation: manifest.generation,
            known,
            applied,
            paused,
            observed_tick,
...
impl<S: Read + Write> ControlRpcClient<S> {
    pub fn negotiate(mut stream: S, token: Vec<u8>, nonce: Vec<u8>) -> Result<Self> {
        ...
        if &reply[..8] != b"DFHack!\n" || reply[8..] != 1i32.to_le_bytes() {
            return Err(invalid("invalid control native handshake"));
        }
        let handshake = bind(&mut stream, "Handshake")?;
        let prepare = bind(&mut stream, "PreparePause")?;
        let commit = bind(&mut stream, "CommitPause")?;
        let query = bind(&mut stream, "QueryPause")?;
        ...
        {
            return Err(invalid("control native method IDs alias"));
        }
        let response = call(
            &mut stream,
            handshake,
            &request(&token, &nonce, None, None, None, None, None),
            4096,
        )?;
        let (manifest, effect) = decode(&response, &nonce)?;
        if effect.known || !effect.prepare_token.is_empty() || !effect.receipt_digest.is_empty() {
            return Err(malformed());
        }
        Ok(Self {
            stream,
    ...
    pub fn manifest(&self) -> &ControlManifest {
        &self.manifest
    }
    fn invoke(
        &mut self,
        ...
        if self.fenced {
            return Err(failure(
                ErrorCode::AdapterUnavailable,
                "control source fenced; reopen session",
            ));
        }
        if key.is_empty() || key.len() > 512 || key.chars().any(char::is_control) {
            return Err(failure(
                ErrorCode::InvalidRequest,
                "invalid control idempotency key",
            ));
        }
        let result = (|| {
            let response = call(
                ...
                || manifest.dfhack_version != self.manifest.dfhack_version
            {
                return Err(failure(
                    ...
                    "control source software changed or generation regressed",
                ));
            }
    ...
    pub fn prepare_pause(
        ...
        self.invoke(self.prepare, key, digest, None, Some(paused), Some(tick))
    }
    pub fn commit_pause(
        &mut self,
        key: &str,
        ...
        if prepare.len() != 16 {
            return Err(failure(
                ErrorCode::InvalidRequest,
                "control prepare token must contain exactly 16 bytes",
            ));
        }
        self.invoke(self.commit, key, digest, Some(prepare), None, None)
    }
    pub fn query_pause(&mut self, key: &str, digest: Digest32) -> Result<PauseEffect> {
        self.invoke(self.query, key, digest, None, None, None)
    }
}

pub struct ControlDeadlineStream {
    stream: TcpStream,
    deadline: Instant,
}
impl ControlDeadlineStream {
    fn new(stream: TcpStream, timeout: Duration) -> Result<Self> {
        let deadline = Instant::now()
            ...
            .ok_or_else(|| failure(ErrorCode::BudgetExceeded, "control deadline overflow"))?;
...
impl ControlRpcClient<ControlDeadlineStream> {
    pub fn connect(
        ...
        if !endpoint.ip().is_loopback() || endpoint.port() == 0 {
            return Err(failure(
                ...
                "control endpoint must be numeric loopback with a nonzero port",
            ));
        }
    ...
    pub fn reset_deadline(&mut self, timeout: Duration) -> Result<()> {
        self.stream.reset(timeout)
    }
}
...
mod tests {
    ...
    fn reply(code: u64, known: Option<bool>, applied: Option<bool>) -> Vec<u8> {
        ...
        if let Some(known) = known {
            number(&mut out, 10, u64::from(known));
        }
    ...
    fn strict_decoder_accepts_ambiguous_known_record_without_terminal_receipt() -> Result<()> {
        let (_, effect) = decode(&reply(5, Some(true), Some(false)), &[b'n'; 16])?;
        assert!(effect.known);
        assert!(!effect.applied);
        assert_eq!(effect.observed_tick, 42);
        assert!(effect.receipt_digest.is_empty());
        Ok(())
    }
    #[test]
    fn strict_decoder_rejects_noncanonical_boolean_and_applied_without_known() {
        let mut bad = reply(0, Some(false), Some(true));
        assert!(decode(&bad, &[b'n'; 16]).is_err());
        bad = reply(0, Some(false), None);
        number(&mut bad, 12, 2);
        assert!(decode(&bad, &[b'n'; 16]).is_err());
    }
    #[test]
    fn parser_rejects_duplicate_fields_and_overlong_varints() {
        assert!(Message::parse(&[8, 1, 8, 1], 14).is_err());
        assert!(Message::parse(&[8, 128, 0], 14).is_err());
    }
    #[test]
    fn fixed_method_bootstrap_uses_four_distinct_bindings() -> Result<()> {
        ...
        let client = ControlRpcClient::negotiate(
            Script {
        ...
        assert_eq!(client.bridge_generation(), 9);
