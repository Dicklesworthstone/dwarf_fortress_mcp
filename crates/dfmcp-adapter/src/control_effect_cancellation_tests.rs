...
fn fixture() -> Result<ControlEffectJournal<Memory>> {
    let mut j = ControlEffectJournal::open(
        Memory::default(),
        &context(),
        true,
        7,
...
fn cancelled_record_survives_replay_without_inventing_a_native_outcome() -> Result<()> {
    ...
    assert_eq!(
        (
            ...
            cancelled.prepare_token,
            cancelled.expected_game_tick
        ),
        (
            prepared.plan_digest,
            prepared.prepare_token,
            prepared.expected_game_tick
        )
    );
    assert!(!cancelled.effect_known);
    assert!(!cancelled.effect_applied);
    assert_eq!(
        (
            cancelled.observed_paused,
            cancelled.observed_game_tick,
            cancelled.receipt_digest
        ),
        (None, None, None)
    );
...
fn repeated_cancellation_is_exact_and_does_not_rewind_a_later_journal_head() -> Result<()> {
    ...
    let result = j.cancel_prepared_with("key", plan(), &context(), |record, bytes, replayed| {
        assert!(replayed);
        assert_eq!(record, &cancelled);
        assert_eq!(bytes, before.2);
        Ok(record.clone())
    })?;
...
fn cancellation_cannot_be_reprepared_committed_or_reconciled_into_an_effect() -> Result<()> {
    ...
    for generation in [7, 8] {
        assert!(
            matches!(j.begin_commit("key",plan(),generation,&context()),Err(e) if e.code==ErrorCode::Conflict)
        );
    }
    assert!(
        matches!(j.mark_indeterminate("key",plan(),&context()),Err(e) if e.code==ErrorCode::Conflict)
    );
    assert!(
        matches!(j.record_reconciliation("key",plan(),7,true,true,true,11,
...
fn started_indeterminate_and_verified_effects_cannot_be_cancelled() -> Result<()> {
    for variant in 0..4 {
        let mut j = fixture()?;
        j.begin_commit("key", plan(), 7, &context())?;
        if variant == 1 {
            j.mark_indeterminate("key", plan(), &context())?;
        }
        if variant >= 2 {
        ...
        assert!(
            matches!(result,Err(e) if e.code==if variant<2 {ErrorCode::EffectIndeterminate} else {ErrorCode::Conflict})
        );
...
fn response_refusal_precedes_storage_and_preserves_dispatch_decision() -> Result<()> {
    ...
    let result: Result<()> =
        j.cancel_prepared_with("key", plan(), &context(), |record, length, replayed| {
            assert!(!replayed);
            assert_ne!(record.record_digest, Digest32::ZERO);
            assert_eq!(record.state, DurablePauseState::CancelledBeforeDispatch);
            assert!(length > before.3);
            Err(exhausted("injected full response rejection"))
        });
    ...
    assert!(current(&j)?.safe_to_dispatch(7));
    assert!(!j.fenced());
    j.cancel_prepared("key", plan(), &context())?;
    Ok(())
}
...
fn uncertain_sync_does_not_acknowledge_but_complete_replay_retains_cancellation() -> Result<()> {
    ...
    assert!(j.cancel_prepared("key", plan(), &context()).is_err());
    assert!(j.fenced());
    ...
    assert!(
        matches!(recovered.begin_commit("key",plan(),7,&context()),Err(e) if e.code==ErrorCode::Conflict)
    );
    Ok(())
}
...
fn partial_cancellation_write_fences_and_recovery_never_silently_drops_it() -> Result<()> {
    let mut j = fixture()?;
    j.storage.remaining_write_bytes = Some(17);
    assert!(j.cancel_prepared("key", plan(), &context()).is_err());
    assert!(j.fenced());
    assert!(j.begin_commit("key", plan(), 7, &context()).is_err());
    assert!(
        matches!(reopen(j.storage.bytes.into_inner()),Err(e) if e.code==ErrorCode::CorruptLedger)
    );
    Ok(())
}
...
fn cancellation_checks_identity_authority_expiry_and_read_only_custody() -> Result<()> {
    ...
    let before = j.head();
    assert!(
        matches!(j.cancel_prepared("key",Digest32::ZERO,&context()),Err(e) if e.code==ErrorCode::Conflict)
    );
    ...
    let mut read_only = ControlEffectJournal::open_read_only(
        Memory {
            bytes: Cursor::new(j.storage.bytes.into_inner()),
            ..Memory::default()
        },
        &context(),
    )?;
    assert!(
        matches!(read_only.cancel_prepared("key",plan(),&context()),Err(e) if e.code==ErrorCode::CapabilityDenied)
    );
...
fn replay_rejects_cancel_after_dispatch_and_cancel_with_outcome_evidence() -> Result<()> {
    for variant in 0..7 {
        ...
        let mut next = current(&j)?;
        next.state = DurablePauseState::CancelledBeforeDispatch;
        next.revision += 1;
        next.transition_number = j.transition_count() as u64 + 1;
        next.previous_digest = j.head();
        match variant {
            2 => next.effect_known = true,
            3 => next.effect_applied = true,
            4 => next.observed_paused = Some(false),
            5 => next.observed_game_tick = Some(11),
            6 => next.receipt_digest = Some(Digest32::of_bytes(b"receipt")),
            _ => {}
        }
        let frame = encode_frame(j.id(), &next)?;
        j.storage.bytes.get_mut().extend_from_slice(&frame);
        assert!(
            matches!(reopen(j.storage.bytes.into_inner()),Err(e) if e.code==ErrorCode::CorruptLedger),
            "variant={variant}"
        );
...
fn cached_cancellation_and_existing_mutator_replays_recheck_changed_storage() -> Result<()> {
    for variant in 0..5 {
        ...
        assert!(matches!(result,Err(e) if e.code==ErrorCode::CorruptLedger));
        assert!(j.fenced());
    }
    Ok(())
}
