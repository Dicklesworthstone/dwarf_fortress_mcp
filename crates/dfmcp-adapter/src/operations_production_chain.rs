...
use super::{
    AnalysisHandle, InventoryIndex, MaterialDemand, OperationsStateView, SUPPLY_POLICY,
    Work, exhausted, exclusion, handle, invalid, invariant, normalized_demands, source,
};
...
fn selectors_overlap(left: &MaterialDemand, right: &MaterialDemand) -> bool {
    left.item_types.iter().any(|kind| right.item_types.binary_search(kind).is_ok())
        && [(left.subtype, right.subtype), (left.material_type, right.material_type),
            (left.material_index, right.material_index)]
            .into_iter().all(|(a, b)| match (a, b) {
                (Some(a), Some(b)) => a == b,
                _ => true,
            })
}
...
fn selectors(input: &[ProductionResource], work: &mut Work) -> Result<Vec<MaterialDemand>> {
    if input.is_empty() || input.len() > MAX_CHAIN_RESOURCES {
        return Err(invalid("production chains require 1..32 disjoint resource definitions"));
    }
    // Bound borrowed inputs before cloning them for the existing selector validator.
    for resource in input {
        work.charge(1)?;
        if !valid_text(&resource.key, 64) || resource.item_types.is_empty()
            || resource.item_types.len() > 8
            || resource.item_types.iter().any(|kind| !valid_text(kind, 128)) {
            return Err(invalid("production resource strings or selector count exceed their bounds"));
        }
    }
    let requested: Vec<_> = input.iter().map(|resource| MaterialDemand {
        key: resource.key.clone(), units: 1, item_types: resource.item_types.clone(),
        subtype: resource.subtype, material_type: resource.material_type,
    ...
    for (index, left) in normalized.iter().enumerate() {
        for right in &normalized[index + 1..] {
            ...
            if selectors_overlap(left, right) {
                return Err(invalid("production resource selectors overlap; one physical stack cannot represent two model resources"));
            }
        }
    }
    Ok(normalized)
...
fn normalize_model(
    ...
    if quotas.is_empty() || quotas.len() > 64 || recipes.len() > MAX_CHAIN_RECIPES {
        return Err(invalid("production chain requires 1..64 quotas and at most 32 recipes"));
    }
    let keys: BTreeSet<_> = resources.iter().map(|resource| resource.key.as_str()).collect();
    ...
    for recipe in recipes {
        work.charge(1)?;
        if !valid_text(&recipe.output_token, 64) || !keys.contains(recipe.output_token.as_str())
            || recipe.output_batch_size == 0 || recipe.input_tokens.len() > MAX_CHAIN_RESOURCES
            || !valid_text(&recipe.job_token, 128) {
            return Err(invalid("invalid production recipe output, yield, job token or input count"));
        }
        match &recipe.workshop {
            BuildingKind::Workshop(name) | BuildingKind::Furnace(name) if valid_text(name, 128) => {}
            _ => return Err(invalid("production recipe must declare a bounded workshop or furnace type")),
        }
        ...
        for (key, amount) in &recipe.input_tokens {
            ...
            if !valid_text(key, 64) || !keys.contains(key.as_str()) || *amount == 0 {
                return Err(invalid("production recipe input references an undefined resource or zero units"));
            }
            let total = inputs.entry(key.clone()).or_default();
            *total = total.checked_add(*amount).ok_or_else(|| exhausted("production input coefficient overflow"))?;
        }
        let value = ProductionRecipe {
            output_token: recipe.output_token.clone(), output_batch_size: recipe.output_batch_size,
            input_tokens: inputs.into_iter().collect(), workshop: recipe.workshop.clone(),
            job_token: recipe.job_token.clone(),
        };
        if normalized.insert(recipe.output_token.clone(), value).is_some() {
            return Err(invalid("production model permits exactly one recipe per output resource"));
        }
    }
    Ok(Model {
        quotas: goals.into_iter().map(|(item_token, minimum_stock)| ProductionQuota { item_token, minimum_stock }).collect(),
...
fn model_digest(
    ...
    for value in [anchor.fortress_id.get(), anchor.cursor.epoch, anchor.cursor.sequence, anchor.tick.0] {
    ...
    for resource in resources {
        ...
        for kind in &resource.item_types { work.charge(1)?; put_text(&mut bytes, kind); }
        for value in [resource.subtype, resource.material_type, resource.material_index] {
    ...
    for recipe in &model.recipes {
        ...
        match &recipe.workshop {
            BuildingKind::Workshop(name) => { bytes.push(0); put_text(&mut bytes, name); }
            BuildingKind::Furnace(name) => { bytes.push(1); put_text(&mut bytes, name); }
...
pub fn plan_production_chain<S: OperationsStateView + ?Sized>(
    ...
    let mut resources: Vec<_> = selectors.iter().map(|selector| ProductionResourceStock {
        resource: ProductionResource {
            key: selector.key.clone(), item_types: selector.item_types.clone(), subtype: selector.subtype,
            material_type: selector.material_type, material_index: selector.material_index,
        },
        stock_units: 0, eligible_items: 0, examples: Vec::new(),
    }).collect();
    let mut excluded_items = BTreeMap::new();
    let mut unmatched_items = 0u64;
    for (index, item) in observation.items.iter().enumerate() {
        work.charge(1)?;
        let reason = if item.stack_size == 0 { Some("zero_stack_size") } else { exclusion(inventory.inherited[index]) };
        ...
        for (selector, resource) in selectors.iter().zip(&mut resources) {
            work.charge(1)?;
            if !selector.matches(item) { continue; }
            if matched { return Err(invariant("validated disjoint production resources matched the same item")); }
            matched = true;
            resource.stock_units = resource.stock_units.checked_add(item.stack_size)
                .ok_or_else(|| exhausted("captured production resource exceeds the u32 model domain"))?;
            resource.eligible_items += 1;
            if resource.examples.len() < EXAMPLES {
                resource.examples.push(handle(snapshot, item_entity_id(item.native_id))?);
            }
        }
        if !matched { unmatched_items += 1; }
    }
    ...
    for recipe in &model.recipes { work.charge(1)?; compiler.register_recipe(recipe.clone()); }
    let plan = compiler.plan_quotas(&model.quotas, &stock, ProductionPlanningLimits {
        max_orders: MAX_CHAIN_RECIPES, max_resources: MAX_CHAIN_RESOURCES,
    ...
    Ok(ProductionChainAnalysis {
        anchor: snapshot.anchor(), source_digest, model_digest, resources,
        quotas: model.quotas, recipes: model.recipes, plan,
        excluded_items, unmatched_items, work_units: work.used,
    })
}
