    ...
    value
        ...
        .map(|pair| {
            let a = char::from(pair[0])
                .to_digit(16)
                .ok_or_else(|| invalid("bad fixture hex"))?;
...
fn plan() -> Result<WorkOrderPlan> {
    WorkOrderPlan::new(
        WorkOrderObservation::decode(&hex(OBSERVATION)?)?,
        "order-001",
...
fn rehash(bytes: &mut [u8]) {
    ...
    proof.extend_from_slice(&bytes[8..16]);
    proof.extend_from_slice(&bytes[227..]);
...
fn created_for(p: &WorkOrderPlan) -> Result<Vec<u8>> {
    let o = p.observation();
    let mut out = b"DFMWOE10".to_vec();
    for n in [o.generation(), o.sequence(), o.tick()] {
        out.extend_from_slice(&n.to_be_bytes());
    }
    out.extend_from_slice(&o.next_order_id().to_be_bytes());
    p.spec().append(&mut out);
    out.extend_from_slice(o.witness().as_bytes());
    out.extend_from_slice(p.digest().as_bytes());
    out.extend_from_slice(p.prepare_token());
    out.extend_from_slice(&[2, 1]);
    out.extend_from_slice(&o.tick().to_be_bytes());
    out.extend_from_slice(o.expected_after_witness()?.as_bytes());
    out.extend_from_slice(p.configuration_witness().as_bytes());
    out.extend_from_slice(&[0; 32]);
    put_text(&mut out, p.key());
    rehash(&mut out);
    Ok(out)
}
...
fn actual_native_vectors_match_complete_sealed_evidence() -> Result<()> {
    ...
    assert_eq!(o.order_ids(), &[2, 6]);
    assert_eq!(o.next_order_id(), 10);
    assert_eq!(o.world_folder(), "region1");
    assert_eq!(o.site_id(), 1);
    assert!(o.paused() && o.eligible());
    assert_eq!(created_for(&p)?, hex(CREATED)?);
    for (raw, state) in [
        (PREPARED, WorkOrderState::Prepared),
        (UNKNOWN, WorkOrderState::Unknown),
        (CREATED, WorkOrderState::Created),
        (REFUSED, WorkOrderState::Refused),
    ] {
        let raw = hex(raw)?;
        let effect = WorkOrderEffect::decode(&raw, &p)?;
        assert_eq!(effect.canonical_bytes(), raw);
        assert_eq!(effect.state(), state);
        assert_eq!(effect.receipt().is_some(), state.terminal());
        assert_eq!(
            effect.created_order_id(),
            (state == WorkOrderState::Created).then_some(10)
        );
        assert_eq!(
            effect.observed_tick(),
            (state == WorkOrderState::Created).then_some(12345)
        );
        assert_eq!(
            effect.after_witness().is_some(),
            state == WorkOrderState::Created
        );
        assert_eq!(
            effect.configuration_witness().is_some(),
            state == WorkOrderState::Created
        );
    }
    Ok(())
...
fn catalog_and_quantities_are_closed_and_keys_are_not_plan_authority() -> Result<()> {
    for code in [0, 5, 69, 257, u32::MAX] {
        assert!(WorkOrderRecipe::from_code(code).is_err());
    }
    for amount in [0, 101, 32768, u32::MAX] {
        assert!(WorkOrderSpec::new(WorkOrderRecipe::WoodenBed, amount).is_err());
    }
    let p = plan()?;
    for code in 1..=4 {
        for amount in 1..=100 {
            let spec = WorkOrderSpec::new(WorkOrderRecipe::from_code(code)?, amount)?;
            assert!(!spec.recipe().as_str().is_empty());
            assert_eq!(spec.amount(), amount);
            let selected = WorkOrderPlan::new(p.observation().clone(), "another-key", spec)?;
            assert_eq!(
                WorkOrderEffect::decode(&created_for(&selected)?, &selected)?.created_order_id(),
                Some(10)
            );
        }
    }
    let another = WorkOrderPlan::new(p.observation().clone(), "another-key", p.spec())?;
    assert_eq!(another.digest(), p.digest());
    assert_ne!(another.prepare_token(), p.prepare_token());
    assert!(WorkOrderEffect::decode(&hex(CREATED)?, &another).is_err());
    for key in ["", "bad/key", "a b", "é", "x\0y"] {
        assert!(validate_key(key).is_err());
    }
    assert!(validate_key(&"a".repeat(129)).is_err());
    validate_key(&"a".repeat(128))?;
    Ok(())
}
...
fn every_bit_of_a_created_record_is_bound_to_the_retained_plan() -> Result<()> {
    ...
    for index in 0..raw.len() {
        for bit in 0..8 {
            ...
            bad[index] ^= 1 << bit;
            assert!(
                WorkOrderEffect::decode(&bad, &p).is_err(),
                "bit {index}:{bit} escaped"
            );
        }
    }
    Ok(())
...
fn self_consistent_receipts_for_wrong_queue_template_or_tick_are_rejected() -> Result<()> {
    let p = plan()?;
    let good = hex(CREATED)?;
    for index in [
        15, 23, 31, 35, 36, 40, 73, 105, 123, 130, 131, 162, 163, 194, 229,
    ] {
        ...
        assert!(
            WorkOrderEffect::decode(&bad, &p).is_err(),
            "rehash forged field {index}"
        );
    }
    let mut absent = good;
    absent[122] = 0;
    absent[123..195].fill(0);
    rehash(&mut absent);
...
{
    let p = plan()?;
    let obs = hex(OBSERVATION)?;
    for size in 0..obs.len() {
        assert!(WorkOrderObservation::decode(&obs[..size]).is_err());
    }
    for raw in [PREPARED, UNKNOWN, CREATED, REFUSED] {
        let raw = hex(raw)?;
        for size in 0..raw.len() {
            assert!(WorkOrderEffect::decode(&raw[..size], &p).is_err());
        }
        let mut trailing = raw;
        trailing.push(0);
        assert!(WorkOrderEffect::decode(&trailing, &p).is_err());
    }
    let mut trailing = obs;
    trailing.push(0);
    assert!(WorkOrderObservation::decode(&trailing).is_err());
...
fn capacity_horizon_clock_and_incarnation_boundaries_fail_closed() -> Result<()> {
    ...
    assert!(WorkOrderPlan::new(full, "full", p.spec()).is_err());
    let almost =
        WorkOrderObservation::decode(&observation(&(0..4095).collect::<Vec<_>>(), 4095, 1))?;
    let almost = WorkOrderPlan::new(almost, "last-slot", p.spec())?;
    assert_eq!(
        WorkOrderEffect::decode(&created_for(&almost)?, &almost)?.created_order_id(),
        Some(4095)
    );
    for (offset, value) in [
        (8, 0u64),
...
fn observation_ownership_utf8_and_fortress_lineage_are_explicit() -> Result<()> {
    ...
    let observed = WorkOrderObservation::decode(&raw)?;
    raw[43] = b'x';
    assert_ne!(
        WorkOrderObservation::decode(&raw)?.witness(),
        observed.witness()
    );
    ...
    let job = crate::job_suspension::JobObservation::decode(&hex(include_str!(
        "../../tests/fixtures/job_suspension_observation_v1_9.hex"
    ))?)?;
    assert_eq!(
        observed.fortress_id(),
        crate::job_suspension::coordinator::job_fortress_id(&job)
    );
    Ok(())
}
