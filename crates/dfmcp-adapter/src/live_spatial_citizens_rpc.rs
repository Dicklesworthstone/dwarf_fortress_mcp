...
use crate::live_jobs_rpc::operations::paged::snapshot::{
    SnapshotAssembler, SnapshotManifest, SnapshotPage,
};
...
pub struct CitizenSpatialLimits {
...
impl CitizenSpatialLimits {
    pub fn validate(self) -> Result<()> {
        ...
        if self.citizens == 0 || self.citizens as usize > MAX_COHERENT_CITIZENS {
            return Err(failure(
                ErrorCode::BudgetExceeded,
                "spatial/1.8 citizen bound must be 1..4096",
            ));
        }
        Ok(())
    }
    pub fn entity_limit(self) -> u32 {
        self.spatial.entity_limit().saturating_add(self.citizens)
    }
}
fn request(
    token: &[u8],
    ...
    let spatial = limits.spatial;
    let op = spatial.operations;
    for (field, value) in [
        (3, 1),
        (4, 8),
        (5, u64::from(op.jobs)),
    ...
    for (i, value) in spatial
        .region
        .origin
        .into_iter()
        .chain(spatial.region.size)
        .enumerate()
    {
...
fn envelope<'a>(data: &'a [u8], nonce: &[u8]) -> Result<(Message<'a>, JobsManifest)> {
    ...
    if m.number(4)? != 1 || m.number(5)? != 8 {
        return Err(failure(
            ErrorCode::VersionMismatch,
            "citizen spatial transport requires exactly protocol 1.8",
        ));
    }
    ...
    if accepted == 0 && code != 0 {
        ...
        return Err(failure(
            ...
            "spatial/1.8 capture refused; no partial world was published",
        ));
    }
...
fn bind<S: Read + Write>(stream: &mut S, name: &str) -> Result<i16> {
    ...
    for (f, text) in [
        (1, name),
        (2, "dfmcp.spatial.v1_8.Request"),
        (3, "dfmcp.spatial.v1_8.Reply"),
        (4, "dfmcp_spatial_v1_8"),
    ] {
        bytes(&mut request, f, text.as_bytes());
    }
...
pub struct CitizenSpatialRpcClient<S> {
    stream: S,
    token: Vec<u8>,
    nonce: Vec<u8>,
    limits: CitizenSpatialLimits,
...
impl<S: Read + Write> CitizenSpatialRpcClient<S> {
    pub fn negotiate(
        ...
        if !(32..=256).contains(&token.len()) || !(16..=64).contains(&nonce.len()) {
            return Err(failure(
                ...
                "invalid spatial/1.8 credentials",
            ));
        }
    ...
    pub fn read_observation(&mut self) -> Result<LiveSpatialCitizenObservation> {
        if self.poisoned {
            return Err(failure(
                ...
                "spatial/1.8 source is fenced; reopen session",
            ));
        }
    ...
    fn acquire(&mut self) -> Result<LiveSpatialCitizenObservation> {
        let op = self.limits.spatial.operations;
        ...
        while !assembly.complete() {
            if count >= 1024 {
                return Err(failure(
                    ErrorCode::BudgetExceeded,
                    "spatial/1.8 acquisition exceeded 1024 pages",
                ));
            }
            ...
            let reply = call(&mut self.stream, self.method, &req, op.page_bytes + 4096)?;
                ...
                || p.manifest.dfhack_version != self.manifest.dfhack_version
            {
                return Err(failure(
                    ...
                    "spatial/1.8 manifest changed incompatibly",
                ));
            }
            assembly.push(p)?;
            count += 1;
        ...
        let value = LiveSpatialCitizenObservation::decode_payload(
            &payload,
            manifest.generation,
        ...
        if observed.jobs.jobs.len() > op.jobs as usize
            ...
            || value.citizens().len() > self.limits.citizens as usize
            || value.spatial().terrain().map.region != self.limits.spatial.region
        {
            return Err(failure(
                ErrorCode::AdapterRejected,
                "spatial/1.8 capture differs from negotiated region or rosters",
            ));
        }
        let req = request(
            &self.token,
        ...
        let reply = call(&mut self.stream, self.method, &req, 4096)?;
    ...
    pub fn connect(
        ...
        if !endpoint.ip().is_loopback() || endpoint.port() == 0 {
            return Err(failure(
                ...
                "spatial/1.8 endpoint must be numeric loopback",
            ));
        }
        let deadline = Instant::now()
            .checked_add(timeout)
            .ok_or_else(|| failure(ErrorCode::BudgetExceeded, "spatial/1.8 deadline overflow"))?;
