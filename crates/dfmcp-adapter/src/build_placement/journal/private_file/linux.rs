...
fn denied() -> io::Error {
    io::Error::new(
        ...
        "furniture journal private custody changed",
    )
}
fn failure(_: io::Error) -> dfmcp_core::DfmcpError {
    error(
        ErrorCode::CorruptLedger,
        "furniture journal private custody or I/O failed; no repair performed",
    )
}
    ...
    let left = deadline
        ...
        .ok_or_else(|| {
            error(
                ...
                "furniture storage open deadline exhausted",
            )
        })?;
...
pub(super) fn open(
            ...
            .any(|part| part.is_empty() || part == b"." || part == b".." || part.contains(&0))
    {
        return Err(error(
            ...
            "furniture journal path must be normalized absolute",
        ));
    }
    ...
    options
        .read(true)
        .write(mode != BuildMode::Offline)
        .custom_flags(
            NOFOLLOW
                | NONBLOCK
                | if mode == BuildMode::Offline {
                    0
                } else {
                    APPEND
                },
    ...
    file.try_lock().map_err(|_| {
        error(
            ErrorCode::Conflict,
            "furniture journal already owned or exclusive locking unavailable",
        )
    })?;
