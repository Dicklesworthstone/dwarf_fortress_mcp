...
fn run(priorities: &[u32], units: &[u64], workers: &[u32], items: &[u32]) -> Result<Selection> {
    let task = tasks(priorities);
    let demand = demands(units);
    let owners: Vec<_> = (0..task.len()).collect();
    select(
        &task,
        Model {
            supplies: &supplies(workers),
            demands: &demand,
            owners: &owners,
        },
        Model {
...
fn satisfiable(supply: &[u32], needed: &mut [u64], next: usize) -> bool {
    ...
    if satisfiable(supply, needed, next + 1) {
        return true;
    }
...
fn oracle(priority: &[u32], units: &[u64], workers: &[u32], items: &[u32]) -> u16 {
    ...
    for mask in 1u16..(1u16 << priority.len()) {
        let indices: Vec<_> = (0..priority.len())
            .filter(|&i| mask & (1u16 << i) != 0)
            .collect();
        let score: u64 = indices.iter().map(|&i| u64::from(priority[i])).sum();
        let better = score > best.0
            || (score == best.0
                && (indices.len() > best.1 || (indices.len() == best.1 && indices < best.2)));
        if !better {
            continue;
        }
        let required: Vec<_> = (0..priority.len())
            .map(|i| if mask & (1u16 << i) != 0 { units[i] } else { 0 })
            .collect();
        if satisfiable(workers, &mut required.clone(), 0)
            && satisfiable(items, &mut required.clone(), 0)
        {
...
fn check_assignment(
    ...
    for a in &allocation.assignments {
        ...
        let supply = resources
            .iter()
            .find(|s| s.id == a.supply_id)
            .expect("assigned supply exists");
    ...
    for (i, d) in demand.iter().enumerate() {
        assert_eq!(
            allocation.allocated_by_demand[i],
            if selection.task_mask & (1u16 << owners[i]) != 0 {
                d.units
            } else {
                0
            }
...
fn priorities_and_complete_task_count_beat_greedy_slot_allocation() -> Result<()> {
    assert_eq!(
        run(&[3, 2, 2], &[2, 1, 1], &[7, 7], &[7, 7])?.task_mask,
        0b110
    );
    assert_eq!(
        run(&[5, 2, 2], &[2, 1, 1], &[7, 7], &[7, 7])?.task_mask,
        0b001
    );
    assert_eq!(
        run(&[2, 1, 1], &[2, 1, 1], &[7, 7], &[7, 7])?.task_mask,
        0b110
    );
...
fn exhaustive_three_task_two_worker_two_item_graphs_match_an_independent_oracle() -> Result<()> {
    for encoded in 0u32..4096 {
        ...
        assert_eq!(
            selected.task_mask,
            oracle(&priority, &units, &workers, &items),
            "graph={encoded}"
        );
        let demand = demands(&units);
        let owners = [0, 1, 2];
        ...
        for rejection in &selected.rejected {
            ...
            let model = if rejection.domain == Domain::Workers {
                &workers
            } else {
                &items
            };
            ...
            let mask = cut
                .demand_indices
                .iter()
                .fold(0u32, |bits, &i| bits | (1u32 << i));
            assert!(
                cut.demand_indices
                    .iter()
                    .all(|&i| rejection.task_mask & (1u16 << i) != 0)
            );
            assert_eq!(cut.required_units, cut.demand_indices.len() as u64);
            assert_eq!(
                cut.eligible_units,
                model.iter().filter(|&&bits| bits & mask != 0).count() as u64
            );
...
fn several_material_inputs_of_one_task_are_indivisible_as_a_set() -> Result<()> {
    let task = tasks(&[2, 1]);
    let wd = demands(&[1, 1]);
    let md = demands(&[1, 1, 1]);
    let ws = supplies(&[3, 3]);
    let ms = supplies(&[0b001, 0b100]);
    let selected = select(
        &task,
        Model {
            supplies: &ws,
    ...
    assert!(
        selected
            .rejected
            .iter()
            .any(|r| r.domain == Domain::Materials && r.shortage.demand_indices.contains(&1))
    );
    Ok(())
}
...
fn eight_tasks_and_capacity_splitting_remain_deterministic() -> Result<()> {
    let task = tasks(&[1; 8]);
    let demand = demands(&[1; 8]);
    let owners: Vec<_> = (0..8).collect();
    ...
    assert_eq!(first.task_mask, 0b0001_1111);
...
fn empty_feasible_set_keeps_all_capacity_unassigned() -> Result<()> {
    ...
    assert_eq!(selected.task_mask, 0);
    assert_eq!(selected.priority, 0);
    assert!(selected.workers.assignments.is_empty());
    assert!(selected.materials.assignments.is_empty());
    assert_eq!(selected.rejected.len(), 3);
    Ok(())
}
...
fn work_time_and_arithmetic_exhaustion_never_return_a_partial_optimum() {
    ...
    for work in [0, 1, 20, flow::MAX_WORK + 1] {
        assert_eq!(
            select(&task, model, model, work, Duration::from_secs(60)),
            Err(AllocationError::BudgetExceeded)
        );
    }
    assert_eq!(
        select(&task, model, model, flow::MAX_WORK, Duration::ZERO),
        Err(AllocationError::BudgetExceeded)
    );
