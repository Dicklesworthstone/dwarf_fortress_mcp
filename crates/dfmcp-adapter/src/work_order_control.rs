...
use crate::work_orders::{
    MAX_EFFECT_BYTES, MAX_OBSERVATION_BYTES, WorkOrderEffect, WorkOrderObservation, WorkOrderPlan,
    WorkOrderRecipe, WorkOrderSpec, WorkOrderState, validate_key,
};
use dfmcp_core::{
...
fn exhausted() -> DfmcpError {
    error(
        ...
        "work-order coordination budget exhausted",
    )
}
...
fn authorize(context: &OperationContext, fortress: FortressId, production: bool) -> Result<()> {
    if context.anchor.fortress_id != fortress {
        return Err(error(
            ...
            "work-order custody belongs to another fortress",
        ));
    }
...
fn manifest_matches(manifest: &WorkOrderManifest, plan: &WorkOrderPlan) -> Result<()> {
            ...
            .any(|s| s.is_empty() || s.len() > 128 || s.contains('\0'))
    {
        return Err(error(
            ...
            "work-order source and sealed plan disagree",
        ));
    }
    Ok(())
}
...
pub trait WorkOrderQuerySource {
    ...
    fn query(&mut self, plan: &WorkOrderPlan, timeout: Duration)
...
impl<S: WorkOrderStream> WorkOrderQuerySource for WorkOrderRpcClient<S> {
    fn manifest(&self) -> &WorkOrderManifest {
        WorkOrderRpcClient::manifest(self)
    }
    fn fence(&mut self) {
        WorkOrderRpcClient::fence(self);
    }
    fn read_orders(&mut self, timeout: Duration) -> Result<WorkOrderObservation> {
...
impl CreationState {
    pub fn terminal(self) -> bool {
        matches!(
            self,
            Self::Created | Self::Refused | Self::CancelledBeforeDispatch
        )
    }
    ...
    fn from_effect(effect: &WorkOrderEffect) -> Self {
        match effect.state() {
            WorkOrderState::Prepared => Self::Prepared,
            WorkOrderState::Unknown => Self::Indeterminate,
            WorkOrderState::Created => Self::Created,
            WorkOrderState::Refused => Self::Refused,
        }
    }
}
...
impl CreationRecord {
    pub fn plan(&self) -> &WorkOrderPlan {
        &self.plan
    }
...
pub struct CreationPage {
    pub head: Digest32,
    pub records: Vec<CreationRecord>,
...
pub struct WorkOrderJournal<S> {
    ...
    length: u64,
    transitions: u64,
    records: BTreeMap<String, CreationRecord>,
    mode: JournalMode,
    fenced: bool,
}
impl<S: EffectJournalStorage> WorkOrderJournal<S> {
    ...
    pub fn open(
        ...
        if mode == JournalMode::Control {
            authorize(context, fortress, true)?;
        }
        if initialize && mode != JournalMode::Control {
            return Err(error(
                ErrorCode::CapabilityDenied,
                "recovery cannot create a work-order journal",
            ));
        }
        ...
        if length == 0 && initialize {
            ...
            let hash = Digest32::of_bytes(&header);
        ...
        if length < HEADER_BYTES as u64 {
            return Err(corrupt("incomplete work-order journal header"));
        }
        storage.seek(SeekFrom::Start(0)).map_err(storage_error)?;
        ...
        if input.take(8)? != MAGIC {
            return Err(corrupt("not a work-orders/1.10 journal"));
        }
        let id = input.digest()?;
        let named = input.u64()?;
        let head = input.digest()?;
        if named != fortress.get() || head != Digest32::of_bytes(&header[..48]) {
            return Err(corrupt(
                "work-order journal identity or header checksum mismatch",
            ));
        }
        ...
        while journal.length < length {
            ...
            if journal.transitions >= MAX_TRANSITIONS
                || length - journal.length < PREFIX_BYTES as u64
            {
            ...
            if input.take(8)? != FRAME {
                return Err(corrupt("creation frame marker mismatch"));
            }
            let size = input.u32()? as usize;
            let number = input.u64()?;
            let previous = input.digest()?;
            if size > MAX_RECORD_BYTES
                || number != journal.transitions + 1
            ...
            journal
                ...
                .map_err(storage_error)?;
            let mut input = Input(&trailer);
            let hash = input.digest()?;
            ...
            if journal.records.len() > context.budget.max_entities as usize {
                return Err(exhausted());
            }
            journal.head = hash;
            journal.transitions = number;
            journal.length += total as u64;
        }
    ...
    pub fn validate_access(&self, context: &OperationContext) -> Result<()> {
        ...
        if self.fenced {
            return Err(corrupt(
                "creation journal fenced; reopen for verified recovery",
            ));
        }
    ...
    fn writable(&self, context: &OperationContext, production: bool) -> Result<()> {
        ...
        if self.mode == JournalMode::Offline || (production && self.mode != JournalMode::Control) {
            return Err(error(
                ErrorCode::CapabilityDenied,
                "this journal mode cannot perform the requested write",
            ));
        }
    ...
    fn plan_context(&self, plan: &WorkOrderPlan, context: &OperationContext) -> Result<()> {
            ...
            || plan.observation().witness() != context.anchor.state_hash
        {
            return Err(error(
                ...
                "creation requires the exact retained queue witness and tick",
            ));
        }
        Ok(())
    }
    fn no_uncertainty(&self) -> Result<()> {
        if self.records.values().any(|r| r.state.unresolved()) {
    ...
    fn matching(&self, plan: &WorkOrderPlan) -> Result<CreationRecord> {
        let record = self.records.get(plan.key()).ok_or_else(|| {
            error(
                ErrorCode::Conflict,
                "no durable creation preparation has this key",
            )
        })?;
        if &record.plan != plan {
            return Err(error(
                ErrorCode::Conflict,
                "creation key already binds different intent",
            ));
        }
        Ok(record.clone())
    }
    fn transition(&self, next: &CreationRecord) -> Result<()> {
        ...
        let shape = match next.state {
            CreationState::Prepared
            | CreationState::DispatchStarted
            | CreationState::CancelledBeforeDispatch => effect.state() == WorkOrderState::Prepared,
            CreationState::Indeterminate => matches!(
                effect.state(),
                WorkOrderState::Prepared | WorkOrderState::Unknown
            ),
            _ => CreationState::from_effect(&effect) == next.state,
        };
        if !shape {
            return Err(corrupt("creation state contradicts native evidence"));
        }
        ...
        if old.state.terminal() {
            return Err(corrupt("terminal creation evidence was rewritten"));
        }
        ...
        let allowed = match old.state {
            CreationState::Prepared => matches!(
                next.state,
                CreationState::DispatchStarted | CreationState::CancelledBeforeDispatch
            ),
            CreationState::DispatchStarted | CreationState::Indeterminate => matches!(
                next.state,
        ...
        if !allowed {
            return Err(corrupt(
                "illegal creation transition could re-enable dispatch",
            ));
        }
        if next.state == CreationState::DispatchStarted {
            self.no_uncertainty()?;
        }
        Ok(())
    }
    ...
    pub fn prepare<N: WorkOrderSource>(
        &mut self,
        source: &mut N,
        plan: &WorkOrderPlan,
        context: &OperationContext,
        ...
        manifest_matches(&manifest, plan)?;
        if self.records.len() >= MAX_RECORDS {
            return Err(exhausted());
        }
        self.reserve(3)?;
        budget.charge(RPC_RESERVE_BYTES)?;
        if budget.bytes < MAX_FRAME_BYTES as u64 {
            return Err(exhausted());
        }
        ...
        self.append(
            CreationRecord {
                plan: plan.clone(),
                manifest,
                state: CreationState::from_effect(&effect),
                effect,
            },
            &mut budget,
        )
    }
    pub fn commit<N: WorkOrderSource>(
        &mut self,
        source: &mut N,
        plan: &WorkOrderPlan,
        context: &OperationContext,
        ...
        if old.state != CreationState::Prepared {
            return Err(uncertain());
        }
        self.no_uncertainty()?;
        self.plan_context(plan, context)?;
        if source.manifest() != &old.manifest {
            return Err(error(
                ErrorCode::StaleAnchor,
                "creation source incarnation or software changed",
            ));
        }
        self.reserve(2)?;
        budget.charge(RPC_RESERVE_BYTES)?;
        if budget.bytes < 2 * MAX_FRAME_BYTES as u64 {
            return Err(exhausted());
        }
        ...
        let result = (|| {
            ...
            let mut next = old;
            next.state = CreationState::Indeterminate;
            if let Ok(effect) = reply {
                if effect.state() == WorkOrderState::Prepared {
                    source.fence();
                } else {
                    next.state = CreationState::from_effect(&effect);
                    next.effect = effect;
                }
            }
    ...
    pub fn reconcile<N: WorkOrderQuerySource>(
        &mut self,
        source: &mut N,
        plan: &WorkOrderPlan,
        context: &OperationContext,
                ...
                Ok(Some(effect)) => {
                    if let Ok(effect) = checked(source, &old.manifest, plan, Ok(effect)) {
                        if old.effect.state() == WorkOrderState::Unknown && effect != old.effect {
                            source.fence();
                            return Err(corrupt(
                                "native Unknown creation changed during reconciliation",
                            ));
                        }
                        if effect.state() != WorkOrderState::Prepared {
                            next.state = CreationState::from_effect(&effect);
    ...
    pub fn cancel(
        ...
        if record.state == CreationState::CancelledBeforeDispatch {
            return Ok(record);
        }
        if record.state != CreationState::Prepared {
            return Err(error(
                ErrorCode::Conflict,
                "cannot cancel creation after dispatch or native terminal evidence",
            ));
        }
    ...
    pub fn records_page(
        ...
        if head != self.head {
            return Err(error(
                ErrorCode::StaleAnchor,
                "creation journal changed; restart discovery",
            ));
        }
        ...
        let start = match after {
            ...
            Some(key) => {
                ...
                if !self.records.contains_key(key) {
                    return Err(error(ErrorCode::Conflict, "unknown creation cursor key"));
                }
                Excluded(key)
            }
        };
...
fn checked<N: WorkOrderQuerySource>(
    source: &mut N,
    manifest: &WorkOrderManifest,
    plan: &WorkOrderPlan,
    reply: Result<WorkOrderEffect>,
) -> Result<WorkOrderEffect> {
    let result = (|| {
        if source.manifest() != manifest {
            return Err(error(
                ErrorCode::StaleAnchor,
                "creation source changed during RPC",
            ));
        }
        let effect = reply?;
...
impl<'a> Input<'a> {
    fn take(&mut self, size: usize) -> Result<&'a [u8]> {
        let value = self
            .0
            .get(..size)
            .ok_or_else(|| corrupt("incomplete creation record"))?;
        self.0 = &self.0[size..];
        Ok(value)
    }
    fn array<const N: usize>(&mut self) -> Result<[u8; N]> {
        self.take(N)?
            .try_into()
            .map_err(|_| corrupt("creation field width mismatch"))
    }
    ...
    fn blob(&mut self, maximum: usize) -> Result<&'a [u8]> {
        ...
        if length > maximum {
            return Err(corrupt("creation blob bound exceeded"));
        }
        self.take(length)
    }
    fn text(&mut self, maximum: usize) -> Result<String> {
        ...
        if value.is_empty() || value.contains('\0') {
            return Err(corrupt("invalid creation text"));
        }
        Ok(value.to_owned())
    }
}
fn decode_record(data: &[u8]) -> Result<CreationRecord> {
    ...
    let state = match input.array::<1>()?[0] {
        1 => CreationState::Prepared,
        2 => CreationState::DispatchStarted,
        3 => CreationState::Indeterminate,
        4 => CreationState::Created,
        5 => CreationState::Refused,
        6 => CreationState::CancelledBeforeDispatch,
    ...
    let manifest = WorkOrderManifest {
        generation: plan.observation().generation(),
        df_version: input.text(128)?,
        dfhack_version: input.text(128)?,
    };
    let effect = WorkOrderEffect::decode(input.blob(MAX_EFFECT_BYTES)?, &plan)?;
    if !input.0.is_empty() {
        return Err(corrupt("trailing creation journal bytes"));
    }
    Ok(CreationRecord {
        plan,
        manifest,
