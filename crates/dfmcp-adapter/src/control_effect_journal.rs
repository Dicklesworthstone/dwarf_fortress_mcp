...
fn storage_error(_: io::Error) -> DfmcpError {
    corrupt("control effect journal I/O failed; reopen for verified recovery")
}
...
impl DurablePauseState {
    ...
    pub const fn terminal(self) -> bool {
        matches!(
            self,
            Self::VerifiedApplied | Self::VerifiedNotApplied | Self::CancelledBeforeDispatch
        )
    }
    #[must_use]
...
impl DurablePauseRecord {
    ...
    pub const fn safe_to_dispatch(&self, bridge_generation: u64) -> bool {
        matches!(self.state, DurablePauseState::Prepared)
...
impl<S: EffectJournalStorage> ControlEffectJournal<S> {
    ...
    fn open_inner(
        ...
        if file_length > MAX_LEDGER_BYTES {
            return Err(exhausted("control effect journal exceeds 64 MiB"));
        }
        ...
        if file_length < HEADER_BYTES as u64 {
            return Err(corrupt(
                "control effect journal header is incomplete; original bytes are unchanged",
            ));
        }
        storage.seek(SeekFrom::Start(0)).map_err(storage_error)?;
        ...
        while journal.length < file_length {
            ...
            if remaining < FRAME_HEADER_BYTES as u64 {
                ...
                if prefix[..comparable] != RECORD[..comparable] {
                    return Err(corrupt(
                        "control journal trailing bytes are not an incomplete record",
                    ));
                }
                break;
            }
        ...
        if journal.length != file_length {
            if recovery != EffectTailRecovery::TruncateIncomplete {
                return Err(corrupt(
                    "incomplete control effect journal tail; explicit operator repair is required",
                ));
            }
            authorize(context)?;
            ...
            {
                return Err(corrupt(
                    "control journal changed during recovery; no repair applied",
                ));
            }
            journal
                .storage
    ...
    fn accept_replay(&mut self, record: DurablePauseRecord) -> Result<()> {
        if !self.records.contains_key(&record.idempotency_key) && self.records.len() >= MAX_EFFECTS
        {
            return Err(exhausted(
                "control effect journal effect limit reached during replay",
            ));
        }
        ...
        {
            return Err(corrupt(
                "control effect journal sequence or predecessor chain disagrees",
            ));
        }
        ...
        if record.revision != expected_revision {
            return Err(corrupt("control effect revision is not contiguous"));
        }
    ...
    fn ensure_healthy(&mut self, context: &OperationContext) -> Result<()> {
        ...
        if self.fenced {
            return Err(corrupt(
                "control effect journal is fenced; reopen for verified recovery",
            ));
        }
    ...
    fn append(
        &mut self,
        next: DurablePauseRecord,
        context: &OperationContext,
    ) -> Result<DurablePauseRecord> {
        self.append_with(next, context, |_, _| Ok(()))
            .map(|(record, ())| record)
    }

    fn append_with<T, F>(
        &mut self,
        mut next: DurablePauseRecord,
        context: &OperationContext,
        publish: F,
    ) -> Result<(DurablePauseRecord, T)>
    where
        F: FnOnce(&DurablePauseRecord, u64) -> Result<T>,
    {
        ...
        let next_length = self
            ...
            .ok_or_else(|| exhausted("control journal length overflow"))?;
        if next_length > MAX_LEDGER_BYTES {
            return Err(exhausted("control effect journal byte limit reached"));
        }
        ...
        if write.is_err() {
            self.fenced = true;
            return Err(corrupt(
                "control journal append outcome is uncertain; reopen and reconcile before any effect retry",
            ));
        }
        ...
        self.transitions += 1;
        self.records
            .insert(decoded.idempotency_key.clone(), decoded.clone());
        Ok((decoded, rendered))
    }
    ...
    pub fn record_prepared(
        ...
        if let Some(existing) = self.records.get(&key) {
            if existing.plan_digest == plan_digest
                && existing.desired_paused == desired_paused
                && existing.expected_game_tick == expected_game_tick
                && existing.bridge_generation == bridge_generation
    ...
    pub fn begin_commit(
        ...
        if current.state == DurablePauseState::CancelledBeforeDispatch {
            return Err(conflict(
                "cancelled pause effect cannot be dispatched; its idempotency key remains retired",
            ));
        }
        ...
        match current.state {
            DurablePauseState::Prepared => {
                ...
                next.state = DurablePauseState::CommitStarted;
                self.append(next, context)
            }
            DurablePauseState::CommitStarted | DurablePauseState::Indeterminate => {
                Err(DfmcpError::new(
                    ErrorCode::EffectIndeterminate,
                    "pause effect has an unresolved commit attempt; reconcile before any retry",
                ))
            }
            _ => Ok(current),
        }
    ...
    pub fn mark_indeterminate(
        ...
        self.ensure_healthy(context)?;
        let current = self.require(key, plan_digest)?;
        if current.state == DurablePauseState::Indeterminate {
            return Ok(current);
        }
        if current.state != DurablePauseState::CommitStarted {
            return Err(conflict(
                "only a started pause commit can become indeterminate",
            ));
        }
        let mut next = current;
        next.state = DurablePauseState::Indeterminate;
        self.append(next, context)
    }
    ...
    pub fn record_reconciliation(
        ...
        if current.state == DurablePauseState::CancelledBeforeDispatch {
            return Err(conflict(
                "cancelled-before-dispatch effect has no coordinator attempt to reconcile",
            ));
        }
        ...
        if current.state == DurablePauseState::Prepared {
            return Err(conflict(
                "pause effect has not begun commit and cannot be reconciled as an effect attempt",
            ));
        }
        ...
        if effect_applied && observed_paused != current.desired_paused {
            return Err(corrupt(
                "bridge claimed a pause effect applied but observed the opposite pause state",
            ));
        }
    ...
    fn require(&self, key: &str, digest: Digest32) -> Result<DurablePauseRecord> {
        ...
        let record = self
            .records
            .get(key)
            .ok_or_else(|| invalid("pause effect is not present in the durable control journal"))?;
        if record.plan_digest != digest {
            return Err(conflict("idempotency key belongs to another plan digest"));
        }
        Ok(record.clone())
    }

    #[must_use]
    pub fn lookup(&self, key: &str) -> Option<&DurablePauseRecord> {
...
fn validate_transition(
    previous: Option<&DurablePauseRecord>,
    next: &DurablePauseRecord,
) -> Result<()> {
    validate_key(&next.idempotency_key)?;
    if next.bridge_generation == 0
        || next.bridge_generation == u64::MAX
        || next.prepare_token == [0u8; 16]
        || next.transition_number == 0
        || next.previous_digest == Digest32::ZERO
    {
    ...
    match previous {
        None => {
            if next.revision != 1
                || next.state != DurablePauseState::Prepared
                || next.effect_known
                || next.effect_applied
                || next.observed_paused.is_some()
                || next.observed_game_tick.is_some()
                || next.receipt_digest.is_some()
            {
                return Err(corrupt(
                    "first durable pause record must be a clean prepared state",
                ));
            }
        }
        Some(previous) => {
            if next.revision
                != previous
                    .revision
                    .checked_add(1)
                    .ok_or_else(|| corrupt("pause revision overflow"))?
                || next.idempotency_key != previous.idempotency_key
                || next.plan_digest != previous.plan_digest
                || next.desired_paused != previous.desired_paused
                || next.expected_game_tick != previous.expected_game_tick
                || next.bridge_generation != previous.bridge_generation
                || next.prepare_token != previous.prepare_token
            {
                return Err(corrupt(
                    "durable pause transition changed immutable effect identity",
                ));
            }
            let allowed = matches!(
            ...
            if !allowed {
                return Err(corrupt("invalid durable pause-effect state transition"));
            }
        }
    }
    if matches!(
        ...
        if !next.effect_known
            || next.observed_paused.is_none()
            || next.observed_game_tick.is_none()
            || next.effect_applied != (next.state == DurablePauseState::VerifiedApplied)
        {
            return Err(corrupt(
                "terminal pause-effect record lacks a complete reconciled outcome",
            ));
        }
        if next.effect_applied
            && (next.receipt_digest.is_none() || next.observed_paused != Some(next.desired_paused))
        {
            return Err(corrupt(
                "verified applied pause effect lacks matching pause-state receipt evidence",
            ));
        }
    } else if next.effect_known
        || next.effect_applied
        || next.observed_paused.is_some()
        || next.observed_game_tick.is_some()
        || next.receipt_digest.is_some()
    {
        return Err(corrupt(
            "undispatched or unresolved pause-effect record carried terminal evidence",
        ));
    }
    Ok(())
}
...
fn header_hash(bytes: &[u8]) -> Digest32 {
    let mut input = b"dfmcp-control-effect-journal-header/1\0".to_vec();
    input.extend_from_slice(bytes);
    Digest32::of_bytes(&input)
}
fn frame_header_hash(id: Digest32, bytes: &[u8]) -> Digest32 {
    let mut input = b"dfmcp-control-effect-journal-frame-header/1\0".to_vec();
    input.extend_from_slice(id.as_bytes());
    input.extend_from_slice(bytes);
    Digest32::of_bytes(&input)
}
fn frame_hash(id: Digest32, bytes: &[u8]) -> Digest32 {
    let mut input = b"dfmcp-control-effect-journal-record/1\0".to_vec();
    input.extend_from_slice(id.as_bytes());
    input.extend_from_slice(bytes);
    Digest32::of_bytes(&input)
}
fn decode_header(bytes: &[u8; HEADER_BYTES]) -> Result<(Digest32, Digest32)> {
    if &bytes[..8] != MAGIC {
        return Err(corrupt("unsupported control effect journal schema"));
    }
    let id = Digest32::from_bytes(
        bytes[8..40]
            .try_into()
            .map_err(|_| corrupt("control ledger id"))?,
    );
    let digest = Digest32::from_bytes(
        bytes[40..72]
            .try_into()
            .map_err(|_| corrupt("control header digest"))?,
    );
    if id == Digest32::ZERO || digest != header_hash(&bytes[..40]) {
        return Err(corrupt("control effect journal header checksum failed"));
    }
    Ok((id, digest))
}
fn decode_frame_header(prefix: &[u8], id: Digest32) -> Result<usize> {
    if prefix.len() != FRAME_HEADER_BYTES || &prefix[..8] != RECORD {
        return Err(corrupt("invalid control record marker"));
    }
    let length = u32::from_be_bytes(
        prefix[8..12]
            .try_into()
            .map_err(|_| corrupt("control record length"))?,
    ) as usize;
    if !(120..=MAX_BODY_BYTES).contains(&length)
        || &prefix[12..44] != frame_header_hash(id, &prefix[..12]).as_bytes()
    {
...
fn encode_frame(id: Digest32, record: &DurablePauseRecord) -> Result<Vec<u8>> {
    ...
    body.push(match record.observed_paused {
        None => 0,
        Some(false) => 1,
        Some(true) => 2,
    });
    ...
    if body.len() > MAX_BODY_BYTES {
        return Err(exhausted("control record body exceeds its bound"));
    }
    let mut frame = RECORD.to_vec();
    frame.extend_from_slice(&(body.len() as u32).to_be_bytes());
    let header = frame_header_hash(id, &frame);
...
fn decode_frame(frame: &[u8], id: Digest32) -> Result<DurablePauseRecord> {
    ...
    if frame.len() != FRAME_HEADER_BYTES + body_length + 32 + FOOTER.len() {
        return Err(corrupt("control record length mismatch"));
    }
    let body_bytes = reader.take(body_length)?;
    let digest = reader.digest()?;
    if reader.take(FOOTER.len())? != FOOTER
        || digest != frame_hash(id, &frame[..FRAME_HEADER_BYTES + body_length])
    {
    ...
    let prepare_token: [u8; 16] = body
        .take(16)?
        .try_into()
        .map_err(|_| corrupt("prepare token length"))?;
    let effect_known = body.boolean()?;
    let effect_applied = body.boolean()?;
    let observed_paused = match body.byte()? {
        0 => None,
        1 => Some(false),
        2 => Some(true),
        _ => return Err(corrupt("invalid observed pause tag")),
    };
    let observed_raw = body.u64()?;
    let receipt_raw = body.digest()?;
    if observed_paused.is_none() && observed_raw != 0 {
        return Err(corrupt(
            "absent pause observation carried a noncanonical game tick",
        ));
    }
    if !body.0.is_empty() {
        return Err(corrupt("trailing control record bytes"));
    }
    Ok(DurablePauseRecord {
        idempotency_key: key,
        plan_digest,
        desired_paused,
        expected_game_tick,
        ...
        receipt_digest: (receipt_raw != Digest32::ZERO).then_some(receipt_raw),
        revision,
        transition_number,
        previous_digest,
...
impl<'a> Reader<'a> {
    fn take(&mut self, count: usize) -> Result<&'a [u8]> {
        let out = self
            .0
            .get(..count)
            .ok_or_else(|| corrupt("truncated control effect record"))?;
        self.0 = &self.0[count..];
        Ok(out)
    }
    ...
    fn boolean(&mut self) -> Result<bool> {
        match self.byte()? {
            0 => Ok(false),
            1 => Ok(true),
            _ => Err(corrupt("noncanonical control Boolean")),
        }
    }
    ...
    fn digest(&mut self) -> Result<Digest32> {
        Ok(Digest32::from_bytes(
            self.take(32)?
                .try_into()
                .map_err(|_| corrupt("control digest"))?,
        ))
    }
    fn text(&mut self, maximum: usize) -> Result<String> {
        let n = u16::from_be_bytes(
            self.take(2)?
                .try_into()
                .map_err(|_| corrupt("control text length"))?,
        ) as usize;
        if n == 0 || n > maximum {
            return Err(corrupt("control text exceeds its bound"));
        }
        let text =
            std::str::from_utf8(self.take(n)?).map_err(|_| corrupt("control text is not UTF-8"))?;
        if text.chars().any(char::is_control) {
            return Err(corrupt("control text contains a control character"));
        }
        Ok(text.to_owned())
    }
}
    ...
    fn require_writable(&self) -> io::Result<()> {
        if self.read_only {
            return Err(io::Error::new(
                ...
                "recovery-only journal descriptor",
            ));
        }
        Ok(())
    }
}
impl Read for PrivateControlJournalFile {
...
impl Seek for PrivateControlJournalFile {
    ...
    fn validate_identity(&self) -> io::Result<()> {
        #[cfg(unix)]
        {
            ...
            let denied = || {
                io::Error::new(
                    io::ErrorKind::PermissionDenied,
                    "control journal custody changed",
                )
            };
        ...
        #[cfg(not(unix))]
        {
            Err(io::Error::new(
                ...
                "private control journals require Unix custody checks",
            ))
        }
    }
}

pub fn open_private_control_journal(
    path: &Path,
    context: &OperationContext,
    bridge_generation: u64,
    recovery: EffectTailRecovery,
...
pub fn open_private_control_recovery(
    path: &Path,
    context: &OperationContext,
) -> Result<ControlEffectJournal<PrivateControlJournalFile>> {
    context.authorize(Capability::Query, RiskTier::ReadOnly, &[], None)?;
...
fn open_private_control_storage(
    path: &Path,
    context: &OperationContext,
    read_only: bool,
    ...
    #[cfg(unix)]
    {
        ...
        if created && read_only {
            return Err(invalid(
                "recovery requires an existing control effect journal",
            ));
        }
        ...
        file.try_lock().map_err(|_| {
            conflict("control effect journal already has a writer or cannot be exclusively locked")
        })?;
        ...
        let storage = PrivateControlJournalFile {
            identity: (
                opened.dev(),
                opened.ino(),
                opened.uid(),
    ...
    #[cfg(not(unix))]
    {
        ...
        Err(DfmcpError::new(
            ErrorCode::CapabilityDenied,
            "private control effect journals are currently Unix-only",
        ))
    }
}
...
mod tests {
    ...
    fn fresh() -> Result<ControlEffectJournal<Memory>> {
        ControlEffectJournal::open(
            Memory::default(),
            &context(),
            true,
            7,
    ...
    fn prepare_commit_and_verified_replay_are_exact() -> Result<()> {
        ...
        let expected = j
            .lookup("k")
            .cloned()
            .ok_or_else(|| corrupt("test record missing"))?;
        let bytes = j.storage.bytes.into_inner();
        let reopened = ControlEffectJournal::open(
            Memory {
        ...
        assert_eq!(reopened.lookup("k"), Some(&expected));
        assert_eq!(expected.state, DurablePauseState::VerifiedApplied);
        Ok(())
    }
    #[test]
    fn commit_attempt_is_durable_before_effect_and_never_retryable_as_prepared() -> Result<()> {
        let mut j = fresh()?;
        let digest = Digest32::of_bytes(b"plan");
        j.record_prepared("k".to_owned(), digest, false, 10, 7, [1u8; 16], &context())?;
        j.begin_commit("k", digest, 7, &context())?;
        assert!(
            matches!(j.begin_commit("k",digest,7,&context()),Err(e) if e.code==ErrorCode::EffectIndeterminate)
        );
        j.mark_indeterminate("k", digest, &context())?;
        assert_eq!(
            j.lookup("k").map(|v| v.state),
    ...
    fn generation_change_or_unknown_reconciliation_stays_indeterminate() -> Result<()> {
        let mut j = fresh()?;
        let digest = Digest32::of_bytes(b"plan");
        j.record_prepared("k".to_owned(), digest, true, 10, 7, [1u8; 16], &context())?;
        j.begin_commit("k", digest, 7, &context())?;
        j.record_reconciliation("k", digest, 8, false, false, false, 0, None, &context())?;
        let record = j
            .lookup("k")
            .ok_or_else(|| corrupt("test record missing"))?;
        assert_eq!(record.state, DurablePauseState::Indeterminate);
        assert!(!record.safe_to_dispatch(8));
        Ok(())
    }
    #[test]
    fn conflicting_key_content_is_rejected() -> Result<()> {
        ...
        j.record_prepared("k".to_owned(), digest, true, 10, 7, [1u8; 16], &context())?;
        assert!(
            matches!(j.record_prepared("k".to_owned(),Digest32::of_bytes(b"other"),true,10,7,[1u8;16],&context()),Err(e) if e.code==ErrorCode::Conflict)
        );
        Ok(())
    }
    #[test]
    fn failed_commit_started_sync_fences_without_dispatchable_state() -> Result<()> {
        ...
        let digest = Digest32::of_bytes(b"plan");
        j.record_prepared("k".to_owned(), digest, true, 10, 7, [1u8; 16], &context())?;
        j.storage.sync_fails = true;
        assert!(j.begin_commit("k", digest, 7, &context()).is_err());
        assert!(j.fenced());
        assert_eq!(
            j.lookup("k").map(|v| v.state),
    ...
    fn incomplete_tail_requires_explicit_repair_and_preserves_last_complete_state() -> Result<()> {
        ...
        let repaired = ControlEffectJournal::open(
            Memory {
                bytes: Cursor::new(bytes),
                ..Memory::default()
            },
            ...
            false,
            7,
            EffectTailRecovery::TruncateIncomplete,
        )?;
        assert_eq!(repaired.retained_bytes(), length);
        assert_eq!(repaired.repaired_tail_bytes(), 3);
        assert_eq!(
            repaired.lookup("k").map(|v| v.state),
            Some(DurablePauseState::Prepared)
        );
        Ok(())
    }
    ...
    fn replay_rejects_validly_hashed_but_wrong_predecessor_chain() -> Result<()> {
        ...
        next.state = DurablePauseState::CommitStarted;
        next.revision = 2;
        next.transition_number = 2;
        next.previous_digest = Digest32::of_bytes(b"wrong");
        let forged = encode_frame(j.id, &next)?;
        j.storage.bytes.get_mut().extend_from_slice(&forged);
        let bytes = j.storage.bytes.into_inner();
        assert!(
            ControlEffectJournal::open(
                Memory {
