...
use dfmcp_core::{
    Capability, DfmcpError, Digest32, ErrorCode, FortressId, GameTick, OperationContext, Result,
    RiskTier, SessionId,
};
...
fn require(valid: bool, text: &str) -> Result<()> {
    if valid {
        ...
        Err(error(ErrorCode::AdapterRejected, text))
    }
}
pub fn validate_targets(ids: &[u32]) -> Result<()> {
    if ids.is_empty()
        || ids.len() > MAX_TARGETS
        || ids.iter().any(|id| *id > i32::MAX as u32)
        || ids.windows(2).any(|pair| pair[0] >= pair[1])
    {
        return Err(error(
            ErrorCode::InvalidRequest,
            "progress selection must contain 1..32 sorted unique native order IDs",
        ));
    }
    Ok(())
}
...
impl<'a> Reader<'a> {
    fn take(&mut self, n: usize) -> Result<&'a [u8]> {
        let out = self
            .bytes
            .get(..n)
            .ok_or_else(|| error(ErrorCode::AdapterRejected, "truncated work-order progress"))?;
    ...
    fn array<const N: usize>(&mut self) -> Result<[u8; N]> {
        self.take(N)?
            .try_into()
            .map_err(|_| error(ErrorCode::AdapterRejected, "invalid progress scalar width"))
    }
    ...
    fn text(&mut self, maximum: usize, empty: bool) -> Result<String> {
        ...
        require(
            n <= maximum && (empty || n > 0),
            "invalid progress string bound",
        )?;
        let s = std::str::from_utf8(self.take(n)?)
            .map_err(|_| error(ErrorCode::AdapterRejected, "progress text is not UTF-8"))?;
        require(!s.contains('\0'), "progress text contains NUL")?;
        Ok(s.to_owned())
    }
}
...
pub struct OrderStatus {
    ...
    pub frequency: i32,
    pub workshop_id: i32,
    pub max_workshops: i32,
    pub next_check_year: i32,
    pub next_check_tick: i32,
    pub item_conditions: u32,
    pub order_conditions: u32,
    pub type_key: String,
    pub reaction: String,
}
impl OrderStatus {
    pub fn validated(&self) -> bool {
        self.status_bits & 1 != 0
    }
    pub fn active(&self) -> bool {
        self.status_bits & 2 != 0
    }
    ...
    fn same_configuration(&self, other: &Self) -> bool {
        self.job_type == other.job_type
            ...
            && self.workshop_id == other.workshop_id
            && self.max_workshops == other.max_workshops
            && self.item_conditions == other.item_conditions
            && self.order_conditions == other.order_conditions
...
pub struct ProgressRow {
    pub native_order_id: u32,
    pub order: Option<OrderStatus>,
}
impl ProgressRow {
    pub fn phase(&self) -> &'static str {
        self.order.as_ref().map_or("absent", OrderStatus::phase)
    }
}
...
impl ProgressObservation {
    ...
    pub fn decode(bytes: &[u8], expected_ids: &[u32]) -> Result<Self> {
        ...
        require(
            ...
            "progress observation exceeds 16 KiB",
        )?;
        ...
        require(
            generation > 0
                ...
                && tick <= MAX_TICK
                && site <= i32::MAX as u32
                && horizon <= i32::MAX as u32
                && queue_count <= MAX_QUEUE as u32
                && queue_count <= horizon,
            "invalid progress identity or queue bound",
        )?;
        let folder = r.text(512, false)?;
        require(
            r.u32()? as usize == expected_ids.len(),
            "progress selection has omitted or extra IDs",
        )?;
        ...
        for expected in expected_ids {
            ...
            require(
                native_order_id == *expected,
                "progress response reordered or substituted a selected ID",
            )?;
            let order = if r.boolean()? {
                require(
                    native_order_id < horizon,
                    "present order lies beyond allocation horizon",
                )?;
                present_count += 1;
                let job_type = r.i32()?;
                let recipe = r.byte()?;
                let remaining = r.i32()?;
                let total = r.i32()?;
                let status_bits = r.u32()?;
                let frequency = r.i32()?;
                let workshop_id = r.i32()?;
                let max_workshops = r.i32()?;
                let next_check_year = r.i32()?;
                let next_check_tick = r.i32()?;
                let item_conditions = r.u32()?;
                let order_conditions = r.u32()?;
                ...
                require(
                    job_type >= 0
                        ...
                        && item_conditions <= MAX_QUEUE as u32
                        && order_conditions <= MAX_QUEUE as u32,
                    "invalid progress counters, type or condition bounds",
                )?;
                if recipe != 0 {
                    let expected_type = match recipe {
                        1 => "ConstructBed",
                        2 => "ConstructDoor",
                        3 => "ConstructTable",
                        _ => "ConstructThrone",
        ...
        require(
            present_count <= queue_count && r.bytes.is_empty(),
            "progress presence count or record extent is invalid",
        )?;
        Ok(Self {
            bytes: bytes.to_vec(),
            generation,
    ...
    pub fn ids(&self) -> Vec<u32> {
        self.rows.iter().map(|r| r.native_order_id).collect()
    }
...
pub struct ProgressChange {
    pub native_order_id: u32,
    pub kind: &'static str,
    pub before_phase: &'static str,
    pub after_phase: &'static str,
    pub remaining_decrease: Option<u32>,
    pub remaining_increase: Option<u32>,
}
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ProgressComparison {
    pub status: &'static str,
    pub reset_reason: Option<&'static str>,
    pub baseline: Option<Digest32>,
    pub elapsed_game_ticks: Option<u64>,
    pub changes: Vec<ProgressChange>,
}
pub fn compare(
    before: Option<&ProgressObservation>,
    after: &ProgressObservation,
) -> Result<ProgressComparison> {
    let mut out = ProgressComparison {
        status: "bootstrap",
        reset_reason: None,
        baseline: None,
        elapsed_game_ticks: None,
        changes: Vec::new(),
    };
    ...
    if before.generation == after.generation && after.sequence <= before.sequence {
        return Err(error(
            ...
            "progress capture was replayed or reordered; acquire fresh evidence",
        ));
    }
    let reset = if before.generation != after.generation
        || before.folder != after.folder
        || before.site != after.site
    {
        Some("incarnation_or_fortress_changed")
    } else if before.ids() != after.ids() {
        Some("selection_changed")
    } else if after.tick < before.tick || after.horizon < before.horizon {
        Some("clock_or_allocation_horizon_regressed")
    } else {
        None
    };
    if let Some(reason) = reset {
        out.status = "reset";
        out.reset_reason = Some(reason);
        return Ok(out);
    }
    out.status = "compared";
    out.baseline = Some(before.witness());
    out.elapsed_game_ticks = Some(after.tick - before.tick);
    for (old, new) in before.rows.iter().zip(&after.rows) {
        if old == new {
            continue;
        }
        let mut change = ProgressChange {
            native_order_id: new.native_order_id,
            kind: "status_changed",
            before_phase: old.phase(),
            after_phase: new.phase(),
            remaining_decrease: None,
            remaining_increase: None,
        };
        match (&old.order, &new.order) {
            ...
            (Some(a), Some(b)) => {
                if !a.same_configuration(b) {
                    change.kind = "configuration_changed";
                } else if a.recipe == 0 {
                    change.kind = "unrecognized_order_changed";
...
pub struct ProgressSession<S> {
    ...
    source: S,
    current: Option<ProgressObservation>,
...
impl<S: ProgressSource> ProgressSession<S> {
    pub fn new(source: S, context: &OperationContext) -> Result<Self> {
        ...
        if context.session_id == SessionId::NIL || context.anchor.fortress_id == FortressId::NIL {
            return Err(error(
                ErrorCode::InvalidRequest,
                "progress requires a concrete session and fortress",
            ));
        }
        ...
        require(
            manifest.generation > 0
                ...
                && [&manifest.df_version, &manifest.dfhack_version]
                    .iter()
                    .all(|v| !v.is_empty() && v.len() <= 128 && !v.contains('\0')),
            "progress source manifest is invalid",
        )?;
        Ok(Self {
            session_id: context.session_id,
            fortress_id: context.anchor.fortress_id,
            source,
            current: None,
            comparison: None,
        })
    ...
    pub fn refresh_with_publication<F>(
        &mut self,
        ids: &[u32],
    ...
    where
        F: FnOnce(&ProgressManifest, &ProgressObservation, &OperationContext) -> Result<()>,
    {
        self.access(context)?;
        self.current_context(context).authorize(
            Capability::Observe,
            RiskTier::ReadOnly,
            &[],
            None,
        ...
        if context.budget.max_entities < MAX_QUEUE as u32
            || context.budget.max_bytes < RPC_BYTE_RESERVE
        {
            return Err(error(
                ErrorCode::BudgetExceeded,
                "progress requires full-queue work and complete RPC capacity",
            ));
        }
        ...
        let result = (|| {
            ...
            require(
                self.source.manifest() == &manifest
                    ...
                    && capture.fortress_id() == self.fortress_id,
                "progress source changed incarnation, software or fortress",
            )?;
            let mut fresh = context.clone();
            fresh.anchor.tick = GameTick(context.anchor.tick.get().max(capture.tick));
            ...
            if started.elapsed() >= timeout {
                return Err(error(
                    ErrorCode::BudgetExceeded,
                    "progress capture exceeded its deadline",
                ));
            }
            fresh.budget.max_wall_millis =
                u64::try_from(timeout.saturating_sub(started.elapsed()).as_millis())
                    .ok()
                    .filter(|millis| *millis > 0)
                    .ok_or_else(|| {
                        error(
                            ErrorCode::BudgetExceeded,
                            "progress publication has no remaining deadline",
                        )
                    })?;
            ...
            if started.elapsed() >= timeout {
                return Err(error(
                    ErrorCode::BudgetExceeded,
                    "progress publication exceeded its deadline",
                ));
            }
        ...
        match result {
            ...
            Err(cause) => {
                self.source.fence();
                Err(cause)
            } // Never serve an old selection as a successful refresh.
        }
    }
}
