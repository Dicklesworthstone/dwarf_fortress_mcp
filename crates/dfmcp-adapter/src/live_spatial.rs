...
impl LiveSpatialObservation {
    pub fn operations(&self) -> &LiveOperationsObservation { &self.operations }
    pub fn terrain(&self) -> &LiveMapObservation { &self.terrain }
    pub fn validate(&self) -> Result<()> {
        self.operations.validate_profile(OperationsProfile::PagedV1_4)?;
        self.terrain.validate()?;
        let a = &self.operations.jobs;
        let b = &self.terrain;
        if a.bridge_generation != b.bridge_generation || a.df_version != b.df_version
            || a.dfhack_version != b.dfhack_version || a.year != b.year || a.year_tick != b.year_tick
            || a.paused != b.paused || a.site_id < 0 || a.site_id as u32 != b.site_id
            || a.world_folder != b.world_folder
        { return Err(invalid("spatial components disagree on native capture identity")); }
        Ok(())
    }
    pub fn decode_payload(bytes: &[u8], generation: u64, df: String, dfhack: String) -> Result<Self> {
        if bytes.len() > MAX_SPATIAL_BYTES { return Err(exhausted("spatial capture exceeds its byte bound")); }
        let mut r = Reader { bytes, offset: 0 };
        if r.take(8)? != MAGIC { return Err(invalid("wrong spatial capture profile")); }
        let n = r.length()?;
        let operations = LiveOperationsObservation::decode_profile(r.take(n)?, generation,
            df.clone(), dfhack.clone(), OperationsProfile::PagedV1_4)?;
        let n = r.length()?;
        if n > MAX_MAP_BYTES { return Err(exhausted("spatial terrain exceeds its byte bound")); }
        let terrain = LiveMapObservation::decode_payload(r.take(n)?, generation, df, dfhack)?;
        if r.offset != bytes.len() { return Err(invalid("trailing spatial capture bytes")); }
        let value = Self { operations, terrain };
        value.validate()?;
        Ok(value)
    }
    pub fn encode_payload(&self) -> Result<Vec<u8>> {
        self.validate()?;
        let operations = self.operations.encode_profile(OperationsProfile::PagedV1_4)?;
        let terrain = self.terrain.encode_payload()?;
        let size = operations.len().checked_add(terrain.len()).and_then(|n|n.checked_add(16))
            .ok_or_else(||exhausted("spatial capture length overflow"))?;
        if size > MAX_SPATIAL_BYTES { return Err(exhausted("spatial capture exceeds its byte bound")); }
...
impl<'a> Reader<'a> {
    fn take(&mut self, n: usize) -> Result<&'a [u8]> {
        let end = self.offset.checked_add(n).ok_or_else(||invalid("spatial length overflow"))?;
        let part = self.bytes.get(self.offset..end).ok_or_else(||invalid("truncated spatial capture"))?;
        self.offset = end;
        Ok(part)
    }
    fn length(&mut self) -> Result<usize> {
        Ok(u32::from_be_bytes(self.take(4)?.try_into().map_err(|_|invalid("invalid spatial length"))?) as usize)
    }
}
...
impl LiveSpatialState {
    pub fn observation(&self) -> Option<&LiveSpatialObservation> { self.observation.as_ref() }
    pub fn snapshot(&self) -> Option<&WorldSnapshot> { self.snapshot.as_ref() }
    pub fn source_digest(&self) -> Result<Digest32> {
        self.observation.as_ref().ok_or_else(||invalid("spatial observation absent"))?.source_digest()
    }
    pub fn publish(&mut self, value: LiveSpatialObservation) -> Result<JobPublication> {
        ...
        if let (Some(prior), Some(snapshot)) = (&self.observation, &self.snapshot) {
            let a = &prior.terrain;
            let b = &value.terrain;
            if a.world_folder != b.world_folder || a.site_id != b.site_id || a.df_version != b.df_version
                || a.dfhack_version != b.dfhack_version || a.map.region != b.map.region
            { return Err(DfmcpError::new(ErrorCode::StaleAnchor, "spatial world, region or software changed; reopen session")); }
            if prior == &value { return Ok(JobPublication::Heartbeat); }
            let reset = a.bridge_generation != b.bridge_generation || b.tick() < a.tick()
            ...
            cursor = if reset { ObservationCursor { epoch: snapshot.cursor.epoch.checked_add(1)
                .ok_or_else(||exhausted("spatial epoch exhausted"))?, sequence: 0 } }
        ...
        let mut graph = operations.snapshot().ok_or_else(||invalid("operations projection absent"))?.graph.clone();
        let map = terrain.snapshot().ok_or_else(||invalid("terrain projection absent"))?;
        for (id, entity) in &map.graph.entities {
            if *id == EntityId::new(1) {
                let root = graph.entities.get_mut(id).ok_or_else(||invalid("spatial root absent"))?;
                for (name, fact) in &entity.fields {
                    if let Some(existing) = root.fields.get(name) {
                        if existing.value != fact.value { return Err(invalid("inconsistent spatial root field")); }
                    } else { root.fields.insert(name.clone(), fact.clone()); }
                }
                root.label = "Coherent fortress operations and terrain".to_owned();
                root.fields.insert("observation_profile".to_owned(), Fact::known(Value::Text("spatial/1.6".to_owned()),
                    tick, FactSource::DfhackField("spatial/1.6.capture".to_owned()), source));
        ...
        let revision = cursor.sequence.checked_add(1).ok_or_else(||exhausted("spatial revision exhausted"))?;
        for (id, entity) in &mut graph.entities {
            ...
            let generation = match generations.get(id).copied() {
                Some(n) if !present || outcome == JobPublication::Reset => n.checked_add(1)
        ...
        for edge in graph.edges.values_mut() {
            ...
            for fact in edge.fields.values_mut() { rebind(fact, source); }
        }
        let snapshot = WorldSnapshot::new(value.terrain.fortress_id()?, tick, cursor, value.terrain.paused, graph);
...
fn rebind(fact: &mut Fact, source: Digest32) {
    ...
    if let FactSource::DfhackField(name) = &mut fact.source {
        if let Some(suffix) = name.strip_prefix("operations/1.4.") { *name = format!("spatial/1.6.operations.{suffix}"); }
        else if let Some(suffix) = name.strip_prefix("map/1.5.") { *name = format!("spatial/1.6.map.{suffix}"); }
    }
}
