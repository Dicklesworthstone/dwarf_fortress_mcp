...
fn context() -> OperationContext {
    OperationContext {
        session_id: SessionId::new(91), request_id: RequestId::new(1),
        ...
        grants: vec![CapabilityGrant { capability: Capability::Query,
            scope: CapabilityScope::default(), max_risk: RiskTier::ReadOnly,
            expires_at_tick: None, remaining_uses: None },
            CapabilityGrant { capability: Capability::ControlClock,
                scope: CapabilityScope::default(), max_risk: RiskTier::Reversible,
                expires_at_tick: None, remaining_uses: None }],
    }
}
fn fixture() -> Result<ControlEffectJournal<Memory>> {
    let mut journal = ControlEffectJournal::open(Memory::default(), &context(), true, 7,
    ...
    for key in ["z-started", "a-prepared", "m-applied", "n-unknown"] {
        ...
        if key != "a-prepared" { journal.begin_commit(key, plan, 7, &context())?; }
        if key == "m-applied" {
            journal.record_reconciliation(key, plan, 7, true, true, true, 11,
                Some(Digest32::of_bytes(b"receipt")), &context())?;
        }
        if key == "n-unknown" { journal.mark_indeterminate(key, plan, &context())?; }
    }
    Ok(journal)
}
...
fn recovery_enumerates_exact_sorted_effects_without_writes_or_a_bridge() -> Result<()> {
    ...
    read_context.grants.retain(|grant| grant.capability == Capability::Query);
    ...
    assert_eq!(actual.iter().map(|r| r.idempotency_key.as_str()).collect::<Vec<_>>(),
        ["a-prepared", "m-applied", "n-unknown", "z-started"]);
    assert!(recovered.read_only());
    assert_eq!((recovered.id(), recovered.head()), (id, head));
    assert_eq!((recovered.storage.writes, recovered.storage.syncs, recovered.storage.truncates), (0, 0, 0));
...
fn recovery_refuses_all_effect_transitions_even_with_clock_authority() -> Result<()> {
    ...
    assert!(matches!(journal.record_prepared("a-prepared".into(), plan, true, 10, 7, [1; 16], &context()),
        Err(e) if e.code == ErrorCode::CapabilityDenied));
    assert!(matches!(journal.begin_commit("a-prepared", plan, 7, &context()),
        Err(e) if e.code == ErrorCode::CapabilityDenied));
    assert!(matches!(journal.mark_indeterminate("z-started", Digest32::of_bytes(b"z-started"), &context()),
        Err(e) if e.code == ErrorCode::CapabilityDenied));
    assert!(matches!(journal.record_reconciliation("m-applied", Digest32::of_bytes(b"m-applied"),
        7, true, true, true, 11, Some(Digest32::of_bytes(b"receipt")), &context()),
        Err(e) if e.code == ErrorCode::CapabilityDenied));
    assert!(!journal.fenced());
    assert_eq!((journal.storage.writes, journal.storage.syncs, journal.storage.truncates), (0, 0, 0));
...
fn recovery_rechecks_current_authority_cancellation_and_storage() -> Result<()> {
    ...
    denied.grants.clear();
    assert!(matches!(ControlEffectJournal::open_read_only(memory(bytes.clone()), &denied),
        Err(e) if e.code == ErrorCode::CapabilityDenied));
    ...
    assert!(matches!(journal.records(&cancelled), Err(e) if e.code == ErrorCode::CancellationRequested));
...
fn recovery_never_initializes_or_repairs_an_incomplete_journal() -> Result<()> {
    ...
    for tail in [&RECORD[..3], &b"invalid"[..]] {
        let mut incomplete = bytes.clone(); incomplete.extend_from_slice(tail);
        assert!(matches!(ControlEffectJournal::open_read_only(memory(incomplete), &context()),
...
fn replay_rejects_hashed_applied_record_with_opposite_pause_evidence() -> Result<()> {
    ...
    let mut next = journal.lookup("z-started").cloned().ok_or_else(||invalid("fixture"))?;
    ...
    next.effect_known = true; next.effect_applied = true;
    next.observed_paused = Some(false); next.observed_game_tick = Some(11);
    ...
    assert!(matches!(ControlEffectJournal::open_read_only(memory(journal.storage.bytes.into_inner()), &context()),
        Err(e) if e.code == ErrorCode::CorruptLedger));
    Ok(())
}
...
fn replay_enforces_the_same_distinct_effect_bound_as_append() -> Result<()> {
    let mut journal = ControlEffectJournal::open(Memory::default(), &context(), true, 7,
        EffectTailRecovery::Refuse)?;
    let prototype = journal.record_prepared("k0".into(), Digest32::of_bytes(b"plan"), true,
        10, 7, [1; 16], &context())?;
...
fn private_recovery_uses_existing_read_only_custody_and_never_creates() -> Result<()> {
    ...
    let directory = std::env::temp_dir().canonicalize().map_err(storage_error)?
        .join(format!("dfmcp-control-recovery-{}-{serial}", std::process::id()));
    std::fs::DirBuilder::new().mode(0o700).create(&directory).map_err(storage_error)?;
    ...
    let result = (|| -> Result<()> {
        ...
        assert!(!path.exists());
        let mut live = open_private_control_journal(&path, &context(), 7, EffectTailRecovery::Refuse)?;
        live.record_prepared("key".into(), Digest32::of_bytes(b"plan"), true, 10, 7, [1; 16], &context())?;
        assert!(open_private_control_recovery(&path, &context()).is_err());
        drop(live);
    ...
    if path.exists() { fs::remove_file(&path).map_err(storage_error)?; }
