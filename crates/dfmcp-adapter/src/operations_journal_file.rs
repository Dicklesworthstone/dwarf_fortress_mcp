...
use super::{
    JournalLimits, JournalProfile, JournalStorage, ObservationJournal, Operations13,
    OperationsJournal, TailRecovery, storage_error,
};
use dfmcp_core::{
    Capability, DfmcpError, ErrorCode, FortressId, OperationContext, Result, RiskTier,
};
    ...
    fn validate_identity(&self) -> io::Result<()> {
        #[cfg(unix)]
        {
            ...
            let denied = || {
                io::Error::new(
                    ...
                    "private journal custody changed",
                )
            };
        ...
        #[cfg(not(unix))]
        {
            Err(io::Error::new(
                ...
                "private journals require Unix custody checks",
            ))
        }
    }
}
...
impl PrivateJournalFile {
    fn require_writable(&self) -> io::Result<()> {
        if self.read_only {
            return Err(io::Error::new(
                io::ErrorKind::PermissionDenied,
                "archive-only journal descriptor",
            ));
        }
        Ok(())
    }
    ...
    pub fn open_recovery<P: JournalProfile>(
        path: &Path,
        context: &OperationContext,
        limits: JournalLimits,
    ) -> Result<ObservationJournal<Self, P>> {
    ...
    fn open_storage(
        ...
        #[cfg(unix)]
        {
            ...
            if created && read_only {
                return Err(DfmcpError::new(
                    ...
                    "archive recovery requires an existing journal; no file was created",
                ));
            }
            ...
            file.try_lock().map_err(|_| {
                DfmcpError::new(
                    ErrorCode::Conflict,
                    "observation journal already has an owner or cannot be exclusively locked",
                )
            })?;
        ...
        #[cfg(not(unix))]
        {
            ...
            Err(DfmcpError::new(
                ErrorCode::CapabilityDenied,
                "private observation journals are currently supported on Unix only",
            ))
        }
    }
}
...
pub fn open_private_journal(
    path: &Path,
    context: &OperationContext,
    limits: JournalLimits,
    recovery: TailRecovery,
) -> Result<OperationsJournal<PrivateJournalFile>> {
...
pub fn open_profile_journal<P: JournalProfile>(
    path: &Path,
    context: &OperationContext,
    limits: JournalLimits,
    recovery: TailRecovery,
) -> Result<ObservationJournal<PrivateJournalFile, P>> {
    context.authorize(Capability::Observe, RiskTier::ReadOnly, &[], None)?;
    context.authorize(Capability::Query, RiskTier::ReadOnly, &[], None)?;
    limits.validate()?;
