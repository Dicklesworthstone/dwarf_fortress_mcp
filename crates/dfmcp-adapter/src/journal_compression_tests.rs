...
use crate::live_spatial::{LiveSpatialObservation, SpatialStateView,
    citizens::LiveSpatialCitizenObservation};
...
fn text(out: &mut Vec<u8>, s: &str) {
    out.extend_from_slice(&(s.len() as u16).to_be_bytes()); out.extend_from_slice(s.as_bytes());
}
...
fn observation(tick: u32, workers: u32, generation: u64) -> Result<LiveSpatialCitizenObservation> {
    ...
    let first = LiveSpatialObservation::decode_payload(&bytes, generation, "df".into(), "dfhack".into())?;
    let mut operations = first.operations().clone(); let mut terrain = first.terrain().clone();
    ...
    for i in 0..workers {
        ...
        for n in [0, 2, 2, 5] { put(&mut citizens, n); }
    ...
    LiveSpatialCitizenObservation::decode_payload(&combined, generation, "df".into(), "dfhack".into())
}
fn context() -> Result<OperationContext> {
    let mut state = Spatial18::empty(); Spatial18::publish(&mut state, observation(3, 8, 7)?)?;
    let anchor = Spatial18::snapshot(&state).ok_or_else(|| corrupt("test snapshot"))?.anchor();
    Ok(OperationContext { session_id: SessionId::new(1881), request_id: RequestId::new(1), anchor,
        budget: WorkBudget { max_wall_millis: 60_000, max_entities: 100_000,
            max_bytes: Spatial18::MAX_PAYLOAD as u64, max_output_tokens: 8192, ..WorkBudget::default() },
...
fn open(bytes: Vec<u8>, c: &OperationContext, repair: TailRecovery) -> Result<SpatialCitizenJournal<Memory>> {
    let new = bytes.is_empty();
    SpatialCitizenJournal::open(Memory { bytes: Cursor::new(bytes), ..Memory::default() }, c,
        JournalLimits::default(), new, repair)
}
fn filled() -> Result<(SpatialCitizenJournal<Memory>, OperationContext)> {
    let c = context()?; let mut j = open(Vec::new(), &c, TailRecovery::Refuse)?;
    j.append(observation(3, 8, 7)?, &c)?; j.append(observation(4, 8, 7)?, &c)?;
    Ok((j, c))
}
...
fn mixed_frames_reopen_and_historical_queries_reproduce_exact_anchors() -> Result<()> {
    let (j, c) = filled()?; let entries = j.entries().to_vec(); let stats = j.storage_stats();
    assert_eq!(stats.raw_records, 1); assert_eq!(stats.delta_records, 1);
    assert!(stats.stored_payload_bytes + 128 <= stats.expanded_payload_bytes);
    let expected = j.state().snapshot().cloned(); let bytes = j.storage.bytes.into_inner();
    ...
    assert_eq!(reopened.entries(), entries); assert_eq!(reopened.storage_stats(), stats);
    assert_eq!(reopened.state().snapshot(), expected.as_ref());
    for entry in entries {
        assert_eq!(reopened.snapshot_at(entry.number, entry.record_digest, &c)?.anchor(), entry.anchor);
    }
    assert_eq!(reopened.state().snapshot(), expected.as_ref());
    let length = reopened.retained_bytes();
    assert_eq!(reopened.append(observation(4, 8, 7)?, &c)?, JobPublication::Heartbeat);
    assert_eq!(reopened.retained_bytes(), length); assert_eq!(reopened.storage_stats(), stats);
    Ok(())
}
...
fn old_raw_prefixes_append_deltas_without_rewriting_any_retained_byte() -> Result<()> {
    ...
    let prefix = j.storage.bytes.get_ref().clone(); let old = j.entries().to_vec();
    let mut recovered = open(prefix.clone(), &c, TailRecovery::Refuse)?;
    recovered.append(observation(5, 8, 7)?, &c)?;
    assert_eq!(&recovered.storage.bytes.get_ref()[..prefix.len()], prefix.as_slice());
...
fn resets_and_periodic_records_are_raw_but_generations_survive_compression() -> Result<()> {
    let c = context()?; let mut j = open(Vec::new(), &c, TailRecovery::Refuse)?;
    for n in 0..65 { j.append(observation(3 + n, 8, 7)?, &c)?; }
    assert_eq!(j.storage_stats().raw_records, 2); assert_eq!(j.storage_stats().delta_records, 63);
    assert_eq!(&record(j.storage.bytes.get_ref(), &j.entries()[64])[..8], RECORD);
    j.append(observation(68, 7, 7)?, &c)?; j.append(observation(69, 8, 7)?, &c)?;
    let snapshot = j.state().snapshot().ok_or_else(|| corrupt("test state"))?;
    let citizen = snapshot.graph.entities.values().find(|e| e.kind == EntityKind::Unit
        && e.fields.get("native_unit_id").is_some_and(|f| f.value == Value::U64(17)))
    ...
    let expected = j.state().snapshot().cloned(); let entries = j.entries().to_vec();
    assert_eq!(&record(j.storage.bytes.get_ref(), &entries[67])[..8], RECORD);
...
fn compressed_capacity_extends_retention_without_widening_limits() -> Result<()> {
    let c = context()?; let first = observation(3, 8, 7)?;
    let mut j = open(Vec::new(), &c, TailRecovery::Refuse)?; j.append(first.clone(), &c)?;
    let raw_length = j.entries()[0].encoded_bytes as u64;
    j.limits.max_bytes = HEADER_BYTES as u64 + raw_length * 3;
    for tick in 4..9 { j.append(observation(tick, 8, 7)?, &c)?; }
    assert_eq!(j.entries().len(), 6); assert!(j.retained_bytes() < j.limits.max_bytes);
    let before = j.storage.bytes.get_ref().clone(); let stats = j.storage_stats();
    j.limits.max_records = 6;
    assert!(matches!(j.append(observation(9, 8, 7)?, &c), Err(e) if e.code == ErrorCode::BudgetExceeded));
    assert_eq!(j.storage.bytes.get_ref(), &before); assert_eq!(j.storage_stats(), stats);
    assert!(!j.fenced());
    Ok(())
}
...
fn every_incomplete_delta_prefix_requires_explicit_tail_repair() -> Result<()> {
    let (j, c) = filled()?; let second = j.entries()[1].clone(); let bytes = j.storage.bytes.into_inner();
    for end in second.offset as usize + 1..bytes.len() {
        let mut memory = Memory { bytes: Cursor::new(bytes[..end].to_vec()), ..Memory::default() };
        assert!(SpatialCitizenJournal::open(&mut memory, &c, JournalLimits::default(), false, TailRecovery::Refuse).is_err());
        assert_eq!(memory.bytes.get_ref(), &bytes[..end]);
        let recovered = open(bytes[..end].to_vec(), &c, TailRecovery::TruncateIncomplete)?;
        assert_eq!(recovered.entries().len(), 1); assert_eq!(recovered.storage_stats().delta_records, 0);
...
fn every_corrupt_delta_byte_is_refused_without_repair() -> Result<()> {
    let (j, c) = filled()?; let start = j.entries()[1].offset as usize; let bytes = j.storage.bytes.into_inner();
    for i in start..bytes.len() {
        let mut changed = bytes.clone(); changed[i] ^= 1;
        let mut memory = Memory { bytes: Cursor::new(changed.clone()), ..Memory::default() };
        assert!(SpatialCitizenJournal::open(&mut memory, &c, JournalLimits::default(), false,
            TailRecovery::TruncateIncomplete).is_err(), "byte {i}");
...
fn checksummed_wrong_predecessor_epoch_source_and_keyframe_are_not_trusted() -> Result<()> {
    let (j, c) = filled()?; let start = j.entries()[1].offset as usize;
    let id = j.id(); let bytes = j.storage.bytes.into_inner();
    for case in 0..4 {
        let mut changed = bytes.clone(); let frame = &mut changed[start..];
        let field = FRAME_HEADER_BYTES + match case { 0 => 8, 1 => 48, 2 => 104, _ => 0 };
        match case {
        ...
        reseal(frame, id);
        let mut memory = Memory { bytes: Cursor::new(changed.clone()), ..Memory::default() };
        assert!(matches!(SpatialCitizenJournal::open(&mut memory, &c, JournalLimits::default(), false,
...
fn partial_writes_and_uncertain_sync_preserve_state_stats_and_delta_base() -> Result<()> {
    let (j, c) = filled()?; let original = j.storage.bytes.into_inner();
    for failure in [Some(0), Some(1), Some(100), None] {
        let mut j = open(original.clone(), &c, TailRecovery::Refuse)?;
        let old = j.state().snapshot().cloned(); let stats = j.storage_stats();
        let base = j.payload_base.as_ref().map(|b| b.digest);
        j.storage.fail_after = failure; j.storage.fail_sync = failure.is_none();
        assert!(j.append(observation(5, 8, 7)?, &c).is_err()); assert!(j.fenced());
        assert_eq!(j.state().snapshot(), old.as_ref()); assert_eq!(j.storage_stats(), stats);
        assert_eq!(j.payload_base.as_ref().map(|b| b.digest), base);
        let reopened = open(j.storage.bytes.into_inner(), &c, TailRecovery::TruncateIncomplete)?;
        assert_eq!(reopened.entries().len(), if failure.is_none() { 3 } else { 2 });
    }
    Ok(())
}
...
fn expanded_budget_and_current_authority_apply_to_compressed_history() -> Result<()> {
    let (mut j, c) = filled()?; let entry = j.entries()[1].clone();
    let first = compression::decode::<Spatial18>(record(j.storage.bytes.get_ref(), &j.entries()[0]),
        j.id(), HEADER_BYTES as u64, None, Spatial18::MAX_PAYLOAD)?;
    let encoded = record(j.storage.bytes.get_ref(), &entry);
    assert!(matches!(compression::decode::<Spatial18>(encoded, j.id(), entry.offset,
        first.payload_base.as_ref(), 1), Err(e) if e.code == ErrorCode::BudgetExceeded));
    let mut denied = c.clone(); denied.grants.clear();
    assert!(matches!(j.state_at(2, entry.record_digest, &denied), Err(e) if e.code == ErrorCode::CapabilityDenied));
    let mut cancelled = c.clone(); cancelled.cancellation_requested = true;
    assert!(j.state_at(2, entry.record_digest, &cancelled).is_err());
    let mut narrow = c.clone(); narrow.budget.max_bytes = 1;
    assert!(matches!(j.state_at(2, entry.record_digest, &narrow), Err(e) if e.code == ErrorCode::BudgetExceeded));
    assert!(!j.fenced()); assert_eq!(j.entries().len(), 2);
    Ok(())
}

#[test]
fn legacy_profiles_keep_raw_bytes_and_reject_delta_markers_before_tail_repair() -> Result<()> {
    let (j, c) = filled()?; let source = observation(4, 8, 7)?;
    let op = source.spatial().operations(); let anchor = j.entries()[1].anchor;
    let expected = encode_profile_frame::<Operations13>(j.id(), 2, j.head(), anchor, op)?;
    assert_eq!(decode_profile_frame::<Operations13>(&expected, j.id(), 0)?.1, *op);
    assert_eq!(compression::encode::<Operations13>(j.id(), 2, j.head(), anchor, op,
        j.payload_base.as_ref())?, expected);
    for marker in [RECORD, compression::DELTA_RECORD] {
        let mut prefix = marker.to_vec(); prefix.extend_from_slice(&200u32.to_be_bytes());
        prefix.extend_from_slice(frame_header_hash(j.id(), &prefix).as_bytes());
        let raw = marker == RECORD;
        assert_eq!(decode_frame_header::<Operations13>(&prefix, j.id()).is_ok(), raw);
        assert_eq!(decode_frame_header::<Operations14>(&prefix, j.id()).is_ok(), raw);
        assert_eq!(decode_frame_header::<Spatial16>(&prefix, j.id()).is_ok(), raw);
    ...
    let mut bytes = j.storage.bytes.into_inner(); bytes[..8].copy_from_slice(Spatial16::MAGIC);
    let hash = header_hash(&bytes[..48]); bytes[48..80].copy_from_slice(hash.as_bytes());
    let mut memory = Memory { bytes: Cursor::new(bytes.clone()), ..Memory::default() };
    assert!(SpatialJournal::open(&mut memory, &c, JournalLimits::default(), false,
        TailRecovery::TruncateIncomplete).is_err());
