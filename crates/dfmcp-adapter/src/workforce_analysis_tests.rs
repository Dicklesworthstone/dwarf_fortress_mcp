...
fn state(masks: &[u8]) -> Result<LiveSpatialCitizenState> {
    ...
    let raw = (0..hex.len())
        .step_by(2)
        .map(|i| {
            u8::from_str_radix(&hex[i..i + 2], 16).map_err(|_| invalid("test spatial fixture hex"))
        })
    ...
    terrain.map = MapRegion {
        ...
        cells: vec![Cell::Visible(floor()); 9],
    };
    ...
    for part in [
        ...
        put(&mut spatial, part.len() as u32);
        spatial.extend_from_slice(&part);
    }
    let mut citizens = b"DFMC1800".to_vec();
    put(&mut citizens, masks.len() as u32);
    for (i, &mask) in masks.iter().enumerate() {
        put(&mut citizens, 10 + i as u32);
        ...
        put(&mut citizens, 6);
        citizens.extend_from_slice(&[1, 1]); // stress and both availability policies
        citizens.extend_from_slice(&(mask.count_ones() as u16).to_be_bytes());
        for (bit, key) in ["CARPENTRY", "MINING", "MASONRY"].iter().enumerate() {
            if mask & (1u8 << bit) != 0 {
                put(&mut citizens, bit as u32);
                text(&mut citizens, key);
                for n in [5, 5, 1] {
                    put(&mut citizens, n);
                }
            }
...
fn overlapping_demands_do_not_double_book_and_preserve_specialists() -> Result<()> {
    ...
    assert_eq!(
        result
            ...
            .assignments
            .iter()
            .map(|a| (a.demand_index, a.supply_id, a.units))
            .collect::<Vec<_>>(),
        [
            (0, citizen_entity_id(11).get(), 1),
            (1, citizen_entity_id(10).get(), 1)
        ]
    );
    ...
    assert_eq!(
        plan(
            &state,
            &context,
            &[demands[1].clone(), demands[0].clone()],
            1_000_000
        )?,
        result
    );
...
fn shortage_certificate_counts_unique_people_not_candidate_rows() -> Result<()> {
    ...
    let result = plan(
        &state,
        &context,
        &[demand("a", "CARPENTRY"), demand("b", "MINING")],
        1_000_000,
    )?;
    assert_eq!(
        result
    ...
    let shortage = result
        .allocation
        .shortage
        .ok_or_else(|| invariant("shortage absent"))?;
    assert_eq!(
        (
            shortage.required_units,
            shortage.eligible_units,
            shortage.deficit
        ),
        (2, 1, 1)
    );
...
fn all_512_three_worker_skill_graphs_match_independent_exhaustive_assignments() -> Result<()> {
    ...
    for mask in 0..512u16 {
        ...
        for assignment in &result.allocation.assignments {
            assert!(people.insert(assignment.supply_id));
            assert_eq!(assignment.units, 1);
            assert!(
                result.analysis.candidates[assignment.demand_index]
                    .iter()
                    .any(|c| c.entity_id == assignment.supply_id)
            );
        }
        if let Some(shortage) = result.allocation.shortage {
            let neighbors: BTreeSet<_> = shortage
                .demand_indices
                .iter()
                .flat_map(|&i| result.analysis.candidates[i].iter().map(|c| c.entity_id))
                .collect();
            assert_eq!(shortage.eligible_units, neighbors.len() as u64);
            assert_eq!(
                shortage.required_units - shortage.eligible_units,
                3 - expected
            );
        }
    }
...
fn multi_worker_demand_uses_each_citizen_once() -> Result<()> {
    ...
    let mut request = demand("a", "CARPENTRY");
    request.workers = 2;
...
fn unknown_skill_cannot_recruit_every_novice_at_minimum_zero() -> Result<()> {
    let state = state(&[1, 0])?;
    let context = context(&state)?;
    let mut requested = demand("a", "MISSPELLED");
    requested.min_effective_skill = 0;
    let result = analyze(&state, &context, &[requested.clone()], 1_000_000)?;
    assert_eq!(result.skill_key_observed, [false]);
    assert!(result.candidates[0].is_empty());
    assert_eq!(
        result.classifications[0].get("skill_key_not_observed"),
        Some(&2)
    );
    requested.skill_key = "CARPENTRY".into();
    assert_eq!(
        analyze(&state, &context, &[requested], 1_000_000)?.candidates[0].len(),
        2
    );
    Ok(())
}
...
fn occupied_endpoint_exception_never_bypasses_other_terrain_exclusions() -> Result<()> {
    for case in 0..8 {
        ...
        let cell = match case {
            ...
            4 => {
                tile.building_occupancy = 1;
                Cell::Visible(tile)
            }
            5 => {
                tile.walkable_region = 0;
                Cell::Visible(tile)
            }
            6 => {
                tile.shape = Shape::Ramp;
                Cell::Visible(tile)
            }
            _ => Cell::Visible(tile),
        };
...
fn denied_stale_and_exhausted_requests_do_not_return_feasibility_claims() -> Result<()> {
    ...
    for case in 0..4 {
        ...
        let expected = match case {
            ...
            2 => {
                context.anchor.cursor.sequence += 1;
                ErrorCode::StaleAnchor
            }
            _ => {
                context.budget.max_entities = 1;
                ErrorCode::BudgetExceeded
            }
        };
        assert!(
            matches!(plan(&state, &context, &requested, 1_000_000), Err(e) if e.code == expected)
        );
    }
    assert!(
        matches!(plan(&state, &context, &requested, 1), Err(e) if e.code == ErrorCode::BudgetExceeded)
    );
    Ok(())
}
...
fn request_shape_and_target_bounds_fail_before_allocation() -> Result<()> {
    let state = state(&[1])?;
    let context = context(&state)?;
    let request = demand("a", "CARPENTRY");
    ...
    for case in 0..5 {
        ...
        match case {
            0 => request.workers = 0,
            1 => request.workers = 129,
            2 => request.target = [3, 0, 5],
            3 => request.min_effective_skill = -1,
