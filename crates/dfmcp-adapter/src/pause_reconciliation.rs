...
fn receipt_digest(record: &DurablePauseRecord, tick: u64) -> Digest32 {
    let mut bytes = identity_bytes(b"dfmcp-control-receipt-v2\0", &record.idempotency_key,
        record.plan_digest, record.bridge_generation);
...
pub fn validate_prepare_reply(key: &str, plan: Digest32, paused: bool, tick: u64,
    effect: &PauseEffect) -> Result<[u8; 16]> {
    if key.is_empty() || key.len() > 512 || key.chars().any(char::is_control)
        || effect.bridge_generation == 0 || effect.bridge_generation == u64::MAX
        || effect.known || effect.applied || !effect.receipt_digest.is_empty() {
        return Err(rejected("invalid new pause preparation identity or outcome"));
    }
    let token: [u8; 16] = effect.prepare_token.as_slice().try_into()
    ...
    if token.as_slice() != &expected.as_bytes()[..16] {
        return Err(rejected("pause prepare token is not bound to the requested identity"));
    }
    Ok(token)
}
...
fn verified_receipt(record: &DurablePauseRecord, effect: &PauseEffect) -> Result<Option<Digest32>> {
    ...
    if !effect.prepare_token.is_empty() && effect.prepare_token.as_slice() != record.prepare_token.as_slice() {
        return Err(rejected("pause reconciliation token names another prepared effect"));
    }
    ...
    let digest: [u8; 32] = effect.receipt_digest.as_slice().try_into()
    ...
    if effect.observed_tick < record.expected_game_tick
        ...
        return Err(rejected("pause terminal receipt or observed outcome disagrees with durable identity"));
    }
    Ok(Some(digest))
}
...
pub fn reconcile_reply<S: EffectJournalStorage>(journal: &mut ControlEffectJournal<S>,
    key: &str, plan: Digest32, effect: &PauseEffect, context: &OperationContext)
    -> Result<DurablePauseRecord> {
    context.authorize(Capability::ControlClock, RiskTier::Reversible, &[], None)?;
    if journal.read_only() {
        return Err(DfmcpError::new(ErrorCode::CapabilityDenied, "offline recovery cannot reconcile or write evidence"));
    }
    let _ = journal.records(context)?;
    let record = journal.lookup(key).cloned()
        .ok_or_else(|| DfmcpError::new(ErrorCode::InvalidRequest, "unknown durable pause effect"))?;
    if record.plan_digest != plan {
        return Err(DfmcpError::new(ErrorCode::Conflict, "pause effect plan digest disagrees"));
    }
    if !record.state.reconciliation_required() {
        return Err(DfmcpError::new(ErrorCode::Conflict, "only unresolved commit attempts may be reconciled"));
    }
    match verified_receipt(&record, effect)? {
        None => journal.mark_indeterminate(key, plan, context),
        Some(receipt) => journal.record_reconciliation(key, plan, effect.bridge_generation,
            true, effect.applied, effect.paused, effect.observed_tick, Some(receipt), context),
    }
}
...
pub fn select_effects<S: EffectJournalStorage>(journal: &mut ControlEffectJournal<S>,
    keys: &[String], context: &OperationContext) -> Result<Vec<DurablePauseRecord>> {
    context.authorize(Capability::ControlClock, RiskTier::Reversible, &[], None)?;
    if journal.read_only() {
        return Err(DfmcpError::new(ErrorCode::CapabilityDenied, "offline recovery cannot contact a bridge"));
    }
    if keys.is_empty() || keys.len() > MAX_RECONCILIATION_EFFECTS
        || keys.len() as u64 > u64::from(context.budget.max_entities) {
        return Err(DfmcpError::new(ErrorCode::BudgetExceeded, "reconciliation selects 1..16 effects within the entity budget"));
    }
    if keys.iter().any(|key| key.is_empty() || key.len() > 512 || key.chars().any(char::is_control)) {
        return Err(DfmcpError::new(ErrorCode::InvalidRequest, "invalid reconciliation idempotency key"));
    }
    let ordered: BTreeSet<_> = keys.iter().collect();
    if ordered.len() != keys.len() {
        return Err(DfmcpError::new(ErrorCode::InvalidRequest, "duplicate reconciliation idempotency key"));
    }
    let _ = journal.records(context)?;
    ordered.into_iter().map(|key| journal.lookup(key).cloned().ok_or_else(||
        DfmcpError::new(ErrorCode::InvalidRequest, "reconciliation selection contains an unknown effect"))).collect()
}
...
pub fn reconcile_batch<S: EffectJournalStorage, Q: PauseReconciliationSource>(
    journal: &mut ControlEffectJournal<S>, source: &mut Q, keys: &[String],
    context: &OperationContext) -> Result<ReconciliationBatch> {
...
fn reconcile_batch_with_clock<S: EffectJournalStorage, Q: PauseReconciliationSource>(
    journal: &mut ControlEffectJournal<S>, source: &mut Q, keys: &[String],
    context: &OperationContext, mut elapsed: impl FnMut() -> Duration) -> Result<ReconciliationBatch> {
    let selected = select_effects(journal, keys, context)?;
    let mut batch = ReconciliationBatch { items: Vec::with_capacity(selected.len()),
        head_before: journal.head(), head_after: journal.head(), stopped: None };
    let budget = Duration::from_millis(context.budget.max_wall_millis);
    for record in selected {
        let mut item = ReconciliationItem { record, queried: false, deferred: false, error: None };
        if item.record.state.reconciliation_required() {
            let remaining = budget.checked_sub(elapsed()).filter(|v| *v >= Duration::from_millis(1));
            ...
            if batch.stopped.is_some() {
                ...
                context.authorize(Capability::ControlClock, RiskTier::Reversible, &[], None)?;
                item.queried = true;
                let outcome = source.query_effect(&item.record, remaining, context).and_then(|effect|
                    reconcile_reply(journal, &item.record.idempotency_key, item.record.plan_digest, &effect, context));
                match outcome {
