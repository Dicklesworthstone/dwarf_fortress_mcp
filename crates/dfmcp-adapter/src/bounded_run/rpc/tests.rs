...
fn reply(record: Option<&[u8]>, observation: Option<&[u8]>, active: bool) -> Vec<u8> {
    ...
    bytes(&mut out, 3, &[b'n'; 32]);
    bytes(&mut out, 7, b"fake-df");
    bytes(&mut out, 8, b"fake-dfhack");
...
fn six_fixed_methods_and_fragmented_observation() -> Result<()> {
    let o = observation()?;
    let mut c = client(&[reply(None, Some(o.canonical_bytes()), false)])?;
    assert_eq!(c.methods, [2, 3, 4, 5, 6, 7]);
    assert_eq!(c.observe(&context())?, o);
    for name in METHODS {
        assert!(
            c.stream
                .output
...
fn absent_query_remains_absent_not_a_receipt() -> Result<()> {
    ...
    assert!(c.query(&plan()?, &context())?.is_none());
    assert!(!c.fenced());
    Ok(())
}
#[test]
fn malformed_commit_fences_and_never_replays() -> Result<()> {
    let p = plan()?;
    let mut invalid = raw_record(&p, 1, 0, true, false, Some(100));
    invalid[10] ^= 1;
    let mut c = client(&[reply(Some(&invalid), None, true)])?;
    assert!(c.commit(&p, &context()).is_err());
    assert!(c.fenced());
...
fn active_record_requires_same_generation_and_owner() -> Result<()> {
    let p = plan()?;
    let running = raw_record(&p, 1, 0, true, false, Some(100));
    let mut c = client(&[reply(Some(&running), None, false)])?;
    assert!(c.query(&p, &context()).is_err());
    Ok(())
}
#[test]
fn unauthorized_commit_does_not_touch_transport() -> Result<()> {
    ...
    let mut cx = context();
    cx.grants
        .retain(|g| g.capability != Capability::ControlClock);
    let sent = c.stream.output.len();
    assert!(c.commit(&plan()?, &cx).is_err());
    assert_eq!(sent, c.stream.output.len());
    assert!(!c.fenced());
    Ok(())
}
...
fn duplicate_unknown_overlong_and_boolean_fields_fail_closed() {
    for raw in [
        ...
        assert!(Message::parse(raw, 12).is_err());
    }
    assert!(
        Message::parse(&[8, 2], 12)
            .and_then(|m| m.boolean(1))
            .is_err()
    );
}
#[test]
fn deadline_and_endpoint_rejected_before_network() {
    let mut cx = context();
    cx.budget.max_wall_millis = 0;
    assert!(
        RunRpcClient::connect(
