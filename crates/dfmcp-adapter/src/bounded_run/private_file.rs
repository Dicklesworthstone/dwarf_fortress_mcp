...
fn io_denied() -> io::Error {
    io::Error::new(
        ...
        "bounded-run journal custody changed",
    )
}
fn storage_error(_: io::Error) -> dfmcp_core::DfmcpError {
    error(
        ErrorCode::CorruptLedger,
        "bounded-run journal I/O or custody failed",
    )
}
impl PrivateRunFile {
...
impl Read for PrivateRunFile {
    fn read(&mut self, out: &mut [u8]) -> io::Result<usize> {
        self.file.read(out)
    }
}
impl Seek for PrivateRunFile {
...
pub fn open_private_run_journal(
    path: &Path,
    context: &OperationContext,
    mode: RunMode,
    binding: Option<RunBinding>,
) -> Result<RunJournal<PrivateRunFile>> {
    ...
    ))]
    {
        ...
        let denied = || {
            error(
                ...
                "run journal requires absolute normalized path, private 0700 parent and single-link 0600 regular file",
            )
        };
        if !path.is_absolute()
        ...
        if created && mode != RunMode::Control {
            return Err(error(
                ErrorCode::InvalidRequest,
                "run recovery requires an existing journal",
            ));
        }
        let mut options = OpenOptions::new();
        options
            .read(true)
            .write(mode != RunMode::Offline)
        ...
        file.try_lock().map_err(|_| {
            error(
                ErrorCode::Conflict,
                "run journal already owned or file locking unavailable",
            )
        })?;
        ...
        let storage = PrivateRunFile {
            file,
            path: path.to_owned(),
            read_only: mode == RunMode::Offline,
            extent: opened.len(),
            identity: (
                opened.dev(),
                opened.ino(),
    ...
    )))]
    {
        ...
        Err(error(
            ErrorCode::CapabilityDenied,
            "run journal custody currently supports Linux x86_64/aarch64 only",
        ))
    }
}
