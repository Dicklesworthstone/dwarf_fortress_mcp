...
pub(super) fn observation() -> Result<RunObservation> {
    ...
    for value in [41u64, 0, 100] {
        bytes.extend_from_slice(&value.to_be_bytes());
    }
    bytes.extend_from_slice(&[1, 1, 1]);
    RunObservation::decode(&bytes)
}
pub(super) fn plan() -> Result<RunPlan> {
    RunPlan::new("test", RunSpec::new(10, 1000)?, observation()?)
}
...
pub(super) fn raw_record(
    ...
    let receipt = hash(b"dfmcp-bounded-run-receipt/1", &bytes);
...
fn sealed_intent_and_observation_round_trip() -> Result<()> {
    let plan = plan()?;
    assert_eq!(RunPlan::decode(&plan.canonical_bytes())?, plan);
    assert_eq!(observation()?.tick(), Some(100));
    for bytes in [vec![], vec![0; 34], vec![0; 36]] {
        assert!(RunObservation::decode(&bytes).is_err());
    }
    let mut bytes = *plan.before().canonical_bytes();
    bytes[32] = 0;
    assert!(RunObservation::decode(&bytes).is_err());
    for spec in [(0, 1), (1201, 1), (1, 0), (1, 60001)] {
        assert!(RunSpec::new(spec.0, spec.1).is_err());
    }
    assert!(RunPlan::new("bad key", plan.spec(), observation()?).is_err());
    Ok(())
}
#[test]
        ...
        for reason in 0..10 {
            for flags in 0..8 {
                ...
                let state_ok = match phase {
                    ...
                    4 => [3, 7, 9].contains(&reason) && !known,
                    _ => reason == 7 && !known,
                };
                let expected = state_ok
                    && attempted == [1, 2, 3, 5].contains(&phase)
                    && verified == (phase == 3);
...
fn record_corruption_and_every_truncated_prefix_rejected() -> Result<()> {
    ...
    for i in 0..bytes.len() {
        ...
        corrupt[i] ^= 1;
        assert!(RunRecord::decode(&corrupt).is_err());
    }
    let mut trailing = bytes;
    trailing.push(0);
    assert!(RunRecord::decode(&trailing).is_err());
    Ok(())
}
#[test]
fn stopped_is_historical_and_overshoot_is_not_hidden() -> Result<()> {
    let plan = plan()?;
    let record = RunRecord::decode(&raw_record(&plan, 3, 2, true, true, Some(125)))?;
    assert_eq!(record.observed_ticks_advanced(), Some(25));
    assert_eq!(record.observed_tick_overshoot(), Some(15));
    let record = RunRecord::decode(&raw_record(&plan, 3, 5, true, true, None))?;
    assert!(record.pause_verified());
    assert_eq!(record.observed_tick(), None);
    let lost = RunRecord::decode(&raw_record(&plan, 5, 7, true, false, None))?;
    assert!(!lost.pause_verified());
    assert!(lost.unpause_attempted());
    Ok(())
}
#[test]
fn canonical_reference_fixture_matches_exact_rust_encoding() -> Result<()> {
    ...
    assert_eq!(RunRecord::decode(&bytes)?.phase(), RunPhase::Stopped);
    Ok(())
}
#[test]
fn control_requires_full_risk_scope_and_run_horizon() -> Result<()> {
    let plan = plan()?;
    let mut c = context();
    rpc::authorize_plan(&c, &plan)?;
    c.grants[2].expires_at_tick = Some(GameTick(109));
    assert!(rpc::authorize_plan(&c, &plan).is_err());
    c.grants[2].expires_at_tick = Some(GameTick(110));
    rpc::authorize_plan(&c, &plan)?;
    c.grants[2].remaining_uses = Some(1);
    assert!(rpc::authorize_plan(&c, &plan).is_err());
    c.grants[2].remaining_uses = None;
    c.budget.max_game_ticks = 9;
    assert!(rpc::authorize_plan(&c, &plan).is_err());
    c.budget.max_game_ticks = 10;
    c.anchor.fortress_id = FortressId::new(1);
    assert!(rpc::authorize_plan(&c, &plan).is_err());
    c.anchor.fortress_id = FortressId::NIL;
