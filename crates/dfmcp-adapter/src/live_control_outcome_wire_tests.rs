...
impl Read for Script {
    fn read(&mut self, out: &mut [u8]) -> io::Result<usize> { self.input.read(out) }
}
impl Write for Script {
    fn write(&mut self, bytes: &[u8]) -> io::Result<usize> {
        self.output.extend_from_slice(bytes); Ok(bytes.len())
    }
    fn flush(&mut self) -> io::Result<()> { Ok(()) }
}
fn reply(known: bool, applied: bool) -> Vec<u8> {
    let mut out = Vec::new();
    number(&mut out, 1, 1); number(&mut out, 2, 0); bytes(&mut out, 3, &[b'n'; 16]);
    number(&mut out, 4, 1); number(&mut out, 5, 7); number(&mut out, 6, 9);
    bytes(&mut out, 7, b"df"); bytes(&mut out, 8, b"dfhack");
    if known {
        number(&mut out, 10, 1); number(&mut out, 11, u64::from(applied));
        number(&mut out, 12, 0); number(&mut out, 13, 42);
        if applied { bytes(&mut out, 14, &[7; 32]); }
    }
    out
}
...
fn client(tail: Vec<u8>) -> Result<ControlRpcClient<Script>> {
    ...
    for id in [2, 3, 4, 5] {
        let mut binding = Vec::new(); number(&mut binding, 1, id);
        input.extend(frame(&binding));
    }
    input.extend(frame(&reply(false, false))); input.extend(tail);
    ControlRpcClient::negotiate(Script { input: Cursor::new(input), output: Vec::new() },
        vec![b't'; 32], vec![b'n'; 16])
}
...
fn missing_receipt_is_ambiguous_not_a_terminal_proof() -> Result<()> {
    ...
    assert!(pending.known); assert!(!pending.applied); assert!(pending.receipt_digest.is_empty());
...
fn oversized_reply_fences_before_a_followup_can_consume_unread_bytes() -> Result<()> {
    ...
    tail.extend(vec![0; 4097]); tail.extend(frame(&reply(true, true)));
    let mut client = client(tail)?;
    assert!(matches!(client.query_pause("k", Digest32::ZERO), Err(e) if e.code == ErrorCode::BudgetExceeded));
    assert!(client.poisoned());
    let output = client.stream.output.len(); let position = client.stream.input.position();
    assert!(matches!(client.query_pause("k", Digest32::ZERO), Err(e) if e.code == ErrorCode::AdapterUnavailable));
    assert_eq!(client.stream.output.len(), output); assert_eq!(client.stream.input.position(), position);
    Ok(())
}
...
fn incomplete_outcome_fences_the_connection_without_synthesizing_false() -> Result<()> {
    ...
    assert!(matches!(client.query_pause("k", Digest32::ZERO), Err(e) if e.code == ErrorCode::AdapterRejected));
...
fn local_key_validation_neither_writes_nor_poison_the_connection() -> Result<()> {
    ...
    assert!(matches!(client.query_pause("", Digest32::ZERO), Err(e) if e.code == ErrorCode::InvalidRequest));
    assert_eq!(client.stream.output.len(), output); assert!(!client.poisoned()); Ok(())
}

#[test]
fn connection_and_handshake_share_one_timeout_budget() -> Result<()> {
    let total = Duration::from_millis(500);
    assert_eq!(remaining_timeout(total, Duration::from_millis(400))?, Duration::from_millis(100));
    assert_eq!(remaining_timeout(total, Duration::from_millis(499))?, Duration::from_millis(1));
    for elapsed in [Duration::from_micros(499_001), total, Duration::from_millis(501)] {
        assert!(matches!(remaining_timeout(total, elapsed), Err(e) if e.code == ErrorCode::BudgetExceeded));
    }
    Ok(())
}
