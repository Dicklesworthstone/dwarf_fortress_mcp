...
fn configuration(token: &[u8], nonce: &[u8], region: Region, maximum: usize) -> Result<()> {
    ...
    if !(32..=256).contains(&token.len())
        ...
        || !(1024..=MAX_MAP_BYTES).contains(&maximum)
    {
        return Err(DfmcpError::new(
            ErrorCode::InvalidRequest,
            "invalid map credentials or byte budget",
        ));
    }
    Ok(())
}
fn request(token: &[u8], nonce: &[u8], region: Region, maximum: usize) -> Vec<u8> {
    ...
    number(&mut out, 3, 1);
    number(&mut out, 4, 5);
    for (index, v) in region.origin.into_iter().chain(region.size).enumerate() {
        number(&mut out, index as u32 + 5, u64::from(v));
    }
    number(&mut out, 11, maximum as u64);
    out
}
fn bind<S: Read + Write>(stream: &mut S, method: &str) -> Result<i16> {
    ...
    for (f, v) in [
        (1, method),
        (2, "dfmcp.map.v1_5.Request"),
        (3, "dfmcp.map.v1_5.Reply"),
        (4, "dfmcp_map_v1_5"),
    ] {
        bytes(&mut input, f, v.as_bytes());
    }
    let reply = call(stream, 0, &input, 1024)?;
    let id = i16::try_from(Message::parse(&reply, 1)?.number(1)?)
        .map_err(|_| invalid("map method ID overflow"))?;
    if id < 2 {
        return Err(invalid(
            "map method resolved to reserved native core method",
        ));
    }
    Ok(id)
}
fn decode<'a>(
    ...
    if m.number(4)? != 1 || m.number(5)? != 5 {
        return Err(DfmcpError::new(
            ErrorCode::VersionMismatch,
            "map bridge must be exactly 1.5",
        ));
    }
    if m.bytes(3, 64)? != nonce {
        return Err(invalid("map nonce mismatch"));
    }
    let accepted = m.number(1)?;
    let code = m.number(2)?;
    if accepted == 0 {
        if code == 0 || m.0.contains_key(&9) {
            return Err(invalid("inconsistent rejected map reply"));
        }
        return Err(DfmcpError::new(
            match code {
            ...
            "map bridge refused; no region published",
        ));
    }
    if accepted != 1 || code != 0 {
        return Err(invalid("invalid map acceptance fields"));
    }
    ...
    if manifest.generation == 0 || manifest.generation == u64::MAX {
        return Err(invalid("invalid map generation"));
    }
    let payload = if read {
        Some(m.bytes(9, maximum)?)
    } else if m.0.contains_key(&9) {
        return Err(invalid("map handshake carried terrain"));
    } else {
        None
    };
    Ok((manifest, payload))
...
impl<S: Read + Write> MapRpcClient<S> {
    pub fn negotiate(
        ...
        if &reply[..8] != b"DFHack!\n" || reply[8..] != 1i32.to_le_bytes() {
            return Err(invalid("invalid map native handshake"));
        }
        let handshake = bind(&mut stream, "Handshake")?;
        let method = bind(&mut stream, "ReadObservation")?;
        if handshake == method {
            return Err(invalid("map native methods alias"));
        }
        let reply = call(
            &mut stream,
            handshake,
            &request(&token, &nonce, region, maximum),
            4096,
        )?;
    ...
    pub fn read_observation(&mut self) -> Result<LiveMapObservation> {
        ...
        let result = (|| {
            let reply = call(
                &mut self.stream,
                self.method,
                &request(&self.token, &self.nonce, self.region, self.maximum),
                self.maximum + 4096,
            )?;
                ...
                || manifest.dfhack_version != self.manifest.dfhack_version
            {
                return Err(DfmcpError::new(
                    ...
                    "map software or generation changed incompatibly",
                ));
            }
            let observation = LiveMapObservation::decode_payload(
                payload.ok_or_else(|| invalid("map payload absent"))?,
                manifest.generation,
                manifest.df_version.clone(),
                manifest.dfhack_version.clone(),
            )?;
            if observation.map.region != self.region {
                return Err(invalid("map bridge returned a different region"));
            }
    ...
    pub fn connect(
        ...
        if !endpoint.ip().is_loopback() || endpoint.port() == 0 {
            return Err(DfmcpError::new(
                ...
                "map endpoint must be numeric loopback",
            ));
        }
        let deadline = Instant::now()
            .checked_add(timeout)
            .ok_or_else(|| invalid("map deadline overflow"))?;
...
mod tests {
    ...
    fn reply(minor: u64, payload: Option<&[u8]>) -> Vec<u8> {
        let mut out = Vec::new();
        for (f, v) in [(1, 1), (2, 0), (4, 1), (5, minor), (6, 7)] {
            number(&mut out, f, v);
        }
    ...
    fn frame(v: &[u8]) -> Vec<u8> {
        let mut b = header(-1, v.len() as i32).to_vec();
        b.extend_from_slice(v);
        b
    }
    fn golden() -> Result<Vec<u8>> {
        let h = include_str!("../tests/fixtures/map_v1_5.hex").trim();
        (0..h.len())
            .step_by(2)
            .map(|i| u8::from_str_radix(&h[i..i + 2], 16).map_err(|_| invalid("bad test hex")))
            .collect()
    }
    #[test]
    fn actual_wire_decodes_native_fixture_and_fences_truncation() -> Result<()> {
        ...
        for id in [2, 3] {
            let mut v = Vec::new();
            number(&mut v, 1, id);
            input.extend(frame(&v));
        }
        input.extend(frame(&reply(5, None)));
        input.extend(frame(&reply(5, Some(&golden()?))));
        let mut c = MapRpcClient::negotiate(
            Script {
        ...
        assert_eq!(c.read_observation()?.map.cells.len(), 24);
        assert!(!c.poisoned());
        assert!(c.read_observation().is_err());
        let sent = c.stream.output.len();
        assert!(c.poisoned());
        assert!(c.read_observation().is_err());
        assert_eq!(sent, c.stream.output.len());
        Ok(())
    }
    #[test]
    fn nonce_protocol_and_payload_roles_are_not_interchangeable() {
        assert!(decode(&reply(4, None), &[b'n'; 16], false, 1024).is_err());
        assert!(decode(&reply(5, None), &[b'x'; 16], false, 1024).is_err());
        assert!(decode(&reply(5, Some(b"x")), &[b'n'; 16], false, 1024).is_err());
        assert!(decode(&reply(5, None), &[b'n'; 16], true, 1024).is_err());
    }
}
