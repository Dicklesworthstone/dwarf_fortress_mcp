...
pub(super) fn is_record_prefix<P: JournalProfile>(prefix: &[u8]) -> bool {
    ...
    prefix[..n] == RECORD[..n]
        || (P::DELTA_STORAGE && prefix[..n] == DELTA_RECORD[..n])
}

pub(super) fn encode<P: JournalProfile>(id: Digest32, number: u64, previous: Digest32,
    anchor: StateAnchor, observation: &P::Observation, base: Option<&PayloadBase>) -> Result<Vec<u8>> {
    ...
    let Some(base) = base.filter(|base| P::DELTA_STORAGE
        && base.digest == previous && number % RAW_INTERVAL != 1
        && base.anchor.fortress_id == anchor.fortress_id
        && base.anchor.cursor.epoch == anchor.cursor.epoch
        && base.anchor.cursor.sequence < anchor.cursor.sequence) else { return Ok(raw); };
    let end = raw.len() - 40;
    ...
    if !body.0.is_empty() { return Err(corrupt("encoded journal payload has trailing data")); }
    let Some(delta) = journal_delta::encode(&base.bytes, payload)
        .map_err(|_| budget("journal payload delta exceeds codec bounds"))? else { return Ok(raw); };
...
pub(super) fn decode<P: JournalProfile>(frame: &[u8], id: Digest32, offset: u64,
    base: Option<&PayloadBase>, maximum_payload: usize) -> Result<Decoded<P>> {
    ...
    let payload = if is_delta {
        let base = base.ok_or_else(|| corrupt("delta record has no verified predecessor payload"))?;
        if previous_digest != base.digest || number % RAW_INTERVAL == 1
            ...
            return Err(corrupt("delta record crosses its predecessor, keyframe or observation epoch"));
        }
        ...
        if expanded > maximum { return Err(budget("expanded archived observation exceeds acquisition budget")); }
        Cow::Owned(journal_delta::decode(&base.bytes, stored, maximum)
            .map_err(|_| corrupt("invalid bounded journal payload delta"))?)
    } else {
        if n > P::MAX_PAYLOAD { return Err(corrupt("journal payload exceeds its fixed profile limit")); }
        if n > maximum { return Err(budget("archived observation exceeds acquisition budget")); }
        Cow::Borrowed(stored)
    };
    ...
    let measurement = JournalStorageStats { raw_records: u64::from(!is_delta), delta_records: u64::from(is_delta),
        expanded_payload_bytes: payload.len() as u64, stored_payload_bytes: n as u64 };
    let payload_base = if P::DELTA_STORAGE {
        Some(PayloadBase { digest: record_digest, anchor, bytes: payload.into_owned() })
    } else { None };
    Ok(Decoded { entry: JournalEntry { number, anchor, source_digest, record_digest, previous_digest,
        offset, encoded_bytes: frame.len() as u32 }, observation, payload_base, measurement })
}
