...
pub(super) fn normalize(
    ...
    overrides: &BTreeMap<String, [u32; 3]>,
) -> Result<Vec<[u32; 3]>> {
    ...
    for (key, origin) in overrides {
        if !tasks.iter().any(|task| &task.key == key) || origin.iter().any(|n| *n >= 32768) {
            return Err(invalid(
                "production site override names an absent task or invalid coordinate",
            ));
        }
    }
    Ok(tasks
        .iter()
        .map(|task| overrides.get(&task.key).copied().unwrap_or(default))
        .collect())
}
...
fn merge_supply(
    pool: &mut BTreeMap<u64, flow::Supply>,
    supply: &flow::Supply,
    mask: u32,
) -> Result<()> {
        ...
        Some(prior) => {
            if prior.units != supply.units {
                return Err(invariant(
                    "same-capture site reports disagree about stack capacity",
                ));
            }
        ...
        None => {
            if pool.len() >= flow::MAX_SUPPLIES {
                return Err(exhausted(
                    "combined production sites exceed the shared stack bound",
                ));
            }
            pool.insert(
                supply.id,
...
pub(super) fn analyze(
    ...
    default: &SpatialInventory,
    task_origins: Vec<[u32; 3]>,
    material_owners: &[usize],
    work: &mut Work,
) -> Result<ProductionSites> {
    ...
    let material_origins = material_owners
        .iter()
        .map(|owner| {
            if *owner == selection::RESERVE_OWNER {
                Ok(default.origin)
            } else {
                task_origins
                    .get(*owner)
                    .copied()
                    .ok_or_else(|| invariant("material site owner missing"))
            }
        })
    ...
    for site in distinct {
        ...
        if report.anchor != default.anchor
            || report.source_digest != default.source_digest
            || report.demands != default.demands
            || report.origin != site
        {
            return Err(invariant(
                "production sites do not share one source and material model",
            ));
        }
