...
use crate::live_operations::{
    LiveItem, LiveOperationsObservation, building_entity_id, item_entity_id,
};
use dfmcp_core::{
    Capability, DfmcpError, Digest32, EntityId, ErrorCode, OperationContext, Result, RiskTier,
    StateAnchor,
};
use dfmcp_world::inventory_allocation::{
    self as flow, Allocation, AllocationError, Demand, Supply,
};
...
impl Work {
    fn new(maximum: u64, wall_millis: u64) -> Result<Self> {
        if maximum == 0 || maximum > MAX_ANALYSIS_WORK || wall_millis == 0 {
            return Err(exhausted("invalid analysis work bound"));
        }
        Ok(Self {
            used: 0,
            maximum,
    ...
    fn charge(&mut self, units: u64) -> Result<()> {
        self.used = self
            .used
            .checked_add(units)
            .ok_or_else(|| exhausted("analysis counter overflow"))?;
        if self.used > self.maximum {
            return Err(exhausted("operations analysis exhausted its work budget"));
        }
        if self.started.elapsed().as_millis() >= u128::from(self.wall_millis) {
            return Err(exhausted(
                "operations analysis exhausted its cooperative wall-time budget",
            ));
        }
        Ok(())
    }
...
fn handle(snapshot: &WorldSnapshot, id: EntityId) -> Result<AnalysisHandle> {
    let entity = snapshot
        ...
        .entities
        .get(&id)
        .ok_or_else(|| invariant("analysis endpoint has no canonical entity"))?;
    Ok(AnalysisHandle {
        entity_id: id,
...
fn source<'a, S: OperationsStateView + ?Sized>(
    state: &'a S,
    ...
    let observation = state
        ...
        .ok_or_else(|| invalid("no coherent operations observation is published"))?;
    let snapshot = state
        .operations_snapshot()
        .ok_or_else(|| invariant("operations projection missing"))?;
    if snapshot.anchor() != context.anchor {
        return Err(DfmcpError::new(
            ErrorCode::StaleAnchor,
            "analysis context names another observation",
        ));
    }
    if snapshot.graph.entities.len() > context.budget.max_entities as usize {
        return Err(exhausted(
            "operations analysis exceeds the session scan budget",
        ));
    }
    if !snapshot.hash_is_valid() {
        return Err(invariant("operations analysis source hash is invalid"));
    }
...
impl InventoryIndex {
    fn build(observation: &LiveOperationsObservation, work: &mut Work) -> Result<Self> {
        let ordinal: BTreeMap<_, _> = observation
            .items
            .iter()
            .enumerate()
            .map(|(i, item)| (item.native_id, i))
            .collect();
        ...
        for attachment in &observation.attachments {
            ...
            attachment_jobs
                ...
                .or_default()
                .insert(attachment.job_native_id);
        }
        ...
        for item in &observation.items {
            work.charge(1)?;
            let ancestor =
                item.container_native_id
                    .map(|id| {
                        ordinal.get(&id).copied().ok_or_else(|| {
                            invariant("analysis encountered an unobserved container")
                        })
                    })
                    .transpose()?;
            parent.push(ancestor);
            let mut flags = item.flags & DISALLOWED_FLAGS;
            if attachment_jobs.contains_key(&item.native_id) {
                flags |= ATTACHED;
            }
            if item.holder_building_native_id.is_some() {
                flags |= BUILDING_HELD;
            }
            direct.push(flags);
        }
...
fn normalized_demands(input: &[MaterialDemand]) -> Result<Vec<MaterialDemand>> {
    if input.is_empty() || input.len() > flow::MAX_DEMANDS {
        return Err(exhausted("request 1..32 material demands"));
    }
    for demand in input {
        if demand.key.is_empty()
            ...
            || demand.item_types.is_empty()
            || demand.item_types.len() > 8
            || demand
                .item_types
                .iter()
                .any(|v| v.is_empty() || v.len() > 128 || v.contains('\0'))
            || demand.subtype.is_some_and(|v| v < -1)
            || demand.material_type.is_some_and(|v| v < -1)
            || demand.material_index.is_some_and(|v| v < -1)
            || (demand.material_index.is_some() && demand.material_type.is_none())
        {
            return Err(invalid(
                "invalid material-demand key, units, type selectors, or raw material pair",
            ));
        }
    }
    let mut demands = input.to_vec();
    demands.sort_by(|a, b| a.key.cmp(&b.key));
    if demands.windows(2).any(|pair| pair[0].key == pair[1].key) {
        return Err(invalid("material-demand keys must be unique"));
    }
...
fn exclusion(flags: u32) -> Option<&'static str> {
    [
        (8, "removed_in_chain"),
        (1, "forbidden_in_chain"),
        (16, "rotten_in_chain"),
        (32, "trader_in_chain"),
        (ATTACHED, "attached_to_job_in_chain"),
        (2, "in_job_in_chain"),
        (4, "dump_in_chain"),
        (128, "in_inventory_in_chain"),
        (256, "in_building_in_chain"),
        (BUILDING_HELD, "building_holder_in_chain"),
    ]
    .into_iter()
    .find_map(|(bit, reason)| (flags & bit != 0).then_some(reason))
}

pub fn plan_inventory<S: OperationsStateView + ?Sized>(
    state: &S,
    context: &OperationContext,
    requested: &[MaterialDemand],
    max_work: u64,
    ...
    for (i, item) in observation.items.iter().enumerate() {
        ...
        let reason = if item.stack_size == 0 {
            ...
            exclusion(inventory.inherited[i])
        };
        ...
        candidate_stack_units = candidate_stack_units
            .checked_add(units)
            .ok_or_else(|| exhausted("inventory unit sum overflow"))?;
        supplies.push(Supply {
            id: item_entity_id(item.native_id).get(),
            units,
            eligible,
        });
    }
    let model: Vec<_> = demands
        .iter()
        .map(|d| Demand {
            key: d.key.clone(),
            units: d.units,
        ...
        .collect();
    let allocation = flow::allocate(&supplies, &model, work.maximum.saturating_sub(work.used))
        .map_err(|failure| match failure {
            AllocationError::BudgetExceeded => {
                exhausted("inventory allocation exhausted its shape or operation budget")
            }
            AllocationError::ArithmeticOverflow => {
                exhausted("inventory allocation exceeds the exact integer domain")
            }
            AllocationError::InvalidInput => {
                invalid("invalid normalized inventory allocation model")
            }
            AllocationError::InvariantViolation => {
                invariant("inventory allocation certificate failed")
            }
        })?;
    ...
    Ok(InventoryAnalysis {
        ...
        demands,
        allocation,
        candidate_items: supplies.len() as u64,
        candidate_stack_units,
        excluded_items,
        unmatched_items,
...
fn check_focus(
    ...
    if let Some((id, generation)) = focus {
        let entity = snapshot
            ...
            .entities
            .get(&id)
            .ok_or_else(|| invalid("diagnostic focus is not observed"))?;
        if entity.kind != kind {
            return Err(invalid("diagnostic focus has the wrong entity kind"));
        }
        if entity.generation != generation {
            return Err(DfmcpError::new(
                ErrorCode::Conflict,
                "diagnostic focus generation changed",
            ));
        }
    }
    Ok(())
...
pub fn diagnose_production<S: OperationsStateView + ?Sized>(
    state: &S,
    context: &OperationContext,
    scope: DiagnosisScope,
    max_work: u64,
) -> Result<ProductionDiagnosis> {
    let mut work = Work::new(max_work, context.budget.max_wall_millis)?;
    let (observation, snapshot, source_digest) = source(state, context)?;
    check_focus(snapshot, scope.job, EntityKind::Job)?;
    check_focus(snapshot, scope.holder, EntityKind::Building)?;
    let inventory = InventoryIndex::build(observation, &mut work)?;
    let buildings: BTreeMap<_, _> = observation
        .buildings
        .iter()
        .map(|v| (v.native_id, v))
        .collect();
    ...
    for attachment in &observation.attachments {
        work.charge(1)?;
        by_job
            .entry(attachment.job_native_id)
            .or_default()
            .push(attachment);
    }
    ...
    for job in &observation.jobs.jobs {
        ...
        if scope.job.is_some_and(|(id, _)| id != job_id)
            || scope.holder.is_some_and(|(id, _)| Some(id) != holder_id)
        {
            continue;
        }
        jobs_considered += 1;
        let holder = holder_id.map(|id| handle(snapshot, id)).transpose()?;
        let holder_stage = job
            .holder_native_id
            .map(|id| {
                buildings
                    .get(&id)
                    .map(|b| (b.build_stage, b.max_build_stage))
                    .ok_or_else(|| invariant("diagnosis holder not observed"))
            })
            .transpose()?;
        ...
        for attachment in by_job.get(&job.native_id).into_iter().flatten() {
            ...
            if attachment.filter_index >= 0 {
                filters.insert(attachment.filter_index as u32);
            }
        }
        ...
        for id in &items {
            ...
            let i = *inventory
                .ordinal
                .get(id)
                .ok_or_else(|| invariant("diagnosis attachment target not observed"))?;
            ...
            for (bit, name) in DIAGNOSTIC_FLAGS {
                work.charge(1)?;
                if item.flags & bit != 0 {
                    *direct_item_flags.entry(name).or_default() += 1;
                    affected = true;
                }
                if ancestor_flags & bit != 0 {
                    *container_item_flags.entry(name).or_default() += 1;
                    affected = true;
                }
            }
            if inventory
            ...
            {
                shared_attached_items += 1;
                affected = true;
            }
            if item.stack_size == 0 {
                zero_size_attached_items += 1;
                affected = true;
            }
            if affected {
                affected_item_count += 1;
                if affected_item_examples.len() < EXAMPLES {
                    affected_item_examples.push(handle(snapshot, item_entity_id(*id))?);
                }
            }
        }
        let uncovered = job
            .required_item_filter_count
            .checked_sub(filters.len() as u32)
            .ok_or_else(|| invariant("diagnosis filter index count exceeds native filter count"))?;
        let mut findings = Vec::new();
        if job.suspended {
            findings.push("job_suspended");
        }
        if job.worker_native_id.is_none() {
            findings.push("no_worker_assigned");
        }
        if holder_stage.is_some_and(|(stage, maximum)| stage < maximum) {
            findings.push("holder_under_construction");
        }
        if uncovered > 0 {
            findings.push("filters_without_indexed_attachments");
        }
        if !direct_item_flags.is_empty() {
            findings.push("attached_item_flags");
        }
        if !container_item_flags.is_empty() {
            findings.push("container_item_flags");
        }
        if shared_attached_items > 0 {
            findings.push("shared_attachment");
        }
        if zero_size_attached_items > 0 {
            findings.push("zero_stack_size_attachment");
        }
        if !findings.is_empty() {
            jobs_with_findings += 1;
        }
        for code in &findings {
            *finding_counts.entry(*code).or_default() += 1;
        }
        if findings.is_empty() && !scope.include_clear_jobs {
            continue;
        }
        rows.push(JobDiagnosis {
            job: handle(snapshot, job_id)?,
            native_job_id: job.native_id,
            type_key: job.type_key.clone(),
            suspended: job.suspended,
            worker_assigned: job.worker_native_id.is_some(),
            holder,
            holder_stage,
            attachment_records: job.attached_item_count,
            distinct_attached_items: items.len() as u32,
            required_filter_count: job.required_item_filter_count,
            filters_without_indexed_attachments: uncovered,
            direct_item_flags,
            container_item_flags,
            shared_attached_items,
            zero_size_attached_items,
            affected_item_count,
            affected_item_examples,
            findings,
        });
    }
    ...
    rows.sort_by_key(|row| {
        let flags = |name| {
            row.direct_item_flags.contains_key(name) || row.container_item_flags.contains_key(name)
        };
        (
            !flags("removed"),
            !flags("rotten"),
            ...
            !row.suspended,
            !row.holder_stage
                .is_some_and(|(stage, maximum)| stage < maximum),
            row.native_job_id,
        )
    });
    work.charge(0)?;
    Ok(ProductionDiagnosis {
        anchor: snapshot.anchor(),
        source_digest,
        jobs_considered,
        jobs_with_findings,
