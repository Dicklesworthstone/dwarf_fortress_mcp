...
use super::super::rpc::{CONNECT_BYTES, DigSource, RPC_BYTES, authorize};
use super::super::{DigObservation, DigPlan, DigRegion, error, validate_key};
use super::record::MAX_BODY_BYTES;
use super::{
    Allowance, DigBinding, DigCursor, DigGuard, DigJournal, DigMode, DigPage, DigRecord, DigState,
    DigSummary, MAX_FRAME_BYTES, exhausted, unknown,
};
...
impl<S: EffectJournalStorage, N: DigSource> DigSession<S, N> {
    ...
    pub fn new(mut journal: DigJournal<S>, context: &OperationContext) -> Result<Self> {
        ...
        let (high_tick, high_sequence) = journal.records.values().fold(
            (context.anchor.tick.get(), 0),
            |(tick, sequence), record| {
                let after =
                    record.plan.before().sequence()
                        + u64::from(record.effect.as_ref().is_some_and(|e| {
                            e.phase().terminal() && e.designated_count().is_some()
                        }));
    ...
    pub fn selected(&self) -> Option<&DigObservation> {
    ...
    fn in_scope(&self, region: DigRegion) -> Result<()> {
        ...
        if !scope.contains_cuboid(region.halo()) || !scope.contains_cuboid(region.write_area()) {
            return Err(error(
                ...
                "dig selection exceeds the operator-owned journal scope",
            ));
        }
        Ok(())
    }
    ...
    fn begin(
        ...
        context: &OperationContext,
        connect: bool,
        journal_operation: bool,
    ) -> Result<Allowance> {
        let context = self.current(context)?;
        let mut required =
            3 * (self.journal.raw.len() + 2 * MAX_FRAME_BYTES) as u64 + 2 * MAX_BODY_BYTES as u64;
        if connect {
            required += SOURCE_RESERVATION_BYTES + RPC_BYTES;
        }
        if journal_operation {
            required += self.journal_reservation();
        }
        ...
        G: DigSessionGuard,
    {
        ...
        if region.halo_count() > current.budget.max_entities as usize {
            return Err(exhausted());
        }
        ...
        let result = (|| {
            ...
            if capture.region() != region
                || capture.tick() < self.high_tick
                || capture.sequence() < self.high_sequence
            {
                return Err(error(
                    ErrorCode::StaleAnchor,
                    "dig observation changed selection or regressed behind retained evidence",
                ));
            }
    ...
    pub fn prepare<G: DigSessionGuard>(
        &mut self,
        key: &str,
        allow_hidden: bool,
        witness: Digest32,
        ...
        if let Some(old) = self.journal.records.get(key) {
            if old.plan.allow_hidden_neighbors() != allow_hidden
                || old.plan.before().witness() != witness
            {
                return Err(error(
                    ErrorCode::Conflict,
                    "existing dig key binds different review inputs",
                ));
            }
            authorize(
                &work.current()?,
                self.binding().fortress_id(),
                old.plan.before().tick(),
                old.plan.before().region(),
                true,
                true,
                true,
            )?;
        ...
        if self.journal.records.values().any(|r| !r.state.terminal()) {
        ...
        let capture = self
            ...
            .ok_or_else(|| {
                error(
                    ErrorCode::StaleAnchor,
                    "dig planning requires this session's exact retained observation",
                )
            })?;
        ...
        if result
            .as_ref()
            .is_ok_and(|record| record.state() == DigState::Prepared)
    ...
    pub fn commit<G: DigSessionGuard>(
        &mut self,
        key: &str,
        ...
        if self.source.is_none() || self.journal.fresh_key.as_deref() != Some(key) {
            return Err(unknown(key));
        }
        let operation = Self::reserve(&mut work, self.journal_reservation())?;
        let source = self.source.as_mut().ok_or_else(|| unknown(key))?;
        let result = self
            .journal
            .commit(source, key, confirmation, &operation, guard);
        self.abandon_preparation();
        if let Ok(record) = &result {
            self.advance_floor(record);
        }
        result
    }
        ...
        let sequence = record.plan.before().sequence()
            + u64::from(
                record
                    .effect
                    .as_ref()
                    .is_some_and(|e| e.designated_count().is_some()),
            );
    ...
    pub fn view(&mut self, context: &OperationContext) -> Result<DigSessionView> {
        ...
        let pending = unsettled.next().map(|r| DigSummary {
            ...
            dispatchable: self.source.is_some()
                && r.state == DigState::Prepared
                && self.journal.fresh_key.as_deref() == Some(r.plan.key()),
        });
        if unsettled.next().is_some() {
            return Err(error(
                ErrorCode::InternalInvariantViolation,
                "multiple unsettled mining keys violate journal custody",
            ));
        }
        work.current()?;
        Ok(DigSessionView {
            ...
            byte_len: self.journal.raw.len(),
            total_records: self.journal.records.len(),
            pending,
        })
    }
    ...
    pub fn list(
        ...
        for row in &mut page.records {
            row.dispatchable &= self.source.is_some();
        }
        Ok(page)
    }
        ...
        G: DigSessionGuard,
    {
        ...
        let result = if cancel {
            self.journal
                .cancel(&mut source, key, digest, &operation, guard)
        } else {
            self.journal
                .reconcile(&mut source, key, digest, &operation, guard)
        };
        if let Ok(record) = &result {
            self.advance_floor(record);
        }
