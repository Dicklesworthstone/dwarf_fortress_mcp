...
mod unsupported {
    ...
    fn denied() -> io::Error { io::Error::new(io::ErrorKind::Unsupported, "dig private storage requires Linux x86_64/aarch64") }
    impl Read for PrivateDigFile { fn read(&mut self, _: &mut [u8]) -> io::Result<usize> { Err(denied()) } }
    impl Write for PrivateDigFile {
        fn write(&mut self, _: &[u8]) -> io::Result<usize> { Err(denied()) }
        fn flush(&mut self) -> io::Result<()> { Err(denied()) }
    }
    impl Seek for PrivateDigFile { fn seek(&mut self, _: SeekFrom) -> io::Result<u64> { Err(denied()) } }
    impl EffectJournalStorage for PrivateDigFile {
        fn sync(&mut self) -> io::Result<()> { Err(denied()) }
        fn truncate(&mut self, _: u64) -> io::Result<()> { Err(denied()) }
        fn validate_identity(&self) -> io::Result<()> { Err(denied()) }
    }
}
...
pub fn open_private_dig(path: &Path, context: &OperationContext, mode: DigMode,
    expected: Option<DigBinding>) -> Result<DigJournal<PrivateDigFile>>
{
    if context.cancellation_requested {
        return Err(error(ErrorCode::CancellationRequested, "dig storage open cancelled"));
    }
    context.budget.validate()?;
    if context.budget.max_wall_millis > 60_000 {
        return Err(error(ErrorCode::BudgetExceeded, "dig storage deadline exceeds profile bound"));
    }
    #[cfg(all(target_os = "linux", any(target_arch = "x86_64", target_arch = "aarch64")))]
    ...
    #[cfg(not(all(target_os = "linux", any(target_arch = "x86_64", target_arch = "aarch64"))))]
    {
        ...
        Err(error(ErrorCode::CapabilityDenied, "dig private journals require Linux x86_64/aarch64"))
    }
}
