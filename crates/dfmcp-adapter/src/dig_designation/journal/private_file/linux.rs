...
fn denied() -> io::Error { io::Error::new(io::ErrorKind::PermissionDenied, "dig journal private custody changed") }
fn failure(_: io::Error) -> dfmcp_core::DfmcpError {
    error(ErrorCode::CorruptLedger, "dig journal private custody or I/O failed; no repair performed")
}
fn remaining(context: &OperationContext, deadline: Instant) -> Result<OperationContext> {
    let left = deadline.checked_duration_since(Instant::now())
        .ok_or_else(|| error(ErrorCode::BudgetExceeded, "dig storage open deadline exhausted"))?;
    let millis = left.as_millis() as u64;
    if millis == 0 { return Err(error(ErrorCode::BudgetExceeded, "dig storage open deadline exhausted")); }
...
fn directory_ok(metadata: &Metadata, uid: u32) -> bool {
    metadata.is_dir() && metadata.mode() & 0o7777 == 0o700 && (metadata.uid() == 0 || metadata.uid() == uid)
}
fn file_ok(metadata: &Metadata, owner: u32) -> bool {
    metadata.is_file() && metadata.mode() & 0o7777 == 0o600 && metadata.nlink() == 1 && metadata.uid() == owner
}
...
impl PrivateDigFile {
    fn writable(&self) -> io::Result<()> {
        if self.read_only { return Err(denied()); }
...
impl EffectJournalStorage for PrivateDigFile {
    ...
    fn validate_identity(&self) -> io::Result<()> {
        ...
        let named = fs::symlink_metadata(descriptor_leaf(&self.directory, self.path.file_name().ok_or_else(denied)?))?;
        ...
        if !directory_ok(&dir, self.directory_identity.2)
            ...
            || !file_ok(&opened, dir.uid()) || !file_ok(&named, dir.uid())
...
pub(super) fn open(path: &Path, context: &OperationContext, mode: DigMode,
    expected: Option<DigBinding>) -> Result<DigJournal<PrivateDigFile>>
{
    ...
    if let Some(binding) = &expected {
        ...
        matches!(grant.capability, Capability::Query | Capability::Admin) && !grant.is_limited()
            && grant.allows(Capability::Query, RiskTier::ReadOnly, context.anchor.tick,
                context.anchor.fortress_id, &[], grant.scope.map_area)
    }) {
        return Err(error(ErrorCode::CapabilityDenied, "dig storage open requires current Query authority"));
    }
    let raw = path.as_os_str().as_bytes();
    if !path.is_absolute() || raw.len() < 2 || raw.len() > 4096
        || raw[1..].split(|b| *b == b'/').any(|part| part.is_empty() || part == b"." || part == b".." || part.contains(&0))
    { return Err(error(ErrorCode::InvalidRequest, "dig journal path must be normalized absolute")); }
    ...
    if !directory_ok(&dir, uid) { return Err(failure(denied())); }
    let leaf = descriptor_leaf(&directory, path.file_name().ok_or_else(|| failure(denied()))?);
    let before = match fs::symlink_metadata(&leaf) {
        Ok(metadata) => {
            if !file_ok(&metadata, dir.uid()) { return Err(failure(denied())); }
            Some(metadata)
        }
    ...
    if created && (mode != DigMode::Control || expected.is_none()) {
        return Err(error(ErrorCode::CapabilityDenied, "dig recovery cannot create a journal"));
    }
    remaining(context, deadline)?;
    let mut options = OpenOptions::new();
    options.read(true).write(mode != DigMode::Offline)
    ...
    file.try_lock().map_err(|_| error(ErrorCode::Conflict, "dig journal already owned or exclusive locking unavailable"))?;
    let opened = file.metadata().map_err(failure)?;
    if before.as_ref().is_some_and(|m| (m.dev(), m.ino()) != (opened.dev(), opened.ino())) {
        return Err(failure(denied()));
    }
    let storage = PrivateDigFile { file, directory, path: path.to_owned(), read_only: mode == DigMode::Offline,
        extent: opened.len(), file_identity: (opened.dev(), opened.ino(), opened.uid()),
        directory_identity: (dir.dev(), dir.ino(), dir.uid()) };
    storage.validate_identity().map_err(failure)?;
    let nonce = if created {
        let mut nonce = [0; 32];
        File::open("/dev/urandom").and_then(|mut source| source.read_exact(&mut nonce)).map_err(failure)?;
        Some(nonce)
    } else { None };
    ...
    DigJournal::open(storage, &remaining(context, deadline)?, mode, expected, nonce)
}
