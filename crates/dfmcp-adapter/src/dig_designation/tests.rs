...
pub(super) fn fixture(name: &str) -> Result<Vec<u8>> {
    let raw = match name {
        ...
        "prepared" => include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/../../tests/native/dig_designation/vectors/prepared.hex")),
        "designated" => include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/../../tests/native/dig_designation/vectors/designated.hex")),
        "cancelled" => include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/../../tests/native/dig_designation/vectors/cancelled.hex")),
    ...
    (0..text.len()).step_by(2).map(|i| u8::from_str_radix(&text[i..i + 2], 16)
        .map_err(|_| error(ErrorCode::InvalidRequest, "invalid native fixture hex"))).collect()
}
pub(super) fn plan() -> Result<DigPlan> {
    DigPlan::new("dig-001", false, DigObservation::decode(&fixture("observation")?)?)
}
...
fn native_vectors_bind_exact_plan_token_and_expected_post_state() -> Result<()> {
    ...
    for (name, phase) in [("prepared", DigPhase::Prepared), ("designated", DigPhase::Designated), ("cancelled", DigPhase::Refused)] {
        ...
        assert_eq!(effect.designated_count(), (phase == DigPhase::Designated).then_some(4));
    }
    assert_eq!(DigEffect::decode(&fixture("cancelled")?, &plan)?.reason(), DigReason::CancelledBeforeDispatch);
    Ok(())
}
...
fn every_effect_bit_is_covered_by_plan_or_receipt_evidence() -> Result<()> {
    ...
    for offset in 0..original.len() {
        for bit in 0..8 {
            ...
            assert!(DigEffect::decode(&mutated, &plan).is_err(), "uncovered effect byte {offset}, bit {bit}");
        }
    }
    Ok(())
}
...
fn rehashing_cannot_forge_requested_terrain_priority_or_scheduling() -> Result<()> {
    ...
    for offset in [8, 16, 24, 32, 52, 53, 85, 117, 135, 139, 140, 171, 206] {
        let mut raw = fixture("designated")?; raw[offset] ^= 1; rehash(&mut raw);
        assert!(DigEffect::decode(&raw, &plan).is_err(), "rehash bypass at {offset}");
    }
    for offset in [133, 134, 135] {
        let mut raw = fixture("prepared")?; raw[offset] = 3;
...
fn all_incomplete_captures_and_effects_fail_without_partial_publication() -> Result<()> {
    ...
    for n in 0..capture.len() { assert!(DigObservation::decode(&capture[..n]).is_err()); }
    let mut trailing = capture; trailing.push(0);
    assert!(DigObservation::decode(&trailing).is_err());
    for name in ["prepared", "designated", "cancelled"] {
        let bytes = fixture(name)?;
        for n in 0..bytes.len() { assert!(DigEffect::decode(&bytes[..n], &plan()?).is_err()); }
...
fn hidden_neighbor_is_redacted_and_requires_explicit_policy() -> Result<()> {
    ...
    let mut leaking = fixture("observation")?; leaking[offset] = 1;
...
fn hidden_or_missing_targets_never_become_eligible() -> Result<()> {
    let before = plan()?.before().clone();
    let index = before.tiles().position(|(p, _)| before.region.target(p)).ok_or_else(|| error(ErrorCode::InvalidRequest, "missing fixture target"))?;
    let offset = before.offsets[index];
    for presence in [0, 1] {
        let mut bytes = fixture("observation")?; bytes.splice(offset..offset + 32, [presence]);
        let changed = DigObservation::decode(&bytes)?;
        assert!(changed.blockers(true).contains(&DigBlocker::UnobservedTarget));
...
fn full_halo_hazards_and_target_configuration_are_load_bearing() -> Result<()> {
    let before = plan()?.before().clone();
    for mask in [1, 2, 4, 8] {
        let mut bytes = fixture("observation")?; bytes[before.offsets[0] + 30] = mask;
        let changed = DigObservation::decode(&bytes)?;
        assert!(changed.blockers(true).contains(&DigBlocker::KnownHazard));
    }
    let index = before.tiles().position(|(p, _)| before.region.target(p)).ok_or_else(|| error(ErrorCode::InvalidRequest, "missing fixture target"))?;
    for (flag, blocker) in [(0, DigBlocker::NotNaturalWall), (3, DigBlocker::ExistingDesignation),
        (5, DigBlocker::OccupiedOrJob), (9, DigBlocker::OccupiedOrJob)] {
        let mut bytes = fixture("observation")?; bytes[before.offsets[index] + 31] = flag;
        assert!(DigObservation::decode(&bytes)?.blockers(false).contains(&blocker));
    }
    Ok(())
}
...
fn geometry_and_scope_cover_complete_shared_blocks() -> Result<()> {
    for width in 1..=8 { for height in 1..=8 {
        ...
        assert_eq!((0..r.halo_count()).filter(|i| r.target(r.position(*i))).count(), (width * height) as usize);
    } }
    ...
    for [x, y, z, w, h] in [[0, 1, 1, 1, 1], [1, 0, 1, 1, 1], [1, 1, 0, 1, 1], [1, 1, 32767, 1, 1],
        [32767, 1, 1, 1, 1], [1, u32::MAX, 1, 1, 1], [1, 1, 1, 0, 1], [1, 1, 1, 9, 1], [1, 1, 1, 1, u32::MAX]] {
...
fn retained_plan_is_canonical_and_never_imports_a_receipt_as_authority() -> Result<()> {
    ...
    for n in 0..bytes.len() { assert!(DigPlan::decode(&bytes[..n]).is_err()); }
    let mut extra = bytes.clone(); extra.push(0);
    assert!(DigPlan::decode(&extra).is_err());
    let mut invalid_bool = bytes; invalid_bool[8 + 2 + plan.key().len()] = 2;
...
fn invalid_clocks_reserved_bytes_and_sequence_exhaustion_fail_closed() -> Result<()> {
    for (offset, value) in [(8, 0), (8, u64::MAX), (16, u64::MAX), (24, MAX_NATIVE_TICK + 1)] {
        let mut raw = fixture("observation")?; raw[offset..offset + 8].copy_from_slice(&value.to_be_bytes());
        assert!(DigObservation::decode(&raw).is_err());
    }
    let mut raw = fixture("observation")?; raw[16..24].copy_from_slice(&(u64::MAX - 1).to_be_bytes());
    let last = DigObservation::decode(&raw)?;
    assert!(DigPlan::new("exhausted", false, last).is_err());
    for offset in [68, plan()?.before().offsets[0], plan()?.before().offsets[0] + 31] {
        let mut raw = fixture("observation")?; raw[offset] = 255;
        assert!(DigObservation::decode(&raw).is_err());
    }
    Ok(())
}
