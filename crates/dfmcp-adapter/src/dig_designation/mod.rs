...
impl DigRegion {
    pub fn new(x: u32, y: u32, z: u32, width: u32, height: u32) -> Result<Self> {
            ...
            || y > 32767 - height
        {
            return Err(error(
                ...
                "dig requires a bounded single-level rectangle and complete halo",
            ));
        }
        Ok(Self {
            x,
    ...
    pub fn halo_count(self) -> usize {
        ((self.width + 2) * (self.height + 2) * 3) as usize
    }
    ...
    pub fn write_area(self) -> MapCuboid {
        MapCuboid {
            ...
            max: MapCoord::new(
                (((self.x + self.width - 1) >> 4) * 16 + 15) as i32,
                (((self.y + self.height - 1) >> 4) * 16 + 15) as i32,
                self.z as i32,
            ),
        }
    }
    ...
    fn position(self, index: usize) -> [u32; 3] {
        ...
        [
            ...
            self.y - 1 + ((index % layer) / width) as u32,
            self.z - 1 + (index / layer) as u32,
        ]
    }
    ...
    fn target_block(self, [x, y, z]: [u32; 3]) -> bool {
        z == self.z
            && ((self.x >> 4)..=((self.x + self.width - 1) >> 4)).contains(&(x >> 4))
...
impl VisibleTile {
    ...
    pub fn designation_other(&self) -> u32 {
        self.designation_other
    }
    ...
    pub fn block_other(&self) -> u32 {
        self.block_other
    }
    pub fn temperatures(&self) -> [u16; 2] {
        self.temperatures
    }
    pub fn dig(&self) -> u8 {
        self.dig
    }
    pub fn hazards(&self) -> u8 {
        self.hazards
    }
...
impl DigObservation {
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        require(
            generation > 0
                ...
                && site <= i32::MAX as u32,
            "invalid dig source or clock",
        )?;
        let dimensions = [r.u32()?, r.u32()?, r.u32()?];
        let region = DigRegion::decode(&mut r)?;
        require(
            dimensions[0] > region.x + region.width
                && dimensions[0] <= 32768
                && dimensions[1] > region.y + region.height
                && dimensions[1] <= 32768
                && dimensions[2] > region.z + 1
                && dimensions[2] <= 32768,
            "dig halo is outside the map",
        )?;
        let paused = r.boolean()?;
        let folder = text(&mut r, 512)?;
        let count = usize::from(u16::from_be_bytes(r.array()?));
        require(
            count == region.halo_count() && count <= MAX_CELLS,
            "incomplete dig halo",
        )?;
            ...
            tiles.push(match r.byte()? {
                ...
                2 => {
                    let tile = VisibleTile {
                        tiletype: r.u32()?,
                        designation_other: r.u32()?,
                        occupancy: r.u32()?,
                        priority: r.u32()?,
                        cooldown: r.u32()?,
                        block_other: r.u32()?,
                        temperatures: [
                            u16::from_be_bytes(r.array()?),
                            u16::from_be_bytes(r.array()?),
                        ],
                        dig: r.byte()?,
                        hazards: r.byte()?,
    ...
    pub fn tiles(&self) -> impl Iterator<Item = ([u32; 3], &DigTile)> {
        self.tiles
            .iter()
            .enumerate()
            .map(|(i, tile)| (self.region.position(i), tile))
    }
    ...
    pub fn blockers(&self, allow_hidden_neighbors: bool) -> BTreeSet<DigBlocker> {
        ...
        if !self.paused {
            out.insert(DigBlocker::Unpaused);
        }
        if self.sequence >= u64::MAX - 1 {
            out.insert(DigBlocker::SequenceExhausted);
        }
        for (position, tile) in self.tiles() {
            match tile {
                DigTile::Missing => {
                    out.insert(DigBlocker::MissingContext);
                }
                DigTile::Hidden if !allow_hidden_neighbors => {
                    out.insert(DigBlocker::HiddenContext);
                }
                DigTile::Visible(v) if v.hazards != 0 => {
                    out.insert(DigBlocker::KnownHazard);
                }
                _ => {}
            }
            ...
            match tile {
                DigTile::Visible(v) => {
                    if v.flags & 1 == 0 {
                        out.insert(DigBlocker::NotNaturalWall);
                    }
                    if v.dig != 0 || v.flags & 2 != 0 {
                        out.insert(DigBlocker::ExistingDesignation);
                    }
                    if v.flags & 12 != 0 {
                        out.insert(DigBlocker::OccupiedOrJob);
                    }
                }
                _ => {
                    out.insert(DigBlocker::UnobservedTarget);
                }
            }
        }
        out
    ...
    fn expected_witness(&self) -> Result<Digest32> {
        require(
            ...
            "dig intervention sequence exhausted",
        )?;
        let mut out = self.bytes.clone();
        out[16..24].copy_from_slice(&(self.sequence + 1).to_be_bytes());
        for (i, tile) in self.tiles.iter().enumerate() {
            if !matches!(tile, DigTile::Visible(_)) {
                continue;
            }
...
impl DigPlan {
    pub fn new(key: &str, allow_hidden: bool, before: DigObservation) -> Result<Self> {
        validate_key(key)?;
        require(
            before.blockers(allow_hidden).is_empty(),
            "dig capture is not eligible for designation",
        )?;
    ...
    pub fn allow_hidden_neighbors(&self) -> bool {
        self.allow_hidden
    }
    pub fn before(&self) -> &DigObservation {
        &self.before
    }
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        require(
            ...
            "retained dig observation exceeds its bound",
        )?;
...
impl DigPhase {
    pub fn terminal(self) -> bool {
        matches!(self, Self::Designated | Self::Refused)
    }
...
impl DigEffect {
    pub fn decode(bytes: &[u8], plan: &DigPlan) -> Result<Self> {
        require(
            (207..=MAX_EFFECT_BYTES).contains(&bytes.len()),
            "invalid dig effect extent",
        )?;
        ...
        for n in [
            ...
            prefix.extend_from_slice(&n.to_be_bytes());
        }
        ...
        let mut r = Reader(bytes);
        require(
            r.take(prefix.len())? == prefix.as_slice(),
            "dig effect differs from retained plan",
        )?;
        let phase = match r.byte()? {
            0 => DigPhase::Prepared,
            1 => DigPhase::Unknown,
            2 => DigPhase::Designated,
            4 => DigPhase::Refused,
            _ => {
                return Err(error(
                    ErrorCode::AdapterRejected,
                    "unknown dig effect phase",
                ));
            }
        };
        let reason = match r.byte()? {
            0 => DigReason::None,
            1 => DigReason::Stale,
            2 => DigReason::CancelledBeforeDispatch,
            _ => {
                return Err(error(
                    ErrorCode::AdapterRejected,
                    "unknown dig effect reason",
                ));
            }
        };
        let known = r.boolean()?;
        ...
        require(
            key == plan.key
                && known == (phase == DigPhase::Designated)
                && (phase == DigPhase::Refused) == (reason != DigReason::None),
            "contradictory dig effect shape",
        )?;
        if known {
            require(
                count == plan.before.region.target_count()
                    && after == plan.before.expected_witness()?,
                "designation lacks exact terrain, priority and scheduling readback",
            )?;
        } else {
            require(
                count == 0 && after == Digest32::from_bytes([0; 32]),
                "absent dig readback is not zero",
            )?;
        }
        ...
        let expected = if phase.terminal() {
        ...
        Ok(Self {
            ...
            phase,
            reason,
            after: known.then_some(after),
            count: known.then_some(count),
            receipt: phase.terminal().then_some(receipt),
        })
    }
    pub fn canonical_bytes(&self) -> &[u8] {
        &self.bytes
    }
    pub fn phase(&self) -> DigPhase {
        self.phase
    }
    pub fn reason(&self) -> DigReason {
        self.reason
    }
    pub fn after_witness(&self) -> Option<Digest32> {
        self.after
    }
    pub fn designated_count(&self) -> Option<u32> {
        self.count
    }
