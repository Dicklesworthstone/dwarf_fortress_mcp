...
fn exhausted() -> dfmcp_core::DfmcpError { error(ErrorCode::BudgetExceeded, "dig journal work or retention allowance exhausted") }
fn corrupt(_: io::Error) -> dfmcp_core::DfmcpError { error(ErrorCode::CorruptLedger, "dig journal I/O or custody failed; reopen without repair") }
fn unknown(key: &str) -> dfmcp_core::DfmcpError {
    error(ErrorCode::EffectIndeterminate, "dig outcome requires exact-record recovery; never retry dispatch")
        .with_detail("key", key).retryable(false)
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum DigMode { Control, Recover, Offline }
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum DigStage { Observe, Prepare, Commit, Query, Cancel }
...
impl Allowance {
    fn new(context: &OperationContext) -> Result<Self> {
        if context.cancellation_requested { return Err(error(ErrorCode::CancellationRequested, "dig operation cancelled")); }
    ...
    fn current(&self) -> Result<OperationContext> {
        let left = self.deadline.checked_duration_since(Instant::now()).ok_or_else(exhausted)?;
...
pub struct DigCursor { session: SessionId, id: Digest32, head: Digest32, offset: usize, limit: usize }
#[derive(Clone, Debug)]
pub struct DigPage {
...
impl<S: EffectJournalStorage> DigJournal<S> {
    ...
    pub fn open(mut storage: S, context: &OperationContext, mode: DigMode,
        expected: Option<DigBinding>, nonce: Option<[u8; 32]>) -> Result<Self>
    {
        ...
        if length > MAX_JOURNAL_BYTES as u64 { return Err(exhausted()); }
        ...
        if let Some(nonce) = nonce {
            ...
            let binding = expected.as_ref().ok_or_else(|| error(ErrorCode::InvalidRequest, "dig journal creation requires an exact binding"))?;
            binding.authorize(context, context.anchor.tick.get())?;
            let binding = binding.encode();
            check(binding.len() <= MAX_BINDING_BYTES)?;
            raw.extend_from_slice(MAGIC); raw.extend_from_slice(&(binding.len() as u16).to_be_bytes());
            raw.extend_from_slice(&binding); raw.extend_from_slice(&nonce);
            let id = hash(b"dfmcp-dig-journal/1", &raw); raw.extend_from_slice(id.as_bytes());
        ...
        let decoded = (|| -> Result<_> {
            ...
            if let Some(expected) = &expected { check(expected == &binding)?; }
            ...
            while !r.0.is_empty() {
                ...
                check(digest == hash(b"dfmcp-dig-journal-frame/1", &raw[start..end]) && r.take(8)? == END)?;
                Self::check_next(&records, &next)?;
                records.insert(next.plan.key().to_owned(), next);
                events += 1; head = digest;
            }
            Ok((binding, id, head, events, records))
        })();
        let (binding, id, head, events, records) = decoded?;
        let mut journal = Self { storage, raw, binding, id, head, events, records,
            mode, session: context.session_id, fresh_key: None, fenced: false };
    ...
    pub fn mode(&self) -> DigMode { self.mode }
    ...
    fn check_next(records: &BTreeMap<String, DigRecord>, next: &DigRecord) -> Result<()> {
        if !records.contains_key(next.plan.key()) {
            ...
            check(records.values().all(|r| {
                let sequence = r.plan.before().sequence() + u64::from(r.effect.as_ref().is_some_and(|e| e.phase() == DigPhase::Designated));
                next.plan.before().tick() >= r.plan.before().tick() && next.plan.before().sequence() >= sequence
            }))?;
        }
        transition(records.get(next.plan.key()), next)
    }
    fn access(&self, context: &OperationContext) -> Result<()> {
        if self.fenced { return Err(error(ErrorCode::CorruptLedger, "dig journal fenced; reopen for verified recovery")); }
        if context.session_id != self.session { return Err(error(ErrorCode::CapabilityDenied, "dig journal belongs to another session")); }
        let tick = self.records.values().map(|r| r.plan.before().tick()).max().map_or(context.anchor.tick.get(), |t| t.max(context.anchor.tick.get()));
    ...
    fn online(&self, context: &OperationContext, control: bool) -> Result<()> {
        ...
        if self.mode == DigMode::Offline || (control && self.mode != DigMode::Control) {
            return Err(error(ErrorCode::CapabilityDenied, "fixed dig recovery mode denies this native operation"));
        }
        Ok(())
    }
    ...
    fn verify(&mut self, budget: &mut Allowance) -> Result<()> {
        ...
        let result = (|| -> io::Result<()> {
            ...
            if self.storage.seek(SeekFrom::End(0))? != self.raw.len() as u64 { return Err(io::Error::other("dig journal extent changed")); }
            ...
            for chunk in self.raw.chunks(buffer.len()) {
                ...
                if &buffer[..chunk.len()] != chunk { return Err(io::Error::other("dig journal bytes changed")); }
            }
            self.storage.validate_identity()
        })();
        if result.is_err() { self.fenced = true; self.fresh_key = None; }
    ...
    fn capacity(&self, next: &DigRecord) -> Result<()> {
        ...
        if self.events + 1 + extra as u64 > MAX_EVENTS
            || self.raw.len() + MAX_FRAME_BYTES * (1 + extra) > MAX_JOURNAL_BYTES { return Err(exhausted()); }
        Ok(())
    }
    fn retain(&mut self, next: DigRecord, budget: &mut Allowance) -> Result<DigRecord> {
        self.access(&budget.current()?)?;
        if self.mode == DigMode::Offline { return Err(error(ErrorCode::CapabilityDenied, "offline dig journal is read-only")); }
        ...
        frame.extend_from_slice(&(self.events + 1).to_be_bytes()); frame.extend_from_slice(self.head.as_bytes());
        frame.extend_from_slice(&body);
        let head = hash(b"dfmcp-dig-journal-frame/1", &frame);
        frame.extend_from_slice(head.as_bytes()); frame.extend_from_slice(END);
        self.verify(budget)?;
        ...
        let written = self.storage.write_all(&frame).and_then(|_| self.storage.flush()).and_then(|_| self.storage.sync());
        if written.is_err() { self.fenced = true; self.fresh_key = None; }
    ...
    fn known(&self, key: &str, digest: Digest32) -> Result<DigRecord> {
        validate_key(key)?;
        let record = self.records.get(key).ok_or_else(|| error(ErrorCode::InvalidRequest, "dig key absent from journal"))?;
        if record.plan.digest() != digest { return Err(error(ErrorCode::Conflict, "dig key and plan digest disagree")); }
        Ok(record.clone())
    }
    pub fn get(&mut self, key: &str, digest: Digest32, context: &OperationContext) -> Result<DigRecord> {
        let mut budget = Allowance::new(context)?;
        self.verify(&mut budget)?;
        budget.charge(MAX_BODY_BYTES)?;
        let record = self.known(key, digest)?;
        if (context.budget.max_entities as usize) < record.plan.before().region().halo_count() { return Err(exhausted()); }
        Ok(record)
    }
    pub fn list(&mut self, context: &OperationContext, limit: usize, cursor: Option<&DigCursor>) -> Result<DigPage> {
        if !(1..=8).contains(&limit) || limit > context.budget.max_entities as usize { return Err(exhausted()); }
        let mut budget = Allowance::new(context)?;
        self.verify(&mut budget)?;
        let offset = if let Some(cursor) = cursor {
            if cursor.session != self.session || cursor.id != self.id || cursor.head != self.head
                || cursor.limit != limit || cursor.offset >= self.records.len() {
                return Err(error(ErrorCode::StaleAnchor, "dig continuation does not match this session and exact journal head"));
            }
            cursor.offset
        } else { 0 };
        budget.charge(limit * 512)?;
        let records = self.records.values().skip(offset).take(limit).map(|r| DigSummary {
            key: r.plan.key().to_owned(), plan_digest: r.plan.digest(), state: r.state,
            native_phase: r.effect.as_ref().map(DigEffect::phase), receipt: r.effect.as_ref().and_then(DigEffect::receipt),
            dispatchable: r.state == DigState::Prepared && self.fresh_key.as_deref() == Some(r.plan.key()),
        }).collect::<Vec<_>>();
        let end = offset + records.len();
        let continuation = (end < self.records.len()).then_some(DigCursor {
            session: self.session, id: self.id, head: self.head, offset: end, limit,
        });
        budget.current()?;
        Ok(DigPage { journal_id: self.id, head: self.head, total_records: self.records.len(),
            unsettled_records: self.records.values().filter(|r| !r.state.terminal()).count(), records, continuation })
    }
    fn edge<N: DigSource, G: DigGuard>(&mut self, source: &N, guard: &mut G,
        stage: DigStage, plan: &DigPlan, budget: &mut Allowance) -> Result<OperationContext>
    {
        self.verify(budget)?;
        self.binding.source(source)?; self.binding.capture(plan.before())?;
        if (budget.context.budget.max_entities as usize) < plan.before().region().halo_count() { return Err(exhausted()); }
        let context = budget.rpc()?;
        authorize(&context, self.binding.fortress_id(), plan.before().tick(), plan.before().region(),
            stage == DigStage::Observe, matches!(stage, DigStage::Prepare | DigStage::Commit | DigStage::Cancel), stage == DigStage::Prepare)?;
        ...
        let mut context = context; context.budget.max_wall_millis = left;
        self.binding.source(source)?;
        Ok(context)
    }
    fn fresh<N: DigSource, G: DigGuard>(&mut self, source: &mut N, guard: &mut G,
    ...
    fn accept(&mut self, mut record: DigRecord, effect: DigEffect, budget: &mut Allowance,
        fresh_preparation: bool) -> Result<DigRecord>
    {
        let effect = DigEffect::decode(effect.canonical_bytes(), &record.plan)?;
        record.state = if effect.phase().terminal() { DigState::Terminal }
            else if fresh_preparation && effect.phase() == DigPhase::Prepared { DigState::Prepared }
            else if record.state == DigState::CancelRequested { DigState::CancelRequested }
            else { DigState::Tracking };
        record.effect = Some(effect);
        self.retain(record, budget)
    }
    pub fn prepare<N: DigSource, G: DigGuard>(&mut self, source: &mut N, plan: &DigPlan,
        context: &OperationContext, guard: &mut G) -> Result<DigRecord>
    {
        ...
        if let Some(old) = self.records.get(plan.key()) {
            if old.plan != *plan { return Err(error(ErrorCode::Conflict, "dig key already binds a different plan")); }
            return Ok(old.clone());
        }
        if self.records.values().any(|r| !r.state.terminal()) { return Err(unknown(plan.key())); }
        let next = DigRecord { plan: plan.clone(), state: DigState::Intent, effect: None };
        self.capacity(&next)?;
        ...
        let result = (|| -> Result<DigRecord> {
            ...
            if fresh { self.fresh_key = Some(plan.key().to_owned()); }
            Ok(record)
        })();
    ...
    pub fn commit<N: DigSource, G: DigGuard>(&mut self, source: &mut N, key: &str, confirmation: Digest32,
        context: &OperationContext, guard: &mut G) -> Result<DigRecord>
    {
        self.online(context, true)?;
        let mut budget = self.operation(context)?;
        let mut record = self.known(key, confirmation)?;
        authorize(context, self.binding.fortress_id(), record.plan.before().tick(), record.plan.before().region(), true, true, false)?;
        if record.state.terminal() { return Ok(record); }
        if record.state != DigState::Prepared || self.fresh_key.as_deref() != Some(key) { return Err(unknown(key)); }
    ...
    pub fn reconcile<N: DigSource, G: DigGuard>(&mut self, source: &mut N, key: &str, digest: Digest32,
        context: &OperationContext, guard: &mut G) -> Result<DigRecord>
    {
        let mut budget = self.operation(context)?;
        let record = self.known(key, digest)?;
    ...
    pub fn cancel<N: DigSource, G: DigGuard>(&mut self, source: &mut N, key: &str, digest: Digest32,
        context: &OperationContext, guard: &mut G) -> Result<DigRecord>
    {
        let mut budget = self.operation(context)?;
