...
impl<'a> Reader<'a> {
    ...
    fn array<const N: usize>(&mut self) -> Result<[u8; N]> {
        self.take(N)?
            .try_into()
            .map_err(|_| invalid("invalid job-control field width"))
    }
    ...
    fn text(&mut self, maximum: usize, empty: bool) -> Result<String> {
        ...
        require(
            length <= maximum && (empty || length > 0),
            "invalid job-control text bound",
        )?;
    ...
    fn finish(self) -> Result<()> {
        require(
            ...
            "trailing job-control record bytes",
        )
    }
}
...
fn identity(generation: u64, sequence: u64, tick: u64, job: u32) -> Result<()> {
    ...
    require(
        tick <= MAX_NATIVE_TICK && job <= i32::MAX as u32,
...
impl JobObservation {
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        require(
            ...
            "job observation exceeds 1024 bytes",
        )?;
        let mut r = Reader { remaining: bytes };
        require(
            r.take(8)? == b"DFMJS019",
            "not a job-control/1.9 observation",
        )?;
        ...
        require(
            job < next_job && next_job <= i32::MAX as u32 && site <= i32::MAX as u32,
        ...
        let flags = r.byte()?;
        require(
            job_type >= 0 && holder >= -1 && holder_type >= -1 && worker >= -1 && timer >= -1,
            "invalid native job scalar",
        )?;
        require(
            attachments <= 65_536 && filters <= 4096 && flags & !63 == 0,
            "invalid job-control counts or reserved flags",
        )?;
        require(
            if holder == -1 {
                holder_type == -1 && flags & (8 | 32) == 0
            } else {
                holder_type >= 0
            },
            "inconsistent native job holder evidence",
        )?;
    ...
    pub fn job_type(&self) -> i32 {
        self.job_type
    }
    pub fn holder_id(&self) -> Option<u32> {
        u32::try_from(self.holder).ok()
    }
    pub fn holder_type(&self) -> Option<u32> {
        u32::try_from(self.holder_type).ok()
    }
    pub fn worker_id(&self) -> Option<u32> {
        u32::try_from(self.worker).ok()
    }
    pub fn position(&self) -> [i32; 3] {
        self.position
    }
    pub fn completion_timer(&self) -> i32 {
        self.timer
    }
    ...
    pub fn holder_complete(&self) -> bool {
        self.flags & 8 != 0
    }
    pub fn supported(&self) -> bool {
        self.flags & 16 != 0
    }
    pub fn production_holder(&self) -> bool {
        self.flags & 32 != 0
    }
    ...
    pub fn eligible(&self) -> bool {
        self.paused()
            && self.holder >= 0
            && self.production_holder()
            && self.holder_complete()
            && self.supported()
            && self.worker == -1
            && self.timer == -1
    }
    ...
    fn expected_after_witness(&self, suspended: bool) -> Result<Digest32> {
        let sequence = self
            ...
            .checked_add(1)
            .filter(|s| *s != u64::MAX)
            .ok_or_else(|| invalid("job sequence exhausted"))?;
...
impl SuspensionEffect {
    pub fn decode(bytes: &[u8], plan: &SuspensionPlan) -> Result<Self> {
        ...
        let state = match r.byte()? {
            0 => SuspensionState::Prepared,
            1 => SuspensionState::Unknown,
            2 => SuspensionState::Applied,
            3 => SuspensionState::NotApplied,
        ...
        require(
            after_known == observed,
            "job outcome and observation presence disagree",
        )?;
        if observed {
            require(
                after_tick == tick && after_witness == before.expected_after_witness(after)?,
                "job readback is not the exact controlled observation transition",
            )?;
            require(
                (after == desired) == (state == SuspensionState::Applied),
                "job effect outcome contradicts readback",
            )?;
        } else {
            require(
                !after && after_tick == 0 && after_witness == Digest32::ZERO,
        ...
        if state.terminal() {
            ...
            require(
                ...
                "job suspension receipt mismatch",
            )?;
        } else {
            require(
                receipt == Digest32::ZERO,
                "nonterminal job effect carries a receipt",
            )?;
        }
        Ok(Self {
            bytes: bytes.to_vec(),
            state,
            after: observed.then_some(after),
            after_witness: observed.then_some(after_witness),
    ...
    pub fn state(&self) -> SuspensionState {
        self.state
    }
    pub fn observed_suspended(&self) -> Option<bool> {
        self.after
    }
