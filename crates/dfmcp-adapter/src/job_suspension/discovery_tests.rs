...
use dfmcp_core::{
    Capability, CapabilityGrant, CapabilityScope, ErrorCode, GameTick, ObservationCursor,
...
impl EffectJournalStorage for Memory {
    ...
    fn validate_identity(&self) -> io::Result<()> {
        if self.invalid.get() {
            Err(io::Error::other("custody lost"))
        } else {
            Ok(())
        }
    }
...
fn observation() -> Result<JobObservation> {
    ...
    for offset in (0..raw.len()).step_by(2) {
        bytes.push(
            u8::from_str_radix(&raw[offset..offset + 2], 16).map_err(|_| corrupt("fixture hex"))?,
        );
    }
...
fn context() -> Result<OperationContext> {
    ...
    Ok(OperationContext {
        ...
        anchor: StateAnchor {
            fortress_id: super::super::job_fortress_id(&o),
...
fn record(key: &str) -> Result<DurableJobRecord> {
    ...
    Ok(DurableJobRecord {
        ...
        effect,
        manifest,
        state: DurableJobState::Prepared,
    })
}
fn populated() -> Result<(JobControlJournal<Memory>, OperationContext)> {
    ...
    for key in ["c", "a", "b"] {
        j.append(record(key)?, &mut Budget::new(&c)?)?;
    }
    Ok((j, c))
}
...
fn complete_keyset_pages_include_cancelled_terminal_work() -> Result<()> {
    ...
    let s = j.summary(&c)?;
    assert_eq!(
        (s.records, s.prepared, s.unresolved, s.terminal),
        (3, 2, 0, 1)
    );
...
fn restart_preserves_page_generation_and_all_records() -> Result<()> {
    ...
    assert!(restored.summary(&c)?.read_only);
    Ok(())
}
#[test]
fn changed_head_unknown_key_and_page_budget_fail_closed() -> Result<()> {
    ...
    assert!(
        matches!(j.records_page(head, Some("a"), 1, &c), Err(e) if e.code == ErrorCode::StaleAnchor)
    );
    let head = j.summary(&c)?.head;
    assert!(j.records_page(head, Some("absent"), 1, &c).is_err());
    for limit in [0, 65] {
        assert!(j.records_page(head, None, limit, &c).is_err());
    }
    let mut tiny = c.clone();
    tiny.budget.max_bytes = 1;
    assert!(j.records_page(head, None, 1, &tiny).is_err());
    tiny = c.clone();
    tiny.budget.max_entities = 1;
    assert!(j.records_page(head, None, 2, &tiny).is_err());
    Ok(())
}
#[test]
fn cached_discovery_and_terminal_replays_recheck_custody() -> Result<()> {
    let (mut j, c) = populated()?;
    let head = j.summary(&c)?.head;
    let plan = record("a")?.plan;
    j.cancel_before_dispatch(&plan, &c)?;
    j.storage.invalid.set(true);
    assert!(j.summary(&c).is_err());
    assert!(j.lookup("a", &c).is_err());
    assert!(j.unresolved(&c).is_err());
    assert!(j.records_page(head, None, 1, &c).is_err());
    assert!(j.cancel_before_dispatch(&plan, &c).is_err());
    Ok(())
}
struct Native {
...
impl JobSuspensionSource for Native {
    ...
    fn prepare(&mut self, _: &SuspensionPlan, _: Duration) -> Result<SuspensionEffect> {
        self.writes += 1;
        Err(corrupt("unexpected prepare"))
    }
    fn commit(
        &mut self,
        _: &SuspensionPlan,
        ...
        self.writes += 1;
        Err(corrupt("unexpected commit"))
    }
    fn query(&mut self, _: &SuspensionPlan, _: Duration) -> Result<Option<SuspensionEffect>> {
        self.queries += 1;
        Ok(None)
    }
}
...
fn query_only_reopen_can_reconcile_but_cannot_dispatch_or_create() -> Result<()> {
    ...
    assert!(j.prepare(&mut n, &r.plan, &c).is_err());
    assert!(j.commit(&mut n, &r.plan, &c).is_err());
    assert!(j.cancel_before_dispatch(&r.plan, &c).is_err());
    assert_eq!(
        j.reconcile(&mut n, &r.plan, &c)?.state(),
