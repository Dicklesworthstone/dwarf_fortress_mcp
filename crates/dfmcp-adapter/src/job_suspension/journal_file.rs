...
use super::{EffectJournalStorage, JobControlJournal, OperationContext, Result, authorize, conflict, fail, io_error};
use dfmcp_core::ErrorCode;

pub struct PrivateJobJournalFile {
    file: File, path: PathBuf, read_only: bool,
    #[cfg(unix)]
    identity: (u64, u64, u32, u64, u64),
}
impl PrivateJobJournalFile {
    fn writable(&self) -> io::Result<()> {
        if self.read_only { return Err(io::Error::new(io::ErrorKind::PermissionDenied, "read-only job recovery file")); }
        Ok(())
    }
}
impl Read for PrivateJobJournalFile { fn read(&mut self, out: &mut [u8]) -> io::Result<usize> { self.file.read(out) } }
impl Seek for PrivateJobJournalFile { fn seek(&mut self, from: SeekFrom) -> io::Result<u64> { self.file.seek(from) } }
impl Write for PrivateJobJournalFile {
    fn write(&mut self, data: &[u8]) -> io::Result<usize> { self.writable()?; self.file.write(data) }
...
impl EffectJournalStorage for PrivateJobJournalFile {
    ...
    fn truncate(&mut self, _: u64) -> io::Result<()> {
        Err(io::Error::new(io::ErrorKind::PermissionDenied, "job journals never truncate or erase recovery evidence"))
    }
    fn validate_identity(&self) -> io::Result<()> {
        #[cfg(unix)] {
            use std::os::unix::fs::MetadataExt;
            let denied = || io::Error::new(io::ErrorKind::PermissionDenied, "job journal custody changed");
        ...
        #[cfg(not(unix))] { Err(io::Error::new(io::ErrorKind::Unsupported, "private job journals require Unix custody checks")) }
    }
}
...
fn open_storage(path: &Path, context: &OperationContext, read_only: bool, allow_create: bool) -> Result<(PrivateJobJournalFile, bool)> {
    ...
    #[cfg(unix)] {
        ...
        if parent.canonicalize().map_err(io_error)? != parent { return Err(denied()); }
        ...
        file.try_lock().map_err(|_| conflict("job journal already has an owner or cannot be exclusively locked"))?;
        let opened = file.metadata().map_err(io_error)?;
        if before.as_ref().is_some_and(|m| (m.dev(), m.ino()) != (opened.dev(), opened.ino())) { return Err(denied()); }
        let storage = PrivateJobJournalFile { file, path: path.to_owned(), read_only,
            identity: (opened.dev(), opened.ino(), opened.uid(), dir.dev(), dir.ino()) };
        storage.validate_identity().map_err(io_error)?;
        if created { File::open(parent).and_then(|dir| dir.sync_all()).map_err(io_error)?; }
        Ok((storage, created))
    }
    #[cfg(not(unix))] {
        let _ = (path, read_only);
        Err(fail(ErrorCode::CapabilityDenied, "private job journals are currently Unix-only"))
    }
}
