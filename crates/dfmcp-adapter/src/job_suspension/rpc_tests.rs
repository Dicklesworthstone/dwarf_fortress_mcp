...
fn prepared() -> Result<Vec<u8>> {
    let mut e = hex(EFFECT)?;
    e[117..192].fill(0);
    Ok(e)
}
fn frame(payload: &[u8]) -> Vec<u8> {
    let mut v = header(-1, payload.len() as i32).to_vec();
...
fn envelope(generation: u64) -> Vec<u8> {
    ...
    number(&mut r, 4, 1);
    number(&mut r, 5, 9);
    number(&mut r, 6, generation);
    bytes(&mut r, 7, b"df");
    bytes(&mut r, 8, b"dfhack");
    r
}
fn observation_reply() -> Result<Vec<u8>> {
    let mut r = envelope(7);
    bytes(&mut r, 9, &hex(OBSERVATION)?);
    Ok(r)
}
fn effect_reply(effect: &[u8], replayed: Option<bool>) -> Vec<u8> {
    let mut r = envelope(7);
    bytes(&mut r, 10, effect);
    if let Some(replayed) = replayed {
        number(&mut r, 11, u64::from(replayed));
    }
    r
}
...
impl Read for Script {
    fn read(&mut self, out: &mut [u8]) -> io::Result<usize> {
        ...
        self.input.read(&mut out[..length])
    }
}
impl Write for Script {
    fn write(&mut self, data: &[u8]) -> io::Result<usize> {
        let length = data.len().min(self.fragment);
        self.output.extend_from_slice(&data[..length]);
        Ok(length)
    }
...
fn script(input: Vec<u8>, fragment: usize) -> Script {
    Script {
...
fn fragmented_full_lifecycle_binds_only_fixed_methods_and_exact_request_fields() -> Result<()> {
    let p = plan()?;
    let pre = prepared()?;
    let applied = hex(EFFECT)?;
    let mut c = client(&[
        observation_reply()?,
        effect_reply(&pre, Some(false)),
        effect_reply(&applied, None),
        effect_reply(&applied, None),
    ])?;
    assert_eq!(c.read_job(3, TIMEOUT)?, *p.observation());
    let prepared = c.prepare(&p, TIMEOUT)?;
    assert!(!prepared.replayed());
    assert_eq!(
        c.commit_prepared(&p, prepared.effect(), TIMEOUT)?.state(),
        SuspensionState::Applied
    );
    assert_eq!(
        c.query(&p, TIMEOUT)?.map(|v| v.state()),
        Some(SuspensionState::Applied)
    );
    assert!(!c.poisoned());
    assert_eq!(c.stream.begins, 5);
    let calls = sent(&c.stream)?;
    assert_eq!(calls.len(), 10);
    for (index, name) in METHODS.iter().enumerate() {
        assert_eq!(calls[index].0, 0);
        let m = Message::parse(&calls[index].1, 4)?;
        assert_eq!(m.bytes(1, 128)?, name.as_bytes());
        assert_eq!(m.bytes(4, 128)?, PLUGIN.as_bytes());
        assert_eq!(m.bytes(2, 128)?, REQUEST_TYPE.as_bytes());
        assert_eq!(m.bytes(3, 128)?, REPLY_TYPE.as_bytes());
    }
    for (index, fields) in [
        (5, vec![1, 2, 3, 4]),
        (6, vec![1, 2, 3, 4, 6]),
        (7, vec![1, 2, 3, 4, 5, 6, 7, 8, 9]),
        (8, vec![1, 2, 3, 4, 5, 9, 10]),
        (9, vec![1, 2, 3, 4, 5, 9]),
    ] {
        let m = Message::parse(&calls[index].1, 10)?;
        assert_eq!(m.0.keys().copied().collect::<Vec<_>>(), fields);
        assert_eq!(m.number(4)?, 9);
        assert_eq!(m.bytes(1, 256)?, &[b't'; 32]);
    }
    assert_eq!(calls[8].0, 5);
    assert_eq!(calls[9].0, 6);
    Ok(())
}
...
fn missing_native_record_and_unknown_effect_never_become_negative_success() -> Result<()> {
    ...
    assert_eq!(effect.state(), SuspensionState::Unknown);
    assert!(effect.receipt().is_none());
    let before = c.stream.output.len();
    assert!(c.commit_prepared(&p, &effect, TIMEOUT).is_err());
    assert_eq!(c.stream.output.len(), before);
    assert!(!c.poisoned());
    Ok(())
}
...
fn local_validation_errors_do_not_write_or_poison_the_connection() -> Result<()> {
    ...
    let mut raw = hex(OBSERVATION)?;
    raw[8..16].copy_from_slice(&8u64.to_be_bytes());
    let other = SuspensionPlan::new(JobObservation::decode(&raw)?, "job-001", true)?;
    assert!(c.prepare(&other, TIMEOUT).is_err());
    assert!(c.query(&other, TIMEOUT).is_err());
    assert_eq!(c.stream.output.len(), before);
    assert!(!c.poisoned());
    assert!(
        JobControlRpcClient::connect(
...
fn reserved_and_aliasing_method_bindings_are_refused() {
    for ids in [[1, 3, 4, 5, 6], [2, 3, 3, 5, 6], [2, 3, 4, 5, 32768]] {
        assert!(
            JobControlRpcClient::negotiate(
                script(startup(ids), 2),
                vec![b't'; 32],
                vec![b'n'; 16],
                TIMEOUT
            )
...
fn malformed_method_shapes_nonce_and_generation_fence_even_after_complete_frames() -> Result<()> {
    let mut duplicate = observation_reply()?;
    number(&mut duplicate, 4, 1);
    let mut extra = observation_reply()?;
    number(&mut extra, 11, 0);
    let mut changed_generation = envelope(8);
    bytes(&mut changed_generation, 9, &hex(OBSERVATION)?);
    let mut wrong_job = hex(OBSERVATION)?;
    wrong_job[35] = 4;
    let mut wrong_job_reply = envelope(7);
    bytes(&mut wrong_job_reply, 9, &wrong_job);
    ...
    for reply in [
        ...
        let mut c = client(&[reply])?;
        assert!(c.read_job(3, TIMEOUT).is_err());
        assert!(c.poisoned());
        let before = c.stream.output.len();
        assert!(c.read_job(3, TIMEOUT).is_err());
        assert_eq!(c.stream.output.len(), before);
...
fn prepare_replay_and_commit_states_must_match_the_native_contract() -> Result<()> {
    ...
    let replay = c.prepare(&p, TIMEOUT)?;
    assert!(replay.replayed());
    assert_eq!(replay.effect().state(), SuspensionState::Applied);
    for response in [
        effect_reply(&applied, Some(false)),
        effect_reply(&pre, None),
    ] {
        let mut c = client(&[response])?;
        assert!(c.prepare(&p, TIMEOUT).is_err());
        assert!(c.poisoned());
    }
    let mut c = client(&[effect_reply(&pre, None)])?;
    assert!(
        c.commit_prepared(&p, &SuspensionEffect::decode(&pre, &p)?, TIMEOUT)
            .is_err()
    );
    assert!(c.poisoned());
    let mut tampered = applied;
    tampered[191] ^= 1;
...
fn native_error_envelope_never_asserts_an_effect_did_not_run() -> Result<()> {
    ...
    number(&mut rejected, 1, 0);
    number(&mut rejected, 2, 5);
    bytes(&mut rejected, 3, &[b'n'; 16]);
    number(&mut rejected, 4, 1);
    number(&mut rejected, 5, 9);
    number(&mut rejected, 6, 0);
    bytes(&mut rejected, 7, b"");
    bytes(&mut rejected, 8, b"");
    let p = plan()?;
    let pre = SuspensionEffect::decode(&prepared()?, &p)?;
    let mut c = client(&[rejected])?;
    let result = c.commit_prepared(&p, &pre, TIMEOUT);
    assert!(matches!(result, Err(e) if e.code == ErrorCode::AdapterFailure));
    assert!(c.poisoned());
    Ok(())
}
...
fn framing_and_protobuf_budgets_reject_before_unbounded_allocation() -> Result<()> {
    ...
    for h in [
        header(-1, MAX_RPC as i32 + 1),
        header(-3, MAX_NOTIFICATION as i32 + 1),
        header(-1, -1),
        header(3, 0),
    ] {
        let mut input = startup([2, 3, 4, 5, 6]);
        input.extend_from_slice(&h);
        let mut c = JobControlRpcClient::negotiate(
            script(input, 3),
            vec![b't'; 32],
            vec![b'n'; 16],
            TIMEOUT,
    ...
    let mut c =
        JobControlRpcClient::negotiate(script(input, 3), vec![b't'; 32], vec![b'n'; 16], TIMEOUT)?;
    assert!(c.read_job(3, TIMEOUT).is_err());
    assert!(c.poisoned());
    Ok(())
}
