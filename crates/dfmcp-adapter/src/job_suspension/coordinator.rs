...
use super::{
    JobObservation, Reader, SuspensionEffect, SuspensionPlan, SuspensionState, put_text,
    validate_key,
};
...
fn io_error(_: io::Error) -> DfmcpError {
    corrupt("job journal I/O or custody failed; reopen without redispatch")
}
...
fn exhausted() -> DfmcpError {
    fail(
        ...
        "job coordination exceeds its explicit work or retention budget",
    )
}
...
pub fn job_fortress_id(observation: &JobObservation) -> FortressId {
    ...
    value.extend_from_slice(observation.world_folder().as_bytes());
    value.push(0);
    value.extend_from_slice(&observation.site_id().to_be_bytes());
    let hash = Digest32::of_bytes(&value);
    let mut bytes = [0; 8];
    ...
    fn new(context: &OperationContext) -> Result<Self> {
        ...
        let until = Instant::now()
            .checked_add(Duration::from_millis(
                context.budget.max_wall_millis.min(60_000),
            ))
...
fn authorize(context: &OperationContext, fortress: FortressId, write: bool) -> Result<()> {
    if context.anchor.fortress_id != fortress {
        return Err(fail(
            ...
            "job journal belongs to another fortress",
        ));
    }
...
fn valid_manifest(manifest: &JobControlManifest, plan: &SuspensionPlan) -> Result<()> {
            ...
            .any(|s| s.is_empty() || s.len() > 128 || s.contains('\0'))
    {
        return Err(fail(
            ...
            "job plan and native manifest disagree",
        ));
    }
    Ok(())
}
...
impl DurableJobState {
    pub fn terminal(self) -> bool {
        matches!(
            self,
            Self::Applied | Self::NotApplied | Self::Refused | Self::CancelledBeforeDispatch
        )
    }
    pub fn reconciliation_required(self) -> bool {
        matches!(self, Self::DispatchStarted | Self::Indeterminate)
    }
    fn from_effect(effect: &SuspensionEffect) -> Self {
        match effect.state() {
            SuspensionState::Prepared => Self::Prepared,
            SuspensionState::Unknown => Self::Indeterminate,
            SuspensionState::Applied => Self::Applied,
            SuspensionState::NotApplied => Self::NotApplied,
            SuspensionState::Refused => Self::Refused,
        }
    }
}
...
impl DurableJobRecord {
    pub fn plan(&self) -> &SuspensionPlan {
        &self.plan
    }
    ...
    pub fn state(&self) -> DurableJobState {
        self.state
    }
}
...
impl<S: JobControlStream> JobSuspensionSource for JobControlRpcClient<S> {
    fn manifest(&self) -> &JobControlManifest {
        JobControlRpcClient::manifest(self)
    }
    fn fence(&mut self) {
        JobControlRpcClient::fence(self);
    }
    fn prepare(&mut self, plan: &SuspensionPlan, timeout: Duration) -> Result<SuspensionEffect> {
...
pub struct JobControlJournal<S> {
    ...
    length: u64,
    transitions: u64,
    records: BTreeMap<String, DurableJobRecord>,
    fenced: bool,
    read_only: bool,
}
impl<S: EffectJournalStorage> JobControlJournal<S> {
    ...
    fn open_inner(
        ...
        authorize(context, fortress, !read_only)?;
        ...
        if length < HEADER_BYTES as u64 {
            return Err(corrupt(
                "job journal header is incomplete; bytes left unchanged",
            ));
        }
        storage.seek(SeekFrom::Start(0)).map_err(io_error)?;
        ...
        if r.take(8)? != MAGIC {
            return Err(corrupt("not a job-control journal"));
        }
        let id = r.digest()?;
        let named_fortress = r.u64()?;
        let head = r.digest()?;
        r.finish()?;
        if named_fortress != fortress.get() || head != Digest32::of_bytes(&header[..48]) {
            return Err(corrupt("job journal header identity or checksum failed"));
        }
        let mut out = Self {
            storage,
            fortress,
        ...
        while out.length < length {
            budget.remaining()?;
            if out.transitions >= MAX_TRANSITIONS || length - out.length < FRAME_PREFIX as u64 {
                return Err(corrupt("incomplete or excessive job journal transitions"));
            }
            let mut prefix = [0; FRAME_PREFIX];
            out.storage.read_exact(&mut prefix).map_err(io_error)?;
            let mut r = Reader { remaining: &prefix };
            if r.take(8)? != FRAME {
                return Err(corrupt("job journal frame marker failed"));
            }
            let count = r.u32()? as usize;
            let number = r.u64()?;
            let previous = r.digest()?;
            if count > MAX_BODY || number != out.transitions + 1 || previous != out.head {
                return Err(corrupt("job journal frame ordering or body bound failed"));
            }
            let frame_length = FRAME_PREFIX + count + 40;
            if frame_length as u64 > length - out.length {
                return Err(corrupt(
                    "job journal tail is incomplete; no automatic truncation",
                ));
            }
            let mut body = vec![0; count];
            out.storage.read_exact(&mut body).map_err(io_error)?;
            let mut trailer = [0; 40];
            out.storage.read_exact(&mut trailer).map_err(io_error)?;
            let mut r = Reader {
                remaining: &trailer,
            };
            let digest = r.digest()?;
            if r.take(8)? != FOOTER || digest != frame_hash(id, &prefix, &body) {
                return Err(corrupt("job journal checksum or commit footer failed"));
            }
            let record = decode_record(&body)?;
            out.validate_transition(&record)?;
            out.records.insert(record.plan.key().to_owned(), record);
            if out.records.len() > context.budget.max_entities as usize {
                return Err(exhausted());
            }
            out.head = digest;
            out.transitions = number;
            out.length += frame_length as u64;
        }
        out.storage.validate_identity().map_err(io_error)?;
        budget.remaining()?;
        Ok(out)
    }
    ...
    pub fn lookup(
        ...
        key: &str,
        context: &OperationContext,
    ) -> Result<Option<&DurableJobRecord>> {
    ...
    pub fn unresolved(&self, context: &OperationContext) -> Result<Vec<DurableJobRecord>> {
        ...
        for record in self.records.values().filter(|r| !r.state.terminal()) {
            if out.len() >= context.budget.max_entities as usize {
                return Err(exhausted());
            }
    ...
    fn writable(&self, context: &OperationContext, production: bool) -> Result<()> {
        ...
        if self.read_only {
            return Err(fail(
                ErrorCode::CapabilityDenied,
                "recovery-only job journal cannot be written",
            ));
        }
        if self.fenced {
            return Err(corrupt("job journal fenced; reopen without redispatch"));
        }
        self.storage.validate_identity().map_err(io_error)
    }
    fn plan_context(&self, plan: &SuspensionPlan, context: &OperationContext) -> Result<()> {
        if job_fortress_id(plan.observation()) != self.fortress
            || context.anchor.tick.get() != plan.observation().tick()
        {
            return Err(fail(
                ErrorCode::StaleAnchor,
                "job preparation requires authority at the selected fortress and observed tick",
            ));
        }
        Ok(())
    }
    fn reserve(&self, transitions: u64) -> Result<()> {
        if self.transitions + transitions > MAX_TRANSITIONS
            || self.length + transitions * MAX_FRAME as u64 > MAX_LEDGER
        {
            return Err(exhausted());
        }
        Ok(())
    ...
    fn matching(&self, plan: &SuspensionPlan) -> Result<DurableJobRecord> {
        let record = self
            .records
            .get(plan.key())
            .ok_or_else(|| conflict("unknown durable job preparation"))?;
        if &record.plan != plan {
            return Err(conflict(
                "job idempotency key was already bound to another plan",
            ));
        }
        Ok(record.clone())
    }
    fn validate_transition(&self, next: &DurableJobRecord) -> Result<()> {
        ...
        if job_fortress_id(next.plan.observation()) != self.fortress {
            return Err(corrupt("job frame crosses fortress lineage"));
        }
        let native = SuspensionEffect::decode(next.effect.canonical_bytes(), &next.plan)?;
        let shape = match next.state {
            DurableJobState::Prepared
            | DurableJobState::DispatchStarted
            | DurableJobState::CancelledBeforeDispatch => {
                native.state() == SuspensionState::Prepared
            }
            DurableJobState::Indeterminate => matches!(
                native.state(),
                SuspensionState::Prepared | SuspensionState::Unknown
            ),
            _ => DurableJobState::from_effect(&native) == next.state,
        };
        if !shape {
            return Err(corrupt("durable job state contradicts native evidence"));
        }
        let Some(old) = self.records.get(next.plan.key()) else {
            ...
            if matches!(
                next.state,
                DurableJobState::DispatchStarted | DurableJobState::CancelledBeforeDispatch
            ) {
                return Err(corrupt("job dispatch or cancellation has no preparation"));
            }
            return Ok(());
        };
        if old.plan != next.plan || old.manifest != next.manifest {
            return Err(conflict("job key identity or software changed"));
        }
        if old.state.terminal() {
            return if old == next {
                Ok(())
            } else {
                Err(corrupt("terminal job evidence was rewritten"))
            };
        }
        if old.effect.state() == SuspensionState::Unknown && next.effect != old.effect {
            return Err(corrupt("native Unknown record is immutable"));
        }
        let allowed = match old.state {
            DurableJobState::Prepared => next.state != DurableJobState::Prepared || old == next,
            DurableJobState::DispatchStarted | DurableJobState::Indeterminate => matches!(
                next.state,
                DurableJobState::Indeterminate
                    | DurableJobState::Applied
                    | DurableJobState::NotApplied
                    | DurableJobState::Refused
            ),
            _ => false,
        };
        if !allowed {
            return Err(corrupt("job journal transition could re-enable dispatch"));
        }
        Ok(())
    }
    fn append(
        ...
        if self.records.get(record.plan.key()) == Some(&record) {
            return Ok(record);
        }
        self.reserve(1)?;
        ...
        prefix.extend_from_slice(&(self.transitions + 1).to_be_bytes());
        ...
        let result = (|| {
            self.storage.validate_identity().map_err(io_error)?;
            if self.storage.seek(SeekFrom::End(0)).map_err(io_error)? != self.length {
                return Err(corrupt("job journal extent changed outside its owner"));
            }
            self.storage.write_all(&bytes).map_err(io_error)?;
            self.storage.sync().map_err(io_error)?;
            self.storage.validate_identity().map_err(io_error)?;
            Ok(())
        })();
        if result.is_err() {
        ...
        self.records
            .insert(record.plan.key().to_owned(), record.clone());
        // Never publish a success when the caller's acknowledgement budget expired.
        budget.remaining()?;
        Ok(record)
    }
    pub fn prepare<N: JobSuspensionSource>(
        &mut self,
        source: &mut N,
        plan: &SuspensionPlan,
        context: &OperationContext,
        ...
        valid_manifest(&manifest, plan)?;
        if self.records.contains_key(plan.key()) {
            let old = self.matching(plan)?;
            if old.manifest != manifest {
                return Err(fail(ErrorCode::StaleAnchor, "job source software changed"));
            }
            return Ok(old);
        }
        if self.records.len() >= MAX_RECORDS {
        ...
        if budget.bytes < MAX_FRAME as u64 {
            return Err(exhausted());
        }
    ...
    pub fn commit<N: JobSuspensionSource>(
        &mut self,
        source: &mut N,
        plan: &SuspensionPlan,
        context: &OperationContext,
        ...
        if old.state != DurableJobState::Prepared {
            return Err(fail(
                ...
                "job dispatch already started; only query reconciliation is allowed",
            )
            .retryable(false));
        }
        self.plan_context(plan, context)?;
        if source.manifest() != &old.manifest {
            return Err(fail(
                ErrorCode::StaleAnchor,
                "job source incarnation or software changed",
            ));
        }
        self.reserve(2)?;
        let mut budget = Budget::new(context)?;
        budget.charge(RPC_BYTES)?;
        if budget.bytes < 2 * MAX_FRAME as u64 {
            return Err(exhausted());
        }
        let mut started = old.clone();
        ...
        let mut next = old;
        next.state = DurableJobState::Indeterminate;
        if let Ok(effect) = checked {
            if effect.state() == SuspensionState::Prepared {
                source.fence();
            } else {
                next.state = DurableJobState::from_effect(&effect);
                next.effect = effect;
            }
        }
    ...
    pub fn reconcile<N: JobSuspensionSource>(
        &mut self,
        source: &mut N,
        plan: &SuspensionPlan,
        context: &OperationContext,
                ...
                Ok(Some(effect)) => match checked_reply(source, &old.manifest, plan, Ok(effect)) {
                    Ok(effect) => {
                        if old.effect.state() == SuspensionState::Unknown && effect != old.effect {
                            source.fence();
                            return Err(corrupt("native Unknown outcome changed during recovery"));
                        }
                        if effect.state() != SuspensionState::Prepared {
                            next.state = DurableJobState::from_effect(&effect);
                            next.effect = effect;
    ...
    pub fn cancel_before_dispatch(
        ...
        if record.state == DurableJobState::CancelledBeforeDispatch {
            return Ok(record);
        }
        if record.state != DurableJobState::Prepared {
            return Err(conflict(
                "cannot cancel after job dispatch or native terminal evidence",
            ));
        }
...
fn checked_reply<N: JobSuspensionSource>(
    source: &mut N,
    manifest: &JobControlManifest,
    plan: &SuspensionPlan,
    reply: Result<SuspensionEffect>,
) -> Result<SuspensionEffect> {
    let result = (|| {
        if source.manifest() != manifest {
            return Err(fail(
                ErrorCode::StaleAnchor,
                "job source changed during request",
            ));
        }
        let effect = reply?;
        SuspensionEffect::decode(effect.canonical_bytes(), plan)
    })();
    if result.is_err() {
        source.fence();
    }
...
fn frame_hash(id: Digest32, prefix: &[u8], body: &[u8]) -> Digest32 {
    let mut data = b"dfmcp-job-journal-frame/1\0".to_vec();
...
fn blob<'a>(r: &mut Reader<'a>, maximum: usize) -> Result<&'a [u8]> {
    ...
    if size > maximum {
        return Err(corrupt("job journal blob exceeds its bound"));
    }
    r.take(size)
}
fn encode_record(record: &DurableJobRecord) -> Result<Vec<u8>> {
    let mut out = vec![record.state as u8];
    put_text(&mut out, record.plan.key());
    put_blob(&mut out, record.plan.observation().canonical_bytes());
    out.push(u8::from(record.plan.desired()));
    put_text(&mut out, &record.manifest.df_version);
    put_text(&mut out, &record.manifest.dfhack_version);
...
fn decode_record(bytes: &[u8]) -> Result<DurableJobRecord> {
    ...
    let state = match r.byte()? {
        1 => DurableJobState::Prepared,
        2 => DurableJobState::DispatchStarted,
        3 => DurableJobState::Indeterminate,
        4 => DurableJobState::Applied,
        5 => DurableJobState::NotApplied,
        6 => DurableJobState::Refused,
        7 => DurableJobState::CancelledBeforeDispatch,
    ...
    let manifest = JobControlManifest {
        generation: plan.observation().generation(),
        df_version: r.text(128, false)?,
        dfhack_version: r.text(128, false)?,
    };
    let effect = SuspensionEffect::decode(blob(&mut r, super::MAX_EFFECT_BYTES)?, &plan)?;
    r.finish()?;
    Ok(DurableJobRecord {
        plan,
        manifest,
