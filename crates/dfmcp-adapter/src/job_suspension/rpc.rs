...
const METHODS: [&str; 5] = ["Handshake", "ReadJob", "PrepareSuspension", "CommitSuspension", "QuerySuspension"];

fn error(code: ErrorCode, message: &str) -> DfmcpError { DfmcpError::new(code, message) }
fn malformed() -> DfmcpError { error(ErrorCode::AdapterRejected, "malformed job-control/1.9 RPC") }
fn io_error(_: io::Error) -> DfmcpError {
    error(ErrorCode::AdapterUnavailable, "job-control I/O failed; a dispatched effect may be indeterminate")
}
fn deadline(timeout: Duration) -> Result<Instant> {
    if !(Duration::from_millis(1)..=Duration::from_secs(60)).contains(&timeout) {
        return Err(error(ErrorCode::BudgetExceeded, "job-control timeout must be 1..60000 milliseconds"));
    }
    Instant::now().checked_add(timeout).ok_or_else(|| error(ErrorCode::BudgetExceeded, "job-control deadline overflow"))
}
fn remaining(until: Instant) -> Result<Duration> {
    until.checked_duration_since(Instant::now()).filter(|t| !t.is_zero())
        .ok_or_else(|| error(ErrorCode::BudgetExceeded, "job-control wall-time budget exhausted"))
}
fn credentials(token: &[u8], nonce: &[u8]) -> Result<()> {
    if !(32..=256).contains(&token.len()) || !(16..=64).contains(&nonce.len()) {
        return Err(error(ErrorCode::InvalidRequest, "invalid job-control credential or nonce length"));
    }
    Ok(())
}
...
impl<'a> Message<'a> {
    fn parse(data: &'a [u8], maximum_field: u32) -> Result<Self> {
        if data.len() > MAX_RPC { return Err(malformed()); }
        ...
        while offset < data.len() {
            ...
            let value = match key & 7 {
                ...
                2 => {
                    ...
                    let value = data.get(offset..end).ok_or_else(malformed)?; offset = end; Field::Bytes(value)
                }
    ...
    fn number(&self, field: u32) -> Result<u64> {
        match self.0.get(&field) { Some(Field::Number(n)) => Ok(*n), _ => Err(malformed()) }
    }
    ...
    fn bytes(&self, field: u32, maximum: usize) -> Result<&'a [u8]> {
        match self.0.get(&field) { Some(Field::Bytes(b)) if b.len() <= maximum => Ok(b), _ => Err(malformed()) }
    }
    fn text(&self, field: u32, empty: bool) -> Result<String> {
        let value = std::str::from_utf8(self.bytes(field, 128)?).map_err(|_| malformed())?;
        if value.contains('\0') || (!empty && value.is_empty()) { return Err(malformed()); }
        Ok(value.to_owned())
    }
}
fn header(method: i16, length: i32) -> [u8; 8] {
    let mut bytes = [0; 8]; bytes[..2].copy_from_slice(&method.to_le_bytes());
    bytes[4..].copy_from_slice(&length.to_le_bytes()); bytes
}
fn call<S: JobControlStream>(stream: &mut S, method: i16, request: &[u8]) -> Result<Vec<u8>> {
    if request.len() > MAX_RPC { return Err(malformed()); }
    ...
    loop {
        ...
        if id == -2 { return Err(error(ErrorCode::AdapterFailure, "DFHack rejected job-control RPC; outcome not established")); }
        ...
        if id == -3 {
            ...
            if notifications > MAX_NOTIFICATIONS || size > MAX_NOTIFICATION || notification_bytes > MAX_NOTIFICATION_TOTAL {
                return Err(error(ErrorCode::BudgetExceeded, "job-control text-notification budget exhausted"));
            }
        } else if size > MAX_RPC { return Err(error(ErrorCode::BudgetExceeded, "job-control reply exceeds 8 KiB")); }
...
pub struct JobControlManifest { pub generation: u64, pub df_version: String, pub dfhack_version: String }
#[derive(Clone, Copy, PartialEq, Eq)]
enum Method { Handshake = 0, Read = 1, Prepare = 2, Commit = 3, Query = 4 }
struct Reply { manifest: JobControlManifest, observation: Option<Vec<u8>>, effect: Option<Vec<u8>>, replayed: Option<bool> }
fn decode(data: &[u8], nonce: &[u8], method: Method) -> Result<Reply> {
    let m = Message::parse(data, 11)?;
    if m.number(4)? != 1 || m.number(5)? != 9 {
        return Err(error(ErrorCode::VersionMismatch, "job-control bridge must be exactly protocol 1.9"));
    }
    if m.bytes(3, 64)? != nonce { return Err(malformed()); }
    let accepted = m.boolean(1)?; let code = m.number(2)?;
    let generation = m.number(6)?; let df_version = m.text(7, !accepted)?; let dfhack_version = m.text(8, !accepted)?;
    if !accepted {
    ...
    if !valid { return Err(malformed()); }
    Ok(Reply { manifest: JobControlManifest { generation, df_version, dfhack_version },
...
pub struct PreparationReply { effect: SuspensionEffect, replayed: bool }
...
pub struct JobControlRpcClient<S> {
    stream: S, token: Vec<u8>, nonce: Vec<u8>, methods: [i16; 5], manifest: JobControlManifest, fenced: bool,
}
impl<S: JobControlStream> JobControlRpcClient<S> {
    ...
    fn negotiate_until(mut stream: S, token: Vec<u8>, nonce: Vec<u8>, until: Instant) -> Result<Self> {
        ...
        for (index, name) in METHODS.iter().enumerate() {
            ...
            if methods[..index].contains(&methods[index]) { return Err(malformed()); }
            remaining(until)?;
        }
        let request = Self::base_request(&token, &nonce);
        let reply = decode(&call(&mut stream, methods[0], &request)?, &nonce, Method::Handshake)?;
        remaining(until)?;
    ...
    fn base_request(token: &[u8], nonce: &[u8]) -> Vec<u8> {
        ...
        number(&mut out, 3, 1); number(&mut out, 4, 9); out
    }
    fn request(&self) -> Vec<u8> { Self::base_request(&self.token, &self.nonce) }
    ...
    fn check_plan(&self, plan: &SuspensionPlan) -> Result<()> {
        if plan.observation().generation() != self.manifest.generation {
            return Err(error(ErrorCode::StaleAnchor, "job plan belongs to another native incarnation"));
        }
        Ok(())
    }
    fn invoke<T>(&mut self, method: Method, request: Vec<u8>, timeout: Duration,
        project: impl FnOnce(Reply) -> Result<T>) -> Result<T>
    {
        if self.fenced { return Err(error(ErrorCode::AdapterUnavailable, "job-control connection is fenced; query recovery requires a new connection")); }
        ...
        let result = (|| {
            ...
            if reply.manifest != self.manifest {
                return Err(error(ErrorCode::StaleAnchor, "job-control incarnation or software changed; do not redispatch"));
            }
            let value = project(reply)?; remaining(until)?; Ok(value)
        })();
        if result.is_err() { self.fenced = true; }
        result
    }
    pub fn read_job(&mut self, job: u32, timeout: Duration) -> Result<JobObservation> {
        if job > i32::MAX as u32 { return Err(error(ErrorCode::InvalidRequest, "native job ID exceeds signed 32-bit range")); }
        let mut request = self.request(); number(&mut request, 6, u64::from(job));
        self.invoke(Method::Read, request, timeout, |reply| {
            ...
            if observation.job_id() != job || observation.generation() != reply.manifest.generation { return Err(malformed()); }
            Ok(observation)
        })
    }
    pub fn prepare(&mut self, plan: &SuspensionPlan, timeout: Duration) -> Result<PreparationReply> {
        self.check_plan(plan)?;
        let mut request = self.request(); bytes(&mut request, 5, plan.key().as_bytes());
        number(&mut request, 6, u64::from(plan.observation().job_id())); number(&mut request, 7, u64::from(plan.desired()));
        ...
        self.invoke(Method::Prepare, request, timeout, |reply| {
            ...
            if !replayed && effect.state() != SuspensionState::Prepared { return Err(malformed()); }
    ...
    pub fn commit_prepared(&mut self, plan: &SuspensionPlan, prepared: &SuspensionEffect,
        timeout: Duration) -> Result<SuspensionEffect>
    {
        self.check_plan(plan)?;
        let checked = SuspensionEffect::decode(prepared.canonical_bytes(), plan)?;
        if checked.state() != SuspensionState::Prepared {
            return Err(error(ErrorCode::Conflict, "only an exact native preparation may be dispatched; reconcile other states"));
        }
        let mut request = self.request(); bytes(&mut request, 5, plan.key().as_bytes());
        ...
        self.invoke(Method::Commit, request, timeout, |reply| {
            let effect = SuspensionEffect::decode(&reply.effect.ok_or_else(malformed)?, plan)?;
            if effect.state() == SuspensionState::Prepared { return Err(malformed()); }
            Ok(effect)
        })
    }
    /// `None` means no retained native record, NOT proof that dispatch never ran.
    pub fn query(&mut self, plan: &SuspensionPlan, timeout: Duration) -> Result<Option<SuspensionEffect>> {
        self.check_plan(plan)?;
        let mut request = self.request(); bytes(&mut request, 5, plan.key().as_bytes());
        bytes(&mut request, 9, plan.digest().as_bytes());
        self.invoke(Method::Query, request, timeout, |reply| {
            reply.effect.map(|bytes| SuspensionEffect::decode(&bytes, plan)).transpose()
        })
    }
}

pub struct DeadlineTcpStream { stream: TcpStream, until: Instant }
impl DeadlineTcpStream {
    fn timeout(&self) -> io::Result<Duration> {
        self.until.checked_duration_since(Instant::now()).filter(|t| !t.is_zero())
            .ok_or_else(|| io::Error::new(io::ErrorKind::TimedOut, "job-control deadline exhausted"))
    }
}
...
impl JobControlRpcClient<DeadlineTcpStream> {
    pub fn connect(endpoint: SocketAddr, token: Vec<u8>, nonce: Vec<u8>, timeout: Duration) -> Result<Self> {
        ...
        if !endpoint.ip().is_loopback() || endpoint.port() == 0 {
            return Err(error(ErrorCode::CapabilityDenied, "job-control endpoint must be numeric loopback with a nonzero port"));
        }
