...
fn hex(text: &str) -> Result<Vec<u8>> {
    ...
    text.as_bytes().chunks_exact(2).map(|pair| {
        let a = char::from(pair[0]).to_digit(16).ok_or_else(|| invalid("fixture hex"))?;
        let b = char::from(pair[1]).to_digit(16).ok_or_else(|| invalid("fixture hex"))?;
        Ok((a * 16 + b) as u8)
    }).collect()
}
...
fn record(state: SuspensionState) -> Result<Vec<u8>> {
    ...
    out[117] = state as u8;
    if !matches!(state, SuspensionState::Applied | SuspensionState::NotApplied) {
        out[118..192].fill(0);
    }
    ...
    if state.terminal() { proof(&mut out); }
    Ok(out)
}
...
fn native_observation_and_applied_receipt_are_byte_exact() -> Result<()> {
    ...
    assert_eq!(o.witness().to_hex(), "5c7a7bf9ecef8a6a8026082bba1c326f233d294e3930360b41d82c07b9abd613");
    assert_eq!(plan.digest().to_hex(), "4173fb3e7bcaf7e907f509a36be6a8d0868dcd93b159ef32c904d8623830acd6");
    assert_eq!(plan.prepare_token().as_slice(), &hex("d001cf4cf833637c7703c69c9772079d")?);
...
fn every_native_effect_byte_and_bit_is_bound_to_the_prepared_intent() -> Result<()> {
    ...
    for offset in 0..native.len() {
        for bit in 0..8 {
            ...
            assert!(SuspensionEffect::decode(&changed, &plan).is_err(), "offset {offset}, bit {bit}");
        }
    }
    Ok(())
}
...
fn truncations_trailing_data_and_oversized_frames_are_refused() -> Result<()> {
    ...
    for end in 0..observation.len() { assert!(JobObservation::decode(&observation[..end]).is_err()); }
    let effect = hex(EFFECT)?;
    for end in 0..effect.len() { assert!(SuspensionEffect::decode(&effect[..end], &plan).is_err()); }
    let mut longer = observation.clone(); longer.push(0);
    assert!(JobObservation::decode(&longer).is_err());
    let mut longer = effect.clone(); longer.push(0);
...
fn pending_unknown_refusal_and_negative_readback_remain_distinct() -> Result<()> {
    let plan = plan()?;
    for state in [SuspensionState::Prepared, SuspensionState::Unknown, SuspensionState::Applied,
        SuspensionState::NotApplied, SuspensionState::Refused]
    {
        ...
        assert_eq!(effect.observed_suspended(), match state {
            SuspensionState::Applied => Some(true), SuspensionState::NotApplied => Some(false), _ => None,
        });
    }
    for state in [SuspensionState::Prepared, SuspensionState::Unknown, SuspensionState::Refused] {
        let valid = record(state)?;
        for index in 118..160 {
            let mut corrupt = valid.clone(); corrupt[index] = 1;
            if state.terminal() { proof(&mut corrupt); }
...
fn a_recomputed_receipt_cannot_certify_unrelated_or_later_readback() -> Result<()> {
    ...
    for offset in [118, 119, 120, 127, 128, 159] {
        let mut corrupt = hex(EFFECT)?; corrupt[offset] ^= 1; proof(&mut corrupt);
        assert!(SuspensionEffect::decode(&corrupt, &plan).is_err(), "offset {offset}");
    }
...
fn keys_targets_worlds_and_control_desires_cannot_be_substituted() -> Result<()> {
    ...
    for offset in [15, 23, 31, 35, 39, 43, 47, 87, 105] {
        let mut changed = hex(OBSERVATION)?; changed[offset] ^= 1;
...
fn native_bounds_unknown_positions_and_utf8_are_preserved() -> Result<()> {
    for (offset, value) in [(8, 0), (8, u64::MAX), (16, u64::MAX), (24, MAX_NATIVE_TICK + 1)] {
        let mut bytes = hex(OBSERVATION)?; bytes[offset..offset + 8].copy_from_slice(&value.to_be_bytes());
        assert!(JobObservation::decode(&bytes).is_err());
    }
    for (offset, value) in [(36, 3u32), (36, u32::MAX), (40, u32::MAX),
        (44, u32::MAX), (48, u32::MAX - 1), (72, u32::MAX - 1), (76, 65_537), (80, 4097)]
    {
        let mut bytes = hex(OBSERVATION)?; bytes[offset..offset + 4].copy_from_slice(&value.to_be_bytes());
        assert!(JobObservation::decode(&bytes).is_err());
    }
    let mut bytes = hex(OBSERVATION)?; bytes[84] |= 64;
    assert!(JobObservation::decode(&bytes).is_err());
    for text in [vec![0], vec![0xff], vec![0xc0, 0x80], vec![0xed, 0xa0, 0x80]] {
...
fn ineligible_observations_are_readable_but_do_not_make_plans() -> Result<()> {
    for flag in [4, 8, 16, 32] {
        let mut bytes = hex(OBSERVATION)?; bytes[84] &= !flag;
        let value = JobObservation::decode(&bytes)?;
        assert!(!value.eligible());
        assert!(SuspensionPlan::new(value, "job-001", true).is_err());
    }
    for offset in [56, 72] {
        let mut bytes = hex(OBSERVATION)?; bytes[offset..offset + 4].copy_from_slice(&0i32.to_be_bytes());
        let value = JobObservation::decode(&bytes)?;
        assert!(!value.eligible());
        assert!(SuspensionPlan::new(value, "job-001", true).is_err());
    }
    let mut bytes = hex(OBSERVATION)?;
    bytes[48..56].fill(0xff); bytes[84] &= !(8 | 32);
    let value = JobObservation::decode(&bytes)?;
