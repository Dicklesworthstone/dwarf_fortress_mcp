...
use dfmcp_core::{
    Capability, CapabilityGrant, CapabilityScope, MapCoord, ObservationCursor, RequestId, RiskTier,
...
fn policy(
    book: &mut LeaseManager,
    mode: DigCheckpointPolicy,
    protected: Vec<MapCuboid>,
) -> Result<DigControlPolicy> {
    let p = plan()?;
    let c = context()?;
    ...
    let binding = DigBinding::new(
        "127.0.0.1:5000"
            .parse()
            .map_err(|_| error(ErrorCode::InvalidRequest, "endpoint"))?,
        DigManifest {
            generation: 7,
    ...
    DigControlPolicy::new(
        binding,
        Digest32::of_bytes(b"journal"),
        c.session_id,
        lease,
        protected,
        mode,
...
fn default_checkpoint_policy_refuses_without_inventing_evidence() -> Result<()> {
    ...
    let policy = policy(&mut book, DigCheckpointPolicy::default(), vec![])?;
...
fn protected_shared_block_outside_target_still_blocks_designation() -> Result<()> {
    let p = plan()?;
    assert_eq!(p.before().region().coordinates(), [15, 15, 2, 2, 2]);
    let protected = MapCuboid {
...
fn protected_scope_is_canonical_bounded_and_part_of_review() -> Result<()> {
    let a = MapCuboid {
        min: MapCoord::new(40, 40, 2),
        max: MapCoord::new(41, 41, 2),
    };
    let b = MapCuboid {
        min: MapCoord::new(50, 50, 2),
        max: MapCoord::new(51, 51, 2),
    };
    let mut book = LeaseManager::new();
    let one = policy(
        &mut book,
    ...
    let other = DigControlPolicy::new(
        one.binding.clone(),
        one.journal,
        one.session,
        one.lease,
    ...
    assert_ne!(
        one.review(&plan()?, &context()?, &book)?.seal(),
        other.review(&plan()?, &context()?, &book)?.seal()
    );
    Ok(())
}
#[test]
fn review_seal_binds_key_journal_session_policy_and_lease() -> Result<()> {
    let mut book = LeaseManager::new();
    let policy = policy(&mut book, DigCheckpointPolicy::DisposableFortress, vec![])?;
    let p = plan()?;
    let c = context()?;
    let review = policy.review(&p, &c, &book)?;
    let seal = review.seal();
    assert!(review.confirm(Digest32::ZERO).is_err());
    let confirmed = policy.review(&p, &c, &book)?.confirm(seal)?;
    let mut runtime = Runtime::default();
    PolicyDigGuard::new(&policy, &book, &mut runtime, Some(&confirmed)).check(
        DigStage::Commit,
        &p,
        &c,
    )?;
    ...
    assert!(
        PolicyDigGuard::new(&policy, &book, &mut runtime, Some(&confirmed))
            .check(DigStage::Commit, &other, &c)
            .is_err()
    );
    let mut changed = policy.clone();
    changed.journal = Digest32::of_bytes(b"different");
    let changed = DigControlPolicy::new(
        changed.binding,
        changed.journal,
        changed.session,
        changed.lease,
    ...
    assert!(
        PolicyDigGuard::new(&changed, &book, &mut runtime, Some(&confirmed))
            .check(DigStage::Commit, &p, &c)
            .is_err()
    );
    Ok(())
}
...
fn actual_lease_release_or_expiry_revokes_even_a_confirmed_review() -> Result<()> {
    ...
    let review = policy.review(&p, &c, &book)?;
    let seal = review.seal();
    let confirmed = review.confirm(seal)?;
    let mut runtime = Runtime::default();
    ...
    c = context()?;
    book.release_lease(policy.lease, c.session_id)?;
    assert!(
        PolicyDigGuard::new(&policy, &book, &mut runtime, Some(&confirmed))
            .check(DigStage::Commit, &p, &c)
            .is_err()
...
fn runtime_revocation_and_current_authority_are_rechecked() -> Result<()> {
    ...
    assert!(
        PolicyDigGuard::new(&policy, &book, &mut runtime, None)
            .check(DigStage::Commit, &p, &c)
            .is_err()
    );
    ...
    assert!(
        PolicyDigGuard::new(&policy, &book, &mut runtime, None)
            .check(DigStage::Prepare, &p, &c)
            .is_err()
    );
    runtime.revoked = false;
    c.grants.retain(|g| g.capability != Capability::Designate);
    assert!(
        PolicyDigGuard::new(&policy, &book, &mut runtime, None)
            .check(DigStage::Prepare, &p, &c)
            .is_err()
    ...
    c = context()?;
    c.session_id = SessionId::new(2);
    assert!(policy.evaluate(&p, &c, &book).is_err());
    c = context()?;
...
fn query_and_retirement_are_not_blocked_by_unavailable_checkpoint_or_lease() -> Result<()> {
    let mut book = LeaseManager::new();
    let policy = policy(&mut book, DigCheckpointPolicy::Required, vec![scope()])?;
    let c = context()?;
    book.release_lease(policy.lease, c.session_id)?;
    let mut runtime = Runtime::default();
    let mut guard = PolicyDigGuard::new(&policy, &book, &mut runtime, None);
    guard.check(DigStage::Query, &plan()?, &c)?;
    guard.check(DigStage::Cancel, &plan()?, &c)?;
