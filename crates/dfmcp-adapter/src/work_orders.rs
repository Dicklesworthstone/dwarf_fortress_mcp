...
impl<'a> Reader<'a> {
    fn take(&mut self, length: usize) -> Result<&'a [u8]> {
        let out = self
            .remaining
            .get(..length)
            .ok_or_else(|| invalid("truncated work-order record"))?;
    ...
    fn array<const N: usize>(&mut self) -> Result<[u8; N]> {
        self.take(N)?
            .try_into()
            .map_err(|_| invalid("invalid work-order field width"))
    }
    ...
    fn boolean(&mut self) -> Result<bool> {
        match self.byte()? {
            0 => Ok(false),
            1 => Ok(true),
            _ => Err(invalid("noncanonical work-order boolean")),
        }
    }
    ...
    fn text(&mut self, maximum: usize) -> Result<String> {
        ...
        require(
            ...
            "invalid work-order text bound",
        )?;
        let value = std::str::from_utf8(self.take(length)?)
            .map_err(|_| invalid("invalid work-order UTF-8"))?;
    ...
    fn finish(self) -> Result<()> {
        require(
            ...
            "trailing work-order record bytes",
        )
    }
}
...
impl WorkOrderRecipe {
    pub fn from_code(value: u32) -> Result<Self> {
        match value {
            1 => Ok(Self::WoodenBed),
            2 => Ok(Self::WoodenDoor),
            3 => Ok(Self::WoodenTable),
            4 => Ok(Self::WoodenChair),
            _ => Err(DfmcpError::new(
                ErrorCode::InvalidRequest,
                "unknown work-order recipe",
            )),
        }
    }
    pub fn as_str(self) -> &'static str {
        match self {
            Self::WoodenBed => "wooden_bed",
            Self::WoodenDoor => "wooden_door",
            Self::WoodenTable => "wooden_table",
            Self::WoodenChair => "wooden_chair",
        }
    }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct WorkOrderSpec {
...
impl WorkOrderSpec {
    pub fn new(recipe: WorkOrderRecipe, amount: u32) -> Result<Self> {
        if !(1..=MAX_AMOUNT).contains(&amount) {
            return Err(DfmcpError::new(
                ErrorCode::InvalidRequest,
                "work-order amount must be 1..100",
            ));
        }
...
impl WorkOrderObservation {
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        require(
            ...
            "work-order observation exceeds 17 KiB",
        )?;
        let mut r = Reader { remaining: bytes };
        require(
            r.take(8)? == b"DFMWO010",
            "not a work-orders/1.10 observation",
        )?;
        let generation = r.u64()?;
        let sequence = r.u64()?;
        let tick = r.u64()?;
        let next_order = r.u32()?;
        let site = r.u32()?;
        let paused = r.boolean()?;
        require(
            generation != 0 && generation != u64::MAX && sequence != u64::MAX,
            "invalid work-order incarnation or sequence",
        )?;
        require(
            tick <= MAX_NATIVE_TICK && next_order <= i32::MAX as u32 && site <= i32::MAX as u32,
        ...
        require(
            count <= MAX_ORDERS && count <= r.remaining.len() / 4,
            "invalid work-order membership bound",
        )?;
        let mut ids = Vec::with_capacity(count);
        for _ in 0..count {
            let id = r.u32()?;
            require(
                id < next_order && ids.last().is_none_or(|previous| *previous < id),
    ...
    pub fn order_ids(&self) -> &[u32] {
        &self.ids
    }
    pub fn eligible(&self) -> bool {
        self.paused
            && self.ids.len() < MAX_ORDERS
            && self.next_order < i32::MAX as u32
    ...
    fn expected_after_witness(&self) -> Result<Digest32> {
        require(
            self.eligible(),
            "ineligible work-order observation cannot form readback",
        )?;
        // Reconstruct from decoded fields rather than trusting a self-hashed reply.
        let mut data = b"DFMWO010".to_vec();
        for value in [self.generation, self.sequence + 1, self.tick] {
        ...
        for id in self
            ...
            .iter()
            .copied()
            .chain(std::iter::once(self.next_order))
        {
...
impl WorkOrderPlan {
    pub fn new(observation: WorkOrderObservation, key: &str, spec: WorkOrderSpec) -> Result<Self> {
        ...
        let mut data = b"dfmcp-work-order-plan/1\0".to_vec();
        spec.append(&mut data);
        data.extend_from_slice(observation.witness().as_bytes());
        let digest = Digest32::of_bytes(&data);
        let mut data = b"dfmcp-work-order-token/1\0".to_vec();
        data.extend_from_slice(&observation.generation.to_be_bytes());
        put_text(&mut data, key);
        data.extend_from_slice(digest.as_bytes());
        let token_hash = Digest32::of_bytes(&data);
        let mut token = [0; 16];
        token.copy_from_slice(&token_hash.as_bytes()[..16]);
        Ok(Self {
            observation,
            spec,
            key: key.to_owned(),
    ...
    pub fn observation(&self) -> &WorkOrderObservation {
        &self.observation
    }
    pub fn spec(&self) -> WorkOrderSpec {
        self.spec
    }
    pub fn key(&self) -> &str {
        &self.key
    ...
    pub fn prepare_token(&self) -> &[u8; 16] {
        &self.token
    }
    fn configuration_witness(&self) -> Digest32 {
        let mut data = b"DFMWOC10".to_vec();
        data.extend_from_slice(&self.observation.next_order.to_be_bytes());
        self.spec.append(&mut data);
...
impl WorkOrderState {
    pub fn terminal(self) -> bool {
        matches!(self, Self::Created | Self::Refused)
    }
}
...
pub struct WorkOrderEffect {
    ...
    configuration_witness: Option<Digest32>,
    receipt: Option<Digest32>,
}
impl WorkOrderEffect {
    pub fn decode(bytes: &[u8], plan: &WorkOrderPlan) -> Result<Self> {
        require(
            bytes.len() <= MAX_EFFECT_BYTES,
            "work-order effect exceeds 357 bytes",
        )?;
        let mut r = Reader { remaining: bytes };
        ...
        let state = match r.byte()? {
            0 => WorkOrderState::Prepared,
            1 => WorkOrderState::Unknown,
            2 => WorkOrderState::Created,
            4 => WorkOrderState::Refused,
            _ => {
                return Err(invalid(
                    "unknown work-order outcome; absence is not not-applied",
                ));
            }
        };
        ...
        let configuration_witness = r.digest()?;
        ...
        require(
            after_known == created,
            "work-order outcome and readback presence disagree",
        )?;
        if created {
            require(
                after_tick == tick
                    && after_witness == before.expected_after_witness()?
            ...
            require(
                after_tick == 0
                    && after_witness == Digest32::ZERO
                    && configuration_witness == Digest32::ZERO,
        ...
        if state.terminal() {
            ...
            proof.extend_from_slice(&[state as u8, u8::from(after_known)]);
            proof.extend_from_slice(&after_tick.to_be_bytes());
            proof.extend_from_slice(after_witness.as_bytes());
            proof.extend_from_slice(configuration_witness.as_bytes());
            require(
                receipt == Digest32::of_bytes(&proof),
                "work-order receipt checksum mismatch",
            )?;
        } else {
            require(
                receipt == Digest32::ZERO,
                "nonterminal work-order effect carries a receipt",
            )?;
        }
        Ok(Self {
            bytes: bytes.to_vec(),
            state,
            created_id: created.then_some(id),
            after_tick: created.then_some(after_tick),
            after_witness: created.then_some(after_witness),
            configuration_witness: created.then_some(configuration_witness),
    ...
    pub fn state(&self) -> WorkOrderState {
        self.state
    }
    pub fn created_order_id(&self) -> Option<u32> {
        self.created_id
    }
    ...
    pub fn configuration_witness(&self) -> Option<Digest32> {
        self.configuration_witness
    }
    pub fn receipt(&self) -> Option<Digest32> {
        self.receipt
    }
}
