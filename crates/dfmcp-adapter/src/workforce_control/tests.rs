...
pub(super) fn plan() -> Result<AssignmentPlan> {
    AssignmentPlan::new("assign", AssignmentSpec::new(0, true)?, capture()?)
}
pub(super) fn applied() -> Result<Vec<u8>> {
    unhex(include_str!(
...
pub(super) fn context() -> Result<OperationContext> {
    ...
    Ok(OperationContext {
        ...
        anchor: StateAnchor {
            ...
            tick: GameTick(c.tick()),
            state_hash: c.witness(),
        },
        budget: WorkBudget {
...
pub(super) fn record(
    ...
    out.extend_from_slice(
        after
            .map_or(Digest32::ZERO, WorkforceCapture::witness)
            .as_bytes(),
    );
    ...
    if let Some(after) = after {
        for unit in after.citizens() {
            unit.append(&mut out);
        }
    }
    let receipt = hash(b"dfmcp-workforce-receipt/1", &out);
...
fn real_cpp_capture_plan_and_applied_readback() -> Result<()> {
    let capture = capture()?;
    assert_eq!(capture.encode_values()?, capture.canonical_bytes());
    let plan = plan()?;
    assert_eq!(AssignmentPlan::decode(&plan.canonical_bytes())?, plan);
    ...
    assert_eq!(result.post_citizens()[1].labors(), [1, 0, 1]);
    Ok(())
}
#[test]
fn corrupt_or_incomplete_effects_never_prove_application() -> Result<()> {
    ...
    let raw = applied()?;
    for n in 0..raw.len() {
        assert!(AssignmentEffect::decode(&raw[..n], &plan).is_err());
    }
    for n in 0..raw.len() {
        let mut bad = raw.clone();
        bad[n] ^= 1;
        assert!(AssignmentEffect::decode(&bad, &plan).is_err());
    }
    let mut extra = raw;
    extra.push(0);
    assert!(AssignmentEffect::decode(&extra, &plan).is_err());
    Ok(())
}
#[test]
fn rehashed_false_poststate_and_unchanged_citizen_writes_are_rejected() -> Result<()> {
    ...
    after.citizens[0].labors[0] = 1;
    after.bytes = after.encode_values()?;
    assert!(
        AssignmentEffect::decode(
            &record(&plan, AssignmentPhase::Applied, Some(&after))?,
            &plan
        )
        .is_ok()
    );
    after.citizens[1].labors[2] = 0;
    after.bytes = after.encode_values()?;
    assert!(
        AssignmentEffect::decode(
            &record(&plan, AssignmentPhase::Applied, Some(&after))?,
...
fn removal_does_not_fabricate_exclusive_labor_denial() -> Result<()> {
    ...
    assert_eq!(changed, [5]);
    let value = AssignmentEffect::decode(
        &record(&plan, AssignmentPhase::Applied, Some(&after))?,
        &plan,
    )?;
    assert_eq!(value.post_citizens()[1].labors()[0], 1);
    Ok(())
}
#[test]
fn exact_preconditions_and_all_membership_subsets() -> Result<()> {
    for mask in 0..256u32 {
        ...
        for assigned in [false, true] {
            let planned =
                AssignmentPlan::new("subset", AssignmentSpec::new(0, assigned)?, c.clone());
    ...
    for field in 0..5 {
        ...
        match field {
            0 => c.paused = false,
            1 => c.automatic = false,
            2 => c.sequence = u64::MAX,
            3 => c.citizens[0].eligible = false,
            _ => c.details[0].selected_only = false,
        }
        c.bytes = c.encode_values()?;
...
fn unknown_and_terminal_evidence_cannot_be_promoted_or_rewritten() -> Result<()> {
    let p = plan()?;
    let unknown = AssignmentEffect::decode(&record(&p, AssignmentPhase::Unknown, None)?, &p)?;
    let applied = AssignmentEffect::decode(&applied()?, &p)?;
    assert!(applied.follows(&unknown).is_err());
    assert!(unknown.follows(&applied).is_err());
    unknown.follows(&unknown)?;
    applied.follows(&applied)?;
    Ok(())
}
#[test]
fn wrong_fortress_expired_limited_or_cancelled_grants_fail() -> Result<()> {
    let c = context()?;
    rpc::authorize(&c, c.anchor.fortress_id, 100, true)?;
    for mode in 0..4 {
        let mut c = c.clone();
        match mode {
