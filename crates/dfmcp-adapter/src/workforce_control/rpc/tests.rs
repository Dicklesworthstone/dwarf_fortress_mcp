...
impl WorkforceStream for Script {
...
fn reply(capture: Option<&[u8]>, effect: Option<&[u8]>) -> Vec<u8> {
    ...
    number(&mut out, 11, 0);
    number(&mut out, 12, u64::from(effect.is_some()));
    out
}
fn script(replies: Vec<Vec<u8>>, alias: bool) -> Script {
    ...
    for n in 2..8 {
        let mut b = Vec::new();
        number(&mut b, 1, if alias { 2 } else { n });
        input.extend(framed(&b));
    }
    input.extend(framed(&reply(None, None)));
    for r in replies {
        input.extend(framed(&r));
    }
    Script {
...
fn actual_protocol_bootstrap_fragmented_reads_and_complete_receipts() -> Result<()> {
    ...
    let stream = script(
        vec![
            reply(Some(cap.canonical_bytes()), None),
            reply(None, Some(&prepared)),
            reply(None, Some(&applied()?)),
        ],
        false,
    );
...
fn absent_query_is_not_nonapplication_and_commit_failure_fences() -> Result<()> {
    let ctx = context()?;
    let p = plan()?;
    let mut c = WorkforceRpcClient::negotiate(
        script(vec![reply(None, None)], false),
        vec![b't'; 32],
        vec![b'n'; 16],
        &ctx,
    )?;
    assert!(c.query(&p, &ctx)?.is_none());
    assert!(c.commit(&p, &ctx).is_err());
    let sent = c.stream.output.len();
    assert!(c.fenced());
    assert!(c.commit(&p, &ctx).is_err());
    assert_eq!(c.stream.output.len(), sent);
    Ok(())
}
#[test]
fn malformed_bindings_fields_and_canonical_varints_are_rejected() -> Result<()> {
    assert!(
        WorkforceRpcClient::negotiate(
            script(vec![], true),
            vec![b't'; 32],
            vec![b'n'; 16],
            &context()?
    ...
    number(&mut r, 12, 0);
    assert!(Message::parse(&r, 12).is_err());
    Ok(())
}
#[test]
fn local_authority_and_invalid_selections_do_not_touch_socket() -> Result<()> {
    let ctx = context()?;
    let mut c =
        WorkforceRpcClient::negotiate(script(vec![], false), vec![b't'; 32], vec![b'n'; 16], &ctx)?;
    let sent = c.stream.output.len();
    assert!(c.observe(&[5, 2], &ctx).is_err());
    let mut denied = ctx;
