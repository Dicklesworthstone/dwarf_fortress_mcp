...
fn u16(r: &mut Reader<'_>) -> Result<usize> {
    Ok(usize::from(u16::from_be_bytes(r.array()?)))
}
fn text(r: &mut Reader<'_>, maximum: usize, empty: bool) -> Result<String> {
    let n = u16(r)?;
    require(
        n <= maximum && (empty || n != 0),
        "invalid workforce text length",
    )?;
...
pub fn validate_ids(ids: &[u32]) -> Result<()> {
    check_ids(ids, MAX_UNITS, false)
}
fn check_ids(ids: &[u32], max: usize, empty: bool) -> Result<()> {
    require(
        ...
        "workforce IDs must be bounded, sorted and unique",
    )
}
...
impl Citizen {
    fn decode(r: &mut Reader<'_>, n: usize) -> Result<Self> {
        let id = r.u32()?;
        let historical_id = r.u32()?;
        let eligible = r.boolean()?;
        require(
            id <= i32::MAX as u32 && historical_id <= i32::MAX as u32,
...
impl WorkforceCapture {
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        require(
            bytes.len() <= MAX_CAPTURE,
            "workforce capture exceeds 64 KiB",
        )?;
        ...
        require(
            generation > 0
                ...
                && site <= i32::MAX as u32,
            "invalid workforce source or clock",
        )?;
        ...
        let mut labor_keys = Vec::with_capacity(columns);
        let mut seen = BTreeSet::new();
        for _ in 0..columns {
            let key = text(&mut r, 64, false)?;
            require(seen.insert(key.clone()), "duplicate labor column")?;
            labor_keys.push(key);
        }
        let count = u16(&mut r)?;
        require(count <= 64, "too many work details")?;
        let mut details = Vec::with_capacity(count);
        let mut total = 0usize;
        for _ in 0..count {
            let name = text(&mut r, 256, true)?;
            let flags = r.u32()?;
            let selected_only = r.boolean()?;
            let labors = bits(&mut r, columns)?;
            let n = u16(&mut r)?;
            total += n;
            require(
                total <= 4096 && n <= r.0.len() / 4,
                "workforce membership bound",
            )?;
        ...
        let count = u16(&mut r)?;
        require(
            (1..=MAX_UNITS).contains(&count),
            "invalid selected citizen count",
        )?;
        let mut citizens = Vec::with_capacity(count);
        for _ in 0..count {
            citizens.push(Citizen::decode(&mut r, columns)?);
        }
        validate_ids(&citizens.iter().map(Citizen::id).collect::<Vec<_>>())?;
        r.finish()?;
        Ok(Self {
            bytes: bytes.to_vec(),
            generation,
    ...
    fn encode_values(&self) -> Result<Vec<u8>> {
        ...
        for k in &self.labor_keys {
            put_text(&mut out, k);
        }
        out.extend_from_slice(&(self.details.len() as u16).to_be_bytes());
        for d in &self.details {
            put_text(&mut out, &d.name);
            out.extend_from_slice(&d.flags.to_be_bytes());
            out.push(u8::from(d.selected_only));
            out.extend_from_slice(&d.labors);
    ...
    fn expected(&self, spec: AssignmentSpec) -> Result<(Self, Vec<u32>)> {
        let d = self.details.get(spec.detail as usize).ok_or_else(|| {
            error(
                ErrorCode::InvalidRequest,
                "work detail is outside this capture",
            )
        })?;
        require(
            self.paused
                ...
                && d.selected_only
                && d.labors.contains(&1)
                && self.citizens.iter().all(Citizen::eligible),
        ...
        let changed: Vec<u32> = self
            ...
            .iter()
            .map(Citizen::id)
            .filter(|id| d.members.binary_search(id).is_ok() != spec.assigned)
            .collect();
        require(
            !changed.is_empty(),
    ...
    pub fn ids(&self) -> Vec<u32> {
        self.citizens.iter().map(Citizen::id).collect()
    }
    pub fn entity_cost(&self) -> usize {
        self.citizens.len()
            + self.details.len()
            + self.details.iter().map(|d| d.members.len()).sum::<usize>()
    }
...
pub fn fortress_id(folder: &str, site: u32) -> FortressId {
    ...
    data.extend_from_slice(folder.as_bytes());
    data.push(0);
    data.extend_from_slice(&site.to_be_bytes());
...
impl AssignmentSpec {
    pub fn new(detail: u32, assigned: bool) -> Result<Self> {
        require(detail < 64, "detail index must be 0..63")?;
        Ok(Self { detail, assigned })
    }
    ...
    fn append(self, out: &mut Vec<u8>) {
        ...
        out.push(u8::from(self.assigned));
    }
}
#[derive(Clone, Debug, PartialEq, Eq)]
...
impl AssignmentPlan {
    pub fn new(key: &str, spec: AssignmentSpec, before: WorkforceCapture) -> Result<Self> {
        validate_key(key)?;
        before.expected(spec)?;
        let mut data = Vec::new();
        spec.append(&mut data);
        data.extend_from_slice(before.witness().as_bytes());
        let digest = hash(b"dfmcp-workforce-plan/1", &data);
        let mut data = Vec::new();
        put_text(&mut data, key);
        data.extend_from_slice(digest.as_bytes());
        let full = hash(b"dfmcp-workforce-token/1", &data);
        let mut token = [0; 16];
    ...
    pub fn spec(&self) -> AssignmentSpec {
        self.spec
    }
    pub fn before(&self) -> &WorkforceCapture {
        &self.before
    }
    ...
    pub fn changed_ids(&self) -> Result<Vec<u32>> {
        Ok(self.before.expected(self.spec)?.1)
    }
    pub fn canonical_bytes(&self) -> Vec<u8> {
        let mut out = Vec::new();
        put_text(&mut out, &self.key);
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        let mut r = Reader(bytes);
        let key = text(&mut r, 128, false)?;
        let spec = AssignmentSpec::new(r.u32()?, r.boolean()?)?;
        let n = r.u32()? as usize;
        require(n <= MAX_CAPTURE, "capture length exceeds bound")?;
        let before = WorkforceCapture::decode(r.take(n)?)?;
        r.finish()?;
...
impl AssignmentPhase {
    pub fn settled(self) -> bool {
        matches!(self, Self::Applied | Self::Refused | Self::Cancelled)
    }
...
impl AssignmentEffect {
    pub fn decode(bytes: &[u8], plan: &AssignmentPlan) -> Result<Self> {
        ...
        let mut r = Reader(bytes);
        require(r.take(8)? == b"DFMWE017", "not a workforce/1.17 effect")?;
        require(
            text(&mut r, 128, false)? == plan.key
                && r.take(32)? == plan.digest.as_bytes()
                && r.take(16)? == plan.token.as_slice(),
            "workforce effect identity mismatch",
        )?;
        require(
            r.u32()? == plan.spec.detail
                && r.boolean()? == plan.spec.assigned
                && r.take(32)? == plan.before.witness().as_bytes(),
            "workforce effect plan mismatch",
        )?;
        require(
            r.u64()? == plan.before.generation
                && r.u64()? == plan.before.sequence
                && r.u64()? == plan.before.tick,
            "workforce effect source mismatch",
        )?;
        let phase = match r.byte()? {
            0 => AssignmentPhase::Prepared,
            1 => AssignmentPhase::Unknown,
            2 => AssignmentPhase::Applied,
            3 => AssignmentPhase::Refused,
            4 => AssignmentPhase::Cancelled,
            _ => {
                return Err(error(
                    ErrorCode::AdapterRejected,
                    "unknown assignment phase",
                ));
            }
        };
        ...
        let count = u16(&mut r)?;
        require(
            columns == plan.before.labor_keys.len() && count <= MAX_UNITS,
            "workforce readback dimensions changed",
        )?;
        let mut post = Vec::with_capacity(count);
        for _ in 0..count {
            post.push(Citizen::decode(&mut r, columns)?);
        }
        let receipt = Digest32::from_bytes(r.array()?);
        r.finish()?;
        require(
            receipt == hash(b"dfmcp-workforce-receipt/1", &bytes[..bytes.len() - 32]),
            "workforce receipt mismatch",
        )?;
        if phase == AssignmentPhase::Applied {
            let (mut expected, changed) = plan.before.expected(plan.spec)?;
            require(
                post.len() == expected.citizens.len(),
                "missing citizen readback",
            )?;
            for (old, new) in expected.citizens.iter_mut().zip(&post) {
                if changed.binary_search(&old.id).is_ok() {
                    if plan.spec.assigned {
                        require(
                            plan.before.details[plan.spec.detail as usize]
                                .labors
                                .iter()
                                .zip(&new.labors)
                                .all(|(allowed, enabled)| *allowed == 0 || *enabled == 1),
                            "recomputed labor permissions missing",
                        )?;
                    }
                ...
                require(
                    &*old == new,
                    "unexpected citizen identity, eligibility or unchanged-mask write",
                )?;
            }
            require(
                after == Digest32::of_bytes(&expected.encode_values()?),
                "full post-configuration witness mismatch",
            )?;
        } else {
            require(
                post.is_empty() && after == Digest32::ZERO,
                "non-applied effect invents readback",
            )?;
        }
        Ok(Self {
            bytes: bytes.to_vec(),
            ...
            phase,
            receipt,
            after: (phase == AssignmentPhase::Applied).then_some(after),
            post,
        })
    }
    pub fn canonical_bytes(&self) -> &[u8] {
    ...
    pub fn after_witness(&self) -> Option<Digest32> {
        self.after
    }
    pub fn post_citizens(&self) -> &[Citizen] {
        &self.post
    }
    /// Unknown is permanent in this native generation, not a later promotion slot.
    pub fn follows(&self, old: &Self) -> Result<()> {
        require(
            self.key == old.key && self.plan == old.plan,
            "assignment receipt changed plans",
        )?;
        if old.phase != AssignmentPhase::Prepared {
            require(
                self.bytes == old.bytes,
                "immutable assignment evidence changed",
            )?;
        }
        Ok(())
    }
