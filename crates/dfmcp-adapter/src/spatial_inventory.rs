...
impl Work {
    fn charge(&mut self, n: u64) -> Result<()> {
        self.used = self
            .used
            .checked_add(n)
            .ok_or_else(|| exhausted("spatial work overflow"))?;
        if self.used > self.maximum {
            return Err(exhausted("spatial inventory work exhausted"));
        }
        Ok(())
    }
}
...
fn normalize(requested: &[MaterialDemand]) -> Result<Vec<MaterialDemand>> {
    if requested.is_empty() || requested.len() > flow::MAX_DEMANDS {
        return Err(exhausted("request 1..32 declared demands"));
    }
    for d in requested {
        if d.key.is_empty()
            || d.key.len() > 64
            ...
            || d.item_types.is_empty()
            || d.item_types.len() > 8
            || d.item_types
                .iter()
                .any(|s| s.is_empty() || s.len() > 128 || s.contains('\0'))
            || d.subtype.is_some_and(|n| n < -1)
            || d.material_type.is_some_and(|n| n < -1)
            || d.material_index.is_some_and(|n| n < -1)
            || (d.material_index.is_some() && d.material_type.is_none())
        {
            return Err(invalid(
                "invalid spatial demand key, units, types or material pair",
            ));
        }
    }
    ...
    if demands.windows(2).any(|p| p[0].key == p[1].key) {
        return Err(invalid("duplicate spatial demand key"));
    }
...
pub fn plan<T: SpatialStateView>(
    state: &T,
    context: &OperationContext,
    origin: [u32; 3],
    requested: &[MaterialDemand],
    maximum_work: u64,
) -> Result<SpatialInventory> {
    context.authorize(Capability::Query, RiskTier::ReadOnly, &[], None)?;
    if maximum_work == 0 || maximum_work > flow::MAX_WORK {
        return Err(exhausted("invalid spatial work budget"));
    }
    let demands = normalize(requested)?;
    let snapshot = state
        .snapshot()
        .ok_or_else(|| invalid("spatial snapshot is absent"))?;
    if snapshot.anchor() != context.anchor {
        return Err(DfmcpError::new(
            ErrorCode::StaleAnchor,
            "spatial plan uses another observation",
        ));
    }
    if snapshot.graph.entities.len() > context.budget.max_entities as usize {
        return Err(exhausted("spatial scan exceeds session budget"));
    }
    if !snapshot.hash_is_valid() {
        return Err(invariant("invalid spatial snapshot hash"));
    }
    let observation = state
        .spatial_observation()
        .ok_or_else(|| invariant("spatial observation absent"))?;
    let map = &observation.terrain().map;
    let field = Reachability::compute(map, &[origin], maximum_work.min(MAX_ROUTE_WORK))
        .map_err(map_error)?;
    let mut work = Work {
        used: field.work_units,
        maximum: maximum_work,
    };
    let operations = observation.operations();
    let items = &operations.items;
    work.charge(items.len() as u64)?;
    let ordinal: BTreeMap<_, _> = items
        .iter()
        .enumerate()
        .map(|(i, item)| (item.native_id, i))
        .collect();
    let mut attached = BTreeSet::new();
    for a in &operations.attachments {
        work.charge(1)?;
        attached.insert(a.item_native_id);
    }
            ...
            item.container_native_id
                .map(|id| {
                    ordinal
                        .get(&id)
                        .copied()
                        .ok_or_else(|| invariant("unobserved spatial container"))
                })
                .transpose()?,
        );
        flags.push(
            (item.flags & 0x1bf)
                | if attached.contains(&item.native_id) {
                    512
                } else {
                    0
                }
    ...
    for start in 0..items.len() {
        ...
        loop {
            work.charge(1)?;
            if color[current] == 1 {
                return Err(invariant("cyclic spatial containment"));
            }
            if color[current] == 2 {
                root = roots[current];
                inherited = flags[current];
                break;
            }
            color[current] = 1;
            chain.push(current);
    ...
    for (i, item) in items.iter().enumerate() {
        ...
        let reason = if flags[i] != 0 {
            Some("excluded_item_or_container_policy")
        } else if item.stack_size == 0 {
            Some("zero_stack_size")
        } else if root.flags & 64 == 0 || p.x < 0 || p.y < 0 || p.z < 0 {
            Some("unestablished_ground_location")
        } else if map.region.index(position).is_none() {
            Some("outside_observed_region")
        } else if field.distance_to(position).is_none() {
            Some("no_candidate_route_in_observed_model")
        } else {
            None
        };
        if let Some(reason) = reason {
            *counts.entry(reason).or_insert(0u64) += 1;
            continue;
        }
        ...
        for (d, demand) in demands.iter().enumerate() {
            ...
            if demand.item_types.binary_search(&item.type_key).is_ok()
                ...
                && demand
                    .material_index
                    .is_none_or(|n| n == item.material_index)
            {
                eligible |= 1u32 << d;
            }
        }
        if eligible == 0 {
            *counts.entry("no_declared_demand_match").or_insert(0) += 1;
            continue;
        }
        if supplies.len() >= flow::MAX_SUPPLIES {
            return Err(exhausted(
                "too many spatially eligible stacks for bounded allocation",
            ));
        }
        let id = item_entity_id(item.native_id);
        let root_id = item_entity_id(root.native_id);
        let entity = snapshot
            .graph
            .entities
            .get(&id)
            .ok_or_else(|| invariant("spatial item lacks entity"))?;
        let root_entity = snapshot
            .graph
            .entities
            .get(&root_id)
            .ok_or_else(|| invariant("spatial container lacks entity"))?;
        let steps = field
            .distance_to(position)
            .ok_or_else(|| invariant("spatial candidate distance lost"))?;
        locations.insert(
            id.get(),
            LocatedSupply {
                item_id: id,
                generation: entity.generation,
                outermost_item_id: root_id,
                outermost_generation: root_entity.generation,
                position,
        ...
        supplies.push(Supply {
            id: id.get(),
            units: u64::from(item.stack_size),
            eligible,
        });
    ...
    let model: Vec<_> = demands
        ...
        .collect();
    let allocation = flow::allocate(&supplies, &model, maximum_work.saturating_sub(work.used))
        .map_err(|e| {
            DfmcpError::new(
                if e == flow::AllocationError::InvariantViolation {
                    ErrorCode::InternalInvariantViolation
                } else {
                    ErrorCode::BudgetExceeded
                },
                "spatial allocation exceeded bounds or failed certificate validation",
            )
        })?;
    ...
    Ok(SpatialInventory {
        ...
        locations,
        item_counts: counts,
        reachable_tiles: field.visited_tiles,
        touched_region_boundary: field.touched_region_boundary,
