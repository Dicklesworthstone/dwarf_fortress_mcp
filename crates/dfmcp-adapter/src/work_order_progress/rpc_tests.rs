...
fn reply(observation: Option<&[u8]>) -> Vec<u8> {
    reply_with_minor(observation, 12)
}
...
fn framed(body: &[u8]) -> Vec<u8> {
    let mut b = header(-1, body.len() as i32).to_vec();
...
fn bootstrap(second_id: u8) -> Vec<u8> {
    ...
    b.extend_from_slice(&framed(&[8, 2]));
    b.extend_from_slice(&framed(&[8, second_id]));
    b.extend_from_slice(&framed(&reply(None)));
    b
}
struct Script {
    input: Cursor<Vec<u8>>,
...
impl Write for Script {
    fn write(&mut self, bytes: &[u8]) -> io::Result<usize> {
        ...
        self.output.borrow_mut().extend_from_slice(&bytes[..n]);
        Ok(n)
    }
...
impl ProgressStream for Script {
...
fn client(tail: Vec<u8>) -> Result<(ProgressRpcClient<Script>, Rc<RefCell<Vec<u8>>>)> {
    ...
    let stream = Script::new(bytes);
    let output = stream.output.clone();
    Ok((
        ProgressRpcClient::negotiate(
            stream,
            vec![b'x'; 32],
            NONCE.to_vec(),
...
fn fragmented_native_vector_round_trip_binds_only_two_methods() -> Result<()> {
    let raw = fixture()?;
    let (mut c, output) = client(framed(&reply(Some(&raw))))?;
    let o = c.read(&[3, 8], Duration::from_secs(1))?;
    assert_eq!(o.canonical_bytes(), raw);
    let bytes = output.borrow();
    let s = String::from_utf8_lossy(&bytes);
    assert!(s.contains("Handshake") && s.contains("ReadObservation"));
...
fn invalid_local_selection_does_not_write_or_fence() -> Result<()> {
    let (mut c, out) = client(Vec::new())?;
    let n = out.borrow().len();
    for ids in [vec![], vec![3, 3], vec![8, 3], vec![u32::MAX]] {
        assert!(c.read(&ids, Duration::from_secs(1)).is_err());
    }
    assert_eq!(out.borrow().len(), n);
    assert!(!c.poisoned());
    Ok(())
}
#[test]
fn lost_reply_fences_and_never_implicitly_reconnects() -> Result<()> {
    ...
    assert!(c.read(&[3, 8], Duration::from_secs(1)).is_err());
    assert!(c.poisoned());
    let n = out.borrow().len();
    assert!(c.read(&[3, 8], Duration::from_secs(1)).is_err());
    assert_eq!(out.borrow().len(), n);
...
fn reply_identity_shape_unknown_and_duplicate_fields_fail_closed() -> Result<()> {
    let raw = fixture()?;
    let good = reply(Some(&raw));
    for minor in [10, 11] {
        assert!(decode_reply(&reply_with_minor(Some(&raw), minor), NONCE, Some(&[3, 8])).is_err());
    }
    assert!(decode_reply(&good, NONCE, None).is_err());
    assert!(decode_reply(&reply(None), NONCE, Some(&[3, 8])).is_err());
    ...
    for suffix in [vec![8, 1], vec![80, 1], vec![0], vec![0x80, 0]] {
        ...
        assert!(decode_reply(&bad, NONCE, Some(&[3, 8])).is_err());
    }
    let mut altered = raw;
    altered[15] = 8;
...
fn invalid_bindings_oversized_frames_and_notifications_are_bounded() -> Result<()> {
    ...
    for tail in [
        header(-1, 32769).to_vec(),
        header(-1, -1).to_vec(),
        header(-3, 65537).to_vec(),
        header(-2, 0).to_vec(),
        header(4, 0).to_vec(),
    ] {
        let (mut c, _) = client(tail)?;
        assert!(c.read(&[3, 8], Duration::from_secs(1)).is_err());
        assert!(c.poisoned());
    }
...
fn nonminimal_varints_and_full_record_corruption_are_rejected() -> Result<()> {
    ...
    for size in 0..good.len() {
        let (mut c, _) = client(framed(&reply(Some(&good[..size]))))?;
        assert!(c.read(&[3, 8], Duration::from_secs(1)).is_err());
        assert!(c.poisoned());
    }
    Ok(())
