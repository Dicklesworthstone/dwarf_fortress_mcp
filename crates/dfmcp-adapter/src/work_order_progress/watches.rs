...
use super::{OrderStatus, ProgressManifest, error};
use dfmcp_core::{Digest32, ErrorCode, FortressId, Result};

#[path = "watch_book.rs"]
mod book;
pub use book::{
    BOOK_OPEN_RESERVE, MAX_BOOK_BYTES, MAX_BOOK_EVENTS, PrivateWatchFile, RetainedWatch,
    WatchBatch, WatchBook, WatchBookSummary, open_watch_book,
};
...
impl WatchSpec {
    pub fn new(
        ...
        if key.is_empty()
            || key.len() > MAX_WATCH_KEY
            || !key
                .bytes()
                .all(|b| b.is_ascii_alphanumeric() || matches!(b, b'-' | b'_' | b'.'))
            || order > i32::MAX as u32
            || deadline > super::MAX_TICK
            || !(1..=10_000).contains(&cadence)
            || !(1..=MAX_STABLE_SAMPLES).contains(&stable)
            || matches!(goal, WatchGoal::RemainingAtMost(n) if n > 100)
        {
            return Err(error(
                ErrorCode::InvalidRequest,
                "invalid bounded progress watch specification",
            ));
        }
        Ok(Self {
            key: key.to_owned(),
    ...
    pub fn native_order_id(&self) -> u32 {
        self.order
    }
    pub fn goal(&self) -> WatchGoal {
        self.goal
    }
    ...
    pub(super) fn encode(&self, out: &mut Vec<u8>) {
        ...
        let (tag, threshold) = match self.goal {
            WatchGoal::Validated => (1, 0),
            WatchGoal::Active => (2, 0),
            WatchGoal::RemainingAtMost(n) => (3, n),
        };
        out.push(tag);
        out.extend_from_slice(&threshold.to_be_bytes());
        out.extend_from_slice(&self.deadline.to_be_bytes());
        out.extend_from_slice(&self.cadence.to_be_bytes());
        out.push(self.stable);
    }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct WatchRecordRef {
    pub number: u64,
    pub digest: Digest32,
}
impl WatchRecordRef {
    pub fn of(record: &ArchivedProgress) -> Self {
        Self {
            number: record.entry.number,
            digest: record.entry.record_digest,
        }
    }
}
...
fn checked_record(record: &ArchivedProgress) -> Result<()> {
    ...
    if record.entry.number == 0
        || record.entry.number > super::archive::MAX_ARCHIVE_RECORDS as u64
        || record.entry.segment == 0
        || record.entry.witness != o.witness()
        || record.entry.game_tick != o.tick()
        || record.entry.native_order_ids != o.ids()
        || record.manifest.generation != o.generation()
        || [&record.manifest.df_version, &record.manifest.dfhack_version]
            .iter()
            .any(|s| s.is_empty() || s.len() > 128 || s.contains('\0'))
    {
        return Err(error(
            ErrorCode::CorruptLedger,
            "watch evidence metadata contradicts its complete observation",
        ));
    }
    Ok(())
}
impl WatchDefinition {
    pub fn new(spec: WatchSpec, archive_id: Digest32, origin: &ArchivedProgress) -> Result<Self> {
        ...
        let horizon = spec
            .deadline
            .checked_sub(o.tick())
            .filter(|n| *n > 0 && *n <= MAX_WATCH_HORIZON)
            .ok_or_else(|| {
                error(
                    ErrorCode::InvalidRequest,
                    "watch deadline must be after registration and within 120000 ticks",
                )
            })?;
        if archive_id == Digest32::ZERO
            || spec.cadence * u64::from(spec.stable) > horizon
            || matches!(spec.goal, WatchGoal::RemainingAtMost(n) if n > baseline.total as u32)
        {
            return Err(error(
                ErrorCode::InvalidRequest,
                "watch predicate or sampling schedule cannot fit its finite bound",
            ));
        }
        let mut bytes = b"dfmcp-progress-watch-definition/1\0".to_vec();
        bytes.extend_from_slice(archive_id.as_bytes());
        ...
        Ok(Self {
            ...
            origin: WatchRecordRef::of(origin),
            segment: origin.entry.segment,
            tick: o.tick(),
            fortress: o.fortress_id(),
            baseline: baseline.clone(),
            manifest: origin.manifest.clone(),
            selection: o.ids(),
            sequence: o.sequence(),
            horizon: o.next_order_id(),
        })
    }
    pub fn spec(&self) -> &WatchSpec {
        &self.spec
    }
    pub fn archive_id(&self) -> Digest32 {
        self.archive_id
    }
    pub fn digest(&self) -> Digest32 {
        self.digest
    }
    pub fn origin(&self) -> WatchRecordRef {
        self.origin
    }
    pub fn segment(&self) -> u64 {
        self.segment
    }
    pub fn registered_tick(&self) -> u64 {
        self.tick
    }
}
...
pub struct WatchEvaluation {
    ...
    state: WatchState,
    through: WatchRecordRef,
    last_tick: u64,
    last_sequence: u64,
    last_horizon: u32,
    last_remaining: i32,
    last_counted_tick: u64,
    evaluated_records: u32,
    samples: Vec<WatchRecordRef>,
}
impl WatchEvaluation {
    ...
    pub fn new(definition: WatchDefinition) -> Self {
        Self {
            ...
            through: definition.origin,
            last_tick: definition.tick,
            last_sequence: definition.sequence,
            last_horizon: definition.horizon,
            last_remaining: definition.baseline.remaining,
            last_counted_tick: definition.tick,
            evaluated_records: 0,
            samples: Vec::new(),
            definition,
        }
    }
    pub fn definition(&self) -> &WatchDefinition {
        &self.definition
    }
    pub fn state(&self) -> WatchState {
        self.state
    }
    pub fn through(&self) -> WatchRecordRef {
        self.through
    }
    pub fn evaluated_records(&self) -> u32 {
        self.evaluated_records
    }
    pub fn samples(&self) -> &[WatchRecordRef] {
        &self.samples
    }
    pub fn next_sample_tick(&self) -> Option<u64> {
        if self.state.terminal() {
            ...
            self.last_counted_tick
                .checked_add(self.definition.spec.cadence)
    ...
    pub fn advance(&mut self, record: &ArchivedProgress) -> Result<()> {
            ...
            || record.observation.fortress_id() != self.definition.fortress
        {
            return Err(error(
                ErrorCode::CorruptLedger,
                "watch replay omitted or substituted an archive record",
            ));
        }
    ...
    fn stop(&mut self, state: WatchState) {
        self.state = state;
        self.samples.clear();
    }
    fn advance_same_segment(&mut self, record: &ArchivedProgress) -> Result<()> {
        let o = &record.observation;
        if record.manifest != self.definition.manifest
            || o.ids() != self.definition.selection
            || o.sequence() <= self.last_sequence
            || o.tick() < self.last_tick
            || o.next_order_id() < self.last_horizon
        {
            return Err(error(
                ErrorCode::CorruptLedger,
                "watch archive segment crosses source, selection, sequence or clock boundaries",
            ));
        }
        self.last_tick = o.tick();
        self.last_sequence = o.sequence();
        self.last_horizon = o.next_order_id();
        if o.tick() > self.definition.spec.deadline {
        ...
        let Some(row) = o
            .rows()
            .iter()
            .find(|r| r.native_order_id == self.definition.spec.order)
            .and_then(|r| r.order.as_ref())
        else {
            self.stop(WatchState::MissingOutcomeUnknown);
            return Ok(());
        };
        if !self.definition.baseline.same_configuration(row) {
            self.stop(WatchState::ConfigurationChanged);
            return Ok(());
        }
        if row.remaining > self.last_remaining {
            self.stop(WatchState::CounterIncreased);
            return Ok(());
        }
        ...
        {
            self.samples.push(self.through);
            self.last_counted_tick = o.tick();
    ...
    pub fn cancel_at(&mut self, at: WatchRecordRef) -> Result<()> {
        if self.state == WatchState::Cancelled && self.through == at {
            return Ok(());
        }
        if self.state.terminal() || self.through != at {
            return Err(error(
                ErrorCode::Conflict,
                "watch cancellation must name the exact pending evaluation frontier",
            ));
        }
        self.stop(WatchState::Cancelled);
        Ok(())
    }
}
