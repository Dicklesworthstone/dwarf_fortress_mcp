...
use super::{
    MAX_OBSERVATION_BYTES, ProgressManifest, ProgressObservation, ProgressSource, validate_targets,
};
...
fn bad() -> DfmcpError {
    DfmcpError::new(ErrorCode::AdapterRejected, "malformed progress/1.12 RPC")
}
fn io_error(_: io::Error) -> DfmcpError {
    DfmcpError::new(
        ErrorCode::AdapterUnavailable,
        "progress I/O failed; connection fenced, no game mutation dispatched",
    )
}
fn deadline(timeout: Duration) -> Result<Instant> {
    if !(Duration::from_millis(1)..=Duration::from_secs(60)).contains(&timeout) {
        return Err(DfmcpError::new(
            ...
            "progress timeout must be 1..60000 milliseconds",
        ));
    }
...
fn credentials(token: &[u8], nonce: &[u8]) -> Result<()> {
    if !(32..=256).contains(&token.len()) || !(16..=64).contains(&nonce.len()) {
        return Err(DfmcpError::new(
            ...
            "progress credentials or nonce exceed bounds",
        ));
    }
    Ok(())
}
...
fn read_varint(data: &[u8], offset: &mut usize) -> Result<u64> {
    let mut value = 0;
    for shift in 0..10 {
        let b = *data.get(*offset).ok_or_else(bad)?;
        *offset += 1;
        if shift == 9 && b > 1 {
            return Err(bad());
        }
...
impl<'a> Message<'a> {
    fn decode(data: &'a [u8], maximum: u32) -> Result<Self> {
        ...
        while offset < data.len() {
            ...
            if field == 0 || field > maximum || fields.contains_key(&field) {
                return Err(bad());
            }
            let value = match key & 7 {
                ...
                2 => {
                    let length =
                        usize::try_from(read_varint(data, &mut offset)?).map_err(|_| bad())?;
                    let end = offset.checked_add(length).ok_or_else(bad)?;
                    let value = data.get(offset..end).ok_or_else(bad)?;
                    offset = end;
                    Field::Bytes(value)
                }
                _ => return Err(bad()),
...
fn header(id: i16, length: i32) -> [u8; 8] {
    let mut h = [0; 8];
    h[..2].copy_from_slice(&id.to_le_bytes());
...
fn call<S: ProgressStream>(stream: &mut S, method: i16, request: &[u8]) -> Result<Vec<u8>> {
    ...
    loop {
        ...
        let size =
            usize::try_from(i32::from_le_bytes([h[4], h[5], h[6], h[7]])).map_err(|_| bad())?;
        if method == -3 {
            notifications += 1;
            notification_bytes = notification_bytes.checked_add(size).ok_or_else(bad)?;
            if notifications > 8 || size > 65_536 || notification_bytes > 262_144 {
                return Err(bad());
            }
        ...
        stream.read_exact(&mut data).map_err(io_error)?;
        if method == -1 {
            return Ok(data);
        }
    }
...
fn bind<S: ProgressStream>(stream: &mut S, name: &str) -> Result<i16> {
    ...
    for (field, s) in [(1, name), (2, REQUEST), (3, REPLY), (4, PLUGIN)] {
        bytes(&mut request, field, s.as_bytes());
    }
    let raw = call(stream, 0, &request)?;
    let message = Message::decode(&raw, 1)?;
...
fn decode_reply(
    ...
    nonce: &[u8],
    ids: Option<&[u32]>,
) -> Result<(ProgressManifest, Option<ProgressObservation>)> {
    let m = Message::decode(raw, 9)?;
    if m.number(4)? != 1 || m.number(5)? != 12 {
        return Err(DfmcpError::new(
            ErrorCode::VersionMismatch,
            "progress requires exact protocol 1.12",
        ));
    }
    ...
    if !accepted {
        ...
        return Err(DfmcpError::new(
            kind,
            "native progress request refused; no capture published",
        ));
    }
    if code != 0
        || generation == 0
        || generation == u64::MAX
        || m.0.contains_key(&9) != ids.is_some()
    {
        return Err(bad());
    }
...
impl<S: ProgressStream> ProgressRpcClient<S> {
    ...
    fn negotiate_until(
        ...
        let (manifest, _) = decode_reply(&raw, &nonce, None)?;
        remaining(until)?;
        Ok(Self {
            stream,
            token,
...
impl<S: ProgressStream> ProgressSource for ProgressRpcClient<S> {
    ...
    fn read(&mut self, ids: &[u32], timeout: Duration) -> Result<ProgressObservation> {
        ...
        for id in ids {
            number(&mut request, 5, u64::from(*id));
        }
        let result = (|| {
            ...
            if manifest != self.manifest {
                return Err(DfmcpError::new(
                    ...
                    "progress source incarnation or software changed",
                ));
            }
            remaining(until)?;
            observation.ok_or_else(bad)
        })();
        if result.is_err() {
            self.fenced = true;
        }
    ...
    pub fn connect(
        ...
        if !endpoint.ip().is_loopback() || endpoint.port() == 0 {
            return Err(DfmcpError::new(
                ...
                "progress endpoint must be numeric loopback with a nonzero port",
            ));
        }
