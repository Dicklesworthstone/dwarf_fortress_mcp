...
impl PrivateProgressArchiveFile {
    fn writable(&self) -> io::Result<()> {
        if self.offline { Err(io::Error::new(io::ErrorKind::PermissionDenied, "offline progress archive is read-only")) } else { Ok(()) }
    }
}
impl Read for PrivateProgressArchiveFile { fn read(&mut self, out: &mut [u8]) -> io::Result<usize> { self.file.read(out) } }
impl Seek for PrivateProgressArchiveFile { fn seek(&mut self, from: SeekFrom) -> io::Result<u64> { self.file.seek(from) } }
...
impl JournalStorage for PrivateProgressArchiveFile {
    ...
    fn truncate(&mut self, _: u64) -> io::Result<()> { Err(io::Error::new(io::ErrorKind::PermissionDenied, "progress history cannot be truncated or repaired")) }
    fn validate_identity(&self) -> io::Result<()> {
        #[cfg(unix)] {
            use std::os::unix::fs::MetadataExt;
            let denied = || io::Error::new(io::ErrorKind::PermissionDenied, "progress archive custody changed");
        ...
        #[cfg(not(unix))] { Err(io::Error::new(io::ErrorKind::Unsupported, "private progress archives currently require Unix")) }
    }
}

pub fn open_progress_archive(path: &Path, mode: ArchiveMode, c: &OperationContext)
    -> Result<ProgressArchive<PrivateProgressArchiveFile>>
{
    c.authorize(Capability::Query, RiskTier::ReadOnly, &[], None)?;
    if mode == ArchiveMode::Live { c.authorize(Capability::Observe, RiskTier::ReadOnly, &[], None)?; }
    #[cfg(unix)] {
        use std::os::unix::fs::{MetadataExt, OpenOptionsExt};
        let denied = || error(ErrorCode::CapabilityDenied, "progress archive requires a normalized absolute path, real private 0700 parent and single-link 0600 regular file");
        ...
        if created && mode == ArchiveMode::Offline { return Err(error(ErrorCode::InvalidRequest, "offline progress recovery requires an existing archive")); }
        ...
        file.try_lock().map_err(|_| error(ErrorCode::Conflict, "progress archive already has an owner or cannot be locked"))?;
        ...
        let storage = PrivateProgressArchiveFile { file, path: path.to_owned(), offline: mode == ArchiveMode::Offline,
    ...
    #[cfg(not(unix))] {
        let _ = path;
        Err(error(ErrorCode::CapabilityDenied, "private progress archives currently require Unix custody"))
    }
}
