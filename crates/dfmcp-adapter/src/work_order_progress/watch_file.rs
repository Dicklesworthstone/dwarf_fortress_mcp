...
use super::{
    ArchiveMode, JournalStorage, ProgressArchive, WatchBook, authorize, error, storage_error,
};
    ...
    fn writable(&self) -> io::Result<()> {
        if self.offline {
            Err(io::Error::new(
                ...
                "offline watch book is read-only",
            ))
        } else {
            Ok(())
        }
...
impl Read for PrivateWatchFile {
    fn read(&mut self, out: &mut [u8]) -> io::Result<usize> {
        self.file.read(out)
    }
}
impl Seek for PrivateWatchFile {
...
impl JournalStorage for PrivateWatchFile {
    ...
    fn truncate(&mut self, _: u64) -> io::Result<()> {
        Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            "watch intent cannot be truncated or repaired",
        ))
    }
    fn validate_identity(&self) -> io::Result<()> {
        ...
        #[cfg(not(unix))]
        {
            Err(io::Error::new(
                ...
                "private watch files require Unix custody",
            ))
        }
    }
}

pub fn open_watch_book<A: JournalStorage>(
    path: &Path,
    mode: ArchiveMode,
    archive: &mut ProgressArchive<A>,
    c: &OperationContext,
) -> Result<WatchBook<PrivateWatchFile>> {
    ...
    #[cfg(unix)]
    {
        ...
        if created && mode == ArchiveMode::Offline {
            return Err(error(
                ...
                "offline recovery requires an existing watch book",
            ));
        }
        ...
        file.try_lock().map_err(|_| {
            error(
                ErrorCode::Conflict,
                "watch book already has an owner or cannot be locked",
            )
        })?;
        ...
        remaining.budget.max_wall_millis = c
            .budget
            .max_wall_millis
            .checked_sub(elapsed)
            .filter(|n| *n > 0)
    ...
    #[cfg(not(unix))]
    {
        ...
        Err(error(
            ErrorCode::CapabilityDenied,
            "private watch books currently require Unix custody",
        ))
    }
}
