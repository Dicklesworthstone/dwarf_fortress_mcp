use super::*;
use dfmcp_core::{
    CapabilityGrant, CapabilityScope, ObservationCursor, RequestId, RiskTier, SessionId,
...
fn context() -> Result<OperationContext> {
    Ok(OperationContext {
        ...
        anchor: StateAnchor {
            fortress_id: capture(1, 10, 5)?.fortress_id(),
...
impl Write for Memory {
    fn write(&mut self, data: &[u8]) -> io::Result<usize> {
        self.shared.events.borrow_mut().push("write");
        let mut bytes = self.shared.bytes.borrow_mut();
        let length = bytes.len().max(self.position + data.len());
        bytes.resize(length, 0);
        bytes[self.position..self.position + data.len()].copy_from_slice(data);
...
impl Seek for Memory {
    fn seek(&mut self, from: SeekFrom) -> io::Result<u64> {
        ...
        self.position = usize::try_from(n).map_err(|_| io::Error::other("invalid test seek"))?;
...
impl JournalStorage for Memory {
    fn sync(&mut self) -> io::Result<()> {
        ...
        if *self.shared.fail_sync.borrow() {
    ...
    fn truncate(&mut self, _: u64) -> io::Result<()> {
        Err(io::Error::other("no truncation permitted"))
    }
}
...
fn reopen(
    ...
    mode: ArchiveMode,
    c: &OperationContext,
) -> Result<ProgressArchive<Memory>> {
    let shared = Shared::default();
    *shared.bytes.borrow_mut() = bytes;
...
fn replay_preserves_complete_native_evidence_and_exact_references() -> Result<()> {
    ...
    assert_eq!(
        shared.events.borrow().as_slice(),
        &["write", "sync", "write", "sync", "write", "sync"]
    );
    let mut recovered = reopen(shared.bytes.borrow().clone(), ArchiveMode::Offline, &c)?;
    assert_eq!(
        recovered
            .record(first.number, first.record_digest, &c)?
            .observation,
        capture(1, 10, 5)?
    );
    let (_, _, comparison) = recovered.compare_records(
        (first.number, first.record_digest),
        (second.number, second.record_digest),
        &c,
    )?;
    assert_eq!(comparison.changes[0].remaining_decrease, Some(3));
    assert_eq!(recovered.summary(&c)?.records, 2);
    Ok(())
}
#[test]
fn reopen_starts_a_new_segment_and_does_not_invent_downtime_continuity() -> Result<()> {
    ...
    assert!(
        recovered
            .compare_records(
                (old.number, old.record_digest),
                (fresh.number, fresh.record_digest),
                &c
            )
            .is_err()
    );
    ...
    assert!(
        recovered
            .compare_records(
                (fresh.number, fresh.record_digest),
                (later.number, later.record_digest),
                &c
            )
            .is_ok()
    );
...
fn offline_mode_cannot_be_promoted_and_old_ticks_cannot_revive_expired_grants() -> Result<()> {
    ...
    for g in &mut denied.grants {
        g.expires_at_tick = Some(GameTick(99));
    }
    assert!(offline.latest(&denied).is_err());
    assert!(reopen(bytes, ArchiveMode::Offline, &denied).is_err());
    denied = c.clone();
    denied.cancellation_requested = true;
    assert!(offline.latest(&denied).is_err());
    denied = c.clone();
    denied.anchor.fortress_id = FortressId::new(99);
    assert!(offline.summary(&denied).is_err());
    Ok(())
}
#[test]
fn failed_sync_fences_owner_but_complete_uncertain_frame_can_be_recovered() -> Result<()> {
    let (mut a, shared, c) = setup()?;
    *shared.fail_sync.borrow_mut() = true;
    assert!(a.append(&manifest(), &capture(1, 10, 5)?, &c).is_err());
    assert!(a.summary(&c).is_err());
    assert!(a.last.is_none());
    let mut reopened = reopen(shared.bytes.borrow().clone(), ArchiveMode::Offline, &c)?;
    assert_eq!(reopened.summary(&c)?.records, 1);
    Ok(())
}
#[test]
fn torn_tails_and_every_byte_corruption_are_refused_without_repair() -> Result<()> {
    let (mut a, shared, c) = setup()?;
    a.append(&manifest(), &capture(1, 10, 5)?, &c)?;
    let bytes = shared.bytes.borrow().clone();
    for end in 0..bytes.len() {
        if end != HEADER_BYTES as usize {
            assert!(
                reopen(bytes[..end].to_vec(), ArchiveMode::Offline, &c).is_err(),
                "prefix {end}"
            );
        }
    }
    ...
    assert!(reopen(Vec::new(), ArchiveMode::Live, &c).is_err());
    Ok(())
}
#[test]
fn same_length_corruption_and_external_extent_changes_invalidate_reads() -> Result<()> {
    ...
    assert!(a.record(e.number, e.record_digest, &c).is_err());
    assert!(a.fenced);
    let (mut a, shared, c) = setup()?;
    shared.bytes.borrow_mut().push(0);
    assert!(a.summary(&c).is_err());
    assert!(a.fenced);
    Ok(())
}
...
fn history_pages_and_exact_lookups_are_head_bound_and_budgeted() -> Result<()> {
    let (mut a, _, c) = setup()?;
    for n in 1..=3 {
        a.append(&manifest(), &capture(n, n * 10, 5)?, &c)?;
    }
    ...
    assert!(a.record(1, Digest32::ZERO, &c).is_err());
    assert!(a.record(0, summary.head, &c).is_err());
    let mut small = c.clone();
    small.budget.max_bytes = 511;
    assert!(a.page(summary.head, 0, 1, &small).is_err());
    small = c.clone();
    small.budget.max_entities = 1;
    assert!(a.latest(&small).is_err());
    a.append(&manifest(), &capture(4, 40, 5)?, &c)?;
    assert!(a.page(summary.head, 2, 2, &c).is_err());
    Ok(())
}
#[test]
fn capacity_refusal_occurs_without_writing_or_silently_evicting() -> Result<()> {
    ...
    let before = shared.bytes.borrow().clone();
    assert!(a.reserve_capture(&c).is_err());
    assert_eq!(*shared.bytes.borrow(), before);
    let mut small = c.clone();
    small.budget.max_bytes = MAX_FRAME_BYTES - 1;
    let (mut empty, _, _) = setup()?;
    assert!(empty.reserve_capture(&small).is_err());
    Ok(())
}
#[test]
fn rehashed_illegal_segment_and_reordered_samples_do_not_replay() -> Result<()> {
    ...
    for segment in [0u64, 3] {
        ...
        let digest = frame_hash(
            a.id,
            &bad[at..at + PREFIX_BYTES],
            &bad[at + PREFIX_BYTES..end],
        );
    ...
    let end = at + PREFIX_BYTES + body.len();
    bad[end..end + 32].copy_from_slice(digest.as_bytes());
    assert!(reopen(bad, ArchiveMode::Offline, &c).is_err());
    Ok(())
}
...
fn private_archive_locks_permissions_offline_descriptor_and_renamed_custody() -> Result<()> {
    ...
    let root = std::env::temp_dir()
        ...
        .join(format!(
            "dfmcp-progress-archive-{}-{}",
            std::process::id(),
            NEXT.fetch_add(1, Ordering::Relaxed)
        ));
    let mut builder = std::fs::DirBuilder::new();
    builder.mode(0o700);
    builder.create(&root).map_err(storage_error)?;
    ...
    let c = context()?;
    assert!(open_progress_archive(&path, ArchiveMode::Offline, &c).is_err());
    assert!(!path.exists());
...
impl crate::work_order_progress::ProgressSource for Source {
    ...
    fn read(&mut self, _: &[u32], _: Duration) -> Result<ProgressObservation> {
        if self.fenced {
            return Err(error(ErrorCode::AdapterUnavailable, "source fenced"));
        }
...
fn durable_publication_barrier_precedes_current_capture_and_failure_fences() -> Result<()> {
    ...
    session.refresh_with_publication(&[3, 8], &c, |manifest, capture, context| {
        assert_eq!(manifest.generation, capture.generation());
        assert_eq!(context.anchor.tick, GameTick(10));
    ...
    assert_eq!(
        session.current(&c)?.map(ProgressObservation::tick),
        Some(10)
    );
    assert!(
        session
            .refresh_with_publication(&[3, 8], &c, |_, _, _| Err(error(
    ...
    assert!(session.refresh(&[3, 8], &c).is_err());
    Ok(())
}
#[test]
fn invalid_native_evidence_never_reaches_publication_callback() -> Result<()> {
    ...
    let mut session = crate::work_order_progress::ProgressSession::new(source, &c)?;
    session.refresh(&[3, 8], &c)?;
    let called = std::cell::Cell::new(false);
    assert!(
        session
            .refresh_with_publication(&[3, 8], &c, |_, _, _| {
                called.set(true);
                Ok(())
            })
            .is_err()
