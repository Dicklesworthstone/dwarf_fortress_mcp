...
use super::{
    ProgressComparison, ProgressManifest, ProgressObservation, Reader, compare, error,
    validate_targets,
};
...
fn exhausted() -> dfmcp_core::DfmcpError {
    error(
        ...
        "progress archive exceeds its explicit work or retention allowance",
    )
}
fn storage_error(_: io::Error) -> dfmcp_core::DfmcpError {
    corrupt("progress archive custody or I/O failed; reopen without repairing evidence")
}
...
impl Budget {
    fn new(c: &OperationContext) -> Result<Self> {
        ...
        Ok(Self {
            start: Instant::now(),
            allowance: Duration::from_millis(c.budget.max_wall_millis.min(60_000)),
            bytes: c.budget.max_bytes,
        })
    }
    fn check(&self) -> Result<()> {
        if self.start.elapsed() >= self.allowance {
            Err(exhausted())
        } else {
            Ok(())
        }
    ...
    fn remaining_context(&self, c: &OperationContext) -> Result<OperationContext> {
        ...
        remaining.budget.max_wall_millis = u64::try_from(
            self.allowance
...
impl<S: JournalStorage> ProgressArchive<S> {
    ...
    pub fn open(
        ...
        if c.anchor.fortress_id == FortressId::NIL {
            return Err(corrupt("progress archive requires a concrete fortress"));
        }
        if mode == ArchiveMode::Live {
            c.authorize(Capability::Observe, super::RiskTier::ReadOnly, &[], None)?;
        }
        if initialize_empty && mode == ArchiveMode::Offline {
            return Err(error(
                ErrorCode::CapabilityDenied,
                "offline archive cannot initialize bytes",
            ));
        }
        ...
        if length == 0 && initialize_empty {
            ...
            header.extend_from_slice(&c.anchor.fortress_id.get().to_be_bytes());
        ...
        if length < HEADER_BYTES {
            return Err(corrupt(
                "incomplete progress archive header; bytes unchanged",
            ));
        }
        storage.seek(SeekFrom::Start(0)).map_err(storage_error)?;
        ...
        if r.take(8)? != MAGIC {
            return Err(corrupt("not a progress/1.12 archive"));
        }
        let fortress = FortressId::new(r.u64()?);
        let id = Digest32::from_bytes(r.array()?);
        let head = Digest32::from_bytes(r.array()?);
        if fortress != c.anchor.fortress_id || head != header_hash(&header[..48]) {
            return Err(corrupt("archive identity or header checksum mismatch"));
        }
        let mut out = Self {
            storage,
            fortress,
        ...
        while out.length < length {
            budget.check()?;
            if out.entries.len() >= MAX_ARCHIVE_RECORDS
                || out.entries.len() >= c.budget.max_entities as usize
            {
                return Err(exhausted());
            }
            let record = out.read_frame(out.length, length)?;
            if record.observation.rows().len() > c.budget.max_entities as usize {
                return Err(exhausted());
            }
            ...
            out.entries.push(record.entry.clone());
            out.last = Some(record);
        }
        out.access(c)?;
        budget.check()?;
        Ok(out)
    ...
    fn authority(&self, c: &OperationContext, write: bool) -> Result<()> {
        if c.anchor.fortress_id != self.fortress {
            return Err(error(
                ErrorCode::CapabilityDenied,
                "archive belongs to another fortress",
            ));
        }
        let mut current = c.clone();
        current.anchor.tick = GameTick(self.tick_floor.max(c.anchor.tick.get()));
        current.authorize(Capability::Query, super::RiskTier::ReadOnly, &[], None)?;
        if write {
            if self.mode != ArchiveMode::Live {
                return Err(error(
                    ErrorCode::CapabilityDenied,
                    "offline archive cannot append, even with an injected Observe grant",
                ));
            }
    ...
    pub fn access(&mut self, c: &OperationContext) -> Result<()> {
        ...
        if self.fenced {
            return Err(corrupt(
                "progress archive fenced; reopen for verified recovery",
            ));
        }
        let check = (|| {
            ...
            if self.storage.seek(SeekFrom::End(0)).map_err(storage_error)? != self.length {
                return Err(corrupt("archive extent changed outside its owner"));
            }
            Ok(())
        })();
        if check.is_err() {
    ...
    pub fn summary(&mut self, c: &OperationContext) -> Result<ProgressArchiveSummary> {
        let budget = Budget::new(c)?;
        self.access(c)?;
        budget.check()?;
        Ok(ProgressArchiveSummary {
            archive_id: self.id,
            head: self.head,
            fortress_id: self.fortress,
            records: self.entries.len(),
            segments: self.last.as_ref().map_or(0, |r| r.entry.segment),
            retained_bytes: self.length,
            authority_tick_floor: self.tick_floor,
    ...
    pub fn reserve_capture(&mut self, c: &OperationContext) -> Result<()> {
        let mut budget = Budget::new(c)?;
        self.access(c)?;
        self.authority(c, true)?;
        if self.entries.len() >= MAX_ARCHIVE_RECORDS
            || self.length + MAX_FRAME_BYTES > MAX_ARCHIVE_BYTES
        {
            return Err(exhausted());
        }
    ...
    pub fn append(
        ...
        c: &OperationContext,
    ) -> Result<ProgressArchiveEntry> {
        let mut budget = Budget::new(c)?;
        self.reserve_capture(c)?;
        let mut fresh = c.clone();
        fresh.anchor.tick = GameTick(c.anchor.tick.get().max(observation.tick()));
        self.authority(&fresh, true)?;
        if observation.rows().len() > c.budget.max_entities as usize {
            return Err(exhausted());
        }
        let observation =
            ProgressObservation::decode(observation.canonical_bytes(), &observation.ids())?;
        validate_manifest(manifest, &observation)?;
        if observation.fortress_id() != self.fortress {
            return Err(corrupt("capture crosses archive fortress lineage"));
        }
        let segment = match &self.last {
            None => 1,
            Some(old) => {
                // Reopen always starts a segment; no downtime continuity is invented.
                let reset = self.new_segment
                    || old.manifest != *manifest
                    || compare(Some(&old.observation), &observation)?.status == "reset";
                old.entry
                    .segment
                    .checked_add(u64::from(reset))
                    .ok_or_else(exhausted)?
            }
        };
        ...
        let entry = ProgressArchiveEntry {
            number,
            segment,
            ...
            native_order_ids: observation.ids(),
            offset: self.length,
            body_bytes: body.len() as u32,
        };
        let record = ArchivedProgress {
            entry: entry.clone(),
        ...
        let result = (|| {
            ...
            {
                return Err(corrupt(
                    "archive write did not retain the exact complete frame",
                ));
            }
            budget.check()?;
            Ok(())
        ...
        self.length += bytes.len() as u64;
        self.head = digest;
        self.tick_floor = self.tick_floor.max(entry.game_tick);
    ...
    pub fn latest(&mut self, c: &OperationContext) -> Result<Option<ArchivedProgress>> {
        ...
        identity
            .map(|(number, digest)| self.record(number, digest, c))
            .transpose()
    }
    pub fn record(
        &mut self,
        ...
        let index = number
            ...
            .ok_or_else(|| error(ErrorCode::InvalidRequest, "invalid archive record number"))?;
        let expected = self
            .entries
            .get(index)
            .ok_or_else(|| error(ErrorCode::InvalidRequest, "archive record is not retained"))?
            .clone();
        if expected.native_order_ids.len() > c.budget.max_entities as usize {
            return Err(exhausted());
        }
        if expected.record_digest != digest {
            return Err(error(
                ErrorCode::StaleAnchor,
                "archive record digest does not match",
            ));
        }
        ...
        let result = (|| {
            ...
            if record.entry != expected {
                return Err(corrupt("archive record changed after replay"));
            }
            self.access(c)?;
            budget.check()?;
            Ok(record)
        ...
        if result.as_ref().is_err_and(|e| {
            matches!(
                e.code,
                ErrorCode::CorruptLedger | ErrorCode::AdapterRejected
            )
        }) {
            self.fenced = true;
        }
    ...
    pub fn page(
        ...
        limit: usize,
        c: &OperationContext,
    ) -> Result<ProgressArchivePage> {
        let mut budget = Budget::new(c)?;
        self.access(c)?;
        if expected_head != self.head {
            return Err(error(
                ErrorCode::StaleAnchor,
                "archive changed; restart history discovery",
            ));
        }
        if !(1..=64).contains(&limit) || limit > c.budget.max_entities as usize {
            return Err(exhausted());
        }
        ...
        if start > self.entries.len() {
            return Err(error(
                ...
                "history offset exceeds retained records",
            ));
        }
        let count = limit.min(self.entries.len() - start);
        budget.charge(count as u64 * 512)?;
    ...
    pub fn compare_records(
        ...
        if before.0 >= after.0 {
            return Err(error(
                ErrorCode::InvalidRequest,
                "history comparison requires strictly ordered distinct records",
            ));
        }
        ...
        if a.entry.segment != b.entry.segment {
            return Err(error(
                ErrorCode::StaleAnchor,
                "history comparison crosses a restart, selection or source discontinuity",
            ));
        }
        let comparison = compare(Some(&a.observation), &b.observation)?;
        if comparison.status != "compared" {
            return Err(corrupt("history segment contradicts endpoint continuity"));
        }
        self.access(c)?;
        budget.check()?;
        Ok((a, b, comparison))
    ...
    fn validate_next(&self, next: &ArchivedProgress) -> Result<()> {
        if next.entry.number != self.entries.len() as u64 + 1
            || next.entry.previous_digest != self.head
            || next.observation.fortress_id() != self.fortress
        {
            return Err(corrupt(
                "archive record order, predecessor or fortress mismatch",
            ));
        }
        validate_manifest(&next.manifest, &next.observation)?;
        match &self.last {
            None if next.entry.segment != 1 => {
                return Err(corrupt("archive first segment must be one"));
            }
            Some(old) => {
                if next.entry.segment == old.entry.segment {
                    if next.manifest != old.manifest
                        || compare(Some(&old.observation), &next.observation)?.status != "compared"
                    {
    ...
    fn read_frame(&mut self, offset: u64, extent: u64) -> Result<ArchivedProgress> {
        if extent.saturating_sub(offset) < PREFIX_BYTES as u64 + 40 {
            return Err(corrupt("incomplete progress archive tail; bytes unchanged"));
        }
        self.storage
        ...
        if r.take(8)? != FRAME {
            return Err(corrupt("invalid progress archive frame"));
        }
        let size = r.u32()?;
        let number = r.u64()?;
        let previous_digest = Digest32::from_bytes(r.array()?);
        ...
        self.storage.read_exact(&mut body).map_err(storage_error)?;
        let mut trailer = [0; 40];
        self.storage
            .read_exact(&mut trailer)
            .map_err(storage_error)?;
        let mut r = Reader { bytes: &trailer };
        let record_digest = Digest32::from_bytes(r.array()?);
        if r.take(8)? != FOOTER || record_digest != frame_hash(self.id, &prefix, &body) {
            return Err(corrupt("progress record checksum or footer mismatch"));
        }
        let (segment, manifest, observation) = decode_body(&body)?;
        Ok(ArchivedProgress {
            entry: ProgressArchiveEntry {
                number,
                segment,
                record_digest,
                previous_digest,
...
fn validate_manifest(m: &ProgressManifest, o: &ProgressObservation) -> Result<()> {
    if m.generation != o.generation()
        || [&m.df_version, &m.dfhack_version]
            .iter()
...
fn header_hash(data: &[u8]) -> Digest32 {
    let mut bytes = b"dfmcp-progress-archive-header/1\0".to_vec();
    bytes.extend_from_slice(data);
    Digest32::of_bytes(&bytes)
}
fn frame_hash(id: Digest32, prefix: &[u8], body: &[u8]) -> Digest32 {
    let mut bytes = b"dfmcp-progress-archive-frame/1\0".to_vec();
...
fn encode_body(segment: u64, m: &ProgressManifest, o: &ProgressObservation) -> Result<Vec<u8>> {
    ...
    let mut out = segment.to_be_bytes().to_vec();
    out.extend_from_slice(&m.generation.to_be_bytes());
    text(&mut out, &m.df_version);
    text(&mut out, &m.dfhack_version);
    let ids = o.ids();
    out.push(ids.len() as u8);
    for id in ids {
        out.extend_from_slice(&id.to_be_bytes());
    }
    out.extend_from_slice(&(o.canonical_bytes().len() as u32).to_be_bytes());
    out.extend_from_slice(o.canonical_bytes());
    if out.len() > MAX_BODY {
        return Err(exhausted());
    }
    Ok(out)
...
fn decode_body(bytes: &[u8]) -> Result<(u64, ProgressManifest, ProgressObservation)> {
    ...
    if segment == 0 || !(1..=super::MAX_TARGETS).contains(&count) {
        return Err(corrupt("invalid progress archive selection or segment"));
    }
    ...
    if !r.bytes.is_empty() {
        return Err(corrupt("trailing progress archive payload"));
    }
    validate_manifest(&manifest, &observation)?;
    Ok((segment, manifest, observation))
}
