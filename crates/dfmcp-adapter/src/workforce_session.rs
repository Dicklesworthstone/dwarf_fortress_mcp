...
use crate::workforce_control::{AssignmentPhase, AssignmentPlan, AssignmentSpec, WorkforceCapture, validate_ids};
use crate::workforce_control::journal::{
    AssignmentRecord, AssignmentState, WorkforceBinding, WorkforceJournal, WorkforceMode, WorkforceView,
};
use crate::workforce_control::rpc::{CONNECT_BYTES, WorkforceSource, authorize};

fn exhausted() -> dfmcp_core::DfmcpError {
    error(ErrorCode::BudgetExceeded, "workforce session work allowance exhausted")
}
fn stale() -> dfmcp_core::DfmcpError {
    error(ErrorCode::StaleAnchor, "observe the selected citizens and use that exact workforce witness")
}
...
pub fn view_cost(view: &WorkforceView) -> u64 {
    view.bytes as u64 + view.records.iter().map(|record| {
        record.plan().canonical_bytes().len() as u64
            + record.effect().map_or(0, |effect| effect.canonical_bytes().len() as u64)
    }).sum::<u64>()
}
...
impl Allowance {
    fn new(context: OperationContext) -> Result<Self> {
        ...
        Ok(Self { bytes: context.budget.max_bytes, context, deadline })
    }
    fn current(&self) -> Result<OperationContext> {
        ...
        if millis == 0 || self.bytes == 0 { return Err(exhausted()); }
    ...
    fn charge(&mut self, bytes: u64) -> Result<()> {
        self.bytes = self.bytes.checked_sub(bytes).filter(|left| *left > 0).ok_or_else(exhausted)?;
        self.current()?;
        Ok(())
    }
    fn connect<N, F>(&mut self, binding: &WorkforceBinding, factory: F) -> Result<N>
    where F: FnOnce(&WorkforceBinding, &OperationContext) -> Result<N> {
...
impl<S: EffectJournalStorage> WorkforceSession<S> {
    ...
    pub fn new(mut journal: WorkforceJournal<S>, context: &OperationContext) -> Result<(Self, WorkforceView)> {
        let view = journal.view(context)?;
        let high_tick = view.records.iter().map(|r| r.plan().before().tick())
            .max().unwrap_or(context.anchor.tick.get()).max(context.anchor.tick.get());
        authorize(context, view.binding.fortress(), high_tick, false)?;
        let session = Self { binding: view.binding.clone(), journal, id: context.session_id,
            high_tick, selected: None };
        Ok((session, view))
    }
    pub fn binding(&self) -> &WorkforceBinding { &self.binding }
    pub fn mode(&self) -> WorkforceMode { self.journal.mode() }
    ...
    fn context(&self, context: &OperationContext, write: bool) -> Result<OperationContext> {
        if context.session_id != self.id || (write && self.mode() != WorkforceMode::Control) {
            return Err(error(ErrorCode::CapabilityDenied, "workforce session identity or fixed recovery mode denies control"));
        }
        let mut current = context.clone();
        current.anchor.tick = GameTick(self.high_tick.max(current.anchor.tick.get()));
        authorize(&current, self.binding.fortress(), current.anchor.tick.get(), write)?;
        Ok(current)
    }
    ...
    fn view_with(&mut self, budget: &mut Allowance) -> Result<WorkforceView> {
        ...
        for record in &view.records { self.high_tick = self.high_tick.max(record.plan().before().tick()); }
    ...
    fn record(view: &WorkforceView, key: &str, digest: Digest32) -> Result<AssignmentRecord> {
        validate_key(key)?;
        let record = view.records.iter().find(|r| r.plan().key() == key)
            .ok_or_else(|| error(ErrorCode::InvalidRequest, "assignment key absent from this journal"))?;
        if record.plan().digest() != digest {
            return Err(error(ErrorCode::Conflict, "assignment key and reviewed digest disagree"));
        }
        Ok(record.clone())
    }
    pub fn inspect(&mut self, key: &str, digest: Digest32, context: &OperationContext) -> Result<AssignmentRecord> {
        Self::record(&self.view(context)?, key, digest)
    }

    pub fn observe<N, F>(&mut self, ids: &[u32], context: &OperationContext, factory: F) -> Result<WorkforceCapture>
    where N: WorkforceSource, F: FnOnce(&WorkforceBinding, &OperationContext) -> Result<N> {
        self.clear_selection(); // Includes invalid, unauthorized and failed refreshes.
        let mut budget = Allowance::new(self.context(context, false)?)?;
        if self.mode() == WorkforceMode::Offline {
            return Err(error(ErrorCode::CapabilityDenied, "offline workforce recovery cannot acquire native state"));
        }
        validate_ids(ids)?;
        ...
        if capture.tick() < self.high_tick { return Err(stale()); }
    ...
    pub fn prepare<N, F>(&mut self, key: &str, spec: AssignmentSpec, witness: Digest32,
        context: &OperationContext, factory: F) -> Result<AssignmentRecord>
    where N: WorkforceSource, F: FnOnce(&WorkforceBinding, &OperationContext) -> Result<N> {
        validate_key(key)?;
        ...
        if let Some(old) = view.records.iter().find(|r| r.plan().key() == key) {
            if old.plan().spec() != spec || old.plan().before().witness() != witness {
                return Err(error(ErrorCode::Conflict, "assignment key already binds another specification or witness"));
            }
            return Ok(old.clone());
        }
        if view.records.iter().any(|r| !r.state().settled()) {
            return Err(error(ErrorCode::EffectIndeterminate, "unsettled workforce work blocks a new key"));
        }
        let before = self.selected.as_ref().filter(|c| c.witness() == witness).cloned().ok_or_else(stale)?;
    ...
    pub fn commit<N, F>(&mut self, key: &str, digest: Digest32, confirmed: bool,
        context: &OperationContext, factory: F) -> Result<AssignmentRecord>
    where N: WorkforceSource, F: FnOnce(&WorkforceBinding, &OperationContext) -> Result<N> {
        let mut budget = Allowance::new(self.context(context, true)?)?;
        if !confirmed {
            return Err(error(ErrorCode::CapabilityDenied, "assignment commit requires explicit confirmation of its reviewed digest"));
        }
        ...
        if record.state() != AssignmentState::Prepared {
            return Err(error(ErrorCode::EffectIndeterminate, "assignment dispatch cannot be retried; inspect or reconcile instead"));
        }
        self.clear_selection();
        let mut source = budget.connect(&self.binding, factory)?;
        ...
        self.journal.commit(&mut source, key, digest, &budget.current()?)
    }

    pub fn reconcile<N, F>(&mut self, key: &str, digest: Digest32,
        context: &OperationContext, factory: F) -> Result<AssignmentRecord>
    where N: WorkforceSource, F: FnOnce(&WorkforceBinding, &OperationContext) -> Result<N> {
        let mut budget = Allowance::new(self.context(context, false)?)?;
        let record = Self::record(&self.view_with(&mut budget)?, key, digest)?;
        if !record.needs_query() { return Ok(record); }
        if self.mode() == WorkforceMode::Offline {
            return Err(error(ErrorCode::CapabilityDenied, "offline uncertainty requires explicit recover-mode reopening"));
        }
        self.clear_selection();
        let mut source = budget.connect(&self.binding, factory)?;
        self.journal.reconcile(&mut source, key, digest, &budget.current()?)
    }

    pub fn cancel<N, F>(&mut self, key: &str, digest: Digest32,
        context: &OperationContext, factory: F) -> Result<AssignmentRecord>
    where N: WorkforceSource, F: FnOnce(&WorkforceBinding, &OperationContext) -> Result<N> {
        let mut budget = Allowance::new(self.context(context, true)?)?;
        let record = Self::record(&self.view_with(&mut budget)?, key, digest)?;
        ...
        if record.state().settled()
            || record.effect().is_some_and(|e| e.phase() == AssignmentPhase::Unknown) {
        ...
        if matches!(record.state(), AssignmentState::Intent | AssignmentState::Prepared) {
            return self.journal.cancel::<N>(None, key, digest, &budget.current()?);
        }
        let mut source = budget.connect(&self.binding, factory)?;
        self.journal.cancel(Some(&mut source), key, digest, &budget.current()?)
    }
}
