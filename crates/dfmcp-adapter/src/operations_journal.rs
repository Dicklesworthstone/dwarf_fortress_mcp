...
fn storage_error(_: io::Error) -> DfmcpError { corrupt("observation journal I/O failed; reopen for verified recovery") }
...
impl JournalLimits {
    fn validate(self) -> Result<()> {
        if !(HEADER_BYTES as u64..=MAX_JOURNAL_BYTES).contains(&self.max_bytes)
            ...
            return Err(budget("observation journal limits exceed the implementation bounds"));
        }
        Ok(())
    }
}
...
fn check(context: &OperationContext, started: Instant) -> Result<()> {
    ...
    if started.elapsed().as_millis() > u128::from(context.budget.max_wall_millis) {
        return Err(budget("observation journal replay exceeded its wall-time budget"));
    }
    Ok(())
}
...
impl<S: JournalStorage, P: JournalProfile> ObservationJournal<S, P> {
    ...
    pub fn open(mut storage: S, context: &OperationContext, limits: JournalLimits,
        ...
        if length > limits.max_bytes { return Err(budget("observation journal exceeds its retained byte limit")); }
        ...
        if length < HEADER_BYTES as u64 { return Err(corrupt("journal header is incomplete; original file is unchanged")); }
        storage.seek(SeekFrom::Start(0)).map_err(storage_error)?;
        ...
        while journal.length < length {
            check(context, started)?;
            if journal.entries.len() >= limits.max_records { return Err(budget("journal exceeds its record limit")); }
        ...
        if journal.length != length {
            if recovery != TailRecovery::TruncateIncomplete {
                return Err(corrupt("incomplete journal tail; operator repair opt-in required; original file is unchanged"));
            }
            context.authorize(Capability::Observe, RiskTier::ReadOnly, &[], None)?;
            check(context, started)?;
    ...
    fn accept_replayed(&mut self, entry: JournalEntry, observation: P::Observation,
        context: &OperationContext) -> Result<()> {
        if entry.number != self.entries.len() as u64 + 1 || entry.previous_digest != self.head
            || entry.anchor.fortress_id != self.fortress || P::source_digest(&observation)? != entry.source_digest {
            return Err(corrupt("journal sequence, predecessor, fortress or source digest disagrees"));
        }
        check_observation::<P>(&observation, context)?;
        let outcome = P::publish(&mut self.state, observation)?;
        if outcome == JobPublication::Heartbeat || P::snapshot(&self.state).map(WorldSnapshot::anchor) != Some(entry.anchor) {
            return Err(corrupt("journal does not reproduce its recorded projection anchor"));
        }
    ...
    pub fn append(&mut self, observation: P::Observation, context: &OperationContext) -> Result<JobPublication> {
        let started = Instant::now();
        check(context, started)?;
        context.authorize(Capability::Observe, RiskTier::ReadOnly, &[], None)?;
        self.ensure_healthy(context)?;
        check_observation::<P>(&observation, context)?;
        if P::fortress(&observation)? != self.fortress { return Err(corrupt("journal observation has another fortress")); }
        let mut candidate = self.state.clone();
        let outcome = P::publish(&mut candidate, observation.clone())?;
        if outcome == JobPublication::Heartbeat { return Ok(outcome); }
        let anchor = P::snapshot(&candidate).map(WorldSnapshot::anchor).ok_or_else(|| corrupt("journal candidate snapshot missing"))?;
        ...
        if write_result.is_err() {
            self.fenced = true;
            return Err(corrupt("journal append outcome uncertain; no new in-memory anchor published; reopen for verified recovery"));
        }
    ...
    fn ensure_healthy(&mut self, context: &OperationContext) -> Result<()> {
        context.authorize(Capability::Query, RiskTier::ReadOnly, &[], None)?;
        if self.fenced { return Err(corrupt("journal is fenced; reopen for verified recovery")); }
        ...
        if self.storage.validate_identity().is_err()
            || self.storage.seek(SeekFrom::End(0)).map_or(true, |size| size != self.length) {
            self.fenced = true;
    ...
    pub fn state_at(&mut self, number: u64, digest: Digest32, context: &OperationContext) -> Result<P::State> {
        let result = self.replay_at(number, digest, context);
        if matches!(&result, Err(error) if error.code == ErrorCode::CorruptLedger) { self.fenced = true; }
        result
    }

    fn replay_at(&mut self, number: u64, digest: Digest32, context: &OperationContext) -> Result<P::State> {
        let started = Instant::now();
        check(context, started)?;
        self.ensure_healthy(context)?;
    ...
    pub fn snapshot_at(&mut self, number: u64, digest: Digest32, context: &OperationContext) -> Result<WorldSnapshot> {
        ...
        P::snapshot(&state).cloned().ok_or_else(|| corrupt("historical snapshot absent"))
    }
    pub fn entries(&self) -> &[JournalEntry] { &self.entries }
    pub fn state(&self) -> &P::State { &self.state }
    pub fn profile(&self) -> &'static str { P::NAME }
...
fn check_observation<P: JournalProfile>(observation: &P::Observation, context: &OperationContext) -> Result<()> {
    if P::entity_count(observation) > context.budget.max_entities as usize
        ...
        return Err(budget("archived observation exceeds the current session's acquisition limits"));
    }
    Ok(())
}
...
fn decode_header<P: JournalProfile>(bytes: &[u8; HEADER_BYTES]) -> Result<(FortressId, Digest32, Digest32)> {
    let mut r = Reader(bytes);
    if r.take(8)? != P::MAGIC { return Err(corrupt("journal profile differs from the selected runtime; no migration or repair applied")); }
...
fn encode_profile_frame<P: JournalProfile>(id: Digest32, number: u64, previous: Digest32,
    anchor: StateAnchor, observation: &P::Observation) -> Result<Vec<u8>> {
    ...
    if body.len() > P::MAX_PAYLOAD + 1024 { return Err(budget("journal frame exceeds its bound")); }
...
impl<'a> Reader<'a> {
    fn take(&mut self, n: usize) -> Result<&'a [u8]> {
        let out = self.0.get(..n).ok_or_else(|| corrupt("truncated journal record"))?;
        self.0 = &self.0[n..];
        Ok(out)
    }
    ...
    fn text(&mut self) -> Result<String> {
        ...
        if text.contains('\0') { return Err(corrupt("journal manifest contains NUL")); }
        Ok(text.to_owned())
    }
}
...
#[cfg(test)]
fn encode_frame(id: Digest32, number: u64, previous: Digest32, anchor: StateAnchor,
    observation: &LiveOperationsObservation) -> Result<Vec<u8>> {
