...
fn malformed() -> DfmcpError {
    error(
        ...
        "malformed order-progress/1.11 RPC",
    )
}
fn io_error(_: io::Error) -> DfmcpError {
    error(
        ErrorCode::AdapterUnavailable,
        "order-progress read failed; connection fenced",
    )
}
fn until(timeout: Duration) -> Result<Instant> {
    if !(Duration::from_millis(1)..=Duration::from_secs(60)).contains(&timeout) {
        return Err(error(
            ...
            "progress wall allowance must be 1..60000ms",
        ));
    }
...
fn left(deadline: Instant) -> Result<Duration> {
    deadline
        ...
        .ok_or_else(|| {
            error(
                ErrorCode::BudgetExceeded,
                "progress read deadline exhausted",
            )
        })
}
fn credentials(token: &[u8], nonce: &[u8]) -> Result<()> {
    if !(32..=256).contains(&token.len()) || !(16..=64).contains(&nonce.len()) {
        return Err(error(
            ...
            "invalid progress credential or nonce size",
        ));
    }
    Ok(())
}
...
fn read_varint(data: &mut &[u8]) -> Result<u64> {
    let mut n = 0;
    for shift in 0..10 {
        let (&b, rest) = data.split_first().ok_or_else(malformed)?;
        *data = rest;
        if shift == 9 && b > 1 {
...
impl<'a> Message<'a> {
    fn parse(mut data: &'a [u8], maximum: u32) -> Result<Self> {
        ...
        while !data.is_empty() {
            ...
            if id == 0 || id > maximum || fields.contains_key(&id) {
                return Err(malformed());
            }
            let value = match tag & 7 {
                ...
                2 => {
                    ...
                    let value = data.get(..len).ok_or_else(malformed)?;
                    data = &data[len..];
                    Field::Bytes(value)
                }
    ...
    loop {
        ...
        if kind == -2 {
            return Err(error(
                ...
                "DFHack refused progress read",
            ));
        }
        if !matches!(kind, -1 | -3) {
            return Err(malformed());
        }
        let size = usize::try_from(i32::from_le_bytes([h[4], h[5], h[6], h[7]]))
            .map_err(|_| malformed())?;
        if kind == -3 {
            notifications += 1;
            if notifications > 8 || size > 65_536 || size > 262_144 - text_bytes {
                return Err(error(
                    ErrorCode::BudgetExceeded,
                    "progress notification bound exceeded",
                ));
            }
            text_bytes += size;
        } else if size > MAX_FRAME {
            return Err(error(
                ErrorCode::BudgetExceeded,
                "progress reply exceeds 4KiB",
            ));
        }
...
fn bind<S: ProgressStream>(stream: &mut S, name: &str) -> Result<i16> {
    ...
    for (id, text) in [(1, name), (2, REQUEST), (3, REPLY), (4, PLUGIN)] {
        bytes(&mut request, id, text.as_bytes());
    }
...
fn decode(
    ...
    nonce: &[u8],
    observation: bool,
) -> Result<(ProgressManifest, Option<Vec<u8>>)> {
    let m = Message::parse(data, 9)?;
    if m.number(4)? != 1 || m.number(5)? != 11 {
        return Err(error(
            ErrorCode::VersionMismatch,
            "progress protocol must be exactly 1.11",
        ));
    }
    ...
    let df_version = m.text(7)?;
    let dfhack_version = m.text(8)?;
    if accepted == 0 {
    ...
    if accepted != 1
        ...
        || df_version.is_empty()
        || dfhack_version.is_empty()
        || m.0.contains_key(&9) != observation
    {
        return Err(malformed());
    }
    let payload = if observation {
        Some(m.bytes(9, super::MAX_PROGRESS_BYTES)?.to_vec())
    } else {
        None
    };
    Ok((
...
impl<S: ProgressStream> ProgressClient<S> {
    pub fn negotiate(stream: S, token: Vec<u8>, nonce: Vec<u8>, timeout: Duration) -> Result<Self> {
        ...
        Self::negotiate_until(stream, token, nonce, until(timeout)?)
    }
    fn negotiate_until(
        mut stream: S,
        token: Vec<u8>,
        ...
        left(deadline)?;
        stream.set_deadline(deadline).map_err(io_error)?;
        ...
        left(deadline)?;
        let read_method = bind(&mut stream, "ReadOrderProgress")?;
        left(deadline)?;
        if handshake == read_method {
            return Err(malformed());
        }
        let (manifest, _) = decode(
            &call(&mut stream, handshake, &Self::base(&token, &nonce))?,
            &nonce,
            false,
        )?;
        left(deadline)?;
    ...
    pub fn manifest(&self) -> &ProgressManifest {
        &self.manifest
    }
    ...
    pub fn read_order(&mut self, order: u32, timeout: Duration) -> Result<OrderProgress> {
        if self.fenced {
            return Err(error(
                ErrorCode::AdapterUnavailable,
                "progress connection fenced; close and reopen explicitly",
            ));
        }
        ...
        number(&mut request, 5, u64::from(order));
        let result = (|| {
            self.stream.set_deadline(deadline).map_err(io_error)?;
            let (manifest, payload) = decode(
                &call(&mut self.stream, self.read_method, &request)?,
                &self.nonce,
                true,
            )?;
            if manifest != self.manifest {
                return Err(error(
                    ...
                    "progress incarnation or software changed",
                ));
            }
        ...
        match &result {
            Ok(o) => self.sequence = o.sequence(),
            Err(_) => self.fence(),
        }
        result
    }
}
pub struct ProgressTcpStream {
    stream: TcpStream,
    deadline: Instant,
}
