...
use crate::live_jobs::{LiveJobObservation, MAX_JOB_FRAME_BYTES, MAX_JOBS};
...
fn failure(code: ErrorCode, text: &str) -> DfmcpError {
    DfmcpError::new(code, text)
}
fn malformed() -> DfmcpError {
    failure(
        ErrorCode::AdapterRejected,
        "invalid jobs/1.2 protobuf reply",
    )
}
fn io_failure(_: io::Error) -> DfmcpError {
    ...
    failure(
        ...
        "jobs RPC I/O failed or exceeded its deadline",
    )
}
...
impl<'a> Message<'a> {
    fn parse(input: &'a [u8], maximum_field: u32) -> Result<Self> {
        if input.len() > MAX_RPC {
            return Err(failure(
                ErrorCode::BudgetExceeded,
                "jobs RPC payload too large",
            ));
        }
        let mut offset = 0;
...
fn call<S: Read + Write>(
    ...
    for _ in 0..9 {
        ...
        if length > ceiling {
            return Err(failure(
                ErrorCode::BudgetExceeded,
                "jobs RPC reply exceeds byte budget",
            ));
        }
        if id == -3 {
    ...
    Err(failure(
        ErrorCode::BudgetExceeded,
        "too many jobs RPC text notifications",
    ))
}
...
fn decode_reply<'a>(
    ...
    if message.number(4)? != 1 || message.number(5)? != 2 {
        return Err(failure(
            ...
            "jobs bridge must implement exactly profile 1.2",
        ));
    }
    ...
    if accepted == 0 {
        if code == 0 || message.0.contains_key(&9) {
            return Err(malformed());
        }
        return Err(failure(
            match code {
    ...
    let manifest = JobsManifest {
        generation,
        df_version: message.text(7)?,
        dfhack_version: message.text(8)?,
    };
    let payload = if observation {
        Some(message.bytes(9, MAX_JOB_FRAME_BYTES)?)
    } else if message.0.contains_key(&9) {
        return Err(malformed());
    } else {
        None
    };
...
impl<S: Read + Write> JobsRpcClient<S> {
    pub fn negotiate(
        ...
        if !(32..=256).contains(&token.len())
            ...
            || max_jobs == 0
            || max_jobs as usize > MAX_JOBS
            || !(1024..=MAX_RPC).contains(&max_bytes)
        {
            return Err(failure(
                ErrorCode::InvalidRequest,
                "invalid jobs bootstrap credentials or budgets",
            ));
        }
    ...
    #[must_use]
    pub fn manifest(&self) -> &JobsManifest {
        &self.manifest
    }
    #[must_use]
    ...
    pub fn read_observation(&mut self) -> Result<LiveJobObservation> {
        if self.poisoned {
            return Err(failure(
                ErrorCode::AdapterUnavailable,
                "jobs connection is fenced; reopen session",
            ));
        }
        let result = (|| {
            ...
            if manifest.df_version != self.manifest.df_version
                || manifest.dfhack_version != self.manifest.dfhack_version
                || manifest.generation < self.manifest.generation
            {
                return Err(failure(
                    ErrorCode::StaleAnchor,
                    "jobs bridge software or generation regressed",
                ));
            }
            let observation = LiveJobObservation::decode_payload(
                payload.ok_or_else(malformed)?,
                manifest.generation,
            ...
            if observation.jobs.len() > self.max_jobs as usize {
                return Err(failure(
                    ErrorCode::BudgetExceeded,
                    "jobs bridge exceeded requested roster bound",
                ));
            }
...
fn request(token: &[u8], nonce: &[u8], max_jobs: u32) -> Vec<u8> {
    ...
    number(&mut out, 3, 1);
    number(&mut out, 4, 2);
    number(&mut out, 5, u64::from(max_jobs));
    out
}
...
fn checked_timeout(timeout: Duration) -> Result<Duration> {
    if timeout < Duration::from_millis(1) || timeout > Duration::from_secs(60) {
        return Err(failure(
            ErrorCode::BudgetExceeded,
            "jobs call deadline must be 1..60000 milliseconds",
        ));
    }
    Ok(timeout)
}
    ...
    pub fn connect(
        ...
        if !endpoint.ip().is_loopback() || endpoint.port() == 0 {
            return Err(failure(
                ...
                "jobs endpoint must be numeric loopback with a nonzero port",
            ));
        }
...
mod tests {
    ...
    fn manifest_nonce_and_method_payload_are_not_interchangeable() -> Result<()> {
        assert_eq!(
            decode_reply(&reply(None), &[b'n'; 16], false)?.0.generation,
            7
        );
    ...
    fn authenticated_bootstrap_and_read_execute_real_wire_decoder() -> Result<()> {
        ...
        for id in [2, 3] {
            ...
            number(&mut bind, 1, id);
            input.extend(frame(&bind));
        }
        input.extend(frame(&reply(None)));
        input.extend(frame(&reply(Some(&payload))));
        let mut client = JobsRpcClient::negotiate(
            Script {
    ...
    fn oversized_and_negative_reply_lengths_fail_before_allocating_payload() {
        for length in [-1, (MAX_RPC + 1) as i32] {
            let mut script = Script {
                input: Cursor::new(header(-1, length).to_vec()),
                output: Vec::new(),
            };
