...
use dfmcp_world::map_region::{Cell, MAX_ROUTE_WORK, MapRegion};
...
impl Work {
    fn new(context: &OperationContext, maximum: u64) -> Result<Self> {
        if maximum == 0 || maximum > flow::MAX_WORK {
            return Err(exhausted("invalid workforce work allowance"));
        }
        Ok(Self {
            used: 0,
            maximum,
            started: Instant::now(),
            wall_millis: context.budget.max_wall_millis,
        })
    }
    fn charge(&mut self, units: u64) -> Result<()> {
        self.used = self
            .used
            .checked_add(units)
            .ok_or_else(|| exhausted("workforce work counter overflow"))?;
        if self.used > self.maximum
            || self.started.elapsed().as_millis() > u128::from(self.wall_millis)
        {
            return Err(exhausted(
                "workforce analysis exceeded its work or cooperative wall-time allowance",
            ));
        }
        Ok(())
    }
...
fn normalize(requested: &[WorkforceDemand]) -> Result<Vec<WorkforceDemand>> {
    ...
    for demand in requested {
        if demand.key.is_empty()
            ...
            || demand.workers == 0
            || demand.workers > MAX_WORKER_SLOTS
            || demand.skill_key.is_empty()
            || demand.skill_key.len() > 96
            || demand.skill_key.chars().any(char::is_control)
            || demand.min_effective_skill < 0
            || demand.target.iter().any(|&n| n >= 32_768)
        {
            return Err(invalid(
                "invalid workforce key, worker count, target or skill selector",
            ));
        }
        total = total
            .checked_add(demand.workers)
            .ok_or_else(|| exhausted("worker count overflow"))?;
    }
    if total > MAX_WORKER_SLOTS {
        return Err(exhausted(
            "workforce request exceeds 128 simultaneous worker slots",
        ));
    }
    let mut result = requested.to_vec();
    result.sort_by(|a, b| a.key.cmp(&b.key));
    if result.windows(2).any(|pair| pair[0].key == pair[1].key) {
        return Err(invalid("duplicate workforce demand key"));
    }
    Ok(result)
}
...
fn approach(
    ...
    field: &Reachability,
    position: [i32; 3],
) -> Option<([u32; 3], u32, bool)> {
    if position.iter().any(|&n| n < 0) {
        return None;
    }
    let p = [position[0] as u32, position[1] as u32, position[2] as u32];
    let index = map.region.index(p)?;
    if let Some(distance) = field.distance_to(p) {
        return Some((p, distance, false));
    }
    let Cell::Visible(mut tile) = *map.cells.get(index)? else {
        return None;
    };
    if tile.unit_occupancy == 0 {
        return None;
    }
    ...
    for axis in 0..2 {
        for up in [false, true] {
            let mut next = p;
            next[axis] = match if up {
                p[axis].checked_add(1)
            } else {
                p[axis].checked_sub(1)
            } {
                Some(value) => value,
                None => continue,
            };
            let Some(distance) = field.distance_to(next).and_then(|n| n.checked_add(1)) else {
                continue;
            };
...
fn analyze_inner(
    ...
    work: &mut Work,
) -> Result<WorkforceAnalysis> {
    context.authorize(Capability::Query, RiskTier::ReadOnly, &[], None)?;
    let demands = normalize(requested)?;
    let snapshot = state
        .snapshot()
        .ok_or_else(|| invalid("workforce snapshot is absent"))?;
    if snapshot.anchor() != context.anchor {
        return Err(DfmcpError::new(
            ErrorCode::StaleAnchor,
            "workforce context names another capture",
        ));
    }
    if snapshot.graph.entities.len() > context.budget.max_entities as usize {
        return Err(exhausted("workforce capture exceeds entity allowance"));
    }
    if !snapshot.hash_is_valid() {
        return Err(invariant("workforce snapshot hash is invalid"));
    }
    let observation = state
        .observation_full()
        .ok_or_else(|| invariant("coherent workforce source is absent"))?;
    let map = &observation.spatial().terrain().map;
    for demand in &demands {
        let index = map
            .region
            .index(demand.target)
            .ok_or_else(|| invalid("workforce target lies outside the captured region"))?;
        if !map.candidate(index) {
            return Err(invalid(
                "workforce target must be an observed candidate tile, not an occupied workshop or hidden tile",
            ));
        }
    }
    ...
    for citizen in observation.citizens() {
        ...
        for skill in &citizen.skills {
            work.charge(1)?;
            known_skills.insert(skill.key.as_str());
        }
    }
    let mut fields = BTreeMap::new();
    let mut result = WorkforceAnalysis {
        anchor: snapshot.anchor(),
        source_digest: state.source_digest()?,
        demands,
        candidates: Vec::new(),
        classifications: Vec::new(),
        skill_key_observed: Vec::new(),
        reachable_tiles: Vec::new(),
        touched_region_boundary: Vec::new(),
        work_units: 0,
    };
    for demand in &result.demands {
        ...
        let field = fields
            .get(&demand.target)
            .ok_or_else(|| invariant("workforce route field lost"))?;
        ...
        for (citizen_index, citizen) in observation.citizens().iter().enumerate() {
            work.charge(1)?;
            let reason = if !citizen.alive() || !citizen.sane() || !citizen.active() {
                Some("inactive_or_unavailable_state")
            } else if demand.adults_only && !citizen.adult() {
                Some("not_adult")
            } else if demand.preserve_social && !citizen.job_available_preserve_social {
                Some("not_job_available_preserve_social")
            } else if !demand.preserve_social && !citizen.job_available_interrupt_social {
                Some("not_job_available_interrupt_social")
            } else if !key_known {
                Some("skill_key_not_observed")
            } else {
                None
            };
            ...
            for skill in &citizen.skills {
                work.charge(1)?;
                if skill.key == demand.skill_key {
                    nominal = skill.nominal;
                    effective = skill.effective;
                    experience = skill.experience;
                    break;
                }
            }
            if effective < demand.min_effective_skill {
                *counts.entry("below_minimum_effective_skill").or_insert(0) += 1;
                continue;
            }
            work.charge(5)?;
            let Some((approach, steps, endpoint_step)) = approach(
                map,
                field,
                [citizen.position.x, citizen.position.y, citizen.position.z],
            ) else {
                *counts
                    .entry("no_candidate_approach_in_observed_model")
                    .or_insert(0) += 1;
                continue;
            };
            work.charge(1)?;
            ...
            let entity = snapshot
                ...
                .entities
                .get(&id)
                .ok_or_else(|| invariant("workforce citizen entity is absent"))?;
            rows.push(WorkforceCandidate {
                citizen_index,
                entity_id: id.get(),
        ...
        rows.sort_by(|a, b| {
            b.effective
                .cmp(&a.effective)
                .then_with(|| b.nominal.cmp(&a.nominal))
        ...
        result
            .touched_region_boundary
            .push(field.touched_region_boundary);
    }
    work.charge(0)?;
...
pub fn plan(
    ...
    for (demand, rows) in analysis.candidates.iter().enumerate() {
        for row in rows {
            work.charge(1)?;
            *eligibility.entry(row.entity_id).or_default() |= 1u32 << demand;
        }
    }
    let supplies: Vec<_> = eligibility
        .into_iter()
        .map(|(id, eligible)| Supply {
            id,
            units: 1,
            eligible,
        })
    ...
    let demands: Vec<_> = analysis
        ...
        .collect();
    let allocation =
        flow::allocate(&supplies, &demands, work.remaining()).map_err(|error| match error {
            flow::AllocationError::InvalidInput | flow::AllocationError::InvariantViolation => {
                invariant("workforce allocation model or certificate disagrees")
            }
    ...
    for assignment in &allocation.assignments {
        if assignment.units != 1 || !assigned.insert(assignment.supply_id) {
            return Err(invariant("workforce allocation double-booked a citizen"));
        }
    }
    Ok(WorkforcePlan {
        analysis,
