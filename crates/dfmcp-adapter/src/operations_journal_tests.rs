...
fn context() -> Result<OperationContext> {
    ...
    Ok(OperationContext {
        ...
        budget: WorkBudget {
            ...
            max_entities: 64,
            max_bytes: MAX_OPERATIONS_BYTES as u64,
...
) -> Result<OperationsJournal<Memory>> {
    let new = bytes.is_empty();
    OperationsJournal::open(
        Memory {
...
fn filled() -> Result<(OperationsJournal<Memory>, OperationContext)> {
    let context = context()?;
    let mut j = open(Vec::new(), &context, TailRecovery::Refuse)?;
    j.append(source(1), &context)?;
    j.append(source(2), &context)?;
    Ok((j, context))
}
...
fn synced_reopen_replays_exact_anchors_and_ignores_heartbeats() -> Result<()> {
    let (mut j, c) = filled()?;
    let before = j.entries.clone();
    let root = j.state.snapshot().cloned();
    assert_eq!(j.append(source(2), &c)?, JobPublication::Heartbeat);
    assert_eq!(j.entries, before);
    let mut recovered = open(j.storage.bytes.into_inner(), &c, TailRecovery::Refuse)?;
    assert_eq!(recovered.entries, before);
    assert_eq!(recovered.state.snapshot(), root.as_ref());
    for entry in before {
        assert_eq!(
            recovered
...
fn every_torn_record_prefix_recovers_only_with_explicit_repair() -> Result<()> {
    ...
    for end in second.offset as usize + 1..bytes.len() {
        ...
        assert_eq!(recovered.length, second.offset);
        assert_eq!(recovered.repaired_tail_bytes, end as u64 - second.offset);
        assert_eq!(
            recovered.storage.bytes.get_ref(),
            &bytes[..second.offset as usize]
        );
    }
    Ok(())
}
...
fn partial_headers_and_complete_corrupt_frames_are_not_tail_repairs() -> Result<()> {
    let (j, c) = filled()?;
    let bytes = j.storage.bytes.into_inner();
    for end in 1..HEADER_BYTES {
        assert!(open(bytes[..end].to_vec(), &c, TailRecovery::TruncateIncomplete).is_err());
    }
    for i in 0..bytes.len() {
        let mut corrupt_bytes = bytes.clone();
        corrupt_bytes[i] ^= 1;
        assert!(
            open(corrupt_bytes, &c, TailRecovery::Refuse).is_err(),
            "byte {i}"
        );
    }
    for i in j.entries[1].offset as usize..bytes.len() {
        ...
        corrupt_bytes[i] ^= 1;
        assert!(
            open(corrupt_bytes, &c, TailRecovery::TruncateIncomplete).is_err(),
            "byte {i}"
        );
    }
    Ok(())
...
fn write_and_sync_failures_never_advance_the_visible_root() -> Result<()> {
    ...
    for count in [0, 1, 4, frame.len() / 2, frame.len() - 1] {
        let mut j = open(bytes.clone(), &c, TailRecovery::Refuse)?;
        let anchor = j.state.snapshot().map(WorldSnapshot::anchor);
        j.storage.fail_after = Some(count);
        assert!(j.append(source(3), &c).is_err());
        assert_eq!(j.state.snapshot().map(WorldSnapshot::anchor), anchor);
        assert_eq!(j.entries.len(), 2);
        assert!(j.fenced);
        j.storage.fail_after = None;
        assert!(j.append(source(3), &c).is_err());
    }
    let mut j = open(bytes, &c, TailRecovery::Refuse)?;
...
fn replay_preserves_generation_and_epoch_transitions() -> Result<()> {
    ...
    missing.jobs.jobs.clear();
    j.append(missing, &c)?;
    j.append(source(3), &c)?;
    let mut reset = source(1);
    reset.jobs.bridge_generation = 8;
    j.append(reset, &c)?;
    let original = j.state.snapshot().cloned();
    let recovered = open(j.storage.bytes.into_inner(), &c, TailRecovery::Refuse)?;
    assert_eq!(recovered.state.snapshot(), original.as_ref());
    let root = recovered
        .state
        .snapshot()
        .ok_or_else(|| corrupt("fixture"))?;
    assert_eq!(root.cursor.epoch, 1);
    assert_eq!(
        root.graph.entities[&dfmcp_core::EntityId::new(2)].generation,
        3
    );
    Ok(())
}
...
fn authority_and_capacity_fail_without_writing() -> Result<()> {
    ...
    c.grants.clear();
    assert!(j.snapshot_at(1, j.entries[0].record_digest, &c).is_err());
    let c = context()?;
    let mut j = open(bytes.clone(), &c, TailRecovery::Refuse)?;
    j.limits.max_records = 2;
    assert!(j.append(source(3), &c).is_err());
    assert!(!j.fenced);
    assert_eq!(j.storage.bytes.get_ref(), &bytes);
    j.limits.max_records = 3;
    j.limits.max_bytes = bytes.len() as u64;
    assert!(j.append(source(3), &c).is_err());
    assert_eq!(j.storage.bytes.get_ref(), &bytes);
    Ok(())
}
...
fn historical_access_uses_present_time_authority_and_exact_record_identity() -> Result<()> {
    let (mut j, mut c) = filled()?;
    let entry = j.entries[0].clone();
    assert!(j.snapshot_at(0, entry.record_digest, &c).is_err());
    assert!(j.snapshot_at(9, entry.record_digest, &c).is_err());
    assert!(j.snapshot_at(1, Digest32::ZERO, &c).is_err());
    for g in &mut c.grants {
        g.expires_at_tick = Some(c.anchor.tick);
    }
    c.anchor.tick = GameTick(c.anchor.tick.0 + 1);
    assert!(j.snapshot_at(1, entry.record_digest, &c).is_err());
    Ok(())
}
...
fn incomplete_tail_default_preserves_every_byte() -> Result<()> {
    ...
    assert_eq!(memory.bytes.into_inner(), bytes);
    Ok(())
}
...
fn validly_hashed_wrong_projector_anchor_is_rejected() -> Result<()> {
    let c = context()?;
    let mut j = open(Vec::new(), &c, TailRecovery::Refuse)?;
    let mut bad = c.anchor;
    bad.cursor.sequence = 42;
...
fn real_file_reopen_and_exclusive_lock_require_private_custody() -> Result<()> {
    ...
    j.append(source(1), &c)?;
    assert!(
        open_private_journal(&path, &c, JournalLimits::default(), TailRecovery::Refuse).is_err()
    );
    let expected = j.state.snapshot().cloned();
    drop(j);
    let recovered =
        open_private_journal(&path, &c, JournalLimits::default(), TailRecovery::Refuse)?;
    ...
    std::fs::remove_dir(&dir).map_err(storage_error)?;
    Ok(())
}
