...
impl PrivateWorkOrderFile {
    fn writable(&self) -> io::Result<()> {
        if self.read_only {
            return Err(io::Error::new(io::ErrorKind::PermissionDenied, "offline creation recovery is read-only"));
        }
        self.validate_identity()
    }
}
impl Read for PrivateWorkOrderFile {
    fn read(&mut self, data: &mut [u8]) -> io::Result<usize> { self.file.read(data) }
}
impl Seek for PrivateWorkOrderFile {
    fn seek(&mut self, position: SeekFrom) -> io::Result<u64> { self.file.seek(position) }
}
impl Write for PrivateWorkOrderFile {
    fn write(&mut self, data: &[u8]) -> io::Result<usize> {
        self.writable()?;
        if self.file.stream_position()? != self.extent {
            return Err(io::Error::new(io::ErrorKind::PermissionDenied, "creation storage is append-only"));
        }
...
impl EffectJournalStorage for PrivateWorkOrderFile {
    ...
    fn truncate(&mut self, _: u64) -> io::Result<()> {
        Err(io::Error::new(io::ErrorKind::PermissionDenied, "creation evidence is never truncated"))
    }
    fn validate_identity(&self) -> io::Result<()> {
        #[cfg(unix)] {
            use std::os::unix::fs::MetadataExt;
            let denied = || io::Error::new(io::ErrorKind::PermissionDenied, "creation journal custody changed");
            ...
            if !dir.is_dir() || dir.mode() & 0o7777 != 0o700
                || !named.is_file() || named.mode() & 0o7777 != 0o600
        ...
        #[cfg(not(unix))] {
            Err(io::Error::new(io::ErrorKind::Unsupported, "private creation journals require Unix"))
        }
    }
}
...
pub fn open_private_work_orders(path: &Path, context: &OperationContext,
    mode: JournalMode) -> Result<WorkOrderJournal<PrivateWorkOrderFile>>
{
    authorize(context, context.anchor.fortress_id, false)?;
    if mode == JournalMode::Control { authorize(context, context.anchor.fortress_id, true)?; }
    #[cfg(unix)] {
        ...
        let before = match fs::symlink_metadata(path) {
            Ok(meta) => {
                if !meta.is_file() || meta.mode() & 0o7777 != 0o600
                    || meta.nlink() != 1 || meta.uid() != dir.uid()
                { return Err(denied()); }
                Some(meta)
            }
        ...
        if created && mode != JournalMode::Control {
            return Err(error(ErrorCode::InvalidRequest, "creation recovery requires an existing journal"));
        }
        ...
        file.try_lock().map_err(|_| error(ErrorCode::Conflict, "creation journal already owned or lock unavailable"))?;
        let opened = file.metadata().map_err(storage_error)?;
        if before.as_ref().is_some_and(|meta| (meta.dev(), meta.ino()) != (opened.dev(), opened.ino())) {
            return Err(denied());
        }
        let storage = PrivateWorkOrderFile { file, path: path.to_owned(),
            read_only: mode == JournalMode::Offline, extent: opened.len(),
    ...
    #[cfg(not(unix))] {
        let _ = path;
        Err(error(ErrorCode::CapabilityDenied, "private work-order journals are currently Unix-only"))
    }
}
