...
impl JournalStorage for Memory {
    ...
    fn validate_identity(&self) -> io::Result<()> {
        if self.identity_failed.get() { Err(io::Error::other("injected custody loss")) } else { Ok(()) }
    }
}
fn put(bytes: &mut Vec<u8>, value: u32) { bytes.extend_from_slice(&value.to_be_bytes()); }
fn text(bytes: &mut Vec<u8>, value: &str) {
    bytes.extend_from_slice(&(value.len() as u16).to_be_bytes()); bytes.extend_from_slice(value.as_bytes());
}
fn source(tick: u32, workers: u32, generation: u64) -> Result<LiveSpatialCitizenObservation> {
    ...
    for part in [spatial, citizens] { put(&mut payload, part.len() as u32); payload.extend(part); }
    LiveSpatialCitizenObservation::decode_payload(&payload, generation, "df".into(), "dfhack".into())
}
fn fixture(count: u32) -> Result<(SpatialCitizenJournal<Memory>, OperationContext)> {
    let initial = source(3, 8, 7)?;
    let mut state = Spatial18::empty(); Spatial18::publish(&mut state, initial)?;
    let anchor = Spatial18::snapshot(&state).ok_or_else(|| corrupt("initial snapshot"))?.anchor();
    let mut context = OperationContext { session_id: SessionId::new(18_933), request_id: RequestId::new(1), anchor,
    ...
    let mut journal = SpatialCitizenJournal::open(Memory::default(), &context,
        JournalLimits::default(), true, TailRecovery::Refuse)?;
    for n in 0..count { journal.append(source(3 + n, 8, 7)?, &context)?; }
    context.anchor = journal.state().snapshot().ok_or_else(|| corrupt("latest snapshot"))?.anchor();
    journal.storage.reads = 0;
    Ok((journal, context))
}
fn selection(j: &SpatialCitizenJournal<Memory>, numbers: &[usize]) -> Vec<(u64, Digest32)> {
    numbers.iter().map(|n| (j.entries()[n-1].number, j.entries()[n-1].record_digest)).collect()
}
...
fn sparse_selection_carries_every_delta_base_and_reads_the_prefix_once() -> Result<()> {
    ...
    let head = j.head(); let stats = j.storage_stats();
    let mutations = (j.storage.writes, j.storage.syncs);
    let values = j.project_records(&wanted, &c, |_, s| {
        Ok(s.snapshot().ok_or_else(|| corrupt("projected snapshot"))?.anchor())
    })?;
    assert_eq!(j.storage.reads as u64, expected);
    assert_eq!(values.len(), 2);
    for (entry, anchor) in values {
        assert_eq!(entry.anchor, anchor);
        assert_eq!(j.state_at(entry.number, entry.record_digest, &c)?.snapshot().map(WorldSnapshot::anchor), Some(anchor));
    }
    assert_eq!(j.state().snapshot().map(WorldSnapshot::anchor), Some(c.anchor));
    assert_eq!(j.head(), head); assert_eq!(j.storage_stats(), stats);
...
fn independent_pages_and_reopen_never_use_the_current_live_payload_as_a_base() -> Result<()> {
    ...
    let mut reopened = SpatialCitizenJournal::open(Memory { bytes: Cursor::new(bytes), ..Memory::default() },
        &c, JournalLimits::default(), false, TailRecovery::Refuse)?;
    for numbers in [&[4, 5][..], &[1, 2][..], &[2, 3][..]] {
        let wanted = selection(&reopened, numbers);
        let values = reopened.project_records(&wanted, &c, |_, s| {
            Ok(s.snapshot().ok_or_else(|| corrupt("projected snapshot"))?.anchor())
        })?;
        for (entry, anchor) in values { assert_eq!(entry.anchor, anchor); }
        assert_eq!(reopened.state().snapshot().map(WorldSnapshot::anchor), Some(c.anchor));
    }
    Ok(())
}
...
fn mixed_keyframes_and_epoch_reset_project_the_recorded_generation_universe() -> Result<()> {
    ...
    c.anchor = j.state().snapshot().ok_or_else(|| corrupt("latest"))?.anchor();
    ...
    for (entry, snapshot) in values {
        ...
        assert_eq!(j.state_at(entry.number, entry.record_digest, &c)?.snapshot(), Some(&snapshot));
    }
    assert!(!j.fenced()); Ok(())
}
...
fn corruption_of_an_unselected_delta_fences_without_returning_partial_rows() -> Result<()> {
    ...
    let mut calls = 0;
    assert!(matches!(j.project_records(&wanted, &c, |_, _| { calls += 1; Ok(()) }),
        Err(e) if e.code == ErrorCode::CorruptLedger));
    assert_eq!(calls, 0); assert!(j.fenced());
    assert_eq!(j.state().snapshot().map(WorldSnapshot::anchor), Some(c.anchor));
    Ok(())
}
...
fn expanded_delta_budget_is_checked_before_projecting_the_target() -> Result<()> {
    ...
    let wanted = selection(&j, &[2]); let mut calls = 0;
    assert!(matches!(j.project_records(&wanted, &c, |_, _| { calls += 1; Ok(()) }),
        Err(e) if e.code == ErrorCode::BudgetExceeded));
    assert_eq!(calls, 0); assert!(!j.fenced()); Ok(())
}
...
fn late_callback_refusal_does_not_change_the_next_append_compression_base() -> Result<()> {
    ...
    let bytes = j.storage.bytes.get_ref().clone(); let stats = j.storage_stats();
    assert!(matches!(j.project_records(&wanted, &c, |e, _| {
        if e.number == 3 { Err(budget("late output refusal")) } else { Ok(e.number) }
    }), Err(e) if e.code == ErrorCode::BudgetExceeded));
    assert_eq!(j.storage.bytes.get_ref(), &bytes); assert_eq!(j.storage_stats(), stats);
    assert!(!j.fenced());
    j.append(source(7, 8, 7)?, &c)?;
    let last = j.entries().last().cloned().ok_or_else(|| corrupt("last entry"))?;
    assert_eq!(j.state_at(last.number, last.record_digest, &c)?.snapshot().map(WorldSnapshot::anchor), Some(last.anchor));
    Ok(())
}
...
fn final_custody_failure_discards_all_projected_results() -> Result<()> {
    ...
    let wanted = selection(&j, &[3]); let loss = Rc::clone(&j.storage.identity_failed);
    let mut calls = 0;
    assert!(matches!(j.project_records(&wanted, &c, |_, _| {
        calls += 1; loss.set(true); Ok(())
    }), Err(e) if e.code == ErrorCode::CorruptLedger));
    assert_eq!(calls, 1); assert!(j.fenced());
    assert_eq!(j.state().snapshot().map(WorldSnapshot::anchor), Some(c.anchor));
    Ok(())
}
...
fn a_bad_later_reference_is_rejected_before_any_prefix_read_or_projection() -> Result<()> {
    let (mut j, c) = fixture(4)?;
    let mut wanted = selection(&j, &[1, 4]); wanted[1].1 = Digest32::ZERO;
    let mut calls = 0;
    assert!(matches!(j.project_records(&wanted, &c, |_, _| { calls += 1; Ok(()) }),
        Err(e) if e.code == ErrorCode::StaleAnchor));
    assert_eq!(calls, 0); assert_eq!(j.storage.reads, 0); assert!(!j.fenced());
    Ok(())
}
