...
impl FloorGoal {
    pub fn new(
        ...
        if region.size[2] != 1
            ...
            || folder.contains('\0')
            || site > i32::MAX as u32
            || deadline > MAX_GAME_TICK
            || stable_ticks > 403_200
            || !(1..=128).contains(&required_samples)
        ...
        for cell in &capture.map.cells {
            match cell {
                ...
                Cell::Visible(tile) => {
                    ...
                    if tile.shape == Shape::Wall {
                        counts.wall += 1;
                    } else if tile.shape != Shape::Floor {
                        counts.other_shape += 1;
                    } else if tile.liquid_depth != 0 {
                        counts.wet_floor += 1;
                    } else if tile.dig_designation != 0 {
                        counts.designated_floor += 1;
                    } else {
                        counts.floor_goal += 1;
                    }
                }
    ...
    pub fn begin(&self, capture: LiveMapObservation) -> Result<FloorProgress> {
        ...
        if capture.world_folder != self.folder
            || capture.site_id != self.site
            || capture.tick().get() > self.deadline
        {
            return Err(invalid(
                "floor goal starts at the wrong fortress or after its deadline",
            ));
        }
...
impl FloorStatus {
    pub fn terminal(self) -> bool {
        matches!(
            self,
            Self::Satisfied | Self::Expired | Self::Invalidated | Self::Cancelled
        )
    }
...
impl FloorInterruption {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::UnfinishedRead => "unfinished_read",
            Self::ReadFailed => "read_failed",
            Self::SampleGap => "sample_gap_reset",
            Self::SourceChanged => "source_identity_or_clock_changed",
...
impl FloorProgress {
    pub fn goal(&self) -> &FloorGoal {
        &self.goal
    }
    pub fn first(&self) -> &LiveMapObservation {
        &self.first
    }
    pub fn latest(&self) -> &LiveMapObservation {
        &self.latest
    }
    pub fn status(&self) -> FloorStatus {
        self.status
    }
    pub fn streak(&self) -> u32 {
        self.streak
    }
    pub fn since_tick(&self) -> Option<u64> {
        self.since_tick
    }
    pub fn observations(&self) -> u32 {
        self.observations
    }
    pub fn interruption(&self) -> Option<FloorInterruption> {
        self.interruption
    }
    pub fn counts(&self) -> Result<FloorCounts> {
        self.goal.classify(&self.latest)
    }

    fn unfinished(&self) -> Result<()> {
        if self.status.terminal() {
            Err(invalid("terminal floor-goal history is immutable"))
        } else {
            Ok(())
        }
    }
    ...
    pub fn interrupt(&self, failed: bool) -> Result<Self> {
        ...
        next.interruption = Some(if failed {
            FloorInterruption::ReadFailed
        } else {
            FloorInterruption::UnfinishedRead
        });
        Ok(next)
    }
    ...
    pub fn sample(&self, capture: LiveMapObservation) -> Result<Self> {
        ...
        if capture.bridge_generation != self.first.bridge_generation
            || capture.df_version != self.first.df_version
            || capture.dfhack_version != self.first.dfhack_version
            || capture.world_folder != self.first.world_folder
            || capture.site_id != self.first.site_id
            || capture.map_dimensions != self.first.map_dimensions
            || capture.tick() < self.latest.tick()
        {
        ...
        if gap
            && matches!(
                next.status,
                FloorStatus::Stabilizing | FloorStatus::Satisfied
            )
        {
    ...
    fn evaluate(&mut self, advanced: bool) -> Result<()> {
        self.observations = self
            ...
            .ok_or_else(|| invalid("floor-goal sample allowance exhausted"))?;
        let counts = self.counts()?;
        let tick = self.latest.tick().get();
        self.interruption = None;
        self.status = if tick > self.goal.deadline {
            FloorStatus::Expired
        } else if counts.hidden != 0 || counts.missing != 0 {
            FloorStatus::Unknown
        } else if counts.floor_goal as usize != self.goal.region.volume().map_err(map_error)? {
            FloorStatus::Pending
        } else {
            if self.streak == 0 {
                self.streak = 1;
            ...
            let since = self
                .since_tick
                .ok_or_else(|| invalid("matching floor streak lost its start"))?;
        ...
        if !matches!(
            self.status,
            FloorStatus::Satisfied | FloorStatus::Stabilizing
        ) {
            self.streak = 0;
