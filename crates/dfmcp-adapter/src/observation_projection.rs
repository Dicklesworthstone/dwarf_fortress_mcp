...
impl<S: JournalStorage, P: JournalProfile> ObservationJournal<S, P> {
    ...
    pub fn project_records<T, F>(&mut self, records: &[(u64, Digest32)],
        context: &OperationContext, mut project: F) -> Result<Vec<(JournalEntry, T)>>
    where F: FnMut(&JournalEntry, &P::State) -> Result<T> {
    ...
    fn project_records_inner<T, F>(&mut self, records: &[(u64, Digest32)],
        context: &OperationContext, project: &mut F) -> Result<Vec<(JournalEntry, T)>>
    where F: FnMut(&JournalEntry, &P::State) -> Result<T> {
        let started = Instant::now();
        let check_now = || -> Result<()> {
            check(context, started)?;
            if started.elapsed().as_millis() >= u128::from(context.budget.max_wall_millis) {
                return Err(budget("historical projections exhausted their shared replay deadline"));
            }
            Ok(())
        };
        check_now()?;
        ...
        if records.is_empty() || records.len() > 33 {
            return Err(budget("project one to 33 exact historical records per pass"));
        }
        ...
        for &(number, digest) in records {
            ...
            let index = usize::try_from(number - 1)
                .map_err(|_| DfmcpError::new(ErrorCode::CursorGap, "historical record index overflow"))?;
            let entry = self.entries.get(index)
                .ok_or_else(|| DfmcpError::new(ErrorCode::CursorGap, "historical projection record is not retained"))?;
            if entry.record_digest != digest {
                return Err(DfmcpError::new(ErrorCode::StaleAnchor, "historical projection digest differs"));
            }
            previous_number = number;
        }
        self.storage.seek(SeekFrom::Start(0)).map_err(storage_error)?;
        let mut header = [0; HEADER_BYTES];
        self.storage.read_exact(&mut header).map_err(storage_error)?;
        if decode_header::<P>(&header)? != (self.fortress, self.id, self.header_digest) {
            return Err(corrupt("historical projection header changed after opening"));
        }
        ...
        for index in 0..last {
            ...
            if &entry != known || entry.previous_digest != predecessor
                ...
                return Err(corrupt("historical projection record or predecessor changed"));
            }
        ...
        if projected.len() != records.len() {
            return Err(corrupt("historical projection did not visit its entire selection"));
        }
...
mod tests {
    ...
    #[derive(Default)]
    struct Memory { bytes: Cursor<Vec<u8>>, read_bytes: usize, writes: usize, syncs: usize }
    impl Read for Memory {
        fn read(&mut self, out: &mut [u8]) -> io::Result<usize> {
            let n = self.bytes.read(out)?; self.read_bytes += n; Ok(n)
        }
    }
    ...
    impl Write for Memory {
        fn write(&mut self, data: &[u8]) -> io::Result<usize> { self.writes += 1; self.bytes.write(data) }
    ...
    pub(super) fn observation(tick: u32) -> Result<LiveSpatialObservation> {
        ...
        let source = LiveSpatialObservation::decode_payload(&bytes,7,"df".into(),"dfhack".into())?;
        let mut operations = source.operations().clone(); let mut terrain = source.terrain().clone();
        ...
        for part in [operations.encode_profile(OperationsProfile::PagedV1_4)?, terrain.encode_payload()?] {
            bytes.extend_from_slice(&(part.len() as u32).to_be_bytes()); bytes.extend_from_slice(&part);
        }
    ...
    fn fixture() -> Result<(ObservationJournal<Memory, Spatial16>, OperationContext)> {
        let mut state = crate::live_spatial::LiveSpatialState::default(); state.publish(observation(3)?)?;
        let anchor = state.snapshot().ok_or_else(|| corrupt("fixture state"))?.anchor();
        let mut context = OperationContext { session_id: SessionId::new(98_301), request_id: RequestId::new(1), anchor,
            budget: WorkBudget { max_entities: 100_000, max_bytes: 16*1024*1024, max_wall_millis: 60_000,
                ..WorkBudget::default() }, cancellation_requested: false,
        ...
        let mut journal = ObservationJournal::open(Memory::default(), &context, JournalLimits::default(), true, TailRecovery::Refuse)?;
        for tick in 3..7 { journal.append(observation(tick)?, &context)?; }
        context.anchor = journal.state().snapshot().ok_or_else(|| corrupt("fixture latest"))?.anchor();
    ...
    fn projects_one_prefix_without_writes_or_replacing_current_state() -> Result<()> {
        ...
        let wanted: Vec<_> = journal.entries().iter().take(3).map(|e| (e.number,e.record_digest)).collect();
        let prior = journal.state().clone(); let bytes = journal.storage.bytes.get_ref().clone();
        let mutations = (journal.storage.writes,journal.storage.syncs);
        let expected_read = journal.entries()[2].offset as usize + journal.entries()[2].encoded_bytes as usize;
        let result = journal.project_records(&wanted,&context,|_,s| Ok(s.snapshot().ok_or_else(||corrupt("snapshot"))?.anchor()))?;
        assert_eq!(result.len(),3);
        for (entry, anchor) in result { assert_eq!(entry.anchor,anchor); }
    ...
    fn validates_all_record_identities_before_running_a_projection() -> Result<()> {
        ...
        for records in [vec![],vec![(0,first.record_digest)],vec![(1,first.record_digest);34],
            vec![(1,first.record_digest),(1,first.record_digest)],
            vec![(1,first.record_digest),(4,Digest32::ZERO)],vec![(1,first.record_digest),(99,first.record_digest)]] {
            let mut calls = 0;
            assert!(journal.project_records(&records,&context,|_,_| {calls += 1; Ok(())}).is_err());
            assert_eq!(calls,0); assert!(!journal.fenced());
        }
        Ok(())
    }
    #[test]
    fn late_projection_refusal_returns_no_partial_result_or_new_root() -> Result<()> {
        let (mut journal, context) = fixture()?;
        let wanted: Vec<_> = journal.entries().iter().map(|e| (e.number,e.record_digest)).collect();
        let result = journal.project_records(&wanted,&context,|e,_| {
            if e.number == 3 {Err(budget("injected late refusal"))} else {Ok(e.number)}
        });
        assert!(matches!(result,Err(e) if e.code == ErrorCode::BudgetExceeded));
        assert_eq!(journal.state().snapshot().map(WorldSnapshot::anchor),Some(context.anchor));
        assert!(!journal.fenced()); Ok(())
    }
    #[test]
    fn corruption_in_unselected_prefix_is_not_skipped() -> Result<()> {
        ...
        let mut calls = 0;
        assert!(matches!(journal.project_records(&[(last.number,last.record_digest)],&context,|_,_| {
            calls += 1; Ok(())
        }),Err(e) if e.code == ErrorCode::CorruptLedger));
        assert_eq!(calls,0); assert!(journal.fenced()); Ok(())
    }
    #[test]
    fn historical_records_do_not_revive_current_authority() -> Result<()> {
        ...
        for case in 0..4 {
            ...
            assert!(journal.project_records(&[(first.number,first.record_digest)],&denied,|_,_|Ok(())).is_err());
