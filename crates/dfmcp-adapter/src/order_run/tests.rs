...
pub(super) fn capture_bytes(
    ...
    put_field(&mut out, folder.as_bytes());
    out.push(u8::from(paused));
    for n in [9u32, 10] {
        out.extend_from_slice(&n.to_be_bytes());
    }
    out.extend_from_slice(&[1, 1]);
    for n in [5u32, 5, status] {
        out.extend_from_slice(&n.to_be_bytes());
    }
    out
}
pub(super) fn plan() -> Result<OrderRunPlan> {
    OrderRunPlan::new(
        "approval",
        OrderRunSpec::new(20, 1000, OrderPredicate::Approved, 2, 2)?,
...
pub(super) fn record_bytes(
    ...
    put_field(&mut out, plan.key().as_bytes());
    put_field(&mut out, plan.native_bytes());
    ...
    out.extend_from_slice(&[
        ...
        reason,
        trigger,
        u8::from(matches!(phase, 1..=3 | 5)),
        u8::from(phase == 3),
        u8::from(known),
    ]);
    out.extend_from_slice(&(if known { 105u64 } else { 0 }).to_be_bytes());
    out.extend_from_slice(&count.to_be_bytes());
    out.extend_from_slice(&(if count > 0 { 105u64 } else { 100 }).to_be_bytes());
    put_field(&mut out, sample.unwrap_or_default());
    let proof = hash(b"dfmcp-order-run-receipt/1", &out);
...
fn independent_predicate_fixture_and_sealed_round_trip() -> Result<()> {
    let fixture = unhex(include_str!(
        "../../tests/fixtures/order_run_predicate_v1_14.hex"
    ))?;
    let value = OrderRunRecord::decode(&fixture)?;
    assert_eq!(value.plan().key(), "approval");
    assert!(value.pause_verified() && value.predicate_observed());
    assert_eq!(value.reported_stable_samples(), 2);
    assert_eq!(value.counted_tick(), 105);
    assert_eq!(value.plan().before().fortress().folder(), "region1");
    assert_eq!(value.plan().before().fortress().site(), 7);
    assert_eq!(
        OrderRunPlan::decode(&value.plan().canonical_bytes())?,
        *value.plan()
    );
    assert_eq!(value.canonical_bytes(), fixture);
    Ok(())
}
#[test]
fn every_corruption_and_truncated_prefix_is_rejected() -> Result<()> {
    let raw = unhex(include_str!(
        "../../tests/fixtures/order_run_predicate_v1_14.hex"
    ))?;
    for i in 0..raw.len() {
        ...
        corrupt[i] ^= 1;
        assert!(OrderRunRecord::decode(&corrupt).is_err());
    }
    let mut extra = raw;
    extra.push(0);
    assert!(OrderRunRecord::decode(&extra).is_err());
    Ok(())
}
#[test]
fn condition_already_true_and_bad_cadence_never_seal() -> Result<()> {
    for predicate in [
        OrderPredicate::Approved,
        OrderPredicate::Active,
        OrderPredicate::RemainingAtMost(5),
    ] {
    ...
    for (samples, interval, ticks) in [
        ...
        assert!(
            OrderRunSpec::new(ticks, 1000, OrderPredicate::Approved, samples, interval).is_err()
        );
    }
    assert!(OrderPredicate::from_code(1, 1).is_err());
    assert!(OrderPredicate::from_code(3, 101).is_err());
    Ok(())
}
#[test]
fn exact_identity_and_intent_changes_are_committed() -> Result<()> {
    ...
    assert_eq!(p.digest(), another_key.digest());
    assert_ne!(p.token(), another_key.token());
    let elsewhere = OrderRunPlan::new(
        p.key(),
        p.spec(),
        OrderCapture::decode(&capture_bytes("region2", 3, 100, true, 0))?,
    )?;
    assert_ne!(p.digest(), elsewhere.digest());
    assert_ne!(
        p.before().fortress().fortress_id(),
        elsewhere.before().fortress().fortress_id()
    );
    assert!(
        OrderRunRecord::decode(&record_bytes(&p, 0, 0, 0, None, 0))?
            .validate_successor(&OrderRunRecord::decode(&record_bytes(
                &elsewhere, 0, 0, 0, None, 0
            ))?)
            .is_err()
    );
    Ok(())
...
fn rehashed_false_goal_and_wrong_sample_source_are_rejected() -> Result<()> {
    ...
    assert!(
        OrderRunRecord::decode(&record_bytes(
            &p,
            3,
            1,
            1,
            Some(&capture_bytes("region1", 4, 105, false, 1)),
            2
        ))
        .is_err()
    );
...
fn maximal_native_and_intent_bounds_round_trip() -> Result<()> {
    let p = OrderRunPlan::new(
        &"k".repeat(128),
        OrderRunSpec::new(20, 1000, OrderPredicate::Approved, 2, 2)?,
        OrderCapture::decode(&capture_bytes(&"x".repeat(512), 3, 100, true, 0))?,
    )?;
    assert_eq!(p.native_bytes().len(), MAX_PLAN_BYTES);
    assert_eq!(p.canonical_bytes().len(), MAX_INTENT_BYTES);
    let sample = capture_bytes(&"x".repeat(512), 4, 105, false, 1);
    let raw = record_bytes(&p, 3, 3, 1, Some(&sample), 2);
    assert_eq!(raw.len(), MAX_RECORD_BYTES);
    assert!(OrderRunRecord::decode(&raw)?.predicate_observed());
    Ok(())
}
#[test]
fn terminal_and_trigger_evidence_cannot_change_on_recovery() -> Result<()> {
    ...
    stopping.validate_successor(&stopped)?;
    stopped.validate_successor(&stopped)?;
...
fn authority_covers_exact_fortress_risk_and_entire_tick_horizon() -> Result<()> {
    let p = plan()?;
    let c = context()?;
    rpc::authorize_plan(&c, &p)?;
    let mut bad = c.clone();
    bad.anchor.fortress_id = FortressId::new(22);
    assert!(rpc::authorize_plan(&bad, &p).is_err());
    bad = c.clone();
    bad.budget.max_game_ticks = 19;
    assert!(rpc::authorize_plan(&bad, &p).is_err());
    bad = c.clone();
    bad.grants.retain(|g| g.capability != Capability::Plan);
    assert!(rpc::authorize_plan(&bad, &p).is_err());
    bad = c.clone();
    bad.cancellation_requested = true;
    assert!(rpc::authorize_plan(&bad, &p).is_err());
    bad = c.clone();
    for g in &mut bad.grants {
        g.expires_at_tick = Some(GameTick(119));
    }
    assert!(rpc::authorize_plan(&bad, &p).is_err());
    bad = c.clone();
    for g in &mut bad.grants {
