...
impl OrderCapture {
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        require(
            (62..=MAX_CAPTURE_BYTES).contains(&bytes.len()),
            "order-run capture size",
        )?;
        ...
        let recipe = r.byte()?;
        let total = i32::from_be_bytes(r.array()?);
        let remaining = i32::from_be_bytes(r.array()?);
        let status = r.u32()?;
        r.finish()?;
        require(
            generation > 0 && generation < u64::MAX && tick <= MAX_NATIVE_TICK,
            "invalid order-run incarnation/clock",
        )?;
        require(
            order_id <= i32::MAX as u32 && horizon <= i32::MAX as u32 && recipe <= 4,
    ...
    pub fn eligible(&self) -> bool {
        self.paused && self.present && self.recipe != 0 && self.sequence != u64::MAX
    }
}
...
impl OrderPredicate {
    pub fn from_code(code: u32, threshold: u32) -> Result<Self> {
        match (code, threshold) {
            (1, 0) => Ok(Self::Approved),
            (2, 0) => Ok(Self::Active),
            (3, 0..=100) => Ok(Self::RemainingAtMost(threshold)),
            _ => Err(error(
                ErrorCode::InvalidRequest,
                "unknown predicate or invalid threshold",
            )),
        }
    }
    pub fn code(self) -> u8 {
        match self {
            Self::Approved => 1,
            Self::Active => 2,
            Self::RemainingAtMost(_) => 3,
        }
    }
    pub fn threshold(self) -> u32 {
        match self {
            Self::RemainingAtMost(n) => n,
            _ => 0,
        }
    }
    ...
    pub fn observed(self, value: &OrderCapture) -> bool {
        value.present
            && value.recipe != 0
            && match self {
                Self::Approved => value.status & 1 != 0,
                Self::Active => value.status & 2 != 0,
                Self::RemainingAtMost(n) => i64::from(value.remaining) <= i64::from(n),
            }
    }
}
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct OrderRunSpec {
    clock: RunSpec,
...
impl OrderRunSpec {
    pub fn new(
        ...
        if !(1..=16).contains(&samples)
            || !(1..=1200).contains(&interval)
            || u64::from(samples) * u64::from(interval) > u64::from(ticks)
        {
            return Err(error(
                ErrorCode::InvalidRequest,
                "sample cadence exceeds bounded run horizon",
            ));
        }
        Ok(Self {
            clock,
    ...
    pub fn predicate(self) -> OrderPredicate {
        self.predicate
    }
...
impl OrderRunPlan {
    pub fn new(key: &str, spec: OrderRunSpec, before: OrderCapture) -> Result<Self> {
        validate_key(key)?;
        if !before.eligible()
            || i64::from(spec.predicate.threshold()) > i64::from(before.total)
            || spec.predicate.observed(&before)
        {
            return Err(error(
                ErrorCode::StaleAnchor,
                "conditional run requires a paused recognized order and a not-yet-observed predicate",
            ));
        }
        let mut bytes = b"DFMOP014".to_vec();
        for n in [spec.game_ticks(), spec.wall_ms()] {
            bytes.extend_from_slice(&n.to_be_bytes());
        }
        bytes.push(spec.predicate.code());
        for n in [spec.predicate.threshold(), spec.samples, spec.interval] {
            bytes.extend_from_slice(&n.to_be_bytes());
        }
        ...
        let mut token = [0; 16];
        token.copy_from_slice(&commitment.as_bytes()[..16]);
        Ok(Self {
            key: key.to_owned(),
            spec,
            before,
    ...
    pub fn from_native(key: &str, bytes: &[u8]) -> Result<Self> {
        require(
            (93..=MAX_PLAN_BYTES).contains(&bytes.len()),
            "order-run plan size",
        )?;
        let mut r = Reader(bytes);
        require(r.take(8)? == b"DFMOP014", "not an order-run/1.14 plan")?;
        let ticks = r.u32()?;
        let wall = r.u32()?;
        let code = r.byte()?;
        let predicate = OrderPredicate::from_code(u32::from(code), r.u32()?)?;
        let spec = OrderRunSpec::new(ticks, wall, predicate, r.u32()?, r.u32()?)?;
        let before = OrderCapture::decode(field(&mut r, MAX_CAPTURE_BYTES)?)?;
        r.finish()?;
        let result = Self::new(key, spec, before)?;
        require(result.bytes == bytes, "noncanonical order-run plan")?;
        Ok(result)
    }
    pub fn key(&self) -> &str {
        &self.key
    }
    pub fn spec(&self) -> OrderRunSpec {
        self.spec
    }
    pub fn before(&self) -> &OrderCapture {
        &self.before
    }
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        let plan = Self::from_native(&key, field(&mut r, MAX_PLAN_BYTES)?)?;
        r.finish()?;
        Ok(plan)
    }
}
...
pub struct OrderRunRecord {
    ...
    count: u32,
    counted_tick: u64,
    sample: Option<OrderCapture>,
    receipt: Digest32,
}
impl OrderRunRecord {
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        require(
            (214..=MAX_RECORD_BYTES).contains(&bytes.len()),
            "order-run receipt size",
        )?;
        let mut r = Reader(bytes);
        require(r.take(8)? == b"DFMOE014", "not an order-run/1.14 receipt")?;
        let key = text(field(&mut r, 128)?, 128)?;
        ...
        let phase = match r.byte()? {
            ...
            _ => return Err(error(ErrorCode::AdapterRejected, "unknown clock phase")),
        };
        ...
        let trigger = match r.byte()? {
            0 => OrderTrigger::None,
            1 => OrderTrigger::PredicateObserved,
            2 => OrderTrigger::TargetAbsent,
            3 => OrderTrigger::TargetChanged,
            4 => OrderTrigger::CounterRegression,
            5 => OrderTrigger::HorizonRegression,
            6 => OrderTrigger::SourceChanged,
            7 => OrderTrigger::NativeFailure,
        ...
        let sample = if sample_bytes.is_empty() {
            ...
            Some(OrderCapture::decode(sample_bytes)?)
        };
        let receipt = Digest32::from_bytes(r.array()?);
        r.finish()?;
        require(
            receipt == hash(b"dfmcp-order-run-receipt/1", &bytes[..bytes.len() - 32]),
            "order-run receipt checksum mismatch",
        )?;
        require(
            attempted
                == matches!(
        ...
        require(
            ...
            "unknown tick has nonzero backing",
        )?;
        let before = plan.before();
        let spec = plan.spec();
        require(
            count <= spec.samples
                && counted_tick <= MAX_NATIVE_TICK
                && counted_tick >= before.tick + u64::from(count) * u64::from(spec.interval),
            "invalid sampled stability count",
        )?;
        if let Some(sample) = &sample {
            require(
                sample.fortress == before.fortress
                    && sample.generation == before.generation
                    && sample.order_id == before.order_id
                    && sample.sequence == before.sequence + 1
                    && !sample.paused
                    && before.tick <= counted_tick
                    && counted_tick <= sample.tick
                    && sample.tick < before.tick + u64::from(spec.game_ticks()),
                "sample outside exact sealed run",
            )?;
        } else {
            require(
                count == 0 && counted_tick == before.tick,
                "stability without native sample",
            )?;
        }
        let phase_valid = match phase {
            RunPhase::Prepared | RunPhase::Refused => {
                !known
                    && trigger == OrderTrigger::None
                    && sample.is_none()
            ...
            RunPhase::Stopping | RunPhase::Stopped => {
                trigger != OrderTrigger::SourceChanged
                    && (matches!(
                        reason,
                        RunReason::TickLimit
                            | RunReason::WallLimit
        ...
        if matches!(
            trigger,
            OrderTrigger::PredicateObserved
                | OrderTrigger::TargetAbsent
                | OrderTrigger::TargetChanged
                | OrderTrigger::CounterRegression
                | OrderTrigger::HorizonRegression
        ) {
            require(
                matches!(
                    phase,
                    RunPhase::Stopping | RunPhase::Stopped | RunPhase::SourceLost
                ) && (phase == RunPhase::SourceLost || reason == RunReason::Cancelled)
                    && sample.is_some(),
        ...
        if trigger == OrderTrigger::PredicateObserved {
            let s = sample
                .as_ref()
                .ok_or_else(|| error(ErrorCode::AdapterRejected, "predicate has no sample"))?;
            require(
                count == spec.samples
                    && counted_tick == s.tick
                    && spec.predicate.observed(s)
                    && s.recipe == before.recipe
                    && s.total == before.total
                    && s.remaining <= before.remaining
                    && s.horizon >= before.horizon,
                "predicate sample does not prove reported condition",
            )?;
        } else {
            require(
                count < spec.samples,
                "terminal stability count without predicate trigger",
            )?;
        }
        match trigger {
            OrderTrigger::TargetAbsent => require(
                sample.as_ref().is_some_and(|s| !s.present),
                "absence trigger has a present target",
            )?,
            OrderTrigger::TargetChanged => require(
                sample.as_ref().is_some_and(|s| {
                    s.present
                        && (s.recipe == 0 || s.recipe != before.recipe || s.total != before.total)
                }),
                "unchanged template trigger",
            )?,
            OrderTrigger::SourceChanged => require(
                phase == RunPhase::SourceLost,
                "source trigger without source loss",
            )?,
            OrderTrigger::NativeFailure => require(
                matches!(
                    phase,
                    RunPhase::Stopping | RunPhase::Stopped | RunPhase::SourceLost
                ),
    ...
    pub fn unpause_attempted(&self) -> bool {
        !matches!(self.phase, RunPhase::Prepared | RunPhase::Refused)
    }
    ...
    pub fn reported_stable_samples(&self) -> u32 {
        self.count
    }
    pub fn counted_tick(&self) -> u64 {
        self.counted_tick
    }
    pub fn sample(&self) -> Option<&OrderCapture> {
        self.sample.as_ref()
    }
    pub fn receipt(&self) -> Digest32 {
        self.receipt
    }
    pub fn predicate_observed(&self) -> bool {
        self.trigger == OrderTrigger::PredicateObserved
    }
    pub fn observed_tick_overshoot(&self) -> Option<u64> {
        Some(
            self.observed_tick?
                .checked_sub(self.plan.before.tick)?
                .saturating_sub(u64::from(self.plan.spec.game_ticks())),
        )
    }
    pub fn validate_successor(&self, next: &Self) -> Result<()> {
        require(
            self.plan == next.plan,
            "native order-run receipt changed its sealed plan",
        )?;
        if self.phase.terminal() {
            return require(self == next, "native terminal receipt changed");
        }
        if self.unpause_attempted() {
            require(
                next.unpause_attempted(),
                "native record regressed before dispatch",
            )?;
        }
        if self.phase == RunPhase::Stopping {
            require(
                ...
                "native stop regressed",
            )?;
            require(
                self.sample == next.sample
                    && self.count == next.count
                    && self.counted_tick == next.counted_tick
                    && (self.reason == next.reason || next.phase == RunPhase::SourceLost),
                "stopping evidence changed",
            )?;
            if self.trigger == OrderTrigger::None {
                require(
                    next.trigger == OrderTrigger::None
                        || (next.trigger == OrderTrigger::SourceChanged
                            && next.phase == RunPhase::SourceLost),
                    "stopping run acquired a new goal trigger",
                )?;
            }
        }
        if self.trigger != OrderTrigger::None {
            require(
                self.trigger == next.trigger
                    ...
                    && self.count == next.count
                    && self.counted_tick == next.counted_tick,
                "native trigger evidence changed",
            )?;
        }
        Ok(())
    }
