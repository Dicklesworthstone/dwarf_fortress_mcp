...
fn setup(
    ...
    east: u32,
    separated: bool,
) -> Result<(LiveSpatialCitizenState, OperationContext)> {
    let mut state = LiveSpatialCitizenState::default();
    state.publish(fixture::observation(3, west, east, separated)?)?;
    let anchor = state.snapshot().expect("fixture snapshot").anchor();
    let c = OperationContext {
        session_id: SessionId::new(98601),
...
fn sites() -> BTreeMap<String, [u32; 3]> {
    BTreeMap::from([("b".into(), EAST)])
}

#[test]
fn disconnected_sites_can_support_separate_complete_tasks() -> Result<()> {
    let (state, c) = setup(1, 1, true)?;
    let tasks = [task("a", 2, 1), task("b", 3, 1)];
    ...
    assert_eq!(joint.selection.task_mask, 3);
    assert_eq!(joint.selection.workers.allocated_units, 2);
    assert_eq!(joint.selection.materials.allocated_units, 2);
    assert_eq!(joint.sites.supplies.len(), 2);
    assert_eq!(joint.sites.supplies[0].eligible, 1);
    assert_eq!(joint.sites.supplies[1].eligible, 2);
    assert_eq!(
        joint
            .workforce
            .candidates
    ...
    for a in &joint.selection.materials.assignments {
        ...
        assert!(local.locations.contains_key(&a.supply_id));
        assert_eq!(local.anchor, c.anchor);
...
fn one_stack_reachable_from_multiple_sites_is_still_one_stack() -> Result<()> {
    let (state, c) = setup(1, 0, false)?;
    let tasks = [task("a", 2, 1), task("b", 3, 1)];
    let joint = portfolio::plan_at_sites(&state, &c, WEST, &tasks, &[], &sites(), 10_000_000)?;
    assert_eq!(joint.sites.supplies.len(), 1);
    assert_eq!(joint.sites.supplies[0].eligible, 3);
    assert_eq!(joint.sites.supplies[0].units, 1);
    assert_eq!(joint.selection.task_mask, 2);
    assert_eq!(joint.selection.materials.allocated_units, 1);
    assert_eq!(joint.selection.rejected[0].shortage.deficit, 1);
    Ok(())
}

#[test]
fn stock_at_the_wrong_site_cannot_support_the_task() -> Result<()> {
    let (state, c) = setup(2, 0, true)?;
    let tasks = [task("a", 1, 1), task("b", 100, 1)];
    let joint = portfolio::plan_at_sites(&state, &c, WEST, &tasks, &[], &sites(), 10_000_000)?;
...
fn reserve_support_stays_at_default_origin_and_competes_globally() -> Result<()> {
    let (state, c) = setup(1, 1, true)?;
    let tasks = [task("a", 100, 1), task("b", 1, 1)];
    let joint = portfolio::plan_at_sites(
        &state,
        &c,
        WEST,
        &tasks,
...
fn unreachable_remote_stock_cannot_repair_a_hard_reserve_shortfall() -> Result<()> {
    ...
    assert!(
        cut.demand_indices
            .iter()
            .all(|i| joint.material_owners[*i] == RESERVE_OWNER)
    );
    Ok(())
}
...
fn task_order_and_explicit_default_sites_do_not_change_the_model() -> Result<()> {
    ...
    let tasks = [task("a", 2, 1), task("b", 3, 1)];
    let first = portfolio::plan_with_reserves(
        &state,
        &c,
        WEST,
        &tasks,
    ...
    let same = portfolio::plan_at_sites(
        &state,
        &c,
        WEST,
        &tasks,
    ...
    assert_eq!(
        joint,
        portfolio::plan_at_sites(&state, &c, WEST, &reversed, &[], &sites(), 10_000_000)?
    );
    Ok(())
}
...
fn malformed_sites_and_budget_or_authority_failures_return_no_plan() -> Result<()> {
    let (state, c) = setup(2, 2, true)?;
    let tasks = [task("a", 1, 1), task("b", 100, 1)];
    for overrides in [
        BTreeMap::from([("missing".into(), EAST)]),
        BTreeMap::from([("a".into(), [2, 0, 5])]),
        BTreeMap::from([("a".into(), [99, 0, 5])]),
        BTreeMap::from([("a".into(), [32768, 0, 5])]),
    ] {
        assert!(
            portfolio::plan_at_sites(&state, &c, WEST, &tasks, &[], &overrides, 10_000_000)
                .is_err()
        );
    }
    ...
    denied.grants.clear();
    assert!(
        matches!(portfolio::plan_at_sites(&state,&denied,WEST,&tasks,&[],&sites(),10_000_000),Err(e)if e.code==ErrorCode::CapabilityDenied)
    );
    assert_eq!(state.snapshot().expect("snapshot").anchor(), c.anchor);
    Ok(())
}
    ...
    for separated in [false, true] {
        for west in 0..=3 {
            for east in 0..=3 {
                ...
                for a in 1..=2u64 {
                    for b in 1..=2u64 {
                        for reserve in 0..=1u64 {
                            let tasks = [task("a", 2, a), task("b", 3, b)];
                            let reserves = if reserve == 0 {
                                vec![]
                            } else {
                                vec![material("buffer", reserve)]
                            };
                            let joint = portfolio::plan_at_sites(
                                &state,
                                &c,
                            ...
                            let feasible = |mask: u16| {
                                let x = if mask & 1 != 0 { a } else { 0 };
                                let y = if mask & 2 != 0 { b } else { 0 };
                                if separated {
                                    x + reserve <= u64::from(west) && y <= u64::from(east)
                                } else {
                                    x + y + reserve <= u64::from(west + east)
                                }
                            };
                            ...
                            assert_eq!(
                                joint.selection.materials.shortage.is_none(),
                                expected.is_some()
                            );
                        }
                    }
