...
use dfmcp_adapter::operations_journal::{JournalLimits, Operations13, PrivateJournalFile,
    Spatial16, TailRecovery, open_profile_journal};
use dfmcp_core::{Capability, CapabilityGrant, CapabilityScope, DfmcpError, ErrorCode,
    FortressId, GameTick, OperationContext, RequestId, Result, RiskTier, SessionId, WorkBudget};
...
impl Fixture {
    fn new() -> Result<Self> {
        let directory = std::env::temp_dir().canonicalize().map_err(io_error)?
            .join(format!("dfmcp-archive-recovery-{}-{}",std::process::id(),NEXT.fetch_add(1,Ordering::Relaxed)));
        fs::DirBuilder::new().mode(0o700).create(&directory).map_err(io_error)?;
        Ok(Self { path: directory.join("spatial.bin"), directory })
    }
    fn populated(&self) -> Result<OperationContext> {
        let observation = observation(3)?;
        let c = context(&observation)?;
        let mut journal = open_profile_journal::<Spatial16>(&self.path,&c,JournalLimits::default(),TailRecovery::Refuse)?;
...
fn observation(tick: u32) -> Result<LiveSpatialObservation> {
    ...
    let bytes = (0..hex.len()).step_by(2).map(|i|u8::from_str_radix(&hex[i..i+2],16)
        .map_err(|_|DfmcpError::new(ErrorCode::InvalidRequest,"fixture hex"))).collect::<Result<Vec<_>>>()?;
    ...
    op.jobs.year_tick = tick; terrain.year_tick = tick;
    let mut bytes = b"DFMS1600".to_vec();
    for part in [op.encode_profile(OperationsProfile::PagedV1_4)?,terrain.encode_payload()?] {
...
fn observation_at_next_tick() -> Result<LiveSpatialObservation> { observation(4) }
fn context(observation: &LiveSpatialObservation) -> Result<OperationContext> {
    let mut state = LiveSpatialState::default(); state.publish(observation.clone())?;
    let anchor = state.snapshot().ok_or_else(||DfmcpError::new(ErrorCode::InvalidRequest,"fixture snapshot"))?.anchor();
    Ok(OperationContext { session_id:SessionId::new(91_200),request_id:RequestId::new(1),anchor,
        budget:WorkBudget { max_bytes:16*1024*1024,max_entities:100_000,max_wall_millis:60_000,..WorkBudget::default() },
...
fn query_only(c: &OperationContext) -> OperationContext {
    let mut c = c.clone(); c.grants.retain(|g|g.capability==Capability::Query); c
}

#[test]
fn archive_recovers_exact_state_and_history_with_query_only_authority() -> Result<()> {
    let f = Fixture::new()?; let c = f.populated()?;
    let before = fs::read(&f.path).map_err(io_error)?;
    let mut bootstrap = query_only(&c); bootstrap.anchor.fortress_id = FortressId::NIL;
    let mut archive = PrivateJournalFile::open_recovery::<Spatial16>(&f.path,&bootstrap,JournalLimits::default())?;
    assert!(archive.recovery_only()); assert_eq!(archive.entries().len(),2);
    ...
    assert_eq!(historical.snapshot().map(|s|s.anchor()),Some(entry.anchor));
...
fn archive_never_initializes_or_repairs_missing_empty_or_incomplete_files() -> Result<()> {
    let f = Fixture::new()?; let c = query_only(&context(&observation(3)?)?);
    assert!(PrivateJournalFile::open_recovery::<Spatial16>(&f.path,&c,JournalLimits::default()).is_err());
    assert!(!f.path.exists());
    let live = open_profile_journal::<Spatial16>(&f.path,&context(&observation(3)?)?,JournalLimits::default(),TailRecovery::Refuse)?;
    drop(live);
    let header = fs::read(&f.path).map_err(io_error)?;
    // A valid header with no observations is readable at the adapter boundary.
    let archive = PrivateJournalFile::open_recovery::<Spatial16>(&f.path,&c,JournalLimits::default())?;
    assert!(archive.entries().is_empty()); drop(archive);
    for bytes in [Vec::new(),header[..header.len()-1].to_vec(),[header.clone(),b"DFM".to_vec()].concat(),
...
fn recovery_preserves_current_authority_time_identity_and_acquisition_bounds() -> Result<()> {
    let f = Fixture::new()?; let original = f.populated()?;
    let before = fs::read(&f.path).map_err(io_error)?;
    for case in 0..6 {
        ...
        match case {
            ...
            3 => { c.grants[0].expires_at_tick=Some(c.anchor.tick); c.anchor.tick=GameTick(c.anchor.tick.0+100); },
            4 => c.budget.max_entities=1,
            _ => c.grants[0].remaining_uses=Some(0),
        }
        assert!(PrivateJournalFile::open_recovery::<Spatial16>(&f.path,&c,JournalLimits::default()).is_err(),"case={case}");
    }
    assert!(PrivateJournalFile::open_recovery::<Operations13>(&f.path,&original,JournalLimits::default()).is_err());
    assert_eq!(fs::read(&f.path).map_err(io_error)?,before);
    Ok(())
}
...
fn recovery_descriptor_refuses_changed_appends_even_with_injected_observe_grant() -> Result<()> {
    let f = Fixture::new()?; let c = f.populated()?;
    let before = fs::read(&f.path).map_err(io_error)?;
    let mut archive = PrivateJournalFile::open_recovery::<Spatial16>(&f.path,&query_only(&c),JournalLimits::default())?;
...
fn recovery_retains_exclusive_custody_and_rejects_links_and_permissive_files() -> Result<()> {
    let f = Fixture::new()?; let c = f.populated()?;
    let archive = PrivateJournalFile::open_recovery::<Spatial16>(&f.path,&c,JournalLimits::default())?;
    assert!(open_profile_journal::<Spatial16>(&f.path,&c,JournalLimits::default(),TailRecovery::Refuse).is_err());
    assert!(PrivateJournalFile::open_recovery::<Spatial16>(&f.path,&c,JournalLimits::default()).is_err());
    drop(archive);
    let alias=f.directory.join("alias.bin");
    std::os::unix::fs::symlink(&f.path,&alias).map_err(io_error)?;
    assert!(PrivateJournalFile::open_recovery::<Spatial16>(&alias,&c,JournalLimits::default()).is_err());
...
fn cached_archive_access_rechecks_authority_and_fences_changed_custody() -> Result<()> {
    use std::io::Write;
    let f=Fixture::new()?;let c=f.populated()?;
    let mut archive=PrivateJournalFile::open_recovery::<Spatial16>(&f.path,&c,JournalLimits::default())?;
    let mut denied=c.clone();denied.grants.clear();
    assert!(archive.validate_custody(&denied).is_err());assert!(!archive.fenced());
    fs::OpenOptions::new().append(true).open(&f.path).and_then(|mut f|f.write_all(b"x")).map_err(io_error)?;
    assert!(matches!(archive.validate_custody(&c),Err(e)if e.code==ErrorCode::CorruptLedger));
    assert!(archive.fenced());Ok(())
}
