...
fn citizen_component(id:u32,name:&str,position:[i32;3],flags:u16,skills:&[(i32,&str,i32,i32,i32)])->Vec<u8>{
    ...
    out.extend_from_slice(&3i32.to_be_bytes());out.extend_from_slice(&[1,1]);out.extend_from_slice(&(skills.len() as u16).to_be_bytes());
    for(id,key,nominal,effective,experience)in skills{out.extend_from_slice(&id.to_be_bytes());text(&mut out,key);out.extend_from_slice(&nominal.to_be_bytes());
        out.extend_from_slice(&effective.to_be_bytes());out.extend_from_slice(&experience.to_be_bytes());}out
}
fn combined_with_skills(worker:Option<u32>,citizen_id:u32,name:&str,flags:u16,skills:&[(i32,&str,i32,i32,i32)])->dfmcp_core::Result<LiveSpatialCitizenObservation>{
    let base=base_fixture()?;let mut op=base.operations().clone();if let Some(job)=op.jobs.jobs.first_mut(){job.worker_native_id=worker;}
    ...
    let citizens=citizen_component(citizen_id,name,[1,1,5],flags,skills);let mut payload=b"DFMS1800".to_vec();
    for part in [&spatial,&citizens]{payload.extend_from_slice(&(part.len() as u32).to_be_bytes());payload.extend_from_slice(part);}
    LiveSpatialCitizenObservation::decode_payload(&payload,7,"df".to_owned(),"dfhack".to_owned())
}
fn combined(worker:Option<u32>,citizen_id:u32,name:&str,flags:u16)->dfmcp_core::Result<LiveSpatialCitizenObservation>{combined_with_skills(worker,citizen_id,name,flags,&[])}

#[test]
fn strict_citizen_worker_becomes_generation_checked_performs_and_location_edges()->dfmcp_core::Result<()>{
    let value=combined(Some(42),42,"Urist",0b1_0101_1111)?;let mut state=LiveSpatialCitizenState::default();assert_eq!(state.publish(value)?,JobPublication::Bootstrap);
    ...
    assert_eq!(entity.fields["stress_category"].value,Value::I64(3));assert_eq!(entity.fields["job_available_preserve_social"].value,Value::Bool(true));
    assert_eq!(entity.fields["job_available_interrupt_social"].value,Value::Bool(true));
    ...
    assert!(matches!(job.fields["worker_entity"].value,Value::Entity(id) if id==unit));assert_eq!(job.fields["worker_is_strict_citizen"].value,Value::Bool(true));
...
fn assigned_non_citizen_is_not_invented_as_a_unit()->dfmcp_core::Result<()>{
    ...
    let snapshot=state.snapshot().ok_or_else(||dfmcp_core::DfmcpError::new(dfmcp_core::ErrorCode::InternalInvariantViolation,"snapshot"))?;
    let job=snapshot.graph.entities.values().find(|e|e.kind==dfmcp_world::EntityKind::Job).ok_or_else(||dfmcp_core::DfmcpError::new(dfmcp_core::ErrorCode::InvalidRequest,"job"))?;
    assert_eq!(job.fields["worker_is_strict_citizen"].value,Value::Bool(false));assert!(matches!(job.fields["worker_entity"].presence,Some(FactPresence::Unknown(_))));
...
fn citizen_only_change_advances_combined_anchor_and_preserves_identity()->dfmcp_core::Result<()>{
    ...
    assert_eq!(second.graph.entities[&citizen_entity_id(42)].generation,first.graph.entities[&citizen_entity_id(42)].generation);assert_eq!(second.graph.entities[&citizen_entity_id(42)].label,"Domas");Ok(())
}
...
fn strict_roster_rejects_residents_malformed_skills_and_trailing_bytes()->dfmcp_core::Result<()>{
    ...
    assert!(LiveSpatialCitizenObservation::decode_payload(&bytes,7,"df".to_owned(),"dfhack".to_owned()).is_err());assert!(combined(None,42,"Resident",0b1_0111_1111).is_err());
