...
fn combined(worker:Option<u32>,citizen_id:u32,name:&str,flags:u16)->dfmcp_core::Result<LiveSpatialCitizenObservation>{
    ...
    let citizens=citizen_component(citizen_id,name,[1,1,5],0b1_0101_1111);
...
#[test]
fn strict_citizen_worker_becomes_generation_checked_performs_edge()->dfmcp_core::Result<()>{
    ...
    assert!(snapshot.graph.edges.values().any(|e|e.kind==EdgeKind::Performs&&e.from==unit&&e.to==job.id));Ok(())
}
...
#[test]
fn strict_roster_rejects_residents_duplicates_and_trailing_bytes()->dfmcp_core::Result<()>{
