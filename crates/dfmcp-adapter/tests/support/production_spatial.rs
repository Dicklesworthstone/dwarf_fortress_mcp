...
use dfmcp_adapter::live_spatial::{
    LiveSpatialObservation, citizens::LiveSpatialCitizenObservation,
};
...
fn invalid() -> DfmcpError {
    DfmcpError::new(
        ...
        "production fixture is incomplete",
    )
}
...
pub fn observation(
    ...
    operations.jobs.next_job_id = 10_000;
    operations.next_item_id = 10_000;
    operations.next_building_id = 10_000;
    ...
    for index in 0..jobs {
        ...
        job.native_id = 7 + index;
        job.suspended = index % 2 == 0;
        job.worker_native_id = if index == 0 { Some(10) } else { None };
        job.attached_item_count = 1;
        job.required_item_filter_count = 1;
        let mut attached = attachment.clone();
        attached.job_native_id = job.native_id;
        attached.filter_index = 0;
        operations.jobs.jobs.push(job);
        operations.attachments.push(attached);
    }
    operations
        .items
        .iter_mut()
        .find(|item| item.native_id == 31)
        .ok_or_else(invalid)?
        .flags = 1;
    let free = operations
        .items
        ...
        .ok_or_else(invalid)?;
    free.stack_size = free_units;
    free.flags = 64;
    free.container_native_id = None;
    free.holder_building_native_id = None;
    if empty {
        operations.jobs.jobs.clear();
        operations.attachments.clear();
    ...
    put(&mut citizens, u32::from(!empty));
    if !empty {
        put(&mut citizens, 10);
        ...
        put(&mut citizens, 6);
        citizens.extend_from_slice(&[0, 0]);
