...
pub fn observation(
    ...
    let mut other =
        operations.items.first().cloned().ok_or_else(|| {
            DfmcpError::new(ErrorCode::InvalidRequest, "site fixture stack missing")
        })?;
    other.native_id = 33;
    other.stack_size = east;
    other.raw_position.x = 3;
    ...
    let mut full = b"DFMS1800".to_vec();
    part(&mut full, &spatial);
    part(&mut full, &citizens);
