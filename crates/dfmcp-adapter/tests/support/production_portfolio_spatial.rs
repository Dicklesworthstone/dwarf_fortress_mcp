...
pub fn observation(tick: u32, workers: u32, units: u32, forbidden: bool) -> Result<LiveSpatialCitizenObservation> {
    ...
    operations.buildings.clear(); operations.items.retain(|i| i.native_id == 32);
    let item = operations.items.first_mut().ok_or_else(|| DfmcpError::new(ErrorCode::InvalidRequest, "portfolio fixture item missing"))?;
    item.flags = 64 | u32::from(forbidden); item.raw_position.x = 1; item.raw_position.y = 1; item.raw_position.z = 5;
    let tile = Tile { native_tiletype: 1, shape: Shape::Floor, liquid_depth: 0, magma: false, traffic: 0,
        dig_designation: 0, building_occupancy: 0, unit_occupancy: 0, walkable_region: 1,
