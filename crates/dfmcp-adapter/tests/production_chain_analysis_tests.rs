...
use dfmcp_adapter::operations_analysis::{self as analysis, MaterialDemand, OperationsStateView, MAX_ANALYSIS_WORK};
...
fn missing() -> DfmcpError { DfmcpError::new(ErrorCode::InvalidRequest, "production-chain fixture missing") }
fn context<S: OperationsStateView>(state: &S) -> Result<OperationContext> {
    ...
    Ok(OperationContext {
        session_id: SessionId::new(98125), request_id: RequestId::new(1), anchor,
        budget: WorkBudget { max_entities: 100_000, max_bytes: 16 * 1024 * 1024,
            max_output_tokens: 65_536, max_wall_millis: 60_000, ..WorkBudget::default() },
...
fn resources() -> Vec<ProductionResource> {
    [("raw", "item_type_3"), ("product", "item_type_999")].into_iter().map(|(key, kind)| ProductionResource {
        key: key.to_owned(), item_types: vec![kind.to_owned()], subtype: None,
        material_type: None, material_index: None,
    }).collect()
}
fn quota(key: &str, amount: u32) -> ProductionQuota {
    ProductionQuota { item_token: key.to_owned(), minimum_stock: amount }
}
fn recipe() -> ProductionRecipe {
    ProductionRecipe {
        output_token: "product".to_owned(), output_batch_size: 1,
...
fn chain_uses_the_existing_conservative_stock_policy_and_preserves_final_reserves() -> Result<()> {
    ...
    let report = plan_production_chain(&state, &c, &resources(),
        &[quota("product", 4), quota("raw", 4)], &[recipe()], MAX_ANALYSIS_WORK)?;
    let raw = report.resources.iter().find(|r| r.resource.key == "raw").ok_or_else(missing)?;
    ...
    let supply = analysis::plan_inventory(&state, &c, &[MaterialDemand {
        key: "raw".to_owned(), units: 1_000, item_types: vec!["item_type_3".to_owned()],
    ...
    assert_eq!(report.source_digest, SpatialStateView::source_digest(&state)?);
...
fn sealed_profiles_share_quantities_but_never_source_or_model_identity() -> Result<()> {
    ...
    let recipes = [recipe()];
    let a = plan_production_chain(&operations, &context(&operations)?, &resources(), &quotas, &recipes, MAX_ANALYSIS_WORK)?;
    let b = plan_production_chain(&spatial, &context(&spatial)?, &resources(), &quotas, &recipes, MAX_ANALYSIS_WORK)?;
    let c = plan_production_chain(&citizens, &context(&citizens)?, &resources(), &quotas, &recipes, MAX_ANALYSIS_WORK)?;
...
fn overlapping_aliases_are_refused_even_when_no_item_matches() -> Result<()> {
    ...
    for variant in 0..4 {
        ...
        match variant {
            ...
            3 => { definitions[1].material_type = Some(3); definitions[1].material_index = Some(2); }
            _ => {}
        }
        assert!(plan_production_chain(&state, &c, &definitions, &[quota("raw", 1)], &[], MAX_ANALYSIS_WORK)
    ...
    let report = plan_production_chain(&state, &c, &definitions,
        &[quota("raw", 1), quota("product", 1)], &[], MAX_ANALYSIS_WORK)?;
...
fn all_declared_references_and_duplicate_outputs_are_checked() -> Result<()> {
    let state = state()?;
    let c = context(&state)?;
    assert!(plan_production_chain(&state, &c, &resources(), &[quota("absent", 1)], &[], MAX_ANALYSIS_WORK)
        .is_err_and(|e| e.code == ErrorCode::InvalidRequest));
    let mut bad = recipe();
    bad.input_tokens[0].0 = "absent".to_owned();
    assert!(plan_production_chain(&state, &c, &resources(), &[quota("product", 1)], &[bad], MAX_ANALYSIS_WORK)
        .is_err_and(|e| e.code == ErrorCode::InvalidRequest));
    assert!(plan_production_chain(&state, &c, &resources(), &[quota("product", 1)], &[recipe(), recipe()], MAX_ANALYSIS_WORK)
        .is_err_and(|e| e.code == ErrorCode::InvalidRequest));
    let mut bad = recipe();
    bad.workshop = BuildingKind::Custom("not-a-workshop".to_owned());
    assert!(plan_production_chain(&state, &c, &resources(), &[quota("raw", 0)], &[bad], MAX_ANALYSIS_WORK)
        .is_err_and(|e| e.code == ErrorCode::InvalidRequest));
    Ok(())
}
...
fn model_identity_is_canonical_and_binds_recipe_semantics() -> Result<()> {
    ...
    let a = plan_production_chain(&state, &c, &resources(), &[quota("product", 2)], &[declaration.clone()], MAX_ANALYSIS_WORK)?;
    let mut reversed = resources();
    reversed.reverse();
    declaration.input_tokens = vec![("raw".to_owned(), 1), ("raw".to_owned(), 1)];
    let b = plan_production_chain(&state, &c, &reversed,
        &[quota("product", 1), quota("product", 2)], &[declaration.clone()], MAX_ANALYSIS_WORK)?;
    assert_eq!(a.model_digest, b.model_digest);
    assert_eq!(a.plan, b.plan);
    declaration.job_token = "DifferentDeclaredJob".to_owned();
    let changed = plan_production_chain(&state, &c, &resources(), &[quota("product", 2)], &[declaration], MAX_ANALYSIS_WORK)?;
...
fn item_examples_retain_generation_history_after_id_reuse() -> Result<()> {
    let mut state = state()?;
    let old = plan_production_chain(&state, &context(&state)?, &resources(), &[quota("product", 1)], &[recipe()], MAX_ANALYSIS_WORK)?;
    state.publish(fixture::observation(4, 0, 0, true)?)?;
    state.publish(fixture::observation(5, 2, 7, false)?)?;
    let current = plan_production_chain(&state, &context(&state)?, &resources(), &[quota("product", 1)], &[recipe()], MAX_ANALYSIS_WORK)?;
    assert_ne!(old.model_digest, current.model_digest);
    let raw = current.resources.iter().find(|r| r.resource.key == "raw").ok_or_else(missing)?;
    assert_eq!(raw.examples.len(), 1);
...
fn authority_anchor_scan_cancellation_expiry_and_work_refusals_are_read_only() -> Result<()> {
    ...
    for case in 0..8 {
        ...
        assert!(plan_production_chain(&state, &c, &resources(), &[quota("product", 4)], &[recipe()], MAX_ANALYSIS_WORK).is_err());
    }
    assert!(plan_production_chain(&state, &original, &resources(), &[quota("product", 4)], &[recipe()], 1)
