...
fn setup(
    ...
    state.publish(fixture::observation(3, workers, units, forbidden)?)?;
    let snapshot = state.snapshot().expect("fixture snapshot");
    let context = OperationContext {
        session_id: SessionId::new(9701),
...
fn joint_portfolio_reuses_candidates_not_a_stranded_full_demand_allocation() -> Result<()> {
    ...
    assert_eq!(report.selection.task_mask, 0b110);
    assert_eq!(report.selection.priority, 4);
    ...
    assert_eq!(report.workforce.anchor, c.anchor);
    assert_eq!(report.inventory.anchor, c.anchor);
...
fn unavailable_materials_or_unobserved_skills_never_support_a_complete_task() -> Result<()> {
    for forbidden in [false, true] {
        ...
        let mut unknown = task("missing-skill", 10, 1, 1);
        unknown.skill_key = "NOT_OBSERVED".into();
        let report = portfolio::plan(
            &state,
            &c,
            [0, 0, 5],
            &[unknown, task("known", 1, 1, 1)],
            10_000_000,
        )?;
        ...
        if forbidden {
            assert_eq!(
                report
                    .inventory
                    .item_counts
                    .get("excluded_item_or_container_policy"),
                Some(&1)
            );
        }
    }
...
fn equivalent_task_order_and_type_sets_produce_the_same_joint_plan() -> Result<()> {
    ...
    let mut other = requested.to_vec();
    other.reverse();
    other[0].materials[0].item_types.push("item_type_3".into());
...
fn authority_anchor_input_and_work_failures_do_not_produce_partial_plans() -> Result<()> {
    let (state, c) = setup(2, 2, false)?;
    let requested = [task("a", 1, 1, 1)];
    let mut denied = c.clone();
    denied.grants.clear();
    assert!(
        matches!(portfolio::plan(&state,&denied,[0,0,5],&requested,10_000_000),Err(e)if e.code==ErrorCode::CapabilityDenied)
    );
    let mut stale = c.clone();
    stale.anchor.cursor.sequence += 1;
    assert!(
        matches!(portfolio::plan(&state,&stale,[0,0,5],&requested,10_000_000),Err(e)if e.code==ErrorCode::StaleAnchor)
    );
    ...
    let duplicate = bad[0].materials[0].clone();
    bad[0].materials.push(duplicate);
    ...
    assert!(
        portfolio::plan(
            ...
            &c,
            [0, 0, 5],
            &[requested[0].clone(), requested[0].clone()],
            10_000_000
        )
        .is_err()
    );
