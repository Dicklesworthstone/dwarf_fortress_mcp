...
use dfmcp_world::{
    CompareOp, EntityKind, FactPresence, Predicate, QueryOrder, Value, WorldQuery, execute_query,
};
...
fn native_roster_reaches_existing_typed_query_without_fake_related_entities() -> Result<()> {
    ...
    assert!(matches!(
        result.entities[0].fields["blocking_reason"].presence,
