...
use dfmcp_adapter::operations_analysis::{
    self as analysis, DiagnosisScope, MAX_ANALYSIS_WORK, MaterialDemand, OperationsStateView,
};
use dfmcp_core::{
    Capability, CapabilityGrant, CapabilityScope, DfmcpError, EntityId, ErrorCode,
...
fn invalid() -> DfmcpError {
    DfmcpError::new(
        ...
        "missing production test projection",
    )
}
fn context<S: OperationsStateView>(state: &S) -> Result<OperationContext> {
    ...
    Ok(OperationContext {
        session_id: SessionId::new(98123),
...
#[test]
fn operations_spatial_and_citizen_profiles_share_rules_without_sharing_source_identity()
-> Result<()> {
    ...
    let mut map = LiveSpatialState::default();
    map.publish(observation.spatial().clone())?;
    let mut citizens = LiveSpatialCitizenState::default();
    citizens.publish(observation)?;
    let a = analysis::diagnose_production(
        &old,
        &context(&old)?,
        DiagnosisScope::default(),
        MAX_ANALYSIS_WORK,
    )?;
    let b = analysis::diagnose_production(
        &map,
        &context(&map)?,
        DiagnosisScope::default(),
        MAX_ANALYSIS_WORK,
    )?;
    let c = analysis::diagnose_production(
        &citizens,
        &context(&citizens)?,
        DiagnosisScope::default(),
        MAX_ANALYSIS_WORK,
    ...
    assert_eq!(a.finding_counts, c.finding_counts);
    ...
    let supply_b = analysis::plan_inventory(
        &citizens,
        &context(&citizens)?,
        &demands(),
        MAX_ANALYSIS_WORK,
    )?;
    assert_eq!(supply_a.allocation, supply_b.allocation);
    assert_eq!(supply_a.excluded_items, supply_b.excluded_items);
    assert_eq!(supply_b.anchor, c.anchor);
    assert_eq!(supply_b.source_digest, c.source_digest);
    Ok(())
}
...
fn recycled_jobs_holders_and_items_retain_the_full_spatial_generation_history() -> Result<()> {
    ...
    state.publish(fixture::observation(3, 1, 7, false)?)?;
    let old = analysis::diagnose_production(
        &state,
        &context(&state)?,
        DiagnosisScope::default(),
        MAX_ANALYSIS_WORK,
    ...
    let c = context(&state)?;
    assert!(
        matches!(analysis::diagnose_production(&state,&c,DiagnosisScope {job:Some((old_job.entity_id,old_job.generation)),
        ..DiagnosisScope::default()},MAX_ANALYSIS_WORK),Err(e)if e.code==ErrorCode::Conflict)
    );
    let report = analysis::diagnose_production(
        &state,
        &c,
        DiagnosisScope {
            job: Some((old_job.entity_id, 2)),
            ..DiagnosisScope::default()
        },
        MAX_ANALYSIS_WORK,
    )?;
    let row = report.rows.first().ok_or_else(invalid)?;
    assert_eq!(row.job.generation, 2);
    assert_eq!(row.holder.map(|h| h.generation), Some(2));
    assert!(!row.affected_item_examples.is_empty());
    assert!(row.affected_item_examples.iter().all(|h| h.generation == 2));
    assert_eq!(
        state
            .snapshot()
            .and_then(|s| s.graph.entities.get(&item_entity_id(32)))
            .map(|e| e.generation),
        Some(2)
    );
...
fn conservative_supply_excludes_inherited_flags_and_never_double_counts_a_stack() -> Result<()> {
    ...
    let c = context(&state)?;
    let diagnosis =
        analysis::diagnose_production(&state, &c, DiagnosisScope::default(), MAX_ANALYSIS_WORK)?;
    assert!(
        diagnosis
            .rows
            .iter()
            .all(|row| row.container_item_flags.get("forbidden") == Some(&1))
    );
    assert!(
        diagnosis
            .rows
            .iter()
            .all(|row| row.shared_attached_items == 1)
    );
    let supply = analysis::plan_inventory(&state, &c, &demands(), MAX_ANALYSIS_WORK)?;
    assert_eq!(supply.candidate_items, 1);
    assert_eq!(supply.candidate_stack_units, 7);
    assert_eq!(supply.allocation.requested_units, 8);
    assert_eq!(supply.allocation.allocated_units, 7);
    assert_eq!(
        supply
            .allocation
            .assignments
    ...
    assert!(
        supply
            ...
            .assignments
            .iter()
            .all(|a| a.supply_id == item_entity_id(32).get())
    );
    assert_eq!(
        supply.allocation.shortage.as_ref().map(|cut| cut.deficit),
        Some(1)
    );
    Ok(())
}
...
fn full_projection_authority_anchor_cancellation_and_work_limits_are_checked() -> Result<()> {
    ...
    assert!(
        matches!(analysis::diagnose_production(&state,&original,DiagnosisScope::default(),1),Err(e)if e.code==ErrorCode::BudgetExceeded)
    );
    assert!(
        matches!(analysis::plan_inventory(&state,&original,&demands(),1),Err(e)if e.code==ErrorCode::BudgetExceeded)
    );
