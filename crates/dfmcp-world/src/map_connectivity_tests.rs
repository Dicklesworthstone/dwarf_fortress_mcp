use super::*;
use crate::map_region::{MAX_MAP_TILES, Region, Tile};
    ...
    while let Some(root) = remaining.pop_first() {
        ...
        while let Some(from) = queue.pop_front() {
            for &to in &graph[from] {
                if edge == Some([from.min(to), from.max(to)]) {
                    continue;
                }
...
fn corridor_reports_exact_vertex_and_edge_failure_impact() -> Result<(), MapError> {
    ...
    assert_eq!(report.candidate_tiles, 5);
...
fn disconnected_areas_isolated_tiles_and_cycles_are_not_conflated() -> Result<(), MapError> {
    let mut map = grid([7, 1, 1])?;
    map.cells[2] = Cell::Hidden;
    map.cells[5] = Cell::Unallocated;
    let report = analyze(&map, MAX_ROUTE_WORK)?;
    assert_eq!(
        report
            .components
            .iter()
            .map(|c| (c.representative, c.tiles))
            .collect::<Vec<_>>(),
        vec![(0, 2), (3, 2), (6, 1)]
    );
    assert_eq!(
        report.component_of,
        vec![Some(0), Some(0), None, Some(1), Some(1), None, Some(2)]
    );
    assert!(report.bottlenecks.is_empty());
    assert_eq!(report.bridges.len(), 2);
    let mut ring = grid([3, 3, 1])?;
    ring.cells[4] = Cell::Hidden;
    let ring = analyze(&ring, MAX_ROUTE_WORK)?;
    assert_eq!(ring.components.len(), 1);
    assert_eq!(ring.edges, 8);
    assert!(ring.bottlenecks.is_empty());
    assert!(ring.bridges.is_empty());
    Ok(())
}
...
fn every_small_map_matches_destructive_vertex_and_edge_oracles() -> Result<(), MapError> {
    for mask in 0..512u32 {
        ...
        for (ordinal, component) in expected.iter().enumerate() {
            ...
            assert_eq!(
                Some(&report.components[ordinal].representative),
                component.first()
            );
            for &node in component {
                assert_eq!(report.component_of[node], Some(ordinal));
                let mut sizes: Vec<_> = groups(&graph, component, Some(node), None)
                    .iter()
                    .map(|s| s.len() as u32)
                    .collect();
                sizes.sort_unstable_by(|a, b| b.cmp(a));
                if sizes.len() > 1 {
                    cuts.insert(node, sizes);
                }
        ...
        assert_eq!(
            report
                .bottlenecks
                .iter()
                .map(|b| (b.tile, b.partition_sizes.clone()))
        ...
        assert_eq!(
            report
                .bridges
                .iter()
                .map(|b| (b.endpoints, b.side_sizes))
                .collect::<BTreeMap<_, _>>(),
            edges
        );
        for &a in &members {
            for &b in &members {
                let route = map.route(
                    map.region.position(a).ok_or(MapError::InvalidRegion)?,
                    map.region.position(b).ok_or(MapError::InvalidRegion)?,
                    MAX_ROUTE_WORK,
                )?;
                assert_eq!(
                    !route.path.is_empty(),
                    report.component_of[a] == report.component_of[b]
                );
            }
        }
    }
...
fn all_vertical_shape_pairs_match_the_existing_route_policy() -> Result<(), MapError> {
    for lower in 0..=8 {
        for upper in 0..=8 {
            ...
            if let Cell::Visible(t) = &mut map.cells[0] {
                t.shape = Shape::from_tag(lower)?;
            }
            if let Cell::Visible(t) = &mut map.cells[1] {
                t.shape = Shape::from_tag(upper)?;
            }
            let report = analyze(&map, MAX_ROUTE_WORK)?;
...
fn stacked_stair_corridor_has_correct_cuts_in_both_directions() -> Result<(), MapError> {
    ...
    for cell in &mut map.cells {
        if let Cell::Visible(t) = cell {
            t.shape = Shape::StairUpDown;
        }
    }
    let report = analyze(&map, MAX_ROUTE_WORK)?;
    assert_eq!(report.components[0].tiles, 4);
    ...
    for case in 0..7 {
        ...
        match case {
            ...
            _ => {
                if let Cell::Visible(t) = &mut map.cells[1] {
                    match case {
                        2 => t.shape = Shape::Ramp,
                        3 => t.liquid_depth = 1,
                        4 => t.building_occupancy = 1,
                        5 => t.unit_occupancy = 1,
                        _ => t.walkable_region = 0,
                    }
                }
            }
        }
        ...
        assert_eq!(report.candidate_tiles, 2);
        assert!(report.bottlenecks.is_empty());
...
fn maximum_map_and_long_snake_use_bounded_iterative_traversal() -> Result<(), MapError> {
    ...
    assert!(report.bottlenecks.is_empty());
    assert!(report.bridges.is_empty());
    assert!(report.work_units < MAX_ROUTE_WORK);
    let mut snake = map;
    for y in 0..128usize {
        for x in 0..128usize {
            if y % 2 == 1 && x != if y % 4 == 1 { 127 } else { 0 } {
                snake.cells[y * 128 + x] = Cell::Hidden;
            }
        }
    }
...
fn cooperative_and_work_refusals_never_return_partial_components() -> Result<(), MapError> {
    ...
    assert_eq!(
        analyze_with_check(&map, MAX_ROUTE_WORK, || {
            checks += 1;
            if checks == 3 {
    ...
    malformed.cells.pop();
    assert_eq!(
        analyze(&malformed, MAX_ROUTE_WORK),
        Err(MapError::InvalidRegion)
    );
    if let Cell::Visible(t) = &mut malformed.cells[0] {
        t.liquid_depth = 8;
    }
    malformed.cells.push(floor());
    assert_eq!(
        analyze(&malformed, MAX_ROUTE_WORK),
        Err(MapError::InvalidTile)
    );
    Ok(())
}
