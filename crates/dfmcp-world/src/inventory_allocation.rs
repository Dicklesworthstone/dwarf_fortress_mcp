...
impl Work {
    fn charge(&mut self, units: u64) -> Result<()> {
        self.used = self.used.checked_add(units).ok_or(AllocationError::ArithmeticOverflow)?;
        if self.used > self.maximum { return Err(AllocationError::BudgetExceeded); }
        Ok(())
    }
}

#[derive(Clone, Copy)]
struct Arc { to: usize, reverse: usize, remaining: u64, original: u64 }

fn add_arc(graph: &mut [Vec<Arc>], from: usize, to: usize, capacity: u64) -> usize {
    let index = graph[from].len();
    let reverse = graph[to].len();
    graph[from].push(Arc { to, reverse, remaining: capacity, original: capacity });
    graph[to].push(Arc { to: from, reverse: index, remaining: 0, original: 0 });
    index
}

fn sum(left: u64, right: u64) -> Result<u64> {
    left.checked_add(right).ok_or(AllocationError::ArithmeticOverflow)
}
...
pub fn allocate(supplies: &[Supply], demands: &[Demand], max_work: u64) -> Result<Allocation> {
    if supplies.len() > MAX_SUPPLIES || demands.is_empty() || demands.len() > MAX_DEMANDS
        || max_work == 0 || max_work > MAX_WORK {
        return Err(AllocationError::BudgetExceeded);
    }
    let mut work = Work { used: 0, maximum: max_work };
    let mask_limit = if demands.len() == 32 { u32::MAX } else { (1u32 << demands.len()) - 1 };
    ...
    for (index, supply) in supplies.iter().enumerate() {
        work.charge(1)?;
        if supply.id == 0 || supply.units == 0 || supply.eligible == 0
            || supply.eligible & !mask_limit != 0 || (index > 0 && supplies[index - 1].id >= supply.id) {
            return Err(AllocationError::InvalidInput);
        }
    ...
    for (i, (mask, (units, _))) in groups.iter().enumerate() {
        ...
        for d in 0..demands.len() {
            work.charge(1)?;
            if mask & (1u32 << d) != 0 {
                if arcs + 2 > MAX_ARCS { return Err(AllocationError::BudgetExceeded); }
    ...
    let reached = loop {
        ...
        if amount == 0 { return Err(AllocationError::InvariantViolation); }
        node = sink;
        while node != source {
            work.charge(1)?;
            let (from, edge) = parents[node].ok_or(AllocationError::InvariantViolation)?;
        ...
        if allocated > requested { return Err(AllocationError::InvariantViolation); }
    };
    let mut cut_capacity = 0u64;
    for (node, neighbors) in graph.iter().enumerate() {
        for arc in neighbors {
            work.charge(1)?;
            if reached[node] && !reached[arc.to] { cut_capacity = sum(cut_capacity, arc.original)?; }
        }
    }
    if cut_capacity != allocated { return Err(AllocationError::InvariantViolation); }
    let shortage = if allocated < requested {
        let demand_indices: Vec<_> = (0..demands.len()).filter(|d| !reached[demand_start + d]).collect();
        let mut required_units = 0;
        for &d in &demand_indices { required_units = sum(required_units, demands[d].units)?; }
        let subset = demand_indices.iter().fold(0u32, |mask, d| mask | (1u32 << d));
        let mut eligible_units = 0;
        for (mask, (units, _)) in &groups {
            work.charge(1)?;
            if mask & subset != 0 { eligible_units = sum(eligible_units, *units)?; }
        }
        let deficit = required_units.checked_sub(eligible_units).ok_or(AllocationError::InvariantViolation)?;
        if deficit != requested - allocated || deficit == 0 { return Err(AllocationError::InvariantViolation); }
        Some(Shortage { demand_indices, required_units, eligible_units, deficit })
    } else { None };
    ...
    for (g, (_, (_, members))) in groups.iter().enumerate() {
        ...
        for &(demand_index, edge) in &links[g] {
            ...
            while remaining > 0 {
                work.charge(1)?;
                let supply = members.get(member).ok_or(AllocationError::InvariantViolation)?;
                let amount = remaining.min(supply.units - used);
                if amount == 0 || assignments.len() >= MAX_ASSIGNMENTS { return Err(AllocationError::BudgetExceeded); }
                assignments.push(Assignment { supply_id: supply.id, demand_index, units: amount });
                allocated_by_demand[demand_index] = sum(allocated_by_demand[demand_index], amount)?;
                used += amount;
                remaining -= amount;
                if used == supply.units { member += 1; used = 0; }
            }
        }
    }
    ...
    for (d, units) in allocated_by_demand.iter().enumerate() {
        if *units > demands[d].units { return Err(AllocationError::InvariantViolation); }
        verified_total = sum(verified_total, *units)?;
    }
    if verified_total != allocated { return Err(AllocationError::InvariantViolation); }
    Ok(Allocation { assignments, allocated_by_demand, requested_units: requested,
        allocated_units: allocated, cut_capacity, shortage, work_units: work.used })
}
...
mod tests {
    ...
    fn demands(units: &[u64]) -> Vec<Demand> {
        units.iter().enumerate().map(|(i, &units)| Demand { key: format!("d{i:02}"), units }).collect()
    }
    fn verify(supplies: &[Supply], demands: &[Demand], result: &Allocation) {
        ...
        for supply in supplies { assert!(used.get(&supply.id).copied().unwrap_or(0) <= supply.units); }
        ...
        for (d, units) in demands.iter().zip(&fulfilled) { assert!(*units <= d.units); }
        if let Some(cut) = &result.shortage {
            let mask = cut.demand_indices.iter().fold(0, |mask, &d| mask | (1u32 << d));
            assert_eq!(cut.required_units, cut.demand_indices.iter().map(|&d| demands[d].units).sum::<u64>());
            assert_eq!(cut.eligible_units, supplies.iter().filter(|s| s.eligible & mask != 0).map(|s| s.units).sum::<u64>());
        ...
        } else { assert_eq!(result.requested_units, result.allocated_units); }
    }
    #[test]
    fn residual_rerouting_finds_the_assignment_greedy_consumption_misses() -> Result<()> {
        let supplies = vec![Supply { id: 1, units: 1, eligible: 3 },
            Supply { id: 2, units: 1, eligible: 4 }, Supply { id: 3, units: 1, eligible: 5 }];
    ...
    fn grouped_stacks_split_without_double_counting() -> Result<()> {
        let supplies = vec![Supply { id: 4, units: 3, eligible: 3 }, Supply { id: 9, units: 5, eligible: 3 }];
        let needs = demands(&[5, 3]);
        let result = allocate(&supplies, &needs, MAX_WORK)?;
        assert_eq!(result.assignments, vec![Assignment { supply_id: 4, demand_index: 0, units: 3 },
            Assignment { supply_id: 9, demand_index: 0, units: 2 }, Assignment { supply_id: 9, demand_index: 1, units: 3 }]);
        verify(&supplies, &needs, &result);
    ...
    fn joint_shortage_is_not_an_independent_per_demand_availability_claim() -> Result<()> {
        let supplies = vec![Supply { id: 1, units: 5, eligible: 3 }];
    ...
    fn absent_supply_and_disconnected_demand_get_exact_certificates() -> Result<()> {
        for supplies in [Vec::new(), vec![Supply { id: 1, units: 10, eligible: 1 }]] {
    ...
    fn highest_mask_bit_and_large_integral_counts_do_not_round() -> Result<()> {
        ...
        let supplies = vec![Supply { id: 1, units: 1, eligible: 1 << 31 }];
        verify(&supplies, &needs, &allocate(&supplies, &needs, MAX_WORK)?);
        let needs = demands(&[9_007_199_254_740_993]);
        let supplies = vec![Supply { id: 1, units: 9_007_199_254_740_993, eligible: 1 }];
        let result = allocate(&supplies, &needs, MAX_WORK)?;
    ...
    fn malformed_overflowing_and_exhausted_inputs_return_no_partial_solution() {
        let needs = demands(&[1]);
        for supply in [Supply { id: 0, units: 1, eligible: 1 }, Supply { id: 1, units: 0, eligible: 1 },
            Supply { id: 1, units: 1, eligible: 2 }, Supply { id: 1, units: 1, eligible: 0 }] {
            assert_eq!(allocate(&[supply], &needs, MAX_WORK), Err(AllocationError::InvalidInput));
        }
        assert_eq!(allocate(&[], &demands(&[u64::MAX, 1]), MAX_WORK), Err(AllocationError::ArithmeticOverflow));
        assert_eq!(allocate(&[], &needs, 1), Err(AllocationError::BudgetExceeded));
        let repeated = vec![Supply { id: 1, units: 1, eligible: 1 }; 2];
        assert_eq!(allocate(&repeated, &needs, MAX_WORK), Err(AllocationError::InvalidInput));
        assert!(allocate(&[], &demands(&[1; 33]), MAX_WORK).is_err());
    }
    fn brute_force(units: &[u32], remaining: &mut [u64], index: usize) -> u64 {
        if index == units.len() { return 0; }
    ...
    fn exhaustive_small_models_match_independent_enumeration_and_cut_checks() -> Result<()> {
        for a in 1..8u32 { for b in 1..8u32 { for x in 1..=2u64 { for y in 1..=2u64 {
            for demand_bits in 0..8 {
                let needs = demands(&(0..3).map(|d| 1 + ((demand_bits >> d) & 1)).collect::<Vec<u64>>());
                let supplies = vec![Supply { id: 1, units: x, eligible: a }, Supply { id: 2, units: y, eligible: b }];
                let mut units = vec![a; x as usize]; units.extend(vec![b; y as usize]);
                let mut remaining = needs.iter().map(|d| d.units).collect::<Vec<_>>();
                let expected = brute_force(&units, &mut remaining, 0);
                let result = allocate(&supplies, &needs, MAX_WORK)?;
                assert_eq!(result.allocated_units, expected, "masks {a}/{b}, units {x}/{y}, demands {demand_bits}");
                verify(&supplies, &needs, &result);
            }
        }}}}
        Ok(())
