...
fn fixed_vector_agrees_with_independent_python_reference() -> Result<(), Error> {
    ...
    let expected = hex(
        "44464d444c543031000002000000020000000004000000000b0001020304050607080963010000000b000001170000000004414243440100000126000000da",
    )?;
...
fn raw_fallback_handles_small_incompressible_and_overfragmented_payloads() -> Result<(), Error> {
    ...
    for byte in fragmented.iter_mut().skip(32).step_by(33) {
        *byte = 2;
    }
...
fn every_incomplete_prefix_and_trailing_byte_is_rejected() -> Result<(), Error> {
    ...
    for end in 0..delta.len() {
        assert!(decode(&base, &delta[..end], MAX_PAYLOAD).is_err());
    }
    let mut trailing = delta.clone();
    trailing.push(0);
    assert_eq!(
        decode(&base, &trailing, MAX_PAYLOAD),
        Err(Error::InvalidEncoding)
    );
    assert_eq!(
        decode(&base[..4095], &delta, MAX_PAYLOAD),
        Err(Error::WrongBaseLength)
    );
    Ok(())
}
...
fn output_and_command_bounds_are_validated_before_reconstruction() -> Result<(), Error> {
    ...
    assert_eq!(
        decode(&base, &delta, MAX_PAYLOAD + 1),
    ...
    for commands in [
        ...
        vec![1, 255, 255, 255, 255, 0, 0, 2, 0],
        vec![1, 0, 0, 2, 0, 0, 0, 0, 1],
        vec![1, 0, 0, 0, 0, 0, 0, 2, 1],
    ] {
        assert_eq!(
            decode(&base, &frame(512, 512, 1, &commands), 512),
            Err(Error::InvalidEncoding)
        );
    }
    for count in [0, MAX_COMMANDS + 1] {
        assert_eq!(
            decode(&base, &frame(512, 512, count, &[]), 512),
            Err(Error::InvalidEncoding)
        );
    }
    let overflow = frame(512, MAX_PAYLOAD + 1, 1, &[1, 0, 0, 0, 0, 0, 0, 2, 0]);
    assert_eq!(
        decode(&base, &overflow, MAX_PAYLOAD),
        Err(Error::LimitExceeded)
    );
    Ok(())
}
...
fn generated_byte_edits_roundtrip_with_deterministic_raw_fallback() -> Result<(), Error> {
    ...
    for case in 0..2048usize {
        ...
        for _ in 0..(case % 9) {
            ...
            match next() % 3 {
                0 => {
                    target.insert(at, next() as u8);
                }
                1 if at < target.len() => {
                    target.remove(at);
                }
                _ if at < target.len() => {
                    target[at] = next() as u8;
                }
                _ => {}
            }
        }
