...
use crate::map_region::{Cell, MAX_ROUTE_WORK, MapError, MapRegion, Region, Shape};
...
impl Reachability {
    ...
    pub fn compute(
        ...
        let mut result = Self {
            region: map.region,
            distance: vec![None; map.cells.len()],
            parent: vec![usize::MAX; map.cells.len()],
            visited_tiles: 0,
            work_units: 0,
        ...
        while let Some(current) = queue.pop_front() {
            ...
            for axis in 0..3 {
                ...
                for up in [false, true] {
                    ...
                    if let Some(value) = value {
                        ...
                        next[axis] = value;
                        if let Some(index) = map.region.index(next) {
                            neighbors.push((index, axis, up));
                        }
                    }
                }
            }
            ...
            for (next, axis, up) in neighbors {
                result.charge(1, maximum_work)?;
                if result.distance[next].is_some() || !map.candidate(next) {
                    continue;
                }
                if axis == 2 {
                    let (Cell::Visible(a), Cell::Visible(b)) =
                        (map.cells[current], map.cells[next])
                    else {
                        continue;
                    };
                    ...
                    if !(if up {
                        ascends(a.shape) && descends(b.shape)
                    } else {
                        descends(a.shape) && ascends(b.shape)
                    }) {
                        continue;
                    }
                }
...
mod tests {
    ...
    fn every_three_by_three_obstacle_map_matches_existing_routes() -> Result<(), MapError> {
        for mask in 0..512u32 {
            ...
            if !map.candidate(0) {
                assert_eq!(
                    Reachability::compute(&map, &[[0; 3]], 1000),
            ...
            for i in 0..9 {
                ...
                assert_eq!(
                    field.distance_to(target),
                    route.path.len().checked_sub(1).map(|n| n as u32)
                );
    ...
    fn normalized_multi_source_ties_and_stairs() -> Result<(), MapError> {
        ...
        if let Cell::Visible(tile) = &mut map.cells[4] {
    ...
    fn invalid_start_and_exhausted_work_are_not_empty_supply() -> Result<(), MapError> {
        ...
        assert_eq!(
            Reachability::compute(&map, &[[0; 3]], 1),
            Err(MapError::BudgetExceeded)
        );
        assert_eq!(
            Reachability::compute(&map, &[[3, 0, 0]], 100),
