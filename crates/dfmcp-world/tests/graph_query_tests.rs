...
use dfmcp_core::{
    Digest32, EdgeId, EntityId, ErrorCode, FortressId, GameTick, ObservationCursor, Result,
};
...
fn scope() -> Digest32 {
    Digest32::of_bytes(b"authorized-fixture-projection")
}
...
fn query(roots: &[u64], direction: GraphDirection, max_depth: u32) -> GraphTraversalQuery {
    GraphTraversalQuery {
        ...
        edge_kinds: vec![EdgeKind::Requires],
        direction,
        max_depth,
    }
}
...
fn diamond_ties_have_one_canonical_shortest_path() -> Result<()> {
    ...
    let result = traverse_graph(
        &snapshot,
        scope(),
        &query(&[1], GraphDirection::Outgoing, 10),
        GraphBudget::default(),
    )?;
...
fn source_order_duplicates_and_parallel_edges_are_deterministic() -> Result<()> {
    ...
    let left = traverse_graph(
        &snapshot,
        scope(),
        &query(&[2, 1, 2], GraphDirection::Outgoing, 10),
        GraphBudget::default(),
    )?;
    let right = traverse_graph(
        &snapshot,
        scope(),
        &query(&[1, 2], GraphDirection::Outgoing, 10),
        GraphBudget::default(),
    )?;
...
fn reverse_and_undirected_traversal_preserve_edge_witnesses() -> Result<()> {
    ...
    let incoming = traverse_graph(
        &snapshot,
        scope(),
        &query(&[3], GraphDirection::Incoming, 5),
        GraphBudget::default(),
    )?;
    assert_eq!(
        incoming
            .visits
            .iter()
            .map(|v| v.entity_id.get())
            .collect::<Vec<_>>(),
        vec![3, 2, 1]
    );
    assert_eq!(incoming.visits[1].via_edge, Some(EdgeId::new(2)));
    let both = traverse_graph(
        &snapshot,
        scope(),
        &query(&[2], GraphDirection::Undirected, 5),
        GraphBudget::default(),
    )?;
    assert_eq!(
        both.visits
...
fn dependency_order_and_critical_chain_put_requirements_first() -> Result<()> {
    ...
    assert_eq!(
        result.dependency_order.as_deref().map(ids),
        Some(vec![4, 2, 3, 1])
    );
    assert_eq!(
        result.critical_chain.as_deref().map(ids),
        Some(vec![1, 2, 4])
    );
...
fn cycles_and_all_their_dependents_are_explained_without_a_fake_schedule() -> Result<()> {
    ...
    let self_loop =
        analyze_dependencies(&world(1, &[(1, 1)]), scope(), &[], GraphBudget::default())?;
...
fn empty_and_disconnected_graphs_have_a_real_total_order() -> Result<()> {
    ...
    assert_eq!(
        disconnected.dependency_order.as_deref().map(ids),
        Some(vec![1, 2, 3])
    );
    Ok(())
}
...
fn bad_inputs_and_exhausted_budgets_never_return_partial_success() {
    ...
    for budget in [
        ...
        assert!(
            matches!(traverse_graph(&snapshot, scope(), &query(&[1], GraphDirection::Outgoing, 10), budget),
    ...
    assert!(
        traverse_graph(
            &snapshot,
            scope(),
            &query(&[99], GraphDirection::Outgoing, 1),
            GraphBudget::default()
        )
        .is_err()
    );
    ...
    assert!(
        matches!(analyze_dependencies(&invalid, scope(), &[], GraphBudget::default()),
        Err(error) if error.code == ErrorCode::InternalInvariantViolation)
    );
    let dangling = world(2, &[(1, 99)]);
    assert!(
        matches!(analyze_dependencies(&dangling, scope(), &[], GraphBudget::default()),
        Err(error) if error.code == ErrorCode::InternalInvariantViolation)
    );
}
...
fn kind_and_scope_identity_do_not_alias() -> Result<()> {
    let snapshot = world(3, &[(1, 2), (2, 3)]);
    let selected = analyze_dependencies(
        &snapshot,
        scope(),
        &[EdgeKind::Requires],
        GraphBudget::default(),
    )?;
    let other = analyze_dependencies(
        &snapshot,
        scope(),
        &[EdgeKind::Custom("requires".to_owned())],
        GraphBudget::default(),
    )?;
    assert_ne!(
        selected.witness.projection_digest,
        other.witness.projection_digest
    );
    assert_eq!(
        other.dependency_order.as_deref().map(ids),
        Some(vec![1, 2, 3])
    );
    let rescope = analyze_dependencies(
        &snapshot,
        Digest32::of_bytes(b"other-authorized-scope"),
        &[EdgeKind::Requires],
        GraphBudget::default(),
    )?;
    assert_eq!(selected.components, rescope.components);
    assert_ne!(
        selected.witness.decision_digest,
        rescope.witness.decision_digest
    );
    Ok(())
}
...
fn corrupted_path_records_fail_instead_of_looping() -> Result<()> {
    ...
    let mut result = traverse_graph(
        &snapshot,
        scope(),
        &query(&[1], GraphDirection::Outgoing, 5),
        GraphBudget::default(),
    )?;
...
fn exhaustive_three_vertex_graphs_match_independent_closure_and_distance_oracles() -> Result<()> {
    for mask in 0u16..512 {
        ...
        let cyclic: BTreeSet<_> = (0..3)
            ...
            .collect();
        assert_eq!(
            analysis.dependency_order.is_some(),
            cyclic.is_empty(),
            "mask={mask}"
        );
        for component in &analysis.components {
            for id in &component.members {
                ...
                assert_eq!(
                    component.cyclic,
                    cyclic.contains(&(id.get() as usize - 1)),
                    "mask={mask}"
                );
            }
        }
        for i in 0..3 {
            for j in 0..3 {
                assert_eq!(
                    component_of[i] == component_of[j],
                    distance[i][j] < 99 && distance[j][i] < 99,
                    "mask={mask}"
                );
            }
        }
        let expected_blocked: Vec<_> = (0..3)
            .filter(|&i| cyclic.iter().any(|&c| distance[i][c] < 99))
            .map(|i| i as u64 + 1)
            .collect();
        assert_eq!(
            ids(&analysis.blocked_by_cycles),
            expected_blocked,
            "mask={mask}"
        );
        ...
        let traversal = traverse_graph(
            &snapshot,
            scope(),
            &query(&[1], GraphDirection::Outgoing, 3),
            GraphBudget::default(),
        )?;
        for vertex in 0..3 {
            let found = traversal
                .visits
                .iter()
                .find(|visit| visit.entity_id.get() == vertex as u64 + 1);
            assert_eq!(
                found.map(|visit| visit.depth as usize),
                (distance[0][vertex] < 99).then_some(distance[0][vertex]),
                "mask={mask}"
            );
        }
