...
use dfmcp_world::{
    ...
    WorldGraph, WorldQuery, WorldSnapshot,
};
...
fn changing_filters_or_order_cannot_reuse_a_page() -> Result<()> {
    ...
    for change in 0..3 {
        ...
        match change {
            ...
            _ => {
                changed.predicate = Some(Predicate::FieldCompare {
...
fn kind_selection_is_a_set_but_custom_names_do_not_alias_builtins() -> Result<()> {
    ...
    assert_eq!(
        execute_query(&snapshot, &query, 100)?.entities[0].id.get(),
        3
    );
...
fn byte_budget_produces_progressing_whole_row_pages() -> Result<()> {
    let snapshot = snapshot(7);
    let bytes = snapshot.graph.entities[&EntityId::new(1)]
        .canonical_bytes()
        .len();
    let mut query = query();
    ...
    assert!(
        matches!(execute_bounded_query(&snapshot, &query, 100, Some(1)),
        Err(error) if error.code == ErrorCode::BudgetExceeded)
    );
    Ok(())
}
...
fn aggregate_query_bytes_and_nested_kind_names_are_bounded() {
    ...
    let mut query = query();
    query.predicate = Some(Predicate::All(
        (0..5)
            .map(|_| Predicate::FieldCompare {
                entity_id: EntityId::NIL,
                field: "stress".to_owned(),
                op: CompareOp::Eq,
    ...
    query.predicate = Some(Predicate::EntityKind {
        entity_id: EntityId::NIL,
        kind: EntityKind::Other("x".repeat(129)),
    });
    assert!(matches!(execute_query(&snapshot, &query, 100),
        Err(error) if error.code == ErrorCode::BudgetExceeded));
}
