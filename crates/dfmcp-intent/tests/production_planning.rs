...
fn joint_goals_cannot_each_spend_the_same_initial_stock() -> Result<()> {
    ...
    assert!(
        compiler
            .compile_quota_work_orders("BARREL", 2, &stock)
            .is_ok()
    );
...
fn output_quota_is_preserved_when_other_recipes_consume_that_output() -> Result<()> {
    ...
    let report = plan(
        &compiler,
        &[quota("CHARCOAL", 2), quota("BAR_IRON", 1)],
        &stock,
    )?;
    assert!(report.model_feasible());
    let balance = report
        .requirements()
        .iter()
        .find(|r| r.item_token == "CHARCOAL");
    assert!(balance.is_some_and(|r| r.minimum_stock == 2
        && r.consumed_units == 1
        && r.stock_units == 1
        && r.planned_units == 2
        && r.surplus_units == 0));
...
fn diamond_consumers_share_one_rounded_supplier_batch() -> Result<()> {
    ...
    assert!(
        report
            .requirements()
            .iter()
            .any(|r| r.item_token == "C" && r.surplus_units == 1)
    );
...
fn complete_shortages_include_raw_goals_and_all_missing_leaves() -> Result<()> {
    ...
    let report = plan(
        &compiler,
        &[quota("A", 2), quota("WATER", 1)],
        &InventoryStockpile::new(),
    )?;
    let deficits: Vec<_> = report
        .shortages()
        .iter()
        .map(|r| (r.item_token.as_str(), r.missing_units))
        .collect();
...
fn quota_order_and_duplicate_minima_do_not_change_the_normal_form() -> Result<()> {
    ...
    let duplicate = plan(
        &compiler,
        &[quota("BIN", 1), quota("BARREL", 3), quota("BIN", 2)],
        &stock,
    )?;
...
fn unused_invalid_or_cyclic_catalog_is_not_an_active_dependency() -> Result<()> {
    ...
    stock.set_stock("B", 0);
    assert!(
        plan(&compiler, &[quota("A", 1)], &stock).is_err_and(|e| e.code == ErrorCode::Conflict)
    );
    stock.set_stock("A", 1);
    assert!(
        plan(&compiler, &[quota("A", 1)], &stock)?
            .steps()
            .is_empty()
    );
    Ok(())
...
fn self_cycle_is_refused_without_recursing() {
    ...
    assert!(
        plan(&compiler, &[quota("A", 1)], &InventoryStockpile::new())
...
fn deep_chain_is_iterative_and_order_limit_is_enforced() -> Result<()> {
    let mut compiler = ProductionLogisticsCompiler::without_recipes();
    for n in 0..250 {
        compiler.register_recipe(recipe(
            &format!("N{n:03}"),
            1,
            &[(&format!("N{:03}", n + 1), 1)],
        ));
    ...
    assert!(
        plan(&compiler, &[quota("N000", 1)], &stock)
...
fn work_resource_edge_and_order_budgets_fail_without_partial_plans() {
    ...
    for limits in [
        ...
        assert!(
            compiler
                .plan_quotas(&[quota("A", 1)], &InventoryStockpile::new(), limits)
                .is_err_and(|e| e.code == ErrorCode::BudgetExceeded)
        );
    }
    assert!(
...
fn reachable_invalid_recipes_and_shape_limits_are_rejected() {
    ...
    for quotas in [
        ...
        vec![quota("X", 1); 65],
        vec![quota("", 1)],
        vec![quota(&"x".repeat(257), 1)],
        vec![quota("x\0y", 1)],
        vec![quota(&"é".repeat(129), 1)],
    ] {
        assert!(
            plan(&compiler, &quotas, &stock).is_err_and(|e| e.code == ErrorCode::InvalidRequest)
        );
    }
}
...
fn checked_arithmetic_refuses_requirement_input_and_output_overflow() {
    ...
    assert!(
        plan(&compiler, &[quota("A", u32::MAX)], &stock)
            .is_err_and(|e| e.code == ErrorCode::BudgetExceeded)
    );
    compiler.register_recipe(recipe("A", 1, &[("X", u32::MAX)]));
    assert!(
        plan(&compiler, &[quota("A", 2)], &stock)
            .is_err_and(|e| e.code == ErrorCode::BudgetExceeded)
    );
    compiler.register_recipe(recipe("A", 1, &[("X", 1)]));
    assert!(
        plan(&compiler, &[quota("A", 1), quota("X", u32::MAX)], &stock)
            .is_err_and(|e| e.code == ErrorCode::BudgetExceeded)
    );
        ...
        for yield_mask in 0..8u32 {
            for stock_mask in 0..8u32 {
                ...
                let actual: BTreeMap<_, _> = report
                    .steps()
                    .iter()
                    .map(|step| (step.output_token.as_str(), step.batches))
                    .collect();
                let actual = names[..3]
                    .iter()
                    .map(|name| actual.get(name).copied().unwrap_or_default())
                    .collect::<Vec<_>>();
                    ...
                    for b in 0..=3u32 {
                        for c in 0..=6u32 {
                                ...
                                && initial[3] >= demand[3]
                            {
                                ...
                                if best.as_ref().is_none_or(|prior| &candidate < prior) {
                                    best = Some(candidate);
                                }
                            }
                        }
                ...
                assert_eq!(
                    best.map(|(_, batches)| batches),
                    Some(actual),
                    "mask={mask}, yields={yield_mask}, stock={stock_mask}"
                );
                for balance in report.requirements() {
                    assert_eq!(
                        u64::from(balance.stock_units) + u64::from(balance.planned_units),
                        u64::from(balance.minimum_stock)
                            + u64::from(balance.consumed_units)
                            + balance.surplus_units
                    );
