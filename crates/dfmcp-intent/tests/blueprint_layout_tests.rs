    ...
    for width in 3_i32..=16 {
        for height in 3_i32..=16 {
            for gap in 0..=(width - 2) as u8 {
                let low = origin();
                let high = MapCoord { x: low.x + width - 1, y: low.y + height - 1, z: low.z };
                let layout = BlueprintPlanner.layout(low, BlueprintTemplate::DefensiveMoat {
                    perimeter_cuboid: MapCuboid::new(low, high)?, drawbridge_span: gap,
                })?;
                let actual = tiles(&layout);
                let start = low.x + (width - i32::from(gap)) / 2;
                let expected: BTreeSet<_> = (low.y..=high.y).flat_map(|y| {
                    (low.x..=high.x).filter_map(move |x| {
                        let boundary = x == low.x || x == high.x || y == low.y || y == high.y;
                        let crossing = gap > 0 && y == low.y && x >= start && x < start + i32::from(gap);
                        (boundary && !crossing).then_some([x, y, low.z])
                    })
                }).collect();
                assert_eq!(actual, expected);
                assert_eq!(layout.excavations().len(), if gap == 0 { 4 } else { 5 });
                assert!(layout.excavations().iter().all(|p| p.mode == DigMode::Channel && p.role == ExcavationRole::Perimeter));
...
fn assert_connected(layout: &BlueprintLayout) {
    ...
    assert_eq!(seen, expected, "an excavated room cannot reach the entrance");
}
...
fn all_bedroom_counts_have_disjoint_connected_rooms_doors_and_corridors() -> Result<()> {
    for count in 1..=24 {
        for width in 1..=7 {
            for height in 1..=7 {
                let template = BlueprintTemplate::BedroomCluster { rooms_count: count, room_size: (width, height) };
                ...
                assert_eq!(layout.excavations().iter().filter(|p| p.role == ExcavationRole::Room).count(), count as usize);
                assert_eq!(layout.excavations().iter().filter(|p| p.role == ExcavationRole::Doorway).count(), count as usize);
                ...
                for part in layout.excavations().iter().filter(|p| p.role == ExcavationRole::Room) {
                    let doorway_x = part.area.min.x + (i32::from(width) - 1) / 2;
                    for x in part.area.min.x..=part.area.max.x {
                        assert_eq!(set.contains(&[x, part.area.max.y + 1, part.area.min.z]), x == doorway_x);
                    }
                }
            }
        }
...
fn workshop_bays_share_a_corridor_without_erasing_separating_walls() -> Result<()> {
    for count in 1..=24 {
        let layout = BlueprintPlanner.layout(origin(), BlueprintTemplate::WorkshopHub { bays_count: count })?;
        assert_connected(&layout);
        let set = tiles(&layout);
...
fn invalid_and_extreme_geometry_is_refused_without_expansion() -> Result<()> {
    for count in [0, 25, u32::MAX] {
        assert_eq!(BlueprintPlanner.layout(origin(), BlueprintTemplate::WorkshopHub { bays_count: count }).expect_err("unbounded bays accepted").code, ErrorCode::BudgetExceeded);
        assert!(BlueprintPlanner.layout(origin(), BlueprintTemplate::BedroomCluster { rooms_count: count, room_size: (3, 3) }).is_err());
    }
    for span in [(2, 3, 0), (3, 2, 0), (5, 5, 4)] {
        assert!(BlueprintPlanner.layout(origin(), BlueprintTemplate::DefensiveMoat {
            perimeter_cuboid: MapCuboid::new(origin(), MapCoord { x: origin().x + span.0 - 1, y: origin().y + span.1 - 1, z: origin().z })?,
            drawbridge_span: span.2,
        }).is_err());
    }
    assert!(BlueprintPlanner.layout(origin(), BlueprintTemplate::DefensiveMoat {
        perimeter_cuboid: MapCuboid::new(origin(), MapCoord { x: 10, y: 10, z: 11 })?, drawbridge_span: 0,
    }).is_err());
    assert!(BlueprintPlanner.layout(origin(), BlueprintTemplate::DefensiveMoat {
        perimeter_cuboid: MapCuboid::new(MapCoord { x: i32::MIN, y: 0, z: 0 }, MapCoord { x: i32::MAX, y: 2, z: 0 })?, drawbridge_span: 0,
    }).is_err());
    assert!(BlueprintPlanner.layout(MapCoord { x: i32::MIN, ..origin() }, BlueprintTemplate::BedroomCluster { rooms_count: 1, room_size: (3, 3) }).is_err());
    assert!(BlueprintPlanner.layout(MapCoord { x: i32::MAX, ..origin() }, BlueprintTemplate::DiningHall { width: 2, height: 2 }).is_err());
    assert!(BlueprintPlanner.layout(origin(), BlueprintTemplate::DiningHall { width: 255, height: 255 }).is_err());
    assert!(BlueprintPlanner.layout(origin(), BlueprintTemplate::StockpileVault { width: 1, height: 1, category: "x".repeat(129) }).is_err());
    Ok(())
}
