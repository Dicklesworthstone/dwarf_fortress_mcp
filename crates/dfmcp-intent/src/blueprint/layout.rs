...
impl BlueprintLayout {
        ...
        match template {
            ...
            BlueprintTemplate::StockpileVault {
                ...
                if category.trim().is_empty() || category.len() > 128 || category.contains('\0') {
                    return Err(invalid(
                        "stockpile category must contain 1..128 UTF-8 bytes without NUL",
                    ));
                }
                layout.push(
                    rectangle(origin, width, height)?,
    ...
    fn push(&mut self, area: MapCuboid, mode: DigMode, role: ExcavationRole) -> Result<()> {
        let count = area
            .tile_count()
            .ok_or_else(|| bounds("blueprint tile count overflow"))?;
        let total = self
            .tile_count
            .checked_add(count)
            .ok_or_else(|| bounds("blueprint tile count overflow"))?;
        if self.excavations.len() >= MAX_BLUEPRINT_ACTIONS || total > MAX_BLUEPRINT_TILES {
            return Err(bounds(
                "blueprint exceeds 64 excavation parts or 16384 tiles",
            ));
        }
        if self
            .excavations
            .iter()
            .any(|part| overlaps(part.area, area))
        {
            return Err(DfmcpError::new(
                ErrorCode::InternalInvariantViolation,
                "blueprint parts overlap",
            ));
        }
        self.tile_count = total;
        self.excavations
            .push(BlueprintExcavation { area, mode, role });
        Ok(())
    }
    ...
    fn cluster(
        ...
        for row in 0..rows {
            ...
            for column in 0..in_row {
                ...
                self.push(
                    rectangle(room, size.0, size.1)?,
                    DigMode::Mine,
                    ExcavationRole::Room,
                )?;
                let doorway = offset(room, (width - 1) / 2, height)?;
                self.push(
                    MapCuboid::new(doorway, doorway)?,
                    DigMode::Mine,
                    ExcavationRole::Doorway,
                )?;
            }
            let left = offset(origin, -1, row_y + height + 1)?;
            let right = offset(
                origin,
                i64::from(in_row) * (width + 1) - 2,
                row_y + height + 1,
            )?;
            self.push(
                MapCuboid::new(left, right)?,
                DigMode::Mine,
                ExcavationRole::Corridor,
            )?;
        }
        let entrance = offset(origin, -2, height + 1)?;
        let end = offset(origin, -2, i64::from(rows - 1) * (height + 3) + height + 1)?;
        self.push(
            MapCuboid::new(entrance, end)?,
            DigMode::Mine,
            ExcavationRole::Corridor,
        )?;
    ...
    fn moat(&mut self, area: MapCuboid, gap: u8) -> Result<()> {
        ...
        if width < 3 || height < 3 || u64::from(gap) > width - 2 {
            return Err(invalid(
                "moat must enclose an interior and its crossing must preserve both north corners",
            ));
        }
        ...
        let north_end = MapCoord {
            x: area.max.x,
            ..area.min
        };
        if gap == 0 {
            self.push(
                MapCuboid::new(area.min, north_end)?,
                DigMode::Channel,
                ExcavationRole::Perimeter,
            )?;
        } else {
            ...
            self.push(
                MapCuboid::new(area.min, offset(start, -1, 0)?)?,
                DigMode::Channel,
                ExcavationRole::Perimeter,
            )?;
            self.push(
                MapCuboid::new(offset(end, 1, 0)?, north_end)?,
                DigMode::Channel,
                ExcavationRole::Perimeter,
            )?;
        }
...
fn offset(origin: MapCoord, dx: i64, dy: i64) -> Result<MapCoord> {
    checked_coord_offset(origin, dx, dy, 0)
        .ok_or_else(|| invalid("blueprint coordinate arithmetic overflow"))
}
