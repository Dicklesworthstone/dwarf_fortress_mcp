...
fn failure(code: ErrorCode, message: &str) -> DfmcpError {
...
fn exhausted(message: &str) -> DfmcpError {
    failure(ErrorCode::BudgetExceeded, message)
}
fn invariant(message: &str) -> DfmcpError {
    failure(ErrorCode::InternalInvariantViolation, message)
}

fn lock(history: &Mutex<History>) -> Result<MutexGuard<'_, History>> {
    history
        .lock()
        .map_err(|_| invariant("query baseline store is poisoned"))
}
...
fn check_input(root: &Value) -> Result<()> {
    ...
    while let Some((value, depth)) = pending.pop() {
        ...
        if bytes > 65_536 {
            return Err(exhausted("history query exceeds its input-byte bound"));
        }
    }
    Ok(())
}
...
fn validate_selection(selection: &Value) -> Result<()> {
    let object = selection
        .as_object()
        .ok_or_else(|| invalid("capture select must be an entities query object"))?;
    if object.get("kind").and_then(Value::as_str) != Some("entities")
        || object.keys().any(|key| {
            !matches!(
                key.as_str(),
                "kind" | "kinds" | "where" | "fields" | "order"
            )
        })
    {
        return Err(invalid(
            "capture select supports entities, kinds, where, fields, order only; pagination is owned by the capture",
        ));
    }
    Ok(())
}
...
fn row_key(row: &Value) -> Result<RowKey> {
    ...
    let id = raw
        .parse::<u64>()
        .map_err(|_| invariant("captured row ID is not u64"))?;
    let generation = row
        .get("generation")
        .and_then(Value::as_u64)
        .and_then(|value| u32::try_from(value).ok())
        .ok_or_else(|| invariant("captured row has no generation"))?;
    if id == 0 || raw != id.to_string() {
        return Err(invariant("captured row ID is not canonical"));
    }
    Ok((id, generation))
}
    ...
    loop {
        ...
        if started.elapsed().as_millis() >= u128::from(context.budget.max_wall_millis) {
            return Err(exhausted(
                "query selection exhausted its foreground wall-time allowance",
            ));
        }
        calls += 1;
        ...
        {
            return Err(exhausted(
                "query baseline acquisition exceeded its foreground page/scan budget; narrow the selection",
            ));
        }
        ...
        if result.get("anchor") != Some(&anchor_json(snapshot.anchor())) {
            return Err(invariant(
                "capture page changed the requested snapshot anchor",
            ));
        }
        let matched = result
            .get("matched")
        ...
        {
            return Err(exhausted(
                "baseline selection exceeds 256 rows or changed during acquisition",
            ));
        }
        ...
        for row in page {
            bytes = bytes
                .checked_add(encode(row)?.len())
                .ok_or_else(|| exhausted("capture size overflow"))?;
            if bytes > MAX_CAPTURE_BYTES {
                return Err(exhausted("query baseline exceeds its retained-byte bound"));
            }
            if rows.insert(row_key(row)?, row.clone()).is_some() {
                return Err(invariant(
                    "query baseline acquisition repeated an entity generation",
                ));
            }
        }
        match result.get("continuation").and_then(Value::as_str) {
            Some(token) => {
                if page.is_empty() || result.get("truncated") != Some(&json!(true)) {
                    return Err(invariant(
                        "capture pagination did not make explicit progress",
                    ));
                }
...
fn rows_digest(rows: &Rows) -> Result<Digest32> {
    Ok(Digest32::of_bytes(&encode(
        &json!({"domain":"dfmcp-query-baseline-rows/1",
...
fn compare_rows(before: &Rows, after: &Rows) -> (Vec<Value>, usize) {
    ...
    for (id, generation) in keys {
        ...
        changes.push(
            json!({"kind":kind, "entity_id":id.to_string(), "generation":generation,
...
fn check_baseline(baseline: &Baseline, current: StateAnchor) -> Result<()> {
    if baseline.anchor.fortress_id != current.fortress_id
        || baseline.anchor.cursor.epoch != current.cursor.epoch
    {
        return Err(failure(
            ErrorCode::StaleAnchor,
            "baseline belongs to another fortress or observation epoch; capture a new baseline",
        ));
    }
    if current.cursor.sequence < baseline.anchor.cursor.sequence
        || current.tick < baseline.anchor.tick
        || (current.cursor == baseline.anchor.cursor && current != baseline.anchor)
    {
        return Err(failure(
            ErrorCode::StaleAnchor,
            "baseline cannot be compared with a regressed or forked anchor",
        ));
    }
    if current.tick.0 >= baseline.expires_at {
        return Err(failure(
            ErrorCode::StaleAnchor,
            "query baseline reached its game-time deadline; capture a new baseline",
        ));
    }
    Ok(())
}
...
fn offset(token: Option<&str>, identity: Digest32, total: usize) -> Result<usize> {
    ...
    if token.len() > 128 {
        return Err(invalid("changes continuation exceeds its byte bound"));
    }
    let mut parts = token.split(':');
    let (Some("qh1"), Some(raw), Some(_), None) =
    ...
    let value = raw
        .parse::<usize>()
        .map_err(|_| invalid("changes continuation offset is invalid"))?;
    if value == 0 || value >= total || raw != value.to_string() {
        return Err(failure(
            ErrorCode::CursorGap,
            "changes continuation makes no progress or exceeds the result horizon",
        ));
    }
    if token != cursor(value, identity)? {
        return Err(failure(
            ErrorCode::StaleAnchor,
            "changes continuation belongs to a different baseline or target snapshot",
        ));
    }
    Ok(value)
}
...
fn change_page(
    ...
    if limit == 0 || limit > hard_limit {
        return Err(invalid("changes limit exceeds the negotiated row limit"));
    }
    ...
    payload["note"] = json!(
        "left_result means no longer selected in this projection, not deleted or dead; intermediate changes are unknown"
    );
    ...
    for change in changes.iter().skip(start).take(limit as usize) {
        ...
        let Some(array) = candidate["changes"].as_array_mut() else {
            return Err(invariant("change page lost its row array"));
        };
    ...
    if start < changes.len() && end == start {
        return Err(exhausted(
            "one complete change cannot fit the result budget; increase the response budget or capture fewer fields",
        ));
    }
    if encode(&payload)?.len() > maximum {
        return Err(exhausted(
            "change-page metadata exceeds the response budget",
        ));
    }
    Ok(payload)
}
...
fn validate_handle(handle: &str) -> Result<()> {
    if handle.len() != 68
        || !handle.starts_with("qb1:")
        || !handle[4..].bytes().all(|byte| byte.is_ascii_hexdigit())
    {
        return Err(invalid(
            "baseline must be a qb1 handle returned by capture or baselines",
        ));
    }
    Ok(())
}
    ...
    F: FnOnce(Value) -> Result<String>,
{
    ...
    if context.anchor != snapshot.anchor() {
        return Err(failure(
            ErrorCode::StaleAnchor,
            "baseline query context has a different anchor",
        ));
    }
    check_input(input)?;
    ...
    if envelope.schema != "dfmcp.query/1" {
        return Err(invalid("baseline query schema must be dfmcp.query/1"));
    }
    if envelope
        .expected_anchor
        .as_ref()
        .is_some_and(|anchor| anchor != &anchor_json(context.anchor))
    {
        return Err(failure(
            ErrorCode::StaleAnchor,
    ...
    if !snapshot.hash_is_valid() {
        return Err(invariant("baseline query snapshot hash is invalid"));
    }
    match envelope.query {
        Request::Capture {
            ...
            {
                return Err(invalid(
                    "capture key must be 1..64 ASCII letters, digits, dots, underscores, or hyphens",
                ));
            }
            if max_game_ticks == 0 || max_game_ticks > context.budget.max_game_ticks {
                return Err(exhausted(
                    "capture lifetime must be positive and within the session game-tick budget",
                ));
            }
            ...
            if let Some((_, existing)) = store
                .entries
                .iter()
                .find(|((session, _), entry)| *session == context.session_id && entry.key == key)
            {
                if existing.anchor != context.anchor
                    || existing.selection != select
                    || existing.max_game_ticks != max_game_ticks
                {
                    return Err(failure(
                        ErrorCode::Conflict,
                        "capture key already names another baseline; list, use, or release it first",
                    ));
                }
            ...
            let session_count = store
                .entries
                .keys()
                .filter(|(session, _)| *session == context.session_id)
                .count();
            if store.entries.len() >= MAX_BASELINES || session_count >= MAX_PER_SESSION {
                return Err(exhausted(
                    "query baseline capacity reached; explicitly release a retained baseline",
                ));
            }
            let sequence = store
                .next_id
                .checked_add(1)
                .ok_or_else(|| exhausted("query baseline ID space exhausted"))?;
            let id = format!(
                "qb1:{}",
                Digest32::of_bytes(&encode(&json!({"domain":"dfmcp-query-baseline/1",
        ...
        Request::Changes {
            ...
            let baseline = lock(history)?
                .entries
                .get(&(context.session_id, baseline))
                .cloned()
                .ok_or_else(|| {
                    invalid(
                        "baseline is not retained by this session; list baselines or capture one",
                    )
                })?;
            publish(change_page(
                &baseline,
        ...
        Request::Baselines => {
            let baselines = lock(history)?
                ...
                .map(|(_, baseline)| describe(baseline, context.anchor))
                .collect::<Vec<_>>();
        ...
        Request::ReleaseBaseline { baseline } => {
            ...
            payload["released"] = json!(
                store
                    .entries
                    .contains_key(&(context.session_id, baseline.clone()))
            );
            let encoded = publish(payload)?;
