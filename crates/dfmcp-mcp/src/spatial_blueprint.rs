    ...
    fn into_template(self) -> Result<BlueprintTemplate> {
        Ok(match self {
            ...
            Self::DefensiveMoat {
                ...
                perimeter_cuboid: MapCuboid::new(coord(min)?, coord(max)?)?,
                drawbridge_span,
            },
        })
    }
...
fn area_json(a: MapCuboid) -> Value {
    json!({"min":position(a.min),"max":position(a.max)})
}
...
impl Coverage {
    fn observe(&mut self, map: &LiveMapObservation, p: [i32; 3]) -> Result<()> {
        ...
        match map
            ...
            .cells
            .get(index)
            .ok_or_else(|| invalid("blueprint capture cell absent"))?
        {
...
fn halo(a: MapCuboid) -> Result<MapCuboid> {
    let min = MapCoord {
        x: a.min
            .x
            .checked_sub(1)
            .ok_or_else(|| budget("blueprint halo overflow"))?,
        y: a.min
            .y
            .checked_sub(1)
            .ok_or_else(|| budget("blueprint halo overflow"))?,
...
fn analyze(
    ...
    for part in layout.excavations() {
        ...
        let count = expanded
            .tile_count()
            .ok_or_else(|| budget("blueprint halo size overflow"))?;
        cost = count
            .checked_mul(2)
            .and_then(|n| cost.checked_add(n))
    ...
    if let Some(crossing) = layout.reserved_crossing() {
        cost = cost
            .checked_add(
                crossing
                    .tile_count()
                    .ok_or_else(|| budget("crossing size overflow"))?,
            )
            .ok_or_else(|| budget("blueprint scan cost overflow"))?;
    }
    if cost > maximum_work {
        return Err(budget("blueprint terrain analysis exceeds max_work"));
    }
    map.validate()?;
    ...
    for (index, part) in layout.excavations().iter().enumerate() {
        ...
        visit(part.area, |p| {
            coverage.observe(map, p)?;
            footprint.observe(map, p)
        })?;
        ...
        let role = match part.role {
            ExcavationRole::Room => "room",
            ExcavationRole::Doorway => "doorway",
            ExcavationRole::Corridor => "corridor",
            ExcavationRole::Perimeter => "perimeter",
        };
...
pub(super) fn execute<T: SpatialStateView>(
    ...
    let monitoring = request
        .monitor
        .map(|options| monitor::proposal(&layout, map.map_dimensions, c, options))
        .transpose()?;
    ...
    if let Some(value) = &monitoring {
        basis["monitoring"] = value.clone();
    }
    ...
    out["unknown_safety_domains"] = json!([
        "aquifers",
        "water_pressure",
        "structural_support",
        "protected_areas",
        "native_designation_eligibility",
        "unit_access"
    ]);
    out["interpretation"] = json!(
        "Disjoint proposed geometry and visible attributes from this capture, not excavation permission, a safe site, a built room/bridge, or proof of completed work. Hidden and uncaptured positions remain unknown; a corridor endpoint is not an existing-fort connection."
    );
    paginate(
        out,
        rows.len(),
