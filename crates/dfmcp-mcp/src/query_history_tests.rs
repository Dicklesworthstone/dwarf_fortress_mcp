...
fn fixture(count: u64) -> (WorldSnapshot, OperationContext) {
    ...
    let context = OperationContext {session_id:SessionId::new(u128::from(NEXT_SESSION.fetch_add(2, Ordering::Relaxed))), request_id:RequestId::new(1),
...
fn retained(world: &WorldSnapshot, context: &OperationContext) -> Result<usize> {
    ...
    result["baselines"].as_array().map(Vec::len).ok_or_else(|| invariant("test baselines"))
}
...
fn capture_finishes_multiple_pages_and_is_idempotent_at_the_same_anchor() -> Result<()> {
    ...
    let page = run(&world, &ctx, json!({"kind":"changes", "baseline":handle(&first)?}))?;
...
fn pages_and_retries_never_consume_the_baseline() -> Result<()> {
    ...
    assert_eq!(retained(&world, &ctx)?, 1);
    let old_page = run(&world, &ctx, json!({"kind":"changes", "baseline":id, "limit":1}))?;
    advance(&mut world, &mut ctx);
    assert!(matches!(run(&world, &ctx, json!({"kind":"changes", "baseline":id,
...
fn session_restore_deadline_and_authority_boundaries_are_enforced() -> Result<()> {
    ...
    let mut other = ctx.clone(); other.session_id = SessionId::new(ctx.session_id.get() + 1);
    assert_eq!(run(&world, &other, json!({"kind":"baselines"}))?["baselines"], json!([]));
    assert_eq!(run(&world, &other, json!({"kind":"release_baseline", "baseline":id}))?["released"], false);
    assert_eq!(retained(&world, &ctx)?, 1);
    Ok(())
}
...
fn failed_publication_does_not_create_or_release_state() -> Result<()> {
    ...
    let result = execute_with_publisher(&world, &ctx, &input, |_| Err(exhausted("packet cannot fit")));
    assert!(result.is_err());
    assert_eq!(retained(&world, &ctx)?, 0);
    let id = handle(&run(&world, &ctx, capture("citizens"))?)?;
    let input = json!({"schema":"dfmcp.query/1", "query":{"kind":"release_baseline", "baseline":id}});
    assert!(execute_with_publisher(&world, &ctx, &input, |_| Err(exhausted("packet cannot fit"))).is_err());
    assert_eq!(retained(&world, &ctx)?, 1);
    Ok(())
}
...
fn capacity_explicit_release_and_fresh_handles_are_bounded() -> Result<()> {
    ...
    assert!(matches!(run(&world, &ctx, capture("overflow")), Err(error) if error.code == ErrorCode::BudgetExceeded));
    run(&world, &ctx, json!({"kind":"release_baseline", "baseline":first}))?;
...
fn partial_capture_and_forged_controls_never_become_a_baseline() -> Result<()> {
    ...
    assert!(run(&world, &ctx, json!({"kind":"changes", "baseline":id,
...
fn baseline_changes_survive_the_complete_agent_turn_budget() -> Result<()> {
    ...
    let mut view = QueryResponseProjection {
        session_id:ctx.session_id.to_string(), request_id:ctx.request_id.to_string(),
        ...
        attention:vec![json!({"finding":"preserve this warning"})], affordances:vec![],
        ...
        budget:json!({"admitted":{"max_bytes":8192}}), references:vec![], maximum_bytes:8192,
    };
    ...
    view.anchor = json!({"fortress_id":"1", "epoch":0, "sequence":1, "game_tick":2,
        "state_hash":world.state_hash.to_string()});
    ctx.budget.max_bytes = view.result_byte_budget()? as u64;
    let mut request = json!({"schema":"dfmcp.query/1", "query":{"kind":"changes", "baseline":id, "limit":128}});
    let (mut seen, mut pages) = (Vec::new(), 0usize);
    loop {
        let encoded = execute_with_publisher(&world, &ctx, &request, |payload| view.finish(payload))?;
        assert!(encoded.len() <= 8192);
        ...
        assert_eq!(page["agent_turn"]["continuity"]["basis"], captured["captured"]["basis"]);
        assert_eq!(page["agent_turn"]["briefing"]["runtime_admitted"], false);
        assert_eq!(page["agent_turn"]["attention"][0]["finding"], "preserve this warning");
        assert_eq!(page["agent_turn"]["uncertainty"][0]["domain"], "history");
        assert_eq!(page["agent_turn"]["changes"][0]["change_count"], 25);
        let rows = page["changes"].as_array().ok_or_else(|| invariant("test changes"))?;
    ...
    assert!(pages > 1);
    assert_eq!(seen, (1..=25).map(|id| json!(id.to_string())).collect::<Vec<_>>());
    let input = json!({"schema":"dfmcp.query/1", "query":{"kind":"release_baseline", "baseline":id}});
