...
pub(crate) fn release_session_resources<F>(
    session: dfmcp_core::SessionId,
...
pub(crate) fn register_watch_set<F>(
...
pub(crate) fn prepare_watch_batch<F>(
    snapshot: &WorldSnapshot,
    context: &OperationContext,
    input: &Value,
    preview: F,
) -> Result<PreparedWatchBatch>
where
    F: FnOnce(Value) -> Result<String>,
{
    query_watch::batch::prepare(snapshot, context, input, preview)
}
pub(crate) fn complete_watch_batch<F>(
    snapshot: &WorldSnapshot,
    context: &OperationContext,
    prepared: PreparedWatchBatch,
...
pub(crate) fn attach_watch_journal<F>(
    snapshot: &WorldSnapshot,
    context: &OperationContext,
    path: &std::path::Path,
    archive: Digest32,
...
pub(super) fn compare_endpoints(
    ...
    query_history::compare_endpoints(
        before,
        after,
        context,
        selection,
...
pub fn prepare_await(
    ...
    let invalid = || {
        DfmcpError::new(
            ...
            "invalid await_watch envelope or handle",
        )
    };
    ...
    if object.len() > 3
        || object
            .keys()
            .any(|key| !matches!(key.as_str(), "schema" | "expected_anchor" | "query"))
        || input["schema"] != "dfmcp.query/1"
    {
        return Err(invalid());
    }
    let request = input["query"].as_object().ok_or_else(invalid)?;
    if request.len() != 2 || input["query"]["kind"] != "await_watch" {
        return Err(invalid());
    }
        ...
        .is_some_and(|value| value != &core_query::anchor_json(context.anchor))
    {
        return Err(DfmcpError::new(
            ...
            "await_watch expected_anchor differs from the pre-refresh anchor",
        ));
    }
        ...
        |metadata| {
            let record = metadata["records"]
                ...
                .and_then(|records| {
                    records
                        .iter()
                        .find(|record| record["watch"].as_str() == Some(handle))
                })
                .ok_or_else(invalid)?;
...
pub fn result_context(context: &OperationContext) -> Result<OperationContext> {
    ...
    query_watch::with_active_work(context, json!({}), |metadata| {
        reserved = u64::try_from(metadata.to_string().len())
            .map_err(|_| {
                DfmcpError::new(
                    ErrorCode::BudgetExceeded,
                    "watch projection byte count cannot be represented",
                )
            })?
            .saturating_add(32);
        Ok(String::new())
    ...
    narrowed.budget.max_bytes = context
        .budget
        .max_bytes
        ...
        .ok_or_else(|| {
            DfmcpError::new(
                ErrorCode::BudgetExceeded,
                "active watches leave no room for query results",
            )
        })?;
    Ok(narrowed)
}

pub fn publish_with_active_work<F>(
    context: &OperationContext,
    value: Value,
    publish: F,
) -> Result<String>
...
pub fn execute_with_publisher<F>(
    snapshot: &WorldSnapshot,
    context: &OperationContext,
    input: &Value,
    publish: F,
    ...
    F: FnOnce(Value) -> Result<String>,
{
        ...
        .and_then(Value::as_str)
    {
        ...
        Some("capture" | "changes" | "baselines" | "release_baseline") => {
            ...
            query_history::execute(snapshot, &narrowed, input, |value| {
                publish_with_active_work(context, value, publish)
            })
        }
        _ => {
            let narrowed = result_context(context)?;
