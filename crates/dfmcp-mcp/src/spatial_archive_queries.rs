...
fn invalid(text: &str) -> DfmcpError {
    error(ErrorCode::InvalidRequest, text)
}
fn bounded(text: &str) -> DfmcpError {
    error(ErrorCode::BudgetExceeded, text)
}
...
fn validate_shape(input: &Value) -> Result<()> {
    ...
    while let Some((value, depth)) = pending.pop() {
        ...
        if nodes > 4096 || depth > 32 {
            return Err(bounded("archive query exceeds its node/depth bound"));
        }
        match value {
            ...
            Value::Array(values) => {
                if nodes
                    ...
                    > 4096
                {
                    return Err(bounded("archive query array too wide"));
                }
            ...
            Value::Object(values) => {
                if nodes
                    ...
                    > 4096
                {
                    return Err(bounded("archive query object too wide"));
                }
        ...
        if bytes > 128 * 1024 {
            return Err(bounded("archive query exceeds aggregate input bound"));
        }
    }
    Ok(())
}
fn kind(query: &Value) -> &str {
    query.get("kind").and_then(Value::as_str).unwrap_or("")
}
...
fn cursor(session: &Session, context: &OperationContext, offset: usize) -> Result<String> {
    let journal = session
        .journal
        .as_ref()
        .ok_or_else(|| invalid("archive absent"))?;
...
fn history_page(
    ...
    if !(1..=64).contains(&limit) {
        return Err(bounded("archive history limit must be 1..64"));
    }
    let start = match continuation {
        None => 0,
        Some(token) => {
            if token.len() > 128 {
                return Err(bounded("archive continuation exceeds byte bound"));
            }
            let mut parts = token.split(':');
            let (Some("ar1"), Some(raw), Some(_), None) =
                (parts.next(), parts.next(), parts.next(), parts.next())
            else {
                return Err(invalid("invalid archive continuation"));
            };
            if raw.is_empty() || raw.starts_with('0') || !raw.bytes().all(|b| b.is_ascii_digit()) {
                return Err(invalid("noncanonical archive offset"));
            }
            let offset = raw
                .parse::<usize>()
                .map_err(|_| invalid("archive offset overflow"))?;
            if token != cursor(session, context, offset)? {
                return Err(error(
                    ErrorCode::StaleAnchor,
                    "archive continuation names another session or journal head",
                ));
            }
            if offset >= entries.len() {
                return Err(error(
                    ErrorCode::CursorGap,
                    "archive continuation is past retained history",
                ));
            }
            offset
        }
    ...
    let latest = entries
        .last()
        .ok_or_else(|| error(ErrorCode::CursorGap, "empty archive"))?;
    ...
    for entry in entries
        .iter()
        .skip(start)
        .take(limit.min(context.budget.max_entities) as usize)
    {
        let mut next = out.clone();
        next["rows"]
            .as_array_mut()
            .ok_or_else(|| invalid("history rows absent"))?
            .push(entry_json(entry));
        next["returned"] = json!(end + 1 - start);
        next["truncated"] = json!(end + 1 < entries.len());
        next["continuation"] = if end + 1 < entries.len() {
            json!(cursor(session, context, end + 1)?)
        } else {
            Value::Null
        };
        if next.to_string().len() as u64 > budget {
            break;
        }
        out = next;
        end += 1;
    }
    if start < entries.len() && end == start {
        return Err(bounded("one complete archived-history row does not fit"));
    }
    ...
    if let Some(rows) = out.get_mut("rows").and_then(Value::as_array_mut) {
        for row in rows {
            if let Some(route) = row.get("route_query") {
                let query = route
                    .get("query")
                    .filter(|q| kind(q) == "map_route")
                    .ok_or_else(|| {
                        error(
                            ErrorCode::InternalInvariantViolation,
                            "route drill-down lacks a fixed map query",
                        )
                    })?;
                ...
                if wrapper.to_string().len() > route.to_string().len() {
                    return Err(error(
                        ErrorCode::InternalInvariantViolation,
                        "archived route wrapper exceeds source row reservation",
                    ));
                }
...
fn execute_on(
    ...
    if !READ_KINDS.contains(&kind(&query)) {
        return Err(error(
            ErrorCode::CapabilityDenied,
            "archive-only queries permit stateless analysis, not watch/baseline changes or live acquisition",
        ));
    }
    let snapshot = state
        .snapshot()
        .ok_or_else(|| error(ErrorCode::CorruptLedger, "archived snapshot absent"))?;
    if snapshot.anchor() != entry.anchor {
        return Err(error(
            ErrorCode::CorruptLedger,
            "archived state does not reproduce selected record",
        ));
    }
    validate_limits(
        state
            .observation_full()
            .ok_or_else(|| error(ErrorCode::CorruptLedger, "archived source absent"))?,
        session.limits,
    )?;
    let narrowed = result_context(session, context, entry)?;
    let input =
        json!({"schema":"dfmcp.query/1","expected_anchor":anchor_json(entry.anchor),"query":query});
    let mut out = if production::handles(&input) {
        production::execute(state, &narrowed, &input)?
    } else if workforce_queries::handles(&input) {
        workforce_queries::execute(state, &narrowed, &input)?
    } else if spatial_queries::handles(&input) {
        spatial_queries::execute(state, &narrowed, &input)?
    } else {
        semantic_query::execute(snapshot, &narrowed, &input)?
    };
        ...
        let resolved = match variant.get("$ref").and_then(Value::as_str) {
            Some(reference) => schema
                .pointer(
                    reference
                        .strip_prefix('#')
                        .ok_or_else(|| invalid("nonlocal query schema reference"))?,
                )
                .ok_or_else(|| invalid("missing query schema reference"))?,
            None => variant,
        };
        if resolved["properties"]["kind"]["const"]
            .as_str()
            .is_some_and(|k| READ_KINDS.contains(&k))
        {
            stateless.push(variant.clone());
        }
    }
    if stateless.len() != READ_KINDS.len() {
        return Err(error(
            ErrorCode::InternalInvariantViolation,
            "archive schema could not resolve every registered stateless query",
        ));
    }
...
pub(in super::super) fn query(
    ...
    if history::changes::handles(input) {
        return history::changes::execute(session, context, input);
    }
    validate_shape(input)?;
    let envelope: Envelope = serde_json::from_value(input.clone())
        .map_err(|_| invalid("invalid archive query envelope"))?;
    if envelope.schema != "dfmcp.query/1" {
        return Err(invalid("archive query requires dfmcp.query/1"));
    }
    if envelope
        .expected_anchor
        .as_ref()
        .is_some_and(|a| a != &anchor_json(context.anchor))
    {
        return Err(error(
            ErrorCode::StaleAnchor,
            "expected_anchor differs from this archive session's latest retained observation",
        ));
    }
    match kind(&envelope.query) {
        "history" | "historical_query" => {
            let request: ArchiveRequest = serde_json::from_value(envelope.query)
                .map_err(|_| invalid("invalid archive record selection"))?;
            match request {
                ArchiveRequest::History {
                    limit,
                    continuation,
                } => history_page(session, context, limit, continuation),
                ArchiveRequest::HistoricalQuery {
                    record,
                    record_digest,
                    query,
                } => {
                    if !READ_KINDS.contains(&kind(&query)) {
                        return Err(error(
                            ErrorCode::CapabilityDenied,
                            "historical queries cannot evaluate watches, change baselines or recursively select history",
                        ));
                    }
                    let digest = Digest32::from_hex(&record_digest)
                        .filter(|d| d.to_string() == record_digest)
                        .ok_or_else(|| {
                            invalid("record_digest must be canonical lowercase SHA-256")
                        })?;
                    ...
                    let index = record
                        ...
                        .filter(|&i| i < journal.entries().len())
                        .ok_or_else(|| {
                            error(ErrorCode::CursorGap, "selected record is not retained")
                        })?;
        ...
        _ => {
            let entry = session
                .journal
                .as_ref()
                .and_then(|j| j.entries().last())
                .cloned()
                .ok_or_else(|| {
                    error(ErrorCode::CursorGap, "archive has no retained observations")
                })?;
