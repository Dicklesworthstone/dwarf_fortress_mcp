...
fn io_error(_: std::io::Error) -> DfmcpError {
    err(ErrorCode::CorruptLedger, "cancellation fixture I/O")
}
fn context() -> OperationContext {
    OperationContext {
        session_id: SessionId::new(710),
...
fn plan(key: &str) -> String {
    Digest32::of_bytes(key.as_bytes()).to_string()
}
fn active(owned: &mut Option<ControlSession>) -> Result<&mut ControlSession> {
    owned
        .as_mut()
        .ok_or_else(|| err(ErrorCode::SessionNotFound, "fixture session closed"))
}
struct Fixture {
    directory: PathBuf,
    path: PathBuf,
...
impl Fixture {
    fn new() -> Result<Self> {
        ...
        for key in ["a-prepared", "b-prepared", "c-started", "d-applied"] {
            ...
            if key == "c-started" || key == "d-applied" {
                j.begin_commit(key, d, 7, &context())?;
            }
            if key == "d-applied" {
                j.record_reconciliation(key, d, 7, true, true, true, 11, Some(d), &context())?;
            }
        }
        Ok(f)
    }
...
fn register(f: &Fixture, read_only: bool) -> Result<Registered> {
    ...
    lock(&SESSIONS)?.insert(id, Arc::new(Mutex::new(Some(s))));
    Ok(Registered { id })
}
fn decode(raw: String) -> Result<Value> {
    serde_json::from_str(&raw).map_err(|_| err(ErrorCode::InvalidRequest, "test JSON"))
}
...
fn cancellation_is_queryable_explainable_and_never_commit_compatible() -> Result<()> {
    ...
    assert_eq!(
        listed["result"]["state_counts"]["cancelled_before_dispatch"],
        1
    );
    assert_eq!(
        success(list(&s, "nonterminal")?)["result"]["matched_effects"],
        2
    );
    let explained = success(decode(fortress_explain(
        s.handle(),
        "a-prepared".into(),
        plan("a-prepared"),
    ))?);
    assert_eq!(explained["result"]["cancelled"], true);
    assert_eq!(explained["result"]["commit_permitted"], false);
    let replanned = success(decode(fortress_plan(
        s.handle(),
        "a-prepared".into(),
        plan("a-prepared"),
        true,
    ...
    assert_eq!(
        replanned["result"]["effect"]["state"],
        "cancelled_before_dispatch"
    );
    let committed = decode(fortress_commit(
        s.handle(),
        "a-prepared".into(),
        plan("a-prepared"),
        "01".repeat(16),
    ...
    assert_eq!(fs::read(&f.path).map_err(io_error)?, bytes);
    {
        ...
        assert!(active(&mut owned)?.connection.is_none());
    }
    Ok(())
}
...
fn cancelled_keys_remain_retired_after_writable_and_offline_recovery() -> Result<()> {
    ...
    for read_only in [true, false] {
        ...
        assert_eq!(
            found["result"]["effects"][0]["idempotency_key"],
            "a-prepared"
        );
        let explained = success(decode(fortress_explain(
            s.handle(),
        ...
        if read_only {
            assert_eq!(
                request(&s, "b-prepared")?["result"]["error"]["code"],
                "capability_denied"
            );
        } else {
            success(request(&s, "a-prepared")?);
...
fn cancellation_never_erases_dispatched_or_verified_effects() -> Result<()> {
    ...
    assert_eq!(
        request(&s, "d-applied")?["result"]["error"]["code"],
        "conflict"
    );
    assert_eq!(
        success(list(&s, "reconciliation_required")?)["result"]["matched_effects"],
        1
    );
...
fn cancellation_budget_authority_and_identity_refusals_leave_the_file_unchanged() -> Result<()> {
    ...
    for bytes in [0, 1, 32, 128, 65537] {
        let v = decode(fortress_cancel(
            s.handle(),
            Some("a-prepared".into()),
            Some(plan("a-prepared")),
            Some(bytes),
            None,
            None,
        ))?;
    ...
    assert_eq!(
        decode(fortress_cancel(s.handle(), None, None, None, None, None))?["result"]["error"]["code"],
        "capability_denied"
    );
    assert_eq!(
        decode(fortress_cancel(
    ...
    active(&mut guard)?.grants.clear();
    drop(guard);
    assert_eq!(
        request(&s, "a-prepared")?["result"]["error"]["code"],
        "capability_denied"
    );
    assert_eq!(fs::read(&f.path).map_err(io_error)?, before);
    Ok(())
...
fn injected_clock_authority_cannot_write_an_offline_recovery_journal() -> Result<()> {
    ...
    let before = fs::read(&f.path).map_err(io_error)?;
    {
        let h = resolve(s.handle())?;
        let mut owned = lock(&h)?;
        active(&mut owned)?.grants = context().grants;
    }
    assert_eq!(
        request(&s, "a-prepared")?["result"]["error"]["code"],
        "capability_denied"
...
fn new_cancellation_invalidates_history_pages_without_rewinding_later_heads() -> Result<()> {
    ...
    let token = before["result"]["continuation"]
        .as_str()
        .ok_or_else(|| err(ErrorCode::InvalidRequest, "token"))?
        .to_owned();
    success(request(&s, "a-prepared")?);
    let later = success(request(&s, "b-prepared")?);
    let replay = success(request(&s, "a-prepared")?);
    assert_eq!(
        replay["result"]["durable_effect_journal"],
        later["result"]["durable_effect_journal"]
    );
    assert_ne!(
        replay["result"]["effect"]["record_digest"],
        replay["result"]["durable_effect_journal"]["head"]
    );
    assert_eq!(
        decode(fortress_query(
            s.handle(),
...
fn cancellation_and_commit_start_races_have_only_one_durable_winner() -> Result<()> {
    ...
    for _ in 0..8 {
        ...
        let dispatched = {
            let h = resolve(s.handle())?;
            let mut owned = lock(&h)?;
            let session = active(&mut owned)?;
            let c = session.context()?;
            session
                .journal
                .begin_commit("a-prepared", Digest32::of_bytes(b"a-prepared"), 7, &c)
        };
        let cancelled = decode(cancel_thread.join().map_err(|_| {
            err(
                ErrorCode::InternalInvariantViolation,
                "cancel thread panicked",
            )
        })?)?;
        let h = resolve(s.handle())?;
        let mut guard = lock(&h)?;
        let state = active(&mut guard)?
            .journal
            .lookup("a-prepared")
            .ok_or_else(|| err(ErrorCode::InvalidRequest, "missing key"))?
            .state;
        match state {
            DurablePauseState::CancelledBeforeDispatch => {
                assert!(matches!(dispatched,Err(e)if e.code==ErrorCode::Conflict));
                success(cancelled);
            }
            DurablePauseState::CommitStarted => {
                assert!(dispatched.is_ok());
                assert_eq!(cancelled["result"]["error"]["code"], "effect_indeterminate");
            }
            _ => {
                return Err(err(
                    ErrorCode::InternalInvariantViolation,
                    "race produced a third outcome",
                ));
            }
        }
    }
...
fn recovery_batches_skip_cancelled_effects_without_native_reads() -> Result<()> {
    ...
    let session = active(&mut owned)?;
    let c = session.context()?;
    let head = session.journal.head();
    let mut source = NoQueries(0);
    let batch = reconcile_batch(
        &mut session.journal,
        &mut source,
        &["a-prepared".into()],
        &c,
    ...
    assert_eq!(batch.head_after, head);
    assert!(batch.items[0].record.state.terminal());
    assert!(!batch.items[0].queried);
    assert!(!batch.items[0].record.effect_known);
