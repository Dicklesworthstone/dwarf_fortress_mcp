...
impl ControlSession{
    ...
    fn writable(&self)->Result<()> {
        if self.journal.read_only() {
            return Err(err(ErrorCode::CapabilityDenied,"recovery-only sessions cannot change coordination or contact the bridge"));
        }
        Ok(())
    }
}
...
#[tool(description="Prepare one pause/resume effect. Requires stable idempotency_key, plan_digest, desired paused state and expected game tick. Prepare does not mutate; its receipt is synced before success. A cancelled key remains retired and is never reprepared. Recovery-only sessions refuse this tool.")]
...
pub fn fortress_cancel(session_id:Option<String>,idempotency_key:Option<String>,plan_digest:Option<String>,
    ...
    match (idempotency_key,plan_digest) {
        (Some(key),Some(plan))=>with_session(session_id,"fortress.cancel",|session,context|{
            session.writable()?;
            cancellation::cancel(&mut session.journal,&context,&key,digest(&plan)?,max_bytes,max_output_tokens)
        }),
        (None,None) if max_bytes.is_none()&&max_output_tokens.is_none()=>denied(session_id,"fortress.cancel"),
        _=>failure("fortress.cancel",&err(ErrorCode::InvalidRequest,
            "pause cancellation requires both idempotency_key and plan_digest; no bulk or game cancellation is inferred"),true),
    }
}
