...
use dfmcp_adapter::workforce_analysis::{quality as quality_adapter, WorkforcePlan};
...
pub(super) fn prepare(state: &LiveSpatialCitizenState, context: &OperationContext,
    demands: Vec<DemandInput>, objective: Option<Objective>, maximum: u64) -> Result<(WorkforcePlan, Option<Details>)> {
    let objective = objective.unwrap_or_default();
    if objective == Objective::MaxFilledSlots && demands.iter().any(|d| d.priority.is_some()) {
        return Err(invalid("demand priority requires objective priority_skill_distance"));
    }
    let priorities: BTreeMap<_, _> = demands.iter().filter_map(|d|
        d.priority.map(|priority| (d.key.clone(), priority))).collect();
    let requested: Vec<_> = demands.into_iter().map(DemandInput::normalized).collect();
    match objective {
        Objective::MaxFilledSlots => Ok((workforce::plan(state, context, &requested, maximum)?, None)),
        Objective::PrioritySkillDistance => {
            let out = quality_adapter::plan(state, context, &requested, &priorities, maximum)?;
            Ok((out.plan, Some(Details { priorities: out.priorities, allocation: out.quality })))
        }
    }
}
...
pub(super) fn model(a: &WorkforceAnalysis, details: Option<&Details>) -> Digest32 {
    ...
    let Some(details) = details else { return base; };
...
pub(super) fn decorate(out: &mut Value, details: Option<&Details>, model: Digest32) -> Result<()> {
    let Some(details) = details else { return Ok(()); };
    let allocation = &details.allocation;
    let total_priority = allocation.cost.0[0].checked_neg().ok_or_else(|| invariant("priority sum overflow"))?;
    let total_effective = allocation.cost.0[1].checked_neg().ok_or_else(|| invariant("effective skill sum overflow"))?;
    let total_nominal = allocation.cost.0[2].checked_neg().ok_or_else(|| invariant("nominal skill sum overflow"))?;
    ...
    let demands = out["demands"].as_array_mut().ok_or_else(|| invariant("workforce demand summary missing"))?;
    if demands.len() != details.priorities.len() { return Err(invariant("workforce priority summary length differs")); }
