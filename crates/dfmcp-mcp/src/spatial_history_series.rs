...
pub(in super::super::super) fn schema() -> Result<Value> {
    let mut schema: Value = serde_json::from_str(include_str!(
        "../../../schemas/mcp_historical_series_v1.json"
    ))
    .map_err(|_| {
        error(
            ErrorCode::InternalInvariantViolation,
            "historical series schema invalid",
        )
    })?;
    ...
    let quantity: Value = serde_json::from_str(include_str!(
        "../../../schemas/mcp_item_quantity_v1.json"
    ))
    .map_err(|_| {
        error(
            ErrorCode::InternalInvariantViolation,
            "quantity schema invalid",
        )
    })?;
    let condition: Value = serde_json::from_str(include_str!(
...
impl QuantityBounds {
    fn read(value: &Value) -> Result<Self> {
        let bad = || {
            error(
                ErrorCode::InternalInvariantViolation,
                "quantity evaluator returned invalid bounds",
            )
        };
    ...
    fn difference(self, previous: Self) -> (Option<i128>, Option<i128>) {
        (
            previous
                .upper
                .map(|n| i128::from(self.lower) - i128::from(n)),
            self.upper
                .map(|n| i128::from(n) - i128::from(previous.lower)),
        )
    }
}
fn transition(previous: &(JournalEntry, Value), current: &(JournalEntry, Value)) -> Result<Value> {
    ...
    if before.number.checked_add(1) != Some(after.number) {
        return Err(error(
            ErrorCode::InternalInvariantViolation,
            "timeline omitted an intermediate retained sample",
        ));
    }
    ...
    if before.anchor.fortress_id != after.anchor.fortress_id
        || before.anchor.cursor.epoch != after.anchor.cursor.epoch
        || after.anchor.tick < before.anchor.tick
    {
        result["status"] = json!("epoch_or_clock_discontinuity");
        result["net_change"] = Value::Null;
        result["net_rate"] = Value::Null;
        return Ok(result);
    }
    if current.1.get("kind").and_then(Value::as_str) == Some("condition_evaluation") {
        return conditions::transition(
            &previous.1,
            &current.1,
            result,
            after.anchor.tick.0 - before.anchor.tick.0,
        );
    }
    let (lower, upper) =
        QuantityBounds::read(&current.1)?.difference(QuantityBounds::read(&previous.1)?);
    ...
    result["status"] = json!(if lower.is_some_and(|n| n > 0) {
        "definite_increase"
    } else if upper.is_some_and(|n| n < 0) {
        "definite_decrease"
    } else if lower == Some(0) && upper == Some(0) {
        "unchanged_quantity"
    } else {
        "indeterminate_change"
    });
    result["net_change"] = interval;
    result["elapsed_game_ticks"] = json!(ticks);
    result["net_rate"] = if ticks == 0 {
        Value::Null
    } else {
        json!({
    ...
    result["rate_unavailable_reason"] = if ticks == 0 {
        json!("same_game_tick")
    } else {
        Value::Null
    };
...
fn continuation(identity: Digest32, record: u64) -> String {
    ...
    format!(
        "hs1:{record}:{}",
        Digest32::of_bytes(bytes.to_string().as_bytes())
    )
}
fn start_record(raw: Option<&str>, identity: Digest32, first: u64, last: u64) -> Result<u64> {
    ...
    if raw.len() > 128 {
        return Err(bounded("timeline continuation exceeds 128 bytes"));
    }
    ...
    {
        return Err(invalid("invalid timeline continuation"));
    }
    let record = parts[1]
        .parse::<u64>()
        .map_err(|_| invalid("timeline continuation overflows"))?;
    if raw != continuation(identity, record) {
        return Err(error(
            ErrorCode::StaleAnchor,
            "timeline continuation names another session, archive head, interval or measurement",
        ));
    }
    if record <= first || record > last {
        return Err(error(
            ErrorCode::CursorGap,
            "timeline continuation is outside the selected interval",
        ));
    }
    Ok(record)
}
fn render_series(
    ...
    let mut packet: Value = serde_json::from_str(&raw).map_err(|_| {
        error(
            ErrorCode::InternalInvariantViolation,
            "timeline packet is not JSON",
        )
    })?;
    packet["agent_turn"]["continuity"]["status"] = json!("partial");
    packet["agent_turn"]["continuity"]["gap"] =
        json!({"reason":"retained_samples_not_continuous_or_current_state"});
    ...
    {
        return Err(bounded(
            "complete historical series packet exceeds output budget",
        ));
    }
    Ok(encoded)
}
...
pub(in super::super::super) fn execute(
    ...
    if session.anchor()? != context.anchor {
        return Err(error(
            ErrorCode::StaleAnchor,
            "timeline context differs from current session",
        ));
    }
    let parsed: SeriesEnvelope = serde_json::from_value(input.clone())
        .map_err(|_| invalid("invalid historical series envelope"))?;
    if parsed.schema != "dfmcp.query/1" {
        return Err(invalid("historical series requires dfmcp.query/1"));
    }
    if parsed
        .expected_anchor
        .as_ref()
        .is_some_and(|a| a != &anchor_json(context.anchor))
    {
        return Err(error(
            ErrorCode::StaleAnchor,
            "timeline expected_anchor differs from current session",
        ));
    }
    let SeriesRequest::HistoricalSeries {
        from,
        to,
        measurement,
        limit,
    ...
    {
        return Err(invalid(
            "timeline requires increasing endpoints and a page limit of 1..32",
        ));
    }
    let condition_measurement = match measurement.get("kind").and_then(Value::as_str) {
        ...
        _ => {
            return Err(invalid(
                "historical series permits stateless item_quantity or condition_evaluation only",
            ));
        }
    };
    let journal = session.journal.as_mut().ok_or_else(|| {
        invalid("historical series requires the configured spatial observation journal")
    })?;
    journal.validate_custody(context)?;
    if journal.state().snapshot().map(|s| s.anchor()) != Some(context.anchor) {
        return Err(error(
            ErrorCode::CorruptLedger,
            "timeline archive differs from the current session root",
        ));
    }
    ...
    let basis = if start > first.number {
        start - 1
    } else {
        start
    };
    let requested: Vec<_> = journal
        .entries()
        .iter()
        .filter(|e| (basis..=end).contains(&e.number))
    ...
    if let Some(p) = projection.as_mut() {
        ...
        p.references = vec![
            json!({"kind":if condition_measurement {"archived_condition_series"} else {"archived_quantity_series"},"journal_id":id.to_string(),
    ...
    let mut sample = out.clone();
    sample["continuation"] = json!("x".repeat(128));
    let overhead = render_series(session, context, &last, projection.as_ref(), sample.clone())?
        .len()
        .saturating_sub(sample.to_string().len())
        + 512;
    ...
    result_context.budget.max_bytes = context
        ...
        .filter(|n| *n > 0)
        .ok_or_else(|| bounded("timeline evidence leaves no result allowance"))?;
    if projection.is_some() {
        result_context = semantic_query::result_context(&result_context)?;
    }
    let maximum = result_context.budget.max_bytes;
    if sample.to_string().len() as u64 >= maximum {
        return Err(bounded("timeline metadata leaves no sample allowance"));
    }
    ...
    let journal = session
        .journal
        .as_mut()
        .ok_or_else(|| invalid("timeline archive disappeared"))?;
    let values = journal.project_records(&requested, &replay_context, |entry, state| {
        archive::validate_limits(
            state
                .observation_full()
                .ok_or_else(|| error(ErrorCode::CorruptLedger, "timeline source absent"))?,
            limits,
        )?;
        let snapshot = state
            .snapshot()
            .ok_or_else(|| error(ErrorCode::CorruptLedger, "timeline snapshot absent"))?;
        let mut current = remaining(context, started)?;
    ...
    journal.validate_custody(context)?;
    if journal.id() != id || journal.head() != head {
        return Err(error(
            ErrorCode::StaleAnchor,
            "timeline archive head changed",
        ));
    }
    let mut returned = 0u64;
    let mut target = first.clone();
    for (index, current) in values.iter().enumerate() {
        if current.0.number < start {
            continue;
        }
        let change = if current.0.number == first.number {
            Value::Null
        } else {
            let previous = index
                .checked_sub(1)
                ...
                .ok_or_else(|| {
                    error(
                        ErrorCode::InternalInvariantViolation,
                        "timeline page lost preceding sample",
                    )
                })?;
            transition(previous, current)?
        };
        let row = if condition_measurement {
            conditions::row(&current.0, &current.1, change)?
        } else {
        ...
        next["rows"]
            ...
            .ok_or_else(|| {
                error(
                    ErrorCode::InternalInvariantViolation,
                    "timeline rows absent",
                )
            })?
            .push(row);
        next["returned"] = json!(returned + 1);
        next["truncated"] = json!(current.0.number < last.number);
        next["continuation"] = if current.0.number < last.number {
            json!(continuation(identity, current.0.number + 1))
        } else {
            Value::Null
        };
        ...
        target = current.0.clone();
        out = next;
        returned += 1;
    }
    if returned == 0 {
        return Err(bounded(
            "one whole timeline sample and its evidence cannot fit",
        ));
    }
    if let Some(p) = projection.as_mut() {
        p.anchor = anchor_json(target.anchor);
    }
    remaining(context, started)?;
    if projection.is_some() {
