...
pub(super) fn query(
    ...
    let outcome = (|| -> Result<String> {
        ...
        let needs_read = if awaiting {
            let projection = guard.adapter.current_projection().ok_or_else(|| {
                error(
                    ...
                    "condition wait has no canonical projection",
                )
            })?;
            let input = query.as_ref().ok_or_else(|| {
                error(
                    ErrorCode::InternalInvariantViolation,
                    "condition wait lost its request",
                )
            })?;
        ...
        let refresh = if needs_read {
            let observation =
                watch_refresh::once(&context, |request, ctx| guard.adapter.observe(request, ctx))?;
        ...
        result_context.budget.max_bytes = u64::try_from(result_bytes).map_err(|_| {
            error(
                ...
                "remaining query budget cannot be represented",
            )
        })?;
        let payload = if query.is_none() && mode.as_deref() == Some("schema") {
            ...
            let projection = guard.adapter.current_projection().ok_or_else(|| {
                error(
                    ErrorCode::InternalInvariantViolation,
                    "query adapter lost its projection",
                )
            })?;
        ...
        semantic_query::publish_with_active_work(&result_context, payload, |value| {
            view.finish(value)
        })
    })();
    match outcome {
        ...
        Err(failure) => {
            ...
            let continuity = if guard.source_poisoned() {
                ContinuityStatus::Stale
            } else if current.cursor.epoch != basis.cursor.epoch {
                ContinuityStatus::Reset
            } else {
                ContinuityStatus::Continuous
            };
            let raw = session_error(
                &guard,
                operation,
                AgentPhase::Inspect,
                ObservationProfile::Tactical,
            ...
            let decorated =
                semantic_query::publish_with_active_work(&context, json!({}), |metadata| {
                    let mut payload: JsonValue = serde_json::from_str(&raw).map_err(|_| {
                        error(
                            ErrorCode::InternalInvariantViolation,
                            "query error packet is not JSON",
                        )
                    })?;
                    payload["agent_turn"]["active_work"]["obligations"] =
                        metadata["_condition_watch_work"].clone();
                    let encoded = payload.to_string();
                    if encoded.len() > response_byte_limit(&guard) {
                        return Err(error(
                            ErrorCode::BudgetExceeded,
                            "query error and active work exceed the response budget",
                        ));
                    }
                    Ok(encoded)
                });
...
fn query_schema() -> Result<JsonValue> {
    serde_json::from_str(include_str!("../../../schemas/mcp_query_v1.json")).map_err(|failure| {
        error(
            ErrorCode::InternalInvariantViolation,
...
fn add_field_catalogs(
    ...
    let size = serde_json::to_vec(payload)
        .map_err(|_| {
            error(
                ErrorCode::InternalInvariantViolation,
                "cannot measure query field catalogs",
            )
        })?
        .len();
    ...
    let mut add = |row: &mut JsonValue| -> Result<()> {
        ...
        let Some(entity) = snapshot.graph.entities.get(&EntityId::new(id)) else {
            return Ok(());
        };
        let fields = entity
            .fields
            .keys()
            .filter(|field| field.len() <= 128)
            .take(128)
            .collect::<Vec<_>>();
        let addition = json!({"available_fields":fields,
            "available_fields_truncated":fields.len() != entity.fields.len()});
        let added_bytes = serde_json::to_vec(&addition)
            .map_err(|_| {
                error(
                    ErrorCode::InternalInvariantViolation,
                    "cannot measure query field catalog",
                )
            })?
            .len();
    ...
    if let Some(row) = payload.get_mut("row") {
        add(row)?;
    }
    if omitted {
...
mod tests {
    ...
    fn structured_query_without_a_session_returns_the_agent_turn_contract() -> Result<()> {
        ...
        let result: JsonValue = serde_json::from_str(&raw)
            .map_err(|failure| error(ErrorCode::InternalInvariantViolation, failure.to_string()))?;
    ...
    fn foreign_runtime_sessions_cannot_enter_structured_inspection() -> Result<()> {
        let raw = query(
            Some("80000000000000000000000000000001".to_owned()),
            None,
            None,
            None,
