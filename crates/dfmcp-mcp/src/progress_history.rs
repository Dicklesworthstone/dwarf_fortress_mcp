...
use super::{
    Digest32, ErrorCode, OperationContext, ProgressComparison, ProgressObservation, Result,
    SessionId, Value, comparison_json, error, json, observation_json, parse_digest,
};
use dfmcp_adapter::operations_journal::JournalStorage;
use dfmcp_adapter::work_order_progress::archive::{
    ArchivedProgress, ProgressArchive, ProgressArchiveEntry, ProgressArchiveSummary,
};
    ...
    pub fn parse(raw: &str) -> Result<Self> {
        if raw.len() > 2048 {
            return Err(error(
                ...
                "progress history request exceeds 2048 bytes",
            ));
        }
        let request: Self = serde_json::from_str(raw).map_err(|_| {
            error(
                ErrorCode::InvalidRequest,
                "invalid or unknown progress history request fields",
            )
        })?;
        match &request {
            Self::List {
                ...
                if !matches!(limit, None | Some(1..=64)) {
                    return Err(error(
                        ErrorCode::InvalidRequest,
            ...
            Self::Record {
                ...
                parse_digest(archive_id)?;
                parse_digest(record_digest)?;
                if *number == 0 {
                    return Err(error(
                        ErrorCode::InvalidRequest,
                        "history record numbers start at one",
                    ));
                }
            }
            Self::Changes {
                ...
                parse_digest(archive_id)?;
                parse_digest(before_digest)?;
                parse_digest(after_digest)?;
                if *before_number == 0 || before_number >= after_number {
                    return Err(error(
                        ErrorCode::InvalidRequest,
                        "history endpoints must be positive, ordered and distinct",
                    ));
                }
            }
        }
...
impl Cursors {
    fn issue(
        ...
        bytes.extend_from_slice(summary.archive_id.as_bytes());
        ...
        if self.retained.iter().any(|c| c.token == token) {
            return token;
        }
        if self.retained.len() == 64 {
            self.retained.pop_front();
        }
        self.retained.push_back(Cursor {
            session,
        ...
        let cursor = self
            ...
            .ok_or_else(|| {
                error(
                    ...
                    "history continuation is unknown or evicted; restart discovery",
                )
            })?;
        if cursor.session != session
            || cursor.archive != summary.archive_id
            || cursor.head != summary.head
            || cursor.limit != limit
        {
            return Err(error(
                ErrorCode::StaleAnchor,
                "history cursor belongs to another session, archive head or page size",
            ));
        }
        Ok(cursor.after)
    }
...
pub fn query<S: JournalStorage>(
    archive: &mut ProgressArchive<S>,
    cursors: &mut Cursors,
    request: Request,
    c: &OperationContext,
) -> Result<Answer> {
    ...
    match request {
        Request::List {
            ...
            let after = continuation
                .as_deref()
                .map(|v| cursors.resolve(v, c.session_id, &summary, limit))
                .transpose()?
                .unwrap_or(0);
            let page = archive.page(summary.head, after, limit, c)?;
            let next = page
                .next_after
                .map(|after| cursors.issue(c.session_id, &summary, after, limit));
            let entries = page
                .entries
                .iter()
                .map(|e| entry_json(e, summary.archive_id))
                .collect::<Vec<_>>();
            Ok(Answer {
                value: json!({"ok":true,"historical":true,"native_calls":0,"entries":entries,
                "total_records":summary.records,"complete_set_in_this_response":after == 0 && next.is_none(),
                "continuation":next,"progress_archive":summary_json(&summary)}),
                capture: None,
                comparison: None,
            })
        }
        Request::Record {
            ...
            if parse_digest(&archive_id)? != summary.archive_id {
                return Err(error(
                    ErrorCode::StaleAnchor,
                    "history reference names a different archive",
                ));
            }
            ...
            Ok(Answer {
                value,
                capture: Some(record.observation),
                comparison: None,
            })
        }
        Request::Changes {
            ...
            if parse_digest(&archive_id)? != summary.archive_id {
                return Err(error(
                    ErrorCode::StaleAnchor,
                    "history comparison names a different archive",
                ));
            }
            let (a, b, comparison) = archive.compare_records(
                (before_number, parse_digest(&before_digest)?),
                (after_number, parse_digest(&after_digest)?),
                c,
            )?;
