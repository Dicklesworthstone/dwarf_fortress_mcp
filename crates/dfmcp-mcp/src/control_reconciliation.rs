...
use dfmcp_adapter::pause_reconciliation::{self, PauseReconciliationSource,
...
impl PauseReconciliationSource for ControlConnection {
    fn query_effect(&mut self, record: &DurablePauseRecord, remaining: Duration,
        ...
        if self.client.poisoned() {
            self.client = ControlRpcClient::connect(self.endpoint, self.token.clone(),
                self.nonce.clone(), remaining)?;
        }
        let remaining = remaining.checked_sub(started.elapsed())
            .filter(|v| *v >= Duration::from_millis(1))
            .ok_or_else(|| err(ErrorCode::BudgetExceeded, "reconciliation deadline exhausted during reconnect"))?;
        self.client.reset_deadline(remaining)?;
        let result = self.client.query_pause(&record.idempotency_key, record.plan_digest);
        if result.is_err() { self.client.fence(); }
        result
    }
}
...
fn ceiling(context: &OperationContext, bytes: Option<u64>, tokens: Option<u32>) -> Result<usize> {
    ...
    if bytes == 0 || bytes > context.budget.max_bytes || tokens == 0
        ...
        return Err(err(ErrorCode::BudgetExceeded, "wait response budgets must be positive and only narrow session limits"));
    }
    usize::try_from(bytes.min(u64::from(tokens) * 4))
        .map_err(|_| err(ErrorCode::BudgetExceeded, "wait response byte limit cannot be represented"))
}
...
fn payload(batch: &ReconciliationBatch, journal: Value, maximum: usize) -> Value {
    ...
    let unresolved = batch.items.iter().filter(|v| v.record.state.reconciliation_required()).count();
    let items: Vec<_> = batch.items.iter().map(|item| json!({
        "effect":record_json(&item.record),"queried":item.queried,"deferred":item.deferred,
        "error_code":item.error.map(|e|e.as_str())})).collect();
...
fn reserve(selected: &[DurablePauseRecord], journal: Value, maximum: usize) -> Result<()> {
    let items = selected.iter().map(|record| {
        ...
        record.revision = u64::MAX; record.transition_number = u64::MAX;
        ReconciliationItem { record, queried: false, deferred: false, error: None }
    }).collect();
    let sample = ReconciliationBatch { items, head_before: Digest32::ZERO,
        head_after: Digest32::ZERO, stopped: None };
    let mut body = payload(&sample, journal, maximum);
    body["ok"] = json!(false); body["all_terminal"] = json!(false); body["pass_complete"] = json!(false);
    body["stopped_error_code"] = json!("x".repeat(64));
    if let Some(items) = body["items"].as_array_mut() {
        for item in items { item["error_code"] = json!("x".repeat(64)); }
    }
    for field in ["effects", "transitions", "retained_bytes", "repaired_tail_bytes"] {
        body["durable_effect_journal"][field] = json!(u64::MAX);
    }
    body["durable_effect_journal"]["fenced"] = json!(false);
    let length = packet("fortress.wait", body, false).len().saturating_add(256);
    if length > maximum {
...
pub(super) fn wait(session: &mut ControlSession, mut context: OperationContext,
    keys: Vec<String>, wall_millis: Option<u64>, bytes: Option<u64>, tokens: Option<u32>) -> Result<Value> {
    let started = Instant::now();
    context.authorize(Capability::ControlClock, RiskTier::Reversible, &[], None)?;
    session.live()?;
    let millis = wall_millis.unwrap_or(context.budget.max_wall_millis);
    if millis == 0 || millis > context.budget.max_wall_millis {
        return Err(err(ErrorCode::BudgetExceeded, "wait wall-time must be positive and may only narrow the session budget"));
    }
    ...
    let remaining = Duration::from_millis(millis).checked_sub(started.elapsed())
        .filter(|v| *v >= Duration::from_millis(1))
        .ok_or_else(||err(ErrorCode::BudgetExceeded,"wait deadline exhausted before any bridge query"))?;
    ...
    let connection = session.connection.as_mut()
        .ok_or_else(||err(ErrorCode::CapabilityDenied,"offline recovery cannot reconcile live effects"))?;
    let batch = pause_reconciliation::reconcile_batch(&mut session.journal, connection, &keys, &context)?;
    let body = payload(&batch, journal_json(&session.journal), maximum);
    if packet("fortress.wait", body.clone(), false).len() > maximum {
        return Err(err(ErrorCode::InternalInvariantViolation,
...
mod tests {
    ...
    fn record(key: String) -> DurablePauseRecord {
        DurablePauseRecord { idempotency_key:key, plan_digest:Digest32::ZERO,
            desired_paused:true, expected_game_tick:10, bridge_generation:7, prepare_token:[1;16],
            state:DurablePauseState::CommitStarted, effect_known:false, effect_applied:false,
            observed_paused:None, observed_game_tick:None, receipt_digest:None, revision:2,
            transition_number:2, previous_digest:Digest32::ZERO, record_digest:Digest32::ZERO }
    }
    fn metadata() -> Value {
    ...
    fn reservation_covers_terminal_unknown_failed_and_deferred_shapes() -> Result<()> {
        for key in ["k".to_owned(), "\\\"".repeat(256), "\u{1f332}".repeat(128)] {
            ...
            for state in [DurablePauseState::Prepared, DurablePauseState::CommitStarted,
                DurablePauseState::Indeterminate, DurablePauseState::VerifiedApplied, DurablePauseState::VerifiedNotApplied] {
                let mut r=base.clone(); r.state=state; r.revision=u64::MAX; r.transition_number=u64::MAX;
                r.observed_paused=Some(false);r.observed_game_tick=Some(u64::MAX);r.receipt_digest=Some(Digest32::ZERO);
                let batch=ReconciliationBatch { items:vec![ReconciliationItem {record:r,queried:true,
                    deferred:true,error:Some(ErrorCode::InternalInvariantViolation)}],
                    head_before:Digest32::ZERO,head_after:Digest32::ZERO,stopped:Some(ErrorCode::InternalInvariantViolation) };
                assert!(packet("fortress.wait",payload(&batch,metadata(),maximum),false).len()<=maximum);
            }
        }
        Ok(())
    }
