...
use super::*;
use dfmcp_adapter::live_jobs_rpc::operations::paged::{PagedOperationsLimits, PagedOperationsRpcClient};
...
impl PagedSlot {
    fn reserve() -> Result<Self> {
        PAGED_SLOTS.fetch_update(Ordering::AcqRel, Ordering::Acquire,
            |count| (count < 2).then_some(count + 1))
            .map_err(|_| error(ErrorCode::BudgetExceeded, "paged operations capacity is two sessions"))?;
        Ok(Self)
    }
}
impl Drop for PagedSlot { fn drop(&mut self) { PAGED_SLOTS.fetch_sub(1, Ordering::AcqRel); } }
struct PagedSource { client: PagedOperationsRpcClient<DeadlineStream>, _slot: PagedSlot }
impl OperationsSource for PagedSource {
    fn read(&mut self, timeout: Duration) -> Result<LiveOperationsObservation> { self.client.refresh(timeout) }
    fn poisoned(&self) -> bool { self.client.poisoned() }
    fn fence(&mut self) { self.client.fence(); }
}
fn next_paged_id() -> Result<SessionId> {
    let mut next = lock(&NEXT_PAGED)?;
    if *next >= 1u128 << 60 { return Err(error(ErrorCode::BudgetExceeded, "paged session IDs exhausted")); }
    let id = SessionId::new((1u128 << 127) | PAGED_FAMILY | *next); *next += 1; Ok(id)
}
fn allowed_paged_environment(name: &str) -> bool {
    !name.starts_with("DFMCP_") || matches!(name, "DFMCP_ALLOW_UNADMITTED_OPERATIONS_V1_4"
        | "DFMCP_OPERATIONS_PAGED_TOKEN" | "DFMCP_OPERATIONS_PAGED_ENDPOINT")
}
fn validate_paged_environment() -> Result<()> {
    if std::env::var("DFMCP_ALLOW_UNADMITTED_OPERATIONS_V1_4").ok().as_deref() != Some("1")
...
#[tool(description = "Open a read-only operations/1.4 session. Capture one coherent native world and transfer immutable pages. Supports up to 65536 items and 16 MiB, not arbitrary-size worlds. Profile, credentials and endpoint are fixed by the operator; no game effects or durable archive are enabled.")]
#[allow(clippy::too_many_arguments)]
pub fn fortress_open_session(max_jobs: Option<u32>, max_buildings: Option<u32>, max_items: Option<u32>,
    max_bytes: Option<u64>, max_output_tokens: Option<u32>, max_wall_millis: Option<u64>,
    page_bytes: Option<u32>, requested_capabilities: Option<Vec<String>>) -> String
{
    let result = (|| -> Result<String> {
        validate_paged_environment()?;
        let capabilities = capabilities(requested_capabilities)?;
        let paged = PagedOperationsLimits { jobs: max_jobs.unwrap_or(4096), buildings: max_buildings.unwrap_or(4096),
            items: max_items.unwrap_or(65536), payload_bytes: usize::try_from(max_bytes.unwrap_or(16 * 1024 * 1024))
                .map_err(|_| error(ErrorCode::BudgetExceeded, "paged bytes do not fit this platform"))?,
            page_bytes: usize::try_from(page_bytes.unwrap_or(65536))
                .map_err(|_| error(ErrorCode::BudgetExceeded, "page width does not fit this platform"))? };
        paged.validate()?;
        let budget = WorkBudget { max_wall_millis: max_wall_millis.unwrap_or(5000), max_game_ticks: 1_000_000,
            max_entities: paged.entity_limit(), max_bytes: paged.payload_bytes as u64,
            max_output_tokens: max_output_tokens.unwrap_or(8192), max_actions: 1 };
        budget.validate()?;
        if budget.max_bytes < 8192 || !(2048..=65536).contains(&budget.max_output_tokens)
            || !(1..=60000).contains(&budget.max_wall_millis)
        {
            return Err(error(ErrorCode::BudgetExceeded, "paged response or deadline is outside its bounds"));
        }
        let slot = SessionSlot::reserve()?;
        let paged_slot = PagedSlot::reserve()?;
        let id = next_paged_id()?;
        let endpoint = std::env::var("DFMCP_OPERATIONS_PAGED_ENDPOINT").unwrap_or_else(|_| "127.0.0.1:5000".to_owned());
        let endpoint = dfmcp_adapter::parse_loopback_endpoint(&endpoint)?;
        let token = std::env::var("DFMCP_OPERATIONS_PAGED_TOKEN")
            .map_err(|_| error(ErrorCode::CapabilityDenied, "DFMCP_OPERATIONS_PAGED_TOKEN is required"))?;
        let client = PagedOperationsRpcClient::connect(endpoint, token.into_bytes(), id.get().to_be_bytes().to_vec(),
            Duration::from_millis(budget.max_wall_millis), paged)?;
        let mut source = PagedSource { client, _slot: paged_slot };
        ...
        let fortress = state.snapshot().ok_or_else(|| error(ErrorCode::InternalInvariantViolation, "paged bootstrap lost snapshot"))?.fortress_id;
        ...
        let limits = OperationsLimits { jobs: paged.jobs, buildings: paged.buildings, items: paged.items, payload_bytes: paged.payload_bytes };
        let mut session = OperationsSession { id, source: Box::new(source), state, journal: None, limits, budget,
            grants, request: 0, _slot: slot };
        let context = session.context()?;
        let response = packet(Some(&session), Some(&context), "fortress.open_session", json!({"ok":true,
        ...
        if registry.contains_key(&id) { return Err(error(ErrorCode::InternalInvariantViolation, "paged session identity collision")); }
...
pub fn run_stdio() {
    if let Err(failure) = validate_paged_environment() { eprintln!("{failure}"); std::process::exit(1); }
