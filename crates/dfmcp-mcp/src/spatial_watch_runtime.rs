...
fn configured_path(
    ...
    let text = value
        ...
        .ok_or_else(|| {
            error(
                ...
                "DFMCP_SPATIAL_CITIZEN_WATCH_JOURNAL must be nonempty UTF-8 without NUL",
            )
        })?;
        ...
        || path.file_name().is_none()
    {
        return Err(error(
            ErrorCode::InvalidRequest,
            "watch journal must be a separate absolute normalized operator-configured file",
        ));
    }
    Ok(Some(path))
}
...
pub(super) fn configuration(observation_path: Option<&Path>) -> Result<Option<PathBuf>> {
    configured_path(
        std::env::var_os("DFMCP_SPATIAL_CITIZEN_WATCH_JOURNAL"),
        observation_path,
    )
}
...
pub(super) fn finish_open(
    ...
    let journal = session.journal.as_ref().ok_or_else(|| {
        error(
            ...
            "durable watch bootstrap has no observation archive",
        )
    })?;
    if journal.fenced()
        || journal.profile() != "spatial/1.8"
        || journal.state().snapshot().map(|snapshot| snapshot.anchor()) != Some(context.anchor)
    {
        return Err(error(
            ErrorCode::CorruptLedger,
            "durable watch bootstrap requires the synced current spatial/1.8 anchor",
        ));
    }
    ...
    let (output, owner) = {
        ...
        let snapshot = view.state.snapshot().ok_or_else(|| {
            error(
                ...
                "durable watch bootstrap snapshot absent",
            )
        })?;
        semantic_query::attach_watch_journal(
            snapshot,
            context,
            path,
            archive,
...
mod tests {
    ...
    fn watch_configuration_requires_separate_archive_and_absolute_path() {
        for value in [
            ...
            "relative.bin",
            "/private/../watches.bin",
            "/private/observations.bin",
            "/private/nul\u{0}",
        ] {
            assert!(
                configured_path(
                    Some(value.into()),
                    Some(Path::new("/private/observations.bin"))
                )
                .is_err()
            );
        }
