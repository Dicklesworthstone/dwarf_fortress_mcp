...
fn register_count(s: &Registered, key: &str, condition: Value) -> Result<Value> {
    let result = ask(
        s,
        json!({"kind":"watch","key":key,"condition":condition,
...
fn without_jobs(tick: u32) -> Result<LiveSpatialCitizenObservation> {
    ...
    part(
        &mut spatial,
        &original.spatial().terrain().encode_payload()?,
    );
...
fn population_watch_tracks_a_changing_roster_without_entity_handles() -> Result<()> {
    ...
    let registered = register_count(
        &s,
        "queue-empty",
        count_condition("job", "eq", 0, json!({"op":"always"})),
    )?;
    assert_eq!(registered["record"]["status"], "waiting");
    {
        ...
        session.source = Box::new(Script {
            values: VecDeque::from([without_jobs(4)?, without_jobs(5)?]),
            calls: s.calls.clone(),
            fenced: false,
            corrupt: None,
        });
    ...
    for step in 1..=2 {
        ...
        assert_eq!(checkpoint(&result)?, before + step);
    }
    let detail = ask(
        &s,
        json!({"kind":"poll_watch","watch":registered["record"]["watch"]}),
    )?;
    require_success(&detail)?;
    assert_eq!(detail["record"]["evaluation"]["facts"][0]["matched_min"], 0);
    assert_eq!(
        detail["record"]["evaluation"]["facts"][0]["complete_world_count_proven"],
        false
    );
...
fn durable_population_definitions_recover_but_restart_does_not_count_as_a_sample() -> Result<()> {
    ...
    let initial = register_count(
        &s,
        "jobs-observed",
        count_condition("job", "ge", 0, json!({"op":"always"})),
    )?;
    watch(&s, "legacy-paused", 2)?;
    let old_handle = initial["record"]["watch"].clone();
    drop(s);
    let s = register(&files, 4, &[5, 6], false)?;
    let list = ask(&s, json!({"kind":"watches"}))?;
    require_success(&list)?;
    let recovered = list["records"]
        .as_array()
        .and_then(|rows| rows.iter().find(|r| r["key"] == "jobs-observed"))
        .ok_or_else(|| error(ErrorCode::InvalidRequest, "recovered count watch absent"))?;
    assert_ne!(recovered["watch"], old_handle);
    assert_eq!(recovered["stable_observations"], 0);
    let detail = ask(&s, json!({"kind":"poll_watch","watch":recovered["watch"]}))?;
    require_success(&detail)?;
    assert_eq!(
        detail["record"]["definition"]["condition"]["op"],
        "entity_count"
    );
...
fn missing_native_fields_do_not_prove_a_negated_population_goal() -> Result<()> {
    ...
    assert_eq!(result["records"][0]["status"], "blocked_unknown");
    let detail = ask(
        &s,
        json!({"kind":"poll_watch","watch":registered["record"]["watch"]}),
    )?;
    assert_eq!(detail["record"]["evaluation"]["facts"][0]["matched_min"], 0);
    assert!(
        detail["record"]["evaluation"]["facts"][0]["matched_max"]
            .as_u64()
            .is_some_and(|n| n > 0)
    );
    Ok(())
...
fn count_schema_is_discoverable_and_refused_registration_leaves_no_checkpoint() -> Result<()> {
    ...
    let conditions = schema["query_schema"]["$defs"]["watch_condition"]["oneOf"]
        .as_array()
    ...
    assert_eq!(
        ask(
            &s,
            json!({"kind":"watch","key":"bad-scope","condition":condition,
        "deadline_tick":105u64*403200+100})
        )?["ok"],
        false
    );
    {
    ...
    assert_eq!(
        ask(
            &s,
            json!({"kind":"watch","key":"cannot-fit","condition":
        count_condition("job","ge",1,json!({"op":"always"})),"deadline_tick":105u64*403200+100})
        )?["ok"],
        false
    );
