...
fn io_error(_: std::io::Error) -> DfmcpError {
    error(ErrorCode::CorruptLedger, "connectivity fixture I/O")
}
struct Files {
    directory: PathBuf,
...
impl Source for NeverRead {
    fn read(&mut self, _: Duration) -> Result<LiveSpatialCitizenObservation> {
        ...
        Err(error(
            ...
            "connectivity must not read the source",
        ))
    }
...
fn observation(tick: u32, split: bool) -> Result<LiveSpatialCitizenObservation> {
    ...
    if split {
        terrain.map.cells[2] = Cell::Hidden;
    }
    let part = |out: &mut Vec<u8>, value: &[u8]| {
...
impl Drop for Registered {
    fn drop(&mut self) {
        let _ = session_release::close(Some(self.0.to_string()), true);
    }
}
...
fn actual_dispatch_preserves_watches_and_supports_fenced_history_and_archive_reopen() -> Result<()>
{
    ...
    state.publish(observation(3, false)?)?;
    let anchor = state
        .snapshot()
        .ok_or_else(|| error(ErrorCode::InvalidRequest, "fixture anchor"))?
        .anchor();
    ...
    let journal = s
        .journal
        .as_mut()
        .ok_or_else(|| error(ErrorCode::CorruptLedger, "fixture journal"))?;
    ...
    let watched = registered.call(
        json!({"schema":"dfmcp.query/1","query":{"kind":"watch","key":"retain-me",
        "condition":{"op":"tick_at_least","value":u64::MAX},"deadline_tick":current.tick.0+100}}),
    )?;
    assert_eq!(watched["record"]["status"], "waiting");
    let watches =
        registered.call(json!({"schema":"dfmcp.query/1","query":{"kind":"watches"}}))?["records"]
            .clone();
    ...
    assert_eq!(old["summary"]["components"], 1);
    ...
    assert_eq!(
        registered.call(input("all"))?["error"]["code"],
        "adapter_unavailable"
    );
    assert_eq!(
        registered.call(historical.clone())?["structural_digest"],
        old["structural_digest"]
    );
    ...
    assert_eq!(out["archive_only"], true);
    assert_eq!(out["current_freshness_proven"], false);
    assert_eq!(out["rows"], old["rows"]);
    assert_eq!(out["structural_digest"], old["structural_digest"]);
    assert_eq!(out["bridge_connection_present"], false);
    assert_eq!(
        fs::read(&files.observations).map_err(io_error)?,
        observation_bytes
    );
    let mut corrupted = observation_bytes;
    corrupted[first.offset as usize + 50] ^= 1;
    fs::write(&files.observations, &corrupted).map_err(io_error)?;
    assert_eq!(
        recovered.call(historical)?["error"]["code"],
        "corrupt_ledger"
    );
