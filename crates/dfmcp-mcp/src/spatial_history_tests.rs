...
fn io_error(_: std::io::Error) -> DfmcpError {
    error(ErrorCode::CorruptLedger, "test archive I/O")
}
struct Archive {
    directory: PathBuf,
    path: PathBuf,
...
impl Archive {
    fn new() -> Result<Self> {
        ...
        let directory =
            std::env::temp_dir().join(format!("dfmcp-spatial-history-{}-{n}", std::process::id()));
        ...
        Ok(Self {
            path: directory.join("observations.bin"),
            directory,
        })
    }
}
    ...
    result["rows"]
        ...
        .ok_or_else(|| {
            error(
                ...
                "test archive missing first row",
            )
        })
}
...
fn watch(s: &Registered) -> Result<Value> {
    let result = ask(
        s,
        json!({"kind":"watch","key":"archive-review","label":"Wait for later live capture",
...
fn archived_inventory_and_its_route_use_past_terrain_without_advancing_current_work() -> Result<()>
{
    ...
    let current = decode(&fortress_observe(s.handle()))?;
    assert_eq!(current["ok"], true);
    assert_eq!(ask(&s, allocation(1, 8))?["model_feasible"], false);
    let w = watch(&s)?;
    let work = ask(&s, json!({"kind":"watches"}))?;
    let history = ask(&s, archived(&record, allocation(1, 8)))?;
    assert_eq!(history["ok"], true, "{history}");
    assert_eq!(history["model_feasible"], true);
    assert_eq!(history["anchor"], record["anchor"]);
    assert_eq!(history["current_live_anchor"], current["anchor"]);
    assert_eq!(history["source_digest"], record["source_digest"]);
    assert_eq!(
        history["agent_turn"]["coverage"]["current_freshness_proven"],
        false
    );
    assert_eq!(
        history["agent_turn"]["briefing"]["active_work_basis"],
        "current_session_not_archived"
    );
    let drill = history["rows"][0]["route_query"].clone();
    assert_eq!(drill["query"]["kind"], "historical_query");
    assert_eq!(drill["query"]["record_digest"], record["record_digest"]);
    let route = decode(&fortress_query(s.handle(), None, Some(drill)))?;
    assert_eq!(route["ok"], true, "{route}");
    assert_eq!(route["status"], "candidate_found");
    assert_eq!(route["anchor"], record["anchor"]);
    assert_eq!(route["source_digest"], record["source_digest"]);
    assert_eq!(route["unit_path_proven"], false);
    assert_eq!(s.calls.load(Ordering::SeqCst), 1);
    let after = ask(&s, json!({"kind":"watches"}))?;
    assert_eq!(after["rows"], work["rows"]);
    assert_eq!(
        after["agent_turn"]["active_work"],
        work["agent_turn"]["active_work"]
    );
...
fn reopened_archive_restores_source_history_not_session_authority() -> Result<()> {
    ...
    let (record, expected, source, old_handle) = {
        ...
        let source = session
            ...
            .observation()
            .cloned()
            .ok_or_else(|| error(ErrorCode::InternalInvariantViolation, "test source"))?;
    ...
    assert_eq!(previous["model_feasible"], true);
    assert_eq!(previous["anchor"], record["anchor"]);
    assert_eq!(s.calls.load(Ordering::SeqCst), 0);
    assert_eq!(
        previous["agent_turn"]["active_work"]["obligations"],
        json!([])
    );
    Ok(())
}
...
fn changed_archive_fences_refresh_without_publishing_the_new_world() -> Result<()> {
    ...
    let mut file = OpenOptions::new()
        ...
        .open(&archive.path)
        .map_err(io_error)?;
    file.write_all(b"D").map_err(io_error)?;
    ...
    assert_eq!(
        ask(&s, archived(&record, allocation(1, 8)))?["error"]["code"],
        "corrupt_ledger"
    );
    Ok(())
}
...
fn historical_allocation_paginates_at_8192_bytes_with_current_work_and_exact_drills() -> Result<()>
{
    ...
    loop {
        ...
        assert_eq!(page["anchor"], record["anchor"]);
        assert_eq!(
            page["agent_turn"]["active_work"]["obligations"]
                .as_array()
                .map(Vec::len),
            Some(1)
        );
        let rows = page["rows"]
            .as_array()
            .ok_or_else(|| error(ErrorCode::InternalInvariantViolation, "archive rows"))?;
        assert!(!rows.is_empty());
        for row in rows {
            ids.push(
                row["item"]["entity_id"]
                    .as_str()
                    .ok_or_else(|| error(ErrorCode::InternalInvariantViolation, "archive item"))?
                    .to_owned(),
            );
            assert_eq!(
                row["route_query"]["query"]["record_digest"],
    ...
    assert_eq!(
        ids,
        (100..140)
            .map(|id| item_entity_id(id).to_string())
            .collect::<Vec<_>>()
    );
    assert_eq!(s.calls.load(Ordering::SeqCst), 0);
    release_watch(&s, w)?;
...
fn discovery_and_historical_requests_reject_wrong_record_scope_and_stateful_work() -> Result<()> {
    ...
    for kind in [
        ...
        assert_eq!(
            ask(&s, archived(&record, json!({"kind":kind})))?["error"]["code"],
            "invalid_request"
        );
    }
    let mut bad = archived(&record, allocation(1, 8));
    bad["record_digest"] = json!("0".repeat(64));
    assert_eq!(ask(&s, bad)?["error"]["code"], "stale_anchor");
    assert_eq!(
        ask(&s, json!({"kind":"history","path":"/tmp/forbidden"}))?["error"]["code"],
        "invalid_request"
    );
    assert!(allowed_environment("DFMCP_SPATIAL_JOURNAL"));
    assert!(allowed_environment("DFMCP_SPATIAL_JOURNAL_REPAIR"));
    ...
    assert!(!allowed_environment("DFMCP_ADMITTED_BRIDGE_PROTOCOL"));
    {
        ...
        for grant in &mut session.grants {
            grant.expires_at_tick = Some(dfmcp_core::GameTick(now.0 - 1));
        }
    }
    assert_eq!(
        ask(&s, archived(&record, allocation(1, 8)))?["error"]["code"],
