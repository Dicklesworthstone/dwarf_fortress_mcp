...
pub(super) fn tactical(s: &Session, c: &OperationContext) -> Result<QueryResponseProjection> {
    ...
    let groups = summary["attention_groups"].as_u64().ok_or_else(|| {
        error(
            ErrorCode::InternalInvariantViolation,
            "situation group count absent",
        )
    })?;
    let unknown = summary["signals"]
        .as_object()
        .ok_or_else(|| {
            error(
                ErrorCode::InternalInvariantViolation,
                "situation signal counts absent",
            )
        })?
        .values()
        .filter(|signal| signal["unestablished"].as_u64().is_some_and(|n| n > 0))
        .count();
    ...
    projection
        ...
        .ok_or_else(|| {
            error(
                ...
                "watch metadata leaves no result allowance",
            )
        })
}
...
pub(super) fn detail(s: &Session, c: &OperationContext) -> Result<String> {
    ...
    if s.source.archive_only() {
        return Err(error(
            ...
            "live situation mode is unavailable in an archive session",
        ));
    }
    if s.source.poisoned() {
        return Err(error(
            ErrorCode::AdapterUnavailable,
            "situation source is fenced; no current operational finding is published",
        ));
    }
    semantic_query::publish_with_active_work(
        c,
        json!({"ok":true,"kind":"situation","native_captures":0,
