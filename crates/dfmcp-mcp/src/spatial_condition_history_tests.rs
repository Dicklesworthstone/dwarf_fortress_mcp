...
fn stock(s: &Registered) -> Result<u64> {
    ok(ask(s,measure())?)["quantity"]["quantity_min"].as_u64().ok_or_else(|| invalid("stock fixture"))
}
fn condition_timeline(s: &Registered, measurement: Value) -> Result<Value> {
    let mut query=timeline(s)?;query["measurement"]=measurement;Ok(query)
}
...
fn compound_condition_history_matches_each_exact_inspection_without_changing_work() -> Result<()> {
    ...
    let measurement=predicate(stock(&s)?);watch(&s)?;
    ...
    let rows=result["rows"].as_array().ok_or_else(||invalid("condition rows"))?;
    assert_eq!(rows.len(),3);
    assert_eq!(rows.iter().map(|r|r["evaluation"]["status"].as_str().unwrap_or("")).collect::<Vec<_>>(),
    ...
    assert_eq!(rows[1]["change_from_previous"]["status"],"evaluation_classification_changed");
...
fn paged_condition_transitions_match_one_page_and_bind_the_failure_guard() -> Result<()> {
    let _serial=lock(&SERIAL)?;let files=Files::new()?;let s=register(&files,&[(4,7),(5,12)])?;
    let measurement=predicate(stock(&s)?);watch(&s)?;capture(&s)?;capture(&s)?;
    ...
    query["limit"]=json!(1);let mut rows=Vec::new();let mut pages=0;let mut first_token=Value::Null;
    loop {
        ...
        let samples=result["rows"].as_array().ok_or_else(||invalid("paged conditions"))?;
        assert!(!samples.is_empty());rows.extend(samples.iter().cloned());pages+=1;assert!(pages<=3);
    ...
    assert_eq!(Value::Array(rows),complete["rows"]);assert!(pages>1);
...
fn missing_and_recycled_entities_never_become_successful_historical_samples() -> Result<()> {
        ...
        "comparison":"eq","value":{"type":"bool","value":true}}});
    {
        ...
        session.source=Box::new(Script {calls:s.calls.clone(),fenced:false,values:vec![
            fixture::observation(4,0,0,true)?,fixture::observation(5,1,10,false)?].into()});
    }
    capture(&s)?;capture(&s)?;
    ...
    assert_eq!(result["rows"][2]["evaluation"]["status"],"invalidated_reference");
    assert_eq!(result["rows"][2]["evaluation"]["eligible_success_sample"],false);
    Ok(())
}
...
fn failed_source_and_offline_reopening_preserve_condition_evidence_not_current_claims() -> Result<()> {
    ...
    let measurement=predicate(stock(&s)?);capture(&s)?;
    let query=condition_timeline(&s,measurement.clone())?;let original=ok(ask(&s,query.clone())?);
    let (limits,budget)={let h=resolve(s.handle())?;let mut session=lock(&h)?;
        session.source.fence();(session.limits,session.budget)};
    assert_eq!(ask(&s,measurement.clone())?["error"]["code"],"adapter_unavailable");
    ...
    assert_eq!(result["archive_only"],true);assert_eq!(result["current_freshness_proven"],false);
    let latest=ok(ask(&s,measurement)?);assert_eq!(latest["historical"],true);
    assert_eq!(latest["evaluation"],result["rows"][1]["evaluation"]);
    assert_eq!(latest["bridge_connection_present"],false);
    assert_eq!(ask(&s,json!({"kind":"watch","key":"not-permitted"}))?["error"]["code"],"capability_denied");
...
fn reset_boundaries_and_unknown_guards_never_establish_continuous_completion() -> Result<()> {
    ...
    assert_eq!(result["rows"][1]["change_from_previous"]["status"],"same_evaluation_classification");
    assert_eq!(result["rows"][1]["change_from_previous"]["elapsed_game_ticks"],0);
    assert_eq!(result["rows"][2]["change_from_previous"]["status"],"epoch_or_clock_discontinuity");
    assert!(result["rows"][2]["change_from_previous"].get("condition_truth").is_none());
    Ok(())
}
...
fn malformed_conditions_output_refusal_and_current_expiry_leave_saved_work_untouched() -> Result<()> {
    let _serial=lock(&SERIAL)?;let files=Files::new()?;let s=register(&files,&[(4,7)])?;
    let measurement=predicate(stock(&s)?);watch(&s)?;capture(&s)?;
    let query=condition_timeline(&s,measurement)?;
    let observations=fs::read(&files.observations).map_err(io_error)?;
    let watches=fs::read(&files.watches).map_err(io_error)?;
    let mut bad=query.clone();bad["measurement"]["condition"]=json!({"op":"all","args":[]});
    ...
    {let h=resolve(s.handle())?;let mut session=lock(&h)?;session.budget.max_output_tokens=65536;
        for g in &mut session.grants {g.expires_at_tick=Some(GameTick(105u64*403200+3));}}
