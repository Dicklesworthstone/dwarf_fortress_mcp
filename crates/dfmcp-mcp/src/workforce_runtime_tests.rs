...
impl Source for Script {
    fn read(&mut self, _: Duration) -> Result<LiveSpatialCitizenObservation> {
        ...
        self.values
            .pop_front()
            .ok_or_else(|| error(ErrorCode::AdapterUnavailable, "workforce fixture exhausted"))
    }
...
fn observation_scored(
    tick: u32,
    workers: u32,
    ratings: Option<&[(u32, u32)]>,
    ...
    let mut terrain = original.terrain().clone();
    ...
    for i in 0..workers {
        ...
        for (id, skill) in skills.iter().enumerate() {
            put(&mut citizens, id as u32);
            text(&mut citizens, skill);
            let (nominal, effective) = ratings
                .and_then(|r| r.get(i as usize))
                .copied()
                .unwrap_or((5, 5));
            for n in [nominal, effective, 1] {
                put(&mut citizens, n);
            }
        }
    }
    let mut all = b"DFMS1800".to_vec();
    part(&mut all, &spatial);
    part(&mut all, &citizens);
...
fn register(workers: u32, tokens: u32) -> Result<Registered> {
    ...
    let anchor = state
        .snapshot()
        .ok_or_else(|| error(ErrorCode::InternalInvariantViolation, "fixture state"))?
        .anchor();
    ...
    let session = Session {
        ...
        source: Box::new(Script {
            values: VecDeque::from([observation(4, workers)?]),
            calls: calls.clone(),
            fenced: false,
        }),
        state,
...
fn decode(text: &str) -> Result<Value> {
    serde_json::from_str(text)
        .map_err(|_| error(ErrorCode::InternalInvariantViolation, "handler JSON"))
}
...
fn planned(workers: u32) -> Value {
    json!({"kind":"workforce_plan","demands":[demand("wood", "CARPENTRY", workers)],"limit":128})
}
...
fn registered_queries_discover_skills_plan_without_conflicts_and_drill_into_routes() -> Result<()> {
    ...
    for name in ["workforce_candidates", "workforce_plan"] {
        assert!(
            variants
                .iter()
                .any(|v| v["properties"]["kind"]["const"] == name)
        );
    }
    let candidates = ask(
        &s,
        json!({"kind":"workforce_candidates","target":[0,0,5],"skill_key":"MINING","min_effective_skill":1}),
    )?;
    assert_eq!(candidates["ok"], true, "{candidates}");
    assert_eq!(candidates["total_rows"], 1);
    ...
    assert_eq!(result["model_feasible"], true);
    assert_eq!(result["cut_capacity"], 2);
    assert_ne!(
        result["rows"][0]["citizen"]["entity_id"],
        result["rows"][1]["citizen"]["entity_id"]
    );
    let route = decode(&fortress_query(
        s.handle(),
        None,
        Some(result["rows"][0]["route_query"].clone()),
    ))?;
...
fn workforce_pages_fit_8192_bytes_keep_watches_and_reject_stale_capture_tokens() -> Result<()> {
    ...
    loop {
        ...
        assert_eq!(page["model_feasible"], true);
        assert_eq!(
        ...
        for row in page["rows"]
            .as_array()
            .ok_or_else(|| error(ErrorCode::InternalInvariantViolation, "workforce rows"))?
        {
            assert!(
                ids.insert(
                    row["citizen"]["entity_id"]
                        .as_str()
                        .ok_or_else(|| error(ErrorCode::InternalInvariantViolation, "worker ID"))?
                        .to_owned()
                )
            );
        }
...
fn workforce_query_preserves_authority_fencing_and_invalid_input_refusal() -> Result<()> {
    ...
    let mut too_much = planned(129);
    too_much["max_work"] = json!(1);
