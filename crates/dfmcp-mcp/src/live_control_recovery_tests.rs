...
fn io_error(_: std::io::Error) -> DfmcpError {
    err(ErrorCode::CorruptLedger, "recovery test filesystem error")
}
...
impl Fixture {
    fn new() -> Result<Self> {
        ...
        let directory = std::env::temp_dir()
            ...
            .map_err(io_error)?
            .join(format!(
                "dfmcp-mcp-control-recovery-{}-{serial}",
                std::process::id()
            ));
        fs::DirBuilder::new()
            .mode(0o700)
        ...
        let mut journal =
            open_private_control_journal(&fixture.path, &context(), 7, EffectTailRecovery::Refuse)?;
...
fn offline_recovery_tools_discover_evidence_without_a_bridge_or_mutations() -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        let first = parse(fortress_query(
            Some(id.to_string()),
            None,
            Some(1),
            None,
        ...
        assert_eq!(
            first["agent_turn"]["briefing"]["development_mutation_enabled"],
            false
        );
        let token = first["result"]["continuation"]
            ...
            .to_owned();
        let second = parse(fortress_query(
            Some(id.to_string()),
            None,
            Some(1),
            Some(token),
        ...
        assert_eq!(
            second["result"]["effects"][0]["idempotency_key"],
            "prepared"
        );
        let explanation = parse(fortress_explain(
            Some(id.to_string()),
            "started".into(),
        ...
        assert_eq!(
            explanation["result"]["effect"]["safe_to_retry_same_effect"],
            false
        );
        let planned = parse(fortress_plan(
            Some(id.to_string()),
            "prepared".into(),
        ...
        assert_eq!(
            planned["result"]["error"]["code"],
            ErrorCode::CapabilityDenied.as_str()
        );
        let committed = parse(fortress_commit(
            Some(id.to_string()),
            "prepared".into(),
            Digest32::of_bytes(b"prepared").to_string(),
            "01".repeat(16),
        ...
        assert_eq!(
            committed["result"]["error"]["code"],
            ErrorCode::CapabilityDenied.as_str()
        );
        let cancelled = parse(fortress_cancel(
            Some(id.to_string()),
            Some("prepared".into()),
        ...
        assert_eq!(
            cancelled["result"]["error"]["code"],
            ErrorCode::CapabilityDenied.as_str()
        );
        let waited = parse(fortress_wait(
            Some(id.to_string()),
            vec!["started".into()],
            None,
            None,
        ...
        assert!(
            lock(&handle)?
                .as_ref()
                .is_some_and(|session| session.connection.is_none())
        );
        Ok(())
    })();
    ...
    let mut reopened = configured_session(
        next_id()?,
        &fixture.path,
        EffectTailRecovery::Refuse,
        WorkBudget::default(),
    ...
    assert_eq!(
        reopened.journal.lookup("started").map(|r| r.state),
...
fn recovery_session_rejects_repair_and_missing_journals_before_any_connection() -> Result<()> {
    ...
    let original = fs::read(&fixture.path).map_err(io_error)?;
    assert!(
        matches!(configured_session(next_id()?, &fixture.path, EffectTailRecovery::TruncateIncomplete,
...
fn recovery_session_cannot_dispatch_even_if_a_caller_injects_clock_grants() -> Result<()> {
    ...
    assert!(
        matches!(session.journal.begin_commit("prepared", Digest32::of_bytes(b"prepared"), 7, &context),
        Err(e) if e.code == ErrorCode::CapabilityDenied)
    );
    assert!(
        matches!(session.journal.cancel_prepared("prepared", Digest32::of_bytes(b"prepared"), &context),
        Err(e) if e.code == ErrorCode::CapabilityDenied)
    );
    assert!(
        matches!(reconciliation::wait(&mut session, context, vec!["started".into()], None, None, None),
        Err(e) if e.code == ErrorCode::CapabilityDenied)
    );
    assert!(session.connection.is_none());
    assert_eq!(fs::read(&fixture.path).map_err(io_error)?, original);
