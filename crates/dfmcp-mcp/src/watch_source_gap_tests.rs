...
fn context(snapshot: &WorldSnapshot) -> OperationContext {
    OperationContext {
        session_id: SessionId::new(7_181_991),
...
fn register(store: &Mutex<Store>, snapshot: &WorldSnapshot, stable: u32) -> Result<Value> {
    call(
        store,
        snapshot,
        json!({"schema":"dfmcp.query/1","query":{"kind":"watch",
...
fn call(store: &Mutex<Store>, snapshot: &WorldSnapshot, input: Value) -> Result<Value> {
    let encoded = watches::execute_in(store, snapshot, &context(snapshot), &input, |v| {
        Ok(v.to_string())
    })?;
...
fn interruption_preserves_intent_but_requires_a_new_success_streak() -> Result<()> {
    ...
    let created = register(&store, &first, 2)?;
    ...
    assert_eq!(info["changed_watches"], 1);
    assert_eq!(info["samples_added"], 0);
    ...
    assert_eq!(
        same["record"]["definition"],
        created["record"]["definition"]
    );
    assert_eq!(
        same["record"]["created_at"],
    ...
    assert_eq!(
        poll(&store, &snapshot(3, 1), handle)?["record"]["status"],
        "satisfied"
    );
    Ok(())
}
...
fn repeated_failed_recovery_does_not_rewrite_the_same_gap() -> Result<()> {
    ...
    let previous = evidence(&store)?;
    let again = interrupt_in(&store, &s, &context(&s), |v| Ok(v.to_string()))?;
    assert_eq!(again["changed_watches"], 0);
    assert_eq!(again["pending_watches"], 1);
    assert_eq!(evidence(&store)?, previous);
    Ok(())
}
...
fn terminal_outcomes_are_not_restarted_or_resealed() -> Result<()> {
    ...
    assert_eq!(info["changed_watches"], 0);
    assert_eq!(info["terminal_watches"], 1);
    assert_eq!(info["pending_watches"], 0);
    assert_eq!(before, evidence(&store)?);
    Ok(())
}
...
fn expired_and_cross_epoch_intents_cannot_be_revived() -> Result<()> {
    for (target, status) in [
        (snapshot(100, 1), "expired"),
        (snapshot(2, 2), "invalidated"),
        (snapshot(0, 1), "invalidated"),
    ] {
        let store = Mutex::new(Store::default());
        let s = snapshot(1, 1);
        let created = register(&store, &s, 2)?;
...
fn permission_cancellation_budget_and_output_refusal_leave_watches_unchanged() -> Result<()> {
    for case in 0..6 {
        ...
        assert!(
            interrupt_in(&store, &s, &c, |v| if case == 5 {
...
fn source_gap_checkpoint_keeps_serialized_watch_identity_and_counters() -> Result<()> {
    ...
    let saved = SavedSet::capture(&guard, &context(&s));
    drop(guard);
    ...
    assert_eq!(decoded.watches[0].samples, 1);
    assert_eq!(decoded.watches[0].streak, 0);
    assert_eq!(decoded.watches[0].status, "blocked_unknown");
    Ok(())
}
