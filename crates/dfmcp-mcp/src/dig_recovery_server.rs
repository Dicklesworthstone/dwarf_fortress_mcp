...
use dfmcp_adapter::dig_designation::journal::{
    DigBinding, DigCursor, DigMode, MAX_FRAME_BYTES, MAX_JOURNAL_BYTES,
};
...
use runtime::{Config, QueryOnly, RequestControl};
...
fn exhausted() -> dfmcp_core::DfmcpError {
    error(
        ...
        "complete recovery work and response exceed the allowance",
    )
}
fn unbound(op: &str, e: &dfmcp_core::DfmcpError) -> String {
    packet(op, failure(e), None, None, None, None)
}
...
impl Query {
    fn parse(raw: &str) -> Result<Self> {
        if raw.len() > 2048 {
            return Err(error(ErrorCode::InvalidRequest, "query exceeds 2 KiB"));
        }
        let query: Self = serde_json::from_str(raw).map_err(|_| {
            error(
                ErrorCode::InvalidRequest,
                "query must match the closed mining recovery schema",
            )
        })?;
        match &query {
            Self::Records {
        ...
        self.entries
            ...
            .ok_or_else(|| {
                error(
                    ...
                    "recovery continuation expired; restart discovery",
                )
            })
    }
}
    ...
    fn new(control: DigSession<S, N>, c: &OperationContext) -> Result<Self> {
        if control.mode() == DigMode::Control {
            return Err(error(
                ...
                "recovery server refuses Control journals",
            ));
        }
        Ok(Self {
            id: c.session_id,
...
impl Work {
    ...
    fn current(&self, c: &OperationContext) -> Result<OperationContext> {
        ...
        let mut out = c.clone();
        out.budget.max_wall_millis = u64::try_from(ms).map_err(|_| exhausted())?;
...
where
    ...
    F: FnOnce(
        &DigBinding,
        dfmcp_adapter::dig_designation::DigRegion,
        &OperationContext,
    ) -> Result<N>,
{
    ...
    match action {
        ...
        Action::Denied => Err(error(
            ErrorCode::CapabilityDenied,
            "mining recovery cannot create or mutate game state",
        )),
        ...
        Action::Query(Query::Schema) => {
            ...
            .map_err(|_| {
                error(
                    ...
                    "invalid embedded recovery schema",
                )
            })?;
        ...
        Action::Query(Query::Records {
            ...
            let page = state
                .control
                .list(&work.take(c, LOCAL_BYTES)?, limit, cursor.as_ref())?;
            let next = page
                .continuation
                .map(|p| cursors.issue(p, c, page.head))
                .transpose()?;
            Ok(
                json!({"ok":true,"journal_id":page.journal_id.to_string(),"head":page.head.to_string(),
        ...
        Action::Query(Query::Tiles {
            ...
            let r = state.control.get(
                &idempotency_key,
                digest(&plan_digest)?,
                &work.take(c, LOCAL_BYTES)?,
            )?;
            Ok(
                json!({"ok":true,"capture":presentation::tiles(&r,offset,limit.map_or(16,|n|n))?,"native_calls":0}),
            )
        }
        Action::Wait(key, plan) => {
            ...
            let reserved = 36 * (view.byte_len + 2 * MAX_FRAME_BYTES) as u64
            ...
            let r =
                state
                    .control
                    .reconcile(&key, plan, &work.take(c, reserved)?, factory, guard)?;
            Ok(
                json!({"ok":true,"record":presentation::record(&r),"query_attempted":true,
                "commit_attempted":false,"historical_evidence_only":true}),
            )
        }
    }
}
fn run_action<S, N, F, G>(
    state: &mut State<S, N>,
    c: OperationContext,
    started: Instant,
    op: &str,
    ...
    ) -> Result<N>,
{
    ...
    let mut work = match Work::new(&c, started) {
        Ok(w) => w,
        Err(e) => return packet(op, failure(&e), Some(&c), Some(mode), None, None),
    };
    ...
    let (value, view) = match after {
        ...
        Err(e) => {
            ...
            failed["pending_at_request_start"] = before
                .pending
                .as_ref()
                .map_or(Value::Null, presentation::summary);
...
fn lock() -> Result<MutexGuard<'static, Option<Entry>>> {
    match SESSION.try_lock() {
        Ok(v) => Ok(v),
        Err(TryLockError::WouldBlock) => Err(exhausted()),
        Err(TryLockError::Poisoned(_)) => Err(error(
            ErrorCode::InternalInvariantViolation,
            "mining recovery session poisoned",
        )),
    }
}
fn session_id(raw: &str) -> Result<SessionId> {
            ...
            .all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b))
    {
        return Err(error(
            ErrorCode::InvalidRequest,
            "invalid mining recovery session ID",
        ));
    }
    let value = u128::from_str_radix(raw, 16)
        .map_err(|_| error(ErrorCode::InvalidRequest, "invalid mining recovery ID"))?;
    let id = SessionId::new(value);
    if id.get() != value
        || !id.is_process_scoped_live()
        || (value & ((1u128 << 62) - 1)) >> 57 != 16
    {
        return Err(error(
            ErrorCode::InvalidRequest,
            "not a mining recovery session ID",
        ));
    }
    Ok(id)
}
fn with_session(
    ...
    let result = (|| -> Result<String> {
        ...
        let entry = locked
            ...
            .ok_or_else(|| error(ErrorCode::SessionNotFound, "mining recovery session absent"))?;
        ...
        let snapshots = entry.goals.load(&c, &binding, control.started, || {
        ...
        let output = run_action(
            ...
            op,
            action,
            |b, r, c| runtime::connect(&control, &config, b, r, c),
            &mut guard,
        );
        ...
        if rendered["result"]["ok"] == true {
            entry.state.cursors = staged_cursors;
        }
        Ok(output)
    })();
    match result {
...
fn open(
    ...
    let result = (|| -> Result<String> {
        ...
        let budget = WorkBudget {
            max_wall_millis: wall.map_or(10_000, |n| n),
            max_bytes: bytes.map_or(MAX_WORK_BYTES, |n| n),
            max_output_tokens: tokens.map_or(8192, |n| n),
            max_entities: 1000,
            max_game_ticks: 0,
            max_actions: 1,
        };
        ...
        if locked.is_some() {
            return Err(error(
                ErrorCode::Conflict,
                "release the current recovery session first",
            ));
        }
        let next = NEXT
        ...
        let inspected = open_private_dig(
            &config.path,
            &work.take(&c, OPEN_BYTES)?,
            DigMode::Offline,
            None,
        )?;
        config.matches(inspected.binding())?;
        let journal = if config.mode() == DigMode::Recover {
            let expected = inspected.binding().clone();
            let id = inspected.id();
            let head = inspected.head();
            drop(inspected);
            runtime::boundary(&control, &config)?;
            let reopened = open_private_dig(
                &config.path,
                &work.take(&c, OPEN_BYTES)?,
                DigMode::Recover,
                Some(expected),
            )?;
            if reopened.id() != id || reopened.head() != head {
                return Err(error(
                    ErrorCode::Conflict,
                    "mining history changed during recovery open",
                ));
            }
            reopened
        } else {
        ...
        let snapshots = goals.load(&c, &state.binding, control.started, || {
            runtime::boundary(&control, &config)
        })?;
        goals.narrow(&mut c);
        ...
        let output = packet(
            ...
            "native_calls":0,"mutation_admissible":false}),
            Some(&c),
            Some(config.mode()),
            Some(&state.binding),
            Some(&view),
    ...
    match result {
        Ok(s) => s,
        Err(e) => unbound("fortress.open_session", &e),
    }
}
fn release(control: RequestControl, raw: String, force: bool) -> String {
    let result = (|| -> Result<String> {
        ...
        let view = if force {
            ...
            if view.pending.is_some() {
                return Err(error(
                    ...
                    "unsettled mining requires explicit release_for_recovery",
                ));
            }
            Some(view)
        };
        let output = packet(
            ...
            "quiescence_of_game_effects_proven":false}),
            Some(&c),
            Some(entry.state.control.mode()),
...
#[tool(
    ...
    description = "Open an existing operator-selected dig/1.16 journal for offline history or operator-enabled query-only recovery. No credentials or native connection are needed to open. Never creates a journal, grants Designate, or restores a commit permit."
)]
pub async fn fortress_open_session(
    ...
    runtime::owned("fortress.open_session", move |c| {
        open(c, max_wall_millis, max_bytes, max_output_tokens)
    })
    .await
}
#[tool(
    name = "fortress.observe",
    description = "Orient to verified mining journal inventory, not live terrain. Includes any unsettled key even when it is not on the first records page. Optional operator-selected excavation journals are also replayed into active work. Zero native calls."
)]
pub async fn fortress_observe(session_id: String) -> String {
    runtime::owned("fortress.observe", move |c| {
        with_session(
            c,
            session_id,
            "fortress.observe",
...
#[tool(
    name = "fortress.query",
    description = "Closed JSON query: records with limit 1..8 and continuation; tiles with exact idempotency_key, plan_digest, offset and limit 1..16; or schema. Historical evidence only. No native calls or renewed mutation authority."
)]
pub async fn fortress_query(session_id: String, query: String) -> String {
    runtime::owned("fortress.query", move |c| {
        with_session(
            c,
            session_id,
            "fortress.query",
...
#[tool(
    ...
    description = "Inspect an exact retained mining plan, scope, hidden-neighbor policy and native receipt. This is historical configuration evidence, never completed excavation or present terrain."
)]
pub async fn fortress_explain(
    ...
    runtime::owned("fortress.explain", move |c| {
        with_session(
            c,
            session_id,
            "fortress.explain",
            None,
            identity(idempotency_key, plan_digest).map(|(k, p)| Action::Explain(k, p)),
        )
    })
    .await
}
...
pub async fn fortress_wait(
    ...
    runtime::owned("fortress.wait", move |c| {
        with_session(
            c,
            session_id,
            "fortress.wait",
            max_wall_millis,
            identity(idempotency_key, plan_digest).map(|(k, p)| Action::Wait(k, p)),
        )
    })
    .await
}
#[tool(
    ...
    description = "Release only this MCP session and its journal lock. Unsettled or fenced history needs explicit release_for_recovery=true. Does not cancel a native effect, undo designation, erase history or prove game quiescence."
)]
pub async fn fortress_cancel(session_id: String, release_for_recovery: Option<bool>) -> String {
    runtime::owned("fortress.cancel", move |c| {
        release(c, session_id, release_for_recovery == Some(true))
    })
    .await
}
#[tool(
    name = "fortress.doctor",
    description = "Verify mining journal custody and report complete local coordination inventory. No live game health, mining safety or compatibility admission is inferred."
)]
pub async fn fortress_doctor(session_id: String) -> String {
    runtime::owned("fortress.doctor", move |c| {
        with_session(
            c,
            session_id,
            "fortress.doctor",
...
#[tool(
    name = "fortress.plan",
    description = "Unavailable in the query-only mining recovery profile. Retained plans cannot create new designation authority."
)]
pub async fn fortress_plan(session_id: String) -> String {
    runtime::owned("fortress.plan", move |c| {
        with_session(c, session_id, "fortress.plan", None, Ok(Action::Denied))
    })
    .await
}
#[tool(
    name = "fortress.commit",
    description = "Unavailable. This recovery profile cannot prepare, commit or replay a mining designation, even with a valid historical digest."
)]
pub async fn fortress_commit(session_id: String) -> String {
    runtime::owned("fortress.commit", move |c| {
        with_session(c, session_id, "fortress.commit", None, Ok(Action::Denied))
    })
    .await
}
#[tool(
    name = "fortress.checkpoint",
    description = "Unavailable. A mining journal is not a fortress save or game checkpoint."
)]
pub async fn fortress_checkpoint(session_id: String) -> String {
    runtime::owned("fortress.checkpoint", move |c| {
        with_session(
            c,
            session_id,
            "fortress.checkpoint",
...
#[tool(
    ...
    description = "Unavailable. Mining receipt recovery neither restores the game nor undoes a designation."
)]
pub async fn fortress_restore(session_id: String) -> String {
    runtime::owned("fortress.restore", move |c| {
        with_session(c, session_id, "fortress.restore", None, Ok(Action::Denied))
    })
    .await
}
pub fn run_stdio() {
