...
fn io_error(_: io::Error) -> DfmcpError {
    corrupt("watch journal I/O failed; reopen for verified recovery")
}
...
fn get_digest(bytes: &[u8]) -> Result<Digest32> {
    let value: [u8; 32] = bytes
        .try_into()
        .map_err(|_| corrupt("watch journal digest length"))?;
...
fn get_u64(bytes: &[u8]) -> Result<u64> {
    Ok(u64::from_be_bytes(
        bytes
            .try_into()
            .map_err(|_| corrupt("watch journal integer length"))?,
    ))
}
...
impl Pending {
    pub(super) fn head(&self) -> Digest32 {
        self.head
    }
...
impl<S: JournalStorage> Journal<S> {
        ...
        F: FnMut(&[u8]) -> Result<()>,
    {
        ...
        if binding == Digest32::ZERO {
            return Err(corrupt("watch journal has no observation binding"));
        }
        storage.validate_identity().map_err(io_error)?;
        let mut end = storage.seek(SeekFrom::End(0)).map_err(io_error)?;
        if end > MAX_BYTES {
            return Err(budget("watch journal exceeds 64 MiB"));
        }
        if created && end == 0 {
            let id = hash(
                b"dfmcp-watch-journal-incarnation/1\0",
                &[
                    binding.as_bytes(),
                    &context.session_id.get().to_be_bytes(),
                    &context.request_id.get().to_be_bytes(),
                    context.anchor.state_hash.as_bytes(),
                ],
            );
        ...
        if end < HEADER as u64 {
            return Err(corrupt("watch journal header incomplete; bytes preserved"));
        }
        ...
        if &header[..8] != MAGIC
            || get_digest(&header[8..40])? != binding
            || head != hash(b"dfmcp-watch-journal-header/1\0", &[&header[..72]])
        {
            return Err(corrupt(
                "watch journal format, observation binding or header checksum differs",
            ));
        }
        let id = get_digest(&header[40..72])?;
        if id == Digest32::ZERO {
            return Err(corrupt("watch journal identity is zero"));
        }
        ...
        while journal.length < end {
            check(context, started)?;
            if journal.count >= MAX_RECORDS {
                return Err(budget("watch journal exceeds 4096 checkpoints"));
            }
            if end - journal.length < PREFIX as u64 {
                return Err(corrupt(
                    "incomplete watch checkpoint prefix; no repair performed",
                ));
            }
            ...
            if &tail[32..] != FOOTER
                || digest
                    != hash(
                        b"dfmcp-watch-journal-record/1\0",
    ...
    fn validate_prefix(&self, prefix: &[u8; PREFIX]) -> Result<usize> {
        if &prefix[..8] != RECORD
            || get_u64(&prefix[8..16])? != self.count + 1
            || get_digest(&prefix[16..48])? != self.head
            || get_digest(&prefix[52..])?
                != hash(
                    b"dfmcp-watch-journal-prefix/1\0",
                    &[self.id.as_bytes(), &prefix[..52]],
                )
        {
            return Err(corrupt(
                "watch checkpoint prefix, sequence or predecessor failed",
            ));
        }
        let size = u32::from_be_bytes(
            prefix[48..52]
                .try_into()
                .map_err(|_| corrupt("watch checkpoint length"))?,
        ) as usize;
        if !(1..=MAX_PAYLOAD).contains(&size) {
            return Err(budget("watch checkpoint exceeds 1 MiB"));
        }
        Ok(size)
    }
    ...
    pub(super) fn verify(&mut self, context: &OperationContext) -> Result<()> {
        ...
        if self.fenced {
            return Err(corrupt(
                "watch journal is fenced; reopen for verified recovery",
            ));
        }
    ...
    pub(super) fn stage(&self, payload: Vec<u8>, context: &OperationContext) -> Result<Pending> {
        ...
        check(context, started)?;
        if self.fenced {
            return Err(corrupt("watch journal is fenced"));
        }
        ...
        if length > MAX_BYTES {
            return Err(budget(
                "watch journal retention is full; no automatic pruning",
            ));
        }
        check(context, started)?;
        Ok(Pending {
    ...
    pub(super) fn commit(&mut self, pending: Pending, context: &OperationContext) -> Result<()> {
        ...
        if pending.previous != self.head || pending.number != self.count + 1 {
            return Err(corrupt(
                "watch checkpoint was staged against another journal head",
            ));
        }
        ...
        if result.is_err() {
            self.fenced = true;
            return Err(corrupt(
                "watch checkpoint durability is uncertain; in-memory change not published",
            ));
        }
    ...
    pub(super) fn latest(&self) -> &[u8] {
        &self.latest
    }
    pub(super) fn id(&self) -> Digest32 {
        self.id
    }
    pub(super) fn head(&self) -> Digest32 {
        self.head
...
mod tests {
    ...
    impl Write for Memory {
        fn write(&mut self, b: &[u8]) -> io::Result<usize> {
            if let Some(remaining) = self.write_limit {
                ...
                let count = remaining.min(b.len());
                self.write_limit = Some(remaining - count);
    ...
    fn context() -> OperationContext {
        OperationContext {
            ...
            anchor: StateAnchor {
                ...
                tick: GameTick(10),
                state_hash: Digest32::of_bytes(b"world"),
            },
            budget: WorkBudget {
    ...
    fn fresh() -> Result<Journal<Memory>> {
        Journal::open(Memory::default(), &context(), binding(), true, |_| Ok(()))
    }
    #[test]
    fn synced_checkpoints_reopen_with_exact_head_and_payload() -> Result<()> {
        let mut j = fresh()?;
        let first = j.stage(b"first".to_vec(), &context())?;
        j.commit(first, &context())?;
        let second = j.stage(b"second".to_vec(), &context())?;
        j.commit(second, &context())?;
        let head = j.head();
        let mut seen = Vec::new();
        let bytes = j.storage.bytes.into_inner();
        let reopened = Journal::open(memory(bytes), &context(), binding(), false, |p| {
            seen.push(p.to_vec());
            Ok(())
        })?;
        assert_eq!(seen, [b"first".to_vec(), b"second".to_vec()]);
        assert_eq!(reopened.latest(), b"second");
    ...
    fn every_incomplete_frame_prefix_is_refused_without_tail_repair() -> Result<()> {
        let mut j = fresh()?;
        let p = j.stage(b"complete".to_vec(), &context())?;
        j.commit(p, &context())?;
        let bytes = j.storage.bytes.into_inner();
        for end in HEADER + 1..bytes.len() {
            assert!(
                Journal::open(
                    memory(bytes[..end].to_vec()),
                    &context(),
                    binding(),
                    false,
                    |_| Ok(())
    ...
    fn every_single_byte_corruption_is_refused() -> Result<()> {
        let mut j = fresh()?;
        let p = j.stage(b"watch-state".to_vec(), &context())?;
        j.commit(p, &context())?;
        let bytes = j.storage.bytes.into_inner();
        for offset in 0..bytes.len() {
            ...
            bad[offset] ^= 1;
            assert!(
                Journal::open(memory(bad), &context(), binding(), false, |_| Ok(())).is_err(),
                "byte {offset}"
            );
        }
        Ok(())
    ...
    fn uncertain_sync_fences_but_restart_can_recover_complete_frame() -> Result<()> {
        let mut j = fresh()?;
        let old = j.head();
        let p = j.stage(b"new".to_vec(), &context())?;
        j.storage.fail_sync = true;
        assert!(j.commit(p, &context()).is_err());
        assert_eq!(j.head(), old);
        assert!(j.fenced);
        assert!(j.verify(&context()).is_err());
        let reopened = Journal::open(
            memory(j.storage.bytes.into_inner()),
            &context(),
            binding(),
            false,
            |_| Ok(()),
        )?;
        assert_eq!(reopened.latest(), b"new");
        Ok(())
    }
    #[test]
    ...
    fn wrong_archive_empty_existing_file_and_duplicate_frames_fail() -> Result<()> {
        assert!(
            Journal::open(Memory::default(), &context(), binding(), false, |_| Ok(())).is_err()
        );
        let mut j = fresh()?;
        let p = j.stage(b"one".to_vec(), &context())?;
        j.commit(p, &context())?;
        let bytes = j.storage.bytes.into_inner();
        assert!(
            Journal::open(
        ...
        duplicate.extend_from_slice(&bytes[HEADER..]);
        assert!(
            Journal::open(memory(duplicate), &context(), binding(), false, |_| Ok(())).is_err()
        );
        Ok(())
    }
    #[test]
    fn invalid_payload_authority_and_budget_refuse_before_publication() -> Result<()> {
        let mut j = fresh()?;
        assert!(j.stage(vec![0; MAX_PAYLOAD + 1], &context()).is_err());
        let mut c = context();
        c.cancellation_requested = true;
        assert!(j.stage(b"x".to_vec(), &c).is_err());
        c.cancellation_requested = false;
        c.grants.clear();
        assert!(j.verify(&c).is_err());
        let p = j.stage(b"bad-state".to_vec(), &context())?;
        j.commit(p, &context())?;
        assert!(
            Journal::open(
                memory(j.storage.bytes.into_inner()),
                ...
                binding(),
                false,
                |_| Err(corrupt("invalid semantic state"))
            )
            .is_err()
        );
        Ok(())
