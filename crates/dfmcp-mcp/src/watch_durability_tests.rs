...
fn io_error(_: std::io::Error) -> dfmcp_core::DfmcpError {
    corrupt("durable watch test filesystem error")
}
struct Files {
...
impl Files {
    fn new() -> Result<Self> {
        ...
        fs::DirBuilder::new()
            ...
            .map_err(io_error)?;
        Ok(Self {
            path: directory.join("watches.bin"),
            directory,
        })
    }
    fn bytes(&self) -> Result<Vec<u8>> {
        fs::read(&self.path).map_err(io_error)
    }
}
impl Drop for Files {
    fn drop(&mut self) {
...
fn archive() -> Digest32 {
    Digest32::of_bytes(b"exact retained spatial18 archive")
}
fn parse(bytes: &str) -> Result<Value> {
    serde_json::from_str(bytes).map_err(|_| invalid("test JSON"))
}
fn open(
    files: &Files,
    snapshot: &WorldSnapshot,
    history: &[StateAnchor],
) -> Result<(SessionId, Value, WatchJournalGuard)> {
    let session = SessionId::new(u128::from(next()));
    let c = context(snapshot, session);
    let (out, guard) = attach(
        snapshot,
        &c,
        &files.path,
        archive(),
...
fn run(snapshot: &WorldSnapshot, session: SessionId, request: Value) -> Result<Value> {
    let output = super::super::execute(
        ...
        &json!({"schema":"dfmcp.query/1","query":request}),
        |v| Ok(v.to_string()),
    )?;
    parse(&output)
}
fn create(stability: u32) -> Value {
    json!({"kind":"watch","key":"healthy","condition":{
...
fn handle(result: &Value) -> Result<String> {
    result["record"]["watch"]
        .as_str()
        .map(str::to_owned)
        .ok_or_else(|| invalid("test watch handle"))
}
fn listed_handle(result: &Value) -> Result<String> {
    result["records"][0]["watch"]
        .as_str()
        .map(str::to_owned)
        .ok_or_else(|| invalid("listed handle"))
}

#[test]
fn restart_restores_intent_but_requires_new_stability_evidence() -> Result<()> {
    let files = Files::new()?;
    let first = world(1, 0, true);
    let boot = world(2, 1, true);
    let (id, _, guard) = open(&files, &first, &[first.anchor()])?;
    let created = run(&first, id, create(2))?;
    let old = handle(&created)?;
    ...
    let listed = run(&boot, new_id, json!({"kind":"watches"}))?;
    let new = listed_handle(&listed)?;
    assert_ne!(new, old);
    assert_eq!(listed["records"][0]["status"], "blocked_unknown");
    ...
    assert_eq!(
        listed["_condition_watch_work"][0]["next_step"]["arguments"]["query"]["query"]["kind"],
        "await_watch"
    );
    ...
    assert_eq!(
        same["record"]["evaluation"]["reason"],
        "restart_gap_requires_fresh_observation"
    );
    let next = run(
        &world(3, 2, true),
        new_id,
        json!({"kind":"poll_watch","watch":new}),
    )?;
...
fn rendering_failure_and_identical_reads_never_append_or_publish() -> Result<()> {
    ...
    let failed = super::super::execute(
        &first,
        &context(&first, id),
        &json!({"schema":"dfmcp.query/1","query":create(2)}),
        |_| Err(bounded("injected full-packet overflow")),
    );
    assert!(failed.is_err());
    assert_eq!(files.bytes()?, before);
    assert_eq!(
        run(&first, id, json!({"kind":"watches"}))?["records"],
        json!([])
    );
    let created = run(&first, id, create(2))?;
    let watch = handle(&created)?;
    let saved = files.bytes()?;
    for _ in 0..4 {
        run(&first, id, create(2))?;
        run(&first, id, json!({"kind":"watches"}))?;
        run(&first, id, json!({"kind":"poll_watch","watch":watch}))?;
    }
    assert_eq!(files.bytes()?, saved);
    drop(guard);
    Ok(())
}
...
fn different_archive_or_missing_observation_proof_refuses_unchanged_file() -> Result<()> {
    ...
    drop(guard);
    let before = files.bytes()?;
    let c = context(&second, SessionId::new(u128::from(next())));
    assert!(
        attach(
            &second,
            &c,
            &files.path,
            Digest32::of_bytes(b"other archive"),
...
fn epoch_reset_invalidates_and_downtime_cannot_extend_deadlines() -> Result<()> {
    for epoch_reset in [true, false] {
        ...
        if epoch_reset {
            boot.cursor = ObservationCursor {
                epoch: 1,
                sequence: 0,
            };
            boot.tick = GameTick(0);
            boot.refresh_hash();
        }
        let (id, _, guard) = open(&files, &boot, &[first.anchor(), boot.anchor()])?;
        let list = run(&boot, id, json!({"kind":"watches"}))?;
        ...
        assert_eq!(list["records"][0]["stable_observations"], 0);
        drop(guard);
    }
    Ok(())
}
...
fn current_authority_and_custody_failures_do_not_acknowledge_watches() -> Result<()> {
    ...
    let (id, _, guard) = open(&files, &first, &[first.anchor()])?;
    let before = files.bytes()?;
    let mut c = context(&first, id);
    c.grants = vec![CapabilityGrant {
        capability: Capability::Query,
        scope: CapabilityScope::default(),
        max_risk: RiskTier::ReadOnly,
        expires_at_tick: Some(GameTick(0)),
        remaining_uses: None,
    }];
    assert!(
        super::super::execute(
    ...
    assert!(run(&first, id, create(2)).is_err());
    drop(guard);
    assert!(open(&files, &first, &[first.anchor()]).is_err());
    Ok(())
}
...
fn failed_recovery_render_leaves_no_session_or_persistent_rebind() -> Result<()> {
    ...
    assert!(
        attach(
            ...
            &[first.anchor(), second.anchor()],
            json!({}),
            |_| Err(bounded("injected open response overflow"))
        )
        .is_err()
    );
    assert_eq!(files.bytes()?, before);
    ...
    assert!(
        !super::super::lock(&WATCHES)?
            .entries
            .keys()
            .any(|(id, _)| *id == c.session_id)
    );
...
fn identical_bootstrap_does_not_relabel_old_terminal_evidence_as_fresh() -> Result<()> {
    ...
    assert_eq!(result["records"][0]["status"], "satisfied");
    drop(guard);
    Ok(())
}
...
fn recovery_cannot_restore_an_old_larger_time_horizon() -> Result<()> {
    ...
    drop(guard);
    let before = files.bytes()?;
    let mut c = context(&second, SessionId::new(u128::from(next())));
    c.budget.max_game_ticks = 10;
    assert!(
        matches!(attach(&second,&c,&files.path,archive(),&[first.anchor(),second.anchor()],json!({}),
...
fn invalid_evaluation_shapes_and_unbounded_json_are_rejected() -> Result<()> {
    ...
    let saved = journals()?
        ...
        .ok_or_else(|| invalid("missing saved fixture"))?;
    ...
    let mut bad = saved;
    bad.watches[0].definition.deadline_tick = 1;
    assert!(
        decode_saved(
            &serde_json::to_vec(&bad).map_err(|_| invalid("fixture serialization"))?,
            &anchors
        )
        .is_err()
    );
...
fn watch_persistence_never_claims_unrelated_baselines_are_durable() -> Result<()> {
    ...
    run(&first, id, create(2))?;
    let before = files.bytes()?;
    let result = super::super::with_active_work(
        &context(&first, id),
        json!({"kind":"capture","durable":false}),
        |v| Ok(v.to_string()),
    )?;
    let result = parse(&result)?;
    assert_eq!(result["durable"], false);
    assert_eq!(
        result["watch_persistence"]["scope"],
        "foreground_condition_watches"
    );
...
fn checkpoint_bytes_match_independent_python_vectors() -> Result<()> {
    ...
    let mut c = context(&world(1, 0, true), SessionId::new(7));
    c.request_id = RequestId::new(3);
    c.anchor.state_hash = Digest32::of_bytes(b"world");
    let mut journal = checkpoint::Journal::open(
        bytes.clone(),
        &c,
        Digest32::of_bytes(b"spatial/1.8 archive"),
        true,
        |_| Ok(()),
    )?;
    assert_eq!(
        journal.id().to_string(),
        "d94a9a394f027513b75f4f042667fe0123aa13de1e66afa1dd3b5c8c3c09edaf"
    );
    assert_eq!(
        journal.head().to_string(),
        "d3404626c30fe42c90c400a4cb31cd574e77077f29415421c73fa11ed651f518"
    );
    let first = journal.stage(b"first".to_vec(), &c)?;
    journal.commit(first, &c)?;
    assert_eq!(
        journal.head().to_string(),
        "06a925e21a650ae34c37cb00a6e3c2e414d468cf2476de849c14c2131aae4f80"
    );
    let second = journal.stage(b"second".to_vec(), &c)?;
    journal.commit(second, &c)?;
    assert_eq!(
        journal.head().to_string(),
        "6e557d1732e84b9fbb9eb3628bf0a1650edd0af766597a7415cb16b2502a4063"
    );
    assert_eq!(journal.count(), 2);
    assert_eq!(journal.retained_bytes(), 363);
    assert_eq!(
        Digest32::of_bytes(bytes.0.borrow().get_ref()).to_string(),
