...
fn state(stock: u32) -> Result<LiveSpatialCitizenState> {
    ...
    state.publish(fixture::observation(3, 2, stock, false)?)?;
    Ok(state)
}
fn context(state: &LiveSpatialCitizenState) -> Result<OperationContext> {
    let anchor = state
        .snapshot()
        .ok_or_else(|| invalid("fixture snapshot absent"))?
        .anchor();
    Ok(OperationContext {
        session_id: SessionId::new(191),
        request_id: RequestId::new(192),
        anchor,
        budget: WorkBudget {
...
fn multi_stage_chain_preserves_reserves_and_exposes_complete_deficits() -> Result<()> {
    ...
    assert_eq!(out["kind"], "production_chain");
    assert_eq!(out["anchor"], anchor_json(c.anchor));
    assert_eq!(out["model_feasible"], false);
    assert_eq!(out["observed_quotas_met"], false);
    assert_eq!(out["total_rows"], 8);
    let raw = row(&out, "resource", "raw")?;
    assert_eq!(raw["observed_stock"]["stack_units"], 7);
    assert_eq!(raw["observed_stock"]["eligible_items"], 1);
    assert_eq!(raw["observed_stock"]["examples"][0]["generation"], 1);
    assert_eq!(raw["modeled_balance"]["minimum_stock"], 4);
    assert_eq!(raw["modeled_balance"]["consumed_units"], 4);
    assert_eq!(row(&out, "shortage", "raw")?["missing_units"], 1);
    assert_eq!(row(&out, "step", "component")?["batches"], 2);
    assert_eq!(
        row(&out, "step", "product")?["depends_on"],
        json!([{"step_index":0,"output":"component"}])
    );
    ...
    for flag in [
        ...
        "commit_compatible",
        "mutation_dispatched",
        "native_recipe_verified",
        "job_readiness_proven",
        "completion_proven",
    ] {
...
fn enough_observed_supply_removes_deficit_but_does_not_claim_finished_products() -> Result<()> {
    ...
    assert_eq!(out["model_feasible"], true);
    assert_eq!(out["observed_quotas_met"], false);
...
fn normalized_models_keep_identity_across_equivalent_declarations() -> Result<()> {
    ...
    let c = context(&state)?;
    let q = query();
    let first = execute(&state, &c, &q)?;
    let mut changed = q;
    changed["query"]["resources"]
        .as_array_mut()
        .ok_or_else(|| invalid("resources"))?
        .reverse();
    changed["query"]["recipes"]
        .as_array_mut()
        .ok_or_else(|| invalid("recipes"))?
        .reverse();
    changed["query"]["quotas"]
        .as_array_mut()
        .ok_or_else(|| invalid("quotas"))?
        .push(json!({"resource":"raw","minimum_stock":3}));
    changed["query"]["recipes"][1]["inputs"] =
        json!([{"resource":"raw","units":1},{"resource":"raw","units":1}]);
...
fn whole_row_pages_allow_width_changes_and_repeated_continuations() -> Result<()> {
    ...
    for _ in 0..8 {
        ...
        q["query"]["continuation"] = page["continuation"].clone();
        q["query"]["limit"] = json!(2);
    }
    assert_eq!(json!(collected), full["rows"]);
    Ok(())
}
...
fn cursors_bind_every_model_capture_and_session_dimension() -> Result<()> {
    ...
    for case in 0..5 {
        ...
        assert!(
            execute(&state, &changed_c, &changed).is_err_and(|e| e.code == ErrorCode::StaleAnchor)
        );
    }
    state.publish(fixture::observation(4, 2, 8, false)?)?;
    assert!(
        execute(&state, &context(&state)?, &q).is_err_and(|e| e.code == ErrorCode::StaleAnchor)
    );
    Ok(())
}
...
fn authority_work_and_output_fail_without_partial_results() -> Result<()> {
    ...
    for case in 0..5 {
        ...
        match case {
            0 => restricted.grants.clear(),
            1 => restricted.cancellation_requested = true,
            2 => restricted.budget.max_entities = 1,
            3 => restricted.budget.max_wall_millis = 0,
    ...
    for limit in [0, 129] {
        let mut q = query();
        q["query"]["limit"] = json!(limit);
        assert!(execute(&state, &c, &q).is_err());
    }
    for work in [0, 1, MAX_ANALYSIS_WORK + 1] {
        let mut q = query();
        q["query"]["max_work"] = json!(work);
        assert!(execute(&state, &c, &q).is_err());
    }
    let mut q = query();
    q["query"]["limit"] = json!(1);
    ...
    tight.budget.max_bytes = page.to_string().len() as u64;
    assert_eq!(execute(&state, &tight, &q)?, page);
    tight.budget.max_bytes -= 1;
    assert!(execute(&state, &tight, &q).is_err());
    let mut bad = query();
    bad["expected_anchor"] = json!({});
...
fn schema_discovery_adds_only_the_new_query() -> Result<()> {
    ...
    assert_eq!(
        out["$defs"]["query"]["oneOf"][1]["properties"]["kind"]["const"],
        "production_chain"
    );
    assert!(!handles(&json!({"query":{"kind":"create_work_order"}})));
    Ok(())
}
