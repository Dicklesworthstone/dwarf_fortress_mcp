...
use super::*;
use dfmcp_world::map_connectivity::{
    self as model, CONNECTIVITY_POLICY, Classification, Connectivity,
};
...
fn position(map: &MapRegion, index: usize) -> Result<[u32; 3]> {
    map.region
        .position(index)
        .ok_or_else(|| invalid("connectivity tile index outside the observed region"))
}
fn representative(map: &MapRegion, report: &Connectivity, component: usize) -> Result<[u32; 3]> {
    let component = report
        .components
        .get(component)
        .ok_or_else(|| invalid("connectivity component missing"))?;
...
fn time(context: &OperationContext, started: Instant) -> Result<()> {
    if started.elapsed().as_millis() >= u128::from(context.budget.max_wall_millis) {
        return Err(budget(
            "connectivity exhausted its shared analysis/rendering deadline",
        ));
    }
    Ok(())
}
fn normalize(mut landmarks: Vec<Landmark>) -> Result<Vec<Landmark>> {
    if landmarks.len() > 128 {
        return Err(budget("connectivity accepts at most 128 landmarks"));
    }
    for landmark in &landmarks {
        if landmark.key.is_empty()
            || landmark.key.len() > 64
            || !landmark
                .key
                .bytes()
        ...
        {
            return Err(invalid(
                "landmarks require bounded ASCII keys and coordinates in 0..32767",
            ));
        }
    }
...
fn structural_digest(
    ...
    bytes.extend_from_slice(CONNECTIVITY_POLICY.as_bytes());
    bytes.push(0);
    bytes.extend_from_slice(ROUTE_POLICY.as_bytes());
    bytes.push(0);
    ...
    for n in map.region.origin.into_iter().chain(map.region.size) {
    ...
    for (i, component) in report.component_of.iter().enumerate() {
        ...
        if let Some(component) = component {
            put(&mut bytes, *component as u64);
        }
    }
    ...
    for c in &report.components {
        time(context, started)?;
        for n in [
            c.representative as u64,
            u64::from(c.tiles),
            u64::from(c.edges),
        ] {
            put(&mut bytes, n);
        }
        for n in c.min.into_iter().chain(c.max) {
            put(&mut bytes, u64::from(n));
        }
    ...
    for b in &report.bottlenecks {
        ...
        for n in [
            b.tile as u64,
            b.component as u64,
            b.separated_tile_pairs,
            b.partition_sizes.len() as u64,
        ] {
            put(&mut bytes, n);
        }
        for n in &b.partition_sizes {
            put(&mut bytes, u64::from(*n));
        }
    }
    ...
    for b in &report.bridges {
        time(context, started)?;
        for n in [
            b.endpoints[0] as u64,
            b.endpoints[1] as u64,
            b.component as u64,
            u64::from(b.side_sizes[0]),
            u64::from(b.side_sizes[1]),
        ] {
            put(&mut bytes, n);
        }
    }
...
pub(super) fn execute<S: SpatialStateView>(
    state: &S,
    context: &OperationContext,
    source: Digest32,
    request: Request,
    ...
    let map = &state
        .spatial_observation()
        .ok_or_else(|| invalid("connectivity requires a spatial capture"))?
        .terrain()
        .map;
    ...
    for cell in &map.cells {
        *classes.entry(model::classify(*cell).name()).or_default() += 1;
    }
    let mut established = 0usize;
    let mut landmark_components = BTreeSet::new();
    for landmark in &landmarks {
        if let Some(component) = map
            .region
            .index(landmark.position)
            .and_then(|i| report.component_of[i])
        {
            established += 1;
            landmark_components.insert(component);
        }
    }
    let connected = if landmarks.is_empty() || established != landmarks.len() {
        None
    } else {
        Some(landmark_components.len() == 1)
    };
    let mut bottlenecks: Vec<_> = (0..report.bottlenecks.len()).collect();
    bottlenecks.sort_unstable_by_key(|&i| {
        (
            Reverse(report.bottlenecks[i].separated_tile_pairs),
            report.bottlenecks[i].tile,
        )
    });
    ...
    bridges.sort_unstable_by_key(|&i| {
        ...
        (
            Reverse(u64::from(b.side_sizes[0]) * u64::from(b.side_sizes[1])),
            b.endpoints,
        )
    });
    let id = identity(
        context,
        source,
        json!({"kind":"map_connectivity","policy":QUERY_POLICY,
    ...
    out["connectivity_policy"] = json!(CONNECTIVITY_POLICY);
    out["query_policy"] = json!(QUERY_POLICY);
    out["structural_digest"] = json!(structural_digest.to_string());
    out["analysis_complete"] = json!(true);
    out["native_captures"] = json!(0);
    ...
    out["interpretation"] = json!(
        "Components and cuts are exact only in the captured dry cardinal floor/stair model. Occupancy, hidden cells and unsupported movement can omit real routes. Removing a tile or edge is a graph counterfactual, not a terrain edit, diagnosis, safe demolition or evacuation guarantee."
    );
    out["order"] = json!(
        "landmarks_by_key_then_bottlenecks_by_separated_pairs_desc_then_bridges_by_separated_pairs_desc_then_components_by_representative"
    );
    let counts = [
        if matches!(section, Section::All | Section::Landmarks) {
            landmarks.len()
        } else {
            0
        },
        if matches!(section, Section::All | Section::Bottlenecks) {
            bottlenecks.len()
        } else {
            0
        },
        if matches!(section, Section::All | Section::Bridges) {
            bridges.len()
        } else {
            0
        },
        if matches!(section, Section::All | Section::Components) {
    ...
    let result = paginate(
        ...
        limit.min(context.budget.max_entities),
        id,
        context,
        |mut index| {
            time(context, started)?;
            if index < counts[0] {
                let l = &landmarks[index];
                let cell = map.region.index(l.position);
                let component = cell.and_then(|i| report.component_of[i]);
                let classification = cell.map(|i| model::classify(map.cells[i]));
                return Ok(
                    json!({"row_kind":"landmark","key":l.key,"position":l.position,
            ...
            if index < counts[1] {
                ...
                return Ok(
                    json!({"row_kind":"bottleneck","position":position(map,b.tile)?,
            ...
            if index < counts[2] {
                ...
                return Ok(
                    json!({"row_kind":"graph_bridge","endpoints":[position(map,b.endpoints[0])?,position(map,b.endpoints[1])?],
            ...
            let c = report
                .components
                .get(index)
                .ok_or_else(|| invalid("connectivity page index invalid"))?;
            Ok(
                json!({"row_kind":"component","representative_position":position(map,c.representative)?,
