...
use dfmcp_adapter::live_jobs_rpc::{
    DeadlineStream,
    operations::{
        map::spatial::{SpatialLimits, SpatialRpcClient},
...
static SESSIONS: LazyLock<Mutex<BTreeMap<SessionId, Arc<Mutex<SpatialSession>>>>> =
    LazyLock::new(|| Mutex::new(BTreeMap::new()));
fn error(code: ErrorCode, s: &str) -> DfmcpError {
    DfmcpError::new(code, s)
}
fn lock<T>(m: &Mutex<T>) -> Result<MutexGuard<'_, T>> {
    m.lock().map_err(|_| {
        error(
            ErrorCode::InternalInvariantViolation,
            "spatial mutex poisoned",
        )
    })
}
struct Slot;
        ...
        SLOTS
            ...
            .map_err(|_| {
                error(
                    ...
                    "spatial runtime retains at most two sessions",
                )
            })?;
        Ok(Self)
    }
...
impl SpatialSource for SpatialRpcClient<DeadlineStream> {
    fn read(&mut self, t: Duration) -> Result<LiveSpatialObservation> {
        self.refresh(t)
    }
    fn poisoned(&self) -> bool {
        SpatialRpcClient::poisoned(self)
    }
    fn fence(&mut self) {
        SpatialRpcClient::fence(self);
    }
    fn pages(&self) -> u32 {
...
impl SpatialSession {
    ...
    fn context(&mut self) -> Result<OperationContext> {
        self.request = self
            .request
            .checked_add(1)
            .ok_or_else(|| error(ErrorCode::BudgetExceeded, "spatial request IDs exhausted"))?;
        Ok(OperationContext {
            session_id: self.id,
    ...
    fn refresh(&mut self, c: &OperationContext) -> Result<JobPublication> {
        ...
        if c.anchor != self.anchor()? {
            return Err(error(
                ErrorCode::StaleAnchor,
                "spatial refresh anchor changed",
            ));
        }
        if self.source.poisoned() {
            return Err(error(
                ErrorCode::AdapterUnavailable,
                "spatial source fenced; reopen session",
            ));
        }
        if let Some(journal) = &self.journal {
            ...
            if journal.fenced() || journal.state().snapshot().map(|s| s.anchor()) != Some(c.anchor)
            {
                ...
                return Err(error(
                    ...
                    "spatial journal is fenced or disagrees with the published anchor",
                ));
            }
        }
        ...
        let result = self
            .source
            .read(Duration::from_millis(c.budget.max_wall_millis))
            .and_then(|v| {
                let op = v.operations();
                let limits = self.limits.operations;
                if op.jobs.jobs.len() > limits.jobs as usize
                    || op.buildings.len() > limits.buildings as usize
                    || op.items.len() > limits.items as usize
                    || v.terrain().map.region != self.limits.region
                    || v.encode_payload()?.len() > limits.payload_bytes
                {
                    return Err(error(
                        ErrorCode::BudgetExceeded,
                        "spatial observation exceeds session acquisition bounds",
                    ));
                }
...
fn resolve(raw: Option<String>) -> Result<Arc<Mutex<SpatialSession>>> {
    let text = raw.ok_or_else(|| {
        error(
            ...
            "open a spatial session and supply session_id",
        )
    })?;
    if text.len() != 32 || !text.bytes().all(|b| b.is_ascii_hexdigit()) {
        return Err(error(
            ErrorCode::InvalidRequest,
            "invalid spatial session handle",
        ));
    }
    let raw = u128::from_str_radix(&text, 16)
        .map_err(|_| error(ErrorCode::InvalidRequest, "invalid spatial session hex"))?;
    let id = SessionId::new(raw);
    if id.get() != raw || !id.is_process_scoped_live() || (raw & ((1u128 << 62) - 1)) >> 58 != 1 {
        return Err(error(
            ErrorCode::InvalidRequest,
            "not an encoded spatial session",
        ));
    }
    lock(&SESSIONS)?
        .get(&id)
        .cloned()
        .ok_or_else(|| error(ErrorCode::SessionNotFound, "spatial session not found"))
}
fn allowed_environment(n: &str) -> bool {
    !n.starts_with("DFMCP_")
        || matches!(
            n,
            "DFMCP_ALLOW_UNADMITTED_SPATIAL_V1_6"
                | "DFMCP_SPATIAL_TOKEN"
                | "DFMCP_SPATIAL_ENDPOINT"
                | "DFMCP_SPATIAL_JOURNAL"
                | "DFMCP_SPATIAL_JOURNAL_REPAIR"
        )
}
fn validate_environment() -> Result<()> {
    if std::env::var("DFMCP_ALLOW_UNADMITTED_SPATIAL_V1_6")
        .ok()
        .as_deref()
        != Some("1")
        ...
        || crate::admission::current_admission_provenance().is_some()
    {
        return Err(error(
            ...
            "spatial/1.6 requires its own opt-in and refuses other DFMCP/admission state",
        ));
    }
    Ok(())
}
fn anchor_json(a: StateAnchor) -> Value {
    json!({"fortress_id":a.fortress_id.to_string(),"epoch":a.cursor.epoch,
...
fn coverage() -> Value {
    json!({"status":"partial","complete_domains":["current_job_roster","building_roster","item_roster","job_item_attachments","requested_region.cell_presence"],
...
fn packet(
    ...
    if let (Some(s), Some(c)) = (s, c) {
        ...
        builder = builder
            ...
            .continuity(
                ...
                None,
                reset.then(|| "spatial_capture_epoch_reset".to_owned()),
            );
    } else {
        builder = builder.briefing(
            json!({"runtime_admitted":false,"mutation_admissible":false,"bridge_protocol":"1.6"}),
        );
    }
    ...
    if out.len() > maximum {
        return Err(error(
            ErrorCode::BudgetExceeded,
            "complete spatial packet exceeds output budget",
        ));
    }
    Ok(out)
}
...
where
    F: FnOnce(&mut SpatialSession, OperationContext) -> Result<String>,
{
...
fn parse_region(input: &Value) -> Result<Region> {
        ...
        })
    {
        return Err(error(
            ErrorCode::InvalidRequest,
            "region requires origin and size triples",
        ));
    }
    let r: RegionInput = serde_json::from_value(input.clone())
        .map_err(|_| error(ErrorCode::InvalidRequest, "invalid spatial region"))?;
    let r = Region {
        origin: r.origin,
        size: r.size,
    };
    ...
    for n in input {
        let c = match n.as_str() {
            ...
            _ => {
                return Err(error(
                    ErrorCode::CapabilityDenied,
                    "spatial profile cannot grant that capability",
                ));
            }
        };
        if out.contains(&c) {
...
#[tool(
    description = "Open an isolated read-only spatial/1.6 session: complete jobs/buildings/inventory plus one terrain region are captured together, then paged immutably. Region is {origin:[x,y,z],size:[width,height,depth]}. No game effects."
)]
...
pub fn fortress_open_session(
    ...
    let result = (|| -> Result<String> {
        ...
        if journal_configuration.is_some()
            && (!caps.contains(&Capability::Query) || !caps.contains(&Capability::Observe))
        {
            return Err(error(
                ErrorCode::CapabilityDenied,
                "durable spatial sessions require Query and Observe authority",
            ));
        }
        let capture = usize::try_from(max_capture_bytes.unwrap_or(MAX_SPATIAL_BYTES as u64))
            ...
            || !(1..=60000).contains(&budget.max_wall_millis)
        {
            return Err(error(
                ErrorCode::BudgetExceeded,
                "spatial output or wall-time budget outside bounds",
            ));
        }
        let slot = Slot::reserve()?;
        let id = next_id()?;
        let endpoint = dfmcp_adapter::parse_loopback_endpoint(
            &std::env::var("DFMCP_SPATIAL_ENDPOINT")
                .unwrap_or_else(|_| "127.0.0.1:5000".to_owned()),
        )?;
        let token = std::env::var("DFMCP_SPATIAL_TOKEN").map_err(|_| {
            error(
                ErrorCode::CapabilityDenied,
                "DFMCP_SPATIAL_TOKEN must be configured",
            )
        })?;
        let timeout = Duration::from_millis(budget.max_wall_millis);
        let mut source = SpatialRpcClient::connect(
            endpoint,
            token.into_bytes(),
        ...
        let fortress = state
            ...
            .ok_or_else(|| {
                error(
                    ErrorCode::InternalInvariantViolation,
                    "spatial bootstrap snapshot absent",
                )
            })?
            .fortress_id;
        let grants = caps
...
#[tool(
    description = "Capture coherent operations and the fixed terrain region once, without unpausing or modifying the game."
)]
pub fn fortress_observe(session_id: Option<String>) -> String {
    observe(session_id, "fortress.observe")
}
#[tool(
    description = "Acquire one spatial capture. Query await_watch evaluates a foreground condition after that same bounded refresh."
)]
...
fn view(s: &SpatialSession, c: &OperationContext) -> Result<QueryResponseProjection> {
    Ok(QueryResponseProjection {
        ...
        coverage: coverage(),
        uncertainty: vec![
            json!({"domain":"spatial_game_feasibility","epistemic_state":"unknown","reason":"candidate routes and declared stack models do not prove unit access, recipe satisfaction or safety"}),
        ],
        budget: json!({"admitted":{"max_bytes":s.budget.max_bytes,"max_output_tokens":s.budget.max_output_tokens}}),
        references: vec![
            json!({"kind":"coherent_spatial_capture","digest":s.state.source_digest()?.to_string()}),
        ],
        maximum_bytes: s
            .budget
            .max_bytes
...
fn finish(view: &QueryResponseProjection, v: Value) -> Result<String> {
    let mut v: Value = serde_json::from_str(&view.finish(v)?).map_err(|_| {
        error(
            ErrorCode::InternalInvariantViolation,
            "spatial query response invalid",
        )
    })?;
    v["agent_turn"]["turn_id"] = json!(format!("spatial-turn-{}", view.request_id));
    let out = v.to_string();
    if out.len() > view.maximum_bytes {
        return Err(error(
            ErrorCode::BudgetExceeded,
            "spatial query response exceeds budget",
        ));
    }
    Ok(out)
}
        ...
        |s, mut c| {
            ...
            let mut input = match query {
                ...
                None => match mode.as_deref() {
                    ...
                    Some("items") => {
                        json!({"schema":"dfmcp.query/1","query":{"kind":"entities","kinds":["item"],"fields":["type_key","stack_size","container","raw_position"],"limit":1}})
                    }
                    Some("jobs") => {
                        json!({"schema":"dfmcp.query/1","query":{"kind":"entities","kinds":["job"],"fields":["type_key","suspended"],"limit":1}})
                    }
                    Some("buildings") => {
            ...
            if s.source.poisoned() && !local && !schema {
                return Err(error(
                    ErrorCode::AdapterUnavailable,
                    "spatial source fenced; reopen or manage local records",
                ));
            }
            ...
            if !schema && kind == "await_watch" {
                ...
                if semantic_query::prepare_await(snapshot, &c, &input)? {
                    ...
                    c.authorize(Capability::Observe, RiskTier::ReadOnly, &[], None)?;
                    refresh = Some(
                        json!({"basis":anchor_json(basis),"reset":outcome==JobPublication::Reset,"kind":if outcome==JobPublication::Heartbeat{"heartbeat"}else{"snapshot"},
            ...
            if schema {
                return semantic_query::publish_with_active_work(
                    &c,
                    json!({"query_schema":history::schema()?,"mode":"schema",
            "profile":"spatial/1.6","source_stale":s.source.poisoned(),"truncated":false,"continuation":null}),
                    |v| finish(&view, v),
                );
            }
...
#[tool(
    description = "Explain coherent spatial coverage and the difference between observed candidate routes and actual game feasibility."
)]
pub fn fortress_explain(session_id: Option<String>) -> String {
    with_session(session_id, "fortress.explain", Capability::Query, |s, c| {
        semantic_query::publish_with_active_work(
            &c,
            json!({"ok":true,"coherence":"one native capture of operations plus requested terrain",
        ...
        "history":"optional fixed-profile archive; history lists captures and historical_query reads an exact past record; no cross-profile import"}),
...
pub fn fortress_doctor(session_id: Option<String>) -> String {
    with_session(session_id, "fortress.doctor", Capability::Doctor, |s, c| {
        packet(
            ...
            Some(&c),
            "fortress.doctor",
            json!({"ok":true,"status":if s.source.poisoned(){"source_fenced"}else{"read_only_unadmitted"}}),
        )
    })
}
fn no_effect(id: Option<String>, operation: &str) -> String {
    with_session(id, operation, Capability::Query, |_, _| {
        Err(error(
            ErrorCode::CapabilityDenied,
            "spatial/1.6 has no live mutation, reservation, game checkpoint or restore path",
        ))
    })
}
#[tool(
    description = "Unavailable: spatial observations and allocations cannot authorize game effects."
)]
...
#[tool(
    description = "Unavailable for game effects. Local condition watches use query cancel_watch."
)]
