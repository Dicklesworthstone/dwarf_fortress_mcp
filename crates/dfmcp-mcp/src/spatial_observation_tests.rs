...
impl Files {
    fn new() -> Result<Self> {
        ...
        fs::DirBuilder::new()
            ...
            .map_err(io_error)?;
        Ok(Self {
            journal: directory.join("observations.bin"),
            directory,
        })
    }
}
...
impl Source for Script {
    fn read(&mut self, allowance: Duration) -> Result<LiveSpatialCitizenObservation> {
        ...
        self.values
            .pop_front()
            .ok_or_else(|| error(ErrorCode::AdapterUnavailable, "injected capture failure"))
    }
...
fn session(files: Option<&Files>, ticks: &[u32]) -> Result<(Session, Calls)> {
    ...
    let source = Script {
        values: ticks
            .iter()
            .map(|tick| fixture::observation(*tick, 2, 2, false))
...
fn forbid_at_next_tick(s: &mut Session, capability: Capability) -> Result<()> {
    ...
    for grant in &mut s.grants {
        if grant.capability == capability {
            grant.expires_at_tick = Some(expiry);
        }
    }
    Ok(())
}
...
fn observe_expiry_preserves_both_journaled_and_memory_roots() -> Result<()> {
    ...
    for durable in [false, true] {
        ...
        let source = s.state.source_digest()?;
        let bytes = if durable {
            Some(fs::read(&files.journal).map_err(io_error)?)
        } else {
            None
        };
        let c = s.context()?;
        assert!(matches!(clocked(&mut s,&c,&[0,0,0]),Err(e)if e.code==ErrorCode::CapabilityDenied));
        assert_eq!(s.state.snapshot(), old.as_ref());
        assert_eq!(s.state.source_digest()?, source);
        assert!(s.source.poisoned());
        assert_eq!(calls.count.load(Ordering::SeqCst), 1);
        if let Some(bytes) = bytes {
            assert_eq!(fs::read(&files.journal).map_err(io_error)?, bytes);
        }
    }
    Ok(())
}
...
fn durable_capture_requires_query_at_the_target_but_memory_observe_does_not() -> Result<()> {
    ...
    assert_eq!(fs::read(&files.journal).map_err(io_error)?, before);
    drop(s);
    let (mut s, calls) = session(None, &[5])?;
    s.grants.retain(|g| g.capability == Capability::Observe);
    let old = s.anchor()?;
    let c = s.context()?;
    clocked(&mut s, &c, &[0, 0, 0])?;
    assert!(s.anchor()?.tick > old.tick);
    assert!(!s.source.poisoned());
    assert_eq!(calls.count.load(Ordering::SeqCst), 1);
    Ok(())
}
...
fn deadline_refusals_before_and_after_capture_never_publish() -> Result<()> {
    ...
    for durable in [false, true] {
        for (times, read) in [
            ...
            let files = Files::new()?;
            let (mut s, calls) = session(durable.then_some(&files), &[4])?;
            let old = s.state.snapshot().cloned();
            ...
            assert_eq!(s.state.snapshot(), old.as_ref());
            assert_eq!(s.source.poisoned(), read);
...
fn stale_cancelled_or_expired_callers_never_enter_the_source() -> Result<()> {
    ...
    for case in 0..5 {
        ...
        assert!(clocked(&mut s, &c, &[0, 0, 0]).is_err());
        assert_eq!(s.anchor()?, old);
...
fn narrowed_entity_and_acquisition_bounds_preserve_generation_history() -> Result<()> {
    ...
    for count_bound in [false, true] {
        ...
        s.source = Box::new(Script {
            values: VecDeque::from([fixture::observation(4, 3, 2, false)?]),
            calls: calls.clone(),
            fenced: false,
            corrupt: None,
        });
        ...
        if count_bound {
            ...
            c.budget.max_entities = old.as_ref().map_or(0, |s| s.graph.entities.len() as u32);
        }
...
fn changed_journal_custody_refuses_before_capture_or_after_source_without_append() -> Result<()> {
    ...
    for during in [false, true] {
        ...
        if during {
            s.source = Box::new(Script {
                values: VecDeque::from([fixture::observation(4, 2, 2, false)?]),
                calls: calls.clone(),
                fenced: false,
                corrupt: Some(files.journal.clone()),
            });
        } else {
            fs::OpenOptions::new()
                .append(true)
        ...
        assert_eq!(
            fs::read(&files.journal).map_err(io_error)?,
