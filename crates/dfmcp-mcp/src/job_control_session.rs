...
use dfmcp_adapter::job_suspension::coordinator::{
    DurableJobRecord, DurableJobState, JobControlJournal, JobJournalSummary,
    JobRecordPage, JobSuspensionSource, job_fortress_id,
};
use dfmcp_adapter::job_suspension::rpc::{JobControlManifest, JobControlRpcClient, JobControlStream};
use dfmcp_core::{
    Capability, DfmcpError, Digest32, ErrorCode, FortressId, OperationContext,
    Result, RiskTier, SessionId,
};
...
fn stale() -> DfmcpError {
    error(ErrorCode::StaleAnchor,
        "select this job again and use its exact witness; no effect was dispatched")
}

struct Selection { observation: JobObservation, manifest: JobControlManifest }
...
impl<S: EffectJournalStorage, N: SelectedJobSource> JobControlSession<S, N> {
    ...
    pub fn new(journal: JobControlJournal<S>, source: Option<N>,
        context: &OperationContext) -> Result<Self>
    {
        let summary = journal.summary(context)?;
        Ok(Self {
            id: context.session_id, fortress: summary.fortress_id,
            journal, source, selected: None,
        })
    }
    ...
    fn production(&self, context: &OperationContext) -> Result<()> {
        self.access(context)?;
        context.authorize(Capability::ConfigureProduction, RiskTier::Reversible, &[], None)?;
        if context.budget.max_actions < 1 {
            return Err(error(ErrorCode::BudgetExceeded, "job control requires one action of budget"));
        }
    ...
    fn no_unresolved_dispatch(&self, context: &OperationContext) -> Result<()> {
        if self.journal.summary(context)?.unresolved != 0 {
            return Err(error(ErrorCode::EffectIndeterminate,
    ...
    pub fn observe(&mut self, job: u32, context: &OperationContext) -> Result<JobObservation> {
        ...
        if job > i32::MAX as u32 {
            return Err(error(ErrorCode::InvalidRequest, "native job ID exceeds signed 32-bit range"));
        }
        ...
        let result = (|| {
            ...
            if source.manifest() != &manifest || observation.generation() != manifest.generation
                || observation.job_id() != job || job_fortress_id(&observation) != self.fortress
            {
                return Err(stale());
            }
            ...
            if started.elapsed() >= timeout {
                return Err(error(ErrorCode::BudgetExceeded, "selected-job acquisition exceeded its deadline"));
            }
    ...
    fn selection(&self, job: u32, witness: Digest32,
        context: &OperationContext) -> Result<JobObservation>
    {
        let selected = self.selected.as_ref().ok_or_else(stale)?;
        let observation = &selected.observation;
        if observation.job_id() != job || observation.witness() != witness
            || context.anchor.tick.get() != observation.tick() || context.anchor.state_hash != witness
        { return Err(stale()); }
        let source = self.source.as_ref().ok_or_else(stale)?;
        if source.manifest() != &selected.manifest { return Err(stale()); }
    ...
    pub fn plan(&mut self, key: &str, job: u32, desired: bool, witness: Digest32,
        context: &OperationContext) -> Result<DurableJobRecord>
    {
        ...
        if let Some(record) = self.journal.lookup(key, context)? {
            ...
            if plan.observation().job_id() != job || plan.desired() != desired
                || plan.observation().witness() != witness
            {
                return Err(error(ErrorCode::Conflict, "job key already names different sealed intent"));
            }
    ...
    pub fn record(&self, key: &str, digest: Digest32,
        context: &OperationContext) -> Result<DurableJobRecord>
    {
        self.access(context)?;
        let record = self.journal.lookup(key, context)?.ok_or_else(||
            error(ErrorCode::InvalidRequest, "no retained job effect has this key"))?;
        if record.plan().digest() != digest {
            return Err(error(ErrorCode::Conflict, "job key and sealed plan digest disagree"));
        }
        Ok(record.clone())
    }
        ...
        context: &OperationContext) -> Result<DurableJobRecord>
    {
        ...
        if record.plan().observation().witness() != witness { return Err(stale()); }
        ...
        match self.journal.commit(source, &plan, context) {
            ...
            Err(cause) => {
                let uncertain = self.journal.poisoned()
                    || self.journal.lookup(key, context)
                        .map(|r| r.is_some_and(|r| r.state().reconciliation_required()))
                        .unwrap_or(true);
                if uncertain {
                    Err(error(ErrorCode::EffectIndeterminate,
    ...
    {
        let record = self.record(key, digest, context)?;
        if !record.state().reconciliation_required() { return Ok(record); }
        let source = self.source.as_mut().ok_or_else(|| error(ErrorCode::CapabilityDenied,
    ...
    pub fn records_page(&self, head: Digest32, after: Option<&str>, limit: usize,
        context: &OperationContext) -> Result<JobRecordPage>
    {
        self.access(context)?;
