...
fn registration_schema(mut schema: Value) -> Result<Value> {
    let invalid = || {
        error(
            ...
            "registration requires one canonical watch request schema",
        )
    };
    ...
    member["required"]
        .as_array_mut()
        .ok_or_else(invalid)?
        .retain(|name| name != "kind");
    let definitions = schema["$defs"].as_object_mut().ok_or_else(invalid)?;
    if definitions.contains_key("watch_set_member") {
        return Err(invalid());
    }
...
pub(super) fn extend_schema(mut schema: Value) -> Result<Value> {
    ...
    let additions: Value = serde_json::from_str(include_str!(
        "../../../schemas/mcp_watch_batch_v1.json"
    ))
    .map_err(|_| {
        error(
            ErrorCode::InternalInvariantViolation,
            "watch batch schema is invalid",
        )
    })?;
    let variants = additions["oneOf"].as_array().ok_or_else(|| {
        error(
            ErrorCode::InternalInvariantViolation,
            "batch variants absent",
        )
    })?;
...
fn remaining(context: &OperationContext, started: Instant) -> Result<OperationContext> {
    ...
    if elapsed >= u128::from(context.budget.max_wall_millis) {
        return Err(error(
            ...
            "watch batch exhausted its shared preflight/capture/evaluation deadline",
        ));
    }
    ...
    if let Some(journal) = session.journal.as_mut() {
        ...
        if journal.state().snapshot().map(|s| s.anchor()) != Some(context.anchor) {
            return Err(error(
                ...
                "watch batch observation archive disagrees with published state",
            ));
        }
    }
    Ok(())
...
pub(super) fn execute(
    ...
    if recovery::handles(input) {
        return recovery::execute(session, context, input);
    }
    ...
    if session.source.archive_only() {
        return Err(error(
            ...
            "archive-only sessions cannot register, evaluate or await watch batches",
        ));
    }
    if session.source.poisoned() {
        return Err(error(
            ErrorCode::AdapterUnavailable,
            "watch batch source is fenced; reopen before evaluating watches",
        ));
    }
    if context.anchor != session.anchor()? {
        return Err(error(
            ErrorCode::StaleAnchor,
            "watch batch context differs from the session anchor",
        ));
    }
    ...
    let snapshot = session.state.snapshot().ok_or_else(|| {
        error(
            ErrorCode::InternalInvariantViolation,
            "batch snapshot absent",
        )
    })?;
    if input["query"]["kind"] == "register_watches" {
        return semantic_query::register_watch_set(
            snapshot,
            &remaining(context, started)?,
            input,
            |mut value| {
    ...
    let prepared = semantic_query::prepare_watch_batch(
        snapshot,
        &remaining(context, started)?,
        input,
        |mut value| {
    ...
    let refresh = if acquired {
        ...
        Some(
            json!({"basis":anchor_json(context.anchor),"reset":outcome==JobPublication::Reset,
    ...
    semantic_query::complete_watch_batch(snapshot, &evaluation, prepared, acquired, |mut value| {
        ...
        value["next_step"] = if value["all_terminal"] == true {
            Value::Null
        } else {
