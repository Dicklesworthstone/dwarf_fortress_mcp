...
fn register(
    ...
    let result = super::super::execute_in(
        store,
        s,
        c,
        &json!({"schema":"dfmcp.query/1","query":{
        "kind":"watch","key":key,"condition":condition,"deadline_tick":100,
        "poll_interval_ticks":1,"stable_observations":stable}}),
        encoded,
    )?;
    decode(&result)?["record"]["watch"]
        .as_str()
        .map(str::to_owned)
        .ok_or_else(|| invalid("test handle"))
}
fn paused(
    store: &Mutex<Store>,
    s: &WorldSnapshot,
...
fn row<'a>(value: &'a Value, key: &str) -> Result<&'a Value> {
    value["records"]
        ...
        .ok_or_else(|| invalid("batch row missing"))
}
...
fn batches_advance_competing_watches_on_every_shared_capture() -> Result<()> {
    ...
    for n in 1..=2 {
        ...
        for key in ["a", "b"] {
            assert_eq!(row(&result, key)?["stable_observations"], n + 1);
        }
    ...
    assert_eq!(result["all_terminal"], true);
    assert_eq!(row(&result, "a")?["terminal_replayed"], true);
    Ok(())
}
...
fn heartbeat_and_cadence_do_not_manufacture_samples() -> Result<()> {
    ...
    assert_eq!(result["advanced"], 0);
    let later = snapshot(1, 1);
    let mut next = c.clone();
    next.anchor = later.anchor();
    let prepared = prepare_in(&store, &s, &c, &request(true, None), encoded)?;
    let result = decode(&complete_in(
        &store, &later, &next, prepared, true, encoded,
    )?)?;
    assert_eq!(result["sampled"], 0);
    assert_eq!(row(&result, "a")?["stable_observations"], 1);
    Ok(())
}
...
fn explicit_selection_is_canonical_and_does_not_advance_unselected_work() -> Result<()> {
    ...
    assert_eq!(result["sampled"], 1);
    assert_eq!(
        record(&*lock(&store)?, next.session_id, &b)?.evidence_digest,
        prior
    );
    Ok(())
}
...
fn malformed_duplicate_foreign_and_stale_selections_fail_before_preview() -> Result<()> {
    ...
    let c = context(&s);
    let a = paused(&store, &s, &c, "a", 3)?;
    let mut stale = request(true, None);
    stale["expected_anchor"] = json!({});
    let mut extra = request(true, None);
    extra["query"]["wait_forever"] = json!(true);
    for input in [
        request(true, Some(&[])),
        request(true, Some(&[a.clone(), a.clone()])),
        request(true, Some(&[format!("watch:{}", "f".repeat(64))])),
        request(true, Some(&["bad".into()])),
        stale,
        extra,
    ] {
        let mut previewed = false;
        assert!(
            prepare_in(&store, &s, &c, &input, |v| {
                previewed = true;
                encoded(v)
            })
            .is_err()
    ...
    let mut other = c;
    other.session_id = SessionId::new(8402);
    assert!(prepare_in(&store, &s, &other, &request(true, Some(&[a])), encoded).is_err());
    Ok(())
}
...
fn empty_and_terminal_only_batches_need_no_observe_authority() -> Result<()> {
    ...
    assert_eq!(result["all_terminal"], true);
    assert_eq!(result["all_satisfied"], false);
    paused(&store, &s, &c, "terminal", 1)?;
    let prepared = prepare_in(&store, &s, &c, &request(true, None), encoded)?;
    assert!(!prepared.needs_observation());
    paused(&store, &s, &c, "pending", 3)?;
    assert!(
        matches!(prepare_in(&store,&s,&c,&request(true,None),encoded),Err(e)if e.code==ErrorCode::CapabilityDenied)
    );
    assert!(prepare_in(&store, &s, &c, &request(false, None), encoded).is_ok());
    Ok(())
}
...
fn failed_render_or_late_watch_error_publishes_none_of_the_batch() -> Result<()> {
    ...
    let prepared = prepare_in(&store, &s, &c, &request(true, None), encoded)?;
    assert!(
        complete_in(&store, &later, &next, prepared, true, |_| Err(bounded(
    ...
    assert_eq!(registry(&*lock(&store)?, c.session_id)?, prior);
    {
        ...
        let w = guard
            .entries
            .get_mut(&(c.session_id, b))
            .ok_or_else(|| invalid("test watch"))?;
        w.samples = u64::MAX;
        w.seal()?;
    }
    let prior = registry(&*lock(&store)?, c.session_id)?;
...
fn registry_change_during_capture_is_a_conflict_not_a_partial_poll() -> Result<()> {
    ...
    next.anchor = later.anchor();
    assert!(
        matches!(complete_in(&store,&later,&next,prepared,true,encoded),Err(e)if e.code==ErrorCode::Conflict)
    );
    assert_eq!(registry(&*lock(&store)?, c.session_id)?, prior);
    let prepared = prepare_in(&store, &s, &c, &request(true, None), encoded)?;
    let mut other = c.clone();
    other.session_id = SessionId::new(8403);
    paused(&store, &s, &other, "other", 3)?;
    assert!(complete_in(&store, &later, &next, prepared, true, encoded).is_ok());
    Ok(())
}
...
fn reset_expiration_and_unknown_remain_distinct_batch_outcomes() -> Result<()> {
    for reset in [false, true] {
        ...
        assert_eq!(
            row(&result, "a")?["status"],
            if reset { "invalidated" } else { "expired" }
        );
    }
    let store = Mutex::new(Store::default());
    let s = snapshot(0, 1);
    ...
    register(
        ...
        &c,
        "unknown",
        json!({"op":"field","entity_id":"1","generation":1,
        "field":"alive","comparison":"eq","value":{"type":"bool","value":true}}),
        2,
    )?;
    let prepared = prepare_in(&store, &s, &c, &request(false, None), encoded)?;
    let result = decode(&complete_in(&store, &s, &c, prepared, false, encoded)?)?;
    assert_eq!(row(&result, "unknown")?["condition"], "unknown");
    assert_eq!(result["all_satisfied"], false);
    Ok(())
}
    ...
    for case in 0..4 {
        ...
        match case {
            ...
            2 => denied
                .grants
                .iter_mut()
                .for_each(|g| g.expires_at_tick = Some(GameTick(1))),
            _ => denied.session_id = SessionId::new(8404),
        }
        assert!(complete_in(&store, &later, &denied, prepared, true, encoded).is_err());
    }
    let prepared = prepare_in(&store, &s, &c, &request(true, None), encoded)?;
    assert!(complete_in(&store, &later, &next, prepared, false, encoded).is_err());
    Ok(())
}
