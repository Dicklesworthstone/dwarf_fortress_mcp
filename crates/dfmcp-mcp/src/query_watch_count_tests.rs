...
fn source() -> Digest32 {
    Digest32::of_bytes(b"count-watch-native-fixture")
}
            ...
            .map(|v| {
                (
                    ...
                    Fact::known(
                        WorldValue::Bool(v),
                        GameTick(tick),
                        FactSource::DfhackField("count-test.ready".into()),
                        source(),
                    ),
                )
            })
...
fn context(snapshot: &WorldSnapshot) -> OperationContext {
    OperationContext {
        session_id: SessionId::new(9_010_017),
...
fn every_small_count_interval_matches_exhaustive_integer_completions() {
        ...
        for upper in lower..=8 {
            for threshold in 0..=10 {
                for comparison in [
                    ...
                    let values: Vec<_> = (lower..=upper)
                        .map(|n| match comparison {
                            Comparison::Eq => n == threshold,
                            Comparison::Ne => n != threshold,
                            Comparison::Lt => n < threshold,
                            Comparison::Le => n <= threshold,
                            Comparison::Gt => n > threshold,
                            Comparison::Ge => n >= threshold,
                        })
                        .collect();
                    let expected = if values.iter().all(|v| *v) {
                        Truth::True
                    ...
                    assert_eq!(
                        interval_truth(lower, upper, comparison, threshold),
                        expected
                    );
                }
            }
    ...
    assert_eq!(
        interval_truth(0, u64::MAX, Comparison::Ne, u64::MAX),
        Truth::Unknown
    );
    assert_eq!(
        interval_truth(u64::MAX, u64::MAX, Comparison::Ge, u64::MAX),
        Truth::True
    );
    assert_eq!(
        interval_truth(0, u64::MAX, Comparison::Gt, u64::MAX),
        Truth::False
    );
}
...
fn missing_rows_widen_counts_without_becoming_false_or_true_under_negation() -> Result<()> {
    ...
    for (cmp, n, expected) in [
        (Comparison::Ge, 1, Truth::True),
        (Comparison::Ge, 2, Truth::Unknown),
        (Comparison::Ge, 3, Truth::False),
        (Comparison::Eq, 1, Truth::Unknown),
        (Comparison::Ne, 1, Truth::Unknown),
    ] {
        let (truth, evidence) = evaluate_count(&s, cmp, n)?;
        assert_eq!(truth, expected);
        assert_eq!(evidence["matched_min"], 1);
        assert_eq!(evidence["matched_max"], 2);
        assert_eq!(evidence["population"], 3);
        assert_eq!(evidence["complete_world_count_proven"], false);
    ...
    assert_eq!(
        probe.evaluate(
            &Condition::Not {
                arg: Box::new(condition(Comparison::Eq, 1))
            },
            &s
        )?,
        Truth::Unknown
    ...
    assert_eq!(
        row_truth(
            &Predicate::Not {
                arg: Box::new(predicate())
            },
            &s.graph.entities[&EntityId::new(3)],
            &s,
            &mut budget
        )?,
        Truth::Unknown
...
fn every_non_native_or_inconsistent_field_remains_unestablished() -> Result<()> {
    ...
    for case in 0..10 {
        ...
        let fact = graph
            ...
            .and_then(|e| e.fields.get_mut("ready"))
        ...
        assert_eq!(evidence["matched_min"], 0);
        assert_eq!(evidence["matched_max"], 1);
        assert_eq!(evidence["unestablished"], 1);
    }
    Ok(())
}
...
fn dynamic_membership_can_change_while_count_stability_is_preserved() -> Result<()> {
    ...
    let first = snapshot(&[(1, Some(true)), (2, Some(true))], 3);
    let created = call(
        &store,
        &first,
        request("dynamic", &condition(Comparison::Ge, 2)),
    )?;
    ...
    assert_eq!(
        result["record"]["evaluation"]["facts"][0]["membership"],
        "dynamic_at_each_sample"
    );
    ...
    assert_eq!(
        replay["record"]["evidence_digest"],
        result["record"]["evidence_digest"]
    );
    assert_eq!(replay["record"]["evaluation_current"], false);
    Ok(())
}
...
fn count_nodes_share_existing_definition_bounds_and_strict_deserialization() -> Result<()> {
    ...
    input["query"]["condition"]
        .as_object_mut()
        .ok_or_else(|| invalid("test condition"))?
        .remove("scope");
    assert!(call(&store, &s, input).is_err());
    let group = Predicate::All {
        args: vec![predicate(); 31],
    };
    let count = Condition::EntityCount {
        scope: Scope::ObservedProjection,
        kind: Kind::Unit,
        predicate: group,
    ...
    let mut input = request("shape", &count);
    input["query"]["failure_condition"] = json!(count);
...
fn evaluation_work_is_shared_and_exhaustion_never_returns_a_partial_count() -> Result<()> {
    let s = snapshot(&[(1, Some(true)), (2, None)], 3);
    let mut probe = Probe::default();
    let mut budget = EvaluationBudget::new(60_000);
    budget.used = MAX_EVALUATION_WORK - 5;
    evaluate(
        &mut probe,
        &s,
        Kind::Unit,
    ...
    assert!(
        matches!(evaluate(&mut probe,&s,Kind::Unit,&predicate(),Comparison::Ge,1,&mut budget),
...
fn rejected_output_cannot_register_or_advance_an_aggregate_watch() -> Result<()> {
    let store = Mutex::new(Store::default());
    let s = snapshot(&[(1, Some(true))], 3);
    assert!(
        execute_in(
            &store,
            &s,
            &context(&s),
            &request("render", &condition(Comparison::Ge, 1)),
            |_| Err(bounded("injected output refusal"))
        )
        .is_err()
    );
    assert!(lock(&store)?.entries.is_empty());
    ...
    assert!(
        execute_in(
            ...
            &json!({"schema":"dfmcp.query/1","query":{
        "kind":"poll_watch","watch":created["record"]["watch"]}}),
            |_| Err(bounded("injected refusal"))
        )
        .is_err()
    );
    assert_eq!(
...
fn empty_projection_is_explicitly_not_a_complete_world_absence_proof() -> Result<()> {
    ...
    assert_eq!(evidence["scope"], "observed_projection");
    assert_eq!(evidence["population"], 0);
    assert_eq!(evidence["complete_world_count_proven"], false);
    Ok(())
}
