...
fn cells() -> Vec<Cell> {
    vec![Cell::Visible(Tile { native_tiletype: 1, shape: Shape::Floor, liquid_depth: 0,
        magma: false, traffic: 0, dig_designation: 0, building_occupancy: 0,
        unit_occupancy: 0, walkable_region: 1, temperature_1: 10015, temperature_2: 10015 }); 5]
}
fn fixture(cells: Vec<Cell>, tick: u32) -> Result<(LiveSpatialState, OperationContext)> {
    ...
    let bytes = (0..raw.len()).step_by(2).map(|i|u8::from_str_radix(&raw[i..i+2],16)
        .map_err(|_|invalid("fixture hex"))).collect::<Result<Vec<_>>>()?;
    ...
    terrain.map = MapRegion { region: Region { origin: [0,0,5], size: [5,1,1] }, cells };
    ...
    state.publish(LiveSpatialObservation::decode_payload(&bytes,7,"df".into(),"dfhack".into())?)?;
    let anchor = state.snapshot().ok_or_else(||invalid("fixture snapshot"))?.anchor();
    let context = OperationContext { session_id: SessionId::new(987654), request_id: RequestId::new(1), anchor,
        budget: WorkBudget { max_entities:100_000,max_bytes:8192,max_output_tokens:2048,max_wall_millis:60_000,
...
fn live_query_ranks_highest_impact_bottlenecks_and_reports_full_counts() -> Result<()> {
    ...
    assert_eq!(out["summary"]["components"],1); assert_eq!(out["summary"]["candidate_tiles"],5);
    assert_eq!(out["summary"]["graph_bridges"],4); assert_eq!(out["summary"]["bottlenecks"],3);
    ...
    assert_eq!(out["analysis_complete"],true); assert_eq!(out["native_captures"],0);
    assert_eq!(out["safety_proven"],false); assert_eq!(out["commit_compatible"],false);
...
fn named_landmarks_distinguish_disconnection_from_unknown_positions() -> Result<()> {
    ...
    assert_eq!(out["landmark_summary"]["all_connected_in_observed_model"],true);
    assert_eq!(out["rows"][0]["key"],"east");
    let mut split = cells(); split[2] = Cell::Hidden;
    let (s,c) = fixture(split,3)?;
    let out = call(&s,&c,&input)?;
    assert_eq!(out["landmark_summary"]["all_connected_in_observed_model"],false);
    assert_eq!(out["global_unreachability_proven"],false);
    let mut input = request("landmarks",points,128);
    input["query"]["landmarks"].as_array_mut().ok_or_else(||invalid("points"))?
        .extend([json!({"key":"hidden","position":[2,0,5]}),json!({"key":"outside","position":[0,1,5]})]);
    let out = call(&s,&c,&input)?;
    assert!(out["landmark_summary"]["all_connected_in_observed_model"].is_null());
...
fn normalized_inputs_page_without_skips_and_bind_the_capture_and_section() -> Result<()> {
    ...
    reordered["query"]["landmarks"].as_array_mut().ok_or_else(||invalid("points"))?.reverse();
    assert_eq!(call(&s,&c,&reordered)?,first);
    let mut rows = Vec::new(); let mut page = first.clone();
    for _ in 0..16 {
        rows.extend(page["rows"].as_array().ok_or_else(||invalid("rows"))?.iter().cloned());
        if page["truncated"]==false { break; }
    ...
    assert_eq!(page["truncated"],false); assert_eq!(rows.len(),10);
...
fn work_output_authority_and_expected_anchor_failures_do_not_mutate_state() -> Result<()> {
    let (s,c) = fixture(cells(),3)?; let before = s.snapshot().cloned();
    let input = request("all",json!([]),8);
    for case in 0..6 {
        let mut c = c.clone(); let mut input = input.clone();
        match case {
            0=>c.grants.clear(),1=>c.cancellation_requested=true,2=>c.budget.max_wall_millis=0,
            3=>c.budget.max_bytes=64,4=>input["query"]["max_work"]=json!(1),
            _=>input["expected_anchor"]=json!({}),
        }
        assert!(call(&s,&c,&input).is_err()); assert_eq!(s.snapshot(),before.as_ref());
    }
    Ok(())
}
...
fn empty_candidate_graph_is_not_a_global_absence_or_safety_claim() -> Result<()> {
    ...
    assert_eq!(out["rows"],json!([])); assert_eq!(out["analysis_complete"],true);
    assert_eq!(out["global_unreachability_proven"],false);
...
fn schema_and_runtime_expose_a_closed_bounded_query() -> Result<()> {
    ...
    let variants = schema["$defs"]["query"]["oneOf"].as_array().ok_or_else(||invalid("variants"))?;
    assert_eq!(variants.iter().filter(|v|v["properties"]["kind"]["const"]=="map_connectivity").count(),1);
    let valid = request("all",json!([]),8);
    for (field,value) in [("unknown",json!(true)),("section",json!("unsafe")),("limit",json!(0)),
        ("limit",json!(129)),("max_work",json!(1000001)),("max_work",json!(0)),("landmarks",Value::Null),
        ("landmarks",json!([{"key":"a","position":[0,0,5],"hidden":true}])),
        ("landmarks",json!([{"key":"a\n","position":[0,0,5]}])),
        ("landmarks",json!([{"key":"a","position":[32768,0,5]}])),
        ("landmarks",json!([{"key":"a","position":[0,0,5]},{"key":"a","position":[1,0,5]}]))] {
        let mut input = valid.clone(); input["query"][field] = value;
