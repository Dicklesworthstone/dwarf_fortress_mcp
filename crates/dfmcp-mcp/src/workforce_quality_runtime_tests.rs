...
use dfmcp_adapter::workforce_analysis::{WorkforceDemand, quality as weighted};

fn request(workers: u32) -> Value {
    let mut q = planned(workers);
    q["objective"] = json!("priority_skill_distance");
    q
}
...
fn quality_query_prefers_observed_skill_without_changing_legacy_default() -> Result<()> {
    ...
    explicit["objective"] = json!("max_filled_slots");
    let old = ask(&s, explicit)?;
    for key in ["rows", "model_digest", "optimization", "work_units"] {
        assert_eq!(old[key], legacy[key]);
    }
    assert_eq!(
        legacy["rows"][0]["citizen"]["entity_id"],
        citizen_entity_id(10).get().to_string()
    );
    let out = ask(&s, request(1))?;
    assert_eq!(out["ok"], true, "{out}");
    assert_eq!(
        out["rows"][0]["citizen"]["entity_id"],
        citizen_entity_id(11).get().to_string()
    );
...
fn priority_handles_shortage_and_global_rerouting_preserves_headcount() -> Result<()> {
    ...
    a["priority"] = json!(0);
    let mut b = demand("b", "MINING", 1);
    b["priority"] = json!(1000);
    let mut q =
        json!({"kind":"workforce_plan","objective":"priority_skill_distance","demands":[a,b]});
    let out = ask(&s, q.clone())?;
    assert_eq!(out["ok"], true, "{out}");
    assert_eq!(out["assigned_workers"], 1);
    ...
    assert_eq!(filled["assigned_workers"], 2);
    assert_eq!(filled["cut_capacity"], 2);
    assert_eq!(
        filled["rows"][0]["citizen"]["entity_id"],
        citizen_entity_id(11).get().to_string()
    );
    assert_eq!(
        filled["rows"][1]["citizen"]["entity_id"],
        citizen_entity_id(10).get().to_string()
    );
    q["demands"].as_array_mut().expect("test demands").reverse();
    let reordered = ask(&second, q)?;
    for key in ["rows", "model_digest", "optimization", "work_units"] {
        assert_eq!(reordered[key], filled[key]);
    }
    let route = decode(&fortress_query(
        second.handle(),
        None,
        Some(filled["rows"][0]["route_query"].clone()),
    ))?;
    assert_eq!(route["ok"], true, "{route}");
    assert_eq!(route["anchor"], filled["anchor"]);
    assert_eq!(
        s.calls.load(Ordering::SeqCst) + second.calls.load(Ordering::SeqCst),
        0
    );
    Ok(())
}
...
fn quality_pages_keep_active_watches_and_bind_objective_priorities_and_capture() -> Result<()> {
    ...
    loop {
        ...
        assert!(pages <= 40);
        if token.is_null() {
            token = out["continuation"].clone();
        }
        if out["continuation"].is_null() {
            break;
        }
        assert!(
            out["continuation"]
        ...
        q["continuation"] = out["continuation"].clone();
        q["limit"] = json!(3);
    ...
    changed["demands"][0]["priority"] = json!(1);
    ...
    legacy["objective"] = json!("max_filled_slots");
    legacy["demands"][0]
        .as_object_mut()
        .expect("test demand")
        .remove("priority");
    ...
    assert_eq!(
        ask(
            &s,
            json!({"kind":"cancel_watch","watch":watch["record"]["watch"]})
        )?["ok"],
        true
    );
    assert_eq!(
...
fn schema_discovers_quality_and_runtime_refuses_invalid_or_ignored_priorities() -> Result<()> {
    ...
    let variants = schema["query_schema"]["$defs"]["query"]["oneOf"]
        .as_array()
        .expect("test variants");
    let plan = variants
        .iter()
        .find(|v| v["properties"]["kind"]["const"] == "workforce_plan")
        .expect("test plan variant");
    assert!(
        plan["properties"]["objective"]["enum"]
            .as_array()
            .expect("test objectives")
            .contains(&json!("priority_skill_distance"))
    );
    for bad in [
        json!(-1),
        json!(1001),
...
fn quality_adapter_keeps_authorization_cancellation_and_priority_key_checks() -> Result<()> {
    ...
    for case in 0..4 {
        ...
        assert!(
            matches!(weighted::plan(&session.state, &c, &demands, &priorities, 1_000_000), Err(e) if e.code == expected)
        );
    }
    let unknown = BTreeMap::from([("missing".into(), 1)]);
    assert!(
        matches!(weighted::plan(&session.state, &context, &demands, &unknown, 1_000_000), Err(e) if e.code == ErrorCode::InvalidRequest)
    );
    assert_eq!(s.calls.load(Ordering::SeqCst), 0);
    Ok(())
}
