...
fn state() -> Result<LiveSpatialCitizenState> {
    let mut s = LiveSpatialCitizenState::default();
    s.publish(fixture::observation(3, 2, 7, false)?)?;
    Ok(s)
}
fn context(s: &LiveSpatialCitizenState) -> Result<OperationContext> {
    let anchor = s
        .snapshot()
        .ok_or_else(|| invalid("comparison snapshot absent"))?
        .anchor();
    Ok(OperationContext {
        session_id: SessionId::new(261),
        request_id: RequestId::new(262),
        anchor,
        budget: WorkBudget {
...
fn equal_and_incomparable_deficits_survive_without_scalar_scoring() -> Result<()> {
    ...
    assert_eq!(
        equal["summary"]["deficit_frontier"],
        json!(["demanding", "efficient"])
    );
    assert_eq!(
        equal["rows"][0]["model_digest"],
        equal["rows"][1]["model_digest"]
    );
    q["query"]["candidates"][1]["recipes"] = json!([]);
    let incomparable = execute(&s, &c, &q)?;
    assert_eq!(incomparable["summary"]["feasible_candidates"], json!([]));
    assert_eq!(
        incomparable["summary"]["deficit_frontier"],
        json!(["demanding", "efficient"])
    );
    assert_eq!(
        incomparable["rows"][1]["balances"][0]["resource"],
        "product"
    );
...
fn reordering_normalized_inputs_preserves_comparison_identity_and_frontier() -> Result<()> {
    ...
    let c = context(&s)?;
    let q = request();
    let before = execute(&s, &c, &q)?;
    let mut after = q;
    after["query"]["candidates"]
        .as_array_mut()
        .ok_or_else(|| invalid("candidates"))?
        .reverse();
    after["query"]["resources"]
        .as_array_mut()
...
fn work_budget_is_shared_by_every_candidate_and_frontier_comparison() -> Result<()> {
    ...
    let c = context(&s)?;
    let out = execute(&s, &c, &request())?;
    let used = out["work_units"]
        .as_u64()
        .ok_or_else(|| invalid("work counter"))?;
    let plans = out["rows"]
        .as_array()
        .ok_or_else(|| invalid("rows"))?
        .iter()
        .map(|r| r["planner_work_units"].as_u64().unwrap_or(0))
        .sum::<u64>();
    assert!(used > plans);
    let mut q = request();
    q["query"]["max_work"] = json!(used);
...
fn pagination_binds_every_alternative_not_only_the_current_page() -> Result<()> {
    ...
    for case in 0..4 {
        ...
        match case {
            0 => bad["query"]["candidates"][0]["recipes"][0]["job_token"] = json!("Changed"),
...
fn invalid_alternatives_never_yield_an_incomplete_frontier() -> Result<()> {
    ...
    q["query"]["candidates"] = json!(
        (0..8)
            .map(|i| json!({"key":format!("c{i}"),"recipes":recipes}))
            .collect::<Vec<_>>()
    );
    ...
    q["query"]["candidates"]
        .as_array_mut()
        .ok_or_else(|| invalid("candidates"))?
        .push(json!({"key":"c8","recipes":recipes}));
    assert!(execute(&s, &c, &q).is_err());
    Ok(())
}
