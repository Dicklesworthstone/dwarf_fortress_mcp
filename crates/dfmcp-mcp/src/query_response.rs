...
impl QueryResponseProjection {
    ...
    pub fn finish(&self, payload: Value) -> Result<String> {
        ...
        if payload
            .get("anchor")
            .is_some_and(|anchor| anchor != &self.anchor)
        {
            return Err(DfmcpError::new(
                ErrorCode::InternalInvariantViolation,
    ...
    fn render(&self, mut payload: Value) -> String {
        ...
        let interval_unknown =
            comparison && payload.get("anchor_advanced").and_then(Value::as_bool) == Some(true);
        ...
        let changes = if comparison {
            vec![
                json!({"kind":"query_endpoint_comparison", "baseline":payload.get("baseline"),
        ...
        if partial {
            ...
            if let Some(domains) = coverage
                .get_mut("partial_domains")
        ...
        if payload
            .get("record")
            .is_some_and(|record| record.get("watch").is_some())
        {
...
mod tests {
    ...
    fn projection(maximum_bytes: usize) -> QueryResponseProjection {
        QueryResponseProjection {
            ...
            uncertainty: vec![
                json!({"domain":"history","state":"unknown","reason":"gap before retained window"}),
            ],
    ...
    fn decode(text: &str) -> Result<Value> {
        serde_json::from_str(text)
            .map_err(|_| DfmcpError::new(ErrorCode::InternalInvariantViolation, "test JSON"))
    }
    ...
    fn reserved_budget_keeps_a_large_page_and_every_required_warning() -> Result<()> {
        ...
        assert!(
            serde_json::to_vec(&payload)
                .map_err(|_| DfmcpError::new(ErrorCode::InvalidRequest, "test"))?
                .len()
                <= remaining
        );
        ...
        assert_eq!(
            result["agent_turn"]["briefing"]["mutation_admissible"],
            false
        );
        assert_eq!(
        ...
        assert_eq!(
            result["agent_turn"]["coverage"]["partial_domains"][0]["domain"],
            "fortress.history"
        );
        assert_eq!(
    ...
    fn a_complete_page_does_not_hide_incomplete_source_history() -> Result<()> {
        let view = projection(8192);
        let result = decode(&view.finish(
            json!({"anchor":view.anchor,"truncated":false,"continuation":null,"rows":[]}),
        )?)?;
        assert_eq!(result["agent_turn"]["continuity"]["status"], "continuous");
        assert_eq!(result["agent_turn"]["coverage"]["status"], "partial");
        assert_eq!(
            result["agent_turn"]["coverage"]["partial_domains"]
                .as_array()
                .map(Vec::len),
            Some(1)
        );
    ...
    fn anchor_drift_is_not_papered_over_by_the_envelope() {
        let view = projection(8192);
        let mut other = view.anchor.clone();
        other["sequence"] = json!(9);
        assert!(
            matches!(view.finish(json!({"anchor":other})),Err(error) if error.code==ErrorCode::InternalInvariantViolation)
        );
    }
    ...
    fn inadequate_budget_and_oversized_cursor_fail_closed() {
        ...
        assert!(
            matches!(view.result_byte_budget(),Err(error) if error.code==ErrorCode::BudgetExceeded)
        );
        let view = projection(8192);
        assert!(
            matches!(view.finish(json!({"continuation":"x".repeat(257)})),Err(error) if error.code==ErrorCode::InternalInvariantViolation)
        );
        assert!(
            matches!(view.finish(json!({"rows":["x".repeat(8192)]})),Err(error) if error.code==ErrorCode::BudgetExceeded)
        );
    }
    ...
    fn actual_search_pages_survive_full_packet_budget_without_loss() -> Result<()> {
        ...
        use dfmcp_world::{
            EntityKind, EntityRecord, Fact, FactSource, Value as WorldValue, WorldGraph,
            WorldSnapshot,
        };
        ...
        for id in 1..=32 {
            graph.entities.insert(
                ...
                EntityRecord {
                    ...
                    label: format!("Report {id}"),
                    fields: BTreeMap::from([(
                        "text".to_owned(),
                        Fact::known(
                            WorldValue::Text("flood at workshop".to_owned()),
                            GameTick(3),
                            FactSource::DfhackField("announcement.text".to_owned()),
        ...
        context.budget.max_bytes = u64::try_from(view.result_byte_budget()?)
            .map_err(|_| DfmcpError::new(ErrorCode::InternalInvariantViolation, "test budget"))?;
        ...
        loop {
            ...
            assert_eq!(
                response["agent_turn"]["briefing"]["runtime_admitted"],
                false
            );
            assert_eq!(
                response["agent_turn"]["attention"][0]["finding"],
                "retain this hazard"
            );
            assert_eq!(
                response["agent_turn"]["uncertainty"][0]["domain"],
                "history"
            );
            ...
            pages += 1;
            assert!(pages <= 32);
            if response["continuation"].is_null() {
                break;
            }
            assert_eq!(
                response["continuation"],
                response["agent_turn"]["coverage"]["continuation"]
            );
