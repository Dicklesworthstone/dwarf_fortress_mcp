...
pub fn digest(raw: &str) -> Result<Digest32> {
    ...
    for (i, pair) in raw.as_bytes().chunks_exact(2).enumerate() {
        ...
        out[i] = u8::from_str_radix(part, 16).map_err(|_| error(ErrorCode::InvalidRequest, "invalid digest"))?;
    }
    Ok(Digest32::from_bytes(out))
}
pub fn mode_name(mode: WorkforceMode) -> &'static str { match mode {
    WorkforceMode::Control => "control", WorkforceMode::Recover => "recover", WorkforceMode::Offline => "offline",
} }
fn identity(c: &WorkforceCapture) -> Value { json!({
...
fn citizen(c: &Citizen, masks: bool) -> Value {
    ...
    if masks { out["labor_mask"] = json!(c.labors()); }
    out
}
pub fn capture_summary(c: &WorkforceCapture) -> Value { json!({
...
fn detail(index: usize, d: &Detail, members: bool) -> Value {
    ...
    if members { out["native_member_ids"] = json!(d.members()); }
    out
}
pub fn details_page(c: &WorkforceCapture, offset: usize, limit: usize) -> Result<Value> {
    if !(1..=MAX_PAGE).contains(&limit) || offset > c.details().len() {
        return Err(error(ErrorCode::InvalidRequest, "detail page is outside this exact capture"));
    }
    let end = (offset + limit).min(c.details().len());
    let next = (end < c.details().len()).then(|| json!({"mode":"details","witness":c.witness().to_string(),"offset":end,"limit":limit}));
...
pub fn record_summary(r: &AssignmentRecord) -> Value { json!({
...
pub fn record_detail(r: &AssignmentRecord) -> Value {
    let p = r.plan(); let c = p.before(); let index = p.spec().detail() as usize;
    let changed: Vec<u32> = c.details().get(index).map_or_else(Vec::new, |d| c.citizens().iter().map(Citizen::id)
        .filter(|id| d.members().binary_search(id).is_ok() != p.spec().assigned()).collect());
...
pub fn summary(v: &WorkforceView) -> Value { json!({"journal_id":v.id.to_string(),"head":v.head.to_string(),"events":v.events,
...
pub fn failure(e: &dfmcp_core::DfmcpError) -> Value { json!({"ok":false,"error":{"code":e.code.as_str(),"message":e.message},
...
impl Filter {
    pub fn matches(self, r: &AssignmentRecord) -> bool { match self {
        Self::All => true, Self::Pending => !r.state().settled(), Self::Unresolved => r.state().unresolved(), Self::Terminal => r.state().settled(),
    } }
}
struct Cursor { session: SessionId, journal: Digest32, head: Digest32, filter: Filter, limit: usize, offset: usize }
#[derive(Default)]
pub struct Cursors { serial: u64, values: VecDeque<(String, Cursor)> }
impl Cursors {
    pub fn issue(&mut self, session: SessionId, view: &WorkforceView, filter: Filter, limit: usize, offset: usize) -> Result<String> {
        self.serial = self.serial.checked_add(1).ok_or_else(super::exhausted)?;
        let mut bytes = b"dfmcp-workforce-mcp-cursor/1\0".to_vec(); bytes.extend_from_slice(&session.get().to_be_bytes());
        ...
        if self.values.len() == 64 { self.values.pop_front(); }
        self.values.push_back((token.clone(), Cursor { session, journal: view.id, head: view.head, filter, limit, offset })); Ok(token)
    }
    pub fn resolve(&self, token: &str, session: SessionId, view: &WorkforceView, filter: Filter, limit: usize) -> Result<usize> {
        digest(token)?;
        let (_, c) = self.values.iter().find(|(t, _)| t == token)
            .ok_or_else(|| error(ErrorCode::StaleAnchor, "workforce continuation expired; start a new first page"))?;
        if c.session != session || c.journal != view.id || c.head != view.head || c.filter != filter || c.limit != limit {
            return Err(error(ErrorCode::StaleAnchor, "workforce continuation belongs to another session, journal head or selection"));
        }
        Ok(c.offset)
    }
}
pub fn packet(op: &str, result: Value, context: Option<&OperationContext>, mode: Option<WorkforceMode>, view: Option<&WorkforceView>) -> String {
    let phase = match op { "fortress.open_session" => AgentPhase::Bootstrap, "fortress.plan" => AgentPhase::Propose,
    ...
    active["scope"] = json!("this_workforce_journal_only"); active["inventory_verified"] = json!(view.is_some());
    active["pending_absence_proven"] = json!(view.is_some_and(|v| v.records.iter().all(|r| r.state().settled())));
    if let Some(v) = view {
        let refs = v.records.iter().filter(|r| !r.state().settled()).take(4).map(record_summary).collect::<Vec<_>>();
        ...
        active["omitted_pending_references"] = json!(v.records.iter().filter(|r| !r.state().settled()).count().saturating_sub(4));
    }
