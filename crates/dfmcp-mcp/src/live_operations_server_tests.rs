...
struct Script {
    observations: VecDeque<LiveOperationsObservation>,
    fenced: bool,
    ...
    let bytes = (0..hex.len())
        ...
        .map(|i| {
            u8::from_str_radix(&hex[i..i + 2], 16)
                .map_err(|_| error(ErrorCode::InternalInvariantViolation, "bad test hex"))
        })
...
fn register(tokens: u32, granted: Vec<Capability>) -> Result<Registered> {
    ...
    second.jobs.year_tick += 1;
    second.items[0].flags |= 1;
    second.items[0].stack_size = 8;
    second.buildings[0].build_stage = 2;
    ...
    let limits = OperationsLimits::default();
    let session = OperationsSession {
        id,
        source: Box::new(Script {
            observations: VecDeque::from([second]),
            fenced: false,
...
fn decode(raw: &str) -> Result<Value> {
    serde_json::from_str(raw)
        .map_err(|_| error(ErrorCode::InternalInvariantViolation, "bad test JSON"))
}
...
fn connected_operations_queries_inspect_real_items_buildings_and_paths() -> Result<()> {
    let session = full()?;
    let items = decode(&fortress_query(
        session.handle(),
        Some("items".to_owned()),
        None,
    ))?;
    ...
    assert_eq!(
        items["rows"][0]["fields"]["stack_size"]["value"]["value"],
        5
    );
    assert_eq!(
        items["rows"][0]["fields"]["forbidden"]["value"]["value"],
        false
    );
    let building = ask(
        &session,
        json!({"kind":"inspect","entity_id":building_entity_id(20).to_string(),
        "generation":1,"fields":["build_stage","max_build_stage"]}),
    )?;
    assert_eq!(building["ok"], true);
    assert_eq!(
        building["row"]["fields"]["build_stage"]["value"]["value"],
        1
    );
    let path = ask(
        &session,
        json!({"kind":"traverse","roots":["9"],"edge_kinds":["uses","contained_in"],
    ...
    assert_eq!(
        ...
        json!([
            "9",
            item_entity_id(30).to_string(),
            item_entity_id(31).to_string()
        ])
    );
    assert_eq!(session.calls.load(Ordering::SeqCst), 0);
    let schema = decode(&fortress_query(
        session.handle(),
...
fn native_inventory_baseline_and_construction_watch_share_one_refresh() -> Result<()> {
    ...
    let captured = ask(
        &session,
        json!({"kind":"capture","key":"inventory","max_game_ticks":100,
        "select":{"kind":"entities","kinds":["item"],"fields":["stack_size","forbidden"]}}),
    )?;
    assert_eq!(captured["ok"], true);
    let watch = ask(
        &session,
        json!({"kind":"watch","key":"building-stage","label":"Observe construction stage 2",
    ...
    let changed = ask(
        &session,
        json!({"kind":"changes","baseline":captured["captured"]["baseline"]}),
    )?;
...
fn source_failure_preserves_anchor_and_permits_local_watch_cleanup() -> Result<()> {
    ...
    assert_eq!(
        decode(&fortress_doctor(session.handle()))?["status"],
        "source_fenced"
    );
    Ok(())
}
...
fn missing_authority_and_foreign_handles_never_reach_observation() -> Result<()> {
    ...
    for raw in [
        (1u128 << 127) | FAMILY | 1,
        SessionId::new((1u128 << 127) | (1u128 << 60) | 1).get(),
        0,
    ] {
        assert!(resolve(Some(format!("{raw:032x}"))).is_err());
    }
    let malformed = decode(&fortress_query(
        session.handle(),
        None,
        Some(json!({"schema":"dfmcp.query/1",
        "query":{"kind":"await_watch","watch":format!("watch:{}","0".repeat(64))}})),
    ))?;
    assert_eq!(malformed["ok"], false);
    assert_eq!(session.calls.load(Ordering::SeqCst), 0);
    assert_eq!(
        decode(&fortress_commit(session.handle()))?["error"]["code"],
        "capability_denied"
    );
