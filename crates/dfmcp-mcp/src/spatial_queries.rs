...
fn validate(input: &Value) -> Result<()> {
    ...
    while let Some((v, depth)) = pending.pop() {
        ...
        if depth > 12 || nodes > 4096 {
            return Err(budget("spatial query shape exceeds bounds"));
        }
        match v {
            Value::String(s) => bytes = bytes.saturating_add(s.len()),
            Value::Array(a) => {
                if nodes + pending.len() + a.len() > 4096 {
                    return Err(budget("spatial query has too many nodes"));
                }
        ...
        if bytes > 65536 {
            return Err(budget("spatial query input exceeds 64 KiB"));
        }
    }
    Ok(())
}
fn identity(c: &OperationContext, source: Digest32, query: Value) -> Digest32 {
    let mut b = b"dfmcp-spatial-query-v1\0".to_vec();
    b.extend_from_slice(&c.session_id.get().to_be_bytes());
    b.extend_from_slice(anchor_json(c.anchor).to_string().as_bytes());
    b.extend_from_slice(source.as_bytes());
    b.extend_from_slice(ROUTE_POLICY.as_bytes());
    b.extend_from_slice(SPATIAL_SUPPLY_POLICY.as_bytes());
    b.extend_from_slice(query.to_string().as_bytes());
    Digest32::of_bytes(&b)
}
fn token(offset: usize, digest: Digest32) -> String {
    let mut b = b"dfmcp-spatial-page-v1\0".to_vec();
    b.extend_from_slice(digest.as_bytes());
    b.extend_from_slice(&(offset as u64).to_be_bytes());
...
fn offset(raw: Option<&str>, digest: Digest32, count: usize) -> Result<usize> {
    ...
    if raw.len() > 128 {
        return Err(budget("spatial cursor too large"));
    }
    ...
    let n = p[1]
        .parse::<usize>()
        .map_err(|_| invalid("invalid spatial offset"))?;
    if raw != token(n, digest) {
        return Err(DfmcpError::new(
            ErrorCode::StaleAnchor,
            "spatial continuation names another session, query or capture",
        ));
    }
    if n >= count {
        return Err(DfmcpError::new(
            ErrorCode::CursorGap,
            "spatial continuation is beyond the result",
        ));
    }
    Ok(n)
}
fn paginate(
    ...
    if !(1..=128).contains(&limit) {
        return Err(budget("spatial page limit must be 1..128"));
    }
    ...
    .map_err(|_| budget("output budget overflow"))?;
    out["analysis_digest"] = json!(id.to_string());
    ...
    while end < count && end - first < limit as usize {
        ...
        candidate["rows"]
            .as_array_mut()
            .ok_or_else(|| invalid("spatial rows absent"))?
            .push(row(end)?);
        candidate["returned"] = json!(end + 1 - first);
    ...
    if (first == end && first < count) || out.to_string().len() > maximum {
        return Err(budget(
            "one spatial row or its summary cannot fit the complete response budget",
        ));
    }
    Ok(out)
}
...
pub(super) fn execute<T: SpatialStateView>(
    ...
    let envelope: Envelope = serde_json::from_value(input.clone())
        .map_err(|_| invalid("invalid spatial query fields"))?;
    if envelope.schema != "dfmcp.query/1" {
        return Err(invalid("spatial query requires dfmcp.query/1"));
    }
    let snapshot = state
        .snapshot()
        .ok_or_else(|| invalid("spatial snapshot absent"))?;
    if snapshot.anchor() != c.anchor
        || envelope
            .expected_anchor
            .is_some_and(|a| a != anchor_json(c.anchor))
    {
        return Err(DfmcpError::new(
            ErrorCode::StaleAnchor,
            "spatial query anchor differs",
        ));
    }
    if snapshot.graph.entities.len() > c.budget.max_entities as usize {
        return Err(budget("spatial query scan exceeds session budget"));
    }
    ...
    match envelope.query {
        ...
        Query::MapRoute {
            ...
            let map = &state
                .spatial_observation()
                .ok_or_else(|| invalid("spatial source absent"))?
                .terrain()
                .map;
            ...
            let id = identity(
                c,
                source,
                json!({"kind":"map_route","start":start,"goal":goal,"max_work":max_work}),
            );
            let mut out = base(c, source, "map_route");
            out["status"] = json!(if route.endpoint_excluded {
                "endpoint_excluded"
            } else if route.path.is_empty() {
                "no_route_in_observed_model"
            } else {
                "candidate_found"
            });
            out["model_steps"] = json!(route.path.len().checked_sub(1));
            out["visited_tiles"] = json!(route.visited_tiles);
            out["work_units"] = json!(route.work_units);
            out["touched_region_boundary"] = json!(route.touched_region_boundary);
            paginate(
                out,
                route.path.len(),
                ...
                id,
                c,
                |i| Ok(json!({"position":route.path[i]})),
            )
        }
        Query::SpatialInventoryPlan {
            origin,
            ...
            let max_work = max_work.unwrap_or(10_000_000);
            ...
            let id = identity(
                c,
                source,
                json!({"kind":"spatial_inventory_plan","origin":origin,"demands":normalized,"max_work":max_work}),
            );
            let mut out = base(c, source, "spatial_inventory_plan");
            out["origin"] = json!(origin);
            out["supply_policy"] = json!(SPATIAL_SUPPLY_POLICY);
            out["quantity_unit"] = json!("stack_units");
            out["model_feasible"] = json!(report.allocation.shortage.is_none());
            ...
            out["interpretation"] = json!(
                "Conditional on declared interchangeable stack units, conservative item policy and this bounded observed route model; not native job feasibility or proof that excluded supply is globally inaccessible."
            );
            ...
            paginate(
                ...
                c,
                |i| {
                    let a = &report.allocation.assignments[i];
                    let location = report
                        .locations
                        .get(&a.supply_id)
                        .ok_or_else(|| invalid("allocation location absent"))?;
                    Ok(
                        json!({"demand_key":report.demands[a.demand_index].key,"units":a.units,
...
pub(super) fn schema() -> Result<Value> {
    let mut base: Value = serde_json::from_str(include_str!("../../../schemas/mcp_query_v1.json"))
        .map_err(|_| invalid("base schema invalid"))?;
    let mut route: Value =
        serde_json::from_str(include_str!("../../../schemas/mcp_map_route_v1.json"))
            .map_err(|_| invalid("map schema invalid"))?;
    route["properties"]["limit"]["maximum"] = json!(128);
    route["properties"]["continuation"]["oneOf"][1]["pattern"] =
        json!("^sp1:[1-9][0-9]*:[0-9a-f]{64}$");
    let inventory: Value = serde_json::from_str(include_str!(
        "../../../schemas/mcp_spatial_inventory_v1.json"
    ))
    .map_err(|_| invalid("spatial schema invalid"))?;
    let blueprint: Value = serde_json::from_str(include_str!(
        "../../../schemas/mcp_spatial_blueprint_v1.json"
    ))
    .map_err(|_| invalid("blueprint schema invalid"))?;
    let connectivity: Value = serde_json::from_str(include_str!(
