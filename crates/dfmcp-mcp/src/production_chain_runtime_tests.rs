...
impl Source for Script {
    fn read(&mut self, _: Duration) -> Result<LiveSpatialCitizenObservation> {
        ...
        self.values
            .pop_front()
            .ok_or_else(|| error(ErrorCode::AdapterUnavailable, "chain fixture exhausted"))
    }
...
impl Drop for Registered {
    fn drop(&mut self) {
        let _ = fortress_cancel(self.handle(), Some("session".into()), Some(true));
    }
}
fn register(tokens: u32) -> Result<Registered> {
    ...
    let anchor = state
        .snapshot()
        .ok_or_else(|| error(ErrorCode::InvalidRequest, "chain snapshot absent"))?
        .anchor();
    let id = next_id()?;
    ...
    let session = Session {
        ...
        source: Box::new(Script {
            values: VecDeque::from([fixture::observation(4, 2, 8, false)?]),
            calls: calls.clone(),
            fenced: false,
        }),
        budget: WorkBudget {
...
fn live_chain_query_exposes_dependencies_and_deficits_without_game_reads_or_writes() -> Result<()> {
    ...
    assert_eq!(out["summary"]["modeled_steps"], 2);
    ...
    assert!(
        schema["query_schema"]["$defs"]["query"]["oneOf"]
            .as_array()
            .ok_or_else(|| error(ErrorCode::InvalidRequest, "chain schema"))?
            .iter()
...
fn paginated_chain_keeps_current_watch_evidence_and_complete_summary() -> Result<()> {
    ...
    let watch = ask(
        &s,
        envelope(json!({"kind":"watch","key":"chain-review",
    ...
    for _ in 0..8 {
        ...
        assert_eq!(page["model_digest"], first["model_digest"]);
        assert_eq!(
        ...
        rows.extend(
            page["rows"]
                .as_array()
                .ok_or_else(|| error(ErrorCode::InvalidRequest, "chain rows"))?
                .iter()
                .cloned(),
        );
    ...
    let current = ask(&s, query())?;
    assert_eq!(current["ok"], true, "{current}");
    assert_eq!(current["model_feasible"], true);
    assert_eq!(current["observed_quotas_met"], false);
    Ok(())
}
