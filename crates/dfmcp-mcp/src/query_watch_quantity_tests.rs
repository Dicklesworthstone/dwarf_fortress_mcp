...
fn fixture(rows: &[(Option<bool>, Option<u64>)]) -> (WorldSnapshot, OperationContext) {
    ...
    let known = |value| {
        Fact::known(
            value,
            tick,
            FactSource::DfhackField("item.test".into()),
            Digest32::of_bytes(b"quantity fixture"),
        )
    };
    let unknown = || {
        Fact::with_presence(
            FactPresence::Omitted("not observed".into()),
            tick,
            FactSource::Replay,
            Digest32::ZERO,
        )
        ...
        graph.entities.insert(
            ...
            EntityRecord {
                ...
                fields: BTreeMap::from([
                    (
                        "keep".into(),
                        keep.map_or_else(unknown, |value| known(WorldValue::Bool(value))),
                    ),
                    (
                        "stack_size".into(),
                        units.map_or_else(unknown, |value| known(WorldValue::U64(value))),
                    ),
                ]),
            },
        );
    ...
    let context = OperationContext {
        session_id: SessionId::new(912_002),
...
fn quantity_sums_units_not_records_and_excludes_other_entity_kinds() -> Result<()> {
    ...
    other.id = EntityId::new(200);
    other.kind = EntityKind::Building;
    snapshot.graph.entities.insert(other.id, other);
    snapshot.refresh_hash();
    let result = scan(&snapshot)?;
    assert_eq!(result.population, 3);
...
fn uncertain_membership_and_unknown_quantity_have_different_sound_bounds() -> Result<()> {
    ...
    assert_eq!(
        unbounded.bounds.compare(Comparison::Le, u64::MAX),
        Truth::Unknown
    );
...
fn zero_sized_unknown_membership_does_not_make_the_quantity_unknown() -> Result<()> {
    ...
    assert_eq!(result.unknown_membership, 1);
    assert_eq!(result.unknown_quantity, 0);
...
fn unavailable_or_wrongly_typed_quantities_never_leak_their_backing_values() -> Result<()> {
    ...
    for case in 0..9 {
        ...
        let fact = snapshot
            .graph
            .entities
            .get_mut(&EntityId::new(100))
            .ok_or_else(|| invalid("fixture item"))?
            .fields
            .get_mut("stack_size")
            .ok_or_else(|| invalid("fixture quantity"))?;
        match case {
            ...
            6 => {
                fact.presence = None;
                fact.value = WorldValue::I64(-7);
            }
            7 => {
                fact.presence = None;
                fact.value = WorldValue::Text("7".into());
            }
            _ => {
                fact.presence = None;
                fact.value = WorldValue::I64(7);
...
fn overflow_and_work_exhaustion_emit_no_partial_quantity_evidence() -> Result<()> {
    for rows in [
        [(Some(true), Some(u64::MAX)), (Some(true), Some(1))],
        [(None, Some(u64::MAX)), (None, Some(1))],
    ] {
        let (snapshot, _) = fixture(&rows);
        let mut probe = Probe::default();
        assert!(
            matches!(probe.evaluate(&condition(Comparison::Ge,1),&snapshot),Err(e) if e.code==ErrorCode::BudgetExceeded)
        );
        assert!(probe.facts.is_empty());
    }
    let (snapshot, _) = fixture(&[(Some(true), Some(9))]);
    let mut budget = EvaluationBudget::new(60000);
    budget.used = MAX_EVALUATION_WORK - 1;
    let mut probe = Probe::default();
    assert!(
        matches!(evaluate(&mut probe,&snapshot,&selector(),Comparison::Ge,1,&mut budget),Err(e) if e.code==ErrorCode::BudgetExceeded)
    );
    assert!(probe.facts.is_empty());
    Ok(())
}
...
fn inspection_and_watch_use_identical_measurements_without_registry_changes() -> Result<()> {
    ...
    let mut probe = Probe::default();
    assert_eq!(
        probe.evaluate(&condition(Comparison::Ge, 8), &snapshot)?,
        Truth::Unknown
    );
    let mut measured = probe.facts.remove(0);
    for key in ["op", "comparison", "threshold", "truth"] {
        measured
            .as_object_mut()
            .ok_or_else(|| invalid("fact"))?
            .remove(key);
    }
    assert_eq!(measured, inspected["quantity"]);
    assert_eq!(inspected["watch_registered"], false);
    assert_eq!(inspected["watch_evaluated"], false);
...
fn query_refusals_preserve_authority_identity_and_complete_output_bounds() -> Result<()> {
    ...
    for case in 0..5 {
        ...
        match case {
            ...
            2 => denied.budget.max_entities = 0,
            3 => denied.budget.max_bytes = 1,
            _ => denied.budget.max_wall_millis = 0,
        }
        assert!(query(&snapshot, &denied, &request()).is_err());
    }
    for change in [
        json!({"quantity_unit":"portions"}),
        json!({"scope":"complete_world"}),
        json!({"limit":1}),
    ] {
...
fn quantity_failure_guard_and_render_errors_do_not_publish_a_false_success() -> Result<()> {
    ...
    let input = register(
        Condition::Paused { value: true },
        Some(condition(Comparison::Lt, 3)),
    );
    ...
    assert!(
        execute_in(&rejected, &snapshot, &context, &input, |_| Err(bounded(
            "injected output refusal"
        )))
        .is_err()
    );
...
fn quantity_and_count_predicates_share_the_same_definition_budget() -> Result<()> {
    ...
    let mut definition = Definition {
        ...
        condition: Condition::All {
            args: vec![condition(Comparison::Ge, 1), condition(Comparison::Ge, 2)],
        },
    ...
    definition.condition = Condition::ItemQuantity {
        scope: Scope::ObservedProjection,
        quantity_unit: QuantityUnit::StackUnits,
