...
use super::{
    Definition, MAX_PER_SESSION, MAX_TOTAL, Status, Store, WATCHES, Watch, active_work, anchor,
    authorize, bounded, digest, failure, validate_definition, validate_handle,
};
use dfmcp_adapter::operations_journal::PrivateJournalFile;
use dfmcp_core::{
    Capability, Digest32, ErrorCode, FortressId, GameTick, ObservationCursor, OperationContext,
...
impl From<StateAnchor> for SavedAnchor {
    fn from(a: StateAnchor) -> Self {
        Self {
            fortress: a.fortress_id.get(),
            epoch: a.cursor.epoch,
            sequence: a.cursor.sequence,
            tick: a.tick.0,
            hash: a.state_hash.to_string(),
        }
    }
}
impl SavedAnchor {
    fn decode(&self) -> Result<StateAnchor> {
        Ok(StateAnchor {
            fortress_id: FortressId::new(self.fortress),
            cursor: ObservationCursor {
                epoch: self.epoch,
                sequence: self.sequence,
            },
            tick: GameTick(self.tick),
            state_hash: parse_digest(&self.hash)?,
        })
    }
}
...
impl From<&Watch> for SavedWatch {
    fn from(w: &Watch) -> Self {
        Self {
            ...
            last_seen: w.last_seen.into(),
            last_sample_tick: w.last_sample_tick,
            streak: w.streak,
            samples: w.samples,
            status: w.status.text().to_owned(),
            evaluation: w.evaluation.clone(),
            evidence_digest: w.evidence_digest.to_string(),
            recovery: w.recovery.clone(),
        }
    }
}
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
...
struct SavedSet {
    schema: String,
    profile: String,
    checkpoint_anchor: SavedAnchor,
    watches: Vec<SavedWatch>,
}
impl SavedSet {
    fn capture(store: &Store, context: &OperationContext) -> Self {
        Self {
            ...
            checkpoint_anchor: context.anchor.into(),
            watches: store
                .entries
                .iter()
                .filter(|((session, _), _)| *session == context.session_id)
                .map(|(_, w)| SavedWatch::from(w))
                .collect(),
        }
    }
}
...
fn bounded_json(bytes: &[u8]) -> Result<Value> {
    if bytes.len() > checkpoint::MAX_PAYLOAD {
        return Err(bounded("watch checkpoint exceeds its byte limit"));
    }
    let value: Value =
        serde_json::from_slice(bytes).map_err(|_| corrupt("invalid watch checkpoint JSON"))?;
    let mut pending = vec![(&value, 0usize)];
    let mut visited = 0usize;
    while let Some((node, depth)) = pending.pop() {
        visited += 1;
        if depth > 32 || visited > 32768 {
            return Err(corrupt("watch checkpoint JSON shape exceeds its bound"));
        }
        match node {
            Value::Array(array) => {
                if visited + pending.len() + array.len() > 32768 {
                    return Err(corrupt("watch checkpoint array too wide"));
                }
                pending.extend(array.iter().map(|node| (node, depth + 1)));
            }
            Value::Object(map) => {
                if visited + pending.len() + map.len() > 32768 {
                    return Err(corrupt("watch checkpoint object too wide"));
                }
                if map.keys().any(|key| key.contains('\0')) {
                    return Err(corrupt("NUL in watch checkpoint key"));
                }
                pending.extend(map.values().map(|node| (node, depth + 1)));
            }
            Value::String(text) if text.contains('\0') => {
                return Err(corrupt("NUL in watch checkpoint text"));
            }
            Value::Number(number) if !number.is_i64() && !number.is_u64() => {
                return Err(corrupt("non-integral watch checkpoint number"));
            }
            _ => {}
        }
    }
...
fn decode_saved(bytes: &[u8], anchors: &BTreeMap<StateAnchor, usize>) -> Result<SavedSet> {
    let set: SavedSet = serde_json::from_value(bounded_json(bytes)?)
        .map_err(|_| corrupt("invalid watch checkpoint fields"))?;
    ...
    let checkpoint_index = *anchors
        .get(&current)
        .ok_or_else(|| corrupt("watch checkpoint anchor is absent from its observation archive"))?;
    let mut handles = BTreeSet::new();
    let mut keys = BTreeSet::new();
    let mut previous = None;
    for w in &set.watches {
        validate_handle(&w.handle).map_err(|_| corrupt("invalid durable watch handle"))?;
        validate_definition(&w.definition)
            .map_err(|_| corrupt("invalid durable watch definition"))?;
        parse_digest(&w.evidence_digest)?;
        let state = status(&w.status)?;
        let created = w.created_at.decode()?;
        let last = w.last_seen.decode()?;
        let created_index = *anchors
            .get(&created)
            .ok_or_else(|| corrupt("watch creation observation is not retained"))?;
        let last_index = *anchors
            .get(&last)
            .ok_or_else(|| corrupt("watch evidence observation is not retained"))?;
        if created.fortress_id != current.fortress_id
            || last.fortress_id != current.fortress_id
            || created_index > last_index
            || last_index > checkpoint_index
            || !handles.insert(w.handle.as_str())
            || !keys.insert(w.definition.key.as_str())
            || previous.is_some_and(|p: &str| p >= w.handle.as_str())
            || !w.evaluation.is_object()
            || w.definition.deadline_tick <= created.tick.0
            || w.streak > w.definition.stable_observations
            || u64::from(w.streak) > w.samples
        ...
        {
            return Err(corrupt(
                "inconsistent durable watch identity, order, evidence or counters",
            ));
        }
        if let Some(recovery) = &w.recovery {
            validate_handle(&recovery.prior_watch)
                .map_err(|_| corrupt("invalid prior watch handle"))?;
            parse_digest(&recovery.prior_evidence_digest)?;
...
static JOURNALS: LazyLock<Mutex<BTreeMap<SessionId, Entry>>> =
    LazyLock::new(|| Mutex::new(BTreeMap::new()));
fn journals() -> Result<std::sync::MutexGuard<'static, BTreeMap<SessionId, Entry>>> {
    JOURNALS.lock().map_err(|_| {
        failure(
            ErrorCode::InternalInvariantViolation,
            "watch journal registry poisoned",
        )
    })
}
fn binding(archive: Digest32, fortress: FortressId) -> Digest32 {
    ...
    bytes.extend_from_slice(archive.as_bytes());
    bytes.extend_from_slice(&fortress.get().to_be_bytes());
    ...
    F: FnOnce(Value) -> Result<String>,
{
    ...
    if context.anchor.fortress_id != entry.fortress {
        return Err(corrupt("watch request names another fortress"));
    }
    ...
    let changed = entry
        .saved
        .as_ref()
        .is_none_or(|old| old.watches != candidate.watches);
    let pending = if changed {
        let bytes =
            serde_json::to_vec(&candidate).map_err(|_| corrupt("watch state cannot be encoded"))?;
...
pub(crate) struct WatchJournalGuard {
    session: SessionId,
}
impl Drop for WatchJournalGuard {
    fn drop(&mut self) {
        if let Ok(mut store) = WATCHES.lock() {
            store.entries.retain(|(id, _), _| *id != self.session);
        }
        if let Ok(mut registry) = JOURNALS.lock() {
...
fn recover_watch(
    ...
    let identity = digest(
        &json!({"domain":"dfmcp-recovered-watch/1","session":context.session_id.to_string(),
        "serial":serial,"prior_watch":saved.handle,"checkpoint":checkpoint.to_string()}),
    )?;
    let mut watch = Watch {
        handle: format!("watch:{identity}"),
        definition: saved.definition.clone(),
        created_at: saved.created_at.decode()?,
        last_seen: saved.last_seen.decode()?,
        last_sample_tick: saved.last_sample_tick,
        streak: saved.streak,
        samples: saved.samples,
        status: status(&saved.status)?,
        evaluation: saved.evaluation.clone(),
        evidence_digest: prior,
        recovery: Some(Recovery {
            prior_watch: saved.handle.clone(),
            prior_evidence_digest: saved.evidence_digest.clone(),
            prior_checkpoint_digest: checkpoint.to_string(),
            continuity_proven: false,
            restart_count: saved
                .recovery
                .as_ref()
                .map_or(0, |r| r.restart_count)
                .checked_add(1)
    ...
    if !watch.status.terminal() {
        ...
        {
            return Err(bounded(
                "recovered watch exceeds the current session's game-tick horizon",
            ));
        }
        watch.status = if incompatible {
            Status::Invalidated
        } else if context.anchor.tick.0 >= watch.definition.deadline_tick {
            Status::Expired
        } else {
            Status::BlockedUnknown
        };
        watch.streak = 0;
        watch.last_sample_tick = None;
    ...
    F: FnOnce(Value) -> Result<String>,
{
    ...
    if archive == Digest32::ZERO
        || observations.is_empty()
        || observations.len() > 4096
        || observations.last() != Some(&context.anchor)
    {
        return Err(corrupt(
            "durable watches require the exact current spatial/1.8 observation archive",
        ));
    }
    let anchors: BTreeMap<_, _> = observations
        .iter()
        .copied()
        .enumerate()
        .map(|(i, a)| (a, i))
        .collect();
    if anchors.len() != observations.len()
        || observations
            .iter()
            .any(|a| a.fortress_id != context.anchor.fortress_id)
    {
        return Err(corrupt(
            "watch observation archive has duplicate or foreign anchors",
        ));
    }
    ...
    if registry.contains_key(&context.session_id)
        || store
            .entries
            .keys()
            .any(|(id, _)| *id == context.session_id)
    {
        return Err(failure(
            ErrorCode::Conflict,
            "attach durable watches before creating session-local watches",
        ));
    }
    let (file, created) = PrivateJournalFile::open(path, context)?;
    let mut saved = None;
    let journal = checkpoint::Journal::open(
        file,
        context,
        binding(archive, context.anchor.fortress_id),
        created,
        |bytes| {
            saved = Some(decode_saved(bytes, &anchors)?);
            Ok(())
        },
    )?;
    let restored = saved.as_ref().map_or(0, |s| s.watches.len());
    if store.entries.len().saturating_add(restored) > MAX_TOTAL {
        return Err(bounded("global watch retention full"));
    }
    ...
    if let Some(saved) = &saved {
        for old in &saved.watches {
            ...
            candidate
                .entries
                .insert((context.session_id, recovered.handle.clone()), recovered);
        }
    }
    let mut entry = Entry {
        journal,
    ...
    let encoded = write_candidate(&mut entry, &candidate, context, value, |value| {
        super::publish_plain(context, value, publish)
    })?;
    *store = candidate;
