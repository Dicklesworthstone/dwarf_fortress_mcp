...
fn world(tick: u64, sequence: u64, values: &[(u64, u32, i64)]) -> WorldSnapshot {
    ...
    for &(id, generation, value) in values {
        let fact = Fact::known(
            WorldValue::I64(value),
            GameTick(tick),
            FactSource::DfhackField("unit.test_value".into()),
            Digest32::of_bytes(&tick.to_be_bytes()),
        );
...
fn selection() -> Value {
    json!({"kind":"entities","kinds":["unit"],"fields":["value"]})
}
fn compare(before: &WorldSnapshot, after: &WorldSnapshot) -> Result<Value> {
    compare_endpoints(
        ...
        &context(after),
        &selection(),
        Digest32::of_bytes(b"archive-pair"),
        None,
        None,
    )
}
...
fn additions_departures_changes_and_recycled_ids_are_separate_ordered_records() -> Result<()> {
    ...
    assert_eq!(
        ...
        json!({"changed_in_result":1,"entered_result":2,"left_result":2})
    );
    let rows = result["changes"]
        .as_array()
        .ok_or_else(|| invalid("test changes absent"))?;
    assert_eq!(
        rows.iter()
            .map(|r| (
                r["entity_id"].as_str(),
            ...
            .collect::<Vec<_>>(),
        vec![
            (Some("1"), Some(1), Some("changed_in_result")),
            (Some("2"), Some(1), Some("left_result")),
            (Some("3"), Some(1), Some("entered_result")),
            (Some("4"), Some(1), Some("left_result")),
            (Some("4"), Some(2), Some("entered_result"))
        ]
    );
    assert_eq!(result["baseline_created"], false);
    assert_eq!(
        result["coverage"]["intermediate_observations_evaluated"],
        false
    );
    Ok(())
}
...
fn observation_bookkeeping_is_disclosed_without_manufacturing_value_changes() -> Result<()> {
    ...
    assert_eq!(result["provenance_only_refreshes"], 2);
...
fn changed_presence_or_source_is_not_hidden_by_equal_underlying_values() -> Result<()> {
    ...
    for case in 0..5 {
        ...
        let fact = after
            ...
            .and_then(|e| e.fields.get_mut("value"))
...
fn leaving_a_filtered_selection_is_not_labeled_deletion() -> Result<()> {
    ...
    select["where"] =
        json!({"op":"compare","field":"value","comparison":"gt","value":{"type":"i64","value":5}});
    let result = compare_endpoints(
        &before,
        &after,
        &context(&after),
        &select,
    ...
    assert_eq!(result["matched_before"], 1);
    assert_eq!(result["matched_after"], 1);
...
#[test]
fn epoch_changes_regressions_and_same_cursor_forks_fail_instead_of_reporting_mass_changes()
-> Result<()> {
    let before = world(10, 5, &[(1, 1, 7)]);
    for case in 0..5 {
        let mut after = world(11, 6, &[(1, 1, 8)]);
        match case {
            0 => after.cursor.epoch = 1,
            1 => after.cursor.sequence = 4,
            2 => after.tick = GameTick(9),
            3 => after.cursor = before.cursor,
...
fn continuations_bind_record_pair_session_and_selection_but_not_page_width() -> Result<()> {
    ...
    let c = context(&after);
    let binding = Digest32::of_bytes(b"pair");
    let first = compare_endpoints(&before, &after, &c, &selection(), binding, Some(1), None)?;
    let token = first["continuation"]
        .as_str()
        .ok_or_else(|| invalid("test continuation absent"))?;
    let rest = compare_endpoints(
        &before,
        &after,
        &c,
        &selection(),
    ...
    assert_eq!(rest["returned"], 2);
    assert_eq!(rest["changes"][0]["entity_id"], "2");
    let mut other = c.clone();
    other.session_id = SessionId::new(782);
    assert!(
        compare_endpoints(
            &before,
            &after,
    ...
    let mut select = selection();
    select["order"] = json!("id_descending");
...
fn current_authority_cannot_be_revived_by_selecting_older_authorized_ticks() -> Result<()> {
    let before = world(1, 0, &[(1, 1, 1)]);
    let after = world(2, 1, &[(1, 1, 2)]);
    for case in 0..4 {
        let mut c = context(&after);
        c.anchor.tick = GameTick(100);
        match case {
            0 => c.grants.clear(),
            1 => c.cancellation_requested = true,
            2 => c.grants[0].expires_at_tick = Some(GameTick(50)),
...
fn tiny_response_or_invalid_selector_never_returns_an_empty_progress_page() -> Result<()> {
    ...
    assert!(
        matches!(compare_endpoints(&before,&after,&c,&selection(),Digest32::ZERO,None,None),Err(e)if e.code==ErrorCode::BudgetExceeded)
    );
    let c = context(&after);
    let mut select = selection();
    select["limit"] = json!(1);
