...
use dfmcp_core::{Capability, DfmcpError, Digest32, EntityId, ErrorCode, MapCoord,
    OperationContext, Result, RiskTier, StateAnchor};
use dfmcp_world::{CompareOp, EdgeKind, EntityKind, EntityRecord, Fact, FactPresence,
    FactSource, Predicate, QueryOrder, Value as WorldValue, WorldQuery, WorldSnapshot,
    execute_bounded_query};
use dfmcp_world::graph_query::{GraphBudget, GraphDirection, GraphTraversalQuery,
    GraphWitness, analyze_dependencies, traverse_graph};
...
enum Query {
    Entities {
        ...
        #[serde(rename = "where")] predicate: Option<Filter>,
    ...
    Traverse {
        roots: Vec<String>,
        #[serde(default)] edge_kinds: Vec<String>,
...
enum Order { IdAscending, IdDescending, LabelAscending, RevisionDescending }
...
fn validate_input(input: &Value) -> Result<()> {
    ...
    while let Some((value, depth)) = pending.pop() {
        ...
        if bytes > MAX_INPUT_BYTES {
            return Err(budget("structured query exceeds its aggregate input-byte bound"));
        }
    }
    Ok(())
}
...
fn bounded_names(mut names: Vec<String>, maximum: usize) -> Result<Vec<String>> {
    if names.len() > maximum || names.iter().any(|name| name.is_empty()
        || name.len() > 128 || name.contains('\0')) {
        return Err(invalid("selector names exceed their count or UTF-8 byte bound"));
    }
    names.sort();
    names.dedup();
    Ok(names)
...
fn entity_id(raw: &str) -> Result<EntityId> {
    if raw.is_empty() || raw.len() > 20 || raw.starts_with('0')
        || !raw.bytes().all(|byte| byte.is_ascii_digit()) {
        return Err(invalid("entity IDs must be canonical positive decimal u64 strings"));
    }
    raw.parse::<u64>().map(EntityId::new)
        .map_err(|_| invalid("entity ID exceeds u64"))
}

fn entity_kind(raw: &str) -> Result<EntityKind> {
    Ok(match raw {
        "fortress" => EntityKind::Fortress, "unit" => EntityKind::Unit,
        "item" => EntityKind::Item, "building" => EntityKind::Building,
        "job" => EntityKind::Job, "work_order" => EntityKind::WorkOrder,
        "stockpile" => EntityKind::Stockpile, "zone" => EntityKind::Zone,
        "burrow" => EntityKind::Burrow, "squad" => EntityKind::Squad,
        "military_order" => EntityKind::MilitaryOrder, "tile_feature" => EntityKind::TileFeature,
        "plant" => EntityKind::Plant, "creature" => EntityKind::Creature,
        "historical_figure" => EntityKind::HistoricalFigure, "civilization" => EntityKind::Civilization,
        "announcement" => EntityKind::Announcement, "syndrome" => EntityKind::Syndrome,
        value if value.starts_with("other:") && value.len() > 6 => EntityKind::Other(value[6..].to_owned()),
...
fn edge_kind(raw: &str) -> Result<EdgeKind> {
    Ok(match raw {
        "located_at" => EdgeKind::LocatedAt, "contained_in" => EdgeKind::ContainedIn,
        "assigned_to" => EdgeKind::AssignedTo, "member_of" => EdgeKind::MemberOf,
        "performs" => EdgeKind::Performs, "requires" => EdgeKind::Requires,
        "produces" => EdgeKind::Produces, "uses" => EdgeKind::Uses,
        "supports" => EdgeKind::Supports, "threatens" => EdgeKind::Threatens,
        "ordered_by" => EdgeKind::OrderedBy, "parent_of" => EdgeKind::ParentOf,
        value if value.starts_with("custom:") && value.len() > 7 => EdgeKind::Custom(value[7..].to_owned()),
...
impl Literal {
    fn into_world(self) -> Result<WorldValue> {
        Ok(match self {
            Self::Null => WorldValue::Null, Self::Bool(value) => WorldValue::Bool(value),
            Self::I64(value) => WorldValue::I64(value), Self::U64(value) => WorldValue::U64(value),
...
impl Filter {
    fn into_world(self) -> Result<Predicate> {
        Ok(match self {
            Self::All { args } => Predicate::All(args.into_iter().map(Self::into_world).collect::<Result<_>>()?),
            Self::Any { args } => Predicate::Any(args.into_iter().map(Self::into_world).collect::<Result<_>>()?),
            ...
            Self::Compare { field, comparison, value } => {
                ...
                Predicate::FieldCompare {
                    entity_id: EntityId::NIL, field,
                    op: match comparison {
                        Comparison::Eq => CompareOp::Eq, Comparison::Ne => CompareOp::Ne,
                        Comparison::Lt => CompareOp::Lt, Comparison::Le => CompareOp::Le,
                        Comparison::Gt => CompareOp::Gt, Comparison::Ge => CompareOp::Ge,
                    },
...
fn world_value(value: &WorldValue) -> Value {
    match value {
        ...
        WorldValue::List(v) => json!({"type":"list","value":v.iter().map(world_value).collect::<Vec<_>>()}),
...
fn fact_json(fact: &Fact) -> Value {
    ...
    let (presence, state, reason, known) = match &fact.presence {
        ...
        Some(FactPresence::Known(value)) if value == &fact.value => ("known", source_state, Value::Null, true),
        Some(FactPresence::Known(_)) => ("unknown", "contradicted", json!("conflicting known-value representations"), false),
...
fn entity_json(entity: &EntityRecord, fields: &[String]) -> Value {
    let fields: BTreeMap<_,_> = fields.iter().map(|field| {
...
fn unwrap_continuation(raw: Option<String>, identity: Digest32) -> Result<Option<String>> {
    ...
    if raw.len() > 256 { return Err(budget("query continuation exceeds its byte bound")); }
    let prefix = format!("sp1:{identity}:");
    let Some(inner) = raw.strip_prefix(&prefix) else {
        return Err(DfmcpError::new(ErrorCode::StaleAnchor,
            "continuation belongs to another session or field projection; restart this query"));
    };
    if !inner.starts_with("q1:") { return Err(invalid("unsupported structured query continuation")); }
...
pub fn execute(snapshot: &WorldSnapshot, context: &OperationContext, input: &Value) -> Result<Value> {
    ...
    if context.anchor != snapshot.anchor() {
        return Err(DfmcpError::new(ErrorCode::StaleAnchor, "query context does not name this snapshot"));
    }
    ...
    if request.schema != SCHEMA { return Err(invalid("structured query schema must be dfmcp.query/1")); }
    ...
    if !snapshot.hash_is_valid() {
        return Err(DfmcpError::new(ErrorCode::InternalInvariantViolation, "query snapshot hash is invalid"));
    }
    let max_bytes = context.budget.max_bytes.min(u64::from(context.budget.max_output_tokens).saturating_mul(4));
    let byte_limit = usize::try_from(max_bytes).map_err(|_| budget("query byte limit cannot be represented"))?;
    let graph_budget = GraphBudget {
        max_vertices: (context.budget.max_entities as usize).min(100_000),
        max_edges: (context.budget.max_entities as usize).saturating_mul(8).min(1_000_000),
    ...
    let mut payload = match request.query {
        Query::Entities { kinds, predicate, fields, order, limit, continuation } => {
            let fields = bounded_names(fields, 32)?;
            let kinds = bounded_names(kinds, 32)?.iter().map(|kind| entity_kind(kind)).collect::<Result<_>>()?;
            ...
            let query = WorldQuery {
                kinds, predicate: predicate.map(Filter::into_world).transpose()?,
                order: match order {
        ...
        Query::Inspect { entity_id: raw_id, generation, fields } => {
            let fields = bounded_names(fields, 32)?;
            let id = entity_id(&raw_id)?;
            let entity = snapshot.graph.entities.get(&id)
                .ok_or_else(|| invalid("entity is not in the observed projection; world absence is not established"))?;
            if entity.generation != generation {
                return Err(DfmcpError::new(ErrorCode::Conflict, "entity generation changed; refresh its handle before inspection"));
            }
            json!({"kind":"inspect", "truncated":false, "continuation":null,
                "row":entity_json(entity,&fields)})
        }
        Query::Traverse { roots, edge_kinds, direction, max_depth, target } => {
            if roots.len() > 64 { return Err(budget("graph root count exceeds 64")); }
            let roots = roots.iter().map(|id| entity_id(id)).collect::<Result<_>>()?;
            let target = target.as_deref().map(entity_id).transpose()?;
            let edge_kinds = bounded_names(edge_kinds,32)?.iter().map(|kind| edge_kind(kind)).collect::<Result<_>>()?;
            let query = GraphTraversalQuery {
                roots, edge_kinds, max_depth,
            ...
            let path = target.map(|id| result.observed_path_to(id)).transpose()?.flatten();
        ...
        Query::Dependencies { edge_kinds } => {
            if edge_kinds.is_empty() { return Err(invalid("dependency analysis requires explicit dependent-to-prerequisite edge kinds")); }
            let edge_kinds = bounded_names(edge_kinds,32)?.iter().map(|kind| edge_kind(kind)).collect::<Result<_>>()?;
    ...
    let encoded = serde_json::to_vec(&payload).map_err(|_| invalid("query result cannot be encoded"))?;
    if encoded.len() > byte_limit {
        return Err(budget("rendered query result exceeds the output budget; reduce limit, selected fields, roots, or graph depth"));
    }
    Ok(payload)
}
...
mod tests {
    ...
    fn fixture() -> (WorldSnapshot,OperationContext) {
        ...
        for id in 1..=4 {
            graph.entities.insert(EntityId::new(id),EntityRecord {
                ...
                label:format!("Urist {id}"), fields:BTreeMap::from([
                    ("sane".to_owned(),Fact::known(WorldValue::Bool(id%2==0),GameTick(1),FactSource::DfhackField("sane".to_owned()),Digest32::of_bytes(b"fixture"))),
                    ("unknown".to_owned(),Fact::with_presence(FactPresence::Omitted("not projected".to_owned()),GameTick(1),FactSource::Replay,Digest32::ZERO)),
                ]),
            });
        }
        for (id,from,to) in [(1,1,2),(2,2,3),(3,3,2),(4,4,1)] {
            graph.edges.insert(EdgeId::new(id),EdgeRecord {id:EdgeId::new(id),revision:1,
                kind:EdgeKind::Requires,from:EntityId::new(from),to:EntityId::new(to),fields:BTreeMap::new()});
        }
        ...
        let context=OperationContext {session_id:SessionId::new(7),request_id:RequestId::new(1),
            anchor:snapshot.anchor(),budget:WorkBudget {max_output_tokens:16384,..WorkBudget::default()},
            grants:vec![CapabilityGrant {capability:Capability::Query,scope:CapabilityScope {fortress_id:Some(snapshot.fortress_id),..CapabilityScope::default()},
                max_risk:RiskTier::ReadOnly,expires_at_tick:None,remaining_uses:None}],cancellation_requested:false};
        (snapshot,context)
    }
    fn envelope(query:Value)->Value {json!({"schema":SCHEMA,"query":query})}

    #[test]
    fn typed_filters_and_fact_provenance_are_executable()->Result<()> {
        let (snapshot,ctx)=fixture();
        let input=envelope(json!({"kind":"entities","kinds":["unit"],"fields":["sane","missing"],
        ...
        assert_eq!(result["rows"][0]["fields"]["sane"]["epistemic_state"],"observed");
        assert_eq!(result["rows"][0]["fields"]["missing"]["presence"],"unknown");
    ...
    fn omitted_fields_do_not_match_even_under_negation()->Result<()> {
        let (snapshot,ctx)=fixture();
        let result=execute(&snapshot,&ctx,&envelope(json!({"kind":"entities","where":{
    ...
    fn pages_resume_but_cannot_cross_session_projection_or_snapshot()->Result<()> {
        let (snapshot,ctx)=fixture();
        let first=execute(&snapshot,&ctx,&envelope(json!({"kind":"entities","limit":1})))?;
        let resume=envelope(json!({"kind":"entities","limit":2,"continuation":first["continuation"]}));
        ...
        assert!(matches!(execute(&snapshot,&other,&resume),Err(e) if e.code==ErrorCode::StaleAnchor));
        let mut changed=resume.clone(); changed["query"]["fields"]=json!(["sane"]);
        assert!(matches!(execute(&snapshot,&ctx,&changed),Err(e) if e.code==ErrorCode::StaleAnchor));
    ...
    fn inspection_checks_generation_and_preserves_unknowns()->Result<()> {
        let (snapshot,ctx)=fixture();
        let input=envelope(json!({"kind":"inspect","entity_id":"1","generation":1,"fields":["unknown","missing"]}));
        let result=execute(&snapshot,&ctx,&input)?;
        assert_eq!(result["row"]["fields"]["unknown"]["presence"],"omitted");
        let mut stale=input; stale["query"]["generation"]=json!(2);
    ...
    fn structured_graph_modes_return_paths_cycles_and_blockers()->Result<()> {
        let (snapshot,ctx)=fixture();
        let traversal=execute(&snapshot,&ctx,&envelope(json!({"kind":"traverse","roots":["4"],
            "edge_kinds":["requires"],"max_depth":4,"target":"3"})))?;
        assert_eq!(traversal["path"]["vertices"],json!(["4","1","2","3"]));
        assert_eq!(traversal["witness"]["anchor"],anchor_json(snapshot.anchor()));
        let dependencies=execute(&snapshot,&ctx,&envelope(json!({"kind":"dependencies","edge_kinds":["requires"]})))?;
        assert_eq!(dependencies["blocked_by_cycles"],json!(["1","2","3","4"]));
    ...
    fn malformed_versions_fields_types_and_ids_are_rejected() {
        let (snapshot,ctx)=fixture();
        for query in [json!({"kind":"entities","limit":0}),json!({"kind":"entities","limit":513}),
            json!({"kind":"entities","unknown_key":true}),json!({"kind":"entities","kinds":["unt"]}),
        ...
        assert!(execute(&snapshot,&ctx,&json!({"schema":"dfmcp.query/99","query":{"kind":"entities"}})).is_err());
    }
    #[test]
    fn aggregate_inputs_and_rendered_output_are_bounded() {
        ...
        assert!(matches!(execute(&snapshot,&ctx,&envelope(json!({"kind":"entities"}))),Err(e) if e.code==ErrorCode::BudgetExceeded));
    }
    #[test]
    fn exact_expected_anchor_and_replay_are_checked()->Result<()> {
        let (snapshot,ctx)=fixture();
        let mut input=envelope(json!({"kind":"entities"})); input["expected_anchor"]=anchor_json(snapshot.anchor());
        assert_eq!(execute(&snapshot,&ctx,&input)?,execute(&snapshot,&ctx,&input)?);
