...
use dfmcp_core::{
    Capability, CapabilityGrant, CapabilityScope, ErrorCode, RequestId, RiskTier, SessionId,
    WorkBudget,
};
...
fn terrain() -> Result<LiveMapObservation> {
    ...
    Ok(LiveMapObservation {
        ...
        paused: true,
        site_id: 1,
        world_folder: "fixture".to_owned(),
        map_dimensions: [32, 32, 8],
        map: MapRegion {
            region,
            cells: vec![tile; region.volume().map_err(|_| invalid("test region"))?],
        },
    })
}
...
fn visible_attributes_never_fill_in_hidden_or_unallocated_tiles() -> Result<()> {
    ...
    let hidden = map
        ...
        .region
        .index([4, 2, 5])
        .ok_or_else(|| invalid("hidden fixture cell"))?;
    let unallocated = map
        .map
        .region
        .index([5, 2, 5])
        .ok_or_else(|| invalid("unallocated fixture cell"))?;
    let wet = map
        .map
        .region
        .index([6, 2, 5])
        .ok_or_else(|| invalid("wet fixture cell"))?;
    ...
    if let Cell::Visible(tile) = &mut map.map.cells[wet] {
        ...
        tile.unit_occupancy = 2;
        tile.dig_designation = 1;
    }
    ...
    assert_eq!(
        summary["footprint"]["visible_attributes"],
        json!({"liquid_tiles":1,"magma_liquid_tiles":1,
...
fn uncaptured_positions_and_map_edges_are_explicit_not_safe_empty_space() -> Result<()> {
    ...
    assert_eq!(
        summary["footprint"]["visible_attributes"]["shapes"],
        json!({})
    );
    let layout = BlueprintPlanner.layout(
        MapCoord { x: 0, y: 0, z: 5 },
...
fn complete_work_bound_is_checked_before_scanning_and_corruption_is_not_absence() -> Result<()> {
    ...
    let (summary, _) = analyze(&map, &layout, MAX_WORK)?;
    let cost = summary["scan_work_upper_bound"]
        .as_u64()
        .ok_or_else(|| invalid("test work bound"))?;
    assert_eq!(analyze(&map, &layout, cost)?.0, summary);
    for bound in [0, 1, cost - 1, MAX_WORK + 1] {
        assert!(
            matches!(analyze(&map, &layout, bound), Err(e) if e.code == ErrorCode::BudgetExceeded)
        );
    }
    let mut corrupt = map;
...
fn state() -> Result<(LiveSpatialState, OperationContext)> {
    ...
    let bytes = (0..text.len())
        .step_by(2)
        .map(|i| {
            u8::from_str_radix(&text[i..i + 2], 16).map_err(|_| invalid("spatial test fixture"))
        })
    ...
    let anchor = state
        .snapshot()
        .ok_or_else(|| invalid("test snapshot"))?
        .anchor();
...
fn shared_spatial_dispatch_returns_anchor_bound_preview_not_a_prepared_plan() -> Result<()> {
    ...
    assert!(
        schema()?["$defs"]["query"]["oneOf"]
            .as_array()
            .ok_or_else(|| invalid("test schema"))?
            .iter()
            .any(|v| v["properties"]["kind"]["const"] == "blueprint_layout")
    );
    Ok(())
}
...
fn pages_can_change_width_but_not_session_layout_or_work_policy() -> Result<()> {
    ...
    assert_eq!(
        ...
        json!(
            &full["rows"]
                .as_array()
                .ok_or_else(|| invalid("test rows"))?[1..]
        )
    );
    ...
    other.session_id = SessionId::new(99);
...
fn authority_staleness_output_limits_and_mutation_fields_fail_closed() -> Result<()> {
    ...
    denied.grants.clear();
    assert!(
        matches!(query(&state, &denied, &input), Err(e) if e.code == ErrorCode::CapabilityDenied)
    );
    let mut stale = c.clone();
    stale.anchor.tick.0 += 1;
