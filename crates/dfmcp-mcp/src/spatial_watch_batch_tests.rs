...
impl Source for Script {
    fn read(&mut self, _: Duration) -> Result<LiveSpatialCitizenObservation> {
        ...
        let value = self
            .values
            .pop_front()
            .ok_or_else(|| error(ErrorCode::AdapterUnavailable, "injected capture exhaustion"))?;
...
fn decode(text: &str) -> Result<Value> {
    serde_json::from_str(text).map_err(|_| error(ErrorCode::InvalidRequest, "test JSON"))
}
fn register(files: &Files, tick: u32, next: &[u32], corrupt: bool) -> Result<Registered> {
    ...
    let source = Script {
        ...
        calls: calls.clone(),
        fenced: false,
        corrupt: corrupt.then(|| files.watches.clone()),
    };
    let mut s = Session {
        id: next_id()?,
...
fn watch(s: &Registered, key: &str, stable: u32) -> Result<Value> {
    let result = ask(
        s,
        json!({"kind":"watch","key":key,"condition":{"op":"paused","value":true},
        "deadline_tick":105u64*403200+100,"poll_interval_ticks":1,"stable_observations":stable}),
    )?;
    assert_eq!(result["ok"], true, "{result}");
    Ok(result["record"]["watch"].clone())
}
fn checkpoint(value: &Value) -> Result<u64> {
    value["watch_persistence"]["checkpoint"]
        .as_u64()
...
fn mcp_await_batch_uses_one_capture_and_one_checkpoint_for_all_watches() -> Result<()> {
    ...
    watch(&s, "a", 3)?;
    watch(&s, "b", 3)?;
    let prior = ask(&s, json!({"kind":"watches"}))?;
    let mut count = checkpoint(&prior)?;
    for step in 1..=2 {
        ...
        assert_eq!(checkpoint(&result)?, count + 1);
        count += 1;
        for row in result["records"]
            .as_array()
            .ok_or_else(|| error(ErrorCode::InvalidRequest, "rows"))?
        {
            assert_eq!(row["stable_observations"], step + 1);
            assert_eq!(row["last_evaluated_anchor"], result["anchor"]);
        }
        assert_eq!(s.calls.load(Ordering::SeqCst), step as usize);
        assert_eq!(result["all_satisfied"], step == 2);
    }
    let bytes = fs::read(&files.watches).map_err(io_error)?;
    let replay = ask(&s, json!({"kind":"await_watches"}))?;
    require_success(&replay)?;
    assert_eq!(replay["native_captures"], 0);
    assert_eq!(replay["advanced"], 0);
    assert_eq!(checkpoint(&replay)?, count);
...
fn mcp_poll_batch_evaluates_existing_capture_without_another_read() -> Result<()> {
    ...
    let result = ask(&s, json!({"kind":"poll_watches"}))?;
    require_success(&result)?;
    assert_eq!(result["all_satisfied"], true);
...
fn invalid_selection_and_inadequate_preflight_budget_do_not_capture_or_write() -> Result<()> {
    ...
    for selection in [
        json!([]),
        json!([a.clone(), a]),
        json!([format!("watch:{}", "f".repeat(64))]),
    ] {
        assert_eq!(
            ask(&s, json!({"kind":"await_watches","watches":selection}))?["ok"],
            false
        );
    }
    {
...
fn bridge_failure_preserves_all_watches_and_restart_batch_restores_stability() -> Result<()> {
    ...
    let before = fs::read(&files.watches).map_err(io_error)?;
    assert_eq!(
        ask(&s, json!({"kind":"await_watches"}))?["error"]["code"],
        "adapter_unavailable"
    );
    assert_eq!(fs::read(&files.watches).map_err(io_error)?, before);
    drop(s);
    ...
    for step in 1..=2 {
        ...
        for row in result["records"]
            ...
            .ok_or_else(|| error(ErrorCode::InvalidRequest, "rows"))?
        {
            assert_eq!(row["stable_observations"], step);
            assert_eq!(row["fresh_observation_required"], false);
        }
        assert_eq!(result["all_satisfied"], step == 2);
    }
    assert_eq!(s.calls.load(Ordering::SeqCst), 2);
...
fn watch_storage_changed_during_capture_cannot_publish_the_candidate_set() -> Result<()> {
    ...
    assert_eq!(
        fs::read(&files.watches).map_err(io_error)?,
        [before, b"x".to_vec()].concat()
    );
    assert_eq!(s.calls.load(Ordering::SeqCst), 1);
    Ok(())
}
...
fn authority_expiring_at_new_capture_does_not_commit_watch_progress() -> Result<()> {
    ...
    let before = fs::read(&files.watches).map_err(io_error)?;
    {
        ...
        for grant in &mut session.grants {
            grant.expires_at_tick = Some(expiration);
        }
    }
    let result = ask(&s, json!({"kind":"await_watches"}))?;
    assert_eq!(result["ok"], false);
...
fn schema_discovers_batches_but_archive_and_historical_queries_refuse_them() -> Result<()> {
    ...
    for kind in ["poll_watches", "await_watches"] {
        assert!(
            matches!(execute(&mut session,&c,&json!({"schema":"dfmcp.query/1","query":{"kind":kind}})),
...
fn full_eight_watch_batch_fits_the_default_8192_token_budget() -> Result<()> {
    ...
    let s = register(&files, 3, &[4], false)?;
    for index in 0..8 {
        watch(&s, &format!("batch-{index}"), 2)?;
    }
    {
        let handle = resolve(s.handle())?;
        let mut session = lock(&handle)?;
        session.budget.max_output_tokens = 8192;
    }
    let raw = fortress_query(
        s.handle(),
        None,
        Some(json!({"schema":"dfmcp.query/1","query":{"kind":"await_watches"}})),
    );
    assert!(raw.len() <= 32768);
    let result = decode(&raw)?;
    require_success(&result)?;
    assert_eq!(result["selected"], 8);
    assert_eq!(result["sampled"], 8);
    assert_eq!(result["all_satisfied"], true);
    assert_eq!(result["records"].as_array().map(Vec::len), Some(8));
    assert_eq!(s.calls.load(Ordering::SeqCst), 1);
    Ok(())
}
