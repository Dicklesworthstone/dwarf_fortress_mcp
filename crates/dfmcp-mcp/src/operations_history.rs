...
use dfmcp_adapter::operations_journal::{
    JournalEntry, JournalLimits, OperationsJournal, PrivateJournalFile, TailRecovery,
    open_private_journal,
};
...
pub(super) fn configuration() -> Result<Option<(PathBuf, TailRecovery)>> {
    let path = match std::env::var("DFMCP_OPERATIONS_JOURNAL") {
        ...
        _ => {
            return Err(error(
                ...
                "DFMCP_OPERATIONS_JOURNAL must be a nonempty UTF-8 operator path",
            ));
        }
    };
    let repair = match std::env::var("DFMCP_OPERATIONS_JOURNAL_REPAIR") {
        ...
        _ => {
            return Err(error(
                ErrorCode::InvalidRequest,
                "journal repair requires a configured journal and the exact operator opt-in value 1",
            ));
        }
    };
...
pub(super) fn summary(journal: Option<&Journal>) -> Value {
    match journal {
        ...
        Some(journal) => {
            json!({"storage":"synced_operations_journal","durable_observation_history":true,
...
fn parse(input: &Value, context: &OperationContext) -> Result<Request> {
    ...
    while let Some((value, depth)) = stack.pop() {
        ...
        if nodes > 4096 || depth > 32 {
            return Err(error(
                ...
                "historical query shape exceeds its bound",
            ));
        }
        match value {
            Value::Object(map) => {
                if stack.len().saturating_add(map.len()) > 4096 {
                    return Err(error(
                        ErrorCode::BudgetExceeded,
                        "history object is too wide",
                    ));
                }
            ...
            Value::Array(values) => {
                if stack.len().saturating_add(values.len()) > 4096 {
                    return Err(error(
                        ErrorCode::BudgetExceeded,
                        "history array is too wide",
                    ));
                }
        ...
        if bytes > 128 * 1024 {
            return Err(error(
                ErrorCode::BudgetExceeded,
                "historical query text exceeds its bound",
            ));
        }
    }
    let envelope: Envelope = serde_json::from_value(input.clone()).map_err(|_| {
        error(
            ErrorCode::InvalidRequest,
            "invalid operations history envelope",
        )
    })?;
    if envelope.schema != "dfmcp.query/1" {
        return Err(error(
            ErrorCode::InvalidRequest,
            "history requires schema dfmcp.query/1",
        ));
    }
    if envelope
        .expected_anchor
        ...
        .is_some_and(|a| a != &anchor_json(context.anchor))
    {
        return Err(error(
            ...
            "history expected_anchor differs from the current session anchor",
        ));
    }
    Ok(envelope.query)
}
...
fn history_page(
    ...
    let start = match continuation {
        ...
        Some(token) => {
            if token.len() > 128 {
                return Err(error(
                    ErrorCode::BudgetExceeded,
                    "history cursor exceeds its bound",
                ));
            }
            let mut parts = token.split(':');
            let (Some("jh1"), Some(offset), Some(_), None) =
            ...
            if value == 0
                || value >= journal.entries().len()
                || cursor(journal, context, value) != token
            {
                return Err(error(
                    ErrorCode::StaleAnchor,
                    "history cursor belongs to another session, journal head or snapshot",
                ));
            }
            value
        }
    ...
    for entry in journal.entries().iter().skip(start).take(limit as usize) {
        ...
        next["truncated"] = json!(end + 1 < journal.entries().len());
        next["continuation"] = if end + 1 < journal.entries().len() {
            json!(cursor(journal, context, end + 1))
        } else {
            Value::Null
        };
        if next.to_string().len() > maximum {
            break;
        }
        result = next;
        end += 1;
    ...
    if (end == start && start < journal.entries().len()) || result.to_string().len() > maximum {
        return Err(error(
            ErrorCode::BudgetExceeded,
            "one journal metadata row and required context cannot fit",
        ));
    }
    Ok(result)
}
...
pub(super) fn execute(
    ...
    result_context.budget.max_bytes = view.result_byte_budget()? as u64;
    ...
    match request {
        Request::History {
            ...
            let mut value = history_page(
                journal,
                context,
                narrowed.budget.max_bytes as usize,
                limit,
                continuation,
            )?;
        ...
        Request::HistoricalQuery {
            ...
            if !matches!(
                kind,
                "entities" | "inspect" | "traverse" | "dependencies" | "aggregate" | "search"
            ) {
                return Err(error(
                    ErrorCode::InvalidRequest,
                    "historical_query permits only stateless observation queries; no watches, baselines, effects or nested history",
                ));
            }
            let digest = Digest32::from_hex(&record_digest)
                .filter(|digest| digest.to_string() == record_digest)
                .ok_or_else(|| {
                    error(
                        ErrorCode::InvalidRequest,
            ...
            view.references = vec![
                json!({"kind":"archived_operations_observation","digest":entry.source_digest.to_string(),
            ...
            historical_context.budget.max_bytes = view
                .result_byte_budget()?
                .checked_sub(metadata.to_string().len() + 128)
                .ok_or_else(|| {
                    error(
                        ErrorCode::BudgetExceeded,
                        "historical metadata leaves no query budget",
                    )
                })? as u64;
            historical_context = semantic_query::result_context(&historical_context)?;
            historical_context.anchor = snapshot.anchor();
            let mut value = semantic_query::execute(
                &snapshot,
                &historical_context,
            ...
            if let (Some(object), Some(additions)) = (value.as_object_mut(), metadata.as_object()) {
                ...
                return Err(error(
                    ErrorCode::InternalInvariantViolation,
                    "historical query returned a non-object",
                ));
            }
            semantic_query::publish_with_active_work(context, value, |value| {
                ...
                let mut packet: Value = serde_json::from_str(&raw).map_err(|_| {
                    error(
                        ErrorCode::InternalInvariantViolation,
                        "historical packet decode failed",
                    )
                })?;
                ...
                if encoded.len() > view.maximum_bytes {
                    return Err(error(
                        ErrorCode::BudgetExceeded,
                        "historical packet exceeds response budget",
                    ));
                }
                Ok(encoded)
            })
...
pub(super) fn query_schema() -> Result<Value> {
    ...
    let extension: Value = serde_json::from_str(include_str!(
        "../../../schemas/mcp_operations_history_v1.json"
    ))
    .map_err(|_| {
        error(
            ErrorCode::InternalInvariantViolation,
            "invalid embedded history schema",
        )
    })?;
    let variants = schema["$defs"]["query"]["oneOf"]
        ...
        .ok_or_else(|| {
            error(
                ErrorCode::InternalInvariantViolation,
                "query schema variants missing",
            )
        })?;
    let additions = extension["oneOf"].as_array().ok_or_else(|| {
        error(
            ErrorCode::InternalInvariantViolation,
            "history variants missing",
        )
    })?;
