...
use dfmcp_adapter::live_jobs_rpc::{
    DeadlineStream,
    operations::{OperationsLimits, OperationsRpcClient},
};
use dfmcp_adapter::live_operations::{
    LiveOperationsObservation, LiveOperationsState, MAX_OPERATIONS_BYTES, OperationsProfile,
};
use dfmcp_core::{
        ...
        SLOTS
            ...
            .map_err(|_| {
                error(
                    ...
                    "operations session capacity reached",
                )
            })?;
        Ok(Self)
    }
...
impl OperationsSource for OperationsRpcClient<DeadlineStream> {
    ...
    fn poisoned(&self) -> bool {
        OperationsRpcClient::poisoned(self)
    }
    fn fence(&mut self) {
        OperationsRpcClient::fence(self);
    }
}
struct OperationsSession {
    ...
    journal: Option<
        dfmcp_adapter::operations_journal::OperationsJournal<
            dfmcp_adapter::operations_journal::PrivateJournalFile,
        >,
    >,
...
impl OperationsSession {
    ...
    fn context(&mut self) -> Result<OperationContext> {
        self.request = self.request.checked_add(1).ok_or_else(|| {
            error(
                ErrorCode::BudgetExceeded,
                "operations request IDs exhausted",
            )
        })?;
        Ok(OperationContext {
            session_id: self.id,
    ...
    fn refresh(&mut self, context: &OperationContext) -> Result<JobPublication> {
        ...
        if context.anchor != self.anchor()? {
            return Err(error(ErrorCode::StaleAnchor, "operations refresh is stale"));
        }
        if self.source.poisoned() {
            return Err(error(
                ErrorCode::AdapterUnavailable,
                "operations source fenced; reopen session",
            ));
        }
        if let Some(journal) = &self.journal {
            context.authorize(Capability::Query, RiskTier::ReadOnly, &[], None)?;
            if self.state.profile() != OperationsProfile::V1_3
                || journal.fenced()
                || journal.state().snapshot().map(|s| s.anchor()) != Some(context.anchor)
            {
                return Err(error(
                    ErrorCode::CorruptLedger,
                    "journal is fenced or disagrees with the published session profile or anchor",
                ));
            }
        }
        let result = self
            ...
            .and_then(|value| {
                if value.jobs.jobs.len() > self.limits.jobs as usize
                    || value.buildings.len() > self.limits.buildings as usize
                    || value.items.len() > self.limits.items as usize
                    || value.encode_profile(self.state.profile())?.len() > self.limits.payload_bytes
                {
                    return Err(error(
                        ErrorCode::BudgetExceeded,
                        "operations observation exceeds session limits",
                    ));
                }
...
fn next_id() -> Result<SessionId> {
    ...
    if *next >= 1u128 << 60 {
        return Err(error(
            ErrorCode::BudgetExceeded,
            "operations session IDs exhausted",
        ));
    }
    let id = SessionId::new((1u128 << 127) | FAMILY | *next);
    *next += 1;
    Ok(id)
}
fn resolve(raw: Option<String>) -> Result<Arc<Mutex<OperationsSession>>> {
    let text = raw.ok_or_else(|| {
        error(
            ...
            "open an operations session and supply session_id",
        )
    })?;
    if text.len() != 32 || !text.bytes().all(|b| b.is_ascii_hexdigit()) {
        return Err(error(
            ErrorCode::InvalidRequest,
            "invalid operations session handle",
        ));
    }
    let raw = u128::from_str_radix(&text, 16)
        .map_err(|_| error(ErrorCode::InvalidRequest, "invalid session hex"))?;
    let id = SessionId::new(raw);
    if id.get() != raw
        || !id.is_process_scoped_live()
        || !matches!((raw & ((1u128 << 62) - 1)) >> 60, 2 | 3)
    {
        return Err(error(
            ErrorCode::InvalidRequest,
            "not an encoded operations runtime handle",
        ));
    }
    lock(&SESSIONS)?
        .get(&id)
        .cloned()
        .ok_or_else(|| error(ErrorCode::SessionNotFound, "operations session not found"))
}
    ...
    !name.starts_with("DFMCP_")
        || matches!(
            ...
            "DFMCP_ALLOW_UNADMITTED_OPERATIONS_V1_3"
                ...
                | "DFMCP_OPERATIONS_ENDPOINT"
                | "DFMCP_OPERATIONS_JOURNAL"
                | "DFMCP_OPERATIONS_JOURNAL_REPAIR"
        )
}
fn validate_environment() -> Result<()> {
    if std::env::var("DFMCP_ALLOW_UNADMITTED_OPERATIONS_V1_3")
        .ok()
        .as_deref()
        != Some("1")
...
fn packet(
    session: Option<&OperationsSession>,
    ...
    if let Some(value) = payload
        .as_object_mut()
        .and_then(|map| map.remove("_condition_watch_work"))
    {
    ...
    if let (Some(session), Some(context)) = (session, context) {
        ...
        builder = builder
            .briefing(json!({"runtime":"unadmitted_development",
    ...
    if encoded.len() > limit {
        return Err(error(
            ErrorCode::BudgetExceeded,
            "complete operations packet exceeds output budget",
        ));
    }
    Ok(encoded)
}
    ...
    let result = match (session, context) {
        (Some(session), Some(context))
            if context
                ...
                .is_ok() =>
        {
            semantic_query::publish_with_active_work(context, payload, |value| {
                packet(Some(session), Some(context), operation, value)
            })
        }
...
where
    F: FnOnce(&mut OperationsSession, OperationContext) -> Result<String>,
{
    let handle = match resolve(id) {
        Ok(v) => v,
        Err(e) => return failure_packet(None, None, operation, &e),
    };
    ...
    match body(&mut session, context.clone()) {
        ...
        Err(e) => {
            ...
            if let Ok(anchor) = session.anchor() {
    ...
    for name in input {
        let value = match name.as_str() {
            ...
            _ => {
                return Err(error(
                    ErrorCode::CapabilityDenied,
                    "operations cannot grant that capability",
                ));
            }
        };
...
#[tool(
    description = "Open an authenticated, explicitly unadmitted operations/1.3 session covering jobs, buildings, inventory and attachments in one native read. Token and loopback endpoint are process configuration, not tool arguments."
)]
...
pub fn fortress_open_session(
    ...
    let result = (|| -> Result<String> {
        ...
        if journal_configuration.is_some()
            && (!capabilities.contains(&Capability::Query)
                || !capabilities.contains(&Capability::Observe))
        {
            return Err(error(
                ErrorCode::CapabilityDenied,
                "durable observation sessions require Query and Observe authority",
            ));
        }
        let bytes = max_bytes.unwrap_or(MAX_OPERATIONS_BYTES as u64);
        let limits = OperationsLimits {
            jobs: max_jobs.unwrap_or(1024),
            buildings: max_buildings.unwrap_or(1024),
            items: max_items.unwrap_or(8192),
            payload_bytes: usize::try_from(bytes).map_err(|_| {
                error(
                    ErrorCode::BudgetExceeded,
                    "operations bytes do not fit this platform",
                )
            })?,
        };
        limits.validate()?;
            ...
            || !(1..=60_000).contains(&budget.max_wall_millis)
        {
            return Err(error(
                ErrorCode::BudgetExceeded,
                "operations response or wall-time budgets exceed supported bounds",
            ));
        }
        let slot = SessionSlot::reserve()?;
        let id = next_id()?;
        let endpoint = std::env::var("DFMCP_OPERATIONS_ENDPOINT")
            .unwrap_or_else(|_| "127.0.0.1:5000".to_owned());
        let endpoint = dfmcp_adapter::parse_loopback_endpoint(&endpoint)?;
        let token = std::env::var("DFMCP_OPERATIONS_TOKEN").map_err(|_| {
            error(
                ErrorCode::CapabilityDenied,
                "DFMCP_OPERATIONS_TOKEN is required",
            )
        })?;
        let mut source = OperationsRpcClient::connect(
            endpoint,
            token.into_bytes(),
        ...
        if registry.contains_key(&id) {
            return Err(error(
                ErrorCode::InternalInvariantViolation,
                "operations session ID collision",
            ));
        }
    ...
    match result {
        Ok(encoded) => encoded,
        Err(e) => failure_packet(None, None, "fortress.open_session", &e),
    }
}
fn observe(id: Option<String>, operation: &str) -> String {
    with_session(id, operation, Capability::Observe, |session, context| {
        ...
        target.anchor = session.anchor()?;
        target.authorize(Capability::Observe, RiskTier::ReadOnly, &[], None)?;
        if target
            .authorize(Capability::Query, RiskTier::ReadOnly, &[], None)
            .is_ok()
        {
            semantic_query::publish_with_active_work(&target, payload, |value| {
                packet(Some(session), Some(&context), operation, value)
            })
        } else {
            packet(Some(session), Some(&context), operation, payload)
        }
    })
}
#[tool(
    description = "Read one coherent jobs/buildings/items observation without unpausing. An oversized or inconsistent domain prevents combined publication."
)]
pub fn fortress_observe(session_id: Option<String>) -> String {
    observe(session_id, "fortress.observe")
}
#[tool(
    description = "Refresh one operations observation without controlling game time. Use query await_watch for bounded condition evaluation."
)]
...
fn query_view(
...
) -> Result<QueryResponseProjection> {
    ...
    Ok(QueryResponseProjection {
        ...
        coverage: coverage(),
        uncertainty: vec![
            json!({"domain":"fortress.operations.feasibility","epistemic_state":"unknown",
...
fn finish_query(view: &QueryResponseProjection, value: Value) -> Result<String> {
    ...
    let mut value: Value = serde_json::from_str(&encoded).map_err(|_| {
        error(
            ErrorCode::InternalInvariantViolation,
            "query packet decode failed",
        )
    })?;
        ...
        |session, mut context| {
            ...
            let mut input = match query {
                ...
                None => match mode.as_deref() {
                    ...
                    Some("jobs") => {
                        json!({"schema":"dfmcp.query/1","query":{"kind":"entities","kinds":["job"],"fields":["type_key","suspended","holder_native_id"]}})
                    }
                    Some("buildings") => {
                        json!({"schema":"dfmcp.query/1","query":{"kind":"entities","kinds":["building"],"fields":["type_key","build_stage","max_build_stage"]}})
                    }
                    Some("items") => {
                        json!({"schema":"dfmcp.query/1","query":{"kind":"entities","kinds":["item"],"fields":["type_key","stack_size","forbidden","in_job","container","holder_building"],"limit":1}})
                    }
                    _ => {
                        return Err(error(
                            ErrorCode::InvalidRequest,
                            "unsupported operations query mode",
                        ));
                    }
                },
            };
            ...
            if !schema && history::handles(&input) {
                if session.state.profile() != OperationsProfile::V1_3 {
                    return Err(error(
                        ErrorCode::InvalidRequest,
                        "durable archive queries are not implemented for operations/1.4; 1.3 archives cannot be reinterpreted",
                    ));
                }
                let result = history::execute(session, &context, &input);
                if matches!(&result,Err(failure) if failure.code==ErrorCode::CorruptLedger) {
            ...
            if session.source.poisoned() && !local && !schema {
                return Err(error(
                    ErrorCode::AdapterUnavailable,
                    "operations source fenced; reopen or manage local records",
                ));
            }
            let mut refreshed = None;
            if !schema && kind == "await_watch" {
                let snapshot = session.state.snapshot().ok_or_else(|| {
                    error(ErrorCode::InternalInvariantViolation, "snapshot missing")
                })?;
            ...
            if schema {
                let schema = if session.state.profile() == OperationsProfile::V1_3 {
                    history::query_schema()?
                } else {
                    production::query_schema()?
                };
                return semantic_query::publish_with_active_work(
                    &context,
                    json!({"query_schema":schema,"mode":"schema",
            ...
            if production::handles(&input) {
                ...
                return semantic_query::publish_with_active_work(&narrowed, value, |value| {
            ...
            let snapshot = session
                .state
                .snapshot()
                .ok_or_else(|| error(ErrorCode::InternalInvariantViolation, "snapshot missing"))?;
            semantic_query::execute_with_publisher(snapshot, &narrowed, &input, |mut value| {
                ...
                if let Some(refresh) = refreshed {
                    value["observation_refresh"] = refresh;
                }
                finish_query(&view, value)
            })
        },
...
#[tool(
    description = "Explain operations fields and observed job/item/building relations; no speculative blocker diagnosis or mutation authority."
)]
...
#[tool(
    description = "Report the operations source health and coverage. Does not reconnect, mutate game state, or claim native qualification."
)]
...
#[tool(
    description = "Unavailable for game actions. Local condition watches use query cancel_watch."
)]
