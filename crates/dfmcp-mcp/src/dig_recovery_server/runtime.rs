...
use dfmcp_adapter::dig_designation::rpc::{
    DigManifest, DigPreparation, DigRpcClient, DigSource, DigTcpStream,
};
...
const ENVIRONMENT: [&str; 8] = [
    ...
    "DFMCP_DIG_TOKEN",
    super::goals::ENVIRONMENT,
];
fn denied() -> dfmcp_core::DfmcpError {
    error(
        ErrorCode::CapabilityDenied,
        "mining recovery boundary refused",
    )
}
...
impl Config {
    ...
    pub fn matches(&self, b: &DigBinding) -> Result<()> {
        if b.folder() != self.folder || b.site() != self.site || b.scope() != self.scope {
            return Err(denied());
        }
        Ok(())
    }
...
fn environment_contract(
    ...
    for name in names {
        count += 1;
        if count > 1024 || name.len() > 512 {
            return Err(denied());
        }
        if name.starts_with("DFMCP_") && !ENVIRONMENT.contains(&name.as_str()) {
            return Err(denied());
        }
    }
    match online {
...
fn configured(
    ...
    Ok(Config {
        ...
        scope: scope(&area)?,
        goal_files: super::goals::Files::default(),
    })
}
pub(super) fn configuration() -> Result<Config> {
    ...
    let online = environment_contract(
        &opt,
        online.as_deref(),
...
impl RequestControl {
    pub(super) fn checkpoint(&self) -> Result<()> {
        if self.abandoned.load(Ordering::Acquire) {
            return Err(error(ErrorCode::CancellationRequested, "request abandoned"));
        }
        self.parent
            .checkpoint()
            .map_err(|_| error(ErrorCode::CancellationRequested, "parent request cancelled"))?;
        self.worker.checkpoint().map_err(|_| {
            error(
                ErrorCode::CancellationRequested,
                "blocking request cancelled",
            )
        })?;
        Ok(())
    }
    ...
    F: FnOnce(RequestControl) -> String + Send + 'static,
{
    ...
    if cx.checkpoint().is_err() {
        return unbound(
            op,
            &error(ErrorCode::CancellationRequested, "request cancelled"),
        );
    }
    ...
    match task.join(&cx).await {
        ...
        Err(_) => unbound(
            ...
            &error(
                ...
                "owned recovery work did not yield an acknowledged result",
            ),
        ),
    }
}
...
impl<N: DigSource> DigSource for QueryOnly<N> {
    ...
    fn prepare(&mut self, _: &DigPlan, _: &OperationContext) -> Result<DigPreparation> {
        Err(denied())
    }
    fn commit(&mut self, _: &DigPlan, _: &OperationContext) -> Result<DigEffect> {
        Err(denied())
    }
    fn cancel(&mut self, _: &DigPlan, _: &OperationContext) -> Result<DigEffect> {
        Err(denied())
    }
...
pub(super) struct Guard<'a> {
    ...
    config: &'a Config,
    binding: &'a DigBinding,
}
impl<'a> Guard<'a> {
    pub fn new(control: &'a RequestControl, config: &'a Config, binding: &'a DigBinding) -> Self {
        Self {
            control,
            config,
            binding,
...
impl DigSessionGuard for Guard<'_> {
    fn connect(&mut self, b: &DigBinding, _: DigRegion, _: &OperationContext) -> Result<()> {
        boundary(self.control, self.config)?;
        if !self.config.online || b != self.binding {
            return Err(denied());
        }
...
pub(super) fn connect(
    ...
    Ok(QueryOnly(DigRpcClient::connect(
        b.endpoint(),
        token,
        nonce,
        r,
...
mod tests {
    ...
    fn environment_isolation_cannot_enable_mutation_or_production() {
        let names = ENVIRONMENT
            ...
            .collect::<Vec<_>>();
        assert_eq!(
            environment_contract("1", None, names.clone(), false).ok(),
            Some(false)
        );
        assert_eq!(
            environment_contract("1", Some("1"), names.clone(), false).ok(),
            Some(true)
        );
        for opt in ["", "true", "0"] {
            assert!(environment_contract(opt, None, names.clone(), false).is_err());
        }
        for online in ["0", "true", ""] {
            assert!(environment_contract("1", Some(online), names.clone(), false).is_err());
        }
    ...
    fn operator_source_and_path_must_be_bounded_exact_and_normalized() {
        let make = |folder: &str, site: &str, path: &str| {
            configured(
                folder.into(),
                site.into(),
                "[0,0,0,63,63,7]".into(),
        ...
        for site in ["01", "-1", "+1", "2147483648", ""] {
            assert!(make("region1", site, "/private/journal").is_err());
        }
        for path in [
            "relative",
            "/private/../journal",
    ...
    #[test]
    fn owned_entry_executes_on_a_blocking_worker_and_restores_ambient_context()
    ...
    #[test]
    fn inherited_runtime_restrictions_cannot_fall_back_to_threads()
    -> std::result::Result<(), Box<dyn std::error::Error>> {
        let called = Arc::new(AtomicBool::new(false));
        let worker_flag = called.clone();
        ...
        assert_eq!(packet["result"]["ok"], false);
        assert!(packet["agent_turn"].is_object());
        Ok(())
    }
}
