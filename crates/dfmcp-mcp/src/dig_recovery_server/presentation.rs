...
use crate::agent_turn::{AgentPhase, AgentTurnBuilder, ContinuityStatus, empty_active_work, empty_budget,
...
pub fn digest(raw: &str) -> Result<Digest32> {
    if raw.len() != 64 || !raw.bytes().all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b)) {
        return Err(error(ErrorCode::InvalidRequest, "expected a lowercase SHA-256 digest"));
    }
    let mut value = [0; 32];
    for (i, pair) in raw.as_bytes().chunks_exact(2).enumerate() {
        ...
        value[i] = u8::from_str_radix(text, 16).map_err(|_| error(ErrorCode::InvalidRequest, "invalid digest"))?;
    }
    Ok(Digest32::from_bytes(value))
}
pub fn mode_name(mode: DigMode) -> &'static str {
    match mode { DigMode::Offline => "offline", DigMode::Recover => "recover", DigMode::Control => "refused_control" }
}
pub fn summary(r: &DigSummary) -> Value { json!({
...
pub fn inventory(v: &DigSessionView) -> Value { json!({
...
fn blocker(value: &DigBlocker) -> &'static str { match value {
...
pub fn tiles(r: &DigRecord, offset: usize, limit: usize) -> Result<Value> {
    let p = r.plan(); let c = p.before(); let total = c.region().halo_count();
    if !(1..=MAX_TILE_PAGE).contains(&limit) || offset >= total {
        return Err(error(ErrorCode::InvalidRequest, "tile page is outside the retained complete halo"));
    }
    ...
    Ok(json!({"idempotency_key":p.key(),"plan_digest":p.digest().to_string(),"witness":c.witness().to_string(),
...
pub fn failure(e: &dfmcp_core::DfmcpError) -> Value {
    let message=match e.code {
        ErrorCode::CapabilityDenied=>"Query-only recovery cannot mutate the game; current runtime and operator authority are required.",
        ErrorCode::BudgetExceeded=>"Complete recovery work and output exceed the admitted allowance.",
        ErrorCode::EffectIndeterminate=>"The original mining outcome remains unresolved. Never retry the designation.",
        ErrorCode::CorruptLedger=>"Journal custody or integrity failed. Preserve the original evidence; do not repair or reset it.",
        ErrorCode::StaleAnchor|ErrorCode::Conflict=>"The requested identity or continuation does not match the retained state.",
...
pub fn packet(op: &str, result: Value, context: Option<&OperationContext>, mode: Option<DigMode>,
    binding: Option<&DigBinding>, view: Option<&DigSessionView>) -> String
{
    ...
    if let Some(v) = view {
        ...
        if let Some(pending) = &v.pending {
            ...
            let query_can_help=pending.native_phase != Some(dfmcp_adapter::dig_designation::DigPhase::Unknown);
    ...
    let phase = match op { "fortress.open_session" => AgentPhase::Bootstrap,
    ...
    if let Some(briefing) = turn["briefing"].as_object_mut() {
        ...
        if let (Some(b),Some(_))=(binding,view) {
            briefing.insert("expected_source".into(),json!({"endpoint":b.endpoint().to_string(),
