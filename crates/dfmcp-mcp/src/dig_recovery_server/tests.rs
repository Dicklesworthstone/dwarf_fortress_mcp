...
impl Read for Memory {
    fn read(&mut self, out: &mut [u8]) -> io::Result<usize> {
        ...
        d.reads += 1;
        if d.fail_read_at == Some(d.reads) {
...
impl Write for Memory {
    fn write(&mut self, raw: &[u8]) -> io::Result<usize> {
        let mut d = self.0.borrow_mut();
        if d.data.position() != d.data.get_ref().len() as u64 {
            return Err(io::Error::other("nonappend"));
        }
        d.data.write(raw)
    }
...
impl Seek for Memory {
    fn seek(&mut self, at: SeekFrom) -> io::Result<u64> {
        self.0.borrow_mut().data.seek(at)
    }
}
...
fn plan(key: &str) -> Result<DigPlan> {
    DigPlan::new(
        key,
        false,
...
fn binding() -> Result<DigBinding> {
    DigBinding::new(
        ...
        plan("dig-001")?.before(),
        dfmcp_core::MapCuboid::new(
            dfmcp_core::MapCoord::new(0, 0, 0),
            dfmcp_core::MapCoord::new(63, 63, 7),
        )?,
    )
}
...
fn proof(domain: &[u8], raw: &[u8]) -> Digest32 {
...
fn effect(p: &DigPlan, phase: DigPhase) -> Result<DigEffect> {
    ...
    for n in p.before().region().coordinates() {
        out.extend_from_slice(&n.to_be_bytes());
    }
    ...
    if phase == DigPhase::Designated {
        out.extend_from_slice(&fixture("designated")?[140..172]);
    } else {
        out.extend_from_slice(&[0; 32]);
    }
    let mut bytes = p.before().generation().to_be_bytes().to_vec();
    text(&mut bytes, p.key());
    bytes.extend_from_slice(p.digest().as_bytes());
    bytes.extend_from_slice(p.token());
    bytes.extend_from_slice(&out[133..172]);
    let receipt = if phase.terminal() {
        proof(b"dfmcp-dig-designation-receipt/1", &bytes)
    } else {
        Digest32::ZERO
    };
...
impl DigSource for Source {
    ...
    fn observe(&mut self, _: DigRegion, _: &OperationContext) -> Result<DigObservation> {
        Ok(plan("dig-001")?.before().clone())
    }
    fn prepare(&mut self, p: &DigPlan, _: &OperationContext) -> Result<DigPreparation> {
        self.calls.borrow_mut().mutations += 1;
        DigPreparation::decode(effect(p, DigPhase::Prepared)?.canonical_bytes(), false, p)
    }
    fn commit(&mut self, p: &DigPlan, _: &OperationContext) -> Result<DigEffect> {
        self.calls.borrow_mut().mutations += 1;
        effect(p, DigPhase::Designated)
    }
    fn cancel(&mut self, p: &DigPlan, _: &OperationContext) -> Result<DigEffect> {
        self.calls.borrow_mut().mutations += 1;
        effect(p, DigPhase::Refused)
    }
...
impl DigGuard for Guard {
    fn check(&mut self, s: DigStage, _: &DigPlan, _: &OperationContext) -> Result<()> {
        if self.build || s == DigStage::Query {
            Ok(())
        } else {
            Err(error(ErrorCode::CapabilityDenied, "test query-only guard"))
        }
    }
}
impl DigSessionGuard for Guard {
    ...
    fn observe(&mut self, _: &DigBinding, _: DigRegion, _: &OperationContext) -> Result<()> {
        Err(error(ErrorCode::CapabilityDenied, "no terrain reads"))
    }
}
fn seeded(memory: Memory, terminal: usize, pending: bool) -> Result<DigPlan> {
    let mut journal = DigJournal::open(
        memory,
        &control_context()?,
        DigMode::Control,
        Some(binding()?),
    ...
    let mut guard = Guard { build: true };
    for i in 0..terminal {
        let p = plan(&format!("old-{i:03}"))?;
        journal.prepare(&mut source, &p, &control_context()?, &mut guard)?;
        journal.cancel(
            &mut source,
            p.key(),
            p.digest(),
...
fn factory(
    calls: Rc<RefCell<Calls>>,
) -> impl FnOnce(&DigBinding, DigRegion, &OperationContext) -> Result<QueryOnly<Source>> {
    move |b, _, _| {
        calls.borrow_mut().connections += 1;
        Ok(QueryOnly(Source {
            calls,
            binding: b.clone(),
        }))
...
fn run(
    s: &mut State<Memory, QueryOnly<Source>>,
    op: &str,
    action: Result<Action>,
    calls: Rc<RefCell<Calls>>,
) -> Result<Value> {
    let c = s.context(None)?;
    let output = run_action(
        ...
        action,
        factory(calls),
        &mut Guard { build: false },
    );
    assert!(output.len() as u64 <= OUTPUT_BYTES);
    let value: Value = serde_json::from_str(&output).map_err(|_| exhausted())?;
    assert!(value["agent_turn"].is_object());
    assert_eq!(
        value["agent_turn"]["briefing"]["mutation_admissible"],
        false
    );
...
fn cold_arrival_discovers_pending_work_outside_the_first_records_page() -> Result<()> {
    ...
    seeded(memory.clone(), 9, true)?;
    let mut s = state(memory, DigMode::Offline)?;
    let calls = Rc::new(RefCell::new(Calls::default()));
    let value = run(
        &mut s,
        "fortress.query",
        Ok(Action::Query(Query::parse(
    ...
    assert_eq!(calls.borrow().connections, 0);
    Ok(())
}
#[test]
fn discovery_continuation_is_pinned_and_staged_without_native_calls() -> Result<()> {
    ...
    let token = first["result"]["continuation"]
        .as_str()
    ...
    assert_eq!(
        second["result"]["records"].as_array().map(Vec::len),
        Some(2)
    );
    let wrong = run(
        &mut s,
    ...
    assert_eq!(wrong["result"]["ok"], false);
...
fn exact_plan_explanation_and_tile_pages_remain_historical() -> Result<()> {
    ...
    let page = run(
        &mut s,
        "fortress.query",
        Ok(Action::Query(query)),
        calls.clone(),
    )?;
    assert_eq!(
        page["result"]["capture"]["tiles"].as_array().map(Vec::len),
        Some(16)
    );
    assert_eq!(page["result"]["capture"]["next_query"], Value::Null);
    assert_eq!(calls.borrow().connections, 0);
    Ok(())
}
#[test]
fn query_only_reconciliation_persists_terminal_proof_without_any_mutation() -> Result<()> {
    let memory = Memory::default();
    let p = seeded(memory.clone(), 0, true)?;
    let mut s = state(memory.clone(), DigMode::Recover)?;
    let calls = Rc::new(RefCell::new(Calls::default()));
    let before = memory.0.borrow().syncs;
    let value = run(
        &mut s,
        "fortress.wait",
        Ok(Action::Wait(p.key().into(), p.digest())),
        calls.clone(),
    )?;
    assert_eq!(value["result"]["record"]["state"], "terminal");
    assert!(memory.0.borrow().syncs > before);
    assert_eq!(calls.borrow().queries, 1);
    assert_eq!(calls.borrow().connections, 1);
    assert_eq!(calls.borrow().mutations, 0);
    drop(s);
    let mut offline = state(memory, DigMode::Offline)?;
    let recovered = run(
        &mut offline,
        "fortress.explain",
        Ok(Action::Explain(p.key().into(), p.digest())),
    ...
    assert_eq!(
        recovered["result"]["record"]["native"]["phase"],
        "designated"
    );
    assert_eq!(calls.borrow().connections, 1);
    Ok(())
...
fn missing_native_evidence_does_not_clear_the_pending_record() -> Result<()> {
    let memory = Memory::default();
    let p = seeded(memory.clone(), 0, true)?;
    let mut s = state(memory, DigMode::Recover)?;
    ...
    assert_eq!(
        value["agent_turn"]["active_work"]["counts"]["unsettled_records"],
        1
    );
    assert_eq!(calls.borrow().mutations, 0);
    Ok(())
...
fn offline_wait_and_all_mutation_tools_refuse_without_evaluating_factory() -> Result<()> {
    let memory = Memory::default();
    let p = seeded(memory.clone(), 0, true)?;
    let mut s = state(memory.clone(), DigMode::Offline)?;
    let calls = Rc::new(RefCell::new(Calls::default()));
    let before = memory.0.borrow().data.get_ref().clone();
    let value = run(
        &mut s,
        "fortress.wait",
        Ok(Action::Wait(p.key().into(), p.digest())),
    ...
    for op in [
        ...
        assert_eq!(
            run(&mut s, op, Ok(Action::Denied), calls.clone())?["result"]["ok"],
            false
        );
    }
    assert_eq!(calls.borrow().connections, 0);
    assert_eq!(memory.0.borrow().data.get_ref(), &before);
    Ok(())
}
#[test]
fn native_query_wrapper_independently_rejects_every_write_and_terrain_read() -> Result<()> {
    let calls = Rc::new(RefCell::new(Calls::default()));
    let mut source = QueryOnly(Source {
        calls: calls.clone(),
        binding: binding()?,
    });
    let p = plan("dig-001")?;
    assert!(
        source
            .observe(p.before().region(), &control_context()?)
            .is_err()
    );
    assert!(source.prepare(&p, &control_context()?).is_err());
    assert!(source.commit(&p, &control_context()?).is_err());
    assert!(source.cancel(&p, &control_context()?).is_err());
    assert_eq!(calls.borrow().mutations, 0);
    assert!(source.query(&p, &c()?)?.is_some());
    assert_eq!(calls.borrow().queries, 1);
    Ok(())
}
#[test]
fn wrong_digest_and_closed_query_shapes_fail_before_native_work() -> Result<()> {
    for raw in [
        ...
        "{\"mode\":\"schema\",\"path\":\"/private\"}",
        "{\"mode\":\"records\",\"limit\":true}",
        "{\"mode\":\"records\",\"continuation\":\"x\"}",
    ...
    assert_eq!(
        run(
            &mut s,
            "fortress.wait",
            Ok(Action::Wait(p.key().into(), Digest32::ZERO)),
            calls.clone()
        )?["result"]["ok"],
        false
    );
...
fn output_and_expired_wall_allowances_are_refused_before_journal_reads() -> Result<()> {
    let memory = Memory::default();
    seeded(memory.clone(), 0, true)?;
    let mut s = state(memory.clone(), DigMode::Offline)?;
    let reads = memory.0.borrow().reads;
    let calls = Rc::new(RefCell::new(Calls::default()));
    for expired in [false, true] {
        let mut c = s.context(None)?;
        if !expired {
            c.budget.max_output_tokens = 1;
        }
        let started = if expired {
            Instant::now()
        ...
        let parsed: Value = serde_json::from_str(&value).map_err(|_| exhausted())?;
        assert_eq!(parsed["result"]["ok"], false);
    }
    assert_eq!(memory.0.borrow().reads, reads);
    assert_eq!(calls.borrow().connections, 0);
    Ok(())
}
#[test]
...
fn unknown_custody_never_reuses_cached_success_metadata() -> Result<()> {
    ...
    seeded(memory.clone(), 0, true)?;
    let mut s = state(memory.clone(), DigMode::Offline)?;
    memory.0.borrow_mut().data.get_mut()[0] ^= 1;
    let calls = Rc::new(RefCell::new(Calls::default()));
    let value = run(
        &mut s,
        "fortress.doctor",
...
fn recovery_state_refuses_a_control_journal_even_with_broad_injected_grants() -> Result<()> {
    ...
    assert!(State::new(session, &control_context()?).is_err());
    Ok(())
}
#[test]
fn full_packets_and_fallbacks_fit_the_admitted_minimum_output_budget() -> Result<()> {
    let memory = Memory::default();
    let p = seeded(memory.clone(), 9, true)?;
    let mut s = state(memory, DigMode::Offline)?;
    let calls = Rc::new(RefCell::new(Calls::default()));
    for action in [
        Action::Inventory,
        Action::Explain(p.key().into(), p.digest()),
        Action::Denied,
    ...
    for op in [
        ...
        let value = unbound(
            op,
            &error(ErrorCode::InternalInvariantViolation, "private-token"),
        );
        assert!(value.len() as u64 <= OUTPUT_BYTES);
...
fn post_page_custody_failure_does_not_publish_an_unreturned_cursor() -> Result<()> {
    ...
    let mut s = state(memory.clone(), DigMode::Offline)?;
    {
        let mut disk = memory.0.borrow_mut();
        let chunks = disk.data.get_ref().len().div_ceil(32768);
    ...
    assert_eq!(value["result"]["ok"], false);
    assert!(s.cursors.entries.is_empty());
    Ok(())
}
#[test]
fn worst_bounded_source_metadata_and_result_shapes_fit_complete_packets() -> Result<()> {
    let raw = fixture("observation")?;
    let mut large = raw[..69].to_vec();
    text(&mut large, &"\u{1}".repeat(512));
    large.extend_from_slice(&raw[78..]);
    let before = DigObservation::decode(&large)?;
    let b = DigBinding::new(
        binding()?.endpoint(),
        DigManifest {
            generation: 7,
            df_version: "\u{1}".repeat(128),
            dfhack_version: "\u{1}".repeat(128),
        },
        &before,
        binding()?.scope(),
    )?;
    let pending = dfmcp_adapter::dig_designation::journal::DigSummary {
        key: "k".repeat(128),
        plan_digest: Digest32::from_bytes([255; 32]),
        state: dfmcp_adapter::dig_designation::journal::DigState::Tracking,
        native_phase: Some(DigPhase::Unknown),
        receipt: None,
        dispatchable: false,
    };
    let view = DigSessionView {
        journal_id: Digest32::from_bytes([255; 32]),
        head: Digest32::from_bytes([255; 32]),
        events: 896,
