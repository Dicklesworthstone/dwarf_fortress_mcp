...
fn object<'a>(value: &'a Value, keys: &[&str]) -> Result<&'a serde_json::Map<String, Value>> {
    ...
    if fields.len() != keys.len() || keys.iter().any(|k| !fields.contains_key(*k)) {
        return Err(denied());
    }
    Ok(fields)
}
fn number(value: &Value) -> Result<u64> {
    value.as_u64().ok_or_else(denied)
}
fn small(value: &Value) -> Result<u32> {
    u32::try_from(number(value)?).map_err(|_| denied())
}
fn string(value: &Value) -> Result<&str> {
    value.as_str().ok_or_else(denied)
}
...
pub(super) fn canonical(value: &Value) -> Result<Vec<u8>> {
    ...
    fn write(value: &Value, out: &mut Vec<u8>, depth: usize) -> Result<()> {
        ...
        match value {
            ...
            Value::Number(v) => {
                let text = if let Some(n) = v.as_u64() {
                    n.to_string()
...
fn goal(v: &Value) -> Result<FloorGoal> {
    ...
    FloorGoal::new(
        region(&v["region"])?,
        string(&v["folder"])?.to_owned(),
        small(&v["site"])?,
        number(&v["deadline_tick"])?,
        number(&v["stable_ticks"])?,
        small(&v["required_samples"])?,
        number(&v["max_gap_ticks"])?,
    )
}
fn capture(v: &Value, selected: Region) -> Result<LiveMapObservation> {
    object(v, &["manifest", "capture_hex"])?;
    let manifest = &v["manifest"];
    object(manifest, &["generation", "df_version", "dfhack_version"])?;
    let bytes = hex(string(&v["capture_hex"])?, 2048)?;
    let value = LiveMapObservation::decode_payload(
        &bytes,
        number(&manifest["generation"])?,
        string(&manifest["df_version"])?.to_owned(),
        string(&manifest["dfhack_version"])?.to_owned(),
    )?;
    if value.map.region != selected {
        return Err(denied());
    }
    Ok(value)
}
...
impl Archive {
    pub fn decode(raw: &[u8], mut checkpoint: impl FnMut() -> Result<()>) -> Result<Self> {
        ...
        for (sequence, line) in raw.split_inclusive(|b| *b == b'\n').enumerate() {
            checkpoint()?;
            if sequence >= 260 || line.len() > MAX_FRAME {
                return Err(denied());
            }
            let v: Value = serde_json::from_slice(line).map_err(|_| denied())?;
            object(&v, &["event", "sequence", "previous", "sha256"])?;
            if number(&v["sequence"])? != sequence as u64
                || string(&v["previous"])? != previous.to_string()
            {
                return Err(denied());
            }
            let mut exact = canonical(&v)?;
            exact.push(b'\n');
            ...
            let body =
                json!({"event":v["event"],"sequence":sequence,"previous":previous.to_string()});
            let mut proof = DOMAIN.to_vec();
            proof.extend_from_slice(&canonical(&body)?);
            let head = digest(string(&v["sha256"])?)?;
            if head != Digest32::of_bytes(&proof) {
                return Err(denied());
            }
            let event = &v["event"];
            let kind = string(&event["kind"])?;
            if let Some(h) = &mut history {
                if h.progress.status().terminal() {
                    return Err(denied());
                }
                match kind {
                    "read_started" => {
                        ...
                        if h.pending_read {
                            h.progress = h.progress.interrupt(false)?;
                        }
                        h.pending_read = true;
                        h.attempts += 1;
                    }
                    "sample" => {
                        ...
                        h.progress = h
                            .progress
                            .sample(capture(&event["sample"], h.progress.goal().region())?)?;
                    ...
                    "read_failed" => {
                        ...
                        h.progress = h.progress.interrupt(true)?;
                        h.pending_read = false;
                    }
                    "cancel" => {
                        object(event, &["kind"])?;
                        h.progress = h.progress.cancel()?;
                        h.pending_read = false;
                    }
                    _ => return Err(denied()),
                }
                ...
                object(
                    event,
                    &["kind", "format", "nonce", "endpoint", "goal", "sample"],
                )?;
                if kind != "begin"
                    || string(&event["format"])? != "dfmcp.excavation-goal/1"
                    || digest(string(&event["nonce"])?)? == Digest32::ZERO
                {
                    return Err(denied());
                }
