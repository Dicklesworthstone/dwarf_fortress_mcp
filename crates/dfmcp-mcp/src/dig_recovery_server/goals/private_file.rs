...
mod linux {
    ...
    impl ReadOnly {
        fn custody(&self) -> Result<()> {
            ...
            if parent.canonicalize().map_err(|_| denied())? != parent {
                return Err(denied());
            }
            ...
            let named = fs::symlink_metadata(leaf(
                &self.parent,
            ...
            for info in [&p, &named_parent] {
                if !info.is_dir()
                    ...
                    || (info.dev(), info.ino()) != (self.identity.0, self.identity.1)
                {
                    return Err(denied());
                }
            }
        ...
        fn read(&mut self, check: &mut impl FnMut() -> Result<()>) -> Result<Vec<u8>> {
            ...
            if stamp(&before) != stamp(&after) || bytes.len() != before.len() as usize {
                return Err(denied());
            }
            self.custody()?;
            check()?;
        ...
        pub fn open(path: &Path, check: &mut impl FnMut() -> Result<()>) -> Result<Self> {
            ...
            if !path.is_absolute()
                ...
                || raw[1..]
                    .split(|b| *b == b'/')
                    .any(|s| s.is_empty() || s == b"." || s == b".." || s.contains(&0))
            {
                return Err(denied());
            }
            ...
            for component in parent_path.iter().skip(1) {
                ...
                parent = OpenOptions::new()
                    .read(true)
                    .custom_flags(NOFOLLOW | DIRECTORY)
                    .open(leaf(&parent, component))
                    .map_err(|_| denied())?;
            }
            let file = OpenOptions::new()
                .read(true)
                .custom_flags(NOFOLLOW | NONBLOCK)
                .open(leaf(&parent, path.file_name().ok_or_else(denied)?))
                .map_err(|_| denied())?;
            file.try_lock().map_err(|_| denied())?;
            let p = parent.metadata().map_err(|_| denied())?;
            let f = file.metadata().map_err(|_| denied())?;
            let mut snapshot = Self {
                file,
                parent,
                path: path.to_owned(),
                raw: Vec::new(),
                identity: (p.dev(), p.ino(), f.dev(), f.ino()),
            };
            snapshot.raw = snapshot.read(check)?;
            Ok(snapshot)
        }
    }
}
...
impl Snapshot {
    pub fn open(path: &Path, check: &mut impl FnMut() -> Result<()>) -> Result<Self> {
        ...
        {
            Ok(Self {
                inner: linux::ReadOnly::open(path, check)?,
            })
        }
        #[cfg(not(all(
            target_os = "linux",
