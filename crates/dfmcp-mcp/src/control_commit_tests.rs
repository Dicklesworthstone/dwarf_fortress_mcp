...
fn io_error(_: io::Error) -> DfmcpError {
    err(ErrorCode::AdapterUnavailable, "control fixture I/O")
}
fn context() -> OperationContext {
    OperationContext {
        session_id: SessionId::new(812),
...
impl Fixture {
    fn new() -> Result<Self> {
        ...
        let mut journal =
            open_private_control_journal(&f.path, &context(), 7, EffectTailRecovery::Refuse)?;
...
fn register(session: ControlSession) -> Result<Registered> {
    let id = session.id;
    publish_session(session)?;
    Ok(Registered(id))
}
fn parse(raw: String) -> Result<Value> {
    serde_json::from_str(&raw).map_err(|_| err(ErrorCode::InvalidRequest, "fixture JSON"))
}
fn commit(s: &Registered) -> Result<Value> {
    parse(fortress_commit(
        s.raw(),
        "key".into(),
        plan().to_string(),
        "01".repeat(16),
...
fn close(s: &Registered) -> Result<Value> {
    parse(fortress_cancel(
        s.raw(),
        None,
        None,
        None,
...
fn commit_budget_refusal_precedes_connection_use_and_durable_intent() -> Result<()> {
    ...
    }
    {
        ...
        session.budget.max_actions = 0;
    }
    assert_eq!(commit(&s)?["result"]["error"]["code"], "invalid_request");
    assert_eq!(fs::read(&f.path).map_err(io_error)?, bytes);
    assert_eq!(close(&s)?["result"]["ok"], true);
    Ok(())
}
...
fn already_resolved_stale_handles_cannot_enter_an_operation_after_close() -> Result<()> {
    ...
    let result = parse(with_handle(&stale, "fortress.commit", |_, _| {
        entered = true;
        Ok(json!({"ok":true}))
    }))?;
    assert!(!entered);
    assert_eq!(result["result"]["error"]["code"], "session_not_found");
...
fn reply(stream: &mut TcpStream, body: &[u8]) -> io::Result<()> {
    let mut h = [0u8; 8];
    h[..2].copy_from_slice(&(-1i16).to_le_bytes());
    h[4..].copy_from_slice(&(body.len() as i32).to_le_bytes());
    stream.write_all(&h)?;
...
fn request(stream: &mut TcpStream) -> io::Result<Option<(i16, Vec<u8>)>> {
    ...
    let length = i32::from_le_bytes(
        h[4..]
            .try_into()
            .map_err(|_| io::Error::other("frame length"))?,
    );
...
fn terminal() -> Vec<u8> {
    ...
    number(&mut out, 11, 1);
    number(&mut out, 12, 1);
    number(&mut out, 13, 11);
    let mut seal = b"dfmcp-control-receipt-v2\0".to_vec();
    seal.extend_from_slice(&7u64.to_be_bytes());
    seal.extend_from_slice(b"key\0");
    seal.extend_from_slice(plan().as_bytes());
    seal.push(1);
    seal.extend_from_slice(&11u64.to_be_bytes());
    bytes(&mut out, 14, Digest32::of_bytes(&seal).as_bytes());
    out
}
struct Bridge {
    endpoint: SocketAddr,
...
impl Bridge {
    fn new(lose_reply: bool) -> Result<Self> {
        ...
        let worker = std::thread::spawn(move || -> io::Result<usize> {
            ...
            if hello.as_slice() != expected.as_slice() {
                return Err(io::Error::other("native hello mismatch"));
            }
            let mut welcome = b"DFHack!\n".to_vec();
            welcome.extend_from_slice(&1i32.to_le_bytes());
            stream.write_all(&welcome)?;
            for id in 2..=5 {
                if !matches!(request(&mut stream)?, Some((0, _))) {
                    return Err(io::Error::other("expected method bind"));
                }
                let mut body = Vec::new();
                number(&mut body, 1, id);
                reply(&mut stream, &body)?;
            }
            if !matches!(request(&mut stream)?, Some((2, _))) {
                return Err(io::Error::other("expected plugin handshake"));
            }
            reply(&mut stream, &common())?;
            let mut commits = 0;
            while let Some((method, body)) = request(&mut stream)? {
                if method != 4 || commits != 0 {
                    return Err(io::Error::other(
                        "mutation retried or unexpected bridge method",
                    ));
                }
                let mut expected = Vec::new();
                bytes(&mut expected, 1, &[b't'; 32]);
                bytes(&mut expected, 2, &[4; 16]);
                number(&mut expected, 3, 1);
                number(&mut expected, 4, 7);
                bytes(&mut expected, 5, b"key");
                bytes(&mut expected, 6, plan().as_bytes());
                bytes(&mut expected, 7, &[1; 16]);
                if body != expected {
                    return Err(io::Error::other("commit identity changed on wire"));
                }
                commits += 1;
                if lose_reply {
                    return Ok(commits);
    ...
    fn connect(&self) -> Result<ControlConnection> {
        ...
        let token = vec![b't'; 32];
        let nonce = vec![4; 16];
        let client =
            ControlRpcClient::connect(self.endpoint, token.clone(), nonce.clone(), timeout)?;
    ...
    fn finish(mut self) -> Result<usize> {
        self.worker
            .take()
            .ok_or_else(|| err(ErrorCode::InternalInvariantViolation, "worker absent"))?
            .join()
            .map_err(|_| {
                err(
                    ErrorCode::InternalInvariantViolation,
                    "protocol fixture panic",
                )
            })?
            .map_err(io_error)
    }
...
fn real_wire_commit_close_and_reopen_preserve_terminal_or_unresolved_evidence() -> Result<()> {
    ...
    for lose_reply in [false, true] {
        ...
        let first = commit(&s)?;
        if lose_reply {
            assert_eq!(first["result"]["error"]["code"], "effect_indeterminate");
        } else {
            assert_eq!(first["result"]["ok"], true);
            assert_eq!(first["result"]["mutation_dispatched"], true);
        }
        let retry = commit(&s)?;
        if lose_reply {
            assert_eq!(retry["result"]["error"]["code"], "effect_indeterminate");
        } else {
            assert_eq!(retry["result"]["replayed_terminal"], true);
            assert_eq!(retry["result"]["mutation_dispatched"], false);
        }
        let bytes = fs::read(&f.path).map_err(io_error)?;
        assert_eq!(close(&s)?["result"]["ok"], true);
        assert_eq!(bridge.finish()?, 1);
        let recovered = open_private_control_recovery(&f.path, &context())?;
        let expected = if lose_reply {
            DurablePauseState::Indeterminate
        } else {
            DurablePauseState::VerifiedApplied
        };
        assert_eq!(recovered.lookup("key").map(|r| r.state), Some(expected));
        drop(recovered);
        assert_eq!(fs::read(&f.path).map_err(io_error)?, bytes);
        let next = register(f.session(None)?)?;
        let replay = commit(&next)?;
        if lose_reply {
            assert_eq!(replay["result"]["error"]["code"], "effect_indeterminate");
        } else {
            assert_eq!(replay["result"]["replayed_terminal"], true);
        }
        assert_eq!(close(&next)?["result"]["ok"], true);
    }
    Ok(())
