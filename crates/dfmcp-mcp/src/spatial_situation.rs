...
const RULES: [Rule; 7] = [
    Rule {
        code: "citizen_not_alive",
        severity: "high",
        finding: "Strict citizens observed not alive",
        limitation: "Does not establish when or why a death occurred.",
        fields: &["alive", "position"],
    },
    Rule {
        code: "living_citizen_not_sane",
        severity: "high",
        finding: "Living strict citizens observed not sane",
        limitation: "Does not establish diagnosis, cause or imminent violence.",
        fields: &["alive", "sane", "position"],
    },
    Rule {
        code: "visible_magma",
        severity: "medium",
        finding: "Magma observed in the requested terrain region",
        limitation: "Magma may be contained; exposure, flooding and danger are not proved.",
        fields: &["visibility", "magma", "liquid_depth", "position"],
    },
    Rule {
        code: "suspended_jobs",
        severity: "medium",
        finding: "Current jobs observed suspended",
        limitation: "Suspension does not establish cause, age or criticality.",
        fields: &["type_key", "suspended", "blocking_reason"],
    },
    Rule {
        code: "unassigned_unsuspended_jobs",
        severity: "low",
        finding: "Unsuspended current jobs have no assigned worker",
        limitation: "May be normal queueing; labor shortage or blockage is not proved.",
        fields: &[
            "type_key",
            "suspended",
            "worker_assigned",
    ...
    Rule {
        code: "incomplete_buildings",
        severity: "low",
        finding: "Buildings observed below their maximum build stage",
        limitation: "Does not prove stalled construction or missing materials.",
        fields: &["build_stage", "max_build_stage"],
    },
    Rule {
        code: "retained_rotten_items",
        severity: "low",
        finding: "Non-removed items observed rotten",
        limitation: "These are item counts, not edible stock or a food-shortage diagnosis.",
        fields: &["type_key", "rotten", "removed"],
    },
];
...
fn known<'a>(
    ...
    if fact.observed_at != anchor.tick
...
fn boolean(
    ...
    match known(entity, field, anchor, source) {
        Some(WorldValue::Bool(value)) => Some(*value),
        _ => None,
    }
}
fn integer(
    ...
    match known(entity, field, anchor, source) {
        Some(WorldValue::I64(value)) => Some(*value),
        _ => None,
    }
}
...
fn applies(index: usize, entity: &EntityRecord) -> bool {
    matches!(
        (index, &entity.kind),
        (0 | 1, EntityKind::Unit)
            | (2, EntityKind::TileFeature)
            | (3 | 4, EntityKind::Job)
            | (5, EntityKind::Building)
            | (6, EntityKind::Item)
    )
}
fn evaluate(
    ...
    match index {
        ...
        2 => {
            ...
            if !matches!(known(entity,"visibility",anchor,source), Some(WorldValue::Text(text)) if text=="visible")
            {
                return None;
            }
            let depth = match known(entity, "liquid_depth", anchor, source) {
                Some(WorldValue::U64(n)) if *n <= 7 => Some(*n > 0),
                _ => None,
            };
        ...
        3 => flag("suspended"),
        4 => both(
            flag("suspended").map(|value| !value),
            flag("worker_assigned").map(|value| !value),
        ),
        5 => match (
            integer(entity, "build_stage", anchor, source),
            integer(entity, "max_build_stage", anchor, source),
        ) {
            (Some(stage), Some(maximum)) if stage >= 0 && maximum >= stage => Some(stage < maximum),
            _ => None,
        },
...
pub(super) fn build(
    ...
    if context.anchor != snapshot.anchor() {
        return Err(failure(
            ...
            "situation context names another snapshot",
        ));
    }
    if snapshot.graph.entities.len() > context.budget.max_entities as usize
        || snapshot.graph.entities.len() > MAX_ENTITIES
    {
        return Err(failure(
            ErrorCode::BudgetExceeded,
            "situation exceeds its entity-scan bound",
        ));
    }
    ...
    let check = || -> Result<()> {
        ...
        if started.elapsed().as_millis() >= u128::from(context.budget.max_wall_millis) {
            return Err(failure(
                ErrorCode::BudgetExceeded,
                "situation exceeded its cooperative wall-time allowance",
            ));
        }
        Ok(())
    };
    ...
    if !snapshot.hash_is_valid() {
        return Err(failure(
            ...
            "situation snapshot hash is invalid",
        ));
    }
    ...
    for (ordinal, (id, entity)) in snapshot.graph.entities.iter().enumerate() {
        ...
        if *id != entity.id {
            return Err(failure(
                ErrorCode::InternalInvariantViolation,
                "situation entity map key differs",
            ));
        }
        let category = match entity.kind {
            EntityKind::Unit => Some("citizens"),
            EntityKind::Job => Some("jobs"),
            EntityKind::Building => Some("buildings"),
            EntityKind::Item => Some("items"),
            EntityKind::TileFeature => Some("terrain_cells"),
            _ => None,
        };
        ...
        if entity.kind == EntityKind::TileFeature {
            match known(entity, "visibility", context.anchor, source) {
                Some(WorldValue::Text(text)) if text == "hidden" => {
                    *counts.entry("hidden_cells").or_default() += 1
                }
                Some(WorldValue::Text(text)) if text == "unallocated" => {
                    *counts.entry("unallocated_cells").or_default() += 1
                }
                _ => {}
            }
        }
        for (index, signal) in signals.iter_mut().enumerate() {
            if !applies(index, entity) {
                continue;
            }
            signal.examined += 1;
            match evaluate(index, entity, context.anchor, source) {
                ...
                Some(true) => {
                    signal.matches += 1;
                    if signal.example.is_none() {
                        signal.example = Some(Subject {
                            id: id.to_string(),
...
impl Report {
    pub(super) fn summary(&self) -> Value {
        let signals: BTreeMap<_, _> = RULES
            .iter()
            .zip(&self.signals)
            .map(|(rule, signal)| {
                (
                    rule.code,
                    json!({"observed":signal.matches,"unestablished":signal.unknown}),
                )
            })
            .collect();
        let findings = self
    ...
    pub(super) fn comparison(&self, before: &Report) -> (Value, Vec<Value>) {
        let compatible = self.anchor.fortress_id == before.anchor.fortress_id
            ...
            && self.anchor.cursor.sequence >= before.anchor.cursor.sequence
            && self.anchor.tick >= before.anchor.tick
            && (self.anchor.cursor != before.anchor.cursor || self.anchor == before.anchor);
        if !compatible {
            return (
                json!({"status":"reset","basis":anchor_json(before.anchor),"comparable":false,
                "reason":"observation_epoch_identity_or_order_changed"}),
                Vec::new(),
            );
        }
        let mut changes = Vec::new();
        if self.paused != before.paused {
            changes.push(
                json!({"kind":"observed_pause_change","before":before.paused,"after":self.paused}),
            );
        }
        ...
        for ((rule, prior), next) in RULES.iter().zip(&before.signals).zip(&self.signals) {
            if (prior.matches, prior.unknown) != (next.matches, next.unknown) {
                changes.push(
                    json!({"kind":"situation_signal_count_change","rule":rule.code,
