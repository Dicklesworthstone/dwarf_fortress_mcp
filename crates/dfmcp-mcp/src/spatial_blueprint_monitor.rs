...
pub(super) fn proposal(layout: &BlueprintLayout, dimensions: [u32; 3],
    context: &OperationContext, options: Options) -> Result<Value> {
    ...
    if options.key.is_empty() || options.key.len() > 64 || options.key.contains('\0') {
        return Err(invalid("blueprint monitor key must contain 1..64 UTF-8 bytes without NUL"));
    }
    if !(1..=1_000_000).contains(&cadence) || !(1..=64).contains(&stability) {
        return Err(invalid("blueprint monitor cadence must be 1..1000000 and stability 1..64"));
    }
    if options.deadline_tick <= context.anchor.tick.0
        || options.deadline_tick.checked_sub(context.anchor.tick.0)
            .is_none_or(|ticks| ticks > context.budget.max_game_ticks) {
        return Err(invalid("blueprint monitor deadline must be future and within the negotiated game-tick horizon"));
    }
    if dimensions.iter().any(|size| *size == 0 || *size > 32_768) {
        return Err(invalid("blueprint monitor requires established map dimensions"));
    }
    let first = layout.excavations().first().ok_or_else(|| invalid("blueprint monitor mask is empty"))?;
    let mut areas = Vec::with_capacity(layout.excavations().len());
    for part in layout.excavations() {
        if part.mode != first.mode {
            return Err(invalid("mixed excavation modes need separate explicit shape watches"));
        }
        for point in [part.area.min, part.area.max] {
            for (coordinate, size) in [point.x, point.y, point.z].into_iter().zip(dimensions) {
                if coordinate < 0 || coordinate as u32 >= size {
                    return Err(invalid("blueprint monitor footprint extends outside the map; no coordinates were dropped or clamped"));
                }
            }
        }
    ...
    let targets: &[&str] = match first.mode {
        ...
        _ => return Err(invalid("blueprint excavation mode has no declared shape-monitor policy")),
    };
    let fields: Vec<Value> = targets.iter().map(|shape| json!({"op":"field",
        "field":"shape","comparison":"eq","value":{"type":"text","value":shape}})).collect();
    let predicate = if fields.len() == 1 { fields[0].clone() } else { json!({"op":"any","args":fields}) };
    ...
    let identity = json!({"policy":POLICY,"session_id":context.session_id.to_string(),"request":request});
    let encoded = serde_json::to_vec(&identity).map_err(|_| invalid("blueprint watch proposal cannot be encoded"))?;
    Ok(json!({"policy":POLICY,"proposal_digest":Digest32::of_bytes(&encoded).to_string(),
