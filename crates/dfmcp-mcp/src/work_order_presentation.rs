...
use dfmcp_adapter::work_order_control::{
    CreationRecord, CreationState, CreationSummary, JournalMode,
};
...
pub(super) fn error(code: ErrorCode, text: &str) -> DfmcpError {
...
pub(super) fn recipe(raw: &str) -> Result<WorkOrderRecipe> {
    match raw {
        "wooden_bed" => Ok(WorkOrderRecipe::WoodenBed),
        "wooden_door" => Ok(WorkOrderRecipe::WoodenDoor),
        "wooden_table" => Ok(WorkOrderRecipe::WoodenTable),
        "wooden_chair" => Ok(WorkOrderRecipe::WoodenChair),
        _ => Err(error(
            ErrorCode::InvalidRequest,
            "unknown finite furniture recipe",
        )),
    }
}
pub(super) fn mode_name(mode: JournalMode) -> &'static str {
    match mode {
        JournalMode::Control => "control",
        JournalMode::Reconcile => "reconcile",
        JournalMode::Offline => "offline",
    }
}
...
pub(super) fn record_json(record: &CreationRecord) -> Value {
    ...
    let native_state = match effect.state() {
        WorkOrderState::Prepared => "prepared",
        WorkOrderState::Unknown => "unknown",
        WorkOrderState::Created => "created",
        WorkOrderState::Refused => "refused",
    };
...
pub(super) struct TurnView<'a> {
    ...
    pub mode: Option<JournalMode>,
    pub summary: Option<&'a CreationSummary>,
    pub selected: Option<&'a WorkOrderObservation>,
}
pub(super) fn packet(operation: &str, result: Value, view: TurnView<'_>) -> String {
    ...
    let production = view.mode == Some(JournalMode::Control)
        && view.context.is_some_and(|c| {
            c.authorize(
    ...
    if let Some(s) = view.summary {
        work["prepared_count"] = json!(s.prepared);
        work["unresolved_count"] = json!(s.unresolved);
        work["terminal_count"] = json!(s.terminal);
        work["details_omitted"] = json!(s.prepared + s.unresolved);
        ...
        if s.prepared != 0 {
            work["pending_plans"] = json!([{"retained_count":s.prepared,"discover":discovery}]);
        }
        if s.unresolved != 0 {
            work["indeterminate_effects"] =
                json!([{"retained_count":s.unresolved,"discover":discovery}]);
        }
...
impl Filter {
    pub fn parse(raw: &str) -> Result<Self> {
        match raw {
            ...
            _ => Err(error(
                ErrorCode::InvalidRequest,
                "creation state must be all, pending or reconciliation_required",
            )),
        }
    }
    pub fn name(self) -> &'static str {
        match self {
            ...
            Self::Unresolved => "reconciliation_required",
        }
    }
    ...
    pub fn total(self, s: &CreationSummary) -> usize {
        match self {
            Self::All => s.records,
            Self::Pending => s.prepared + s.unresolved,
...
impl Continuations {
    pub fn resolve(
        ...
        let cursor = self
            ...
            .ok_or_else(|| {
                error(
                    ...
                    "creation continuation expired or was not issued; restart discovery",
                )
            })?;
            ...
            || cursor.limit != limit
        {
            return Err(error(
                ErrorCode::StaleAnchor,
                "creation continuation scope or journal head changed",
            ));
        }
    ...
    pub fn issue(
        ...
        self.serial = self.serial.checked_add(1).ok_or_else(|| {
            error(
                ...
                "creation continuation identities exhausted",
            )
        })?;
