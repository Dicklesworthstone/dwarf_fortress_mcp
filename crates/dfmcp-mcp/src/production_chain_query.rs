...
use dfmcp_adapter::operations_analysis::{AnalysisHandle, OperationsStateView, MAX_ANALYSIS_WORK, SUPPLY_POLICY};
use dfmcp_adapter::operations_analysis::production_chain::{self as chain, ProductionChainAnalysis, ProductionResource};
...
fn invariant(s: &str) -> DfmcpError { DfmcpError::new(ErrorCode::InternalInvariantViolation, s) }

#[derive(Clone, Deserialize)]
#[serde(deny_unknown_fields)]
struct ResourceInput {
    key: String, item_types: Vec<String>, subtype: Option<i32>,
    material_type: Option<i32>, material_index: Option<i32>,
}
impl From<ResourceInput> for ProductionResource {
    fn from(v: ResourceInput) -> Self { Self { key: v.key, item_types: v.item_types,
        subtype: v.subtype, material_type: v.material_type, material_index: v.material_index } }
}
#[derive(Clone, Deserialize)]
#[serde(deny_unknown_fields)]
struct QuotaInput { resource: String, minimum_stock: u32 }
impl From<QuotaInput> for ProductionQuota {
    fn from(v: QuotaInput) -> Self { Self { item_token: v.resource, minimum_stock: v.minimum_stock } }
}
#[derive(Clone, Deserialize)]
#[serde(deny_unknown_fields)]
struct Ingredient { resource: String, units: u32 }
#[derive(Clone, Deserialize)]
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
enum Workshop { Workshop { type_key: String }, Furnace { type_key: String } }
#[derive(Clone, Deserialize)]
#[serde(deny_unknown_fields)]
struct RecipeInput {
    output: String, output_batch_size: u32, inputs: Vec<Ingredient>,
    job_token: String, workshop: Workshop,
}
impl From<RecipeInput> for ProductionRecipe {
    fn from(v: RecipeInput) -> Self { Self { output_token: v.output, output_batch_size: v.output_batch_size,
        input_tokens: v.inputs.into_iter().map(|i| (i.resource, i.units)).collect(), job_token: v.job_token,
        workshop: match v.workshop { Workshop::Workshop { type_key } => BuildingKind::Workshop(type_key),
            Workshop::Furnace { type_key } => BuildingKind::Furnace(type_key) } } }
}
#[derive(Clone, Copy, Deserialize, Serialize)]
#[serde(rename_all = "snake_case")]
enum Section { All, Resources, Recipes, Steps, Shortages }
#[derive(Deserialize)]
#[serde(rename_all = "snake_case")]
enum Unit { StackUnits }
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
...
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
enum Query {
    ProductionChain { quantity_unit: Unit, resources: Vec<ResourceInput>, quotas: Vec<QuotaInput>,
        recipes: Vec<RecipeInput>, section: Option<Section>, limit: Option<u32>,
...
fn validate_shape(input: &Value) -> Result<()> {
    ...
    while let Some((value, depth)) = pending.pop() {
        ...
        if nodes > 4096 || depth > 16 { return Err(budget("production-chain input exceeds shape bounds")); }
        match value {
        ...
        if bytes > 131_072 { return Err(budget("production-chain input exceeds 128 KiB")); }
    }
    Ok(())
}
fn remaining(context: &OperationContext, start: Instant) -> Result<OperationContext> {
    ...
    if elapsed >= u128::from(context.budget.max_wall_millis) {
        return Err(budget("production-chain request exhausted its wall-time allowance"));
    }
    let mut c = context.clone(); c.budget.max_wall_millis -= elapsed as u64; Ok(c)
}
...
fn ingredients(items: &[(String, u32)]) -> Vec<Value> {
    items.iter().map(|(resource, units)| json!({"resource":resource,"units":units})).collect()
}
fn rows(report: &ProductionChainAnalysis, section: Section) -> Result<Vec<Value>> {
    ...
    if matches!(section, Section::All | Section::Resources) {
        for stock in &report.resources {
            ...
            let balance = report.plan.requirements().iter().find(|r| &r.item_token == key);
    ...
    if matches!(section, Section::All | Section::Steps) {
        for (index, step) in report.plan.steps().iter().enumerate() {
            let dependencies = step.depends_on.iter().map(|&dependency| {
                if dependency >= index { return Err(invariant("production dependency is not an earlier step")); }
                let earlier = report.plan.steps().get(dependency).ok_or_else(|| invariant("production dependency absent"))?;
...
fn base(report: &ProductionChainAnalysis) -> Result<Value> {
    ...
    for quota in &report.quotas {
        let stock = report.resources.iter().find(|r| r.resource.key == quota.item_token)
            .ok_or_else(|| invariant("production quota resource absent"))?;
        goals_met &= stock.stock_units >= quota.minimum_stock;
    }
    Ok(json!({"schema":"dfmcp.query.result/1","kind":"production_chain","anchor":anchor_json(report.anchor),
...
fn page_identity(c: &OperationContext, model: Digest32, section: Section, work: u64) -> Digest32 {
    Digest32::of_bytes(json!({"domain":POLICY,"session":c.session_id.to_string(),"anchor":anchor_json(c.anchor),
        "model":model.to_string(),"section":section,"max_work":work}).to_string().as_bytes())
}
fn token(offset: usize, id: Digest32) -> String {
    let mut bytes = b"dfmcp-production-chain-page-v1\0".to_vec();
    bytes.extend_from_slice(id.as_bytes()); bytes.extend_from_slice(&(offset as u64).to_be_bytes());
...
fn offset(raw: Option<&str>, id: Digest32, count: usize) -> Result<usize> {
    ...
    if raw.len() > 128 { return Err(budget("production-chain cursor exceeds 128 bytes")); }
    let mut p = raw.split(':');
    ...
    let n = n.parse::<usize>().map_err(|_| invalid("production-chain offset overflow"))?;
    if raw != token(n, id) { return Err(DfmcpError::new(ErrorCode::StaleAnchor, "production-chain cursor names another session, capture, model, section or budget")); }
    if n >= count { return Err(DfmcpError::new(ErrorCode::CursorGap, "production-chain cursor is beyond the result")); }
    Ok(n)
}
fn paginate(mut out: Value, rows: &[Value], c: &OperationContext, limit: u32,
    continuation: Option<&str>, identity: Digest32) -> Result<Value> {
    if !(1..=128).contains(&limit) { return Err(budget("production-chain page limit must be 1..128")); }
    let start = offset(continuation, identity, rows.len())?;
    let maximum = c.budget.max_bytes.min(u64::from(c.budget.max_output_tokens) * 4);
    out["analysis_digest"] = json!(identity.to_string()); out["total_rows"] = json!(rows.len());
    out["rows"] = json!([]); out["returned"] = json!(0); out["truncated"] = json!(false); out["continuation"] = Value::Null;
    let mut end = start;
    for row in rows.iter().skip(start).take(limit.min(c.budget.max_entities) as usize) {
        let mut next = out.clone();
        next["rows"].as_array_mut().ok_or_else(|| invariant("production-chain rows absent"))?.push(row.clone());
        next["returned"] = json!(end + 1 - start); next["truncated"] = json!(end + 1 < rows.len());
        next["continuation"] = if end + 1 < rows.len() { json!(token(end + 1, identity)) } else { Value::Null };
        if next.to_string().len() as u64 > maximum { break; }
        out = next; end += 1;
    }
    if (end == start && start < rows.len()) || out.to_string().len() as u64 > maximum {
        return Err(budget("complete production-chain summary and one row do not fit"));
    }
    Ok(out)
}
...
pub(super) fn execute<S: OperationsStateView + ?Sized>(state: &S, c: &OperationContext, input: &Value) -> Result<Value> {
    ...
    let envelope: Envelope = serde_json::from_value(input.clone()).map_err(|_| invalid("invalid production-chain request"))?;
    if envelope.schema != "dfmcp.query/1" { return Err(invalid("production-chain requires dfmcp.query/1")); }
    if envelope.expected_anchor.is_some_and(|a| a != anchor_json(c.anchor)) {
        return Err(DfmcpError::new(ErrorCode::StaleAnchor, "production-chain expected anchor differs"));
    }
    let Query::ProductionChain { quantity_unit: Unit::StackUnits, resources, quotas, recipes,
        section, limit, continuation, max_work } = envelope.query;
    let limit = limit.unwrap_or(8); let work = max_work.unwrap_or(MAX_ANALYSIS_WORK);
    if !(1..=128).contains(&limit) || work == 0 || work > MAX_ANALYSIS_WORK {
        return Err(budget("production-chain page or work allowance exceeds bounds"));
    }
    let resources = resources.into_iter().map(ProductionResource::from).collect::<Vec<_>>();
    let quotas = quotas.into_iter().map(ProductionQuota::from).collect::<Vec<_>>();
    let recipes = recipes.into_iter().map(ProductionRecipe::from).collect::<Vec<_>>();
    let report = chain::plan_production_chain(state, &remaining(c, started)?, &resources, &quotas, &recipes, work)?;
    let section = section.unwrap_or(Section::All);
    let identity = page_identity(c, report.model_digest, section, work);
    let mut out = base(&report)?; out["section"] = json!(section);
    let out = paginate(out, &rows(&report, section)?, c, limit, continuation.as_deref(), identity)?;
    c.authorize(Capability::Query, RiskTier::ReadOnly, &[], None)?; remaining(c, started)?;
    Ok(out)
}
