...
struct Script {
    value: Option<LiveSpatialCitizenObservation>,
    calls: Calls,
    fenced: bool,
    archive: bool,
}
impl Source for Script {
    fn read(&mut self, allowance: Duration) -> Result<LiveSpatialCitizenObservation> {
        ...
        self.value
            .take()
            .ok_or_else(|| error(ErrorCode::AdapterUnavailable, "injected read loss"))
    }
...
fn call(s: &mut Session, input: Value) -> Result<Value> {
    ...
    let snapshot = s
        .state
        .snapshot()
        .ok_or_else(|| error(ErrorCode::InvalidRequest, "snapshot"))?;
    let encoded =
        semantic_query::execute_with_publisher(snapshot, &c, &input, |v| Ok(v.to_string()))?;
    decode(&encoded)
}
...
fn register(s: &mut Session) -> Result<Value> {
    ...
    call(
        s,
        json!({"schema":"dfmcp.query/1","query":{"kind":"watch","key":"survive-outage",
...
fn poll(s: &mut Session, handle: &Value) -> Result<Value> {
    call(
        s,
        json!({"schema":"dfmcp.query/1","query":{"kind":"poll_watch","watch":handle}}),
    )
}
fn run(s: &mut Session, tick: Option<u32>, calls: &Calls, times: &[u64]) -> Result<Value> {
    let c = s.context()?;
    let calls = calls.clone();
    let mut times: VecDeque<_> = times.iter().copied().collect();
    let output = recover(
        s,
        &c,
        move |allowance| {
            calls.connections.fetch_add(1, Ordering::SeqCst);
            lock(&calls.allowances)?.push(allowance);
            ...
            Ok(Box::new(Script {
                value: Some(fixture::observation(tick, 2, 2, false)?),
                calls,
                fenced: false,
                archive: false,
            }) as Box<dyn Source>)
...
fn recovery_request(s: &Session) -> Result<Value> {
    Ok(
        json!({"schema":"dfmcp.query/1","expected_anchor":anchor_json(s.anchor()?),"query":{"kind":KIND}}),
    )
}
...
fn one_reconnect_preserves_handles_and_resets_progress_before_a_fresh_sample() -> Result<()> {
    ...
    assert_eq!(out["source_gap"]["changed_watches"], 1);
    assert_eq!(out["samples_added"], 0);
    assert_eq!(
        out["agent_turn"]["active_work"]["obligations"][0]["stable_observations"],
        0
    );
    ...
    assert_eq!(
        ...
        created["record"]["definition"]
    );
    assert_eq!(next["record"]["sample_count"], 2);
    Ok(())
}
...
fn failed_and_unchanged_recovery_never_manufacture_success_samples() -> Result<()> {
    ...
    assert_eq!(failure["source_recovery"]["transfer_pages"], 0);
...
fn recovery_revalidates_both_read_grants_before_memory_or_durable_publication() -> Result<()> {
    ...
    for durable in [false, true] {
        for capability in [Capability::Observe, Capability::Query] {
            let files = Files::new()?;
            let mut s = session(durable.then_some(&files), durable)?;
            register(&mut s)?;
            let old = s.anchor()?;
            let before = if durable {
                Some(fs::read(&files.observations).map_err(io_error)?)
            } else {
                None
            };
            for grant in &mut s.grants {
                if grant.capability == capability {
                    grant.expires_at_tick = Some(GameTick(old.tick.0 + 1));
                }
            }
            let calls = Calls::default();
            let result = run(&mut s, Some(5), &calls, &[0, 0, 0, 0])?;
            assert_eq!(result["ok"], false);
            assert_eq!(
                result["source_recovery"]["error"]["code"],
...
fn malformed_or_unprivileged_requests_are_refused_before_gap_or_network() -> Result<()> {
    ...
    let created = register(&mut s)?;
    let c = s.context()?;
    let valid = recovery_request(&s)?;
    request(&s, &c, &valid)?;
    for field in ["endpoint", "token", "protocol", "path", "repair"] {
        let mut input = valid.clone();
        input["query"][field] = json!("arbitrary");
        assert!(request(&s, &c, &input).is_err());
    }
    for millis in [
        json!(0),
        json!(60001),
        ...
        let mut input = valid.clone();
        input["query"]["max_wall_millis"] = millis;
        assert!(request(&s, &c, &input).is_err());
    }
    let mut missing = valid.clone();
    missing
        .as_object_mut()
        .ok_or_else(|| error(ErrorCode::InvalidRequest, "object"))?
        .remove("expected_anchor");
    assert!(request(&s, &c, &missing).is_err());
    let mut stale = valid;
    stale["expected_anchor"]["sequence"] = json!(999);
    assert!(request(&s, &c, &stale).is_err());
    for case in 0..3 {
        let mut c = c.clone();
        match case {
        ...
        assert!(
            recover(
                ...
                |_| {
                    connections += 1;
                    Err(error(ErrorCode::AdapterUnavailable, "unexpected network"))
                },
                || Duration::ZERO
            )
            .is_err()
    ...
    assert_eq!(
        same["record"]["evidence_digest"],
        created["record"]["evidence_digest"]
    );
    Ok(())
}
...
fn full_packet_reservation_refuses_without_interrupting_or_connecting() -> Result<()> {
    ...
    let created = register(&mut s)?;
    let mut c = s.context()?;
    let before = created["record"]["evidence_digest"].clone();
    // Force a refusal below even the fixed recovery renderer's minimum packet.
    c.budget.max_bytes = 1024;
    let mut connections = 0;
    assert!(
        matches!(recover(&mut s,&c,|_| {connections+=1;Err(error(ErrorCode::AdapterUnavailable,"unexpected"))},||Duration::ZERO),
        Err(e) if e.code==ErrorCode::BudgetExceeded)
    );
    assert_eq!(connections, 0);
    assert_eq!(
        poll(&mut s, &created["record"]["watch"])?["record"]["evidence_digest"],
        before
    );
    Ok(())
}
...
fn connection_and_capture_share_the_remaining_budget_and_expiry_stops_capture() -> Result<()> {
    ...
    assert_eq!(out["ok"], true);
    let allowances = lock(&calls.allowances)?;
    assert_eq!(allowances[0], Duration::from_millis(57000));
    assert!(allowances[1] <= Duration::from_millis(56000));
    drop(allowances);
    drop(s);
    let mut s = session(None, false)?;
    let old = s.anchor()?;
    ...
    for (fenced, archive) in [(false, false), (true, true)] {
        ...
        assert!(
            recover(
                ...
                |_| {
                    calls += 1;
                    Err(error(ErrorCode::AdapterUnavailable, "unexpected"))
                },
                || Duration::ZERO
            )
            .is_err()
...
fn journal_corruption_prevents_reconnect_without_repair_or_watch_changes() -> Result<()> {
    ...
    fs::OpenOptions::new()
        ...
        .map_err(io_error)?;
    let corrupt = fs::read(&files.observations).map_err(io_error)?;
    let old = s.anchor()?;
    let calls = Calls::default();
    assert!(
        matches!(run(&mut s,Some(4),&calls,&[0,0,0]),Err(e)if e.code==ErrorCode::CorruptLedger)
    );
...
fn paired_journals_record_gap_before_reconnect_and_do_not_sample_on_success() -> Result<()> {
    ...
    let mut s = session(Some(&files), true)?;
    let created = register(&mut s)?;
    let previous = fs::read(&files.watches).map_err(io_error)?;
    let c = s.context()?;
    let out = recover(
        &mut s,
        &c,
        |_| {
            ...
            assert!(gap.len() > previous.len());
            assert!(
                gap.windows(b"source_gap_requires_fresh_observation".len())
                    .any(|w| w == b"source_gap_requires_fresh_observation")
            );
            Ok(Box::new(Script {
                value: Some(fixture::observation(4, 2, 2, false)?),
                calls: Calls::default(),
                fenced: false,
                archive: false,
            }) as Box<dyn Source>)
    ...
    assert_eq!(
        out["source_gap"]["records"][0]["watch"],
        created["record"]["watch"]
    );
    assert_eq!(s.journal.as_ref().map(|j| j.entries().len()), Some(2));
    assert_eq!(
        out["agent_turn"]["active_work"]["obligations"][0]["status"],
        "blocked_unknown"
    );
    Ok(())
}
...
fn reconnect_epoch_reset_stays_explicit_and_invalidates_the_old_watch_on_poll() -> Result<()> {
    ...
    assert_eq!(out["source_recovery"]["capture_outcome"], "reset");
    assert!(s.anchor()?.cursor.epoch > old.cursor.epoch);
    assert_eq!(out["agent_turn"]["continuity"]["status"], "partial");
    assert_eq!(
        poll(&mut s, &created["record"]["watch"])?["record"]["status"],
        "invalidated"
    );
    Ok(())
}
...
fn diagnostic_is_utf8_bounded_and_final_metadata_uses_the_reserved_shape() -> Result<()> {
    ...
    let out = recover(
        &mut s,
        &c,
        |_| Err(error(ErrorCode::AdapterUnavailable, &text)),
        || Duration::ZERO,
    )?;
    let out = decode(&out)?;
    let message = out["source_recovery"]["error"]["message"]
        .as_str()
        .ok_or_else(|| error(ErrorCode::InvalidRequest, "message"))?;
    assert_eq!(message.len(), 256);
    assert_eq!(out["source_recovery"]["error"]["message_truncated"], true);
    assert_eq!(out["mutation_dispatched"], false);
    assert_eq!(out["samples_added"], 0);
    assert_eq!(out["agent_turn"]["anchor"], anchor_json(s.anchor()?));
    assert!(out.to_string().len() <= c.budget.max_bytes as usize);
    Ok(())
}
...
fn failed_reconnect_gap_is_recovered_from_the_same_paired_archive_after_restart() -> Result<()> {
    ...
    let old_handle = created["record"]["watch"].clone();
    drop(s);
    let mut recovered = session(Some(&files), true)?;
    let listing = call(
        &mut recovered,
        json!({"schema":"dfmcp.query/1","query":{"kind":"watches"}}),
    )?;
    let handle = listing["records"][0]["watch"].clone();
    assert_ne!(handle, old_handle);
    assert_eq!(listing["records"][0]["status"], "blocked_unknown");
    assert_eq!(
        listing["records"][0]["recovery"]["prior_evidence_digest"],
        gap_digest
    );
    let record = poll(&mut recovered, &handle)?;
    assert_eq!(record["record"]["sample_count"], 1);
    assert_eq!(record["record"]["stable_observations"], 0);
    assert_eq!(
