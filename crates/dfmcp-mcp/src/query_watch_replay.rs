...
impl Replay {
    ...
    pub(crate) fn new(
        ...
        if first.fortress_id != context.anchor.fortress_id || binding == Digest32::ZERO {
            return Err(failure(
                ...
                "historical monitor names another observation universe",
            ));
        }
        if first_record == 0
            || last_record
        ...
        {
            return Err(bounded(
                "historical monitor replays one to 32 consecutive retained observations",
            ));
        }
        ...
        let definition = Definition {
            key: "historical-replay".into(),
            label: "Historical monitor replay".into(),
            condition: input.condition,
            failure_condition: input.failure_condition,
            deadline_tick: input.deadline_tick,
            poll_interval_ticks: input.poll_interval_ticks.unwrap_or(1),
            stable_observations: input.stable_observations.unwrap_or(2),
        };
        validate_definition(&definition)?;
        if definition.deadline_tick <= first.tick.0 {
            return Err(invalid(
                "historical monitor deadline must follow its first selected capture",
            ));
        }
    ...
    pub(crate) fn advance(
        ...
        if self.failed {
            return Err(invalid("historical monitor replay is already failed"));
        }
    ...
    fn advance_inner(
        ...
        {
            return Err(invalid(
                "historical monitor observations must be complete and consecutive",
            ));
        }
        if snapshot.fortress_id != context.anchor.fortress_id
            || snapshot.fortress_id != self.watch.created_at.fortress_id
        {
            return Err(failure(
                ErrorCode::StaleAnchor,
                "historical monitor snapshot differs from its bound fortress",
            ));
        }
        if snapshot.graph.entities.len() > context.budget.max_entities as usize {
            return Err(bounded(
                "historical monitor snapshot exceeds its current entity allowance",
            ));
        }
        if !snapshot.hash_is_valid() {
            return Err(failure(
                ErrorCode::CorruptLedger,
                "historical monitor snapshot hash is invalid",
            ));
        }
        ...
        if initial && snapshot.anchor() != self.watch.created_at {
            return Err(failure(
                ErrorCode::StaleAnchor,
                "historical monitor first capture differs from its bound basis",
            ));
        }
        self.visited += 1;
        if self.watch.status.terminal() {
            return Ok(());
        }
        let before = self.watch.status;
        self.watch
            .advance_bounded(snapshot, initial, &mut self.budget)?;
        self.evaluated += 1;
        ...
        if self.watch.status.terminal() {
            self.first_terminal_record = Some(record);
        }
        self.budget.check()
    }
    ...
    pub(crate) fn finish(self, details: bool, context: &OperationContext) -> Result<Value> {
        ...
        if self.failed || self.visited != self.last_record - self.first_record + 1 {
            return Err(invalid(
                "historical monitor cannot publish an incomplete replay",
            ));
        }
        ...
        if value.to_string().len() as u64 > maximum {
            return Err(bounded(
                "complete historical monitor evidence exceeds the response budget",
            ));
        }
        self.budget.check()?;
        Ok(value)
