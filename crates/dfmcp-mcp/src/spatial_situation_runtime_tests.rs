...
impl Source for Script {
    fn read(&mut self, _: Duration) -> Result<LiveSpatialCitizenObservation> {
        ...
        self.values
            .pop_front()
            .ok_or_else(|| error(ErrorCode::AdapterUnavailable, "situation fixture exhausted"))
    }
...
fn observation(
    ...
    let bytes = (0..hex.len())
        ...
        .map(|i| {
            u8::from_str_radix(&hex[i..i + 2], 16)
                .map_err(|_| error(ErrorCode::InvalidRequest, "situation fixture hex"))
        })
    ...
    for i in 0..40 {
        ...
        let flags: u16 = if warnings && i == 0 {
            0x11e
        } else if warnings && i == 1 {
            0x11d
        } else {
            0x11f
        };
        citizens.extend_from_slice(&flags.to_be_bytes());
        put(&mut citizens, 6);
        citizens.extend_from_slice(&[1, 1]);
        citizens.extend_from_slice(&0u16.to_be_bytes());
    }
    let mut bytes = b"DFMS1800".to_vec();
    part(&mut bytes, &spatial);
...
fn register(warnings: bool, next: &[(u32, bool, u64)], tokens: u32) -> Result<(Registered, Value)> {
    let initial = observation(3, warnings, 7)?;
...
fn watch() -> Value {
    json!({"kind":"watch","key":"situation-monitor","condition":{"op":"tick_at_least","value":105u64*403200+100},
...
fn release_watch(s: &Registered, handle: &Value) -> Result<()> {
    assert_eq!(
        ask(s, json!({"kind":"cancel_watch","watch":handle}))?["ok"],
        true
    );
    assert_eq!(
        ask(s, json!({"kind":"release_watch","watch":handle}))?["ok"],
        true
    );
    Ok(())
}
...
fn live_bootstrap_and_query_attention_drill_into_exact_observed_entities() -> Result<()> {
    ...
    assert_eq!(
        compact["agent_turn"]["attention"].as_array().map(Vec::len),
        Some(1)
    );
    assert_eq!(
        compact["agent_turn"]["briefing"]["situation"]["detail_query"]["arguments"]["mode"],
        "situation"
    );
    let summary = decode(&fortress_query(s.handle(), Some("situation".into()), None))?;
    assert_eq!(summary["ok"], true, "{summary}");
    assert_eq!(
        summary["agent_turn"]["attention"].as_array().map(Vec::len),
        Some(2)
    );
    ...
    assert_eq!(
        decode(&fortress_query(s.handle(), None, Some(next)))?["error"]["code"],
        "stale_anchor"
    );
    Ok(())
}
...
fn observation_count_changes_preserve_watches_and_refuse_cross_epoch_comparisons() -> Result<()> {
    ...
    assert_eq!(
        observed["situation_comparison"]["continuous_between_observations"],
        false
    );
    assert!(
        observed["agent_turn"]["changes"]
            .as_array()
            .is_some_and(|v| v.iter().any(|c| c["rule"] == "citizen_not_alive"))
    );
    let listed = ask(&s, json!({"kind":"watches"}))?;
    assert_eq!(listed["ok"], true, "{listed}");
    assert_eq!(
        listed["records"][0]["evidence_digest"],
        created["record"]["evidence_digest"]
    );
    assert_eq!(
        listed["records"][0]["last_evaluated_anchor"],
        created["record"]["last_evaluated_anchor"]
    );
    let heartbeat = decode(&fortress_wait(s.handle()))?;
    assert_eq!(heartbeat["ok"], true);
    assert_eq!(heartbeat["situation_comparison"]["status"], "heartbeat");
    assert_eq!(heartbeat["agent_turn"]["changes"], json!([]));
    let reset = decode(&fortress_observe(s.handle()))?;
    assert_eq!(reset["ok"], true, "{reset}");
    assert_eq!(reset["situation_comparison"]["status"], "reset");
    assert_eq!(reset["agent_turn"]["changes"], json!([]));
    assert_eq!(s.calls.load(Ordering::SeqCst), 3);
    release_watch(&s, &handle)?;
    Ok(())
}
...
fn entity_pages_keep_attention_and_watch_context_within_8192_bytes() -> Result<()> {
    ...
    loop {
        ...
        assert_eq!(
            v["agent_turn"]["attention"].as_array().map(Vec::len),
            Some(1)
        );
        assert_eq!(
        ...
        for row in v["rows"]
            .as_array()
            .ok_or_else(|| error(ErrorCode::InvalidRequest, "rows missing"))?
        {
...
fn watch_specific_reservation_still_refuses_overflow_before_registration() -> Result<()> {
    ...
    {
        let h = resolve(s.handle())?;
        lock(&h)?.budget.max_output_tokens = 2048;
    }
    let listed = ask(&s, json!({"kind":"watches"}))?;
    assert_eq!(listed["ok"], true, "{listed}");
    ...
    let created = ask(&s, watch())?;
    assert_eq!(created["ok"], true, "{created}");
    release_watch(&s, &created["record"]["watch"])?;
    Ok(())
}
...
fn expired_query_authority_and_poisoned_sources_do_not_publish_current_attention() -> Result<()> {
    ...
    assert_eq!(
        observed["agent_turn"]["briefing"]["situation"]["status"],
        "unavailable"
    );
    assert!(observed.get("situation_comparison").is_none());
    drop(s);
    ...
    assert_eq!(
        failed["agent_turn"]["briefing"]["situation"]["status"],
        "unavailable"
    );
    Ok(())
}
...
impl Files {
    fn new() -> Result<Self> {
        ...
        fs::DirBuilder::new()
            ...
            .map_err(io_error)?;
        Ok(Self {
            path: directory.join("archive.bin"),
            directory,
        })
    }
}
...
fn io_error(_: std::io::Error) -> DfmcpError {
    error(
        ...
        "situation fixture filesystem error",
    )
}
...
fn historical_queries_and_offline_sessions_never_inherit_current_attention() -> Result<()> {
    ...
    let (first, limits, budget) = {
        ...
        let first = session
            ...
            .cloned()
            .ok_or_else(|| error(ErrorCode::InvalidRequest, "first record"))?;
        (first, session.limits, session.budget)
    };
    let latest = decode(&fortress_observe(s.handle()))?;
    assert_eq!(latest["ok"], true, "{latest}");
    assert_eq!(
        latest["agent_turn"]["attention"].as_array().map(Vec::len),
        Some(2)
    );
    let old = ask(
        &s,
        json!({"kind":"historical_query","record":first.number,"record_digest":first.record_digest.to_string(),
        "query":{"kind":"aggregate","group_by":{"kind":"entity_kind"}}}),
    )?;
    assert_eq!(old["ok"], true, "{old}");
    assert_eq!(old["historical"], true);
    assert_eq!(old["agent_turn"]["attention"], json!([]));
    assert!(old["agent_turn"]["briefing"].get("situation").is_none());
    drop(s);
    ...
    let session = archive::open(
        ...
        limits,
        budget,
        &[Capability::Query, Capability::Doctor],
    )?;
    lock(&SESSIONS)?.insert(id, Arc::new(Mutex::new(session)));
    let s = Registered {
        id,
    ...
    assert_eq!(old["agent_turn"]["attention"], json!([]));
    assert_eq!(
        decode(&fortress_query(s.handle(), Some("situation".into()), None))?["error"]["code"],
        "capability_denied"
    );
    drop(s);
