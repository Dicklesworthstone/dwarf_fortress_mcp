...
enum Query {
    ProductionDiagnosis {
        job: Option<Focus>, holder: Option<Focus>, include_clear_jobs: Option<bool>,
        limit: Option<u32>, continuation: Option<String>, max_work: Option<u64>,
    },
    InventoryPlan {
        quantity_unit: QuantityUnit, demands: Vec<DemandInput>,
        limit: Option<u32>, continuation: Option<String>, max_work: Option<u64>,
    },
}
#[derive(Deserialize)]
...
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct Focus { entity_id: String, generation: u32 }
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct DemandInput {
...
impl From<DemandInput> for MaterialDemand {
    fn from(d: DemandInput) -> Self {
        Self { key: d.key, units: d.units, item_types: d.item_types, subtype: d.subtype,
            material_type: d.material_type, material_index: d.material_index }
    }
}
impl Focus {
    fn decode(self) -> Result<(EntityId, u32)> {
        if self.entity_id.is_empty() || self.entity_id.len() > 20 || self.entity_id.starts_with('0')
            || !self.entity_id.bytes().all(|b| b.is_ascii_digit()) { return Err(invalid("focus IDs must be canonical positive decimal u64 strings")); }
        let id = self.entity_id.parse::<u64>().map_err(|_| invalid("focus ID exceeds u64"))?;
...
fn validate_shape(input: &Value) -> Result<()> {
    ...
    while let Some((value, depth)) = stack.pop() {
        ...
        if depth > 12 || nodes > MAX_INPUT_NODES { return Err(exhausted("production query exceeds its shape bound")); }
        match value {
            Value::String(text) => bytes = bytes.saturating_add(text.len()),
            Value::Array(values) => {
                if nodes.saturating_add(stack.len()).saturating_add(values.len()) > MAX_INPUT_NODES {
            ...
            Value::Object(values) => {
                ...
                for (key, value) in values { bytes = bytes.saturating_add(key.len()); stack.push((value, depth + 1)); }
            }
            Value::Number(n) => bytes = bytes.saturating_add(n.to_string().len()),
            _ => {}
        }
        if bytes > MAX_INPUT_BYTES { return Err(exhausted("production query exceeds its input byte bound")); }
    }
    Ok(())
}
...
fn identity_prefix(context: &OperationContext, source: Digest32, kind: &str) -> Vec<u8> {
    ...
    put_text(&mut bytes, ANALYSIS_POLICY); put_text(&mut bytes, kind);
    bytes.extend_from_slice(&context.session_id.get().to_be_bytes());
    for value in [context.anchor.fortress_id.get(), context.anchor.cursor.epoch,
        context.anchor.cursor.sequence, context.anchor.tick.0] { bytes.extend_from_slice(&value.to_be_bytes()); }
    bytes.extend_from_slice(context.anchor.state_hash.as_bytes());
    bytes.extend_from_slice(source.as_bytes()); bytes
}
fn put_focus(out: &mut Vec<u8>, value: Option<(EntityId, u32)>) {
    out.push(u8::from(value.is_some()));
    if let Some((id, generation)) = value { out.extend_from_slice(&id.get().to_be_bytes()); out.extend_from_slice(&generation.to_be_bytes()); }
}
fn put_demand(out: &mut Vec<u8>, d: &MaterialDemand) {
    put_text(out, &d.key); out.extend_from_slice(&d.units.to_be_bytes());
    out.extend_from_slice(&(d.item_types.len() as u32).to_be_bytes());
    for kind in &d.item_types { put_text(out, kind); }
    for value in [d.subtype, d.material_type, d.material_index] {
        out.push(u8::from(value.is_some())); if let Some(value) = value { out.extend_from_slice(&value.to_be_bytes()); }
    }
}
fn token(offset: usize, identity: Digest32) -> String {
    let mut bytes = b"dfmcp-operations-analysis-cursor-v1\0".to_vec();
    bytes.extend_from_slice(identity.as_bytes()); bytes.extend_from_slice(&(offset as u64).to_be_bytes());
    format!("op1:{offset}:{}", Digest32::of_bytes(&bytes))
}
fn page_offset(raw: Option<&str>) -> Result<usize> {
    let Some(raw) = raw else { return Ok(0); };
    if raw.len() > 128 { return Err(exhausted("production continuation exceeds 128 bytes")); }
    let parts = raw.split(':').collect::<Vec<_>>();
    if parts.len() != 3 || parts[0] != "op1" || parts[1].is_empty() || parts[1].starts_with('0')
        || parts[1].len() > 6 || !parts[1].bytes().all(|b|b.is_ascii_digit())
        || parts[2].len() != 64 || !parts[2].bytes().all(|b|b.is_ascii_digit() || (b'a'..=b'f').contains(&b)) {
        return Err(invalid("invalid production continuation"));
    }
    parts[1].parse::<usize>().map_err(|_| invalid("production offset overflow"))
}
...
struct Page<'a> { raw: Option<&'a str>, offset: usize, limit: u32, identity: Digest32, maximum_bytes: usize }
fn page<F>(mut payload: Value, count: usize, spec: Page<'_>, row: F) -> Result<Value>
where F: Fn(usize) -> Result<Value> {
    if let Some(raw) = spec.raw {
        if raw != token(spec.offset, spec.identity) {
            return Err(DfmcpError::new(ErrorCode::StaleAnchor,"production continuation belongs to another session, query, policy, or observation"));
        }
        if spec.offset >= count { return Err(DfmcpError::new(ErrorCode::CursorGap,"production continuation is past the result set")); }
    }
    payload["analysis_digest"] = json!(spec.identity.to_string());
    payload["total_rows"] = json!(count); payload["returned"] = json!(0);
    payload["rows"] = json!([]); payload["truncated"] = json!(false); payload["continuation"] = Value::Null;
    let mut end = spec.offset;
    while end < count && end - spec.offset < spec.limit as usize {
        let mut candidate = payload.clone();
        candidate["rows"].as_array_mut().ok_or_else(|| invariant("production page lost its rows"))?.push(row(end)?);
        candidate["returned"] = json!(end + 1 - spec.offset);
        candidate["truncated"] = json!(end + 1 < count);
        candidate["continuation"] = if end + 1 < count { json!(token(end + 1, spec.identity)) } else { Value::Null };
        if serde_json::to_vec(&candidate).map_err(|_| invariant("production page cannot be encoded"))?.len() > spec.maximum_bytes { break; }
        payload = candidate; end += 1;
    }
    if end == spec.offset && spec.offset < count { return Err(exhausted("one complete production row cannot fit; increase the negotiated output budget")); }
    if serde_json::to_vec(&payload).map_err(|_| invariant("production metadata cannot be encoded"))?.len() > spec.maximum_bytes {
        return Err(exhausted("production summary cannot fit the remaining output budget"));
    }
    Ok(payload)
}
...
pub(super) fn execute(state: &LiveOperationsState, context: &OperationContext, input: &Value) -> Result<Value> {
    ...
    if envelope.schema != "dfmcp.query/1" { return Err(invalid("production query requires dfmcp.query/1")); }
    if envelope.expected_anchor.as_ref().is_some_and(|a| a != &anchor_json(context.anchor)) {
        return Err(DfmcpError::new(ErrorCode::StaleAnchor,"production query expected_anchor is not current"));
    }
    let (limit, continuation, maximum_work) = match &envelope.query {
        Query::ProductionDiagnosis { limit, continuation, max_work, .. }
        | Query::InventoryPlan { limit, continuation, max_work, .. } =>
            (limit.unwrap_or(8), continuation.as_deref(), max_work.unwrap_or(MAX_ANALYSIS_WORK)),
    };
    if limit == 0 || limit > MAX_PAGE { return Err(exhausted("production page limit must be 1..128")); }
    ...
    match envelope.query {
        Query::ProductionDiagnosis { job, holder, include_clear_jobs, continuation, .. } => {
            ...
            put_focus(&mut bytes, scope.job); put_focus(&mut bytes, scope.holder); bytes.push(u8::from(scope.include_clear_jobs));
            ...
            payload["interpretation"] = json!("Unfinished holders may be normal for construction jobs; attachment indices, shared references and flags are observations, not material-shortage or causal-blocker proofs.");
        ...
        Query::InventoryPlan { quantity_unit: QuantityUnit::StackUnits, demands, continuation, .. } => {
            ...
            put_text(&mut bytes, SUPPLY_POLICY); bytes.extend_from_slice(&(report.demands.len() as u32).to_be_bytes());
            for d in &report.demands { put_demand(&mut bytes, d); }
            let identity = Digest32::of_bytes(&bytes);
            let mut payload = base_payload(context, report.source_digest, "inventory_plan");
            payload["quantity_unit"] = json!("stack_units"); payload["supply_policy"] = json!(SUPPLY_POLICY);
            payload["model_feasible"] = json!(report.allocation.shortage.is_none());
            payload["interpretation"] = json!("Exact only for your declared interchangeable stack-unit requests and this conservative supply subset. No full DF requirement matching, access proof, reservation, or execution plan is created.");
            ...
            payload["demands"] = json!(report.demands.iter().enumerate().map(|(i,d)| json!({
            ...
            page(payload, report.allocation.assignments.len(), Page {raw:continuation.as_deref(),offset,limit,identity,maximum_bytes}, |index| {
                ...
                let item = snapshot.graph.entities.get(&EntityId::new(assignment.supply_id))
                    .ok_or_else(|| invariant("allocation item is missing from its source projection"))?;
                Ok(json!({"demand_key":report.demands[assignment.demand_index].key,
...
pub(super) fn query_schema() -> Result<Value> {
    ...
    let target = base["$defs"].as_object_mut().ok_or_else(|| invariant("base query definitions missing"))?;
    for (name, definition) in definitions {
        if target.insert(name.clone(), definition.clone()).is_some() { return Err(invariant("operations schema definition collision")); }
    }
