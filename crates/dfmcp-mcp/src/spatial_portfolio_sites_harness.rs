...
impl Source for Script {
    fn read(&mut self, _: Duration) -> Result<LiveSpatialCitizenObservation> {
        ...
        self.values
            .pop_front()
            .ok_or_else(|| error(ErrorCode::AdapterUnavailable, "site fixture exhausted"))
    }
...
fn decode(raw: &str) -> Result<Value> {
    serde_json::from_str(raw).map_err(|_| error(ErrorCode::InvalidRequest, "site fixture JSON"))
}
...
fn retain_watch(s: &Registered) -> Result<()> {
    ok(ask(
        s,
        json!({"kind":"watch","key":"site-progress","condition":{"op":"paused","value":true},
