...
impl Files {
    ...
    fn populate(&self, workers: u32) -> Result<()> {
        let first = observation(3, workers)?;
        let mut state = LiveSpatialCitizenState::default();
        state.publish(first.clone())?;
        let anchor = state
            .snapshot()
        ...
        let c = OperationContext {
            ...
            grants: grants(
                &[Capability::Observe, Capability::Query],
        ...
        journal.append(observation(4, workers.saturating_sub(1))?, &c)?;
        Ok(())
    }
}
impl Drop for Files {
    ...
    let bytes = (0..hex.len())
        ...
        .map(|i| {
            u8::from_str_radix(&hex[i..i + 2], 16)
                .map_err(|_| error(ErrorCode::InvalidRequest, "fixture hex"))
        })
...
fn register(files: &Files, tokens: u32) -> Result<(Registered, Value)> {
    ...
    assert!(session.source.archive_only());
    assert_eq!(session.source.pages(), 0);
    assert!(session._watch_journal.is_none());
    assert!(
        session
            .grants
            .iter()
            .all(|g| matches!(g.capability, Capability::Query | Capability::Doctor))
    );
...
fn plan(workers: u32) -> Value {
    json!({"kind":"workforce_plan","demands":[{"key":"wood","workers":workers,
    "target":[0,0,5],"skill_key":"CARPENTRY"}],"limit":128})
}
fn historical(entry: &Value, q: Value) -> Value {
    json!({"kind":"historical_query","record":entry["record"],
...
fn archive_bootstrap_and_actual_queries_never_claim_live_freshness() -> Result<()> {
    ...
    let (s, opened) = register(&files, 65536)?;
    assert_eq!(opened["archive_only"], true);
    ...
    assert_eq!(p["assigned_workers"], 2);
    let doctor = decode(&fortress_doctor(s.handle()))?;
    assert_eq!(doctor["status"], "archive_only");
    assert_eq!(
        doctor["agent_turn"]["briefing"]["watch_evidence_loaded"],
        false
    );
    drop(s);
    assert_eq!(fs::read(&files.path).map_err(io_error)?, before);
    Ok(())
}
...
fn archived_workforce_and_route_drilldowns_stay_on_the_selected_record() -> Result<()> {
    ...
    let first = history["rows"][0].clone();
    let newest = ask(&s, plan(2))?;
    assert_eq!(newest["assigned_workers"], 1);
    let old = ask(&s, historical(&first, plan(2)))?;
    assert_eq!(old["ok"], true, "{old}");
    assert_eq!(old["assigned_workers"], 2);
    assert_eq!(old["anchor"], first["anchor"]);
    let route = old["rows"][0]["route_query"].clone();
    assert_eq!(route["query"]["kind"], "historical_query");
    assert_eq!(route["query"]["record"], 1);
    let drilled = decode(&fortress_query(s.handle(), None, Some(route)))?;
    assert_eq!(drilled["ok"], true, "{drilled}");
    assert_eq!(drilled["anchor"], first["anchor"]);
    assert_eq!(
        drilled["journal_record"]["record_digest"],
        first["record_digest"]
    );
    assert_eq!(ask(&s, plan(2))?["anchor"], newest["anchor"]);
    Ok(())
}
...
fn archive_rejects_monitoring_baselines_refresh_and_all_effect_tools() -> Result<()> {
    ...
    for name in [
        ...
        assert_eq!(
            ask(&s, json!({"kind":name}))?["error"]["code"],
            "capability_denied",
            "{name}"
        );
    }
    for call in [
        ...
        assert_eq!(
            decode(&call(s.handle()))?["error"]["code"],
            "capability_denied"
        );
    }
    assert_eq!(
    ...
    );
    {
        ...
        session
            .grants
            .push(grants(&[Capability::Observe], None).remove(0));
        let c = session.context()?;
...
fn archive_history_continuations_bind_session_and_record_identity() -> Result<()> {
    ...
    let mut selection = historical(&first["rows"][0], plan(1));
    selection["record_digest"] = json!("00".repeat(32));
    assert_eq!(ask(&s, selection)?["error"]["code"], "stale_anchor");
    assert_eq!(
        ask(&s, historical(&first["rows"][0], json!({"kind":"watch"})))?["error"]["code"],
        "capability_denied"
    );
    drop(s);
    let (next, _) = register(&files, 65536)?;
    assert_eq!(
        ask(&next, json!({"kind":"history","continuation":token}))?["error"]["code"],
        "stale_anchor"
    );
    Ok(())
}
...
fn archive_query_and_doctor_fence_changed_storage_without_hiding_the_error() -> Result<()> {
    ...
    assert_eq!(
        decode(&fortress_doctor(s.handle()))?["error"]["code"],
        "corrupt_ledger"
    );
    Ok(())
}
...
fn archive_schema_advertises_nineteen_read_variants_including_condition_inspection() -> Result<()> {
    ...
    for kind in [
        ...
        assert!(
            variants
                .iter()
                .any(|v| v["properties"]["kind"]["const"] == kind)
        );
    }
    assert_eq!(
...
fn archive_configuration_and_bootstrap_fail_without_creating_replacement_history() -> Result<()> {
    ...
    assert!(
        validate_configuration(Some(&files.path), TailRecovery::TruncateIncomplete, None).is_err()
    );
    assert!(
        validate_configuration(Some(&files.path), TailRecovery::Refuse, Some(&files.path)).is_err()
    );
    for caps in [
        vec!["observe".into(), "query".into()],
        vec!["doctor".into()],
        vec!["control_clock".into()],
    ] {
    ...
    wrong.spatial.region.origin[0] = 1;
    assert!(
        open(
            next_id()?,
            Slot::reserve()?,
