...
use dfmcp_adapter::workforce_analysis::portfolio::{
    self as joint, ProductionPortfolio, ProductionTask,
};
...
struct TaskInput {
    ...
    workers: u32,
    skill_key: String,
    min_effective_skill: Option<i32>,
    preserve_social: Option<bool>,
...
impl TaskInput {
    fn normalized(self) -> ProductionTask {
        ProductionTask {
            ...
            preserve_social: self.preserve_social.unwrap_or(true),
            adults_only: self.adults_only.unwrap_or(true),
            materials: self
                .materials
                .into_iter()
                .map(MaterialInput::normalized)
                .collect(),
        }
    }
}
...
enum Request {
    ProductionPortfolio {
        ...
        tasks: Vec<TaskInput>,
        reserves: Option<Vec<MaterialInput>>,
        limit: Option<u32>,
...
pub(in super::super) fn handles(input: &Value) -> bool {
    input
        ...
        == Some("production_portfolio")
}
fn multisite(report: &ProductionPortfolio) -> bool {
    report.sites.multiple_origins(report.inventory.origin)
}
...
fn request_json(report: &ProductionPortfolio) -> Value {
    ...
    let mut value =
        json!({"origin":report.inventory.origin,"quantity_unit":"stack_units","tasks":tasks});
...
fn task_keys(report: &ProductionPortfolio, mask: u16) -> Vec<&str> {
    report
        ...
        .iter()
        .enumerate()
        .filter_map(|(i, t)| (mask & (1u16 << i) != 0).then_some(t.key.as_str()))
        .collect()
}
fn reserve_key(report: &ProductionPortfolio, index: usize) -> Result<Option<&str>> {
    let owner = report
        .material_owners
        .get(index)
        .ok_or_else(|| invariant("material owner absent"))?;
    if *owner != RESERVE_OWNER {
        return Ok(None);
    }
    let demand = report
        .inventory
        .demands
        .get(index)
        .ok_or_else(|| invariant("reserve demand absent"))?;
    Ok(Some(
        demand
            .key
            .strip_prefix("reserve.")
            .ok_or_else(|| invariant("reserve namespace absent"))?,
    ))
}
fn cut_json(report: &ProductionPortfolio, index: usize) -> Result<Value> {
    let rejected = report
        ...
        .rejected
        .get(index)
        .ok_or_else(|| invariant("production rejection row absent"))?;
    let cut = &rejected.shortage;
    let keys: Vec<_> = cut
        .demand_indices
        .iter()
        .map(|&i| {
            match rejected.domain {
            ...
            .ok_or_else(|| invariant("production shortage references an absent demand"))
        })
        .collect::<Result<_>>()?;
...
fn reserve_shortfall(report: &ProductionPortfolio) -> Result<Option<Value>> {
    let Some(cut) = &report.selection.materials.shortage else {
        return Ok(None);
    };
    if report.selection.task_mask != 0 || report.reserves.is_empty() {
        return Err(invariant("invalid reserve-only failure"));
    }
    let keys = cut
        .demand_indices
        .iter()
        .map(|&i| {
            reserve_key(report, i)?
                .ok_or_else(|| invariant("reserve-only cut contains an optional task"))
        })
        .collect::<Result<Vec<_>>>()?;
    Ok(Some(
        json!({"row_kind":"reserve_shortfall","domain":"materials","reserve_keys":keys,
...
fn row(
    ...
    report: &ProductionPortfolio,
    index: usize,
) -> Result<Value> {
    if let Some(shortfall) = reserve_shortfall(report)? {
        if index != 0 {
            return Err(invariant("reserve shortfall has exactly one evidence row"));
        }
        return Ok(shortfall);
    }
    ...
    if index < w.len() {
        ...
        let candidate = report.workforce.candidates[assignment.demand_index]
            .iter()
            .find(|c| c.entity_id == assignment.supply_id)
            .ok_or_else(|| invariant("joint worker lost its candidate evidence"))?;
        let mut value =
            candidate_row(state, &report.workforce, assignment.demand_index, candidate)?;
        value["row_kind"] = json!("worker_assignment");
        value["task_key"] = json!(report.tasks[assignment.demand_index].key);
        if multisite(report) {
            value["origin"] = json!(report.sites.task_origins[assignment.demand_index]);
        }
        value["worker_slots"] = json!(assignment.units);
        return Ok(value);
    }
    ...
    if index < m.len() {
        ...
        let local_key = demand
            .key
            .split_once('.')
            .map(|(_, key)| key)
            .ok_or_else(|| invariant("joint material key absent"))?;
        let site = report.inventory_for(assignment.demand_index)?;
        let location = site
            .locations
            .get(&assignment.supply_id)
            .ok_or_else(|| invariant("joint supply lacks evidence at its assigned site"))?;
        let protected = owner == RESERVE_OWNER;
        let task = if protected {
            None
        } else {
            Some(
                report
                    .tasks
                    .get(owner)
                    .ok_or_else(|| invariant("material task owner invalid"))?
                    .key
                    .as_str(),
            )
        };
        ...
        if multisite(report) {
            value["origin"] = json!(site.origin);
        }
        if protected {
            value["reserve_key"] = json!(local_key);
            value["consumed_by_selected_tasks"] = json!(false);
...
pub(in super::super) fn execute(
    ...
    let input: Input = serde_json::from_value(input.clone())
        .map_err(|_| invalid("invalid production portfolio request"))?;
    if input.schema != "dfmcp.query/1" {
        return Err(invalid("production portfolio requires dfmcp.query/1"));
    }
    if input
        .expected_anchor
        .as_ref()
        ...
        || state.snapshot().map(|s| s.anchor()) != Some(context.anchor)
    {
        return Err(DfmcpError::new(
            ...
            "production portfolio names another coherent capture",
        ));
    }
    let Request::ProductionPortfolio {
        origin,
    ...
    let task_sites: BTreeMap<_, _> = tasks
        .iter()
        .filter_map(|t| t.origin.map(|site| (t.key.clone(), site)))
        .collect();
    let requested: Vec<_> = tasks.into_iter().map(TaskInput::normalized).collect();
    let reserves: Vec<_> = reserves
        .unwrap_or_default()
        .into_iter()
        .map(MaterialInput::normalized)
        .collect();
    ...
    if elapsed >= u128::from(context.budget.max_wall_millis) {
        return Err(budget("portfolio parsing exhausted its deadline"));
    }
    let mut timed = context.clone();
    timed.budget.max_wall_millis -= elapsed as u64;
    let report = joint::plan_at_sites(
        state,
        &timed,
        origin,
        &requested,
    ...
    let policy = if multisite(&report) {
        joint::MULTISITE_POLICY
    } else if report.reserves.is_empty() {
        joint::PORTFOLIO_POLICY
    } else {
        joint::RESERVE_POLICY
    };
    let shortfall = reserve_shortfall(&report)?;
    let feasible = shortfall.is_none();
    ...
    let cuts = (0..report.selection.rejected.len())
        .map(|i| cut_json(&report, i))
        .collect::<Result<Vec<_>>>()?;
    ...
    if let Some(shortfall) = &shortfall {
        proof["reserve_shortfall"] = shortfall.clone();
    }
    ...
    out["quantity_unit"] = json!("stack_units");
    ...
    out["all_tasks_supported"] =
        json!(feasible && report.selection.task_mask.count_ones() as usize == report.tasks.len());
    out["selected_set_model_feasible"] = json!(feasible);
    out["native_job_readiness_proven"] = json!(false);
    out["production_schedule_proven"] = json!(false);
    ...
    if multisite(&report) {
        ...
        out["supply_model"]["classification_and_reachability_scope"] =
            json!("default_origin_only; site_counts_are_not_additive");
    ...
    for (i, demand) in report.inventory.demands.iter().enumerate() {
        ...
        if let Some(key) = reserve_key(&report, i)? {
            protected = protected
                .checked_add(units)
                .ok_or_else(|| budget("reserve total overflow"))?;
            let request = report
                .reserves
                .iter()
                .find(|r| r.key == key)
                .ok_or_else(|| invariant("reserve request lost"))?;
            pools.push(
                json!({"request":material_json(request),"supported_units":units,
                "fully_supported":units==demand.units,"protected_in_model":feasible}),
            );
        } else {
            consumed = consumed
                .checked_add(units)
                .ok_or_else(|| budget("task consumption overflow"))?;
        }
    }
    ...
    out["interpretation"] = json!(
        "Complete tasks share worker capacity one and finite conservative stack capacity across declared sites. Each assignment needs a candidate route at its own site. Reserves remain distinct unconsumed units accessible from the default origin. This is not inferred native requirements, a reservation, a carrying assignment, a timed schedule, or authorization to execute."
    );
    let count = if feasible {
        report.selection.workers.assignments.len()
            + report.selection.materials.assignments.len()
            + report.selection.rejected.len()
    } else {
        1
    };
    let result = paginate(
    ...
    if started.elapsed().as_millis() >= u128::from(context.budget.max_wall_millis) {
        return Err(budget(
            "production portfolio exhausted its shared analysis/render deadline",
        ));
    }
    Ok(result)
}
...
pub(in super::super) fn extend_schema(mut base: Value) -> Result<Value> {
    let extension: Value = serde_json::from_str(include_str!(
        "../../../schemas/mcp_production_portfolio_v1.json"
    ))
