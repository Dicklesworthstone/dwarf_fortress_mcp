...
use dfmcp_adapter::work_orders::{
    WorkOrderSpec,
    rpc::{WorkOrderRpcClient, WorkOrderTcpStream},
};
use dfmcp_core::{
...
use presentation::{
    ...
    failure, mode_name, observation_json, packet, recipe, record_json, summary_json,
};
    ...
    fn context(&mut self, production_enabled: bool, cancelled: bool) -> Result<OperationContext> {
        self.request = self.request.checked_add(1).ok_or_else(|| {
            error(
                ...
                "creation request identities exhausted",
            )
        })?;
        Ok(OperationContext {
            session_id: self.id,
            ...
            grants: self
                .grants
                .iter()
                .filter(|g| g.capability != Capability::ConfigureProduction || production_enabled)
                .cloned()
                .collect(),
    ...
    fn perform(&mut self, action: Action, context: &OperationContext) -> Result<Value> {
        match action {
            Action::Observe => {
                ...
                self.anchor.tick = GameTick(observation.tick());
                self.anchor.state_hash = observation.witness();
                Ok(
                    json!({"ok":true,"observation":observation_json(&observation),"game_mutation_dispatched":false}),
                )
            }
            Action::Plan { key, spec, witness } => {
                let record = self.control.plan(&key, spec, witness, context)?;
                Ok(
                    json!({"ok":true,"effect":record_json(&record),"game_mutation_dispatched":false}),
                )
            }
            Action::Commit { key, plan, witness } => {
                let record = self.control.commit(&key, plan, witness, context)?;
                Ok(
                    json!({"ok":true,"operation_acknowledged":true,"effect":record_json(&record),
            ...
            Action::Cancel { key, plan } => {
                ...
                Ok(
                    json!({"ok":true,"effect":record_json(&record),"native_calls":0,"manager_order_deleted":false}),
                )
            }
            Action::Query {
                filter,
                ...
                let after = continuation
                    ...
                    .map(|token| {
                        self.cursors
                            .resolve(token, self.id, &summary, filter, limit)
                    })
                    .transpose()?;
                let scan = 64usize
                    .min((context.budget.max_bytes / MAX_RECORD_BYTES as u64) as usize)
                    .min(context.budget.max_entities as usize);
                if scan == 0 {
                    return Err(error(
                        ErrorCode::BudgetExceeded,
                        "no complete creation record fits the work budget",
                    ));
                }
                let page =
                    self.control
                        .records_page(summary.head, after.as_deref(), scan, context)?;
                ...
                let next = if more {
                    match page.records.get(consumed.saturating_sub(1)) {
                        Some(last) => Some(self.cursors.issue(
                            self.id,
                            &summary,
                ...
                Ok(
                    json!({"ok":true,"records":rows,"scanned_records":consumed,"state":filter.name(),
            ...
            Action::Doctor => Ok(
                json!({"ok":true,"journal":summary_json(&self.control.summary(context)?),
...
fn narrowed(
    ...
    if let Some(v) = limits.wall {
        context.budget.max_wall_millis = v.min(context.budget.max_wall_millis);
    }
    if let Some(v) = limits.bytes {
        context.budget.max_bytes = v.min(context.budget.max_bytes);
    }
    if let Some(v) = limits.tokens {
        context.budget.max_output_tokens = v.min(context.budget.max_output_tokens);
    }
    context.budget.validate()?;
    if rows > MAX_PAGE {
        return Err(error(
            ErrorCode::InvalidRequest,
            "creation output page exceeds eight records",
        ));
    }
        ...
        || u64::from(context.budget.max_output_tokens) * 4 < reserve
    {
        return Err(error(
            ErrorCode::BudgetExceeded,
            "complete creation response and recovery warnings do not fit; no work started",
        ));
    }
...
fn remaining(
    ...
    context.budget.max_wall_millis = total
        ...
        .ok_or_else(|| {
            error(
                ErrorCode::BudgetExceeded,
                "creation request deadline expired before native work",
            )
        })?;
    Ok(context)
}
...
fn render<S: EffectJournalStorage, N: WorkOrderSource>(
    state: &State<S, N>,
    ...
    let selected = state.control.selected(&current).ok().flatten();
    let out = packet(
        operation,
        result,
        TurnView {
    ...
    let cause = error(
        ...
        "complete creation response exceeded reservation; inspect retained evidence before further control",
    );
    packet(
        operation,
        failure(&cause, operation),
...
fn run_action<S: EffectJournalStorage, N: WorkOrderSource>(
    state: &mut State<S, N>,
    context: OperationContext,
    operation: &str,
    limits: Limits,
...
fn runtime_io() -> Result<()> {
    let cx = asupersync::Cx::current().ok_or_else(|| {
        error(
            ErrorCode::CapabilityDenied,
            "creation MCP I/O requires its owned runtime context",
        )
    })?;
...
fn session_lock() -> Result<MutexGuard<'static, Option<LiveState>>> {
    match SESSION.try_lock() {
        ...
        Err(TryLockError::WouldBlock) => Err(error(
            ErrorCode::BudgetExceeded,
            "creation session is serving another bounded request",
        )),
        Err(TryLockError::Poisoned(_)) => Err(error(
            ErrorCode::InternalInvariantViolation,
            "creation session poisoned; restart and recover journal",
        )),
    }
}
fn session_id(raw: &str) -> Result<SessionId> {
    ...
    if id.get() != value
        ...
        || (value & ((1u128 << 62) - 1)) >> 57 != 10
    {
        return Err(error(
            ErrorCode::InvalidRequest,
            "not a work-orders/1.10 session ID",
        ));
    }
    Ok(id)
}
const ALLOWED: [&str; 6] = [
    "DFMCP_ALLOW_UNADMITTED_WORK_ORDERS_V1_10",
    "DFMCP_WORK_ORDERS_TOKEN",
    "DFMCP_WORK_ORDERS_ENDPOINT",
    "DFMCP_WORK_ORDERS_JOURNAL",
    "DFMCP_WORK_ORDERS_FORTRESS_ID",
...
fn environment_contract(
            ...
            .any(|k| k.starts_with("DFMCP_") && !ALLOWED.contains(&k.as_str()))
    {
        return Err(error(
            ErrorCode::CapabilityDenied,
            "work-orders/1.10 requires exact development opt-in and refuses admission or other DFMCP state",
        ));
    }
    Ok(())
}
fn production_enabled() -> bool {
    std::env::var("DFMCP_WORK_ORDERS_ALLOW_PRODUCTION")
        .ok()
        .as_deref()
        == Some("1")
}
fn validate_environment() -> Result<()> {
    ...
    if matches!(production, Err(std::env::VarError::NotUnicode(_))) {
        return Err(error(
            ErrorCode::CapabilityDenied,
            "creation production opt-in must be absent or exactly 1",
        ));
    }
    environment_contract(
        std::env::var("DFMCP_ALLOW_UNADMITTED_WORK_ORDERS_V1_10")
            .ok()
            .as_deref(),
...
fn configured(name: &str, maximum: usize) -> Result<String> {
    let value = std::env::var(name).map_err(|_| {
        error(
            ErrorCode::CapabilityDenied,
            "required operator creation configuration absent or not UTF-8",
        )
    })?;
    if value.is_empty() || value.len() > maximum || value.contains('\0') {
        return Err(error(
            ErrorCode::InvalidRequest,
            "operator creation configuration exceeds its bound",
        ));
    }
    Ok(value)
}
fn parse_mode(raw: &str) -> Result<JournalMode> {
    match raw {
        "offline" => Ok(JournalMode::Offline),
        "reconcile" => Ok(JournalMode::Reconcile),
        "control" => Ok(JournalMode::Control),
        _ => Err(error(
            ErrorCode::InvalidRequest,
            "mode must be offline, reconcile or control",
        )),
    }
}
fn grants(mode: JournalMode, fortress: FortressId, enabled: bool) -> Result<Vec<CapabilityGrant>> {
    if mode == JournalMode::Control && !enabled {
        return Err(error(
            ErrorCode::CapabilityDenied,
            "control requires operator DFMCP_WORK_ORDERS_ALLOW_PRODUCTION=1",
        ));
    }
    let mut result = vec![CapabilityGrant {
...
pub fn fortress_open_session(
    ...
    let result = (|| -> Result<String> {
        ...
        if guard.is_some() {
            return Err(error(
                ErrorCode::Conflict,
                "close the retained creation session first",
            ));
        }
        ...
        let fortress = FortressId::new(number);
        let grants = grants(mode, fortress, production_enabled())?;
        ...
        if budget.max_wall_millis > 60_000
            ...
            || budget.max_output_tokens > 262_144
        {
            return Err(error(
                ErrorCode::BudgetExceeded,
                "creation limits exceed 60000ms, 68MiB or 262144 output proxy units",
            ));
        }
        let sequence = NEXT
        ...
        let source = if mode == JournalMode::Offline {
            None
        } else {
            let endpoint = match std::env::var("DFMCP_WORK_ORDERS_ENDPOINT") {
                ...
                _ => {
                    return Err(error(
                        ErrorCode::InvalidRequest,
                        "creation endpoint must be bounded UTF-8 numeric loopback",
                    ));
                }
            };
            ...
            work.budget.max_bytes = work
                .budget
                .max_bytes
                .checked_sub(BOOTSTRAP_RPC_BYTES)
                .filter(|n| *n > 0)
                .ok_or_else(|| {
                    error(
                        ErrorCode::BudgetExceeded,
                        "creation handshake and journal exceed work budget",
                    )
                })?;
            work = remaining(work, started, budget.max_wall_millis)?;
            Some(WorkOrderRpcClient::connect(
                endpoint,
                token,
        ...
        if mode == JournalMode::Control && !production_enabled() {
            return Err(error(
                ErrorCode::CapabilityDenied,
                "creation production grant revoked during opening",
            ));
        }
                ...
                .min(u64::from(display.budget.max_output_tokens) * 4)
        {
            return Err(error(
                ErrorCode::BudgetExceeded,
                "creation opening response does not fit; session was not published",
            ));
        }
...
#[tool(
    description = "Observe complete bounded native order queue membership, not order configuration or feasibility. Use its exact witness to seal finite creation intent. Failed refresh clears old selection; no mutation occurs."
)]
...
#[tool(
    description = "Discover complete retained creation records without native calls. State: all, pending or reconciliation_required. Limit 1..8, default 2. Whole-record pages and continuations bind session, journal, exact head, filter and limit. Empty filtered pages may still require continuation."
)]
    ...
    with_session(
        ...
        || {
            if limit == 0 || limit > MAX_PAGE {
                return Err(error(
                    ErrorCode::InvalidRequest,
                    "creation page limit must be 1..8",
                ));
            }
            Ok(Action::Query {
...
#[tool(
    description = "Seal and durably prepare finite furniture creation: wooden_bed, wooden_door, wooden_table or wooden_chair, amount 1..100. Requires exact retained queue witness, stable key and ConfigureProduction. Server constructs plan digest; no order is inserted and replay does not renew TTL."
)]
pub fn fortress_plan(
    ...
    with_session(session_id, "fortress.plan", Limits::default(), 1, || {
        Ok(Action::Plan {
            key: idempotency_key,
            spec: WorkOrderSpec::new(recipe(&recipe_name)?, amount)?,
...
#[tool(
    description = "Perform one bounded Query-only receipt reconciliation. No polling loop, reconnect or insertion retry. Prepared/terminal records return stored evidence. Missing native records never prove non-creation. Offline unresolved work requires explicit close and reopen in reconcile mode."
)]
...
#[tool(
    description = "Inspect one exact retained creation plan and receipt without native calls, including offline. Historical evidence is not current order state or production completion."
)]
...
fn close(raw: String) -> String {
    let result = (|| -> Result<String> {
        ...
        let state = guard.as_mut().filter(|s| s.id == id).ok_or_else(|| {
            error(
                ErrorCode::SessionNotFound,
                "creation session absent or already closed",
            )
        })?;
...
#[tool(
    description = "With scope=effect and exact key/digest, permanently retire a still-prepared local creation without deleting a manager order. Requires production authority. With scope=session and no effect identity, release custody even after fencing; no evidence or effects are erased or cancelled."
)]
pub fn fortress_cancel(
    ...
    match (scope.as_str(), idempotency_key, plan_digest) {
        ...
        _ => with_session(session_id, "fortress.cancel", Limits::default(), 0, || {
            Err(error(
                ErrorCode::InvalidRequest,
                "cancel requires scope=effect with key/digest, or scope=session without either",
            ))
        }),
    }
}
#[tool(
    description = "Unavailable in isolated work-orders/1.10; no game checkpoint is implied by the coordination journal."
)]
...
#[tool(
    description = "Unavailable in isolated work-orders/1.10; restarting or replaying the journal never restores game state."
)]
...
#[tool(
    description = "Report custody-checked journal counts and connection presence without native calls. This is not a live health check, compatibility admission or proof of completed production."
)]
