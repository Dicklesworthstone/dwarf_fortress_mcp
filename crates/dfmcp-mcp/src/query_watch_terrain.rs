    ...
    for (index, area) in areas.iter().enumerate() {
        ...
        for axis in 0..3 {
            if area.min[axis] > area.max[axis] || area.max[axis] >= 32_768 {
                return Err(invalid("terrain mask coordinates must be ordered and within 0..32767"));
            }
            volume *= u64::from(area.max[axis] - area.min[axis] + 1);
        }
        total += volume; // At most 64 * 32768^3, well within u64.
        if total > MAX_TILES { return Err(bounded("terrain mask exceeds 16384 requested tiles")); }
        if areas[..index].iter().any(|other| (0..3).all(|axis| {
            area.min[axis] <= other.max[axis] && other.min[axis] <= area.max[axis]
        })) {
...
fn observed<'a>(entity: &'a EntityRecord, field: &str, suffix: &str,
    prefix: &str, source: Digest32, tick: GameTick) -> Option<&'a WorldValue> {
...
fn coordinates(value: &WorldValue) -> Option<[u32; 3]> {
    let WorldValue::Coord(p) = value else { return None; };
    Some([u32::try_from(p.x).ok()?, u32::try_from(p.y).ok()?, u32::try_from(p.z).ok()?])
}

fn capture(snapshot: &WorldSnapshot) -> Option<Capture> {
    let root = snapshot.graph.entities.get(&EntityId::new(1))?;
    if root.id != EntityId::new(1) || root.kind != EntityKind::Fortress
        || root.generation == 0 || root.revision == 0 { return None; }
    let origin = root.fields.get("region_origin")?;
    let FactSource::DfhackField(path) = &origin.source else { return None; };
    ...
    dfmcp_world::map_region::Region { origin: region_min, size: region_size }.volume().ok()?;
    if (0..3).any(|i| dimensions[i] == 0 || dimensions[i] > 32_768
        || region_min[i] + region_size[i] > dimensions[i]) { return None; }
    Some(Capture { prefix, source, region_min, region_size, dimensions })
}

fn native_suffix(field: &str) -> Option<&'static str> {
    match field {
        "tiletype" => Some("tiletype"), "shape" => Some("shape"),
        "liquid_depth" => Some("liquid_depth"), "magma" => Some("magma"),
        "traffic" => Some("traffic"), "dig_designation" => Some("dig_designation"),
        "building_occupancy" => Some("building_occupancy"), "unit_occupancy" => Some("unit_occupancy"),
        "walkable_region" => Some("walkable_region"),
        "temperature_1_raw" => Some("temperature_1_raw"), "temperature_2_raw" => Some("temperature_2_raw"),
        "position" => Some("tile_position"), "visibility" => Some("tile_visibility"),
        _ => None,
    }
}

fn row_truth(predicate: &Predicate, entity: &EntityRecord, snapshot: &WorldSnapshot,
    capture: &Capture, budget: &mut EvaluationBudget) -> Result<Truth> {
    budget.charge()?;
    Ok(match predicate {
...
fn tile_truth(snapshot: &WorldSnapshot, capture: &Capture, p: [u32; 3],
    predicate: &Predicate, budget: &mut EvaluationBudget) -> Result<(Truth, Option<&'static str>, bool)> {
    if (0..3).any(|i| p[i] >= capture.dimensions[i]) {
        return Ok((Truth::Unknown, Some("outside_map"), false));
    }
    if (0..3).any(|i| p[i] < capture.region_min[i] || p[i] - capture.region_min[i] >= capture.region_size[i]) {
    ...
    if entity.id != id || entity.kind != EntityKind::TileFeature || entity.generation == 0 || entity.revision == 0 {
        return Ok((Truth::Unknown, Some("tile_identity_unestablished"), false));
    }
    let value = |field, suffix| observed(entity, field, suffix, capture.prefix, capture.source, snapshot.tick);
    ...
    match value("visibility", "tile_visibility") {
        ...
        Some(WorldValue::Text(text)) if text == "hidden" => return Ok((Truth::Unknown, Some("hidden"), false)),
        Some(WorldValue::Text(text)) if text == "unallocated" => return Ok((Truth::Unknown, Some("unallocated"), false)),
    ...
    Ok((truth, (truth == Truth::Unknown).then_some("predicate_unestablished"), true))
}
            ...
            for y in area.min[1]..=area.max[1] {
                for x in area.min[0]..=area.max[0] {
                    ...
                    let (truth, reason, is_visible) = match &capture {
                        ...
                        None => (Truth::Unknown, Some("capture_provenance_unestablished"), false),
                    };
                    ...
                    if let Some(reason) = reason {
                        *reasons.entry(reason).or_default() += 1;
                        if examples.len() < 2 { examples.push(json!({"position":p,"reason":reason})); }
                    }
                }
            }
        }
...
pub(super) fn extend_schema(mut schema: Value) -> Result<Value> {
    let condition: Value = serde_json::from_str(include_str!("../../../schemas/mcp_watch_terrain_v1.json"))
        .map_err(|_| invalid("embedded terrain-condition schema is invalid"))?;
    schema["$defs"]["watch_condition"]["oneOf"].as_array_mut()
        .ok_or_else(|| invalid("watch condition schema variants absent"))?.push(condition);
    Ok(schema)
}
