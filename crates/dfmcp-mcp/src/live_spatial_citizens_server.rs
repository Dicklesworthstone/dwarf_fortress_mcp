...
#[path = "spatial_archive_runtime.rs"]
mod archive;
#[path = "spatial_watch_runtime.rs"]
mod durable_watches;
#[path = "spatial_citizen_history.rs"]
mod history;
...
mod session_release;
#[path = "spatial_situation.rs"]
mod situation;
#[path = "spatial_situation_presentation.rs"]
mod situation_presentation;
#[cfg(test)]
...
mod watch_batch;
#[path = "workforce_queries.rs"]
mod workforce_queries;
...
use dfmcp_adapter::live_jobs_rpc::{
    DeadlineStream,
    operations::{
        map::spatial::{
            SpatialLimits,
            citizens::{CitizenSpatialLimits, CitizenSpatialRpcClient},
        },
        paged::PagedOperationsLimits,
    },
};
...
static SESSIONS: LazyLock<Mutex<BTreeMap<SessionId, Arc<Mutex<Session>>>>> =
...
fn lock<T>(m: &Mutex<T>) -> Result<MutexGuard<'_, T>> {
    m.lock().map_err(|_| {
        error(
            ...
            "spatial/1.8 mutex poisoned",
        )
    })
}
trait Source: Send {
    fn read(&mut self, timeout: Duration) -> Result<LiveSpatialCitizenObservation>;
...
impl Source for CitizenSpatialRpcClient<DeadlineStream> {
    fn read(&mut self, t: Duration) -> Result<LiveSpatialCitizenObservation> {
        self.refresh(t)
    }
    fn poisoned(&self) -> bool {
        CitizenSpatialRpcClient::poisoned(self)
    }
    fn fence(&mut self) {
        CitizenSpatialRpcClient::fence(self)
    }
    fn pages(&self) -> u32 {
...
struct Session {
    ...
    request: u128,
    _watch_journal: Option<semantic_query::WatchJournalGuard>,
    _slot: Slot,
}
impl Session {
    fn anchor(&self) -> Result<StateAnchor> {
        self.state.snapshot().map(|s| s.anchor()).ok_or_else(|| {
            error(
                ErrorCode::InternalInvariantViolation,
                "spatial/1.8 snapshot absent",
            )
        })
    }
    fn context(&mut self) -> Result<OperationContext> {
        ...
        self.request = self.request.checked_add(1).ok_or_else(|| {
            error(
                ...
                "spatial/1.8 request IDs exhausted",
            )
        })?;
        Ok(OperationContext {
            session_id: self.id,
...
fn next_id() -> Result<SessionId> {
    ...
    if *n >= FAMILY {
        return Err(error(
            ErrorCode::BudgetExceeded,
            "spatial/1.8 session IDs exhausted",
        ));
    }
...
fn parse_session_id(raw: Option<String>) -> Result<SessionId> {
    let text = raw.ok_or_else(|| {
        error(
            ErrorCode::InvalidRequest,
            "open a spatial/1.8 session first",
        )
    })?;
    if text.len() != 32 || !text.bytes().all(|b| b.is_ascii_hexdigit()) {
        return Err(error(
            ErrorCode::InvalidRequest,
            "invalid spatial/1.8 session",
        ));
    }
    let raw = u128::from_str_radix(&text, 16)
        .map_err(|_| error(ErrorCode::InvalidRequest, "invalid spatial/1.8 session"))?;
    let id = SessionId::new(raw);
    if id.get() != raw || !id.is_process_scoped_live() || (raw & ((1u128 << 62) - 1)) >> 56 != 1 {
        return Err(error(
            ErrorCode::InvalidRequest,
            "not a spatial/1.8 session",
        ));
    }
    Ok(id)
}
fn resolve(raw: Option<String>) -> Result<Arc<Mutex<Session>>> {
    let id = parse_session_id(raw)?;
    lock(&SESSIONS)?
        .get(&id)
        .cloned()
        .ok_or_else(|| error(ErrorCode::SessionNotFound, "spatial/1.8 session not found"))
}
fn allowed_environment(name: &str) -> bool {
    !name.starts_with("DFMCP_")
        || matches!(
            name,
            "DFMCP_ALLOW_UNADMITTED_SPATIAL_V1_8"
                | "DFMCP_SPATIAL_CITIZEN_TOKEN"
                | "DFMCP_SPATIAL_CITIZEN_ENDPOINT"
                | "DFMCP_SPATIAL_CITIZEN_JOURNAL"
                | "DFMCP_SPATIAL_CITIZEN_JOURNAL_REPAIR"
                | "DFMCP_SPATIAL_CITIZEN_WATCH_JOURNAL"
        )
}
fn validate_environment() -> Result<()> {
    if std::env::var("DFMCP_ALLOW_UNADMITTED_SPATIAL_V1_8")
        .ok()
        .as_deref()
        != Some("1")
        || std::env::vars_os().any(|(n, _)| !allowed_environment(&n.to_string_lossy()))
        || crate::admission::current_admission_provenance().is_some()
    {
        return Err(error(
            ErrorCode::CapabilityDenied,
            "spatial/1.8 requires its own opt-in and refuses other DFMCP/admission state",
        ));
    }
    Ok(())
}
...
fn briefing(s: &Session) -> Value {
    let full = s.state.observation_full();
    let base = full.map(|v| v.spatial());
...
fn coverage() -> Value {
    json!({"status":"partial","complete_domains":["fortress.citizens.strict_roster","current_job_roster","building_roster",
...
fn situation_briefing(s: &Session, report: Option<&situation::Report>) -> Value {
    ...
    value["situation"] = report.map_or_else(
        || {
            json!({"status":"unavailable","all_clear_proven":false,
        "reason":"live_source_or_current_query_authority_unavailable"})
        },
...
fn packet_with_basis(
    s: Option<&Session>,
    c: Option<&OperationContext>,
    operation: &str,
    mut v: Value,
    before: Option<&situation::Report>,
) -> Result<String> {
    ...
    if let (Some(s), Some(c)) = (s, c) {
        ...
        v["session_id"] = json!(s.id.to_string());
        v["anchor"] = anchor_json(a);
        let report = situation_report(s, c)?;
        if let Some(report) = &report {
            builder = builder.attention(report.attention(s.id));
            if let Some(before) = before {
                let (summary, changes) = report.comparison(before);
                v["situation_comparison"] = summary;
        ...
        let recovered = v
            ...
            .is_some_and(|n| n > 0);
        let continuity = if s.source.poisoned() {
        ...
        builder = builder.briefing(
            json!({"runtime_admitted":false,"mutation_admissible":false,"bridge_protocol":"1.8"}),
        );
    }
    let out = builder.attach(v);
    if out.len() > maximum {
        return Err(error(
            ErrorCode::BudgetExceeded,
            "complete spatial/1.8 packet exceeds output budget",
        ));
    }
    Ok(out)
}
fn failure(
    ...
    let result = match (s, c) {
        (Some(s), Some(c))
            if c.authorize(Capability::Query, RiskTier::ReadOnly, &[], None)
                .is_ok() =>
        {
            semantic_query::publish_with_active_work(c, value.clone(), |v| {
                packet(Some(s), Some(c), operation, v)
            })
        }
        _ => packet(None, None, operation, value.clone()),
    };
...
fn with_session<F>(id: Option<String>, operation: &str, cap: Capability, body: F) -> String
where
    F: FnOnce(&mut Session, OperationContext) -> Result<String>,
{
    ...
    if let Err(e) = c.authorize(cap, RiskTier::ReadOnly, &[], None) {
        return failure(None, None, operation, &e);
    }
    if let Err(e) = archive::validate(&mut s, &c) {
        return failure(Some(&s), Some(&c), operation, &e);
    }
...
fn parse_region(input: &Value) -> Result<Region> {
    let object = input
        .as_object()
        .ok_or_else(|| error(ErrorCode::InvalidRequest, "region must be an object"))?;
    if object.len() != 2
        || !["origin", "size"].iter().all(|k| {
            object.get(*k).and_then(Value::as_array).is_some_and(|a| {
                a.len() == 3 && a.iter().all(|v| v.as_u64().is_some_and(|n| n <= 32768))
            })
        })
    {
        return Err(error(
            ErrorCode::InvalidRequest,
            "region requires bounded origin and size triples",
        ));
    }
    let r: RegionInput = serde_json::from_value(input.clone())
        .map_err(|_| error(ErrorCode::InvalidRequest, "invalid spatial/1.8 region"))?;
    let r = Region {
        origin: r.origin,
        size: r.size,
    };
    ...
    for name in input {
        let c = match name.as_str() {
            ...
            _ => {
                return Err(error(
                    ErrorCode::CapabilityDenied,
                    "spatial/1.8 cannot grant that capability",
                ));
            }
        };
        if out.contains(&c) {
...
pub fn fortress_open_session(
    ...
    let result = (|| -> Result<String> {
        ...
        let caps = if recovery_only {
            archive::requested_capabilities(requested_capabilities)?
        } else {
            capabilities(requested_capabilities)?
        };
        let journal_configuration = history::configuration()?;
        let watch_path = durable_watches::configuration(
            journal_configuration
                .as_ref()
                .map(|(path, _)| path.as_path()),
        )?;
        if recovery_only {
            archive::validate_configuration(
                journal_configuration
                    .as_ref()
                    .map(|(path, _)| path.as_path()),
                journal_configuration
                    .as_ref()
                    .map(|(_, repair)| *repair)
                    .unwrap_or(dfmcp_adapter::operations_journal::TailRecovery::Refuse),
                watch_path.as_deref(),
            )?;
        } else if journal_configuration.is_some()
            && (!caps.contains(&Capability::Query) || !caps.contains(&Capability::Observe))
        {
            return Err(error(
                ErrorCode::CapabilityDenied,
                "durable spatial/1.8 sessions require Query and Observe authority",
            ));
        }
        let capture =
            usize::try_from(max_capture_bytes.unwrap_or(MAX_SPATIAL_CITIZEN_BYTES as u64))
                .map_err(|_| error(ErrorCode::BudgetExceeded, "capture size overflow"))?;
        let spatial = SpatialLimits {
            operations: PagedOperationsLimits {
                items: max_items.unwrap_or(65536),
                payload_bytes: capture,
                page_bytes: page_bytes.unwrap_or(65536) as usize,
        ...
        let limits = CitizenSpatialLimits {
            spatial,
            citizens: max_citizens.unwrap_or(4096),
        };
        limits.validate()?;
            ...
            || !(1..=60000).contains(&budget.max_wall_millis)
        {
            return Err(error(
                ErrorCode::BudgetExceeded,
                "spatial/1.8 output or wall budget outside bounds",
            ));
        }
        let slot = Slot::reserve()?;
        let id = next_id()?;
        if recovery_only {
            let path = journal_configuration
                .as_ref()
                .map(|(path, _)| path.as_path())
                .ok_or_else(|| error(ErrorCode::InvalidRequest, "archive path missing"))?;
            let mut s = archive::open(id, slot, path, limits, budget, &caps)?;
            let c = s.context()?;
            let out = packet(
                Some(&s),
                Some(&c),
                "fortress.open_session",
                json!({"ok":true,"recovery_only":true,
        ...
        let endpoint = dfmcp_adapter::parse_loopback_endpoint(
            &std::env::var("DFMCP_SPATIAL_CITIZEN_ENDPOINT")
                .unwrap_or_else(|_| "127.0.0.1:5000".to_owned()),
        )?;
        let token = std::env::var("DFMCP_SPATIAL_CITIZEN_TOKEN").map_err(|_| {
            error(
                ErrorCode::CapabilityDenied,
                "DFMCP_SPATIAL_CITIZEN_TOKEN required",
            )
        })?;
        let (source, state) = observation::connect_and_capture(
            endpoint,
            token.into_bytes(),
        ...
        let fortress = state
            ...
            .ok_or_else(|| {
                error(
                    ErrorCode::InternalInvariantViolation,
                    "spatial/1.8 bootstrap snapshot absent",
                )
            })?
            .fortress_id;
        let grants = caps
...
fn observe(id: Option<String>, operation: &str) -> String {
    with_session(id, operation, Capability::Observe, |s, c| {
        let before = situation_report(s, &c)?;
        ...
        let v = json!({"ok":true,"kind":if outcome==JobPublication::Heartbeat{"heartbeat"}else{"snapshot"},
        "reset":outcome==JobPublication::Reset,"native_captures":1,"transfer_pages":s.source.pages(),"game_clock_controlled":false});
        if target
            .authorize(Capability::Query, RiskTier::ReadOnly, &[], None)
            .is_ok()
        {
            semantic_query::publish_with_active_work(&target, v, |v| {
                packet_with_basis(Some(s), Some(&c), operation, v, before.as_ref())
            })
        } else {
            packet_with_basis(Some(s), Some(&c), operation, v, None)
        }
    })
}
#[tool(
    description = "Acquire one coherent capture without changing game time. Query-authorized live sessions receive operational attention and bounded endpoint-count changes, not event history or causal diagnoses. Archive-only sessions refuse."
)]
pub fn fortress_observe(session_id: Option<String>) -> String {
    observe(session_id, "fortress.observe")
}
#[tool(
    description = "Acquire one coherent capture and summarize operational count changes. For monitoring use query await_watch or await_watches; await_watches evaluates up to eight watches after one shared capture. Archive-only sessions refuse."
)]
pub fn fortress_wait(session_id: Option<String>) -> String {
    observe(session_id, "fortress.wait")
}
fn view(s: &Session, c: &OperationContext) -> Result<QueryResponseProjection> {
    Ok(QueryResponseProjection {
        ...
        coverage: coverage(),
        uncertainty: vec![
            json!({"domain":"unit_navigation_and_labor","epistemic_state":"partial","reason":"sparse skill and job-availability evidence is observed; needs, native labor eligibility and full movement rules are not"}),
        ],
        budget: json!({"admitted":{"max_bytes":s.budget.max_bytes,"max_output_tokens":s.budget.max_output_tokens}}),
        references: vec![
            json!({"kind":"coherent_citizen_spatial_capture","digest":s.state.source_digest()?.to_string()}),
        ],
        maximum_bytes: s
            .budget
            .max_bytes
...
fn finish(view: &QueryResponseProjection, v: Value) -> Result<String> {
    let mut v: Value = serde_json::from_str(&view.finish(v)?).map_err(|_| {
        error(
            ErrorCode::InternalInvariantViolation,
            "spatial/1.8 response invalid",
        )
    })?;
    v["agent_turn"]["turn_id"] = json!(format!("spatial-citizen-turn-{}", view.request_id));
    let out = v.to_string();
    if out.len() > view.maximum_bytes {
        return Err(error(
            ErrorCode::BudgetExceeded,
            "spatial/1.8 query exceeds budget",
        ));
    }
    Ok(out)
}
...
pub fn fortress_query(
    ...
    with_session(
        ...
        |s, mut c| {
            ...
            if mode.as_deref() == Some("situation") {
                return situation_presentation::detail(s, &c);
            }
            let mut input = match query {
                Some(v) => v,
                None => match mode.as_deref() {
                    None | Some("summary" | "schema") => {
                        json!({"schema":"dfmcp.query/1","query":{"kind":"aggregate","group_by":{"kind":"entity_kind"}}})
                    }
                    Some("history") => json!({"schema":"dfmcp.query/1","query":{"kind":"history"}}),
                    Some("production") => {
                        json!({"schema":"dfmcp.query/1","query":{"kind":"production_diagnosis"}})
                    }
                    Some("citizens") => {
                        json!({"schema":"dfmcp.query/1","query":{"kind":"entities","kinds":["unit"],"fields":["name","profession","position","alive","active"],"limit":1}})
                    }
                    Some("items") => {
                        json!({"schema":"dfmcp.query/1","query":{"kind":"entities","kinds":["item"],"fields":["type_key","stack_size","raw_position"],"limit":1}})
                    }
                    Some("jobs") => {
                        json!({"schema":"dfmcp.query/1","query":{"kind":"entities","kinds":["job"],"fields":["type_key","suspended","worker_entity","worker_is_strict_citizen"],"limit":1}})
                    }
                    Some("buildings") => {
                        json!({"schema":"dfmcp.query/1","query":{"kind":"entities","kinds":["building"],"fields":["type_key","build_stage"],"limit":1}})
                    }
                    Some("tiles") => {
                        json!({"schema":"dfmcp.query/1","query":{"kind":"entities","kinds":["tile_feature"],"fields":["position","visibility","shape"],"limit":1}})
                    }
                    _ => {
                        return Err(error(
                            ErrorCode::InvalidRequest,
                            "unknown spatial/1.8 query mode",
                        ));
                    }
                },
            };
            if s.source.archive_only() {
                return archive::query(s, &c, &input, schema);
            }
            if !schema && watch_batch::handles(&input) {
                return watch_batch::execute(s, &c, &input);
            }
            let kind = input
                .get("query")
            ...
            if !schema && production::handles(&input) {
                let result = production::live(s, &c, &input);
                if matches!(&result,Err(e)if e.code==ErrorCode::CorruptLedger) {
                    s.source.fence();
                }
                return result;
            ...
            if s.source.poisoned() && !local && !schema {
                return Err(error(
                    ...
                    "spatial/1.8 source fenced; reopen or manage local records",
                ));
            }
            let mut refresh = None;
            if !schema && kind == "await_watch" {
                let snapshot = s.state.snapshot().ok_or_else(|| {
                    error(ErrorCode::InternalInvariantViolation, "snapshot absent")
                })?;
                if semantic_query::prepare_await(snapshot, &c, &input)? {
                    ...
                    refresh = Some(
                        json!({"basis":anchor_json(basis),"reset":outcome==JobPublication::Reset,"kind":if outcome==JobPublication::Heartbeat{"heartbeat"}else{"snapshot"},"native_captures":1,"transfer_pages":s.source.pages()}),
                    );
                }
                if let Some(obj) = input.as_object_mut() {
                    obj.remove("expected_anchor");
                }
                input["query"]["kind"] = json!("poll_watch");
            }
            let view = situation_view(s, &c)?;
            let mut narrowed = c.clone();
            narrowed.budget.max_bytes =
                situation_presentation::result_budget(&view, &input)? as u64;
            if schema {
                return semantic_query::publish_with_active_work(
                    &c,
                    json!({"query_schema":watch_batch::extend_schema(workforce_queries::extend_schema(history::schema()?)?)?,"mode":"schema","profile":"spatial/1.8","source_stale":s.source.poisoned(),"truncated":false,"continuation":null}),
                    |v| finish(&view, v),
                );
            }
            if workforce_queries::handles(&input) {
                let rc = semantic_query::result_context(&narrowed)?;
                let v = workforce_queries::execute(&s.state, &rc, &input)?;
                return semantic_query::publish_with_active_work(&narrowed, v, |v| {
                    finish(&view, v)
                });
            }
            if spatial_queries::handles(&input) {
                let rc = semantic_query::result_context(&narrowed)?;
                let v = spatial_queries::execute(&s.state, &rc, &input)?;
                return semantic_query::publish_with_active_work(&narrowed, v, |v| {
                    finish(&view, v)
                });
            }
            ...
            semantic_query::execute_with_publisher(snapshot, &narrowed, &input, |mut v| {
                v["source_stale"] = json!(s.source.poisoned());
                if let Some(r) = refresh {
                    v["observation_refresh"] = r;
                }
                finish(&view, v)
            })
        },
...
#[tool(
    description = "Explain coherent evidence, workforce limits and the deterministic situation-attention rule catalog. Findings are inspection priorities, not causal diagnoses or effect authority."
)]
pub fn fortress_explain(session_id: Option<String>) -> String {
    with_session(session_id, "fortress.explain", Capability::Query, |s, c| {
        ...
        "unknown":["noncitizen unit details","complete native skill-key registry","needs","labor eligibility","full unit pathfinding","outside-region routes"]}),
            |v| packet(Some(s), Some(&c), "fortress.explain", v),
        )
    })
}
#[tool(
    description = "Report spatial/1.8 source or archive health; no reconnect or qualification claim."
)]
pub fn fortress_doctor(session_id: Option<String>) -> String {
    with_session(session_id, "fortress.doctor", Capability::Doctor, |s, c| {
        packet(
            ...
            Some(&c),
            "fortress.doctor",
            json!({"ok":true,"status":if s.source.poisoned(){"source_fenced"}else if s.source.archive_only(){"archive_only"}else{"read_only_unadmitted"}}),
        )
    })
}
fn no_effect(id: Option<String>, operation: &str) -> String {
    with_session(id, operation, Capability::Query, |_, _| {
        Err(error(
            ErrorCode::CapabilityDenied,
            "spatial/1.8 has no live mutation or reservation path",
        ))
    })
}
#[tool(description = "Unavailable: coherent observations do not authorize effects.")]
pub fn fortress_plan(session_id: Option<String>) -> String {
    no_effect(session_id, "fortress.plan")
}
#[tool(description = "Unavailable: no executable plan is created.")]
...
pub fn fortress_cancel(
    session_id: Option<String>,
    scope: Option<String>,
    discard_process_local_work: Option<bool>,
) -> String {
