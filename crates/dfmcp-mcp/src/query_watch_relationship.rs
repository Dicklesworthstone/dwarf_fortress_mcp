...
pub(in super::super) enum Relation {
    ContainedIn,
    Uses,
    Performs,
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub(in super::super) enum Direction {
    Incoming,
    Outgoing,
}
...
impl BindingState {
    fn text(self) -> &'static str {
        match self {
            Self::Ready => "bound",
            Self::Missing => "root_not_observed",
...
pub(super) struct Bindings {
    entries: BTreeMap<Selector, Binding>,
}
impl Bindings {
    pub(super) fn ready(&self) -> bool {
        self.entries
            .values()
            .all(|binding| binding.state == BindingState::Ready)
    }
    pub(super) fn invalid_generation(&self) -> bool {
        self.entries
            .values()
            .any(|binding| binding.state == BindingState::GenerationMismatch)
    }
    ...
    pub(super) fn row_truth(
        ...
        let selector = Selector {
            root: positive_id(entity_id)?,
            generation,
            relation,
            direction,
        };
        let Some(binding) = self.entries.get(&selector) else {
    ...
    pub(super) fn annotate(&self, fact: &mut Value) {
        if self.entries.is_empty() {
            return;
        } // Keep old evidence bytes unchanged.
...
fn edge_truth(
    ...
    if edge.revision == 0 || edge.fields.is_empty() {
        return Ok(Truth::Unknown);
    }
    let mut source = None;
...
pub(super) fn bind(
    ...
    for selector in selectors {
        ...
        let state = match root {
            ...
            Some(root)
                if root.id != selector.root || root.generation == 0 || root.revision == 0 =>
            {
                BindingState::IdentityUnestablished
            }
            Some(root) if root.generation != selector.generation => {
                BindingState::GenerationMismatch
            }
            Some(_) => BindingState::Ready,
        };
        let mut binding = Binding {
            state,
            current_generation: root.map(|root| root.generation),
            members: BTreeMap::new(),
        };
        if state == BindingState::Ready {
            for (edge_id, edge) in &snapshot.graph.edges {
                budget.charge()?;
                if !selector.relation.matches(&edge.kind) {
                    continue;
                }
                ...
                let truth = if edge.id == *edge_id {
                    edge_truth(edge, snapshot, budget)?
                } else {
                    Truth::Unknown
                };
