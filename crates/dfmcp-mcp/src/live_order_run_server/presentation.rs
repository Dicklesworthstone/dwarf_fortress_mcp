...
use super::error;
use crate::agent_turn::{AgentPhase,AgentTurnBuilder,ContinuityStatus,empty_active_work,recommendation,uncertainty};
...
pub fn digest(raw:&str)->Result<Digest32>{
    ...
    let mut out=[0;32];for (index,pair) in raw.as_bytes().chunks_exact(2).enumerate(){
        let part=std::str::from_utf8(pair).map_err(|_|error(ErrorCode::InvalidRequest,"invalid digest"))?;
        out[index]=u8::from_str_radix(part,16).map_err(|_|error(ErrorCode::InvalidRequest,"invalid digest"))?;
    }
    Ok(Digest32::from_bytes(out))
}
pub fn mode_name(mode:OrderRunMode)->&'static str{match mode{OrderRunMode::Control=>"control",OrderRunMode::Recover=>"recover",OrderRunMode::Offline=>"offline"}}
pub fn capture(value:&OrderCapture)->Value{json!({
...
fn plan(value:&OrderRunPlan)->Value{let s=value.spec();json!({"idempotency_key":value.key(),"plan_digest":value.digest().to_string(),
...
pub fn entry(value:&OrderRunEntry)->Value{json!({"plan":plan(value.plan()),"state":value.state().as_str(),
...
pub fn summary(view:&OrderRunView)->Value{json!({"journal_id":view.id.to_string(),"head":view.head.to_string(),"transitions":view.transitions,
...
pub fn failure(cause:&dfmcp_core::DfmcpError)->Value{json!({"ok":false,"error":{"code":cause.code.as_str(),"message":cause.message},
    "effect_outcome_inferred":false,"retry_commit":false,"recovery":"inspect retained journal; query or cancel uncertain work"})}
#[derive(Clone,Copy,Debug,PartialEq,Eq)]
pub enum Filter{All,Pending,Unresolved,Terminal}
impl Filter{
    pub fn parse(raw:&str)->Result<Self>{match raw{"all"=>Ok(Self::All),"pending"=>Ok(Self::Pending),"unresolved"=>Ok(Self::Unresolved),
        "terminal"=>Ok(Self::Terminal),_=>Err(error(ErrorCode::InvalidRequest,"state must be all, pending, unresolved or terminal"))}}
    pub fn name(self)->&'static str{match self{Self::All=>"all",Self::Pending=>"pending",Self::Unresolved=>"unresolved",Self::Terminal=>"terminal"}}
    pub fn matches(self,e:&OrderRunEntry)->bool{match self{Self::All=>true,Self::Pending=>!e.settled(),Self::Unresolved=>e.unresolved(),Self::Terminal=>e.settled()}}
}
struct Cursor{session:SessionId,journal:Digest32,head:Digest32,filter:Filter,limit:usize,offset:usize}
#[derive(Default)]
pub struct Cursors{serial:u64,issued:VecDeque<(String,Cursor)>}
impl Cursors{
    pub fn resolve(&self,raw:&str,session:SessionId,view:&OrderRunView,filter:Filter,limit:usize)->Result<usize>{
        if raw.len()!=64{return Err(error(ErrorCode::InvalidRequest,"invalid conditional-run continuation"));}
        let (_,c)=self.issued.iter().find(|(token,_)|token==raw)
            .ok_or_else(||error(ErrorCode::StaleAnchor,"continuation expired; restart record discovery"))?;
        if c.session!=session || c.journal!=view.id || c.head!=view.head || c.filter!=filter || c.limit!=limit {
            return Err(error(ErrorCode::StaleAnchor,"continuation belongs to another session, journal head, filter or page size"));
        }
        Ok(c.offset)
    }
    pub fn issue(&mut self,session:SessionId,view:&OrderRunView,filter:Filter,limit:usize,offset:usize)->Result<String>{
        self.serial=self.serial.checked_add(1).ok_or_else(||error(ErrorCode::BudgetExceeded,"continuation IDs exhausted"))?;
        let mut bytes=b"dfmcp-order-run-mcp-cursor/1\0".to_vec();bytes.extend_from_slice(&session.get().to_be_bytes());
        bytes.extend_from_slice(view.id.as_bytes());bytes.extend_from_slice(view.head.as_bytes());bytes.extend_from_slice(&self.serial.to_be_bytes());
        let token=Digest32::of_bytes(&bytes).to_string();
        if self.issued.len()==64{self.issued.pop_front();}
        self.issued.push_back((token.clone(),Cursor{session,journal:view.id,head:view.head,filter,limit,offset}));Ok(token)
    }
}
pub fn packet(operation:&str,result:Value,context:Option<&OperationContext>,mode:Option<OrderRunMode>,view:Option<&OrderRunView>)->String{
    ...
    active["scope"]=json!("this_verified_conditional_run_journal_only");active["inventory_verified"]=json!(view.is_some());
    active["absence_proven"]=json!(view.is_some_and(|v|v.entries.iter().all(OrderRunEntry::settled)));
    if let Some(view)=view{
        ...
        active["pending_plans"]=json!(refs.iter().filter(|r|matches!(r["state"].as_str(),Some("intent"|"prepared"))).collect::<Vec<_>>());
        active["actions"]=json!(refs.iter().filter(|r|r["reconciliation_required"]==true).collect::<Vec<_>>());
        active["indeterminate_effects"]=json!(refs.iter().filter(|r|r["reconciliation_required"]==true).collect::<Vec<_>>());
        active["cancellation_drains"]=json!(refs.iter().filter(|r|r["state"]=="cancel_requested").collect::<Vec<_>>());
        active["counts"]=summary(view);
        active["omitted_pending_references"]=json!(view.entries.iter().filter(|e|!e.settled()).count().saturating_sub(4));
    }
    ...
    if let Some(briefing)=turn["briefing"].as_object_mut(){briefing.remove("admission");}
