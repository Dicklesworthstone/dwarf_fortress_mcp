...
impl Read for Memory {
    fn read(&mut self, out: &mut [u8]) -> io::Result<usize> {
        ...
        let at = (self.offset as usize).min(b.len());
        let n = out.len().min(b.len() - at);
        out[..n].copy_from_slice(&b[at..at + n]);
...
impl Seek for Memory {
    fn seek(&mut self, from: SeekFrom) -> io::Result<u64> {
        ...
        self.offset = u64::try_from(n).map_err(|_| io::Error::other("seek"))?;
        Ok(self.offset)
    }
}
...
fn fixture() -> Result<OrderRunRecord> {
    ...
    OrderRunRecord::decode(&bytes)
}
fn native_record(plan: &OrderRunPlan, phase: u8, reason: u8) -> Result<OrderRunRecord> {
    fn field(out: &mut Vec<u8>, v: &[u8]) {
        out.extend_from_slice(&(v.len() as u16).to_be_bytes());
        out.extend_from_slice(v);
    }
    let mut out = b"DFMOE014".to_vec();
    field(&mut out, plan.key().as_bytes());
    field(&mut out, plan.native_bytes());
    ...
    out.extend_from_slice(&(if known { plan.before().tick() } else { 0 }).to_be_bytes());
    out.extend_from_slice(&0u32.to_be_bytes());
    out.extend_from_slice(&plan.before().tick().to_be_bytes());
    field(&mut out, &[]);
    let mut proof = b"dfmcp-order-run-receipt/1\0".to_vec();
    proof.extend_from_slice(&out);
    out.extend_from_slice(Digest32::of_bytes(&proof).as_bytes());
    OrderRunRecord::decode(&out)
}
impl Source {
    fn new(calls: Rc<RefCell<Calls>>) -> Result<Self> {
        let p = fixture()?.plan().clone();
        Ok(Self {
            calls,
            manifest: OrderRunManifest {
                generation: 41,
            ...
            fortress: p.before().fortress().clone(),
...
impl OrderRunSource for Source {
    ...
    fn observe(&mut self, _: u32, _: &OperationContext, _: Duration) -> Result<OrderCapture> {
        self.calls.borrow_mut().observe += 1;
    ...
    fn prepare(
        ...
        let r = native_record(p, 0, 0)?;
        let mut c = self.calls.borrow_mut();
        c.prepare += 1;
        c.record = Some(r.clone());
        Ok(r)
    }
    fn commit(
        &mut self,
        ...
        let r = native_record(p, 1, 0)?;
        let mut c = self.calls.borrow_mut();
        c.commit += 1;
        c.record = Some(r.clone());
        if c.lost_commit {
            return Err(error(ErrorCode::AdapterUnavailable, "lost commit reply"));
        }
        Ok(r)
    }
    fn query(
...
fn setup(
    ...
    let n = Source::new(calls)?;
    let fortress = n.fortress().clone();
...
fn dispatch_limits(
    ...
    let output = run_action(
        ...
        move |_| {
            calls.borrow_mut().connect += 1;
            Source::new(calls)
        },
    );
    serde_json::from_str(&output).map_err(|_| error(ErrorCode::InvalidRequest, "MCP JSON result"))
}
fn condition() -> Result<Condition> {
    Condition::parse(
        r#"{"order_id":9,"predicate":"approved","game_ticks":20,"wall_millis":1000,"stable_samples":2,"interval_ticks":2}"#,
    )
}
fn prepare(state: &mut State<Memory>, calls: &Rc<RefCell<Calls>>, key: &str) -> Result<Digest32> {
    let observed = dispatch(state, calls, "fortress.observe", Action::Observe(9))?;
    assert_eq!(observed["result"]["ok"], true);
    let witness = digest(
        observed["result"]["observation"]["witness"]
            .as_str()
    ...
    digest(
        result["result"]["effect"]["plan"]["plan_digest"]
            .as_str()
            .ok_or_else(budget_error)?,
    )
}
...
fn dispatcher_lifecycle_and_offline_recovery_keep_goal_pause_and_goods_distinct() -> Result<()> {
    ...
    let mut state = setup(m.clone(), calls.clone(), OrderRunMode::Control, true)?;
    ...
    assert_eq!(result["result"]["effect"]["native"]["phase"], "running");
    calls.borrow_mut().record = Some(fixture()?);
    let result = dispatch(
        &mut state,
        &calls,
        "fortress.wait",
    ...
    assert_eq!(
        result["result"]["effect"]["native"]["predicate_observed"],
        true
    );
    assert_eq!(result["result"]["effect"]["native"]["pause_verified"], true);
    assert_eq!(result["result"]["goal_completion_proven"], false);
    assert_eq!(
        result["agent_turn"]["anchor"]["scope"],
        "coordination_root_not_world_state"
    );
    drop(state);
    let count = calls.borrow().connect;
    let mut offline = setup(m, calls.clone(), OrderRunMode::Offline, false)?;
    ...
    assert_eq!(recovered["result"]["effect"], result["result"]["effect"]);
    assert_eq!(calls.borrow().connect, count);
    assert_eq!(calls.borrow().commit, 1);
    assert_eq!(calls.borrow().query, 1);
    Ok(())
}
#[test]
fn missing_commit_reply_reopen_cannot_recommit_or_erase_uncertainty() -> Result<()> {
    ...
    let mut state = setup(m.clone(), calls.clone(), OrderRunMode::Control, true)?;
    let p = prepare(&mut state, &calls, "approval")?;
    calls.borrow_mut().lost_commit = true;
    assert_eq!(
        dispatch(
            &mut state,
            &calls,
    ...
    drop(state);
    let mut state = setup(m, calls.clone(), OrderRunMode::Control, false)?;
    let connects = calls.borrow().connect;
    assert_eq!(
        dispatch(
            &mut state,
            &calls,
    ...
    assert_eq!(calls.borrow().connect, connects);
    calls.borrow_mut().record = None;
    let absent = dispatch(
        &mut state,
        &calls,
        "fortress.wait",
    ...
    assert_eq!(
        absent["agent_turn"]["active_work"]["counts"]["unresolved"],
        1
    );
    assert_eq!(calls.borrow().commit, 1);
    Ok(())
...
fn confirmation_and_mode_refusals_precede_connection_even_with_injected_grants() -> Result<()> {
    ...
    for mode in [OrderRunMode::Offline, OrderRunMode::Recover] {
        let mut state = setup(m.clone(), calls.clone(), mode, false)?;
        state.grants = grants(
            OrderRunMode::Control,
            fixture()?.plan().before().fortress(),
            true,
        )?;
        assert_eq!(
            dispatch(
...
fn local_cancel_avoids_connection_active_cancel_sends_only_safety_request() -> Result<()> {
    let m = Memory::default();
    let calls = Rc::new(RefCell::new(Calls::default()));
    let mut state = setup(m, calls.clone(), OrderRunMode::Control, true)?;
    let p = prepare(&mut state, &calls, "local")?;
    let count = calls.borrow().connect;
    ...
    assert_eq!(
        stopped["result"]["effect"]["native"]["pause_verified"],
        true
    );
    assert_eq!(calls.borrow().cancel, 1);
    assert_eq!(calls.borrow().commit, 1);
    Ok(())
}
#[test]
fn output_work_and_tick_limits_refuse_before_native_effects() -> Result<()> {
    ...
    assert_eq!(calls.borrow().connect, 0);
    let p = prepare(&mut state, &calls, "approval")?;
    let count = calls.borrow().connect;
    let out = dispatch_limits(
        &mut state,
    ...
    assert_eq!(out["result"]["ok"], false);
    assert_eq!(calls.borrow().connect, count);
    assert_eq!(calls.borrow().commit, 0);
    Ok(())
}
#[test]
fn whole_row_pagination_binds_filter_head_and_session_without_native_calls() -> Result<()> {
    ...
    let mut state = setup(m, calls.clone(), OrderRunMode::Control, true)?;
    for key in ["a", "b", "c"] {
        let p = prepare(&mut state, &calls, key)?;
        dispatch(
            &mut state,
            &calls,
            "fortress.cancel",
    ...
    assert_eq!(
        second["result"]["records"][0]["plan"]["idempotency_key"],
        "b"
    );
    assert_eq!(
        second["result"]["complete_matching_set_in_this_response"],
        false
    );
    assert_eq!(
        dispatch(
    ...
    let c = state.context(true, false)?;
    let mut view = state.journal.view(&c)?;
    view.head = Digest32::ZERO;
    assert!(
        state
            .cursors
            .resolve(&token, state.id, &view, Filter::All, 1)
            .is_err()
    );
    assert!(
        state
            .cursors
            .resolve(&token, SessionId::new(999), &view, Filter::All, 1)
            .is_err()
    );
    assert_eq!(calls.borrow().connect, count);
    Ok(())
...
fn native_failure_does_not_remove_healthy_local_history() -> Result<()> {
    ...
    let p = prepare(&mut state, &calls, "approval")?;
    let c = state.context(true, false)?;
    let failed = run_action::<_, Source, _>(
        &mut state,
        c,
        Dispatch {
            ...
            rows: 1,
            action: Ok(Action::Observe(9)),
        },
        |_| Err(error(ErrorCode::AdapterUnavailable, "offline native test")),
    );
    assert!(state.selected.is_none());
    assert_eq!(
        serde_json::from_str::<Value>(&failed).map_err(|_| budget_error())?["result"]["ok"],
        false
    );
    assert_eq!(
        dispatch(
...
fn per_effect_guard_can_revoke_after_dispatch_marker_without_unpause() -> Result<()> {
    fn deny(clock: bool, _: &FortressIdentity) -> Result<()> {
        if clock {
    ...
    let out = run_action(
        ...
        move |_| {
            Ok(CheckedSource {
                inner: Source::new(calls_for_source)?,
                check: deny,
            })
        },
    );
    ...
    let c = state.context(true, false)?;
    assert_eq!(
        state.journal.view(&c)?.entries[0].state(),
...
fn strict_condition_and_digest_parsing_reject_unknown_or_ambiguous_inputs() -> Result<()> {
    condition()?;
    for raw in [
        r#"{"order_id":9,"predicate":"shell","game_ticks":20,"wall_millis":1000}"#,
        ...
        r#"{"order_id":9,"predicate":"approved","game_ticks":20,"wall_millis":1000,"stable_samples":16,"interval_ticks":2}"#,
    ] {
        assert!(Condition::parse(raw).is_err());
    }
    assert!(Condition::parse(&" ".repeat(2049)).is_err());
    assert!(digest(&"A".repeat(64)).is_err());
    assert!(digest("0").is_err());
    Ok(())
}
#[test]
fn packets_keep_scope_and_do_not_invent_verified_empty_history() -> Result<()> {
    let cause = error(ErrorCode::AdapterUnavailable, "no source");
    let encoded = unbound("fortress.observe", &cause);
    let parsed: Value = serde_json::from_str(&encoded).map_err(|_| budget_error())?;
    assert_eq!(parsed["agent_turn"]["schema"], "dfmcp.agent_turn/1");
    assert_eq!(parsed["agent_turn"]["active_work"]["absence_proven"], false);
    assert!(parsed["agent_turn"]["anchor"].is_null());
    assert!(parsed["agent_turn"]["briefing"]["admission"].is_null());
    assert_eq!(parsed["agent_turn"]["briefing"]["runtime_admitted"], false);
    assert!(encoded.len() < BASE_OUTPUT as usize);
    Ok(())
}
