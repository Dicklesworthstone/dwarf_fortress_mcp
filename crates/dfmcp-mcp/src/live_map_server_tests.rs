...
impl MapSource for Script {
    fn read(&mut self, _: Duration) -> Result<LiveMapObservation> {
        self.calls.fetch_add(1, Ordering::SeqCst);
        self.values
            .pop_front()
            .ok_or_else(|| error(ErrorCode::AdapterUnavailable, "test map source exhausted"))
    }
...
fn fixture() -> Result<LiveMapObservation> {
    let hex = include_str!("../../dfmcp-adapter/tests/fixtures/map_v1_5.hex").trim();
    let bytes = (0..hex.len())
        .step_by(2)
        .map(|i| {
            u8::from_str_radix(&hex[i..i + 2], 16)
                .map_err(|_| error(ErrorCode::InternalInvariantViolation, "test hex"))
        })
...
fn register(
    ...
    second.year_tick += 1;
    if let Some(Cell::Visible(t)) = second.map.cells.get_mut(2) {
        t.liquid_depth = 0;
    }
    let volume = first.map.cells.len();
    ...
    let s = MapSession {
        ...
        source: Box::new(Script {
            values: VecDeque::from([second]),
            fenced: false,
            calls: Arc::clone(&calls),
        }),
        state,
        budget: WorkBudget {
            max_entities: volume as u32 + 1,
...
fn route(start: [u32; 3], goal: [u32; 3], limit: u32) -> Value {
    json!({"kind":"map_route","start":start,"goal":goal,"limit":limit})
}
...
fn native_tile_queries_and_complementary_stair_route_use_actual_handlers() -> Result<()> {
    ...
    let hidden = ask(
        &s,
        json!({"kind":"inspect","entity_id":tile_entity_id([15,15,1])?.to_string(),"generation":1,"fields":["shape"]}),
    )?;
    assert_eq!(hidden["ok"], true);
    assert!(
        hidden["row"]["fields"]["shape"]
            .to_string()
            .contains("redacted")
    );
    let path = ask(&s, route([14, 15, 1], [14, 15, 2], 64))?;
    assert_eq!(path["ok"], true);
    assert_eq!(path["status"], "candidate_found");
    assert_eq!(path["model_steps"], 1);
    assert_eq!(path["rows"][1]["position"], json!([14, 15, 2]));
    assert_eq!(path["unit_path_proven"], false);
    assert_eq!(path["global_unreachability_proven"], false);
    assert_eq!(path["safety_proven"], false);
    assert_eq!(
        ask(&s, route([14, 15, 1], [16, 15, 1], 64))?["status"],
        "endpoint_excluded"
    );
...
fn liquid_watch_and_baseline_share_one_real_refresh_without_claiming_safe_terrain() -> Result<()> {
    ...
    let capture = ask(
        &s,
        json!({"kind":"capture","key":"liquid","max_game_ticks":100,
        "select":{"kind":"entities","kinds":["tile_feature"],"fields":["liquid_depth"]}}),
    )?;
    assert_eq!(capture["ok"], true);
    let watch = ask(
        &s,
        json!({"kind":"watch","key":"dry-tile","label":"Observed zero liquid depth",
    ...
    assert_eq!(watch["record"]["terminal"], false);
    let request = json!({"kind":"await_watch","watch":watch["record"]["watch"]});
    let completed = ask(&s, request.clone())?;
    assert_eq!(completed["ok"], true);
    assert_eq!(completed["record"]["terminal"], true);
    assert_eq!(completed["observation_refresh"]["native_observations"], 1);
    assert_eq!(s.calls.load(Ordering::SeqCst), 1);
    assert_eq!(ask(&s, request)?["record"], completed["record"]);
    assert_eq!(s.calls.load(Ordering::SeqCst), 1);
    let changes = ask(
        &s,
    ...
    assert_eq!(
        changes["changes"][0]["entity_id"],
        tile_entity_id([16, 15, 1])?.to_string()
    );
    assert_eq!(
        ask(
            &s,
            json!({"kind":"release_watch","watch":watch["record"]["watch"]})
        )?["ok"],
        true
    );
    assert_eq!(
...
fn flat() -> Result<LiveMapObservation> {
    let mut v = fixture()?;
    v.map = MapRegion {
        region: Region {
            origin: [0, 0, 0],
...
fn route_paging_keeps_active_work_and_binds_session_snapshot_and_endpoints() -> Result<()> {
    ...
    let created = ask(
        &s,
        json!({"kind":"watch","key":"review","label":"Wait for map review",
    ...
    loop {
        ...
        let rows = v["rows"]
            .as_array()
            .ok_or_else(|| error(ErrorCode::InternalInvariantViolation, "route rows"))?;
        assert!(!rows.is_empty());
        positions.extend(rows.iter().map(|r| r["position"].clone()));
        pages += 1;
        assert!(pages <= 31);
    ...
    assert_eq!(positions[0], json!([0, 0, 0]));
    assert_eq!(positions[30], json!([15, 15, 0]));
    assert_eq!(s.calls.load(Ordering::SeqCst), 0);
    ...
    assert_eq!(
        ask(
            &s,
            json!({"kind":"cancel_watch","watch":created["record"]["watch"]})
        )?["ok"],
        true
    );
    assert_eq!(
        ask(
            &s,
            json!({"kind":"release_watch","watch":created["record"]["watch"]})
        )?["ok"],
        true
    );
    Ok(())
...
fn map_schema_adds_only_its_route_and_does_not_widen_base_variants() -> Result<()> {
    ...
    let variants = out["query_schema"]["$defs"]["query"]["oneOf"]
        .as_array()
        .ok_or_else(|| error(ErrorCode::InternalInvariantViolation, "variants"))?;
    assert_eq!(variants.len(), 17);
    assert_eq!(json!(&variants[..16]), base["$defs"]["query"]["oneOf"]);
    assert_eq!(variants[16]["properties"]["kind"]["const"], "map_route");
    assert_eq!(s.calls.load(Ordering::SeqCst), 0);
    Ok(())
}
#[test]
fn source_failure_retains_previous_anchor_and_local_watch_management() -> Result<()> {
    ...
    let w = ask(
        &s,
        json!({"kind":"watch","key":"future","label":"Later observation","condition":{"op":"tick_at_least","value":105u64*403200+50},
        "deadline_tick":105u64*403200+100,"poll_interval_ticks":1,"stable_observations":1}),
    )?;
    assert_eq!(w["ok"], true);
    let observed = decode(&fortress_observe(s.handle()))?;
    assert_eq!(observed["ok"], true);
    let failed = decode(&fortress_observe(s.handle()))?;
    assert_eq!(failed["ok"], false);
    assert_eq!(failed["anchor"], observed["anchor"]);
    assert_eq!(failed["agent_turn"]["continuity"]["status"], "stale");
    assert_eq!(
        ask(&s, route([14, 15, 1], [14, 15, 2], 1))?["error"]["code"],
        "adapter_unavailable"
    );
    ...
    assert_eq!(
        decode(&fortress_doctor(s.handle()))?["status"],
        "source_fenced"
    );
    Ok(())
}
...
fn authority_region_and_malformed_routes_fail_without_io() -> Result<()> {
    ...
    let allowed = full()?;
    for q in [
        json!({"kind":"map_route","start":[0,0],"goal":[0,0,0]}),
        route([0, 0, 0], [14, 15, 2], 1),
        json!({"kind":"map_route","start":[14,15,1],"goal":[14,15,2],"max_work":1}),
        json!({"kind":"map_route","start":[14,15,1],"goal":[14,15,2],"reveal":true}),
    ] {
        assert_eq!(ask(&allowed, q)?["ok"], false);
    }
    ...
    assert!(!allowed_environment("DFMCP_OPERATIONS_TOKEN"));
    assert!(capabilities(Some(vec!["control_clock".to_owned()])).is_err());
    assert_eq!(
        decode(&fortress_commit(allowed.handle()))?["error"]["code"],
        "capability_denied"
    );
    assert_eq!(allowed.calls.load(Ordering::SeqCst), 0);
    Ok(())
}
