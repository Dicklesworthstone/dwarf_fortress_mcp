...
use dfmcp_adapter::live_jobs::{JobPublication, LiveJobObservation, LiveJobsState, MAX_JOBS, MAX_JOB_FRAME_BYTES};
...
fn lock<T>(value: &Mutex<T>) -> Result<MutexGuard<'_, T>> {
    value.lock().map_err(|_| error(ErrorCode::InternalInvariantViolation, "jobs session mutex is poisoned"))
}
struct SessionSlot;
impl SessionSlot {
    fn reserve() -> Result<Self> {
        SLOTS.fetch_update(Ordering::AcqRel, Ordering::Acquire,
            ...
            .map_err(|_| error(ErrorCode::BudgetExceeded, "jobs runtime session capacity reached"))?;
        Ok(Self)
    }
}
...
impl JobSession {
    fn anchor(&self) -> Result<StateAnchor> {
        self.state.snapshot().map(|snapshot| snapshot.anchor())
            .ok_or_else(|| error(ErrorCode::InternalInvariantViolation, "jobs session has no published snapshot"))
    }
        ...
        let result = self.source.read(Duration::from_millis(context.budget.max_wall_millis))
            .and_then(|observation| {
                if observation.jobs.len().saturating_add(1) > context.budget.max_entities as usize
                    ...
                    return Err(error(ErrorCode::BudgetExceeded, "job observation exceeds session budget"));
                }
...
fn next_id() -> Result<SessionId> {
    ...
    if *counter >= FAMILY { return Err(error(ErrorCode::BudgetExceeded, "jobs session IDs exhausted")); }
    let id = SessionId::new((1u128 << 127) | FAMILY | *counter);
    *counter += 1;
    Ok(id)
}
fn resolve(id: Option<String>) -> Result<Arc<Mutex<JobSession>>> {
    let raw = id.ok_or_else(|| error(ErrorCode::InvalidRequest, "open a jobs session and supply session_id"))?;
    if raw.len() != 32 || !raw.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        return Err(error(ErrorCode::InvalidRequest, "invalid jobs session handle"));
    }
...
fn allowed_environment(name: &str) -> bool {
    !name.starts_with("DFMCP_") || matches!(name,
...
fn tool_packet(session: Option<&JobSession>, context: Option<&OperationContext>, operation: &str,
    mut payload: Value) -> Result<String> {
    let mut work = empty_active_work();
    if let Some(value) = payload.as_object_mut().and_then(|object| object.remove("_condition_watch_work")) {
...
fn error_packet(session: Option<&JobSession>, context: Option<&OperationContext>, operation: &str, failure: &DfmcpError) -> String {
...
where F: FnOnce(&mut JobSession, OperationContext) -> Result<String> {
    let handle = match resolve(id) { Ok(value) => value, Err(failure) => return error_packet(None,None,operation,&failure) };
    let mut session = match lock(&handle) { Ok(value) => value, Err(failure) => return error_packet(None,None,operation,&failure) };
    let context = match session.context() { Ok(value) => value, Err(failure) => return error_packet(None,None,operation,&failure) };
    let result = context.authorize(capability, RiskTier::ReadOnly, &[], None)
        .and_then(|()| body(&mut session, context.clone()));
    match result { Ok(text) => text, Err(failure) => error_packet(Some(&session),Some(&context),operation,&failure) }
}
fn read_capabilities(requested: Option<Vec<String>>) -> Result<Vec<Capability>> {
    let requested = requested.unwrap_or_else(|| vec!["observe".to_owned(),"query".to_owned(),"doctor".to_owned()]);
    if requested.is_empty() || requested.len() > 3 { return Err(error(ErrorCode::CapabilityDenied,"request 1..3 read-only capabilities")); }
    let mut capabilities = Vec::new();
    for name in requested {
        let capability = match name.as_str() { "observe" => Capability::Observe,
            "query" => Capability::Query,"doctor" => Capability::Doctor,
            _ => return Err(error(ErrorCode::CapabilityDenied,"jobs runtime cannot grant that capability")) };
        if capabilities.contains(&capability) { return Err(error(ErrorCode::InvalidRequest,"duplicate capability")); }
        capabilities.push(capability);
    }
    Ok(capabilities)
}

#[tool(description = "Open an explicitly unadmitted, authenticated jobs-only DFHack session. Token and numeric loopback endpoint come only from process configuration. No mutation authority exists.")]
pub fn fortress_open_session(max_jobs: Option<u32>, max_output_tokens: Option<u32>, max_bytes: Option<u64>,
    max_wall_millis: Option<u64>, requested_capabilities: Option<Vec<String>>) -> String {
    let result = (|| -> Result<String> {
        ...
        let budget = WorkBudget { max_wall_millis:max_wall_millis.unwrap_or(5000), max_game_ticks:1_000_000,
            max_entities:maximum.saturating_add(1), max_bytes:max_bytes.unwrap_or(MAX_JOB_FRAME_BYTES as u64),
            max_output_tokens:max_output_tokens.unwrap_or(8192), max_actions:1 };
        if maximum == 0 || maximum as usize > MAX_JOBS || !(1..=60_000).contains(&budget.max_wall_millis)
            || !(8192..=MAX_JOB_FRAME_BYTES as u64).contains(&budget.max_bytes)
            || !(2048..=65_536).contains(&budget.max_output_tokens) {
            return Err(error(ErrorCode::BudgetExceeded,"jobs session budgets exceed their supported bounds"));
        }
        budget.validate()?;
        let slot = SessionSlot::reserve()?;
        let id = next_id()?;
        let endpoint = std::env::var("DFMCP_JOBS_ENDPOINT").unwrap_or_else(|_|"127.0.0.1:5000".to_owned());
        let endpoint = dfmcp_adapter::parse_loopback_endpoint(&endpoint)?;
        let token = std::env::var("DFMCP_JOBS_TOKEN").map_err(|_|error(ErrorCode::CapabilityDenied,"DFMCP_JOBS_TOKEN must be configured"))?;
        let mut source = JobsRpcClient::connect(endpoint,token.into_bytes(),id.get().to_be_bytes().to_vec(),
            Duration::from_millis(budget.max_wall_millis),maximum,budget.max_bytes as usize)?;
        let mut state = LiveJobsState::default();
        state.publish(source.refresh(Duration::from_millis(budget.max_wall_millis))?)?;
        let fortress = state.snapshot().ok_or_else(||error(ErrorCode::InternalInvariantViolation,"jobs bootstrap lost snapshot"))?.fortress_id;
        ...
        let mut session = JobSession { id, source:Box::new(source), state, budget, grants, request:0, _slot:slot };
        let context = session.context()?;
        let output = tool_packet(Some(&session),Some(&context),"fortress.open_session",json!({"ok":true,
        ...
        if registry.contains_key(&id) { return Err(error(ErrorCode::InternalInvariantViolation,"jobs session ID collision")); }
        registry.insert(id,Arc::new(Mutex::new(session)));
        Ok(output)
    })();
    match result { Ok(text)=>text,Err(failure)=>error_packet(None,None,"fortress.open_session",&failure) }
}
...
#[tool(description = "Perform one bounded jobs observation, without controlling time or claiming completion. For conditions use query kind await_watch.")]
...
fn query_view(session: &JobSession, context: &OperationContext) -> Result<QueryResponseProjection> {
    let source = session.state.observation().ok_or_else(||error(ErrorCode::InternalInvariantViolation,"jobs source missing"))?;
    Ok(QueryResponseProjection { session_id:session.id.to_string(),request_id:context.request_id.to_string(),
        anchor:anchor_json(session.anchor()?),briefing:briefing(session),attention:Vec::new(),affordances:Vec::new(),
        uncertainty:vec![json!({"domain":"fortress.jobs.blockers","epistemic_state":"unknown",
            "reason":"no material-availability, path, building-completion or labor-eligibility observation"})],
        coverage:coverage(),budget:json!({"admitted":{"max_bytes":session.budget.max_bytes,
            "max_output_tokens":session.budget.max_output_tokens}}),
        references:vec![json!({"kind":"jobs_observation","digest":source.source_digest()?.to_string()})],
...
fn finish_query(view: &QueryResponseProjection, result: Value) -> Result<String> {
    ...
    let mut payload: Value = serde_json::from_str(&encoded)
        .map_err(|_|error(ErrorCode::InternalInvariantViolation,"query response cannot be decoded"))?;
    payload["agent_turn"]["turn_id"] = json!(format!("jobs-turn-{}",view.request_id));
    let encoded = payload.to_string();
    if encoded.len()>view.maximum_bytes { return Err(error(ErrorCode::BudgetExceeded,"jobs query packet overflow")); }
    Ok(encoded)
}
#[tool(description = "Query current job facts, aggregates, search, baselines, or condition watches. Use mode=schema for the shared query envelope. This profile observes jobs only, not citizens or inventory.")]
pub fn fortress_query(session_id: Option<String>, mode: Option<String>, query: Option<Value>) -> String {
    with_session(session_id,"fortress.query",Capability::Query,|session,mut context| {
        if mode.is_some() && query.is_some() { return Err(error(ErrorCode::InvalidRequest,"do not mix mode and query")); }
        let schema = mode.as_deref()==Some("schema");
        if mode.is_some() && !schema && mode.as_deref()!=Some("summary") {
            return Err(error(ErrorCode::InvalidRequest,"jobs mode must be summary or schema"));
        }
        ...
        if session.source.poisoned() && !local && !schema {
            return Err(error(ErrorCode::AdapterUnavailable,"jobs source fenced; reopen or manage retained local records"));
        }
        let mut refreshed = None;
        if !schema && kind=="await_watch" {
            let snapshot = session.state.snapshot().ok_or_else(||error(ErrorCode::InternalInvariantViolation,"jobs snapshot missing"))?;
        ...
        if schema {
            let schema:Value=serde_json::from_str(include_str!("../../../schemas/mcp_query_v1.json"))
                .map_err(|_|error(ErrorCode::InternalInvariantViolation,"invalid embedded query schema"))?;
            return semantic_query::publish_with_active_work(&context,json!({"schema":schema,
                ...
                "truncated":false,"continuation":null}),|result|finish_query(&view,result));
        }
        let snapshot=session.state.snapshot().ok_or_else(||error(ErrorCode::InternalInvariantViolation,"jobs snapshot missing"))?;
        semantic_query::execute_with_publisher(snapshot,&narrowed,&input,|mut result| {
            result["source_stale"]=json!(session.source.poisoned());
            if let Some(refresh)=refreshed { result["observation_refresh"]=refresh; }
            finish_query(&view,result)
        })
    })
}
#[tool(description = "Explain jobs-profile coverage and provenance. Inspect a specific generation with fortress.query kind inspect.")]
pub fn fortress_explain(session_id: Option<String>) -> String {
    with_session(session_id,"fortress.explain",Capability::Query,|session,context| {
        tool_packet(Some(session),Some(&context),"fortress.explain",json!({"ok":true,
...
#[tool(description = "Report the jobs source connection, current anchor and explicit unadmitted coverage; performs no network repair or game effect.")]
pub fn fortress_doctor(session_id: Option<String>) -> String {
    with_session(session_id,"fortress.doctor",Capability::Doctor,|session,context| {
        tool_packet(Some(session),Some(&context),"fortress.doctor",json!({"ok":true,
...
#[tool(description = "Unavailable for game actions. Cancel a foreground condition using query kind cancel_watch.")]
...
mod tests {
    ...
    struct Script { values:VecDeque<LiveJobObservation>, poisoned:bool }
    impl JobSource for Script {
        fn read(&mut self,_:Duration)->Result<LiveJobObservation> {
            self.values.pop_front().ok_or_else(||error(ErrorCode::AdapterUnavailable,"fixture exhausted"))
        }
        fn poisoned(&self)->bool {self.poisoned}
        fn fence(&mut self){self.poisoned=true;}
    }
    fn observation(suspended:bool,tick:u32)->LiveJobObservation {
        LiveJobObservation {bridge_generation:7,df_version:"test-df".to_owned(),dfhack_version:"test-dfhack".to_owned(),
            year:105,year_tick:tick,paused:true,site_id:1,world_folder:"region1".to_owned(),next_job_id:4,
            jobs:vec![LiveJob {native_id:0,job_type:5,type_key:"Dig".to_owned(),reaction:String::new(),
                suspended,repeating:false,position:MapCoord::new(1,2,3),worker_native_id:None,
                holder_native_id:Some(4),completion_timer:-1,attached_item_count:0,required_item_filter_count:1}]}
    }
    fn register()->Result<SessionId> {
        let id=next_id()?;let mut state=LiveJobsState::default();state.publish(observation(true,1))?;
        let fortress=state.snapshot().ok_or_else(||error(ErrorCode::InternalInvariantViolation,"fixture"))?.fortress_id;
        let session=JobSession {id,source:Box::new(Script {values:VecDeque::from([observation(false,2)]),poisoned:false}),
            state,budget:WorkBudget {max_bytes:32768,max_output_tokens:8192,..WorkBudget::default()},
            grants:[Capability::Observe,Capability::Query,Capability::Doctor].into_iter().map(|capability|CapabilityGrant {
                ...
                max_risk:RiskTier::ReadOnly,expires_at_tick:None,remaining_uses:None}).collect(),request:0,_slot:SessionSlot::reserve()?};
        lock(&SESSIONS)?.insert(id,Arc::new(Mutex::new(session)));Ok(id)
    }
    fn decode(text:&str)->Result<Value> {serde_json::from_str(text).map_err(|_|error(ErrorCode::InternalInvariantViolation,"fixture JSON"))}
    #[test]
    fn actual_query_handlers_filter_refresh_and_keep_effects_unavailable()->Result<()> {
        ...
        assert_eq!(before["matched"],1);assert_eq!(before["agent_turn"]["briefing"]["runtime_admitted"],false);
        assert_eq!(decode(&fortress_observe(handle.clone()))?["ok"],true);
        assert_eq!(decode(&fortress_query(handle.clone(),None,Some(input)))?["matched"],0);
        assert_eq!(decode(&fortress_commit(handle.clone()))?["error"]["code"],"capability_denied");
        assert_eq!(decode(&fortress_observe(handle.clone()))?["ok"],false);
        assert_eq!(decode(&fortress_query(handle.clone(),None,None))?["ok"],false);
    ...
    fn baseline_changes_use_the_actual_native_job_projection()->Result<()> {
        let id=register()?;let handle=Some(id.to_string());
        let capture=decode(&fortress_query(handle.clone(),None,Some(json!({"schema":"dfmcp.query/1","query":{
