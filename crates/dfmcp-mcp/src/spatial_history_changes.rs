...
pub(in super::super) fn handles(input: &Value) -> bool {
    monitor::handles(input)
        || series::handles(input)
        || input
            .get("query")
            .and_then(|q| q.get("kind"))
            .and_then(Value::as_str)
            == Some("historical_changes")
}
pub(in super::super) fn schema() -> Result<Value> {
    serde_json::from_str(include_str!(
        "../../../schemas/mcp_historical_changes_v1.json"
    ))
    .map_err(|_| {
        error(
            ErrorCode::InternalInvariantViolation,
            "historical changes schema is invalid",
        )
    })
}
fn shape(input: &Value) -> Result<()> {
    ...
    while let Some((value, depth)) = pending.pop() {
        ...
        if nodes > 4096 || depth > 24 {
            return Err(bounded("historical changes input exceeds shape bounds"));
        }
        match value {
            ...
            Value::Array(values) => {
                if nodes
                    ...
                    > 4096
                {
                    return Err(bounded("historical changes array too wide"));
                }
            ...
            Value::Object(values) => {
                if nodes
                    ...
                    > 4096
                {
                    return Err(bounded("historical changes object too wide"));
                }
        ...
        if bytes > 65536 {
            return Err(bounded("historical changes input exceeds byte bound"));
        }
    }
    Ok(())
}
fn remaining(context: &OperationContext, started: Instant) -> Result<OperationContext> {
    ...
    if elapsed >= u128::from(context.budget.max_wall_millis) {
        return Err(bounded(
            "historical changes exhausted the shared replay/comparison deadline",
        ));
    }
...
fn selected(journal: &history::Journal, requested: &RecordRef) -> Result<JournalEntry> {
    if !(1..=4096).contains(&requested.record) {
        return Err(invalid("historical record must be 1..4096"));
    }
    let digest = Digest32::from_hex(&requested.record_digest)
        .filter(|d| d.to_string() == requested.record_digest)
        .ok_or_else(|| invalid("record_digest must be canonical lowercase SHA-256"))?;
    let index =
        usize::try_from(requested.record - 1).map_err(|_| invalid("record index overflow"))?;
    let entry = journal
        .entries()
        .get(index)
        .ok_or_else(|| error(ErrorCode::CursorGap, "comparison record is not retained"))?;
    if entry.record_digest != digest {
        return Err(error(
            ErrorCode::StaleAnchor,
            "comparison record digest differs from retained history",
        ));
    }
    Ok(entry.clone())
}
fn replay(
    journal: &mut history::Journal,
    entry: &JournalEntry,
    ...
    archive::validate_limits(
        state
            .observation_full()
            .ok_or_else(|| error(ErrorCode::CorruptLedger, "comparison source absent"))?,
        limits,
    )?;
    state
        .snapshot()
        .cloned()
        .ok_or_else(|| error(ErrorCode::CorruptLedger, "comparison projection absent"))
}
...
fn render(
    ...
    {
        return Err(bounded(
            "complete historical change packet exceeds output budget",
        ));
    }
    Ok(encoded)
}
...
pub(in super::super) fn execute(
    ...
    if monitor::handles(input) {
        return monitor::execute(session, context, input);
    }
    if series::handles(input) {
        return series::execute(session, context, input);
    }
    ...
    if session.anchor()? != context.anchor {
        return Err(error(
            ErrorCode::StaleAnchor,
            "comparison context is not the current session anchor",
        ));
    }
    let envelope: Envelope = serde_json::from_value(input.clone())
        .map_err(|_| invalid("invalid historical changes envelope"))?;
    if envelope.schema != "dfmcp.query/1" {
        return Err(invalid("historical changes requires dfmcp.query/1"));
    }
    if envelope
        .expected_anchor
        .as_ref()
        .is_some_and(|a| a != &anchor_json(context.anchor))
    {
        return Err(error(
            ErrorCode::StaleAnchor,
            "historical changes expected_anchor differs from current session",
        ));
    }
    let Request::HistoricalChanges {
        from,
        to,
        select,
        limit,
    ...
    if from.record > to.record {
        return Err(invalid(
            "historical comparison requires from.record <= to.record",
        ));
    }
    if limit.is_some_and(|n| n == 0 || n > 128 || n > context.budget.max_entities)
        || continuation.as_ref().is_some_and(|token| token.len() > 128)
    {
        return Err(invalid(
            "historical changes page arguments exceed their bounds",
        ));
    }
    let selector = select
        .as_object()
        .ok_or_else(|| invalid("select must be an entities selection"))?;
    if select["kind"] != "entities"
        || selector
            .keys()
            .any(|k| !matches!(k.as_str(), "kind" | "kinds" | "where" | "fields" | "order"))
    {
        return Err(invalid(
            "select supports only entities, kinds, where, fields and order; comparison owns pagination",
        ));
    }
    let journal = session.journal.as_mut().ok_or_else(|| {
        invalid("historical changes requires the operator-configured spatial/1.8 journal")
    })?;
    ...
    if first.anchor.cursor.epoch != last.anchor.cursor.epoch || last.anchor.tick < first.anchor.tick
    {
        return Err(error(
            ErrorCode::StaleAnchor,
            "comparison crosses an observation reset or regressed game clock",
        ));
    }
    ...
    if let Some(p) = projection.as_mut() {
        ...
        p.references = vec![
            json!({"kind":"verified_comparison_endpoints","journal_id":archive_id.to_string(),
    ...
    sample["basis"] = anchor_json(first.anchor);
    sample["anchor"] = anchor_json(last.anchor);
    sample["kind"] = json!("historical_changes");
    sample["change_count"] = json!(u64::MAX);
    sample["returned"] = json!(u64::MAX);
    sample["truncated"] = json!(true);
    sample["continuation"] = json!("x".repeat(128));
    sample["basis_result_digest"] = json!("f".repeat(64));
    sample["target_result_digest"] = json!("f".repeat(64));
    let overhead = render(session, context, &last, projection.as_ref(), sample)?.len() as u64;
    let mut comparison = remaining(context, started)?;
    comparison.budget.max_bytes = context
        .budget
        .max_bytes
        .min(u64::from(context.budget.max_output_tokens) * 4)
        .checked_sub(overhead + 64)
        .filter(|n| *n > 0)
        .ok_or_else(|| bounded("archive evidence leaves no comparison result budget"))?;
    if projection.is_some() {
        comparison = semantic_query::result_context(&comparison)?;
    }
    ...
    let journal = session
        .journal
        .as_mut()
        .ok_or_else(|| invalid("comparison journal disappeared"))?;
    ...
    let after = if first.number == last.number {
        before.clone()
    } else {
        replay(journal, &last, &read_context, limits)?
    };
    comparison.budget.max_wall_millis = remaining(context, started)?.budget.max_wall_millis;
    let mut value = semantic_query::compare_endpoints(
        &before,
        &after,
        &comparison,
        &select,
    ...
    if journal.id() != archive_id || journal.head() != archive_head {
        return Err(error(
            ErrorCode::StaleAnchor,
            "comparison archive head changed",
        ));
    }
    let result = value.as_object_mut().ok_or_else(|| {
        error(
            ErrorCode::InternalInvariantViolation,
            "comparison result is not an object",
        )
    })?;
    if let Some(fields) = metadata.as_object() {
        result.extend(fields.iter().map(|(k, v)| (k.clone(), v.clone())));
    }
    remaining(context, started)?;
    if projection.is_some() {
