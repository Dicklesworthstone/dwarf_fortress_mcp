...
fn cursors_bind_session_journal_head_filter_and_limit_and_support_replay() -> Result<()> {
    ...
    assert_eq!(
        cursors.resolve(&token, id, &s, Filter::Pending, 8)?,
        "job-001"
    );
    ...
    assert_eq!(
        cursors.issue(id, &s, "job-001".into(), Filter::Pending, 8),
        token
    );
    assert!(
        cursors
            .resolve(&token, SessionId::new(10), &s, Filter::Pending, 8)
            .is_err()
    );
    assert!(cursors.resolve(&token, id, &s, Filter::All, 8).is_err());
    assert!(cursors.resolve(&token, id, &s, Filter::Pending, 1).is_err());
    let mut changed = s.clone();
    changed.journal_id = Digest32::ZERO;
    assert!(
        cursors
            .resolve(&token, id, &changed, Filter::Pending, 8)
            .is_err()
    );
    changed = s;
    changed.head = Digest32::ZERO;
    assert!(
        matches!(cursors.resolve(&token, id, &changed, Filter::Pending, 8),
...
fn unissued_tokens_and_expired_cursors_never_become_offsets() -> Result<()> {
    ...
    assert!(
        cursors
            .resolve(&Digest32::ZERO.to_string(), id, &s, Filter::All, 8)
            .is_err()
    );
    let first = cursors.issue(id, &s, "job-000".into(), Filter::All, 8);
    for number in 1..=64 {
        cursors.issue(id, &s, format!("job-{number:03}"), Filter::All, 8);
    }
    assert_eq!(cursors.issued.len(), 64);
    assert_eq!(cursors.order.len(), 64);
...
fn pending_filter_includes_preparations_but_excludes_cancelled_and_terminal_effects() -> Result<()>
{
    ...
    for state in [
        ...
        assert!(!Filter::Pending.matches(state));
        assert!(Filter::All.matches(state));
    }
    assert_eq!(Filter::Pending.total(&summary()), 2);
    assert!(Filter::parse("unknown-filter").is_err());
    Ok(())
}
...
fn error_turn_preserves_active_work_and_never_claims_admission_or_live_anchor() -> Result<()> {
    ...
    let out = packet(
        "fortress.commit",
        failure(&error(ErrorCode::BudgetExceeded, "test"), "fortress.commit"),
        TurnView {
            context: Some(&c),
            mode: "control",
            summary: Some(&s),
    ...
    let value: Value = serde_json::from_str(&out)
        .map_err(|_| error(ErrorCode::InternalInvariantViolation, "invalid JSON"))?;
    assert_eq!(value["agent_turn"]["schema"], "dfmcp.agent_turn/1");
    assert_eq!(
        value["agent_turn"]["active_work"]["counts"]["reconciliation_required"],
        1
    );
    ...
    assert_eq!(
        value["agent_turn"]["briefing"]["development_mutation_enabled"],
        false
    );
    ...
    assert!((out.len() as u64) < BASE_RESERVE);
    Ok(())
}
...
fn closing_turn_redirects_recovery_to_new_offline_session_without_empty_work_claim() -> Result<()> {
    ...
    assert_eq!(value["agent_turn"]["active_work"]["details_omitted"], true);
    assert_eq!(
        value["agent_turn"]["active_work"]["discovery"]["tool"],
