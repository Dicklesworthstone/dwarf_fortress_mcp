...
use dfmcp_adapter::work_orders::{
    MAX_NATIVE_TICK, WorkOrderEffect, WorkOrderObservation, WorkOrderPlan, WorkOrderRecipe,
    WorkOrderState,
};
...
impl Seek for Memory {
    fn seek(&mut self, from: SeekFrom) -> io::Result<u64> {
        self.bytes.borrow_mut().seek(from)
    }
}
...
fn invalid(text: &str) -> dfmcp_core::DfmcpError {
    ...
    raw.as_bytes()
        ...
        .map(|pair| {
            let a = char::from(pair[0])
                .to_digit(16)
                .ok_or_else(|| invalid("bad fixture"))?;
...
fn observation() -> Result<WorkOrderObservation> {
    WorkOrderObservation::decode(&decode_hex(include_str!(
        "../../dfmcp-adapter/tests/fixtures/work_order_observation_v1_10.hex"
    ))?)
}
fn after(plan: &WorkOrderPlan) -> Result<WorkOrderObservation> {
    ...
    bytes[count_offset..count_offset + 4]
        .copy_from_slice(&((before.order_ids().len() + 1) as u32).to_be_bytes());
...
fn native_effect(plan: &WorkOrderPlan, state: WorkOrderState) -> Result<WorkOrderEffect> {
    ...
    bytes.push(plan.spec().recipe() as u8);
    bytes.extend_from_slice(&plan.spec().amount().to_be_bytes());
    ...
    let (after_hash, config_hash) = if known {
        ...
        config.push(plan.spec().recipe() as u8);
        config.extend_from_slice(&plan.spec().amount().to_be_bytes());
        (after(plan)?.witness(), Digest32::of_bytes(&config))
    } else {
        (Digest32::ZERO, Digest32::ZERO)
    };
    bytes.extend_from_slice(after_hash.as_bytes());
    bytes.extend_from_slice(config_hash.as_bytes());
    let mut key = (plan.key().len() as u16).to_be_bytes().to_vec();
    key.extend_from_slice(plan.key().as_bytes());
    let receipt = if state.terminal() {
        let mut data = b"dfmcp-work-order-receipt/1\0".to_vec();
        data.extend_from_slice(&o.generation().to_be_bytes());
        data.extend_from_slice(&key);
        data.extend_from_slice(&bytes[73..195]);
...
struct Native {
    ...
    missing_record: bool,
    fail_read: bool,
    retained: BTreeMap<String, WorkOrderEffect>,
}
    ...
    fn ready(&self) -> Result<()> {
        if self.fenced {
            Err(error(
                ...
                "test source fenced; reopen explicitly",
            ))
        } else {
            Ok(())
        }
...
impl WorkOrderQuerySource for Source {
    ...
    fn query(&mut self, p: &WorkOrderPlan, _: Duration) -> Result<Option<WorkOrderEffect>> {
        ...
        let mut n = self.native.borrow_mut();
        n.queries += 1;
        Ok(if n.missing_record {
            None
        } else {
            n.retained.get(p.key()).cloned()
        })
    }
}
impl WorkOrderSource for Source {
    fn prepare(&mut self, p: &WorkOrderPlan, _: Duration) -> Result<WorkOrderEffect> {
        ...
        n.retained.insert(p.key().to_owned(), effect.clone());
        Ok(effect)
    }
    fn commit(
        &mut self,
        ...
        n.observation = after(p)?;
        n.retained.insert(p.key().to_owned(), effect.clone());
        if n.lost_reply {
            Err(error(
                ErrorCode::EffectIndeterminate,
                "created, but reply lost",
            ))
        } else {
            Ok(effect)
        }
...
impl Fixture {
    ...
    fn open(
        ...
        Ok(State {
            ...
            grants,
            mode,
            control: WorkOrderSession::new(journal, source, &c)?,
        ...
        assert!(
            out.len() as u64
                <= self
                    ...
                    .budget
                    .max_bytes
                    .min(u64::from(self.state.budget.max_output_tokens) * 4)
        );
        let value: Value =
            serde_json::from_str(&out).map_err(|_| invalid("invalid rendered JSON"))?;
    ...
    fn prepare_spec(&mut self, key: &str, spec: WorkOrderSpec) -> Result<CreationRecord> {
        ...
        let plan = digest(
            result["result"]["effect"]["plan_digest"]
                .as_str()
    ...
    fn commit_action(record: &CreationRecord) -> Action {
        Action::Commit {
            key: record.plan().key().into(),
            plan: record.plan().digest(),
            witness: record.plan().observation().witness(),
        }
    }
}
...
fn actual_handler_loop_seals_discovers_commits_and_replays_once() -> Result<()> {
    ...
    let r = f.prepare("order-001")?;
    let explained = f.call(
        "fortress.explain",
        Action::Explain {
            key: r.plan().key().into(),
            plan: r.plan().digest(),
    ...
    assert_eq!(
        done["result"]["effect"]["native_effect_hex"],
        include_str!("../../dfmcp-adapter/tests/fixtures/work_order_created_v1_10.hex").trim()
    );
    let replay = f.call("fortress.commit", Fixture::commit_action(&r), 1)?;
    assert_eq!(replay["result"]["effect"], done["result"]["effect"]);
    assert_eq!(f.native.borrow().commits, 1);
    assert_eq!(f.native.borrow().prepares, 1);
    Ok(())
}
...
fn lost_reply_is_visible_as_active_work_and_query_recovers_without_production() -> Result<()> {
    ...
    assert_eq!(
        uncertain["agent_turn"]["active_work"]["unresolved_count"],
        1
    );
    let blocked = f.call(
        "fortress.plan",
        Action::Plan {
            ...
            spec: r.plan().spec(),
            witness: r.plan().observation().witness(),
        },
        1,
    )?;
    assert_eq!(blocked["result"]["error"]["code"], "effect_indeterminate");
    let fenced = f.call_with(
        "fortress.wait",
        Action::Wait {
            key: r.plan().key().into(),
            plan: r.plan().digest(),
    ...
    assert_eq!(
        recovered["agent_turn"]["briefing"]["development_production_granted"],
        false
    );
    assert_eq!(f.native.borrow().commits, 1);
    assert_eq!(f.native.borrow().queries, 1);
    Ok(())
}
...
fn restart_offline_discovers_uncertainty_and_connected_query_mode_cannot_promote() -> Result<()> {
    ...
    assert_eq!(blocked["result"]["error"]["code"], "capability_denied");
    assert_eq!(f.native.borrow().queries, 0);
    ...
    assert_eq!(done["result"]["effect"]["state"], "created");
    assert_eq!(f.native.borrow().commits, 1);
    Ok(())
}
...
fn cancelled_preparation_is_retained_and_wait_never_queries_it() -> Result<()> {
    let mut f = Fixture::new()?;
    let r = f.prepare("cancelled")?;
    let cancelled = f.call(
        "fortress.cancel",
        Action::Cancel {
            key: r.plan().key().into(),
            plan: r.plan().digest(),
...
{
    let mut f = Fixture::new()?;
    let r = f.prepare("budget")?;
    let before = f.store.bytes.borrow().get_ref().clone();
    for limits in [
        Limits {
            bytes: Some(BASE_RESERVE + RECORD_RESERVE),
            ..Limits::default()
        },
        Limits {
            tokens: Some(1),
        ...
        let refused = f.call_with(
            "fortress.commit",
            Fixture::commit_action(&r),
            1,
            limits,
            true,
            false,
    ...
    assert_eq!(
        f.call("fortress.commit", Fixture::commit_action(&r), 1)?["result"]["creation_verified"],
        true
    );
    Ok(())
}
...
fn current_grants_cancellation_and_exact_identity_are_rechecked_before_commit() -> Result<()> {
    let mut f = Fixture::new()?;
    let r = f.prepare("authority")?;
    let denied = f.call_with(
        "fortress.commit",
        Fixture::commit_action(&r),
        1,
        Limits::default(),
...
fn filtered_pages_advance_without_false_absence_and_reject_stale_or_rebound_cursors() -> Result<()>
{
    ...
    assert_eq!(first["result"]["records"], json!([]));
    assert_eq!(first["result"]["matching_records_in_journal"], 1);
    assert_eq!(
        first["result"]["complete_matching_set_in_this_response"],
        false
    );
    ...
    f.call(
        "fortress.cancel",
        Action::Cancel {
            key: pending.plan().key().into(),
            plan: pending.plan().digest(),
        },
        1,
    )?;
    let stale = f.call(
...
fn largest_complete_queue_and_record_fit_the_reserved_final_response() -> Result<()> {
    ...
    let result = f.call_with(
        ...
        1,
        Limits {
            tokens: Some(((BASE_RESERVE + RECORD_RESERVE) / 4) as u32),
            ..Limits::default()
        },
        true,
        false,
    ...
    assert_eq!(
        result["result"]["effect"]["original_observation"]["order_ids"]
            .as_array()
            .map(Vec::len),
        Some(4095)
    );
...
#[test]
fn runtime_io_preserves_inherited_restrictions_and_cancellation()
-> std::result::Result<(), Box<dyn std::error::Error>> {
    use asupersync::{cx::cap, types::CancelKind};
