...
fn io_error(_: std::io::Error) -> DfmcpError {
    error(ErrorCode::CorruptLedger, "monitor fixture I/O")
}
struct Files {
    directory: PathBuf,
    observation: PathBuf,
...
impl Files {
    fn new() -> Result<Self> {
        ...
        fs::DirBuilder::new()
            ...
            .map_err(io_error)?;
        Ok(Self {
            observation: directory.join("observations.bin"),
            watch: directory.join("watches.bin"),
            directory,
        })
    }
}
impl Drop for Files {
    fn drop(&mut self) {
        let _ = fs::remove_file(&self.watch);
        let _ = fs::remove_file(&self.observation);
...
impl Source for NoRead {
    fn read(&mut self, _: Duration) -> Result<LiveSpatialCitizenObservation> {
        ...
        Err(error(
            ...
            "historical analysis must not enter source",
        ))
    }
...
impl Registered {
    ...
    fn query(&self, input: Value) -> Result<Value> {
        serde_json::from_str(&fortress_query(
    ...
    fn current(&self) -> Result<StateAnchor> {
        lock(&self.session)?.anchor()
    }
}
impl Drop for Registered {
    fn drop(&mut self) {
        let _ = fortress_cancel(
            Some(self.id.to_string()),
            Some("session".into()),
            Some(true),
        );
...
fn live(files: &Files) -> Result<Registered> {
    ...
    let c = s.context()?;
    history::attach(&mut s, &files.observation, TailRecovery::Refuse, &c)?;
    for tick in 4..=6 {
        let c = s.context()?;
        let read = history::replay_context(&s, &c);
        let journal = s
            .journal
            .as_mut()
            .ok_or_else(|| invalid("fixture journal"))?;
...
fn request(session: &Registered, stable: u32, details: bool) -> Result<Value> {
    ...
    Ok(
        json!({"schema":"dfmcp.query/1","query":{"kind":"historical_watch_replay",
...
fn bytes(files: &Files) -> Result<(Vec<u8>, Vec<u8>)> {
    Ok((
        fs::read(&files.observation).map_err(io_error)?,
        fs::read(&files.watch).map_err(io_error)?,
    ))
}
...
fn live_handler_keeps_paired_watches_unchanged_and_accepts_fenced_sources() -> Result<()> {
    ...
    assert_eq!(result["kind"], "historical_watch_replay");
    assert_eq!(result["status"], "satisfied");
    assert_eq!(result["first_terminal_record"], 2);
    assert_eq!(result["records_verified"], 4);
    assert_eq!(result["records_after_terminal"], 2);
    ...
    let work = result["agent_turn"]["active_work"]["obligations"]
        .as_array()
        .ok_or_else(|| invalid("active work missing"))?;
    assert!(work.iter().any(
        |w| w["watch"] == watch && w["evidence_digest"] == created["record"]["evidence_digest"]
    ));
    assert_eq!(bytes(&files)?, before);
    assert_eq!(live.current()?, current);
    assert_eq!(live.calls.load(Ordering::SeqCst), 0);
    let mut s = lock(&live.session)?;
    let c = s.context()?;
    let input = request_without_lock(&result)?;
    // Deliberately tiny final output cannot commit even a monitoring transition.
    let mut narrow = c;
    narrow.budget.max_bytes = 1;
    assert!(execute(&mut s, &narrow, &input).is_err());
    drop(s);
    assert_eq!(bytes(&files)?, before);
    Ok(())
}
fn request_without_lock(result: &Value) -> Result<Value> {
    let from = result
        .pointer("/range/from")
        .ok_or_else(|| invalid("range from"))?;
    let to = result
        .pointer("/range/to")
        .ok_or_else(|| invalid("range to"))?;
    Ok(
        json!({"schema":"dfmcp.query/1","query":{"kind":"historical_watch_replay",
...
fn archive_reopen_reproduces_identity_without_observe_or_loading_watch_journal() -> Result<()> {
    ...
    assert_eq!(result["status"], "satisfied");
    assert_eq!(result["evidence_digest"], expected["evidence_digest"]);
    assert_eq!(result["replay_id"], expected["replay_id"]);
    ...
    assert!(
        schema["query_schema"]["$defs"]["query"]["oneOf"]
            .as_array()
            .ok_or_else(|| invalid("schema variants"))?
            .iter()
            .any(|v| v["properties"]["kind"]["const"] == "historical_watch_replay")
    );
...
fn exact_references_current_grants_and_range_bounds_refuse_without_writes() -> Result<()> {
    ...
    }
    {
        ...
        for grant in &mut s.grants {
            grant.expires_at_tick = Some(GameTick(current.tick.0 - 1));
        }
    }
    assert_eq!(live.query(input)?["ok"], false);
    assert_eq!(bytes(&files)?, before);
    assert_eq!(live.calls.load(Ordering::SeqCst), 0);
    Ok(())
}
...
fn corruption_after_first_terminal_sample_still_refuses_the_complete_replay() -> Result<()> {
    ...
    byte[0] ^= 1;
    file.seek(SeekFrom::Start(offset)).map_err(io_error)?;
    file.write_all(&byte).map_err(io_error)?;
    let before = bytes(&files)?;
...
fn summary_fits_small_packet_or_refuses_whole_evidence_without_truncation() -> Result<()> {
    ...
    assert!(result.get("evaluation").is_none());
...
fn schema_discovery_and_failure_guard_are_wired_through_the_live_handler() -> Result<()> {
    ...
    assert!(
        discovery["query_schema"]["$defs"]["query"]["oneOf"]
            .as_array()
            .ok_or_else(|| invalid("schema variants"))?
            .iter()
            .any(|v| v["properties"]["kind"]["const"] == "historical_watch_replay")
    ...
    assert_eq!(result["first_terminal_record"], 1);
    assert_eq!(result["records_after_terminal"], 3);
    assert_eq!(live.calls.load(Ordering::SeqCst), 0);
    Ok(())
}
