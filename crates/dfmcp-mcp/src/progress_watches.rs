...
use dfmcp_adapter::work_order_progress::watches::{
    MAX_WATCH_KEY, RetainedWatch, WatchBook, WatchBookSummary, WatchDefinition, WatchEvaluation,
    WatchGoal, WatchRecordRef, WatchSpec,
};
...
enum WireRequest {
    ...
    Register {
        ...
        deadline_game_tick: u64,
        cadence_game_ticks: u64,
        stable_samples: u8,
        origin_number: u64,
...
impl Request {
    pub(super) fn parse(raw: &str) -> Result<Self> {
        if raw.is_empty() || raw.len() > 2048 || raw.contains('\0') {
            return Err(error(
                ErrorCode::InvalidRequest,
                "watch request must be 1..2048 UTF-8 bytes without NUL",
            ));
        }
        let request: WireRequest = serde_json::from_str(raw).map_err(|_| {
            error(
                ErrorCode::InvalidRequest,
                "invalid closed progress-watch request",
            )
        })?;
        match request {
            WireRequest::Register {
                ...
                let goal = match (goal, threshold) {
                    (Goal::Validated, None) => WatchGoal::Validated,
                    (Goal::Active, None) => WatchGoal::Active,
                    (Goal::RemainingAtMost, Some(n)) => WatchGoal::RemainingAtMost(n),
                    _ => {
                        return Err(error(
                            ErrorCode::InvalidRequest,
                            "threshold belongs only to remaining_at_most",
                        ));
                    }
                };
                if !(1..=4096).contains(&origin_number) {
                    return Err(error(
                        ErrorCode::InvalidRequest,
                        "invalid watch origin number",
                    ));
                }
                Ok(Self::Register {
                    archive: parse_digest(&archive_id)?,
                    spec: WatchSpec::new(
                        &key,
                        native_order_id,
                        goal,
                    ...
                    origin: WatchRecordRef {
                        ...
                        digest: parse_digest(&origin_digest)?,
                    },
                })
            }
            ...
            WireRequest::Status {
                ...
                Ok(Self::Status {
                    archive: parse_digest(&archive_id)?,
                    key: raw,
                    digest: parse_digest(&definition_digest)?,
                })
            }
            WireRequest::Cancel {
                archive_id,
                ...
                Ok(Self::Cancel {
                    ...
                    key: raw,
                    digest: parse_digest(&definition_digest)?,
                    head: parse_digest(&expected_archive_head)?,
                })
            }
        }
    }
    fn archive(&self) -> Option<Digest32> {
        match self {
            Self::List => None,
            Self::Register { archive, .. }
            | Self::Status { archive, .. }
            | Self::Cancel { archive, .. } => Some(*archive),
        }
    }
}
fn reference(value: WatchRecordRef) -> Value {
    json!({"number":value.number,"record_digest":value.digest.to_string()})
}
fn definition(value: &WatchDefinition) -> Value {
    let spec = value.spec();
    json!({"key":spec.key(),"definition_digest":value.digest().to_string(),
...
fn retained(value: &RetainedWatch) -> Value {
    json!({"definition":definition(&value.definition),
    "cancelled_at":value.cancelled_at.map(reference),"outcome_evaluated":false})
}
fn evaluation(value: &WatchEvaluation) -> Value {
    json!({"definition":definition(value.definition()),
...
pub(super) fn query<S: JournalStorage, A: JournalStorage>(
    book: &mut WatchBook<S>,
    archive: &mut ProgressArchive<A>,
    request: Request,
    c: &OperationContext,
) -> Result<Value> {
    let a = archive.summary(c)?;
    if request.archive().is_some_and(|id| id != a.archive_id) {
        return Err(error(
            ErrorCode::StaleAnchor,
            "watch request belongs to another archive",
        ));
    }
    let result = match request {
        ...
        Request::Status { key, digest, .. } => {
            ...
            let value = batch
                .results
                .iter()
                .find(|e| e.definition().spec().key() == key)
                .ok_or_else(|| {
                    error(
                        ErrorCode::CorruptLedger,
                        "watch evaluation omitted retained intent",
                    )
                })?;
    ...
    Ok(
        json!({"ok":true,"watch_result":result,"native_calls":0,"game_mutation_dispatched":false,
...
pub(super) fn attach(packet: &mut Value, book: Option<&WatchBookSummary>, id: SessionId) {
    let value = match book {
        Some(b) => {
            json!({"configured":true,"book_id":b.book_id.to_string(),"book_head":b.head.to_string(),
        ...
        None => {
            json!({"configured":false,"outcomes_evaluated":false,"pending_absence_proven":false,
            "scope":"No watch book was selected; persisted monitors elsewhere were not examined."})
        }
    };
    packet["agent_turn"]["active_work"]["scope"] = json!(
        "Configured progress-watch definitions are listed separately; game effects and canonical obligations in other profiles were not examined."
    );
