...
fn close(s: &Registered) -> Result<()> {
    ok(decode(&fortress_cancel(s.handle(),Some("session".into()),None))?)?; Ok(())
}
...
fn partition(tick: u32, units: &[u32]) -> Result<LiveSpatialCitizenObservation> {
    ...
    let prototype=operations.items.iter().find(|item|item.native_id==32).cloned()
    ...
    for (index,units) in units.iter().enumerate() {
        let mut item=prototype.clone();item.native_id=100+index as u32;item.stack_size=*units;
    ...
    let length=original.get(8..12).and_then(|bytes|<[u8;4]>::try_from(bytes).ok())
        .map(u32::from_be_bytes).ok_or_else(||error(ErrorCode::InvalidRequest,"quantity fixture framing"))? as usize;
    let citizens=original.get(12+length..).ok_or_else(||error(ErrorCode::InvalidRequest,"quantity fixture citizens"))?;
    let mut combined=b"DFMS1800".to_vec();part(&mut combined,&spatial);combined.extend_from_slice(citizens);
...
fn quantity_inspection_is_complete_and_does_not_sample_or_persist_watch_progress() -> Result<()> {
    ...
    let raw=fortress_query(s.handle(),None,Some(json!({"schema":"dfmcp.query/1","query":quantity()})));
    assert!(raw.len()<=16384);let result=ok(decode(&raw)?)?;
    assert_eq!(result["quantity"]["quantity_min"],7);assert_eq!(result["quantity"]["quantity_max"],7);
    assert_eq!(result["quantity"]["matched_records_min"],1);assert_eq!(result["quantity"]["quantity_exact"],true);
    assert_eq!(result["watch_registered"],false);assert_eq!(result["watch_evaluated"],false);
    assert_eq!(result["quantity"]["usable_supply_proven"],false);
    assert_eq!(result["quantity"]["quantity_min"],created["record"]["evaluation"]["facts"][0]["quantity_min"]);
    assert_eq!(ok(ask(&s,json!({"kind":"watches"}))?)?["records"],records);
    assert_eq!(fs::read(&files.watches).map_err(io_error)?,bytes);assert_eq!(s.calls.load(Ordering::SeqCst),0);
    close(&s)
}

#[test]
fn splitting_and_merging_stacks_preserves_quantity_stability_in_a_shared_capture_batch() -> Result<()> {
    ...
    ok(ask(&s,json!({"kind":"watch","key":"records","condition":{"op":"entity_count",
    ...
    for (step,stacks) in [(1,2),(2,1)] {
        ...
        assert_eq!(result["native_captures"],1);assert_eq!(result["all_satisfied"],step==2);
        for record in result["records"].as_array().ok_or_else(||error(ErrorCode::InvalidRequest,"batch records"))? {
            assert_eq!(record["stable_observations"],step+1);
        }
        let measured=ok(ask(&s,quantity())?)?;
        assert_eq!(measured["quantity"]["quantity_min"],7);assert_eq!(measured["quantity"]["matched_records_min"],stacks);
    }
    assert_eq!(s.calls.load(Ordering::SeqCst),2);close(&s)
}
...
fn quantity_definitions_recover_without_counting_restart_as_a_successful_sample() -> Result<()> {
    ...
    let before=fs::read(&files.watches).map_err(io_error)?;close(&s)?;
    assert_eq!(fs::read(&files.watches).map_err(io_error)?,before);
    let next=register(&files,1,vec![fixture::observation(4,1,7,false)?,fixture::observation(5,1,7,false)?])?;
    let records=ok(ask(&next,json!({"kind":"watches"}))?)?["records"].clone();
    assert_ne!(records[0]["watch"],old);assert_eq!(records[0]["stable_observations"],0);
    assert_eq!(records[0]["fresh_observation_required"],true);
    let record=ok(ask(&next,json!({"kind":"poll_watch","watch":records[0]["watch"]}))?)?;
    assert_eq!(record["record"]["definition"]["condition"]["op"],"item_quantity");
    ok(ask(&next,quantity())?)?;
    assert_eq!(ok(ask(&next,json!({"kind":"watches"}))?)?["records"],records);
    for step in 1..=2 {
    ...
    assert_eq!(next.calls.load(Ordering::SeqCst),2);close(&next)
}
...
fn historical_and_offline_quantity_queries_remain_on_the_exact_record() -> Result<()> {
    ...
    let first=first_record(&s)?;retain_watch(&s)?;
    ok(decode(&fortress_observe(s.handle()))?)?;
    let before=fs::read(&files.watches).map_err(io_error)?;
    let current=ok(ask(&s,quantity())?)?;assert_eq!(current["quantity"]["quantity_min"],12);
    let old=ok(ask(&s,historical(&first,quantity()))?)?;
    assert_eq!(old["quantity"]["quantity_min"],7);assert_eq!(old["anchor"],first["anchor"]);
    assert_eq!(old["historical"],true);assert_eq!(old["watch_evaluated"],false);
    assert_eq!(ok(ask(&s,quantity())?)?["anchor"],current["anchor"]);
    assert_eq!(fs::read(&files.watches).map_err(io_error)?,before);
    let (limits,budget)={let handle=resolve(s.handle())?;let session=lock(&handle)?;(session.limits,session.budget)};
    close(&s)?;
    ...
    let offline=Registered {id,calls:Arc::new(AtomicUsize::new(0))};
    let latest=ok(ask(&offline,quantity())?)?;
    assert_eq!(latest["quantity"]["quantity_min"],12);assert_eq!(latest["current_freshness_proven"],false);
    assert_eq!(latest["bridge_connection_present"],false);
    assert_eq!(ok(ask(&offline,historical(&first,quantity()))?)?["quantity"]["quantity_min"],7);
    assert_eq!(ask(&offline,watch_quantity("no-monitoring",2))?["ok"],false);
    close(&offline)?;
...
fn quantity_schema_is_composed_once_and_rejected_results_leave_monitoring_intact() -> Result<()> {
    ...
    let conditions=definitions["watch_condition"]["oneOf"].as_array().ok_or_else(||error(ErrorCode::InvalidRequest,"conditions"))?;
    for op in ["entity_count","item_quantity"] {
        assert_eq!(conditions.iter().filter(|v|v["properties"]["op"]["const"]==op).count(),1);
    }
    let queries=definitions["query"]["oneOf"].as_array().ok_or_else(||error(ErrorCode::InvalidRequest,"queries"))?;
    assert_eq!(queries.iter().filter(|v|v["properties"]["kind"]["const"]=="item_quantity").count(),1);
    ok(ask(&s,watch_quantity("stock",3))?)?;
    let bytes=fs::read(&files.watches).map_err(io_error)?;
    {let handle=resolve(s.handle())?;lock(&handle)?.budget.max_output_tokens=1;}
    ...
    let mut wrong=quantity();wrong["quantity_unit"]=json!("food_portions");
