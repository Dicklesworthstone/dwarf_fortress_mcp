...
fn check(context: &OperationContext, started: Instant) -> Result<()> {
    ...
    if started.elapsed().as_millis() >= u128::from(context.budget.max_wall_millis) {
        return Err(bounded(
            "watch batch exhausted its foreground evaluation budget",
        ));
    }
    Ok(())
}
...
fn record<'a>(store: &'a Store, session: SessionId, handle: &str) -> Result<&'a Watch> {
    store
        .entries
        .get(&(session, handle.to_owned()))
        .ok_or_else(|| invalid("batch watch is not retained by this session"))
}
fn output(
    before: &Store,
    after: &Store,
    ...
    for handle in &prepared.selected {
        ...
        advanced += usize::from(changed);
        sampled += usize::from(sample_added);
        terminal += usize::from(current.status.terminal());
        satisfied += usize::from(current.status == Status::Satisfied);
        ...
        row["condition"] = current
            .evaluation
            .get("condition")
            .cloned()
            .unwrap_or(Value::Null);
        row["failure_condition"] = current
            .evaluation
            .get("failure_condition")
            .cloned()
            .unwrap_or(Value::Null);
        row["advanced_in_batch"] = json!(changed);
        row["sample_added"] = json!(sample_added);
    ...
    value["selected"] = json!(prepared.selected.len());
    value["advanced"] = json!(advanced);
    value["sampled"] = json!(sampled);
    value["terminal"] = json!(terminal);
    value["remaining"] = json!(prepared.selected.len() - terminal);
    value["all_terminal"] = json!(terminal == prepared.selected.len());
    value["all_satisfied"] =
        json!(!prepared.selected.is_empty() && satisfied == prepared.selected.len());
    ...
    F: FnOnce(Value) -> Result<String>,
{
    publish_work(store, context, value, |mut value| {
        ...
        if value["watch_persistence"]["durable"] == true {
    ...
    F: FnOnce(Value) -> Result<String>,
{
    ...
    let request: BatchEnvelope = serde_json::from_value(input.clone())
        .map_err(|_| invalid("invalid watch batch envelope"))?;
    if request.schema != "dfmcp.query/1" {
        return Err(invalid("watch batch requires dfmcp.query/1"));
    }
    if request
        .expected_anchor
        .as_ref()
        .is_some_and(|a| a != &anchor(context.anchor))
    {
        return Err(failure(
            ...
            "watch batch expected_anchor differs before acquisition",
        ));
    }
    ...
    let mut selected = match selection {
        Some(handles) => {
            if handles.is_empty() || handles.len() > MAX_PER_SESSION {
                return Err(invalid(
                    "select one to eight watch handles, or omit watches to select the session set",
                ));
            }
            handles
        }
        None => store
            .entries
            .keys()
            .filter(|(id, _)| *id == context.session_id)
            .map(|(_, handle)| handle.clone())
            .collect(),
    };
    if selected.len() > MAX_PER_SESSION {
        return Err(bounded("watch batch exceeds session retention bound"));
    }
    selected.sort();
    if selected.windows(2).any(|pair| pair[0] == pair[1]) {
        return Err(invalid("watch batch handles must be unique"));
    }
    let mut needs_observation = false;
    for handle in &selected {
        validate_handle(handle)?;
        needs_observation |= awaiting
            && !record(&store, context.session_id, handle)?
                .status
                .terminal();
    }
    ...
    let prepared = Prepared {
        ...
        basis: context.anchor,
        selected,
        registry: registry(&store, context.session_id)?,
        awaiting,
        needs_observation,
    };
    ...
    publish(&store, context, value, |value| {
    ...
    F: FnOnce(Value) -> Result<String>,
{
    ...
    if context.session_id != prepared.session
        || context.anchor.fortress_id != prepared.basis.fortress_id
        || observation_acquired != prepared.needs_observation
        || (!observation_acquired && context.anchor != prepared.basis)
    {
        return Err(failure(
            ErrorCode::StaleAnchor,
            "watch batch session or acquisition boundary changed",
        ));
    }
    if observation_acquired {
        context.authorize(Capability::Observe, RiskTier::ReadOnly, &[], None)?;
    }
    ...
    if registry(&store, context.session_id)? != prepared.registry {
        return Err(failure(
            ErrorCode::Conflict,
            "watch set changed during acquisition; no batch watch transition published",
        ));
    }
    ...
    for handle in &prepared.selected {
        ...
        let watch = candidate
            .entries
            .get_mut(&(context.session_id, handle.clone()))
