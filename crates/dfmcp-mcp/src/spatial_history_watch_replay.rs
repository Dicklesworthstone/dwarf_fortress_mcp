...
enum Request {
    HistoricalWatchReplay { from: RecordRef, to: RecordRef, definition: Value, detail: Option<Detail> },
}
#[derive(Clone, Copy, Deserialize)]
#[serde(rename_all = "snake_case")]
enum Detail { Summary, Evidence }

pub(in super::super::super) fn handles(input: &Value) -> bool {
    input.get("query").and_then(|q|q.get("kind")).and_then(Value::as_str) == Some("historical_watch_replay")
}
pub(in super::super::super) fn schema() -> Result<Value> {
    serde_json::from_str(include_str!("../../../schemas/mcp_historical_watch_replay_v1.json"))
        .map_err(|_|error(ErrorCode::InternalInvariantViolation,"historical watch replay schema invalid"))
}
fn render(session: &Session, context: &OperationContext, last: &JournalEntry,
    ...
    let mut packet:Value=serde_json::from_str(&raw)
        .map_err(|_|error(ErrorCode::InternalInvariantViolation,"historical monitor packet is not JSON"))?;
    packet["agent_turn"]["continuity"]["status"]=json!("partial");
    packet["agent_turn"]["continuity"]["basis"]=basis;
    packet["agent_turn"]["continuity"]["gap"]=json!({"reason":"retrospective_monitor_not_original_watch_activity_or_continuous_history"});
    ...
    if encoded.len() as u64>context.budget.max_bytes.min(u64::from(context.budget.max_output_tokens)*4) {
        return Err(bounded("complete historical monitor packet exceeds the response budget"));
    }
    Ok(encoded)
}

pub(in super::super::super) fn execute(session: &mut Session, context: &OperationContext,
    input: &Value) -> Result<String> {
    let started=Instant::now(); remaining(context,started)?; shape(input)?;
    if session.anchor()?!=context.anchor {
        return Err(error(ErrorCode::StaleAnchor,"historical monitor context differs from the current session"));
    }
    let envelope:Envelope=serde_json::from_value(input.clone())
        .map_err(|_|invalid("invalid historical watch replay envelope"))?;
    if envelope.schema!="dfmcp.query/1" {return Err(invalid("historical monitor requires dfmcp.query/1"));}
    if envelope.expected_anchor.as_ref().is_some_and(|a|a!=&anchor_json(context.anchor)) {
        return Err(error(ErrorCode::StaleAnchor,"historical monitor expected_anchor differs"));
    }
    let Request::HistoricalWatchReplay {from,to,definition,detail}=envelope.query;
    if to.record.checked_sub(from.record).is_none_or(|n|n>=32) {
        return Err(bounded("historical monitor requires one to 32 consecutive records"));
    }
    let journal=session.journal.as_mut().ok_or_else(||invalid("historical monitor requires the configured observation archive"))?;
    journal.validate_custody(context)?;
    if journal.state().snapshot().map(|s|s.anchor())!=Some(context.anchor) {
        return Err(error(ErrorCode::CorruptLedger,"historical monitor archive differs from the current session root"));
    }
    ...
    let requested:Vec<_>=journal.entries().iter().filter(|e|(first.number..=last.number).contains(&e.number))
        .map(|e|(e.number,e.record_digest)).collect();
    if requested.len() as u64!=last.number-first.number+1 {
        return Err(error(ErrorCode::CorruptLedger,"historical monitor range has missing record identities"));
    }
    ...
    if let Some(p)=projection.as_mut() {
        ...
        p.references=vec![json!({"kind":"historical_monitor_range","journal_id":archive_id.to_string(),
    ...
    let overhead=render(session,context,&last,projection.as_ref(),metadata.clone())?.len() as u64+128;
    ...
    result_context.budget.max_bytes=context.budget.max_bytes.min(u64::from(context.budget.max_output_tokens)*4)
        .checked_sub(overhead).filter(|n|*n>0).ok_or_else(||bounded("historical monitor witnesses leave no result budget"))?;
    if projection.is_some(){result_context=semantic_query::result_context(&result_context)?;}
    let mut monitor=semantic_query::HistoricalWatchReplay::new(&definition,first.anchor,
        first.number,last.number,binding,&remaining(context,started)?)?;
    let replay_context=history::replay_context(session,&remaining(context,started)?);
    let limits=session.limits;
    let journal=session.journal.as_mut().ok_or_else(||invalid("historical monitor archive disappeared"))?;
    journal.project_records(&requested,&replay_context,|entry,state| {
        archive::validate_limits(state.observation_full().ok_or_else(||error(ErrorCode::CorruptLedger,"replay source absent"))?,limits)?;
        let snapshot=state.snapshot().ok_or_else(||error(ErrorCode::CorruptLedger,"replay snapshot absent"))?;
        monitor.advance(entry.number,snapshot,&remaining(context,started)?)
    })?;
    journal.validate_custody(context)?;
    if journal.id()!=archive_id||journal.head()!=archive_head {
        return Err(error(ErrorCode::StaleAnchor,"historical monitor archive changed"));
    }
    ...
    let out=value.as_object_mut().ok_or_else(||error(ErrorCode::InternalInvariantViolation,"monitor result is not an object"))?;
    if let Some(metadata)=metadata.as_object(){out.extend(metadata.iter().map(|(k,v)|(k.clone(),v.clone())));}
    if projection.is_some() {
        semantic_query::publish_with_active_work(context,value,|v| {
            let out=render(session,context,&last,projection.as_ref(),v)?;
            remaining(context,started)?; Ok(out)
        })
    }else{
