...
impl Filter {
    ...
    pub(super) fn matches(self, r: &DurableRun) -> bool {
        match self {
            Self::All => true,
            Self::Pending => !r.terminal(),
            Self::Unresolved => r.unresolved(),
...
pub(super) struct Cursors {
    serial: u64,
    entries: BTreeMap<String, (PageKey, usize)>,
}
impl Cursors {
    pub(super) fn resolve(&self, token: &str, key: &PageKey) -> Result<usize> {
        if token.len() != 64 {
            return Err(error(ErrorCode::InvalidRequest, "invalid run continuation"));
        }
        self.entries
            .get(token)
            .filter(|(issued, _)| issued == key)
            .map(|(_, offset)| *offset)
            .ok_or_else(|| {
                error(
                    ErrorCode::StaleAnchor,
                    "run continuation expired or session/head/filter/limit changed",
                )
            })
    }
    pub(super) fn issue(&mut self, key: PageKey, offset: usize) -> Result<String> {
        if let Some((token, _)) = self
            .entries
            .iter()
            .find(|(_, entry)| entry.0 == key && entry.1 == offset)
        {
        ...
        self.serial = self
            .serial
            .checked_add(1)
            .ok_or_else(|| error(ErrorCode::BudgetExceeded, "run continuation IDs exhausted"))?;
        let mut bytes = b"dfmcp-run-page/1\0".to_vec();
        bytes.extend_from_slice(&key.session.get().to_be_bytes());
        bytes.extend_from_slice(key.journal.as_bytes());
...
pub(super) struct View<'a> {
    ...
    pub records: &'a [DurableRun],
    pub selected: Option<&'a RunObservation>,
...
pub(super) fn packet(operation: &str, result: Value, view: View<'_>) -> String {
    ...
    active["actions"] = json!(
        view.records
            .iter()
            .filter(|r| !r.terminal() || r.unresolved())
            .take(4)
            .map(reference)
            .collect::<Vec<_>>()
    );
    active["pending_count"] = json!(pending);
    active["unresolved_count"] = json!(unresolved);
    ...
    active["count_evidence"] = json!(if view.custody_verified {
        "verified_retained_history"
    } else {
        "unverified_or_unbound; empty does not prove absence"
    });
    ...
    if let Some(c) = view.context {
        let mut next = recommendation(
            "run-journal-discovery",
            "fortress.query",
            "Rediscover durable pending work before starting another run.",
            "control_safety",
            "high",
            "read_only",
            "not_applicable",
            false,
            json!({"session_id":c.session_id.to_string(),"state":"unresolved","limit":2}),
        );
