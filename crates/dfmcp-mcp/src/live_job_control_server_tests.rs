...
fn context() -> Result<OperationContext> {
    ...
    Ok(OperationContext {
        ...
        grants: operator_grants(Mode::Control, fortress, true)?,
...
fn development_environment_is_closed_and_production_is_separately_opted_in() -> Result<()> {
    let allowed: Vec<String> = ALLOWED_ENVIRONMENT
        .iter()
        .map(|s| (*s).to_owned())
        .collect();
    ...
    for forbidden in [
        ...
        "DFMCP_CONTROL_TOKEN",
        "DFMCP_JOB_CONTROL_JOURNAL_REPAIR",
    ] {
        let mut keys = allowed.clone();
        keys.push(forbidden.to_owned());
...
fn query_modes_never_acquire_production_even_with_operator_enablement() -> Result<()> {
    ...
    for mode in [Mode::Offline, Mode::Reconcile] {
        for enabled in [false, true] {
            ...
            assert_eq!(grants[0].capability, Capability::Query);
    ...
    assert!(Mode::parse("arbitrary-profile").is_err());
    Ok(())
}
...
fn response_reservation_precedes_work_and_cannot_expand_a_session_budget() -> Result<()> {
    ...
    assert_eq!(
        work.budget.max_bytes + BASE_RESERVE + 8 * RECORD_RESERVE,
        display.budget.max_bytes
    );
    let (display, _) = narrowed(
        original.clone(),
        &Limits {
            bytes: Some(u64::MAX),
            wall: Some(u64::MAX),
    ...
    assert!(narrowed(original, &Limits::default(), MAX_PAGE + 1).is_err());
    Ok(())
}
...
fn deadlines_do_not_restart_between_bootstrap_stages() -> Result<()> {
    ...
    assert!(remaining_context(original, started, 1).is_err());
    Ok(())
}
...
fn job_session_ids_are_canonical_process_scoped_and_not_pause_handles() -> Result<()> {
    ...
    assert!(first.is_process_scoped_live());
    ...
    assert!(session_id(&"0".repeat(33)).is_err());
    Ok(())
}
