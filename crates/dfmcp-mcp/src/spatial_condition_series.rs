...
fn malformed() -> DfmcpError {
    error(
        ...
        "condition evaluator returned an invalid timeline sample",
    )
}
fn truth(value: Option<&Value>) -> Result<&str> {
    value
        .and_then(Value::as_str)
        .filter(|v| matches!(*v, "true" | "false" | "unknown"))
        .ok_or_else(malformed)
}
fn classification(value: &Value) -> Result<Classification<'_>> {
    if value.get("kind").and_then(Value::as_str) != Some("condition_evaluation") {
    ...
    let generation_mismatch = evaluation
        .get("generation_mismatch")
    ...
    let status = evaluation
        ...
        .ok_or_else(malformed)?;
    if evaluation
        .get("watch_completion_proven")
        .and_then(Value::as_bool)
        != Some(false)
        || evaluation
            .get("eligible_success_sample")
            .and_then(Value::as_bool)
            != Some(status == "condition_met")
    {
        return Err(malformed());
    }
    Ok(Classification {
...
pub(super) fn transition(
    ...
    let a = classification(before)?;
    let b = classification(after)?;
    if before.get("predicate_digest") != after.get("predicate_digest")
        || before
            .get("predicate_digest")
            .and_then(Value::as_str)
            .is_none()
    {
        return Err(error(
            ErrorCode::InternalInvariantViolation,
            "timeline condition identity changed between samples",
        ));
    }
    result["status"] = json!(if a == b {
        "same_evaluation_classification"
    } else {
        "evaluation_classification_changed"
    });
    result["from_status"] = json!(a.status);
    result["to_status"] = json!(b.status);
    ...
    result["generation_mismatch"] =
        json!({"from":a.generation_mismatch,"to":b.generation_mismatch});
    ...
    result["interpretation"] = json!(
        "Compare classifications at adjacent retained observations only; equal classifications do not establish unchanged facts or continuous satisfaction, and no watch receives these samples."
    );
    Ok(result)
}
...
pub(super) fn row(entry: &JournalEntry, value: &Value, change: Value) -> Result<Value> {
    validate(value)?;
    Ok(
        json!({"record":archive::entry_json(entry),"evaluation":value["evaluation"],
...
mod tests {
    ...
    fn sample(condition: &str, guard: &str, mismatch: bool) -> Value {
        let status = if mismatch {
            ...
            "failure_condition_met"
        } else if condition == "unknown" || guard == "unknown" {
            "blocked_unknown"
