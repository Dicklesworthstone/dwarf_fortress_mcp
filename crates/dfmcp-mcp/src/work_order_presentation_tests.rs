...
fn summary() -> CreationSummary {
    CreationSummary {
        ...
        journal_id: Digest32::of_bytes(b"journal"),
        head: Digest32::of_bytes(b"head"),
        records: 6,
        prepared: 2,
        unresolved: 1,
        terminal: 3,
...
fn recipes_digests_and_state_filters_are_closed_and_canonical() -> Result<()> {
    for (text, expected) in [
        ("wooden_bed", WorkOrderRecipe::WoodenBed),
        ("wooden_door", WorkOrderRecipe::WoodenDoor),
        ("wooden_table", WorkOrderRecipe::WoodenTable),
        ("wooden_chair", WorkOrderRecipe::WoodenChair),
    ] {
        assert_eq!(recipe(text)?, expected);
    }
    for text in ["WoodenBed", "1", "wooden_bed ", "custom_reaction", "lua"] {
        assert!(recipe(text).is_err());
    }
    ...
    for text in [
        ...
        assert!(digest(&text).is_err());
    }
    let s = summary();
    assert_eq!(Filter::All.total(&s), 6);
    assert_eq!(Filter::Pending.total(&s), 3);
    assert_eq!(Filter::Unresolved.total(&s), 1);
...
fn complete_record_projection_preserves_exact_evidence_without_claiming_production() -> Result<()> {
    ...
    let record = fixture.prepare("evidence")?;
    ...
    assert!(value["created_order_id"].is_null());
...
fn issued_cursor_binds_every_identity_and_retains_bounded_replay_history() -> Result<()> {
    ...
    let mut cursors = Continuations::default();
    let token = cursors.issue(session, &s, "key".into(), Filter::Pending, 2)?;
    assert_eq!(
        cursors.resolve(&token, session, &s, Filter::Pending, 2)?,
        "key"
    );
    assert_eq!(
        cursors.resolve(&token, session, &s, Filter::Pending, 2)?,
    ...
    assert!(
        cursors
            .resolve(&token, session, &other, Filter::Pending, 2)
            .is_err()
    );
    other = s.clone();
    other.journal_id = Digest32::ZERO;
    assert!(
        cursors
            .resolve(&token, session, &other, Filter::Pending, 2)
            .is_err()
    ...
    assert!(
        cursors
            .resolve(&token, SessionId::new(8), &s, Filter::Pending, 2)
            .is_err()
    );
    assert!(
        cursors
            .resolve(&token, session, &s, Filter::All, 2)
            .is_err()
    );
    assert!(
        cursors
            .resolve(&token, session, &s, Filter::Pending, 1)
            .is_err()
    );
    assert!(
        cursors
            .resolve(&Digest32::ZERO.to_string(), session, &s, Filter::Pending, 2)
            .is_err()
    );
    for n in 0..64 {
        cursors.issue(session, &s, format!("key-{n}"), Filter::All, 2)?;
    }
    assert_eq!(cursors.issued.len(), 64);
    assert!(
        cursors
            .resolve(&token, session, &s, Filter::Pending, 2)
            .is_err()
    );
    cursors.serial = u64::MAX;
    assert!(
        cursors
            .issue(session, &s, "end".into(), Filter::All, 2)
            .is_err()
    );
    Ok(())
}
...
#[test]
fn common_turn_exposes_unfinished_work_and_never_inherits_admission_or_empty_world_claims()
-> Result<()> {
    let s = summary();
    let text = packet(
        "fortress.query",
        ...
        TurnView {
            context: None,
            mode: Some(JournalMode::Reconcile),
            summary: Some(&s),
            selected: None,
        },
    );
    let value: Value =
        serde_json::from_str(&text).map_err(|_| error(ErrorCode::InvalidRequest, "JSON"))?;
