...
use dfmcp_adapter::workforce_analysis::{
    self as workforce, WorkforceAnalysis, WorkforceCandidate, WorkforceDemand,
};
...
enum Query {
    ...
    WorkforcePlan {
        ...
        objective: Option<quality::Objective>,
        limit: Option<u32>,
        continuation: Option<String>,
        max_work: Option<u64>,
    },
...
fn validate_shape(input: &Value) -> Result<()> {
    ...
    while let Some((value, depth)) = pending.pop() {
        ...
        if nodes > 4096 || depth > 16 {
            return Err(budget("workforce query exceeds shape bound"));
        }
        match value {
            ...
            Value::Array(values) => {
                if values
                    ...
                    > 4096
                {
                    return Err(budget("workforce array is too wide"));
                }
            ...
            Value::Object(values) => {
                if values
                    ...
                    > 4096
                {
                    return Err(budget("workforce object is too wide"));
                }
        ...
        if bytes > 65_536 {
            return Err(budget("workforce query exceeds aggregate input bound"));
        }
    }
    Ok(())
}
...
fn model_digest(a: &WorkforceAnalysis, kind: &str) -> Digest32 {
    Digest32::of_bytes(
        json!({"domain":"dfmcp-workforce-model/1","kind":kind,"policy":workforce::WORKFORCE_POLICY,
        "anchor":anchor_json(a.anchor),"source_digest":a.source_digest.to_string(),
        "demands":a.demands.iter().map(demand_json).collect::<Vec<_>>()})
        .to_string()
        .as_bytes(),
    )
}
fn page_identity(context: &OperationContext, model: Digest32, maximum_work: u64) -> Digest32 {
    Digest32::of_bytes(
        json!({"domain":"dfmcp-workforce-page/2","session":context.session_id.to_string(),
        "anchor":anchor_json(context.anchor),"model":model.to_string(),"max_work":maximum_work})
        .to_string()
        .as_bytes(),
    )
}
fn token(prefix: &str, offset: usize, identity: Digest32) -> String {
    ...
    format!(
        "{prefix}:{offset}:{}",
        Digest32::of_bytes(value.to_string().as_bytes())
    )
}
fn page_offset(raw: Option<&str>, prefix: &str, identity: Digest32, count: usize) -> Result<usize> {
    ...
    if raw.len() > 128 {
        return Err(budget("workforce continuation exceeds 128 bytes"));
    }
    ...
    {
        return Err(invalid("invalid workforce continuation"));
    }
    let offset = parts[1]
        .parse::<usize>()
        .map_err(|_| invalid("workforce continuation offset overflow"))?;
    if raw != token(prefix, offset, identity) {
        return Err(DfmcpError::new(
            ErrorCode::StaleAnchor,
            "workforce continuation names another session, capture or model",
        ));
    }
    if offset >= count {
        return Err(DfmcpError::new(
            ErrorCode::CursorGap,
            "workforce continuation is past the result set",
        ));
    }
    Ok(offset)
}
...
fn paginate(
    ...
    page: Page<'_>,
    row: impl Fn(usize) -> Result<Value>,
) -> Result<Value> {
    if !(1..=128).contains(&page.limit) {
        return Err(budget("workforce page limit must be 1..128"));
    }
    ...
    out["rows"] = json!([]);
    out["returned"] = json!(0);
    out["total_rows"] = json!(count);
    out["truncated"] = json!(false);
    out["continuation"] = Value::Null;
    if out.to_string().len() > maximum {
        return Err(budget(
            "complete workforce summary does not fit the response allowance",
        ));
    }
    let mut end = start;
    ...
    if start < count && end == start {
        return Err(budget("one complete workforce row and summary cannot fit"));
    }
    Ok(out)
}
...
fn candidate_row(
    ...
    let observation = state
        .observation_full()
        .ok_or_else(|| invariant("workforce source absent"))?;
    let citizen = observation
        .citizens()
        .get(candidate.citizen_index)
        .ok_or_else(|| invariant("workforce citizen index lost"))?;
    let target = a.demands[demand].target;
    Ok(
        json!({"citizen":{"entity_id":candidate.entity_id.to_string(),"generation":candidate.generation,"revision":candidate.revision},
...
pub(super) fn execute(
    ...
    let envelope: Envelope = serde_json::from_value(input.clone())
        .map_err(|_| invalid("invalid workforce query fields"))?;
    if envelope.schema != "dfmcp.query/1" {
        return Err(invalid("workforce query requires dfmcp.query/1"));
    }
    if envelope
        .expected_anchor
        .as_ref()
        ...
        || state.snapshot().map(|s| s.anchor()) != Some(context.anchor)
    {
        return Err(DfmcpError::new(
            ErrorCode::StaleAnchor,
            "workforce query anchor differs",
        ));
    }
    match envelope.query {
        Query::WorkforceCandidates {
            target,
            skill_key,
            min_effective_skill,
            preserve_social,
            ...
            if maximum == 0 || maximum > MAX_ROUTE_WORK {
                return Err(budget("candidate max_work exceeds route allowance"));
            }
            let demand = WorkforceDemand {
                key: "candidate".into(),
                workers: 1,
                target,
                skill_key,
                min_effective_skill: min_effective_skill.unwrap_or(0),
                preserve_social: preserve_social.unwrap_or(true),
                adults_only: adults_only.unwrap_or(true),
            };
            let a = workforce::analyze(state, context, &[demand], maximum)?;
            let model = model_digest(&a, "workforce_candidates");
            let identity = page_identity(context, model, maximum);
            let mut out = base(&a, "workforce_candidates", model);
            out["request"] = demand_json(&a.demands[0]);
            out["classification"] = json!(a.classifications[0]);
            ...
            out["reachable_tiles"] = json!(a.reachable_tiles[0]);
            out["touched_region_boundary"] = json!(a.touched_region_boundary[0]);
        ...
        Query::WorkforcePlan {
            ...
            let a = &planned.analysis;
            let allocation = &planned.allocation;
            let model = quality::model(a, details.as_ref());
            let identity = page_identity(context, model, maximum);
            let mut out = base(a, "workforce_plan", model);
            out["model_feasible"] = json!(allocation.allocated_units == allocation.requested_units);
            out["requested_workers"] = json!(allocation.requested_units);
            out["assigned_workers"] = json!(allocation.allocated_units);
            out["worker_capacity"] = json!(1);
            out["cut_capacity"] = json!(allocation.cut_capacity);
            out["work_units"] = json!(planned.work_units);
            out["optimization"] = json!(
                "maximum_filled_slots_only; deterministic eligibility-mask/demand-key/citizen-id ties; no global skill, distance or priority optimum"
            );
            ...
            out["shortage"] = match &allocation.shortage {
                None => Value::Null,
                Some(shortage) => {
                    let neighbors: BTreeSet<_> = shortage
                        .demand_indices
                        .iter()
                        .flat_map(|&i| a.candidates[i].iter().map(|c| c.entity_id))
                        .collect();
                    if neighbors.len() as u64 != shortage.eligible_units {
                        return Err(invariant("workforce shortage neighbors disagree"));
                    }
            ...
            paginate(
                ...
                |i| {
                    ...
                    let candidate = a.candidates[assignment.demand_index]
                        .iter()
                    ...
                    row["demand_key"] = json!(a.demands[assignment.demand_index].key);
                    row["worker_slots"] = json!(1);
                    Ok(row)
                },
            )
        }
...
pub(super) fn extend_schema(mut schema: Value) -> Result<Value> {
    for source in [
        include_str!("../../../schemas/mcp_workforce_candidates_v1.json"),
        include_str!("../../../schemas/mcp_workforce_plan_v1.json"),
    ] {
        let variant: Value = serde_json::from_str(source)
            .map_err(|_| invariant("embedded workforce schema invalid"))?;
        schema["$defs"]["query"]["oneOf"]
            .as_array_mut()
            .ok_or_else(|| invariant("query variants absent"))?
            .push(variant);
    }
    Ok(schema)
}
