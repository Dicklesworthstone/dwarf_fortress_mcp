...
fn request(
    ...
    if session.source.archive_only() {
        return Err(error(
            ...
            "an archive-only session cannot acquire a live source",
        ));
    }
    if !session.source.poisoned() {
        return Err(error(
            ErrorCode::Conflict,
            "source is healthy; use observe instead of recover_source",
        ));
    }
    if context.session_id != session.id || context.anchor != session.anchor()? {
        return Err(error(
            ErrorCode::StaleAnchor,
            "source recovery names another session or anchor",
        ));
    }
    let invalid = || {
        error(
            ErrorCode::InvalidRequest,
            "recover_source requires schema, exact expected_anchor, and a bounded query with no source selector",
        )
    };
    ...
    if object.len() != 3
        || object
            .keys()
            .any(|k| !matches!(k.as_str(), "schema" | "expected_anchor" | "query"))
        || input["schema"] != "dfmcp.query/1"
    {
        return Err(invalid());
    }
    if input["expected_anchor"] != anchor_json(context.anchor) {
        return Err(error(
            ErrorCode::StaleAnchor,
            "recover_source expected_anchor differs from the retained complete anchor",
        ));
    }
    let query = input["query"].as_object().ok_or_else(invalid)?;
    if !(1..=2).contains(&query.len())
        || query
            .keys()
            .any(|k| !matches!(k.as_str(), "kind" | "max_wall_millis"))
        || query.get("kind").and_then(Value::as_str) != Some(KIND)
    {
        return Err(invalid());
    }
    ...
    if let Some(value) = query.get("max_wall_millis") {
        ...
        if millis == 0 || millis > 60_000 || millis > context.budget.max_wall_millis {
            return Err(error(
                ...
                "recovery wall-time must narrow the session allowance",
            ));
        }
...
fn remaining(context: &OperationContext, elapsed: Duration) -> Result<OperationContext> {
    context.authorize(Capability::Query, RiskTier::ReadOnly, &[], None)?;
    context.authorize(Capability::Observe, RiskTier::ReadOnly, &[], None)?;
    let allowance = Duration::from_millis(context.budget.max_wall_millis)
        .checked_sub(elapsed)
        .filter(|v| *v >= Duration::from_millis(1))
        .ok_or_else(|| {
            error(
                ErrorCode::BudgetExceeded,
                "source recovery exhausted its shared foreground deadline",
            )
        })?;
    let mut narrowed = context.clone();
    narrowed.budget.max_wall_millis = allowance.as_millis() as u64;
    Ok(narrowed)
}
...
fn details(
    outcome: Option<JobPublication>,
    failure: Option<&DfmcpError>,
    connections: u32,
    captures: u32,
    pages: u32,
) -> Value {
...
fn render(
    ...
    let mut obligations = work
        .as_object_mut()
        .and_then(|v| v.remove("_condition_watch_work"))
        .unwrap_or_else(|| json!([]));
    ...
    if packet.len() as u64 > maximum {
        return Err(error(
            ErrorCode::BudgetExceeded,
            "complete source-recovery packet exceeds the output allowance",
        ));
    }
    Ok(packet)
}
...
fn reserve(session: &Session, context: &OperationContext, mut work: Value) -> Result<String> {
    ...
    if let Some(rows) = work
        .get_mut("_condition_watch_work")
    ...
    for key in ["epoch", "sequence", "game_tick"] {
        target[key] = json!(u64::MAX);
    }
    ...
    let mut success = worst;
    success["reconnected"] = json!(true);
...
pub(super) fn execute(
    ...
    let token = std::env::var("DFMCP_SPATIAL_CITIZEN_TOKEN")
        .map_err(|_| {
            error(
                ErrorCode::CapabilityDenied,
                "DFMCP_SPATIAL_CITIZEN_TOKEN required for source recovery",
            )
        })?
        .into_bytes();
    if !(32..=256).contains(&token.len()) {
        return Err(error(
            ErrorCode::CapabilityDenied,
            "spatial source recovery credential length is invalid",
        ));
    }
    let nonce = session.id.get().to_be_bytes().to_vec();
...
where
    F: FnOnce(Duration) -> Result<Box<dyn Source>>,
    C: FnMut() -> Duration,
{
    ...
    if session
        .journal
        .as_ref()
        .is_some_and(|journal| journal.recovery_only())
    {
        return Err(error(
            ErrorCode::CapabilityDenied,
            "source recovery cannot write an archive-only journal",
        ));
    }
    ...
    let attempt = (|| {
        ...
        if source.poisoned() || source.archive_only() || source.closed() {
            return Err(error(
                ...
                "replacement source is not an unfenced live reader",
            ));
        }
    ...
    let (outcome, failed) = match attempt {
...
pub(super) fn extend_schema(mut schema: Value) -> Result<Value> {
    let extension: Value =
        serde_json::from_str(include_str!("../../../schemas/mcp_source_recovery_v1.json"))
            .map_err(|_| {
                error(
                    ErrorCode::InternalInvariantViolation,
                    "source recovery schema is invalid",
                )
            })?;
    schema["$defs"]["query"]["oneOf"]
        ...
        .ok_or_else(|| {
            error(
                ErrorCode::InternalInvariantViolation,
                "source recovery query variants absent",
            )
        })?
    ...
    let object = schema.as_object_mut().ok_or_else(|| {
        error(
            ErrorCode::InternalInvariantViolation,
            "query schema root is not an object",
        )
    })?;
    object
        .entry("allOf")
        ...
        .ok_or_else(|| {
            error(
                ErrorCode::InternalInvariantViolation,
                "query schema allOf is not an array",
            )
        })?
        .push(
            json!({"if":{"required":["query"],"properties":{"query":{"required":["kind"],
