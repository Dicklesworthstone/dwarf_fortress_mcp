...
fn comparison() -> Value {
    ...
    envelope(json!({"kind":"production_chain_compare","quantity_unit":"stack_units",
...
fn compare_models_in_live_handler_without_sampling_existing_watches() -> Result<()> {
    ...
    let watched = ask(&s, envelope(json!({"kind":"watch","key":"comparison-review",
    ...
    let first = ask(&s, comparison())?; assert_eq!(first["ok"], true, "{first}");
    assert_eq!(first["summary"]["feasible_candidates"], json!(["efficient"]));
    ...
    let current = ask(&s, comparison())?; assert_eq!(current["ok"], true, "{current}");
    assert_eq!(current["summary"]["feasible_candidates"], json!(["demanding","efficient"]));
    assert_eq!(current["summary"]["deficit_frontier"], json!(["demanding","efficient"]));
...
fn invalid_candidate_and_source_fencing_never_yield_a_partial_comparison() -> Result<()> {
    ...
    let variants = schema["query_schema"]["$defs"]["query"]["oneOf"].as_array()
        .ok_or_else(|| error(ErrorCode::InvalidRequest, "comparison schema variants absent"))?;
    assert!(variants.iter().any(|v| v["properties"]["kind"]["const"] == "production_chain_compare"));
    let mut bad = comparison();
    bad["query"]["candidates"][1]["recipes"][0]["inputs"][0]["resource"] = json!("missing");
    let denied = ask(&s, bad)?; assert_eq!(denied["ok"], false);
    assert!(denied.get("comparison_digest").is_none()); assert!(denied.get("rows").is_none());
    let mut bad = comparison(); bad["query"]["max_work"] = json!(1);
    assert_eq!(ask(&s, bad)?["error"]["code"], "budget_exceeded");
    { let handle = resolve(s.handle())?; lock(&handle)?.source.fence(); }
    assert_eq!(ask(&s, comparison())?["error"]["code"], "adapter_unavailable");
