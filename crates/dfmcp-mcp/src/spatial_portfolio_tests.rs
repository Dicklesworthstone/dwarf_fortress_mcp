...
impl Source for Script {
    fn read(&mut self, _: Duration) -> Result<LiveSpatialCitizenObservation> {
        ...
        self.values
            .pop_front()
            .ok_or_else(|| error(ErrorCode::AdapterUnavailable, "portfolio source exhausted"))
    }
...
fn decode(raw: &str) -> Result<Value> {
    serde_json::from_str(raw)
        .map_err(|_| error(ErrorCode::InvalidRequest, "portfolio fixture JSON"))
}
...
fn register(files: &Files, next: Vec<LiveSpatialCitizenObservation>) -> Result<Registered> {
    ...
    let anchor = state
        .snapshot()
        .ok_or_else(|| error(ErrorCode::InvalidRequest, "fixture state absent"))?
        .anchor();
...
fn retain_watch(s: &Registered) -> Result<()> {
    ok(ask(
        s,
        json!({"kind":"watch","key":"portfolio-progress","condition":{"op":"paused","value":true},
...
fn portfolio_handler_selects_complete_tasks_without_mutating_watches_or_observations() -> Result<()>
{
    ...
    assert_eq!(result["selected_task_keys"], json!(["b", "c"]));
    assert_eq!(result["selected_priority"], 4);
    assert_eq!(result["assigned_workers"], 2);
    assert_eq!(result["assigned_stack_units"], 2);
    assert_eq!(result["native_captures"], 0);
    assert_eq!(result["reservations_created"], false);
    assert_eq!(result["native_job_readiness_proven"], false);
    assert_eq!(result["production_schedule_proven"], false);
    let rows = result["rows"]
        .as_array()
        .ok_or_else(|| error(ErrorCode::InvalidRequest, "rows absent"))?;
    assert_eq!(
        rows.iter()
            .filter(|r| r["row_kind"] == "worker_assignment")
            .count(),
        2
    );
    assert_eq!(
        rows.iter()
            .filter(|r| r["row_kind"] == "material_assignment")
            .count(),
        2
    );
    assert_eq!(
        rows.iter()
            .filter(|r| r["row_kind"] == "rejected_combination")
            .count(),
        3
    );
...
fn portfolio_pagination_preserves_the_optimum_and_rejects_other_models() -> Result<()> {
    ...
    loop {
        ...
        assert_eq!(result["selected_task_mask"], 6);
        assert_eq!(result["assigned_stack_units"], 2);
        if evidence.is_null() {
            evidence = result["optimization"]["exclusion_evidence_digest"].clone();
        }
        assert_eq!(
            result["optimization"]["exclusion_evidence_digest"],
            evidence
        );
            ...
            .ok_or_else(|| error(ErrorCode::InvalidRequest, "rows absent"))?
        {
            match row["row_kind"].as_str() {
                Some("worker_assignment") => {
                    assert!(workers.insert(row["citizen"]["entity_id"].to_string()));
                }
                Some("material_assignment") => units += row["units"].as_u64().unwrap_or(0),
                Some("rejected_combination") => {
                    cuts += 1;
                    assert!(row["deficit"].as_u64().unwrap_or(0) > 0);
                }
                _ => {
                    return Err(error(
                        ErrorCode::InternalInvariantViolation,
                        "unknown portfolio row",
                    ));
                }
            }
        }
    ...
    assert_eq!((workers.len(), units, cuts), (2, 2, 3));
    assert!(pages > 1);
    let mut different = request();
    different["continuation"] = first_token.clone();
    different["tasks"][0]["priority"] = json!(20);
    assert_eq!(ask(&s, different)?["error"]["code"], "stale_anchor");
    ok(decode(&fortress_observe(s.handle()))?);
    let mut stale = request();
    stale["continuation"] = first_token;
    assert_eq!(ask(&s, stale)?["error"]["code"], "stale_anchor");
    Ok(())
}
...
fn historical_portfolios_and_each_route_remain_bound_to_the_selected_record() -> Result<()> {
    ...
    assert_eq!(current["selected_tasks"], 0);
    ...
    assert_eq!(old["selected_task_mask"], old_current["selected_task_mask"]);
    assert_eq!(old["anchor"], entry["anchor"]);
        ...
        .ok_or_else(|| error(ErrorCode::InvalidRequest, "rows absent"))?
    {
        if let Some(route) = row.get("route_query") {
            ...
            assert_eq!(result["anchor"], entry["anchor"]);
    ...
    assert_eq!(fs::read(&f.watches).map_err(io_error)?, bytes);
...
fn offline_reopening_can_plan_but_cannot_claim_current_readiness() -> Result<()> {
    ...
    ok(ask(&s, historical(&entry, request()))?);
    drop(s);
    let archive_before = fs::read(&f.observations).map_err(io_error)?;
    let watches_before = fs::read(&f.watches).map_err(io_error)?;
    ...
    assert!(
        schema["query_schema"]["$defs"]["query"]["oneOf"]
            ...
            .is_some_and(|a| a
                .iter()
                .any(|v| v["properties"]["kind"]["const"] == "production_portfolio"))
    );
    assert_eq!(fs::read(&f.observations).map_err(io_error)?, archive_before);
    assert_eq!(fs::read(&f.watches).map_err(io_error)?, watches_before);
    Ok(())
}
...
fn malformed_tasks_small_budgets_and_revoked_authority_leave_state_unchanged() -> Result<()> {
    ...
    assert_eq!(fs::read(&f.watches).map_err(io_error)?, watches);
    assert_eq!(s.calls.load(Ordering::SeqCst), 0);
    Ok(())
}
