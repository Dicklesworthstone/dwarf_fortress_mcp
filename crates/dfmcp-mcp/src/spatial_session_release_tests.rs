...
use dfmcp_adapter::operations_journal::{JournalLimits, PrivateJournalFile, Spatial18, TailRecovery};
...
fn io_error(_: std::io::Error) -> DfmcpError { error(ErrorCode::CorruptLedger, "release fixture I/O") }
...
impl Files {
    fn new() -> Result<Self> {
        let directory = std::env::temp_dir().canonicalize().map_err(io_error)?.join(format!(
            "dfmcp-session-release-{}-{}", std::process::id(), FILE_ID.fetch_add(1, Ordering::Relaxed)));
...
impl Drop for Files { fn drop(&mut self) {
    let _ = fs::remove_file(&self.watches); let _ = fs::remove_file(&self.observations);
    let _ = fs::remove_dir(&self.directory);
} }
struct Script { values: VecDeque<LiveSpatialCitizenObservation>, calls: Arc<AtomicUsize>, drops: Arc<AtomicUsize>, fenced: bool }
impl Source for Script {
    fn read(&mut self, _: Duration) -> Result<LiveSpatialCitizenObservation> {
        self.calls.fetch_add(1, Ordering::SeqCst);
        self.values.pop_front().ok_or_else(|| error(ErrorCode::AdapterUnavailable, "injected source exhausted"))
    }
    fn poisoned(&self) -> bool { self.fenced }
    fn fence(&mut self) { self.fenced = true; }
    fn pages(&self) -> u32 { 1 }
}
impl Drop for Script { fn drop(&mut self) { self.drops.fetch_add(1, Ordering::SeqCst); } }
struct Registered { id: SessionId, calls: Arc<AtomicUsize>, drops: Arc<AtomicUsize> }
...
impl Drop for Registered { fn drop(&mut self) {
    ...
    let removed = SESSIONS.lock().ok().and_then(|mut sessions| sessions.remove(&self.id));
    if let Some(handle) = removed {
        ...
        session._watch_journal = None; session.journal = None; session._slot.release();
    }
} }
fn register(files: Option<&Files>, durable: bool, tick: u32, next: &[u32]) -> Result<Registered> {
    ...
    let source = Script { values:next.iter().map(|t| fixture::observation(*t, 1, 5, false)).collect::<Result<VecDeque<_>>>()?,
        calls:calls.clone(), drops:drops.clone(), fenced:false };
    ...
    if let Some(files) = files {
        ...
        if durable { durable_watches::finish_open(&mut session, &c, Some(&files.watches), json!({"ok":true}))?; }
    }
    let id = session.id; lock(&SESSIONS)?.insert(id, Arc::new(Mutex::new(session)));
    Ok(Registered { id, calls, drops })
}
fn decode(raw: &str) -> Result<Value> { serde_json::from_str(raw).map_err(|_| error(ErrorCode::InvalidRequest, "fixture response")) }
...
fn successful(value: Value) -> Value { assert_eq!(value["ok"], true, "{value}"); value }
fn watch(s: &Registered) -> Result<Value> {
    let handle = resolve(s.handle())?;
    let deadline = lock(&handle)?.anchor()?.tick.0 + 50;
    Ok(successful(ask(s, json!({"kind":"watch", "key":"keep-intent", "condition":{"op":"paused","value":true},
        "deadline_tick":deadline, "stable_observations":2}))?)["record"]["watch"].clone())
}
fn baseline(s: &Registered) -> Result<Value> {
    Ok(successful(ask(s, json!({"kind":"capture", "key":"keep-baseline", "max_game_ticks":50,
        "select":{"kind":"entities","kinds":["job"],"fields":["suspended"]}}))?)["captured"].clone())
}
fn request_close(s: &Registered, discard: bool) -> Result<Value> {
    decode(&fortress_cancel(s.handle(), Some("session".into()), Some(discard)))
}

#[test]
fn close_reclaims_slots_and_sources_even_with_already_resolved_arcs() -> Result<()> {
    let _serial = lock(&SERIAL)?;
    let a = register(None, false, 3, &[])?; let b = register(None, false, 3, &[])?;
    assert!(Slot::reserve().is_err());
    let old = resolve(a.handle())?; let context = lock(&old)?.context()?;
    let raw = fortress_cancel(a.handle(), Some("session".into()), None);
    successful(decode(&raw)?);
    assert_eq!(a.drops.load(Ordering::SeqCst), 1); assert_eq!(a.calls.load(Ordering::SeqCst), 0);
    {
        ...
        assert!(stale.state.snapshot().is_none()); assert!(stale.grants.is_empty());
    }
    let c = register(None, false, 3, &[])?;
    assert_eq!(fortress_cancel(a.handle(), Some("session".into()), None), raw);
    assert_eq!(SLOTS.load(Ordering::SeqCst), 2);
    drop(old); assert_eq!(SLOTS.load(Ordering::SeqCst), 2);
    successful(request_close(&b, false)?); successful(request_close(&c, false)?);
...
fn volatile_records_need_consent_and_failed_close_keeps_both_registries() -> Result<()> {
    let _serial = lock(&SERIAL)?; let s = register(None, false, 3, &[])?;
    watch(&s)?; baseline(&s)?;
    let watches = successful(ask(&s, json!({"kind":"watches"}))?)["records"].clone();
    assert_eq!(request_close(&s, false)?["error"]["code"], "conflict");
    assert_eq!(successful(ask(&s, json!({"kind":"watches"}))?)["records"], watches);
    assert_eq!(successful(ask(&s, json!({"kind":"baselines"}))?)["baselines"].as_array().map(Vec::len), Some(1));
    let closed = successful(request_close(&s, true)?);
    assert_eq!(closed["process_local_resources"]["process_local_baselines_discarded"], 1);
    assert_eq!(closed["process_local_resources"]["process_local_watch_records_discarded"], 1);
    assert_eq!(ask(&s, json!({"kind":"watches"}))?["error"]["code"], "session_not_found");
...
fn render_refusal_keeps_durable_files_locks_and_volatile_records() -> Result<()> {
    ...
    let s = register(Some(&files), true, 3, &[])?; watch(&s)?; baseline(&s)?;
    ...
    assert_eq!(successful(ask(&s, json!({"kind":"baselines"}))?)["baselines"].as_array().map(Vec::len), Some(1));
    assert_eq!(successful(ask(&s, json!({"kind":"watches"}))?)["records"].as_array().map(Vec::len), Some(1));
...
fn durable_close_preserves_original_journals_and_reopen_recovers_fresh_handles() -> Result<()> {
    let _serial = lock(&SERIAL)?; let files = Files::new()?;
    let s = register(Some(&files), true, 3, &[])?; let old_watch = watch(&s)?;
    ...
    assert_eq!(closed["process_local_resources"]["durable_watch_recovery_required"], true);
    assert_eq!(fs::read(&files.observations).map_err(io_error)?, old_observation);
    ...
    assert_ne!(records[0]["watch"], old_watch); assert_eq!(records[0]["fresh_observation_required"], true);
    assert_eq!(records[0]["stable_observations"], 0);
    assert_eq!(ask(&next, json!({"kind":"await_watches","watches":[old_watch]}))?["ok"], false);
    for step in 1..=2 {
    ...
    assert_eq!(s.calls.load(Ordering::SeqCst), 0); assert_eq!(next.calls.load(Ordering::SeqCst), 2);
    drop(old); successful(request_close(&next, false)?);
    Ok(())
}
...
fn damaged_journals_and_fenced_source_do_not_trap_session_ownership() -> Result<()> {
    let _serial = lock(&SERIAL)?; let files = Files::new()?;
    let s = register(Some(&files), true, 3, &[])?; watch(&s)?;
    ...
    assert_eq!(result["process_local_resources"]["watch_evidence_revalidated"], false);
    ...
    for case in 0..3 {
        ...
        { let mut session = lock(&handle)?;
          match case {
              ...
              1 => { let expired = GameTick(session.anchor()?.tick.0 - 1);
                     for grant in &mut session.grants { grant.expires_at_tick = Some(expired); } },
              _ => session.grants.clear(),
          }
        }
        let result = successful(request_close(&s, false)?);
        assert!(result.get("anchor").is_none()); assert!(result["agent_turn"]["anchor"].is_null());
        assert_eq!(result["game_state_observed"], false); assert_eq!(s.calls.load(Ordering::SeqCst), 0);
    }
    Ok(())
}
...
fn concurrent_closers_wait_for_session_ownership_and_return_one_receipt() -> Result<()> {
    let _serial = lock(&SERIAL)?; let s = register(None, false, 3, &[])?;
    let handle = resolve(s.handle())?; let guard = lock(&handle)?;
    let raw_a = s.handle(); let raw_b = s.handle();
...
fn closed_receipts_are_bounded_and_eviction_cannot_close_a_new_session() -> Result<()> {
    let _serial = lock(&SERIAL)?;
    let first = register(None, false, 3, &[])?; successful(request_close(&first, false)?);
    for _ in 0..MAX_CLOSE_RECEIPTS { let s = register(None, false, 3, &[])?; successful(request_close(&s, false)?); }
    let live = register(None, false, 3, &[])?;
    assert!(lock(&CLOSED)?.len() <= MAX_CLOSE_RECEIPTS);
    assert_eq!(request_close(&first, false)?["error"]["code"], "session_not_found");
    assert!(resolve(live.handle()).is_ok()); assert_eq!(live.drops.load(Ordering::SeqCst), 0);
...
fn scope_errors_and_legacy_cancel_never_release_a_session() -> Result<()> {
    ...
    for (scope, discard) in [(None,Some(true)),(Some("game".into()),None),(Some("".into()),None)] {
        assert_eq!(decode(&fortress_cancel(s.handle(), scope, discard))?["error"]["code"], "invalid_request");
    }
    for raw in [None,Some("bad".into()),Some("00000000000000000000000000000007".into())] {
        assert_eq!(decode(&fortress_cancel(raw, Some("session".into()), None))?["ok"], false);
    }
    assert!(resolve(s.handle()).is_ok()); assert_eq!(s.drops.load(Ordering::SeqCst), 0);
...
fn poisoned_session_can_be_released_without_clearing_or_reusing_its_world() -> Result<()> {
    ...
    assert_eq!(result["game_state_observed"], false); assert_eq!(s.drops.load(Ordering::SeqCst), 1);
    Ok(())
}
