...
fn release(s: &Registered, watch: &Value) -> Result<()> {
    assert_eq!(ask(s, json!({"kind":"cancel_watch","watch":watch}))?["ok"], true);
    assert_eq!(ask(s, json!({"kind":"release_watch","watch":watch}))?["ok"], true);
    Ok(())
}

#[test]
fn preview_then_explicit_registration_and_wait_produce_stable_terrain_evidence() -> Result<()> {
    let _serial = lock(&SERIAL)?; let s = register(2, 2048)?;
    let p = ask(&s, preview())?; assert_eq!(p["ok"], true, "{p}");
    ...
    assert_eq!(created["record"]["evaluation"]["facts"][0]["requested_tiles"], 9);
...
fn paged_preview_keeps_complete_monitor_and_existing_watch_without_sampling() -> Result<()> {
    let _serial = lock(&SERIAL)?; let s = register(2, 2048)?;
    let existing = ask(&s, json!({"kind":"watch","key":"existing","condition":{"op":"paused","value":true},
        "deadline_tick":105u64*403200+100,"stable_observations":2}))?;
    assert_eq!(existing["ok"], true, "{existing}");
    let mut q = preview(); q["origin"] = json!([2,0,5]); q["limit"] = json!(1);
    ...
    assert_eq!(first["monitoring"]["watch_request"]["query"]["condition"]["areas"].as_array().map(Vec::len), Some(4));
    ...
    for _ in 0..4 {
        ...
        for row in page["rows"].as_array().ok_or_else(|| error(ErrorCode::InvalidRequest, "test rows"))? {
    ...
    q["continuation"] = first["continuation"].clone(); q["monitor"]["key"] = json!("different-goal");
...
fn invalid_or_unrenderable_proposals_do_not_register_or_dispatch() -> Result<()> {
    ...
    for field in ["commit", "raw_lua", "override_environmental_hazards"] {
        let mut q = preview(); q["monitor"][field] = json!(true);
        assert_eq!(ask(&s, q)?["error"]["code"], "invalid_request");
    }
    let mut expired = preview(); expired["monitor"]["deadline_tick"] = json!(1);
    assert_eq!(ask(&s, expired)?["error"]["code"], "invalid_request");
    { let handle = resolve(s.handle())?; lock(&handle)?.budget.max_bytes = 1; }
    assert_eq!(ask(&s, preview())?["ok"], false);
    { let handle = resolve(s.handle())?; lock(&handle)?.budget.max_bytes = 1024*1024; }
    assert_eq!(ask(&s, json!({"kind":"watches"}))?["records"], json!([]));
    assert_eq!(s.calls.load(Ordering::SeqCst), 0);
    Ok(())
}
