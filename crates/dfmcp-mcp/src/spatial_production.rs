...
pub(super) fn extend_schema(base: Value) -> Result<Value> {
    chain::extend_schema(workforce_queries::portfolio::extend_schema(shared::extend_schema(base)?)?)
}
pub(super) fn execute(state: &LiveSpatialCitizenState, context: &OperationContext, input: &Value) -> Result<Value> {
    if chain::handles(input) { chain::execute(state,context,input) }
    else if workforce_queries::portfolio::handles(input) { workforce_queries::portfolio::execute(state,context,input) }
...
fn custody(session: &mut Session, context: &OperationContext) -> Result<()> {
    ...
    if session.anchor()?!=context.anchor {
        return Err(error(ErrorCode::StaleAnchor,"production context differs from the coherent session"));
    }
    if let Some(journal)=session.journal.as_mut() {
        ...
        if journal.state().snapshot().map(|s|s.anchor())!=Some(context.anchor) {
            return Err(error(ErrorCode::CorruptLedger,"production observation archive disagrees with published state"));
        }
    }
    Ok(())
}

pub(super) fn live(session: &mut Session, context: &OperationContext, input: &Value) -> Result<String> {
    let started=Instant::now();
    if session.source.archive_only() {
        return Err(error(ErrorCode::CapabilityDenied,"archive production reads must use the historical response path"));
    }
    if session.source.poisoned() {
        return Err(error(ErrorCode::AdapterUnavailable,"production source fenced; use verified historical queries"));
    }
...
pub(super) fn pin_historical(value: &mut Value, record: u64, digest: Digest32) -> Result<()> {
    if let Some(rows)=value.get_mut("rows").and_then(Value::as_array_mut) {
        for row in rows {
            for (field,kind) in [("inspect_assignment","inspect"),("inspect_relationships","traverse")] {
                if let Some(request)=row.get(field) {
                    let query=request.get("query").filter(|q|q.get("kind").and_then(Value::as_str)==Some(kind))
                        .ok_or_else(||error(ErrorCode::InternalInvariantViolation,"production inspection has the wrong fixed query kind"))?;
                    ...
                    if replacement.to_string().len()>request.to_string().len() {
                        return Err(error(ErrorCode::InternalInvariantViolation,"historical inspection exceeds reserved production row size"));
                    }
