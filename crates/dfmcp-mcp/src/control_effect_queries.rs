...
use dfmcp_adapter::control_effect_journal::{
    ControlEffectJournal, DurablePauseState, EffectJournalStorage,
};
...
pub(super) fn effects<S: EffectJournalStorage>(
    ...
    if !(1..=128).contains(&query.limit)
        || query.max_bytes == Some(0)
        || query.max_output_tokens == Some(0)
    {
        return Err(invalid(
            "effect limit must be 1..128 and response budgets must be nonzero",
        ));
    }
    ...
    for record in records.clone() {
        if started.elapsed().as_millis() >= u128::from(context.budget.max_wall_millis) {
            return Err(DfmcpError::new(
                ErrorCode::BudgetExceeded,
                "effect enumeration exceeded its work deadline",
            ));
        }
    ...
    let token_limit = query
        .max_output_tokens
        .unwrap_or(context.budget.max_output_tokens)
        .min(context.budget.max_output_tokens);
    let byte_limit = query
        .max_bytes
        .unwrap_or(context.budget.max_bytes)
        .min(context.budget.max_bytes)
        .min(u64::from(token_limit) * 4);
    ...
    let fits = |value: &Value| {
        packet("fortress.query", value.clone(), recovery_only).len() as u64 <= byte_limit
    };
    ...
    for record in records
        .clone()
        .filter(|r| matches_state(query.state, r.state))
        .skip(offset)
        .take(limit)
    {
        if started.elapsed().as_millis() >= u128::from(context.budget.max_wall_millis) {
            return Err(DfmcpError::new(
                ErrorCode::BudgetExceeded,
                "effect rendering exceeded its work deadline",
            ));
        }
    ...
    if rows.is_empty() && offset < matching {
        return Err(exhausted());
    }
    Ok(payload)
}
...
mod tests {
    ...
    use dfmcp_core::{
        Capability, CapabilityGrant, CapabilityScope, FortressId, GameTick, ObservationCursor,
    ...
    impl EffectJournalStorage for Memory {
        ...
        fn truncate(&mut self, n: u64) -> io::Result<()> {
            self.0.get_mut().truncate(n as usize);
            Ok(())
        }
    }
        ...
        for key in [
            ...
            if key == "d-applied" || key == "e-not-applied" {
                ...
                j.record_reconciliation(
                    ...
                    applied,
                    11,
                    applied.then(|| Digest32::of_bytes(b"receipt")),
                    &context(),
                )?;
            }
        }
    ...
    fn pages_are_sorted_replayable_and_bound_to_the_full_packet_budget() -> Result<()> {
        ...
        let token = first["continuation"]
            .as_str()
            .ok_or_else(|| invalid("missing test continuation"))?;
        let next = EffectQuery {
        ...
        assert!(
            packet("fortress.query", second.clone(), false).len() as u64
                <= second["response_budget"]["max_bytes"]
                    .as_u64()
    ...
    fn continuations_reject_other_sessions_filters_heads_and_tampering() -> Result<()> {
        ...
        other.session_id = SessionId::new(92);
        assert!(
            matches!(effects(&mut j, &other, EffectQuery { continuation: Some(token), ..query("all", 1) }),
            Err(e) if e.code == ErrorCode::Conflict)
        );
        assert!(
            matches!(effects(&mut j, &context(), EffectQuery { continuation: Some(token), ..query("nonterminal", 1) }),
            Err(e) if e.code == ErrorCode::Conflict)
        );
        ...
        assert!(
            matches!(effects(&mut j, &context(), EffectQuery { continuation: Some(token), ..query("all", 1) }),
            Err(e) if e.code == ErrorCode::Conflict)
        );
        Ok(())
    }
    ...
    fn filters_counts_and_empty_results_preserve_uncertainty() -> Result<()> {
        ...
        let mut empty = ControlEffectJournal::open(
            Memory::default(),
            &context(),
            true,
            7,
    ...
    fn zero_progress_tiny_budgets_invalid_input_and_lost_authority_are_refused() -> Result<()> {
        ...
        for bytes in [1, 32, 128] {
            assert!(
                matches!(effects(&mut j, &context(), EffectQuery { max_bytes: Some(bytes), ..query("all", 1) }),
        ...
        denied.grants.clear();
        assert!(
            matches!(effects(&mut j, &denied, query("all", 1)), Err(e) if e.code == ErrorCode::CapabilityDenied)
        );
        denied = context();
        denied.cancellation_requested = true;
        assert!(
            matches!(effects(&mut j, &denied, query("all", 1)), Err(e) if e.code == ErrorCode::CancellationRequested)
        );
        assert_eq!(j.head(), head);
        Ok(())
    }
    ...
    fn cancellation_is_distinct_from_native_failure_and_changes_the_page_identity() -> Result<()> {
        ...
        let token = old["continuation"]
            .as_str()
            .ok_or_else(|| invalid("test continuation"))?;
        ...
        assert_eq!(
            effects(&mut j, &context(), query("nonterminal", 128))?["matched_effects"],
            2
        );
        assert!(
            matches!(effects(&mut j,&context(),EffectQuery {continuation:Some(token),..query("all",1)}),
