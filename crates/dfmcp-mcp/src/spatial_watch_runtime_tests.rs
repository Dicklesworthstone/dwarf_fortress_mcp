...
fn io_error(_:std::io::Error)->DfmcpError{error(ErrorCode::CorruptLedger,"spatial watch test filesystem failure")}
...
impl Files{
    fn new()->Result<Self>{
        ...
        let directory=std::env::temp_dir().canonicalize().map_err(io_error)?
            .join(format!("dfmcp-spatial-watch-handlers-{}-{n}",std::process::id()));
...
impl Drop for Files{
    fn drop(&mut self){let _=fs::remove_file(&self.watches);let _=fs::remove_file(&self.observations);let _=fs::remove_dir(&self.directory);}
}
...
impl Source for Script{
    fn read(&mut self,_:Duration)->Result<LiveSpatialCitizenObservation>{
        ...
        self.values.pop_front().ok_or_else(||error(ErrorCode::AdapterUnavailable,"injected capture source exhausted"))
    }
...
fn observation(tick:u32)->Result<LiveSpatialCitizenObservation>{
    ...
    let bytes=(0..hex.len()).step_by(2).map(|i|u8::from_str_radix(&hex[i..i+2],16)
        .map_err(|_|error(ErrorCode::InternalInvariantViolation,"invalid spatial fixture hex"))).collect::<Result<Vec<_>>>()?;
    let base=LiveSpatialObservation::decode_payload(&bytes,7,"df".to_owned(),"dfhack".to_owned())?;
    ...
    LiveSpatialCitizenObservation::decode_payload(&combined,7,"df".to_owned(),"dfhack".to_owned())
}
fn session(files:&Files,tick:u32,next:&[u32])->Result<(Session,OperationContext,Arc<AtomicUsize>)>{
    ...
    let anchor=state.snapshot().ok_or_else(||error(ErrorCode::InternalInvariantViolation,"fixture snapshot absent"))?.anchor();
    let calls=Arc::new(AtomicUsize::new(0));
    let values=next.iter().map(|tick|observation(*tick)).collect::<Result<VecDeque<_>>>()?;
    let mut session=Session{id:next_id()?,source:Box::new(Script{values,calls:Arc::clone(&calls),fenced:false}),state,limits,
        journal:None,budget:WorkBudget{max_entities:limits.entity_limit(),max_bytes:1024*1024,
    ...
    let mut context=session.context()?;history::attach(&mut session,&files.observations,TailRecovery::Refuse,&context)?;
    context.anchor=session.anchor()?;Ok((session,context,calls))
}
...
fn decode(text:&str)->Result<Value>{serde_json::from_str(text).map_err(|_|error(ErrorCode::InternalInvariantViolation,"handler JSON invalid"))}
...
fn watch()->Value{json!({"kind":"watch","key":"remain-paused","condition":{"op":"paused","value":true},
...
fn mcp_restart_discovers_watches_and_only_fresh_captures_restore_stability()->Result<()>{
    ...
    let created=ask(&first,watch())?;assert_eq!(created["ok"],true,"{created}");
    assert_eq!(created["durable"],true);assert_eq!(created["record"]["stable_observations"],1);
    let old=created["record"]["watch"].clone();drop(first);
    ...
    assert_eq!(work["fresh_observation_required"],true);assert_eq!(work["evaluation_current"],false);
    let listed=ask(&second,json!({"kind":"watches"}))?;
    let handle=listed["records"][0]["watch"].clone();assert_ne!(handle,old);
    assert_eq!(ask(&second,json!({"kind":"poll_watch","watch":old}))?["ok"],false);
    let same=ask(&second,json!({"kind":"poll_watch","watch":handle}))?;
    assert_eq!(same["record"]["stable_observations"],0);assert_eq!(second.calls.load(Ordering::SeqCst),0);
    let one=ask(&second,json!({"kind":"await_watch","watch":handle}))?;
    assert_eq!(one["ok"],true,"{one}");assert_eq!(one["record"]["stable_observations"],1);
    assert_eq!(one["record"]["terminal"],false);
    let done=ask(&second,json!({"kind":"await_watch","watch":handle}))?;
    assert_eq!(done["ok"],true,"{done}");assert_eq!(done["record"]["status"],"satisfied");
    assert_eq!(done["record"]["sample_count"],3);assert_eq!(second.calls.load(Ordering::SeqCst),2);
    let bytes=fs::read(&files.watches).map_err(io_error)?;
    let record_digest={let h=resolve(second.handle())?;let s=lock(&h)?;
        s.journal.as_ref().and_then(|j|j.entries().first()).map(|e|e.record_digest.to_string())
            .ok_or_else(||error(ErrorCode::InternalInvariantViolation,"archive fixture absent"))?};
    let historical=ask(&second,json!({"kind":"historical_query","record":1,"record_digest":record_digest,
        "query":{"kind":"aggregate","group_by":{"kind":"entity_kind"}}}))?;
    assert_eq!(historical["ok"],true,"{historical}");assert_eq!(historical["historical"],true);
    assert_eq!(fs::read(&files.watches).map_err(io_error)?,bytes);
    assert_eq!(second.calls.load(Ordering::SeqCst),2);
    assert_eq!(ask(&second,json!({"kind":"release_watch","watch":handle}))?["ok"],true);drop(second);
    let (third,opened)=register(&files,6,&[])?;
    assert_eq!(opened["watch_recovery"]["restored"],0);
    assert_eq!(ask(&third,json!({"kind":"watches"}))?["records"],json!([]));
    drop(third);Ok(())
}
...
fn mcp_reports_watch_corruption_without_hiding_it_as_output_overflow()->Result<()>{
    use std::io::Write;
    let _serial=lock(&SERIAL)?;let files=Files::new()?;let (s,_)=register(&files,3,&[])?;
    assert_eq!(ask(&s,watch())?["ok"],true);
    fs::OpenOptions::new().append(true).open(&files.watches)
    ...
    assert_eq!(result["agent_turn"]["briefing"]["active_work_unavailable"],true);
    assert_eq!(s.calls.load(Ordering::SeqCst),0);drop(s);
    assert!(register(&files,4,&[]).is_err());
    assert_eq!(fs::read(&files.watches).map_err(io_error)?,original);Ok(())
}

#[test]
fn rejected_bootstrap_packet_does_not_publish_a_recovered_handle()->Result<()>{
    let _serial=lock(&SERIAL)?;let files=Files::new()?;let (s,_)=register(&files,3,&[])?;
    assert_eq!(ask(&s,watch())?["ok"],true);drop(s);
    let original=fs::read(&files.watches).map_err(io_error)?;
    let (mut candidate,mut context,_)=session(&files,4,&[])?;
    candidate.budget.max_output_tokens=1;context.budget=candidate.budget;
    assert!(finish_open(&mut candidate,&context,Some(&files.watches),json!({"ok":true})).is_err());
    assert!(candidate._watch_journal.is_none());
    assert_eq!(fs::read(&files.watches).map_err(io_error)?,original);drop(candidate);
    let (s,opened)=register(&files,4,&[])?;assert_eq!(opened["watch_recovery"]["restored"],1);
    assert_eq!(ask(&s,json!({"kind":"watches"}))?["records"].as_array().map(Vec::len),Some(1));
    drop(s);Ok(())
}
