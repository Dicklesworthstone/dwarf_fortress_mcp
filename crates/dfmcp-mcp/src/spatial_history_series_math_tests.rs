    ...
    (
        JournalEntry {
            ...
            anchor: StateAnchor {
                ...
                tick: GameTick(tick),
                state_hash: Digest32::of_bytes(&number.to_be_bytes()),
            },
            source_digest: Digest32::of_bytes(b"source"),
            record_digest: Digest32::of_bytes(&number.to_be_bytes()),
...
fn finite_delta_bounds_equal_exhaustive_endpoint_differences() {
    for a in 0..8u64 {
        for b in a..8 {
            for c in 0..8u64 {
                for d in c..8 {
                    let old = QuantityBounds {
                        lower: a,
                        upper: Some(b),
                    };
                    let new = QuantityBounds {
                        lower: c,
                        upper: Some(d),
                    };
                    let possible: Vec<_> = (a..=b)
                        .flat_map(|x| (c..=d).map(move |y| i128::from(y) - i128::from(x)))
                        .collect();
                    assert_eq!(
                        new.difference(old),
                        (
                            possible.iter().min().copied(),
                            possible.iter().max().copied()
                        )
                    );
                }
            }
...
fn unknown_upper_bounds_remain_open_and_can_still_prove_a_direction() -> Result<()> {
    ...
    assert!(value["net_change"]["minimum"].is_null());
    assert!(value["net_change"]["maximum"].is_null());
    assert_eq!(value["net_change"]["exact"], false);
    Ok(())
}

#[test]
fn rates_retain_full_u64_differences_without_floating_point_or_signed_overflow() -> Result<()> {
    for (old, new, numerator) in [
        (u64::MAX, 0, "-18446744073709551615"),
        (0, u64::MAX, "18446744073709551615"),
    ] {
        let value = transition(
            &sample(1, 1, 0, old, Some(old)),
            &sample(2, 1, u64::MAX, new, Some(new)),
        )?;
...
fn same_tick_still_has_a_delta_but_never_a_division_by_zero() -> Result<()> {
    ...
    assert_eq!(value["rate_unavailable_reason"], "same_game_tick");
    Ok(())
}
...
fn reset_or_regressed_clock_is_not_a_large_resource_loss() -> Result<()> {
    ...
    for next in [sample(2, 2, 120, 1, Some(1)), sample(2, 1, 99, 1, Some(1))] {
        ...
        assert!(value["net_change"].is_null());
        assert!(value["net_rate"].is_null());
    }
    assert!(transition(&first, &sample(3, 1, 120, 1, Some(1))).is_err());
    Ok(())
}
...
fn equal_uncertain_bounds_do_not_claim_unchanged_quantity() -> Result<()> {
    ...
    assert_eq!(value["status"], "unchanged_quantity");
    Ok(())
}
...
fn timeline_continuations_bind_identity_and_require_canonical_forward_progress() -> Result<()> {
    ...
    assert!(
        matches!(start_record(Some(&token),Digest32::ZERO,1,4),Err(e) if e.code==ErrorCode::StaleAnchor)
    );
    for bad in [
        token.replacen(":2:", ":02:", 1),
        continuation(id, 1),
        continuation(id, 5),
        "x".repeat(129),
        "hs1:+2:bad".into(),
