...
inline std::size_t utf8_width(const std::string &value,std::size_t offset){
    ...
    if(lead>=0xc2&&lead<=0xdf){if(offset+2>value.size())return 0;return (static_cast<unsigned char>(value[offset+1])&0xc0)==0x80?2:0;}
    if(lead>=0xe0&&lead<=0xef){
        if(offset+3>value.size())return 0;const auto b=static_cast<unsigned char>(value[offset+1]);
        if((b&0xc0)!=0x80||(static_cast<unsigned char>(value[offset+2])&0xc0)!=0x80)return 0;
        if((lead==0xe0&&b<0xa0)||(lead==0xed&&b>=0xa0))return 0;return 3;
    }
    if(lead>=0xf0&&lead<=0xf4){
        if(offset+4>value.size())return 0;const auto b=static_cast<unsigned char>(value[offset+1]);
        ...
        if((lead==0xf0&&b<0x90)||(lead==0xf4&&b>=0x90))return 0;return 4;
    }
    return 0;
}
...
inline std::uint32_t capture(std::uint32_t maximum,std::size_t maximum_bytes,std::string &out){
    ...
    citizens.erase(std::remove(citizens.begin(),citizens.end(),nullptr),citizens.end());
    ...
    for(auto *unit:citizens){
        ...
        const auto name=bounded_utf8(visible?Translation::translateName(visible,false):std::string(),256);
