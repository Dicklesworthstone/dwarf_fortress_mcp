"""Private append-only excavation journals and a complete-directory pending fence.
...
from excavation_run_rpc import Budget, Manifest, Reply, endpoint
from excavation_run_wire import Capture, Plan, Record, Spec, canonical, digest, exact_hex, integer, key_text, require

MAX_FILE = 32768
MAX_FRAME = 16384
MAX_RUNS = 256
SUFFIX = '.exrun'
FORMAT = 'dfmcp.excavation-run-journal/1'
...
def intent(plan: Plan, manifest: Manifest, address: tuple[str, int]) -> dict:
    ...
    return {'key': plan.key, 'spec': list(plan.spec.values()), 'capture_hex': plan.before.raw.hex(),
...
class State:
    ...
    def view(self) -> dict:
        return {'key': self.plan.key, 'plan_digest': self.plan.digest.hex(),
                'stage': ('terminal' if self.terminal else 'dispatch_recorded' if self.dispatched
                ...
                'retry_permitted': False, 'current_pause_proved': False,
...
def apply(state: State | None, kind: str, payload: object) -> State:
    if state is None:
        ...
        p = exact_object(payload, {'key', 'spec', 'capture_hex', 'plan_digest', 'token', 'manifest', 'endpoint'})
        require(type(p['spec']) is list and len(p['spec']) == 6, 'invalid intent spec')
        plan = Plan(p['key'], Spec(*p['spec']), Capture.decode(exact_hex(p['capture_hex'], maximum=1024)))
    ...
    require(state.terminal is None, 'record after terminal receipt')
    ...
    value = Record.decode(exact_hex(p['record_hex']), state.plan)
    manifest = manifest_from(p['manifest'])
    ...
    require(value.terminal, 'nonterminal record cannot close journal')
...
def replay(raw: bytes) -> State:
    ...
    for index, line in enumerate(lines):
        ...
        checksum = digest('dfmcp-excavation-run-frame/1', canonical(payload)).hex()
...
def frame_bytes(state: State | None, kind: str, payload: dict) -> bytes:
    ...
    frame['checksum'] = digest('dfmcp-excavation-run-frame/1', canonical(frame)).hex()
...
class Journal:
    def __init__(self, owner: RunDirectory, name: str, fd: int):
    ...
    def retain(self, reply: Reply, prepared: bool = False) -> bool:
        ...
        require(software_equal(reply.manifest, self.state.manifest), 'native software changed')
        if not prepared and not value.terminal:
            return False
        if self.state.terminal is not None:
            require(value.raw == self.state.terminal.raw, 'conflicting terminal receipt')
            self.owner.check()
            return False
...
class RunDirectory:
    ...
    def names(self) -> list[str]:
        ...
        with os.scandir(self.fd) as entries:
            for entry in entries:
                require(len(names) < MAX_RUNS, 'run directory capacity exceeded')
                require(entry.name.endswith(SUFFIX), 'unrecognized run directory entry')
                key_text(entry.name[:-len(SUFFIX)])
                names.append(entry.name)
        return sorted(names)

    def check(self) -> None:
        require(not self.fenced and self.fd is not None, 'run directory fenced or closed')
        self.budget.remaining()
        private(os.fstat(self.fd), 0o700, True)
        named = open_directory(self.path)
        try:
            info = os.fstat(named)
            require((info.st_dev, info.st_ino) == self.identity, 'run directory pathname replaced')
        finally:
            os.close(named)
        require(self.names() == sorted(self.journals), 'run directory membership changed')
    ...
    def ready(self, key: str) -> None:
        self.check()
        require(filename(key) not in self.journals, 'run key already retained; use query/cancel, never start')
        require(len(self.journals) < MAX_RUNS, 'run directory full')
        require(not any(j.state.pending for j in self.journals.values()),
                'unresolved run blocks new keys in this directory')

    def create(self, plan: Plan, manifest: Manifest, address: tuple[str, int]) -> Journal:
        require(self.writable, 'read-only run directory')
        self.ready(plan.key)
        ...
        try:
            self.budget.remaining()
            fd = os.open(name, os.O_RDWR | os.O_CREAT | os.O_EXCL | os.O_NOFOLLOW | os.O_CLOEXEC | os.O_NONBLOCK,
    ...
    def get(self, key: str) -> Journal:
        self.check()
        name = filename(key)
        require(name in self.journals, 'run key not present in this directory')
    ...
    def inventory(self, limit: int = 32, continuation: str | None = None) -> dict:
        ...
        identity = digest('dfmcp-excavation-run-inventory/1', canonical(witness)).hex()
        offset = 0
        if continuation is not None:
            ...
            offset = integer(token['offset'], 1, MAX_RUNS)
            require(offset < len(self.journals) and offset % limit == 0, 'invalid inventory page offset')
        # Pending first makes unresolved work visible even after many terminal runs.
